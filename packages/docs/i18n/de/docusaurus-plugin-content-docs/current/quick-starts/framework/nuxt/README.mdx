...
Die Composables `useLogtoClient()` und `useLogtoUser()` sind in Middleware oder API-Routen nicht verfügbar. Du kannst die Funktion `logtoEventHandler()` verwenden, um den Logto-Client und anderen Kontext zu erhalten:
