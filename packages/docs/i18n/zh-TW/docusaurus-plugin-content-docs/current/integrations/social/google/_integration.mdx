...
使用前一節中提到的 OAuth 應用程式詳細資訊頁面獲得的 _Client ID_ 和 _Client Secret_ 填寫 `clientId` 和 `clientSecret` 欄位。
...
- **在 ITP 瀏覽器上啟用升級的 One Tap UX**：在智能追蹤防護 (ITP) 瀏覽器上啟用升級的 Google One Tap 使用者體驗。請參閱 [此頁面](https://developers.google.com/identity/gsi/web/guides/features#upgraded_ux_on_itp_browsers) 以獲取更多資訊。
