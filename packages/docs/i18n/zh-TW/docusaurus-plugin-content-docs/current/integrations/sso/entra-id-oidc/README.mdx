---
slug: /integrations/entra-id-oidc
sidebar_label: Microsoft Entra ID (OIDC)
sidebar_custom_props:
  description: 前稱 Azure AD，一個全面的雲端身分管理服務。
  logoFilename: 'entra-id.svg'
tutorial_name: Microsoft Entra ID OIDC enterprise SSO
tutorial_config_name: Azure AD SSO 應用程式
---

import GuideTip from '../../fragments/_sso_guide_tip.mdx';

import Step1 from './_step-1.mdx';
import Step2 from './_step-2.mdx';
import Step3 from './_step-3.mdx';

設定 Microsoft Entra ID 單一登入 (SSO)

透過最少的配置工作，此連接器允許與 Microsoft Entra ID（前稱 Azure AD）整合以實現企業級單一登入 (SSO)。

<GuideTip />

## 步驟 1：建立 Microsoft Entra ID OIDC 應用程式 \{#step-1-create-an-microsoft-entraid-oidc-application}

<Step1 />

## 步驟 2：在 Logto 配置 Microsoft Entra ID OIDC SSO \{#step-2-configure-microsoft-entra-id-oidc-sso-at-logto}

<Step2 />

## 步驟 3：設定電子郵件網域並啟用 SSO 連接器 \{#step-3-set-email-domains-and-enable-the-sso-connector}

<Step3 />
