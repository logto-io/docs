{/* Used to generate saml social tutorials, for backwards compatibility purposes only. */}

### Create social IdP's account and register SAML application (IdP) \{#create-social-idps-account-and-register-saml-application-idp}
...
### Configure SAML connector (SP) \{#configure-saml-connector-sp}
...
#### signInEndpoint `Required` \{#signinendpoint-required}
...
#### x509Certificate `Required` \{#x509certificate-required}
...
#### idpMetadataXml `Required` \{#idpmetadataxml-required}
...
#### assertionConsumerServiceUrl `Required` \{#assertionconsumerserviceurl-required}
...
The `signAuthnRequest` and `encryptAssertion` attributes should align with corresponding parameters of IdP setting, otherwise error will be thrown to show that configuration does not match.
...
#### privateKey and privateKeyPass \{#privatekey-and-privatekeypass}
...
#### encPrivateKey and encPrivateKeyPass \{#encprivatekey-and-encprivatekeypass}
