---
id: email-templates
title: แม่แบบอีเมล
sidebar_label: แม่แบบอีเมล
sidebar_position: 3
---

Logto มีแม่แบบสำหรับปรับแต่งเนื้อหาอีเมลในแต่ละกรณีการใช้งานที่แตกต่างกัน

ขอแนะนำอย่างยิ่งให้คุณใช้แม่แบบที่แตกต่างกันในแต่ละสถานการณ์ มิฉะนั้นผู้ใช้อาจได้รับเนื้อหาอีเมลที่ไม่ตรงกับการดำเนินการปัจจุบัน ซึ่งอาจทำให้เกิดความสับสน หากมีแม่แบบที่ขาดหายและไม่ได้กำหนดค่า อาจทำให้เกิดข้อผิดพลาดใน flow ที่ต้องพึ่งพาแม่แบบนั้นและส่งผลกระทบต่อการพัฒนาธุรกิจตามปกติ

## ตัวเลือกการปรับแต่งแม่แบบอีเมล \{#email-template-customization-options}

Logto มี 3 วิธีหลักสำหรับการจัดการแม่แบบอีเมล:

1. **ปรับแต่งแม่แบบใน Logto**

   - **ตัวเชื่อมต่อ (Connectors)**:
     - [SMTP](/integrations/smtp)
     - [SendGrid](/integrations/sendgrid-email)
     - [Mailgun](/integrations/mailgun)
     - [AWS Direct Mail](/integrations/aws-ses)
     - [Aliyun Direct Mail](/integrations/aliyun-dm)
   - **ความสามารถ**:
     - ✅ แทรกตัวแปรหลากหลายลงในแม่แบบได้อย่างยืดหยุ่น
     - ✅ สร้างแม่แบบหลายภาษาแบบกำหนดเองผ่าน Management APIs
     - ✅ แก้ไขแม่แบบได้เต็มรูปแบบภายใน Logto

2. **ปรับแต่งแม่แบบในแพลตฟอร์มผู้ให้บริการ**

   - **ตัวเชื่อมต่อ (Connectors)**:
     - [Postmark](/integrations/postmark)
     - [HTTP Email](/integrations/http-email)
   - **ความสามารถ**:
     - ✅ ส่งตัวแปรไปยังแพลตฟอร์มผู้ให้บริการ
     - ✅ ส่งพารามิเตอร์ `locale` ไปยังแพลตฟอร์มผู้ให้บริการเพื่อการแปลภาษา
     - ✅ แก้ไขแม่แบบได้เต็มรูปแบบในแดชบอร์ดของผู้ให้บริการ (ใช้ Logto Management APIs)

3. **แม่แบบสำเร็จรูป (ไม่สามารถปรับแต่งได้)**

   - **ตัวเชื่อมต่อ (Connector)**:
     - [Logto Built-in Email Service](/connectors/email-connectors/built-in-email-service)
   - **ความสามารถ**:
     - ✅ รองรับตัวแปรในระบบ
     - ✅ แม่แบบหลายภาษา
     - ❌ ไม่สามารถแก้ไขแม่แบบ / UI ได้

## ประเภทของแม่แบบอีเมล \{#email-template-types}

| usageType                | สถานการณ์                                                                                                                                                                                                                                                                                                                                                                                            | ตัวแปรที่ใช้ได้                                                                       |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| SignIn                   | ผู้ใช้ [ลงชื่อเข้าใช้ด้วยอีเมล](/end-user-flows/sign-up-and-sign-in/sign-in) และยืนยันโดยกรอกรหัสยืนยัน แทนการกรอกรหัสผ่าน                                                                                                                                                                                                                                                                           | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| Register                 | ผู้ใช้ [สร้างบัญชีด้วยอีเมล](/end-user-flows/sign-up-and-sign-in/sign-up) และยืนยันโดยกรอกรหัสยืนยันที่ Logto ส่งไปยังอีเมลของตน                                                                                                                                                                                                                                                                     | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| ForgotPassword           | หากผู้ใช้ลืมรหัสผ่านขณะเข้าสู่ระบบ สามารถเลือกยืนยันตัวตนผ่านอีเมลก่อนเพื่อ [รีเซ็ตรหัสผ่าน](/end-user-flows/sign-up-and-sign-in/reset-password)                                                                                                                                                                                                                                                     | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| Generic                  | แม่แบบนี้สามารถใช้เป็นตัวเลือกสำรองทั่วไปในหลายสถานการณ์ เช่น ทดสอบการตั้งค่าตัวเชื่อมต่อ, [ยืนยันหรือเชื่อมโยงอีเมลหลังลงชื่อเข้าใช้](/end-user-flows/account-settings/by-management-api#email-and-phone-number-verification) เป็นต้น                                                                                                                                                               | code: string                                                                          |
| OrganizationInvitation   | ใช้แม่แบบนี้เพื่อ [ส่งลิงก์เชิญผู้ใช้](/end-user-flows/organization-experience/invite-organization-members#configure-your-email-connector) ให้เข้าร่วมองค์กร                                                                                                                                                                                                                                         | link: string<br/>organization: `OrganizationInfo`<br/>inviter?: `UserInfo`            |
| UserPermissionValidation | ระหว่างการใช้งานแอป อาจมีบางการดำเนินการที่มีความเสี่ยงสูงหรือมีระดับความเสี่ยงค่อนข้างสูงที่ [ต้องการการยืนยันตัวตนเพิ่มเติมจากผู้ใช้](/end-user-flows/account-settings/by-account-api#verify-by-sending-a-verification-code-to-the-users-email-or-phone) เช่น โอนเงิน ลบทรัพยากรที่ใช้งานอยู่ หรือยกเลิกสมาชิก แม่แบบ `UserPermissionValidation` ใช้กำหนดเนื้อหาอีเมลรหัสยืนยันในสถานการณ์เหล่านี้ | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
| BindNewIdentifier        | เมื่อผู้ใช้แก้ไขโปรไฟล์ อาจ [ผูกอีเมลใหม่กับบัญชีปัจจุบัน](/end-user-flows/account-settings/by-account-api#update-or-link-new-email) ในกรณีนี้สามารถใช้แม่แบบ `BindNewIdentifier` เพื่อปรับแต่งเนื้อหาอีเมลยืนยันได้                                                                                                                                                                                 | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
| MfaVerification          | เมื่อเปิดใช้ [email MFA](/end-user-flows/mfa/email-mfa) แม่แบบนี้ใช้ส่งรหัสยืนยันให้ผู้ใช้ในกระบวนการยืนยันตัวตนหลายปัจจัย                                                                                                                                                                                                                                                                           | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| BindMfa                  | เมื่อเปิดใช้ [email MFA](/end-user-flows/mfa/email-mfa) แม่แบบนี้ใช้สำหรับตั้งค่ารหัสยืนยัน MFA ทางอีเมล ผู้ใช้จะได้รับรหัสนี้เมื่อผูกหรือกำหนดอีเมลเป็นปัจจัย MFA ให้บัญชีตนเอง                                                                                                                                                                                                                     | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |

## ตัวแปรในแม่แบบอีเมล \{#email-template-variables}

### Code \{#code}

รหัสยืนยันที่ผู้ใช้ต้องกรอกเพื่อดำเนินการยืนยันให้เสร็จสมบูรณ์ ใช้ได้ในแม่แบบ `SignIn`, `Register`, `ForgotPassword`, `Generic`, `UserPermissionValidation` และ `BindNewIdentifier`

- รหัสยืนยันหมดอายุใน 10 นาที ขณะนี้ยังไม่รองรับการปรับแต่งเวลาหมดอายุของรหัสยืนยัน
- ต้องมี placeholder `{{code}}` ในแม่แบบ เมื่อส่งรหัสยืนยัน ระบบจะสุ่มรหัสและแทนที่ placeholder นี้ก่อนส่งอีเมลถึงผู้ใช้

### ApplicationInfo \{#applicationinfo}

ข้อมูลสาธารณะของแอปพลิเคชันที่ผู้ใช้กำลังโต้ตอบด้วย ใช้ได้ในแม่แบบ `SignIn`, `Register`, `ForgotPassword`, `UserPermissionValidation` และ `BindNewIdentifier`

```ts
type ApplicationInfo = {
  id: string;
  name: string;
  displayName?: string;
  branding?: {
    logoUrl?: string;
    darkLogoUrl?: string;
    favicon?: string;
    darkFavicon?: string;
  };
};
```

- สามารถเข้าถึงฟิลด์ย่อยทั้งหมดของ application info ในแม่แบบผ่าน dot notation เช่น `{{application.name}}` จะถูกแทนที่ด้วยชื่อแอปจริงจากการตั้งค่าของคุณ
- หากไม่ส่งตัวแปร `application` เลย placeholder handlebars จะถูกละเว้นและไม่ถูกแทนที่
- หาก object `application` ที่ส่งมาไม่มีฟิลด์ที่ต้องการหรือค่าเป็น undefined placeholder handlebars จะถูกแทนที่ด้วยสตริงว่าง เช่น `{{application.foo.bar}}` จะถูกแทนที่ด้วย ``

### OrganizationInfo \{#organizationinfo}

ข้อมูลสาธารณะขององค์กรที่ผู้ใช้กำลังโต้ตอบด้วย

```ts
type OrganizationInfo = {
  id: string;
  name: string;
  branding?: {
    logoUrl?: string;
    darkLogoUrl?: string;
    favicon?: string;
    darkFavicon?: string;
  };
};
```

- สำหรับแม่แบบ `SignIn`, `Register` และ `ForgotPassword` ตัวแปร `organization` เป็นตัวเลือก ใช้ได้เฉพาะเมื่อมีพารามิเตอร์ `organization_id` ในคำขอการอนุญาต ดู [การตั้งค่าแบรนด์เฉพาะองค์กร](/customization/match-your-brand#organization-specific-branding) สำหรับรายละเอียดเพิ่มเติม
- สำหรับแม่แบบ `OrganizationInvitation` ตัวแปร `organization` จำเป็นต้องมี

### UserInfo \{#userinfo}

ข้อมูลสาธารณะของผู้ใช้ที่อีเมลถูกส่งถึง ใช้ได้ในแม่แบบ `UserPermissionValidation`, `BindNewIdentifier` และ `OrganizationInvitation`

```ts
type UserInfo = {
  id: string;
  name?: string;
  username?: string;
  primaryEmail?: string;
  primaryPhone?: string;
  avatar?: string;
  profile?: Profile;
};
```

- ดู [profile](/user-management/user-data#profile) สำหรับรายละเอียดเพิ่มเติมเกี่ยวกับชนิด `Profile`
- ตัวแปร `user` จำเป็นสำหรับแม่แบบ `UserPermissionValidation` และ `BindNewIdentifier`
- ตัวแปร `inviter` เป็นตัวเลือกสำหรับแม่แบบ `OrganizationInvitation` ใช้ได้เมื่อมี `inviterId` ในคำขอเชิญองค์กร

### UI Locales \{#ui-locales}

ค่าต้นฉบับของ `ui_locales` ที่ส่งมาในคำขอการยืนยันตัวตน OIDC ที่เริ่มต้นการโต้ตอบนี้

- ชนิด: `string` (รายการ tag ภาษา BCP 47 คั่นด้วยช่องว่าง ตามสเปค OIDC) เช่น `"fr-CA fr en"`
- การใช้งาน: มีเมื่อการโต้ตอบการลงชื่อเข้าใช้ปัจจุบันถูกเริ่มด้วย `ui_locales` หากไม่มีจะละเว้นตัวแปรนี้
- ตัวอย่างการใช้งาน: ใส่ในเนื้อหาอีเมลหรือหัวเรื่องเพื่อบันทึกภาษาที่ผู้ใช้ร้องขอสำหรับการรองรับ i18n หรือการตรวจสอบ เช่น `Requested languages: {{uiLocales}}`

## ตัวอย่างแม่แบบอีเมล \{#email-template-examples}

คุณสามารถใช้ตัวอย่างโค้ดแม่แบบอีเมลที่ให้ไว้เป็นจุดเริ่มต้นในการปรับแต่ง UI ของคุณ เพื่อสร้างอินเทอร์เฟซผู้ใช้คล้ายกับตัวอย่างต่อไปนี้:

<img src="/img/assets/built-in-email-template-sample.jpeg" alt="ตัวอย่างแม่แบบอีเมลสำเร็จรูป" />

เนื่องจากแม่แบบอีเมลที่ใช้ในแต่ละสถานการณ์ของ Logto มีความคล้ายคลึงกันมาก โดยแตกต่างกันเพียงคำอธิบายสถานการณ์และการดำเนินการปัจจุบัน

เราจะไม่แสดงโค้ด HTML ของทุกแม่แบบโดยละเอียดที่นี่ แต่จะยกตัวอย่างเฉพาะกรณี **ลงชื่อเข้าใช้** เท่านั้น กรณีอื่น ๆ เช่น สมัครสมาชิกหรือลืมรหัสผ่าน มีโครงสร้างคล้ายตัวอย่างนี้

ผู้ใช้สามารถอ้างอิงแม่แบบนี้และปรับแต่งให้เหมาะสมกับสถานการณ์จริงของตนเอง

```html
<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>ยืนยันอีเมลของคุณเพื่อเข้าสู่ระบบ</title>
    <style>
      .auth-service-by:hover .mini-logo {
        display: none !important;
      }
      .auth-service-by:hover .mini-logo-color {
        display: block !important;
      }
      body {
        font-family:
          'SF Pro Text',
          -apple-system,
          system-ui,
          BlinkMacSystemFont,
          'Segoe UI',
          Roboto,
          Arial,
          sans-serif;
        -webkit-font-smoothing: antialiased;
        -moz-osx-font-smoothing: grayscale;
        font-smooth: always;
        background-color: #fff;
        color: #191c1d;
        max-width: 640px;
        padding: 32px 0;
        font-size: 14px;
        font-weight: 400;
        line-height: 20px;
      }
      h1 {
        font-size: 24px;
        font-weight: 700;
        line-height: 32px;
        margin-top: 32px;
      }
      .verification-code {
        margin: 20px 0;
        background: #eff1f1;
        border-radius: 12px;
        padding: 36px;
        font-size: 32px;
        font-weight: 600;
        line-height: 40px;
      }
      .footer {
        text-align: center;
        color: #a9acac;
        margin-top: 48px;
      }
    </style>
  </head>
  <body>
    <div style="max-width: 698px; border-radius: 16px; border: 1px solid #E0E3E3;">
      <div style="padding: 0 24px;">
        <center>
          <img src="{{logoUrl}}" alt="Logo" width="auto" height="40" />
          <h1>ยืนยันอีเมลของคุณเพื่อเข้าสู่ระบบ</h1>
          <p>
            เราได้รับความพยายามในการเข้าสู่ระบบโดยใช้รหัสต่อไปนี้
            กรุณากรอกรหัสนี้ในหน้าที่คุณเปิดไว้เพื่อดำเนินการเข้าสู่ระบบให้เสร็จสมบูรณ์
          </p>
          <div class="verification-code">000000</div>
          <p style="color: #747778;">
            หากคุณไม่ได้พยายามเข้าสู่ระบบแต่ได้รับอีเมลนี้ กรุณาเพิกเฉย รหัสนี้จะใช้งานได้ 10 นาที
          </p>
          <hr style="margin: 64px 0 24px; max-width: 420px;" />
          <p style="color: #747778; margin: 16px 0 0;">{{companyInfo}}</p>
        </center>
      </div>
    </div>
    <div class="footer">
      <hr />
      <p style="font-size: 14px; line-height: 20px; margin: 20px 0;">
        <a href="https://logto.io" style="color: #A9ACAC; text-decoration: underline;">Logto</a>:
        โครงสร้างพื้นฐานข้อมูลระบุตัวตนที่ดีกว่าสำหรับนักพัฒนา
      </p>
      <table style="margin: 0 auto; width: auto; border-spacing: 0;">
        <tbody>
          <tr>
            <td style="vertical-align: middle;">
              <a href="{{discordServerUrl}}" style="display: block; margin: 0 12px;">
                <img src="{{discordLogoUrl}}" style="width: 20px;" />
              </a>
            </td>
            <td style="vertical-align: middle;">
              <a href="{{githubUrl}}" style="display: block; margin: 0 12px;">
                <img src="{{githubLogoUrl}}" style="width: 20px;" />
              </a>
            </td>
            <td style="vertical-align: middle;">
              <a href="{{twitterUrl}}" style="display: block; margin: 0 12px;">
                <img src="{{twitterLogoUrl}}" style="width: 20px;" />
              </a>
            </td>
            <td style="vertical-align: middle;">
              <a href="{{mailToUrl}}" style="display: block; margin: 0 12px;">
                <img src="{{emailIconUrl}}" style="width: 20px;" />
              </a>
            </td>
          </tr>
        </tbody>
      </table>
      <p style="font-size: 12px; line-height: 16px;">
        © Silverhand, Inc., 2810 North Church Street, Wilmington, DE 19802
      </p>
      <p style="color: #A9ACAC; font-size: 12px; line-height: 16px;">
        มีคำถามหรือต้องการความช่วยเหลือ?
        <a href="{{mailToUrl}}" style="color: #A9ACAC; text-decoration: underline;">ติดต่อเรา</a>
      </p>
    </div>
  </body>
</html>
```

จากนั้นคุณสามารถ escape โค้ด HTML ข้างต้นและเพิ่มลงในฟิลด์ "Template" ของตัวเชื่อมต่อใน configs ได้ดังนี้ (สมมติใช้ SendGrid connector):

```json
{
  "subject": "<sign-in-template-subject>",
  "content": "<table cellpadding=\"0\" cellspacing=\"0\" ...",
  "usageType": "SignIn",
  "type": "text/html"
}
```

## การแปลภาษาแม่แบบอีเมล \{#email-template-localization}

### สร้างแม่แบบอีเมลหลายภาษาด้วยตนเอง \{#custom-email-templates-for-different-languages}

Logto รองรับการสร้างแม่แบบอีเมลหลายภาษาด้วย Management API คุณสามารถสร้างแม่แบบอีเมลสำหรับแต่ละภาษาและแต่ละประเภทแม่แบบ เพื่อมอบประสบการณ์ที่แปลเป็นภาษาท้องถิ่นให้ผู้ใช้ของคุณ

```ts
type EmailTemplate = {
  languageTag: string;
  templateType: TemplateType;
  details: {
    subject: string;
    content: string;
    contentType?: 'text/html' | 'text/plain';
    replyTo?: string;
    sendFrom?: string;
  };
};
```

| ฟิลด์       | คำอธิบาย                                                                                                                                   |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| subject     | แม่แบบหัวเรื่องของอีเมล                                                                                                                    |
| content     | แม่แบบเนื้อหาอีเมล                                                                                                                         |
| contentType | ผู้ให้บริการอีเมลบางรายอาจแสดงผลแม่แบบอีเมลแตกต่างกันตามชนิดเนื้อหา (เช่น Sendgrid, Mailgun) ใช้ฟิลด์นี้เพื่อระบุชนิดเนื้อหาของแม่แบบอีเมล |
| replyTo     | อีเมลที่จะรับการตอบกลับ ตรวจสอบกับผู้ให้บริการอีเมลของคุณว่ารองรับฟิลด์นี้หรือไม่                                                          |
| sendFrom    | ชื่อผู้ส่งอีเมล (alias) ตรวจสอบกับผู้ให้บริการอีเมลของคุณว่ารองรับฟิลด์นี้หรือไม่                                                          |

เมื่อสร้างแม่แบบอีเมลแล้ว Logto จะเลือกแม่แบบที่เหมาะสมโดยอัตโนมัติโดยอิงจากลำดับความชอบทางภาษา (language preference) ของผู้ใช้

ลำดับการเลือกภาษามีดังนี้:

1. หากคำขอการยืนยันตัวตน OIDC มี `ui_locales` Logto จะเลือก tag แรกใน `ui_locales` ที่รองรับโดยไลบรารีภาษา tenant ของคุณ ดู [ui_locales](/end-user-flows/authentication-parameters/ui-locales) สำหรับรายละเอียด
2. หากไม่ใช่กรณีข้างต้น สำหรับ [Experience APIs](/end-user-flows/sign-up-and-sign-in) และ [User Account APIs](/end-user-flows/account-settings/by-account-api) ฝั่ง client Logto จะใช้ header `Accept-Language` สำหรับ Management APIs (เช่น [Organization Invitation](/end-user-flows/organization-experience/invite-organization-members)) คุณสามารถระบุภาษาได้ผ่านฟิลด์ `locale` ใน `messagePayload`
3. หากไม่มีทั้งสองอย่าง Logto จะ fallback เป็นภาษาหลักของ tenant ที่ตั้งค่าไว้ใน Sign-in & account > Content ดู [Localized languages](/customization/localized-languages#customization-steps-in-logto-console) สำหรับรายละเอียดการตั้งค่า

การเลือกแม่แบบ:

4. เมื่อได้ภาษาที่ต้องการแล้ว Logto จะค้นหาแม่แบบอีเมลที่ตรงกันโดยใช้ `languageTag` และ `templateType` หากพบจะใช้แม่แบบนั้น
5. หากไม่มีแม่แบบที่ตรงกัน Logto จะใช้แม่แบบอีเมลเริ่มต้นที่กำหนดไว้ในตัวเชื่อมต่อ

**ตัวเชื่อมต่ออีเมลที่รองรับ**:

- [Aliyun Direct Mail](/integrations/aliyun-dm)
- [Amazon Direct Mail](/integrations/aws-ses)
- [Mailgun](/integrations/mailgun)
- [SendGrid](/integrations/sendgrid-email)
- [SMTP](/integrations/smtp)

### การแปลภาษาแม่แบบอีเมลฝั่งผู้ให้บริการ \{#provider-side-email-template-localization}

สำหรับนักพัฒนาที่ใช้ตัวเชื่อมต่ออีเมลที่จัดการแม่แบบอีเมลโดยผู้ให้บริการ:

- [HTTP Email](/integrations/http-email)
- [Postmark](/integrations/postmark)

ภาษาที่ผู้ใช้ต้องการจะถูกส่งไปยังผู้ให้บริการผ่านพารามิเตอร์ `locale` ใน payload ของแม่แบบ คุณสามารถสร้างแม่แบบหลายภาษาบนคอนโซลของผู้ให้บริการและใช้ `locale` เพื่อระบุภาษาที่ต้องการ

:::note

เมื่อมี `ui_locales` ในคำขอการยืนยันตัวตน ตัวแปร `locale` และ `uiLocales` จะพร้อมใช้งานใน context ของแม่แบบ  
ตัวแปร `uiLocales` จะเก็บค่าต้นฉบับของ `ui_locales` จากคำขอ ส่วน `locale` จะถูกกำหนดจาก tag แรกที่รองรับใน `ui_locales` หากไม่มี `ui_locales` ตัวแปร `locale` จะใช้กฎการเลือกภาษาตามปกติ (เช่น `Accept-Language` แล้ว fallback เป็นภาษาหลัก)

:::

## คำถามที่พบบ่อย \{#faqs}

<details>
<summary>

### จะใช้บริการแม่แบบอีเมลของบุคคลที่สามได้อย่างไร หากไม่ได้ตั้งค่าแม่แบบใน Logto? \{#how-to-use-third-party-email-template-services-if-templates-are-not-configured-in-logto}

</summary>

คุณสามารถเพิ่ม endpoint ใหม่ในเว็บเซอร์วิสของคุณเองเพื่อส่งอีเมล แล้วใช้ [Logto HTTP email connector](/integrations/http-email) เรียก endpoint ที่คุณดูแล

วิธีนี้ช่วยให้คุณจัดการตรรกะแม่แบบอีเมลบนเซิร์ฟเวอร์ของคุณเอง

</details>

<details>
<summary>

### มีวิธีใช้ Logto email เพื่อส่ง "Welcome email" แบบกำหนดเองให้ผู้ใช้ของเราหรือไม่? \{#is-there-a-way-to-use-logto-email-for-sending-our-users-a-customized-welcome-email}

</summary>

เรามีฟีเจอร์ [Webhook](/developers/webhooks) คุณสามารถสร้าง endpoint API ของคุณเองเพื่อรับ event `User.Created` ที่ส่งโดย Logto Webhook และเพิ่มตรรกะเพื่อส่ง welcome email แบบกำหนดเองใน handler ของ webhook

ตัวเชื่อมต่ออีเมลของ Logto ให้บริการแจ้งเตือนอีเมลเฉพาะเหตุการณ์ที่เกี่ยวข้องกับ flow การยืนยันตัวตนเท่านั้น Welcome email ถือเป็นความต้องการทางธุรกิจและไม่ได้รองรับโดยตรงในตัวเชื่อมต่ออีเมล แต่สามารถทำได้ผ่าน Webhook

</details>

## แหล่งข้อมูลที่เกี่ยวข้อง \{#related-resources}

<Url href="https://blog.logto.io/verification-email-delivery">
  เพิ่มประสิทธิภาพการส่งอีเมลยืนยันเพื่อรับประกันการเข้าถึงของผู้ใช้
</Url>
