...
# การตั้งค่าบัญชีผู้ใช้ด้วย UI ศูนย์บัญชีผู้ใช้ (Account Center) ที่สร้างไว้ล่วงหน้า
...
- ดูและแก้ไขโปรไฟล์ผู้ใช้ (ชื่อ, อวาตาร์, และฟิลด์โปรไฟล์แบบกำหนดเอง)
- อัปเดตที่อยู่อีเมลและหมายเลขโทรศัพท์
- อัปเดตชื่อผู้ใช้
...
UI ศูนย์บัญชีผู้ใช้ที่สร้างไว้ล่วงหน้าถูกออกแบบมาให้ทำงานร่วมกับแอปของคุณได้อย่างไร้รอยต่อ มอบประสบการณ์ผู้ใช้ที่สอดคล้องกันโดยไม่ต้องสร้างหน้าจัดการบัญชีเอง

## ข้อดีของการใช้ UI ที่สร้างไว้ล่วงหน้า \{#benefits-of-using-the-prebuilt-ui}

- **ไม่ต้องพัฒนาเอง**: หน้าพร้อมใช้งานทันที ไม่ต้องเขียนโค้ดเพิ่ม
...
| Path                                 | คำอธิบาย                                                                   |
| ------------------------------------ | -------------------------------------------------------------------------- |
| `/account/profile`                   | หน้าข้อมูลโปรไฟล์ผู้ใช้ (ชื่อ, อวาตาร์, และฟิลด์โปรไฟล์แบบกำหนดเอง)        |
...
ศูนย์บัญชีผู้ใช้จะแสดงหน้า **Profile** ที่ผู้ใช้ปลายทางสามารถดูและจัดการข้อมูลโปรไฟล์ของตนเองได้ หากต้องการควบคุมว่าฟิลด์โปรไฟล์ใดจะปรากฏในหน้านี้:

1. ตรวจสอบให้แน่ใจว่าสิทธิ์ของฟิลด์ **Name**, **Avatar**, **Profile** หรือ **Custom data** ถูกตั้งเป็น `Edit` หรือ `ReadOnly` ในส่วน Account security / User profile
2. ในการ์ด **Integrate prebuilt UI** ให้เพิ่มฟิลด์โปรไฟล์ที่คุณต้องการแสดงจากตัวเลือกฟิลด์โปรไฟล์ ฟิลด์เหล่านี้ดึงมาจากแค็ตตาล็อกเดียวกับ [การเก็บข้อมูลโปรไฟล์ผู้ใช้](/end-user-flows/collect-user-profile) — ฟิลด์ใดที่กำหนดไว้ที่นั่นก็สามารถแสดงในศูนย์บัญชีผู้ใช้ได้เช่นกัน
3. ลากและวางเพื่อจัดเรียงลำดับฟิลด์ตามต้องการ

เฉพาะฟิลด์ที่อยู่ในรายการฟิลด์โปรไฟล์ **และ** สิทธิ์ของฟิลด์นั้นไม่ใช่ `Off` เท่านั้นที่จะปรากฏในหน้า Profile

### ขั้นตอนที่ 2: ลิงก์ไปยังหน้าที่สร้างไว้ล่วงหน้าจากแอปของคุณ \{#step-2-link-to-prebuilt-pages}

หากต้องการใช้ UI ศูนย์บัญชีผู้ใช้ที่สร้างไว้ล่วงหน้า คุณต้องเปลี่ยนเส้นทางผู้ใช้จากแอปของคุณไปยังหน้าของ Logto ที่เหมาะสม มี 2 วิธี:

#### วิธีที่ A: ลิงก์โดยตรงพร้อมพารามิเตอร์ redirect \{#approach-a-direct-linking}

เพิ่มลิงก์ในแอปของคุณที่เปลี่ยนเส้นทางผู้ใช้ไปยังหน้าที่สร้างไว้ล่วงหน้า โดยใส่พารามิเตอร์ `redirect` เพื่อให้ผู้ใช้กลับมายังแอปของคุณหลังจากดำเนินการเสร็จสิ้น:
...
คุณสามารถผสานหน้าที่สร้างไว้ล่วงหน้าเข้ากับ flow การตั้งค่าบัญชีที่มีอยู่ของคุณได้:

1. ในหน้าตั้งค่าบัญชีของแอป แสดงข้อมูลปัจจุบันของผู้ใช้
2. ให้ปุ่ม "Edit" หรือ "Update" ที่ลิงก์ไปยังหน้าที่สร้างไว้ล่วงหน้าที่เกี่ยวข้อง
...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `redirect`     | URL แบบสัมบูรณ์ที่ใช้เปลี่ยนเส้นทางผู้ใช้กลับหลังจากดำเนินการเสร็จสิ้น รับเฉพาะ URL `http(s)` เท่านั้น                                                                        |
| `show_success` | เมื่อกำหนดเป็น `true` ปลายทางสำเร็จ (เช่น URL `redirect` ของคุณ) จะได้รับ query parameter `show_success` เพื่อให้คุณแสดงข้อความยืนยันได้                                      |
| `identifier`   | เติมค่าช่องระบุผู้ใช้ล่วงหน้าในหน้าปลายทาง (`/account/email`, `/account/phone`, `/account/username`) เหมาะสำหรับ deep-link จากแอปของคุณเมื่อทราบ identifier ของผู้ใช้อยู่แล้ว |
| `ui_locales`   | รายการ tag ภาษา BCP-47 คั่นด้วยช่องว่าง (เช่น `fr-CA fr en`) เพื่อควบคุมภาษาของ UI ศูนย์บัญชีผู้ใช้ หากไม่ระบุจะ fallback เป็นภาษาของเบราว์เซอร์ผู้ใช้                        |
...
:::note
ค่าของ `identifier` จะถูกเก็บใน session storage ก่อน redirect ไปลงชื่อเข้าใช้ และจะถูกใช้ครั้งเดียวโดยหน้าปลายทาง
:::

## ลิงก์ลบบัญชีผู้ใช้ \{#account-deletion-link}

ศูนย์บัญชีผู้ใช้ไม่รองรับการลบบัญชีโดยตรง คุณสามารถกำหนด **Account deletion URL** ที่ชี้ไปยัง flow การลบบัญชีของคุณเอง (โดยปกติจะใช้ [Management API](/end-user-flows/account-settings/by-management-api)) เมื่อกำหนดแล้ว จะมีรายการ **Delete your account** ปรากฏในหน้าความปลอดภัยของศูนย์บัญชีผู้ใช้และลิงก์ผู้ใช้ไปยัง URL ของคุณ

หากต้องการกำหนดค่า ให้ไปที่ <CloudLink to="/sign-in-experience/account-center">Console > Sign-in & account > Account center</CloudLink> และตั้งค่าฟิลด์ **Account deletion URL** คุณสามารถอัปเดตผ่าน Management API ได้เช่นกัน:
...
การเชื่อมโยงบัญชีโซเชียลจากศูนย์บัญชีผู้ใช้จะใช้ callback URL ของตัวเชื่อมต่อ:

```
https://[your-tenant-endpoint]/callback/{connectorId}
```

โดยที่ `{connectorId}` คือ ID ของตัวเชื่อมต่อโซเชียล ซึ่งเป็น **Callback URI** ที่แสดงในหน้ารายละเอียดตัวเชื่อมต่อใน <CloudLink to="/connectors/social">Console > Connectors > Social connectors</CloudLink> ที่คุณต้องลงทะเบียนกับผู้ให้บริการโซเชียลของคุณเมื่อ [ตั้งค่าตัวเชื่อมต่อ](/connectors/social-connectors/)
...
คุณสามารถปรับแต่งรูปลักษณ์ของ UI ศูนย์บัญชีผู้ใช้เพิ่มเติมได้โดยเพิ่ม CSS ของคุณเอง ไปที่ <CloudLink to="/sign-in-experience/account-center">Console > Sign-in & account > Account center</CloudLink> และเพิ่ม CSS ของคุณในตัวแก้ไข **Custom CSS**
...
- [อุปกรณ์ที่เชื่อถือได้สำหรับ MFA](/end-user-flows/mfa/trusted-devices) - กำหนดและจัดการความไว้วางใจเบราว์เซอร์แบบจำกัดเวลา
