...
หากต้องการเรียนรู้เพิ่มเติมเกี่ยวกับ API ที่มีอยู่ โปรดเยี่ยมชม [Logto Account API Reference](https://openapi.logto.io/group/endpoint-my-account) และ [Logto Verification API Reference](https://openapi.logto.io/group/endpoint-verifications)

:::note

ฟีเจอร์การดูบัญชี SSO และการลบบัญชีมีให้ใช้งานผ่าน Logto Management APIs เท่านั้น ดูรายละเอียดการใช้งานที่ [การตั้งค่าบัญชีผู้ใช้ด้วย Management API](/end-user-flows/account-settings/by-management-api)
...
4. **การจัดการเซสชัน**:
   ...
   - ก่อนที่ผู้ใช้ปลายทางจะเข้าถึงการจัดการเซสชัน ต้องยืนยันตัวตนผ่านรหัสผ่าน อีเมล หรือ SMS เพื่อรับรหัสบันทึกการยืนยัน (verification record ID) ที่มีอายุ 10 นาที ดู [รับรหัสบันทึกการยืนยัน](#get-a-verification-record-id)
...
เพื่ออัปเดตข้อมูลโปรไฟล์อื่น ๆ เช่น **familyName, givenName, middleName, nickname, profile (profile page URL), website, gender, birthdate, zoneinfo, locale, และ address** คุณสามารถใช้ endpoint [`PATCH /api/my-account/profile`](https://openapi.logto.io/operation/operation-updateotherprofile)
...
:::note
เพื่อใช้วิธีนี้ คุณต้อง [ตั้งค่า email connector](/connectors/email-connectors/) หรือ [SMS connector](/connectors/sms-connectors/) และตรวจสอบให้แน่ใจว่าได้ตั้งค่า template `UserPermissionValidation` แล้ว
:::
...
หากต้องการเรียนรู้เพิ่มเติมเกี่ยวกับการยืนยัน โปรดดู [การยืนยันความปลอดภัยด้วย Account API](/end-user-flows/security-verification)
...
เพื่ออัปเดตรหัสผ่านผู้ใช้ คุณสามารถใช้ endpoint [`POST /api/my-account/password`](https://openapi.logto.io/operation/operation-updatepassword)
...
:::note
เพื่อใช้วิธีนี้ คุณต้อง [ตั้งค่า email connector](/connectors/email-connectors/) และตรวจสอบให้แน่ใจว่าได้ตั้งค่า template `BindNewIdentifier` แล้ว
:::
...
หลังจากยืนยันโค้ดแล้ว คุณสามารถเรียก [`PATCH /api/my-account/primary-email`](https://openapi.logto.io/operation/operation-updateprimaryemail) เพื่ออัปเดตอีเมลของผู้ใช้ โดยใส่ `verificationId` ลงใน request body เป็น `newIdentifierVerificationRecordId`

:::info[รหัสบันทึกการยืนยัน 2 แบบที่แตกต่างกัน]

คำขอนี้ต้องใช้ verification record ID สองชุด:

- **`logto-verification-id` (header)**: พิสูจน์ตัวตนผู้ใช้ก่อนเปลี่ยนแปลงข้อมูลสำคัญ รับได้โดย [ยืนยันรหัสผ่านผู้ใช้](#verify-the-users-password) หรือ [ส่งโค้ดยืนยันไปยังอีเมล/โทรศัพท์เดิมของผู้ใช้](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)
...
:::tip
เช่นเดียวกับอีเมลที่เก็บระหว่างสมัครใช้งาน อีเมลใด ๆ ที่เชื่อมโยงผ่าน Account API ต้องผ่านการตรวจสอบ [blocklist](/security/blocklist) ที่คุณตั้งค่าไว้ใน <CloudLink to="/security/blocklist">Console > Security > Blocklist</CloudLink> Logto จะปฏิเสธคำขอและส่งข้อความผิดพลาดโดยละเอียดหากอีเมลละเมิดนโยบาย
:::
...
:::note
เพื่อใช้วิธีนี้ คุณต้อง [ตั้งค่า SMS connector](/connectors/sms-connectors/) และตรวจสอบให้แน่ใจว่าได้ตั้งค่า template `BindNewIdentifier` แล้ว
:::
...
เพื่อเชื่อมโยงการเชื่อมต่อโซเชียลใหม่ ก่อนอื่นคุณต้องขอ authorization URL ด้วย [`POST /api/verifications/social`](https://openapi.logto.io/operation/operation-createverificationbysocial)
...
หลังผู้ใช้อำนวยความสะดวกแอป คุณจะได้รับ callback ที่ `redirectUri` พร้อมพารามิเตอร์ `state` จากนั้นใช้ endpoint [`POST /api/verifications/social/verify`](https://openapi.logto.io/operation/operation-verifyverificationbysocial) เพื่อยืนยันการเชื่อมต่อโซเชียล
...
สุดท้าย คุณสามารถใช้ endpoint [`POST /api/my-account/identities`](https://openapi.logto.io/operation/operation-adduseridentities) เพื่อเชื่อมโยงการเชื่อมต่อโซเชียล
...
เพื่อลบการเชื่อมต่อโซเชียล คุณสามารถใช้ endpoint [`DELETE /api/my-account/identities`](https://openapi.logto.io/operation/operation-deleteidentity)
...
:::note
เพื่อใช้วิธีนี้ คุณต้องเปิดฟิลด์ `mfa` ใน [การตั้งค่า account center](#how-to-enable-account-api)
:::
...
- **Custom domain**: หากคุณตั้งค่า [custom domain](/logto-cloud/custom-domain) เช่น `https://auth.example.com` RP ID จะเป็น `auth.example.com`
...
หากคุณต้องการรองรับมากกว่า 5 โดเมนสำหรับ Related Origins โปรดดูเอกสาร [Related Origin Requests](https://passkeys.dev/docs/advanced/related-origins/)
...
ใช้ [`@simplewebauthn/browser`](https://simplewebauthn.dev/) เป็นตัวอย่าง คุณสามารถใช้ฟังก์ชัน `startRegistration` เพื่อลงทะเบียน passkey ในเบราว์เซอร์
...
- `verification_record_id`: verification record ID ที่ถูกต้อง ได้จากการยืนยันปัจจัยเดิมของผู้ใช้ ดูรายละเอียดที่ [รับรหัสบันทึกการยืนยัน](#get-a-verification-record-id)
...
เพื่อจัดการ WebAuthn passkey ที่มีอยู่ คุณสามารถใช้ endpoint [`GET /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-getmfaverifications) เพื่อดึง passkey ปัจจุบันและปัจจัย MFA อื่น ๆ
...
### แทนที่ TOTP ที่มีอยู่ \{#replace-an-existing-totp}

หากผู้ใช้มีปัจจัย TOTP อยู่แล้ว คุณสามารถแทนที่ด้วยชุดใหม่โดยใช้ endpoint [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-createorreplacetotpmfaverification) การดำเนินการนี้เป็น idempotent คือจะลบ TOTP เดิมและผูกชุดใหม่
...
ใช้ endpoint [`POST /api/my-account/mfa-verifications/totp-secret/generate`](https://openapi.logto.io/operation/operation-generatetotpsecret) เพื่อสร้าง TOTP secret ใหม่ แล้วแสดงให้ผู้ใช้ตามขั้นตอนใน [เชื่อมโยง TOTP ใหม่](#link-a-new-totp)
...
### จัดการการตั้งค่าการยืนยัน 2 ขั้นตอน \{#manage-2-step-verification-settings}

:::note
เพื่อใช้วิธีเหล่านี้ คุณต้องเปิดฟิลด์ `mfa` ใน [การตั้งค่า account center](#how-to-enable-account-api)
:::
...
ใช้ endpoint [`GET /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-getmfasettings) เพื่อดึงการตั้งค่าการยืนยัน 2 ขั้นตอนของผู้ใช้
...
เพื่อเปิดหรือปิด 2-step verification ใช้ endpoint [`PATCH /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-updatemfasettings)
...
- `verification_record_id`: verification record ID ที่ถูกต้อง ดูรายละเอียดที่ [รับรหัสบันทึกการยืนยัน](#get-a-verification-record-id)
...
### จัดการแอปที่ผู้ใช้อนุญาต (grants) \{#manage-user-authorized-apps-grants}
...
เพื่อแสดงรายการ app grants ที่ใช้งานของผู้ใช้ปัจจุบัน ใช้ endpoint [`GET /api/my-account/grants`](https://openapi.logto.io/operation/operation-getgrants)
...
เพื่อเพิกถอน app grant เฉพาะ ใช้ endpoint [`DELETE /api/my-account/grants/{grantId}`](https://openapi.logto.io/operation/operation-deletegrantbyid)
