---
description: เพิ่มการยืนยันตัวตนแบบไม่ต้องเขียนโค้ดให้กับเว็บแอปของคุณได้อย่างง่ายดายด้วย Protected App อันล้ำสมัยของ Logto ที่ขับเคลื่อนด้วย Cloudflare รองรับ HTTP Basic Authentication และการตรวจสอบ JWT
...
```mermaid
graph LR
  A("ไคลเอนต์<br/>(เบราว์เซอร์)") -->|Request| B(Logto<br/>Protected App)
  B --> Condition{{เส้นทาง<br/>ตรงกันหรือไม่?}}
  Condition -->|Yes| Matched{{ได้รับการยืนยันตัวตนหรือไม่?}}
...
หากต้องการตรวจสอบ header ภายในแอปของคุณ ดูตัวอย่าง [HTTP Basic Authentication](https://developers.cloudflare.com/workers/examples/basic-auth/) ที่ Cloudflare ให้ไว้เพื่อเรียนรู้วิธีจำกัดการเข้าถึงด้วย schema HTTP Basic
...
JWT นี้เรียกว่า [ID Token](https://auth.wiki/id-token) ซึ่งถูกเซ็นโดย Logto และมีข้อมูลผู้ใช้ โดยการตรวจสอบ JWT นี้ คุณสามารถยืนยันได้ว่าคำขอมาจาก Protected App และปฏิเสธคำขอที่ไม่มี header นี้

โทเค็นนี้ถูกเข้ารหัสและเซ็นเป็นโทเค็น [JWS](https://auth.wiki/jws)
...
## ปรับแต่งการอ้างสิทธิ์ใน ID token \{#customize-id-token-claims}

โดยปกติ header `Logto-ID-Token` จะมีการอ้างสิทธิ์ OIDC มาตรฐาน (เช่น `sub`, `name`, และ `email`) หากต้องการเพิ่ม [การอ้างสิทธิ์เพิ่มเติม](/developers/custom-id-token#extended-claims) เช่น บทบาท (roles) หรือข้อมูลองค์กร ต้องตั้งค่าทั้งสองส่วนดังนี้:
