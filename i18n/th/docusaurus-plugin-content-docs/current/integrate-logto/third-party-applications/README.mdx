...
การผสานรวมแอปพลิเคชันบุคคลที่สามของ Logto ช่วยให้คุณใช้ Logto เป็น [ผู้ให้บริการข้อมูลระบุตัวตน (Identity Provider; IdP)](https://auth.wiki/identity-provider) สำหรับแอปพลิเคชันภายนอก
...
แตกต่างจากแอปพลิเคชันที่คุณสร้างในคู่มือ [ผสานรวม Logto เข้ากับแอปพลิเคชันของคุณ](/integrate-logto/integrate-logto-into-your-application) ซึ่งคุณเป็นผู้พัฒนาและควบคุมโดยสมบูรณ์ แอปพลิเคชันบุคคลที่สามเป็นบริการอิสระที่พัฒนาโดยนักพัฒนาภายนอกหรือพันธมิตรทางธุรกิจ
...
Logto เป็นบริการข้อมูลระบุตัวตนที่สร้างขึ้นบนโปรโตคอล [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) โดยให้ทั้ง [การยืนยันตัวตน (Authentication)](https://auth.wiki/authentication) และ [การอนุญาต (Authorization)](https://auth.wiki/authorization) ซึ่งทำให้การผสานรวมแอป OIDC บุคคลที่สามเป็นเรื่องง่ายเหมือนกับแอปเว็บแบบดั้งเดิม
...
4. กรอกชื่อและคำอธิบายสำหรับแอปของคุณแล้วคลิกปุ่ม “Create” จะมีการสร้างแอปพลิเคชันบุคคลที่สามใหม่
...
ในหน้ารายละเอียดแอปพลิเคชัน คุณจะพบ [**Client ID**](/integrate-logto/application-data-structure#application-id), [**Client secret**](/integrate-logto/application-data-structure#application-secret) (สำหรับเว็บแอปแบบดั้งเดิมเท่านั้น) และ endpoint OIDC ที่จำเป็นสำหรับการผสานรวม
...
4. คัดลอก **Redirect URI** จากหน้าตั้งค่าของบริการและเพิ่มลงใน allowed redirect URIs ของแอป Logto ของคุณ
5. ตั้งค่า **scopes** หากบริการนั้นอนุญาต เนื่องจาก Logto เป็น OIDC provider ให้เพิ่ม scope `openid` หากคุณต้องการยืนยันตัวตนผู้ใช้ (จะได้รับ ID token และ endpoint UserInfo) scope `openid` เป็นตัวเลือกหากคุณต้องการเพียง OAuth resource access
...
แอปเว็บแบบดั้งเดิมเป็น **confidential clients** ที่สามารถเก็บ client secret ไว้ที่ backend server ได้อย่างปลอดภัย ดูรายละเอียดการใช้งานเต็มที่ได้ที่ [Authorization Code Flow](https://auth.wiki/authorization-code-flow)

**ขั้นตอนสำคัญ:**

1. **เริ่มต้นการอนุญาต**: เปลี่ยนเส้นทางผู้ใช้ไปที่ authorization endpoint ของ Logto พร้อม `client_id`, `redirect_uri`, `response_type=code` และ `scope`
2. **จัดการ callback**: รับ authorization `code` จาก redirect
3. **แลกเปลี่ยนโทเค็น**: จาก backend ของคุณ POST ไปที่ token endpoint พร้อม code, `client_id` และ `client_secret`
...
2. **เริ่มต้นการอนุญาต**: เปลี่ยนเส้นทางผู้ใช้ไปที่ authorization endpoint พร้อม `code_challenge` และ `code_challenge_method=S256`
3. **จัดการ callback**: รับ authorization `code` จาก redirect
4. **แลกเปลี่ยนโทเค็น**: POST ไปที่ token endpoint พร้อม code และ `code_verifier` เดิม
...
:::note
ก่อนใช้งาน device flow อย่าลืมตั้งค่า [สิทธิ์ (permissions)](/integrate-logto/third-party-applications/permission-management) ที่จำเป็นสำหรับแอปบุคคลที่สามของคุณใน Logto Console แอปบุคคลที่สามที่ร้องขอ scopes ที่ไม่ได้เปิดใช้งานจะถูกปฏิเสธการเข้าถึง
:::

ดู [Device flow quick start](/quick-starts/device-flow) สำหรับรายละเอียดการใช้งานเต็มที่
...
### เราจะมั่นใจได้อย่างไรว่าผู้ใช้สามารถให้สิทธิ์เฉพาะที่ตนเองมีบนหน้าขอความยินยอม? \{#how-do-we-ensure-users-can-only-grant-permissions-they-actually-have-on-the-consent-screen}
...
- กำหนด [บทบาทระดับโกลบอล](/authorization/role-based-access-control) หรือ [บทบาทองค์กร](/authorization/organization-template) พร้อม scopes เฉพาะ
