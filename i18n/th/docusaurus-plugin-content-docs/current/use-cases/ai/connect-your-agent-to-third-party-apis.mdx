...
- **📧 การทำงานอีเมลอัตโนมัติ**: ส่งอีเมลติดตาม จัดระเบียบกล่องจดหมาย หรือร่างคำตอบโดยใช้ Gmail API
...
เพื่อดำเนินงานเหล่านี้ AI agent ของคุณต้องเข้าถึง API ของบุคคลที่สามที่ผู้ใช้อนุญาตอย่างปลอดภัย ซึ่งหมายถึงการจัดการ OAuth token อย่างถูกต้องและปลอดภัย
...
sequenceDiagram
    ...
    rect rgba(200, 230, 200, 0.5)
        ...
        User->>Agent: "เพิ่มการนัดหมายในปฏิทินของฉันพรุ่งนี้ 15.00 น."
    ...
    rect rgba(200, 200, 230, 0.5)
        ...
        Agent->>Google: เรียกใช้ Google Calendar API
        Google->>Agent: ส่งผลลัพธ์สำเร็จกลับ
        Agent->>User: "เรียบร้อย! ฉันได้เพิ่มการนัดหมายในปฏิทินของคุณแล้ว"
    end
```
...
3. **จัดเก็บโทเค็น**: หลังจากผู้ใช้ออกสิทธิ์ Logto จะจัดเก็บ access token และ refresh token ไว้ใน Secret Vault อย่างปลอดภัย
4. **ดำเนินงาน**: agent ดึงโทเค็นที่จัดเก็บไว้และเรียกใช้ API ของบุคคลที่สามเพื่อทำงานให้เสร็จ
...
เพื่อให้ AI agent ของคุณเข้าถึง API ของบุคคลที่สามได้ คุณต้องกำหนดค่าตัวเชื่อมต่อโซเชียลพร้อมเปิดใช้งานการจัดเก็บโทเค็น ซึ่งจะทำให้ Logto สามารถจัดเก็บและจัดการ access token เมื่อผู้ใช้ออกสิทธิ์บริการบุคคลที่สามระหว่างโต้ตอบกับ AI agent ของคุณ
...
3. ทำตาม [คู่มือการตั้งค่า Google connector](/integrations/google) เพื่อกำหนดค่า OAuth client credentials
...
เมื่อ AI agent ของคุณต้องเข้าถึง API ของบุคคลที่สาม (เช่น Google Calendar) ควรตรวจสอบก่อนว่าผู้ใช้ได้อนุญาตไว้แล้วหรือไม่ หากยัง ให้แจ้งผู้ใช้เพื่อขออนุญาต

:::info เปิดใช้งาน Account API
ก่อนดำเนินการต่อ โปรดเปิดใช้งาน Account API ที่ <CloudLink to="/sign-in-experience/account-center">Console > Sign-in & account > Account center</CloudLink> ดูเพิ่มเติมเกี่ยวกับ [การเปิดใช้งาน Account API](/end-user-flows/account-settings/by-account-api#how-to-enable-account-api)
:::
...
หลังจากผู้ใช้ให้สิทธิ์ Google จะเปลี่ยนเส้นทางกลับมายังแอปของคุณ ดำเนินการยืนยันและจัดเก็บโทเค็น:
...
### ขั้นตอนที่ 4: เรียกใช้ API ของบุคคลที่สาม \{#step-4-call-the-third-party-api}

ตอนนี้ AI agent ของคุณสามารถดึงโทเค็นและเรียกใช้ API ได้แล้ว:
...
Logto จะจัดการรีเฟรชโทเค็นให้อัตโนมัติ หาก access token หมดอายุแต่มี refresh token อยู่ Logto จะขอ access token ใหม่โดยอัตโนมัติเมื่อคุณเรียก endpoint สำหรับดึงโทเค็น
...
4. แต่ละการเชื่อมต่อจะแสดงสถานะโทเค็น:
   ...
   - **Expired**: access token หมดอายุ หากมี refresh token จะรีเฟรชอัตโนมัติเมื่อดึงครั้งถัดไป
   - **Inactive**: ไม่มีโทเค็นจัดเก็บสำหรับการเชื่อมต่อนี้

## แนวทางปฏิบัติด้านความปลอดภัยที่ดีที่สุด \{#security-best-practices}

เมื่อสร้าง AI agent ที่เข้าถึง API ของบุคคลที่สาม โปรดคำนึงถึงแนวทางเหล่านี้:
...
- **ปกป้อง user access token**: Logto access token ของผู้ใช้คือกุญแจในการดึงโทเค็นของบุคคลที่สาม ต้องปกป้องอย่างเหมาะสม
