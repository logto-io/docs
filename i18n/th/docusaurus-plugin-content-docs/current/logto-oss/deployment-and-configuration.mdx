...
ในบทความก่อนหน้านี้ เราได้พูดถึงพื้นฐานของ [เริ่มต้นใช้งาน Logto อย่างรวดเร็ว](/logto-oss/get-started-with-oss) บทความนี้จะเจาะลึกยิ่งขึ้น โดยเน้นแนวปฏิบัติที่ดีที่สุดและขั้นตอนการตั้งค่าโดยละเอียดสำหรับการปรับใช้ Logto ในสภาพแวดล้อม production
...
- `DATABASE_STATEMENT_TIMEOUT` (v1.36.0+) PostgreSQL `statement_timeout` หน่วยเป็นมิลลิวินาที ใช้เป็นสตริงตัวเลข (เช่น `5000`) หรือ `DISABLE_TIMEOUT` สำหรับ PgBouncer / RDS Proxy ดู [การตั้งค่า](/concepts/core-service/configuration/) สำหรับรายละเอียด
- `PORT` พอร์ตที่ Logto รับฟัง ค่าเริ่มต้น `3001`
- `ENDPOINT` คุณสามารถระบุ URL ที่เป็นโดเมนของคุณเองสำหรับ production (เช่น `ENDPOINT=https://logto.domain.com`) ซึ่งจะมีผลกับค่าของ [OIDC issuer identifier](https://openid.net/specs/openid-connect-core-1_0.html#IssuerIdentifier) ด้วย

**เปิดใช้งาน Admin Console**

- `ADMIN_PORT` พอร์ตที่ Logto Admin Console รับฟัง ค่าเริ่มต้น `3002`
- `ADMIN_ENDPOINT` คุณสามารถระบุ URL ที่เป็นโดเมนของคุณเองสำหรับ production (เช่น `ADMIN_ENDPOINT=https://admin.domain.com`) ซึ่งจะมีผลกับค่า Admin Console Redirect URIs ด้วย

**ปิดใช้งาน Admin Console**

- `ADMIN_DISABLE_LOCALHOST` ตั้งค่าเป็น `1` หรือ `true` เพื่อปิดพอร์ตสำหรับ Admin Console หาก `ADMIN_ENDPOINT` ไม่ถูกตั้งค่า จะปิด Admin Console โดยสมบูรณ์
...
คุณสามารถใช้ Node.js เพื่อให้บริการ HTTPS โดยตรง หรือจะตั้งค่า proxy / balancer สำหรับ HTTPS ไว้หน้าตัว Node.js ก็ได้ ดู [การเปิดใช้งาน HTTPS](/concepts/core-service/configuration/#enabling-https) สำหรับรายละเอียด
...
## การทำให้เป็นคอนเทนเนอร์ (Containerization) \{#reverse-proxy}
...
### การตั้งค่าฐานข้อมูล (Database setup) \{#containerization}
...
หากสภาพแวดล้อมที่คุณปรับใช้ไม่สามารถเข้าถึง `api.pwnedpasswords.com` ให้เพิ่ม `--disable-admin-pwned-password-check` ไปที่ `logto init` หรือ `npm run cli db seed` เพื่อไม่ให้การสมัครแอดมินครั้งแรกค้างที่การตรวจสอบ Have I Been Pwned ดู [Seed สำหรับการปรับใช้แบบ air-gapped หรือออฟไลน์](/logto-oss/using-cli#seed-for-air-gapped-or-offline-deployments) สำหรับรายละเอียด

:::

### โฟลเดอร์ connectors ร่วม (Shared connectors folder) \{#database-setup}
...
### การเปลี่ยนแปลงฐานข้อมูล (Database alteration) \{#shared-connectors-folder}
