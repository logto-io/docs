...
ในบทความก่อนหน้านี้ เราได้พูดถึงพื้นฐานของ [เริ่มต้นใช้งาน Logto อย่างรวดเร็ว](/logto-oss/get-started-with-oss) บทความนี้จะลงลึกยิ่งขึ้น โดยเน้นแนวปฏิบัติที่ดีที่สุดและขั้นตอนการตั้งค่าอย่างละเอียดสำหรับการปรับใช้ Logto ในสภาพแวดล้อม production
...
- `DATABASE_STATEMENT_TIMEOUT` (v1.36.0+) PostgreSQL `statement_timeout` หน่วยเป็นมิลลิวินาที ใช้เป็น string ตัวเลข (เช่น `5000`) หรือ `DISABLE_TIMEOUT` สำหรับ PgBouncer / RDS Proxy ดูรายละเอียดที่ [Configuration](/concepts/core-service/configuration/)
- `PORT` พอร์ตที่ Logto รับฟัง ค่าเริ่มต้น `3001`
- `ENDPOINT` คุณสามารถระบุ URL ที่เป็นโดเมนของคุณเองสำหรับ production (เช่น `ENDPOINT=https://logto.domain.com`) ค่านี้จะมีผลกับ [OIDC issuer identifier](https://openid.net/specs/openid-connect-core-1_0.html#IssuerIdentifier) ด้วย

**เปิดใช้งาน Admin Console**

- `ADMIN_PORT` พอร์ตที่ Logto Admin Console รับฟัง ค่าเริ่มต้น `3002`
- `ADMIN_ENDPOINT` คุณสามารถระบุ URL ที่เป็นโดเมนของคุณเองสำหรับ production (เช่น `ADMIN_ENDPOINT=https://admin.domain.com`) ค่านี้จะมีผลกับค่า Admin Console Redirect URIs ด้วย

**ปิดใช้งาน Admin Console**

- `ADMIN_DISABLE_LOCALHOST` ตั้งค่าเป็น `1` หรือ `true` เพื่อปิดพอร์ตของ Admin Console หากไม่ได้ตั้งค่า `ADMIN_ENDPOINT` จะปิด Admin Console โดยสมบูรณ์

ดูรายละเอียดเพิ่มเติมเกี่ยวกับตัวแปรสภาพแวดล้อมได้ที่ [Configuration](/concepts/core-service/configuration/)

**เปิดใช้งาน Secret Vault**

- หากต้องการใช้ [Secret Vault](/secret-vault) คุณต้องตั้งค่า `SECRET_VAULT_KEK` เป็น string ที่เข้ารหัส base64 ของ Key Encryption Key (KEK) ของคุณ ใช้สำหรับเข้ารหัส Data Encryption Keys (DEK) ใน Secret Vault แนะนำให้ใช้ AES-256 (32 ไบต์) ตัวอย่าง: `crypto.randomBytes(32).toString('base64')`

### HTTPS \{#https}

คุณสามารถใช้ Node.js เพื่อให้บริการ HTTPS โดยตรง หรือจะตั้งค่า proxy / balancer สำหรับ HTTPS ไว้หน้า Node.js ก็ได้ ดูรายละเอียดที่ [Enabling HTTPS](/concepts/core-service/configuration/#enabling-https)
...
## การทำให้เป็นคอนเทนเนอร์ (Containerization) \{#containerization}
...
:::tip สำหรับการติดตั้งแบบ air-gapped หรือออฟไลน์

หากสภาพแวดล้อมที่คุณปรับใช้ไม่สามารถเข้าถึง `api.pwnedpasswords.com` ให้เพิ่ม `--disable-admin-pwned-password-check` ในคำสั่ง `logto init` หรือ `npm run cli db seed` เพื่อไม่ให้การสมัครแอดมินคนแรกค้างที่การตรวจสอบ breach ของ Have I Been Pwned ดูรายละเอียดที่ [Seed for air-gapped or offline deployments](/logto-oss/using-cli#seed-for-air-gapped-or-offline-deployments)

:::

### โฟลเดอร์ connectors ที่ใช้ร่วมกัน (Shared connectors folder) \{#shared-connectors-folder}
