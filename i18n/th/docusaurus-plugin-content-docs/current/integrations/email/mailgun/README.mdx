...
- กรอกช่อง `from` ด้วยที่อยู่อีเมลที่คุณต้องการใช้ส่งอีเมล ที่อยู่อีเมลนี้ต้องลงทะเบียนในบัญชี Mailgun ของคุณ ที่อยู่อีเมลควรอยู่ในรูปแบบ `Sender Name \<sender@example.com>`

## การจัดส่ง (Deliveries) \{#deliveries}

### อ็อบเจกต์ config \{#config-object}

ส่วน "Deliveries" ช่วยให้คุณกำหนดเนื้อหาของอีเมลที่จะส่งในแต่ละสถานการณ์ได้ เป็นแผนที่ JSON แบบ key-value โดยที่ key คือประเภทการใช้งาน และ value คืออ็อบเจกต์ที่มีการตั้งค่าเนื้อหาของอีเมลที่จะส่ง
...
:::note
หากไม่ได้ระบุประเภทการใช้งานใน config ของ deliveries จะใช้อีเมล generic แทน หากไม่ได้ระบุอีเมล generic ตัวเชื่อมต่อจะส่งคืนข้อผิดพลาด
:::

### การตั้งค่าเนื้อหา (Content config) \{#content-config}

ตัวเชื่อมต่อรองรับทั้งเนื้อหา HTML โดยตรงและ Mailgun template คุณสามารถเลือกใช้แบบใดแบบหนึ่งสำหรับแต่ละประเภทการใช้งาน

ทั้งในหัวข้อ (subject) และเนื้อหา (content) คุณสามารถใช้ placeholder `{{code}}` เพื่อแทรกรหัสยืนยันได้

หากต้องการใช้เนื้อหา HTML โดยตรง ให้กรอกช่องต่อไปนี้:

- `subject`: หัวข้อของอีเมลที่จะส่ง
...
- `variables`: ตัวแปรที่จะส่งไปยัง Mailgun template ควรเป็นแผนที่ JSON แบบ key-value เนื่องจากจะถูกแปลงเป็นสตริงก่อนส่งไปยัง Mailgun ไม่จำเป็นต้องใส่ตัวแปร `code` เพราะตัวเชื่อมต่อจะเพิ่มให้อัตโนมัติ
...
เพียงเท่านี้ อย่าลืม <MainSiteUrl href="/connectors/email-connectors/#enable-email-address-for-end-user-flows">เปิดใช้งานตัวเชื่อมต่อในขั้นตอนสมัครและลงชื่อเข้าใช้</MainSiteUrl>
