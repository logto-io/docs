import BlockUserIcon from '@site/src/assets/block-user.svg';
import CheckboxIcon from '@site/src/assets/checkbox.svg';
import HideIcon from '@site/src/assets/hide.svg';
import KeyIcon from '@site/src/assets/key.svg';
import LockIcon from '@site/src/assets/lock.svg';
import LockoutIcon from '@site/src/assets/lockout.svg';
import SecurityIcon from '@site/src/assets/security.svg';
import SendEmailIcon from '@site/src/assets/send-mail.svg';
import SuspendIcon from '@site/src/assets/suspend.svg';

# ความปลอดภัย

ความปลอดภัยของการยืนยันตัวตน (Authentication) สมัยใหม่ต้องต่อสู้กับภัยคุกคามหลากหลาย ตั้งแต่ฟิชชิ่ง, การโจมตีด้วยข้อมูลรับรองซ้ำ (credential stuffing), การเดารหัสผ่าน (brute-force), แรนซัมแวร์, DDoS ไปจนถึงการโจมตีที่ขับเคลื่อนด้วย AI การปกป้องตัวตนของผู้ใช้จึงเป็นหัวใจสำคัญในการรักษาความน่าเชื่อถือของแบรนด์และการปฏิบัติตามข้อกำหนด

Logto มอบการจัดการการเข้าถึงที่ปลอดภัยและแข็งแกร่ง ออกแบบมาเพื่อตอบโต้ความเสี่ยงเหล่านี้โดยตรง ด้วยการให้ความสำคัญกับการป้องกันเชิงรุกและความยืดหยุ่นต่อภัยคุกคาม เราจึงมั่นใจว่าระบบของคุณจะได้รับการปกป้องโดยไม่ลดทอนประสบการณ์การใช้งาน ด้วย Logto ความปลอดภัยไม่ใช่เรื่องที่คิดทีหลัง—แต่เป็นรากฐานที่ช่วยให้ธุรกิจเติบโตในยุคที่ภัยคุกคามเปลี่ยนแปลงตลอดเวลา แต่การป้องกันก็พัฒนาเร็วกว่า

## ตั้งค่าการป้องกันความปลอดภัยขั้นสูง \{#set-up-advanced-security-protection}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'นโยบายรหัสผ่าน',
      href: '/security/password-policy',
      description:
        'เพิ่มความเข้มงวดของรหัสผ่านเพื่อป้องกันการโจมตีด้วย credential stuffing และรหัสผ่านที่อ่อนแอ',
      customProps: {
        icon: <KeyIcon />,
      },
    },
    {
      type: 'link',
      label: 'CAPTCHA',
      href: '/security/captcha',
      description:
        'เพิ่ม CAPTCHA (เช่น Google reCAPTCHA, Cloudflare Turnstile) ในประสบการณ์การลงชื่อเข้าใช้เพื่อป้องกันการโจมตีจากบอทอัตโนมัติ',
      customProps: {
        icon: <CheckboxIcon />,
      },
    },
    {
      type: 'link',
      label: 'Blocklist',
      href: '/security/blocklist',
      description:
        'ควบคุมการสมัครใหม่และอีเมลที่เชื่อมโยงใหม่ด้วย allowlist และบล็อกอีเมลชั่วคราวหรือที่ไม่ต้องการ',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    {
      type: 'link',
      label: 'ล็อกเอาต์ตัวระบุ (Identifier lockout)',
      href: '/security/identifier-lockout',
      description:
        'ล็อกตัวระบุชั่วคราวหลังจากพยายามยืนยันตัวตนล้มเหลวหลายครั้งเพื่อป้องกันการเข้าถึงแบบ brute force',
      customProps: {
        icon: <LockoutIcon />,
      },
    },
    {
      type: 'link',
      label: 'จำกัดอัตราการส่ง (Send rate limit)',
      href: '/security/send-rate-limit',
      description:
        'จำกัดจำนวนการส่งรหัสยืนยันและข้อความต่อผู้รับแต่ละรายเพื่อป้องกันสแปมในกล่องจดหมายและการโจมตี SMS pumping',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'ซ่อนการมีอยู่ของบัญชี (Hide account existence)',
      href: '/end-user-flows/sign-up-and-sign-in',
      description:
        'ซ่อนสถานะบัญชีเพื่อป้องกันการโจมตีแบบ account enumeration และหลีกเลี่ยงการเปิดเผยข้อมูลสถานะบัญชีที่ละเอียดอ่อน',
      customProps: {
        icon: <HideIcon />,
      },
    },
  ]}
/>

## ค้นพบวิธีปกป้องแอปของคุณเพิ่มเติม \{#discover-more-ways-to-protect-your-apps}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'การยืนยันตัวตนหลายปัจจัย (MFA)',
      href: '/end-user-flows/mfa',
      description:
        'เพิ่มชั้นการป้องกันพิเศษให้กับกระบวนการลงชื่อเข้าใช้ ด้วยการรองรับ OTP จากแอป authenticator, passkey (WebAuthn) และรหัสสำรอง',
      customProps: {
        icon: <LockIcon />,
      },
    },
    {
      type: 'link',
      label: 'การยืนยันตัวตนแบบ Step-up',
      href: '/end-user-flows/security-verification',
      description:
        'ให้แอปสามารถร้องขอการยืนยันตัวตนแบบ step-up เมื่อผู้ใช้เข้าถึงข้อมูลสำคัญหรือดำเนินการที่มีความเสี่ยงสูง',
      customProps: {
        icon: <SecurityIcon />,
      },
    },
    {
      type: 'link',
      label: 'ระงับผู้ใช้ (Suspend users)',
      href: '/user-management/manage-users#suspend-user',
      description:
        'ปิดใช้งานบัญชีผู้ใช้ชั่วคราวเพื่อป้องกันการเข้าถึงโดยไม่ต้องลบข้อมูลหรือประวัติผู้ใช้',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'หมุนเวียนคีย์สำหรับลงนาม (Rotate signing keys)',
      href: '/developers/signing-keys',
      description:
        'หมุนเวียนคีย์สำหรับลงนามเป็นระยะเพื่อป้องกันการรั่วไหลของคีย์และการปลอมแปลงโทเค็น',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'OIDC back-channel logout',
      href: '/end-user-flows/sign-out#federated-sign-out-back-channel-logout',
      description:
        'เปิดใช้งานการออกจากระบบแบบศูนย์กลางเพื่อลดความเสี่ยงจากการแย่งชิง session และการเข้าถึงโดยไม่ได้รับอนุญาต',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'ลงทะเบียนด้วยคำเชิญเท่านั้น (Invitation-only sign up)',
      href: '/end-user-flows',
      description:
        'จำกัดการสมัครให้เฉพาะผู้ที่ได้รับเชิญ โดยใช้ magic link ทางอีเมลเพื่อความปลอดภัยในการเริ่มต้นใช้งาน',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
  ]}
/>
