...
ลองจินตนาการว่า Sarah วิศวกรฝ่ายสนับสนุนที่ TechCorp ได้รับตั๋วแจ้งปัญหาด่วนจาก Alex ลูกค้าที่ไม่สามารถเข้าถึงทรัพยากรสำคัญได้ เพื่อวิเคราะห์และแก้ไขปัญหาอย่างมีประสิทธิภาพ Sarah จำเป็นต้องเห็นสิ่งเดียวกับที่ Alex เห็นในระบบ นี่คือจุดที่ฟีเจอร์การสวมรอยผู้ใช้ของ Logto มีประโยชน์อย่างมาก

การสวมรอยผู้ใช้ช่วยให้ผู้ใช้ที่ได้รับอนุญาต เช่น Sarah สามารถดำเนินการแทนผู้ใช้อื่น เช่น Alex ได้ชั่วคราวภายในระบบ ฟีเจอร์นี้มีคุณค่าอย่างยิ่งสำหรับการแก้ไขปัญหา การให้บริการลูกค้า และการดำเนินงานด้านผู้ดูแลระบบ
...
ตอนนี้แอปของ Sarah จะแลก subject token นี้เป็น access token ที่แทน Alex โดยระบุ resource ที่จะใช้ token นี้
...
async function impersonateUser(
  ...
  resource: string,
  // highlight-next-line
  clientSecret?: string // สำหรับเว็บแอปแบบดั้งเดิมหรือแอป machine-to-machine เท่านั้น
): Promise<string> {
  try {
...
2. access token สำหรับการสวมรอยจะไม่มี [refresh token](https://auth.wiki/refresh-token) หาก token หมดอายุ Sarah ต้องทำกระบวนการนี้ใหม่อีกครั้ง
...
เมื่อใช้ token exchange flow สำหรับการสวมรอย access token ที่ออกมาอาจมี `act` (actor) claim เพิ่มเติม ซึ่ง claim นี้แสดงตัวตนของ “ผู้ดำเนินการ” — ในตัวอย่างนี้คือ Sarah ผู้ที่กำลังสวมรอย
...
`act` claim นี้แสดงอย่างชัดเจนว่า Sarah (sarah789) กำลังดำเนินการในนามของ Alex (alex123) ซึ่งมีประโยชน์สำหรับการตรวจสอบและติดตามการสวมรอย

## การปรับแต่ง token claims \{#customizing-token-claims}

Logto อนุญาตให้คุณ [ปรับแต่ง token claims](/developers/custom-token-claims) สำหรับ access token ที่ใช้ในการสวมรอย ซึ่งมีประโยชน์สำหรับการเพิ่มข้อมูลบริบทหรือ metadata เพิ่มเติม เช่น เหตุผลในการสวมรอย หรือหมายเลขตั๋วสนับสนุนที่เกี่ยวข้อง
...
[context](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) นี้สามารถนำไปใช้ในฟังก์ชัน `getCustomJwtClaims()` เพื่อเพิ่ม claim เฉพาะลงใน access token ตัวอย่างการใช้งาน:
...
ด้วยการปรับแต่ง access token claims แบบนี้ TechCorp สามารถใส่ข้อมูลสำคัญเกี่ยวกับบริบทของการสวมรอย ช่วยให้ตรวจสอบและเข้าใจการดำเนินการสวมรอยในระบบได้ง่ายขึ้น

:::note
โปรดระวังเมื่อเพิ่ม custom claim ลงใน token ของคุณ หลีกเลี่ยงการใส่ข้อมูลสำคัญที่อาจก่อให้เกิดความเสี่ยงด้านความปลอดภัยหาก token ถูกดักจับหรือรั่วไหล JWT จะถูกเซ็นชื่อแต่ไม่ถูกเข้ารหัส ดังนั้น claim ทั้งหมดจะมองเห็นได้สำหรับผู้ที่เข้าถึง token
:::

## แหล่งข้อมูลที่เกี่ยวข้อง \{#related-resources}

<Url href="https://blog.logto.io/impersonation">
  การสวมรอยในโลกไซเบอร์และการจัดการเอกลักษณ์คืออะไร? เอเจนต์ AI ใช้งานได้อย่างไร?
</Url>
