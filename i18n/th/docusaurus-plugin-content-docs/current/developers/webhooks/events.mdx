...
### สิทธิ์ (ขอบเขต; Scope) \{#permission-scope}
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| การเชื่อมโยงอีเมล / เบอร์โทรศัพท์ของผู้ใช้                                                                                                              | User.Data.Updated               |
...
| ประเภทเหตุการณ์     | คำอธิบาย                                                                                                                                                                                                                                              |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identifier.Lockout  | บัญชีผู้ใช้ถูกล็อกเนื่องจากพยายามยืนยันตัวตนล้มเหลวติดต่อกัน สามารถถูกกระตุ้นในขั้นตอน:<br /><ul><li>ยืนยันรหัสผ่านล้มเหลว</li><li>ยืนยันรหัสล้มเหลว</li><li>ยืนยันโทเค็นแบบครั้งเดียวล้มเหลว</li></ul>                                               |
| Message.RateLimited | ผู้ใช้ปลายทางส่งเกิน [ขีดจำกัดการส่ง](/security/send-rate-limit) สำหรับรหัสยืนยัน เหตุการณ์นี้จะเกิดเฉพาะในเส้นทางส่งของผู้ใช้ปลายทาง (Experience API และ Account API) โดย payload คือ `{ action, recipient }`                                        |
| Grant.LimitExceeded | การอนุญาต (Authorization) สำเร็จทำให้ผู้ใช้เกิน [จำนวนอุปกรณ์ที่ลงชื่อเข้าใช้พร้อมกันสูงสุด](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) (`maxAllowedGrants`) สำหรับแอปนั้น Logto จะเพิกถอน grant ที่เก่าที่สุดของแอปนั้น |
