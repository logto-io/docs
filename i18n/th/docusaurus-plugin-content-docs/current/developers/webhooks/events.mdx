---
id: webhooks-events
title: เหตุการณ์ Webhooks (Webhooks events)
sidebar_label: เหตุการณ์ Webhooks (Webhooks events)
sidebar_position: 3
---

# เหตุการณ์ Webhooks (Webhooks events)

คู่มือนี้จะแสดงรายการเหตุการณ์ webhook ของ Logto ที่แตกต่างกันและอธิบายว่าแต่ละเหตุการณ์เกิดขึ้นเมื่อใด

## เหตุการณ์ webhook จากการโต้ตอบของผู้ใช้ \{#user-interaction-hook-events}
...
## เหตุการณ์ webhook จากการเปลี่ยนแปลงข้อมูล \{#data-mutation-hook-events}
...
### อุปกรณ์ที่เชื่อถือได้ (Trusted device) \{#trusted-device}

| ประเภทเหตุการณ์       | คำอธิบาย                                                          |
| --------------------- | ----------------------------------------------------------------- |
| TrustedDevice.Created | มีการสร้างระเบียนอุปกรณ์ที่เชื่อถือได้หลังจากการยืนยันตัวตนสำเร็จ |
| TrustedDevice.Deleted | ผู้ใช้หรือผู้ดูแลระบบลบระเบียนอุปกรณ์ที่เชื่อถือได้ออก            |

การหมดอายุโดยธรรมชาติจะไม่ส่ง webhook `TrustedDevice.Expired` เพราะการหมดอายุเป็นการเปลี่ยนแปลงความถูกต้องตามเวลา ไม่ใช่การเปลี่ยนแปลงข้อมูลโดยแอปพลิเคชัน Payload ของ `TrustedDevice.Created` จะมี `expiresAt` เพื่อให้ผู้ใช้งานสามารถคำนวณเวลาหมดอายุที่คาดไว้ได้ การใช้งานสำเร็จและการเปลี่ยนนโยบายอุปกรณ์ที่เชื่อถือได้จะไม่ส่ง lifecycle webhook
...
| ผู้ใช้ถูกจัดเตรียมเข้าสู่องค์กรโดยอัตโนมัติผ่าน [Just-in-Time provisioning](/organizations/just-in-time-provisioning) (โดเมนอีเมลตรงหรือ SSO connector) | Organization.Membership.Updated |

## เหตุการณ์ webhook ข้อยกเว้น \{#exception-hook-events}

### ความปลอดภัย (Security) \{#security}

| ประเภทเหตุการณ์     | คำอธิบาย                                                                                                                                                                                                                                      |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identifier.Lockout  | บัญชีผู้ใช้ถูกล็อกเนื่องจากพยายามยืนยันตัวตนผิดติดต่อกันหลายครั้ง สามารถเกิดขึ้นได้ในขั้นตอน:<br /><ul><li>ตรวจสอบรหัสผ่านผิด</li><li>ตรวจสอบรหัสยืนยันผิด</li><li>ตรวจสอบโทเค็นแบบใช้ครั้งเดียวผิด</li></ul>                                 |
| Message.RateLimited | ผู้ใช้ปลายทางส่งคำขอรับรหัสยืนยันเกิน [ขีดจำกัดการส่งต่อผู้รับ](/security/send-rate-limit) จะถูกส่งเฉพาะเส้นทางส่งของผู้ใช้ปลายทาง (Experience API และ Account API) โดย payload คือ `{ action, recipient }`                                   |
| Grant.LimitExceeded | การอนุญาต (Authorization) สำเร็จทำให้ผู้ใช้เกิน [จำนวนอุปกรณ์ที่ลงชื่อเข้าใช้พร้อมกันสูงสุดต่อแอป](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) (`maxAllowedGrants`) Logto จะเพิกถอน grant ที่เก่าที่สุดของแอปนั้น |
...
`PostRegister` จะถูกกระตุ้นเมื่อผู้ใช้สร้างบัญชีใหม่สำเร็จผ่านขั้นตอนลงทะเบียนของผู้ใช้ ส่วน `User.Created` จะถูกกระตุ้นเมื่อมีการสร้างบัญชีผู้ใช้ใหม่ผ่าน Management API
