import BlockUserIcon from '@site/src/assets/block-user.svg';
import CheckboxIcon from '@site/src/assets/checkbox.svg';
import HideIcon from '@site/src/assets/hide.svg';
import KeyIcon from '@site/src/assets/key.svg';
import LockIcon from '@site/src/assets/lock.svg';
import LockoutIcon from '@site/src/assets/lockout.svg';
import SecurityIcon from '@site/src/assets/security.svg';
import SendEmailIcon from '@site/src/assets/send-mail.svg';
import SuspendIcon from '@site/src/assets/suspend.svg';

# Segurança

A segurança moderna de autenticação enfrenta ameaças que vão desde phishing, stuffing de credenciais, ataques de força bruta, ransomware, DDoS, até ataques impulsionados por IA. Proteger as identidades dos usuários é fundamental para garantir a confiança na marca e a conformidade.

O Logto oferece uma gestão de acesso segura e robusta, projetada para enfrentar esses riscos de frente. Ao priorizar a prevenção proativa de ameaças e a resiliência, garantimos que seus sistemas permaneçam protegidos sem comprometer a usabilidade. Com o Logto, a segurança não é um pensamento tardio — é a base, capacitando empresas a prosperar em uma era onde as ameaças evoluem, mas as defesas evoluem mais rápido.

## Configure proteção de segurança avançada \{#set-up-advanced-security-protection}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Política de senha',
      href: '/security/password-policy',
      description:
        'Aprimore os requisitos de senha para se defender contra stuffing de credenciais e ataques de senhas fracas.',
      customProps: {
        icon: <KeyIcon />,
      },
    },
    {
      type: 'link',
      label: 'CAPTCHA',
      href: '/security/captcha',
      description:
        'Adicione CAPTCHA (por exemplo, Google reCAPTCHA, Cloudflare Turnstile) à sua experiência de login para evitar ataques automatizados de bots.',
      customProps: {
        icon: <CheckboxIcon />,
      },
    },
    {
      type: 'link',
      label: 'Blocklist',
      href: '/security/blocklist',
      description:
        'Assuma o controle da sua base de usuários bloqueando domínios ou endereços de e-mail descartáveis ou indesejados.',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    {
      type: 'link',
      label: 'Bloqueio de identificador',
      href: '/security/identifier-lockout',
      description:
        'Bloqueie temporariamente um identificador após múltiplas tentativas de autenticação falhadas para evitar acesso por força bruta.',
      customProps: {
        icon: <LockoutIcon />,
      },
    },
    {
      type: 'link',
      label: 'Ocultar existência da conta',
      href: '/end-user-flows/sign-up-and-sign-in',
      description:
        'Oculte o status da conta para bloquear ataques de enumeração de contas e evitar a divulgação de informações sensíveis sobre o status da conta.',
      customProps: {
        icon: <HideIcon />,
      },
    },
  ]}
/>

## Descubra mais formas de proteger seus aplicativos \{#discover-more-ways-to-protect-your-apps}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Autenticação multifatorial (MFA)',
      href: '/end-user-flows/mfa',
      description:
        'Adiciona uma camada extra de proteção ao processo de login com suporte a OTPs de aplicativos autenticadores, passkeys (WebAuthn) e códigos de backup.',
      customProps: {
        icon: <LockIcon />,
      },
    },
    {
      type: 'link',
      label: 'Autenticação step-up',
      href: '/end-user-flows/security-verification',
      description:
        'Permita que o aplicativo solicite autenticação step-up quando os usuários acessarem informações sensíveis ou realizarem ações de alto risco.',
      customProps: {
        icon: <SecurityIcon />,
      },
    },
    {
      type: 'link',
      label: 'Suspender usuários',
      href: '/user-management/manage-users#suspend-user',
      description:
        'Desative temporariamente contas de usuários para bloquear o acesso sem excluir dados ou registros de usuários.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Rotacionar chaves de assinatura',
      href: '/developers/signing-keys',
      description:
        'Rotacione periodicamente as chaves de assinatura para proteger contra vazamento de chaves e falsificação de tokens.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'Logout back-channel OIDC',
      href: '/end-user-flows/sign-out#federated-sign-out-back-channel-logout',
      description:
        'Habilite logout centralizado para reduzir o risco de sequestro de sessão e acesso não autorizado.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Cadastro somente por convite',
      href: '/end-user-flows',
      description:
        'Restrinja cadastros apenas para usuários convidados, utilizando magic links por e-mail para onboarding seguro.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
  ]}
/>
