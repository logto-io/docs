...
Os Webhooks do Logto fornecem notificações em tempo real para vários eventos, incluindo alterações na conta do usuário, papel, permissão, organização, papel da organização, permissão da organização e interação do usuário.
...
Os Webhooks oferecem comunicação em tempo real entre aplicativos, eliminando a necessidade de polling e permitindo atualizações imediatas de dados. Eles simplificam a integração de aplicativos e a automação de fluxos de trabalho sem código complexo ou APIs proprietárias.
...
- **Realizar chamadas adicionais de API**: Configure um Webhook para verificar o acesso do usuário verificando seu domínio de email ou endereço IP e, em seguida, use o Logto Management API para atribuir papéis apropriados com permissões de recurso.
...
- **Gerar relatórios**: Configure um Webhook para receber dados de atividade de login do usuário e aproveite-os para criar relatórios sobre engajamento ou padrões de uso do usuário.
...
| Webhook                                                                                                                                                                                                                        | Um subtipo de hook que indica chamar uma API com o payload do evento.                                                                                                                                                                |
| Digamos que um desenvolvedor queira enviar uma notificação quando o usuário fizer login via um novo dispositivo, o desenvolvedor pode adicionar um webhook que chama sua API de serviço de segurança para o evento PostSignIn. |
...
<details>
<summary>O Logto suporta webhooks sincronizados?</summary>

Embora webhooks sincronizados tornassem o fluxo de login do usuário mais suave, ainda não os suportamos (mas o faremos no futuro). Portanto, cenários que dependem de webhooks sincronizados atualmente exigem diferentes soluções alternativas. Se você tiver alguma dúvida, não hesite em nos contatar.

</details>

<details>
<summary>Como lidar com a mudança de permissão do usuário?</summary>
