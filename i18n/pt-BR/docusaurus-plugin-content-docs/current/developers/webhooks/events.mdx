...
| Message.RateLimited | Um usuário final excede o [limite de envio por destinatário](/security/send-rate-limit) para códigos de verificação. Dispara apenas nos caminhos de envio do usuário final (Experience API e Account API), com um payload de `{ action, recipient }`.                                     |
