...
1. No contexto do RBAC, possuir o escopo para acessar recursos pagos.
...
Em um cenário semelhante, suponha que seu app exiba felicitações de aniversário na página de login se o aniversário do usuário estiver próximo. Você pode usar reivindicações personalizadas em tokens para adicionar um campo de aniversário ao [payload do token](/user-management/personal-access-token#example-token-exchange), que pode ser usado para determinar se uma mensagem específica deve ser exibida.
