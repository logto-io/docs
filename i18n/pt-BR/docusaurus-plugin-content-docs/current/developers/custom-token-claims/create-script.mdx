...
2. Existem dois tipos diferentes de tokens de acesso para os quais você pode personalizar as reivindicações do token de acesso:
   ...
   - **Token de acesso do usuário (User access token)**: O token de acesso emitido para usuários finais. Ex.: para aplicações web ou aplicações móveis.
   - **Token de acesso máquina para máquina (Machine-to-Machine access token)**: O token de acesso emitido para serviços ou aplicativos. Ex.: para [aplicações máquina para máquina](/quick-starts/m2m).
...
- [Locatários Logto Cloud com ambiente de desenvolvimento](/logto-cloud/tenant-settings#development)
...
Na página de detalhes do **JWT personalizado**, você encontrará o editor de scripts para escrever seu script de reivindicações personalizadas de token. O script deve ser uma função `JavaScript` que retorna um objeto de reivindicações personalizadas.

<img
  src="/img/assets/custom-jwt-detail-page.png"
  alt="Página de detalhes das reivindicações personalizadas de token"
/>
...
Você encontrará a definição de tipo detalhada do objeto payload do token e do objeto de dados do usuário no lado direito da página. O IntelliSense do editor também ajudará você a acessar corretamente essas propriedades do objeto de entrada.
...
- **Objeto de dados da organização**
  Para [tokens de acesso da organização](/authorization/organization-template) (um token de acesso solicitado para uma organização específica), o Logto fornece a organização alvo através de `context.organization`, para que você possa anexar reivindicações por organização — por exemplo, mapeando o ID da organização Logto para um ID interno armazenado nos dados personalizados da organização. Este objeto de dados só está presente quando o token é emitido para uma organização específica e contém:

  | Propriedade   | Descrição                              | Tipo                      |
  | ------------- | -------------------------------------- | ------------------------- |
  | `id`          | O id único da organização              | `string`                  |
  | `name`        | O nome da organização                  | `string`                  |
  | `description` | A descrição da organização             | `string \| null`          |
  | `customData`  | Os dados personalizados da organização | `Record<string, unknown>` |

  Consulte [Organizações](/organizations/organization-management#organization-data-structure) para mais detalhes.

- **Objeto de dados da concessão**
  Para token de acesso do usuário concedido por troca de token de imitação, o Logto fornece contexto adicional de dados da concessão para você acessar. O objeto de dados da concessão contém o contexto personalizado do token de origem. Consulte [Imitação de usuário (User impersonation)](/developers/user-impersonation) para mais detalhes.
- **Objeto de dados de interação do usuário**
  ...
  :::note
  Pode haver múltiplos registros de verificação no objeto de dados de interação do usuário, especialmente quando o usuário passou por múltiplos processos de login ou registro.
...
Use a seção **Definir variáveis de ambiente** à direita para configurar as variáveis de ambiente para seu script. Você pode usar essas variáveis para armazenar informações sensíveis ou dados de configuração que não deseja codificar diretamente no script, como chaves de API, segredos ou URLs.
...
A função `api.denyAccess()` permite negar o processo de emissão do token com uma mensagem personalizada. Você pode usar essa função para impor validações adicionais de acesso durante o processo de emissão do token.

## APIs de tempo de execução \{#runtime-apis}

Scripts personalizados de token de acesso podem usar estas APIs Web padrão tanto no Logto Cloud quanto no Logto auto-hospedado:

- `fetch`, `Request`, `Response` e `Headers`
- Web Crypto através de `crypto` e `crypto.subtle`
- `TextEncoder` e `TextDecoder`
- `URL` e `URLSearchParams`

Por exemplo, você pode calcular valores SHA-256 e HMAC-SHA-256 com Web Crypto:
...
Os scripts não podem importar pacotes. Evite globais e módulos específicos do Node.js, pois eles não são portáveis entre o Logto auto-hospedado e o Logto Cloud e não fazem parte do contrato de script suportado.

## Passo 3: Buscar dados externos \{#step-3-fetch-external-data}

Você pode usar a função padrão `fetch` para buscar dados externos em seu script. A função `fetch` é baseada em promessas e permite fazer requisições HTTP para APIs externas.
...
Certifique-se de testar seu script antes de salvá-lo. Clique na aba **Contexto de teste** no lado direito da página para modificar o payload do token simulado e o contexto de dados do usuário para teste.

Clique em **Executar teste** no canto superior direito do editor para rodar o script com os dados simulados. O resultado do script será exibido no painel **Resultado do teste**.
...
Clique no botão **Criar** para salvar o script. O script de reivindicações personalizadas será salvo e aplicado ao processo de emissão do token de acesso.
