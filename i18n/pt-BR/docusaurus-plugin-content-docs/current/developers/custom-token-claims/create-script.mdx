...
# Criar um script personalizado de token de acesso
...
2. Existem dois tipos diferentes de tokens de acesso para os quais você pode personalizar as reivindicações do token de acesso:
   ...
   - **Token de acesso do usuário (User access token)**: O token de acesso emitido para usuários finais. Ex.: para aplicativos Web ou aplicativos móveis.
   - **Token de acesso máquina para máquina (Machine-to-Machine access token)**: O token de acesso emitido para serviços ou aplicativos. Ex.: para [aplicativos máquina para máquina](/quick-starts/m2m).
...
- **Objeto de dados de interação do usuário**
  ...
  Ex.: o usuário fez login usando um registro de verificação `Social`, depois vinculou um novo endereço de e-mail através de um registro de verificação `EmailVerificationCode` e, em seguida, verificou o status MFA com um registro de verificação `Totp`. Nesse caso, você pode precisar lidar com todos os registros de verificação adequadamente em seu script.
...
Certifique-se de testar seu script antes de salvá-lo. Clique na guia **Contexto de teste** no lado direito da página para modificar o payload de token simulado e o contexto de dados do usuário para teste.
