---
id: create-script
title: Criar um script personalizado de token de acesso
sidebar_label: Criar um script personalizado de token de acesso
sidebar_position: 3
---

# Criar um script personalizado de token de acesso (access token)
...
O objeto de contexto contém os dados do usuário e dados da concessão relevantes para o processo atual de autorização (Authorization).
...
- **Objeto de dados da organização**
  Para [tokens de acesso da organização](/authorization/organization-template) (um token de acesso solicitado para uma organização específica), o Logto fornece a organização alvo através de `context.organization`, para que você possa anexar reivindicações por organização — por exemplo, mapeando o ID da organização do Logto para um ID interno armazenado nos dados personalizados da organização. Este objeto de dados só está presente quando o token é emitido para uma organização específica e contém:
...
- **Objeto de dados de interação do usuário**
  Para um determinado token de acesso do usuário, pode haver situações em que você precise acessar os detalhes de interação do usuário para a sessão de autorização (Authorization) atual. Por exemplo, você pode precisar recuperar a identidade SSO corporativa do usuário usada para login. Este objeto de dados de interação do usuário contém os dados de interação mais recentes enviados pelo usuário, incluindo:
  ...
  Ex.: o usuário fez login usando um registro de verificação `Social`, depois vinculou um novo endereço de e-mail através de um registro de verificação `EmailVerificationCode` e, em seguida, verificou o status de MFA com um registro de verificação `Totp`. Nesse caso, você pode precisar tratar todos os registros de verificação adequadamente em seu script.
...
<img src="/img/assets/test-custom-jwt-script.png" alt="Testar script personalizado de JWT" />
