...
1. Navegue para <CloudLink to="/customize-jwt">Console > Custom JWT</CloudLink>.
...
![Testar script de JWT personalizado](../assets/test-custom-jwt-script.png)

:::note
O resultado do teste é a saída da função `getCustomJwtClaims` com os dados simulados que você definiu ("reivindicações de token extras" obtidas após completar o passo 3 no [diagrama de sequência](/developers/custom-token-claims/#how-do-custom-token-claims-work)). A carga útil real do token e o contexto de dados do usuário serão diferentes quando o script for executado no processo de emissão de tokens.
:::
