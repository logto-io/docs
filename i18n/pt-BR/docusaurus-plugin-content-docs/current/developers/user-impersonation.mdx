...
### Etapa 1: Solicitando imitação

Primeiro, o aplicativo de suporte de Sarah precisa solicitar a imitação ao servidor backend da TechCorp.
...
### Etapa 2: Obtendo um token de sujeito
...
### Etapa 3: Trocando o token de sujeito por um token de acesso
...
Aqui está como Sarah pode usar isso em um aplicativo de suporte Node.js:
...
2. O token de acesso de imitação não vem com um token de atualização. Sarah precisará repetir esse processo se o token expirar antes de resolver o problema de Alex.
...
Para incluir a reivindicação `act`, o aplicativo de Sarah precisa fornecer um `actor_token` na solicitação de troca de token. Esse token deve ser um token de acesso válido para Sarah com o escopo `openid`. Veja como incluí-lo na solicitação de troca de token:
...
Logto permite que você personalize as reivindicações de token para tokens de imitação. Isso pode ser útil para adicionar contexto ou metadados adicionais ao processo de imitação, como o motivo da imitação ou o ticket de suporte associado.
...
Esse contexto pode então ser usado em uma função `getCustomJwtClaims()` para adicionar reivindicações específicas ao token de acesso final. Aqui está um exemplo de como isso pode ser implementado:
