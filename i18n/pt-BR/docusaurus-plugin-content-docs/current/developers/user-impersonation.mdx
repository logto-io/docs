...
### Etapa 1: Solicitando a imitação \{#step-1-requesting-impersonation}
...
Após validar a solicitação da Sarah, o servidor da TechCorp irá chamar a [Management API](/integrate-logto/interact-with-management-api) do Logto para obter um token de sujeito.
...
Para aplicativos web tradicionais ou aplicativos máquina para máquina com segredo do app, inclua as credenciais no cabeçalho `Authorization`:
...
Para single-page applications (SPA) ou aplicativos nativos sem segredo do app, inclua o `client_id` no corpo da requisição:
...
async function impersonateUser(
  ...
  try {
    ...
    if (!impersonationResponse.ok) {
      throw new Error(`Ocorreu um erro HTTP. Status: ${impersonationResponse.status}`);
    }
    ...
    // highlight-start
    // Para apps web tradicionais ou M2M, use autenticação Basic com segredo do cliente
    // Para SPA ou apps nativos, inclua client_id no corpo da requisição
    ...
    if (clientSecret) {
      // Cliente confidencial: use autenticação Basic
      headers['Authorization'] =
        `Basic ${Buffer.from(`${clientId}:${clientSecret}`).toString('base64')}`;
    } else {
      // Cliente público: inclua client_id no corpo
...
async function performImpersonation(): Promise<void> {
  try {
    // highlight-start
    // Para apps web tradicionais ou M2M, passe o segredo do cliente
    const accessToken = await impersonateUser(
      'alex123',
      'techcorp_support_app',
      'TECH-1234',
      'https://api.techcorp.com/customer-data',
      'your-client-secret' // Omitir para SPA ou apps nativos
    );
    // highlight-end
...
3. O servidor backend da TechCorp deve implementar verificações de autorização adequadas para garantir que apenas membros autorizados da equipe de suporte, como Sarah, possam solicitar imitação.
...
:::note
Tenha cautela ao adicionar reivindicações personalizadas aos seus tokens. Evite incluir informações sensíveis que possam representar riscos de segurança caso o token seja interceptado ou vazado. Os JWTs são assinados, mas não criptografados, então as reivindicações são visíveis para qualquer pessoa com acesso ao token.
:::
