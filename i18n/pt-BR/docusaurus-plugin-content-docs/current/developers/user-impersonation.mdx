...
sequenceDiagram
    ...
    Note over TechCorp,Logto: Solicitação de token de sujeito para Alex
...
O servidor da TechCorp, após validar a solicitação de Sarah, chamará a [Management API](/integrate-logto/interact-with-management-api) do Logto para obter um token de sujeito.

**Solicitação (servidor da TechCorp para a Management API do Logto)**
...
2. O token de acesso de imitação não vem com um [token de atualização](https://auth.wiki/refresh-token). Sarah precisará repetir esse processo se o token expirar antes de resolver o problema de Alex.
...
Este [contexto](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) pode então ser usado em uma função `getCustomJwtClaims()` para adicionar reivindicações específicas ao token de acesso final. Veja um exemplo de como isso pode ser implementado:
