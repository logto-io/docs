...
sequenceDiagram
    ...
    Note over Sarah,LogtoToken: Troca o token de sujeito por token de acesso
...
O `access_token` retornado será vinculado ao recurso especificado, garantindo que só possa ser usado com a API de dados do cliente da TechCorp.
...
Para SPA ou aplicativos nativos, inclua `client_id` no corpo da requisição:
