...
1. Acesse <CloudLink to="/api-resources">Console → Recursos de API</CloudLink>.
...
1. Acesse <CloudLink to="/roles">Console → Papéis</CloudLink>.
...
Os JWTs emitidos pelo Logto contêm reivindicações que sua API pode usar para aplicar a autorização.
...
- Garantir que o `aud` (público) corresponda ao identificador do recurso de API que você registrou (por exemplo, `https://api.yourapp.com`).
