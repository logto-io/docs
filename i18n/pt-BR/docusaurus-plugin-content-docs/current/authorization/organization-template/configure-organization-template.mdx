...
- [Logto Management API - papel da organização](https://openapi.logto.io/operation/operation-updateorganizationrole).
- [Logto Management API - permissão da organização](https://openapi.logto.io/operation/operation-listorganizationscopes).
...
Chame periodicamente os endpoints do [Logto Management API](/integrate-logto/interact-with-management-api/) ou estabeleça uma conexão de longa duração (por exemplo, usando WebSocket) para buscar dinamicamente as permissões da organização do usuário. Ao detectar mudanças, limpe o token de organização existente e os tokens recém-emitidos refletirão automaticamente as mudanças no escopo de permissão da organização.
...
Quando mudanças de permissão forem detectadas, limpe o token de organização do armazenamento primeiro e, em seguida, chame o método SDK `getOrganizationToken(organizationId)` para adquirir um novo. O token de organização recém-emitido deve refletir as mudanças de permissão.
