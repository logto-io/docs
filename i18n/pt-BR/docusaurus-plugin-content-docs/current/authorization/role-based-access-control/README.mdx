...
Se nenhum Recurso de API for fornecido, a permissão será tratada como "para OIDC". Geralmente, isso não é o que você deseja em RBAC.
...
É aceitável atribuir tanto `order_admin` quanto `product_admin` a um usuário, então ele terá todas as seis permissões que você acabou de definir.

Note que o administrador de pedidos compartilha a permissão `read:product` com o administrador de produtos, e as permissões finais que um usuário possui são a união de todas as permissões dos papéis que lhe foram atribuídos.
