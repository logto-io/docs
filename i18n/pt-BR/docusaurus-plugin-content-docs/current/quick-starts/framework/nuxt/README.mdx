...
sidebar_custom_props:
  description: Nuxt é um framework open source que torna o desenvolvimento web intuitivo e poderoso.
language: js
...
# Adicionar autenticação ao seu aplicativo Nuxt 3 (Add authentication to your Nuxt 3 application)
...
Os composables `useLogtoClient()` e `useLogtoUser()` não estão disponíveis em middleware ou rotas de API. Você pode usar a função `logtoEventHandler()` para obter o cliente Logto e outros contextos:
