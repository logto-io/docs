---
slug: /quick-starts/expo
sidebar_label: Expo (React Native)
sidebar_custom_props:
  darkLogoFilename: 'expo-dark.svg'
  description: Criar aplicativos nativos universais com React que funcionam no Android, iOS e na web.
---

import FurtherReadings from '../../fragments/_further-readings.md';

import GuideTip from './_guide-tip.md';
import Installation from './_installation.mdx';
import Integration from './_integration.mdx';
import ApiResources from './api-resources/_index.mdx';
import GetUserInformation from './get-user-information/_index.mdx';

# Adicionar autenticação ao seu aplicativo Expo (React Native)

<GuideTip />

## Pré-requisitos

- Uma conta [Logto Cloud](https://cloud.logto.io) ou um [Logto auto-hospedado](/introduction/set-up-logto-oss).
- Um aplicativo nativo Logto criado.

## Instalação

<Installation />

## Integração

<Integration />

## Obter informações do usuário

<GetUserInformation />

## Recursos de API e organizações

<ApiResources />

## Solução de problemas

### Incapaz de resolver `@logto/client/shim`

Para projetos Expo, se você encontrar o erro `Unable to resolve "@logto/client/shim" from "node_modules/@logto/rn/lib/index.js"`, você pode resolvê-lo adicionando o seguinte ao seu arquivo `metro.config.js`:

```js title="metro.config.js"
const config = {
  // ...
  resolver: {
    unstable_enablePackageExports: true,
  },
};

module.exports = config;
```

Este erro indica que o pacote `@logto/rn` não está conseguindo resolver o módulo `@logto/client/shim`.

Como as exportações de nó foram usadas no pacote `@logto/client`, e as exportações de pacotes não são habilitadas por padrão no Metro bundler, você precisa habilitá-las manualmente.

Veja o [suporte a exportações de pacotes do React Native](https://reactnative.dev/blog/2023/06/21/package-exports-support#enabling-package-exports-beta) para mais detalhes.

## Leituras adicionais

<FurtherReadings />
