...
![Tabela de posts](./posts-table.webp)

O campo `user_id` na tabela representa o usuário a quem cada dado de post pertence. Você pode restringir cada usuário a acessar apenas seus próprios dados de post com base no campo `user_id`.
...
![Página de configurações da API do Supabase](./supabase-api-settings.webp)
...
![Criar função de obtenção de ID do usuário](./create-get-user-id-function.webp)
...
3. Na janela solicitada, clique em "Create policy".
...
![Criar política de RLS](./create-rls-policy.webp)
...
Tanto aplicativos nativos quanto SPAs são executados no seu dispositivo, e as credenciais (token de acesso) obtidas após o login são armazenadas localmente no seu dispositivo.

Portanto, ao integrar seu aplicativo com Supabase, você precisa interagir com o Supabase através do seu serviço de backend, pois não pode expor informações sensíveis (como o segredo JWT do Supabase) no dispositivo de cada usuário.

Vamos supor que você está construindo seu SPA usando React e Express. Você integrou com sucesso o Logto ao seu aplicativo seguindo o [Guia do Logto React SDK](/quick-starts/react) (você pode consultar o código em nosso [exemplo de react](https://github.com/logto-io/js/tree/master/packages/react-sample)). Além disso, você adicionou validação do token de acesso do Logto ao seu servidor backend de acordo com a documentação [Proteja sua API no Node (Express)](/authorization/api-resources/node-express).
...
### Aplicativo máquina para máquina
...
![Segredo do papel de serviço](./service-role-secret.webp)
