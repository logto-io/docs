...
5. Adicione assinatura e modelo seguindo as diretrizes e forneça os materiais ou informações necessários para análise.
   ...
   - Além disso, use `{{code}}` como um placeholder onde deseja inserir seu código de verificação digital no conteúdo do modelo.
6. Após enviar sua solicitação de assinatura e modelo SMS, será necessário aguardar a aprovação. Neste momento, podemos voltar à [página do console do serviço SMS](https://dysms.console.aliyun.com/overview) e enviar um SMS de teste. Se suas assinaturas e modelos estiverem prontos para uso, você pode testá-los diretamente; se ainda não estiverem ativos, a Aliyun também fornece modelos de teste.
   - Pode ser necessário recarregar um pequeno valor antes de enviar mensagens de teste.
   - Você também pode ser solicitado a vincular um número de telefone de teste antes de enviar mensagens de teste. Para mais detalhes, acesse a aba "Aprendizado Rápido" (快速学习) na barra lateral da [página do console do serviço SMS](https://dysms.console.aliyun.com/overview).
...
2. Vá para a aba "China Continental" (国内消息) ou "Fora da China Continental" (国际/港澳台消息) que você acabou de acessar, onde encontrará facilmente "Assinatura" (签名名称) e "Código do Modelo" (模板 CODE).
   - Se quiser usar a assinatura e o modelo apenas para teste, acesse a aba "Aprendizado Rápido" (快速学习), onde encontrará ambos em "Assinatura & Modelos (Apenas para Teste)".
3. Preencha as configurações do Aliyun SMS Connector:
   ...
   - Preencha o campo `signName` com a "Assinatura" (签名名称) mencionada no passo 2. Todos os modelos compartilharão esse nome de assinatura.
...
4. Configurar "Verificação Estrita do Código de Região do Telefone":
   ...
   - **Atenção**: Se seus usuários existentes têm números de telefone sem códigos de região, ativar isso pode afetar o processo de login deles. Ative apenas se necessário para envio de SMS internacional.
