...
Estamos agora oferecendo guias sobre como usar o conector SMTP para enviar emails seguindo provedores para sua melhor compreensão:
...
Esperamos que você consiga configurar todos os outros fornecedores de email com os exemplos a seguir :rocket:

### Configurar para uso do Gmail \{#set-up-for-gmail-use}
...
### Integrar com a API SMTP do SendGrid \{#integrate-with-sendgrid-smtp-api}

Inicialmente, assumimos que você já tem uma conta SendGrid. Caso contrário, crie uma nova conta no [site do SendGrid](https://app.sendgrid.com/).
...
Os desenvolvedores podem acessar os detalhes do _remetente_ em ["Gerenciamento de Remetentes"](https://mc.sendgrid.com/senders).

### Configurar com conta de email direto do Aliyun \{#configure-with-aliyun-direct-mail-account}

Faça login no [site do Aliyun](https://cn.aliyun.com/). Registre uma nova conta se você não tiver uma.
...
Você pode ir para a [página de endereço do serviço SMTP](https://www.alibabacloud.com/help/en/directmail/latest/smtp-service-address) para escolher um host de endereço de serviço SMTP adequado e número de porta.
...
Você deve alterar os valores envolvidos com "\<" e ">" de acordo com as configurações da sua conta do Gmail, SendGrid ou Aliyun e escolher manter outros campos sem "\<" e ">".
...
**Opções de Autenticação por Nome de Usuário e Senha**
