...
Acesse [Aliyun](https://aliyun.com/) e crie sua conta Aliyun caso ainda não tenha uma.
...
1. Acesse "Email Domains" (发信域名) na barra lateral e adicione um "New Domain" (新建域名) seguindo as instruções.
2. Personalize "Sender Addresses" (发信地址) e "Email Tags" (邮件标签) respectivamente.

Após concluir a configuração, há duas maneiras diferentes de testar:

- Vá para a [página de visão geral do DirectMail](https://dm.console.aliyun.com/), encontre "Operation Guide" (操作引导) na parte inferior da página e clique em "Send Emails" (发送邮件). Você encontrará todos os diferentes métodos de teste.
...
2. Vá para a aba "Sender Addresses" (发信地址) ou "Email Tags" (邮件标签) que você acabou de visitar na [página do console de administração do DM](https://dm.console.aliyun.com/), onde você pode encontrar facilmente o _Sender Address_ ou _Email Tag_.
3. Preencha as configurações do Aliyun DM Connector:
   ...
   - Preencha os campos `accountName` e `fromAlias` com o "Sender Address" e "Email Tag" encontrados no passo 2. Todos os templates compartilharão este nome de assinatura. (Você pode deixar `fromAlias` em branco, pois é OPCIONAL.)
