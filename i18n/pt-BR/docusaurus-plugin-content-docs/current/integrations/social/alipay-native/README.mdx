...
# Configurar login social com Alipay (Native)

O conector oficial do Logto para login social com Alipay em aplicativos nativos para dispositivos móveis.

<GuideTip />

## Introdução \{#get-started}

O conector Alipay Native funciona em estreita colaboração com o Logto SDK em plataformas móveis. Ele aproveita o fluxo de autenticação OAuth 2.0 do Alipay e permite que os usuários do Alipay façam login em outros aplicativos usando perfis públicos de usuários do Alipay sem passar por um processo de registro complicado.
...
4. Nomeie seu aplicativo em "Application Name" (应用名称) seguindo as convenções de nomenclatura e faça o upload do seu "Application Icon" (应用图标), certifique-se de escolher "mobile application" (移动应用) como "App type" (应用类型). Para construir um aplicativo iOS, é necessário um "Bundle ID" único. Além disso, "application signature" (应用签名) e "application package name" (应用包名) são necessários para aplicativos Android.
5. Após terminar de criar o aplicativo, chegamos à página de Visão Geral, onde devemos clicar em "add ability" (添加能力) para adicionar "Third-party application authorization" (第三方应用授权), "Get member information" (获取会员信息) e "App Alipay login" (App 支付宝登录) antes de habilitar o login com Alipay.
6. Vá para o [Centro de Atendimento ao Cliente Alipay](https://b.alipay.com/index2.htm) e faça login com a conta de desenvolvedor Alipay. Clique em "Account Center" (账号中心) na barra superior e vá para "APPID binding" (APPID 绑定), cuja entrada pode ser encontrada na parte inferior da barra lateral. "Add binding" (添加绑定) digitando o APPID do aplicativo móvel que você acabou de criar no passo 4.
...
10. Após concluir todas essas etapas, volte para o canto superior direito da página do console do Alipay e clique em "Submit for review" (提交审核). Uma vez que a revisão for aprovada, você estará pronto para um fluxo de login suave com Alipay.
...
## Configurar as configurações do conector Logto Alipay Native \{#set-up-the-logto-alipay-native-connector-settings}
...
## Habilitar login nativo com Alipay no seu aplicativo \{#enable-alipay-native-sign-in-in-your-app}
...
<img src={addFrameworkImage} alt="Adicionar framework" />

:::note

O plugin inclui o "minimalist SDK" do Alipay (极简版 SDK). Você pode usar diretamente `import AFServiceSDK` uma vez importado o plugin.
