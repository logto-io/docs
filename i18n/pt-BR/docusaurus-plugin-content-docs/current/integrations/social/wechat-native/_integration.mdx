...
Você pode configurar uma ou ambas as plataformas iOS e Android para integrar Logto com o login nativo do WeChat.
...
O WeChat requer um link universal para login nativo. Se você ainda não configurou ou não sabe o que é, consulte o [documento oficial da Apple](https://developer.apple.com/ios/universal-links/).
...
Assumimos que você integrou o [Logto iOS SDK](/quick-starts/swift) no seu aplicativo. Nesse caso, as coisas são bem simples, e você nem precisa ler o documento do WeChat SDK:
...
**4. Lide com `onOpenURL` corretamente**

:::note

A função `LogtoClient.handle(url:)` lidará com todos os conectores nativos que você ativou. Você só precisa chamá-la uma vez.
...
Assumimos que você integrou o [Logto Android SDK](/quick-starts/android) no seu aplicativo. Nesse caso, as coisas são bem simples, e você nem precisa ler o documento do WeChat SDK:
...
Crie um pacote `wxapi` sob o seu pacote raiz e adicione o `WXEntryActivity` no pacote `wxapi` (Tome `com.sample.app` como exemplo):
