## Entrar com conta do GitLab \{#sign-in-with-gitlab-account}

Acesse o [site do GitLab](https://gitlab.com/) e faça login com sua conta do GitLab. Você pode registrar uma nova conta se ainda não tiver uma.
...
Dê um nome ao seu novo aplicativo OAuth em **Name** e preencha o **Redirect URI** do aplicativo. Personalize os **Redirect URIs** como `${your_logto_origin}/callback/${connector_id}`. O `connector_id` pode ser encontrado na barra superior da página de detalhes do conector no Logto Admin Console.

Nos escopos, selecione `openid`. Você também pode querer habilitar `profile` e `email`. O escopo `profile` é necessário para obter as informações do perfil do usuário, e o escopo `email` é necessário para obter o endereço de e-mail do usuário. Certifique-se de que você permitiu esses escopos em seu aplicativo OAuth do GitLab se quiser usá-los. Esses escopos também serão usados ao configurar seu conector posteriormente.
...
`scope` é uma lista de [escopos](https://docs.gitlab.com/ee/integration/oauth_provider.html#authorized-applications) separados por espaço. Se não for fornecido, o escopo padrão será `openid`. Para o conector GitLab, os escopos que você pode querer usar são `openid`, `profile` e `email`. O escopo `profile` é necessário para obter as informações do perfil do usuário, e o escopo `email` é necessário para obter o endereço de e-mail do usuário. Certifique-se de que você permitiu esses escopos em seu aplicativo OAuth do GitLab (configurado na seção [Criar e configurar aplicativo OAuth](#create-and-configure-oauth-app)).
...
## Testar conector GitLab \{#test-gitlab-connector}

É isso. O conector GitLab já deve estar disponível. Não se esqueça de [Habilitar o conector no cadastro e login](https://docs.logto.io/docs/recipes/configure-connectors/social-connector/enable-social-sign-in/).
