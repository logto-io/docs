...
Cada provedor de identidade social pode ter sua própria variação no protocolo padrão OIDC. Se o seu provedor de identidade social desejado seguir estritamente o protocolo padrão OIDC, então você não precisa se preocupar com o `customConfig`.
...
### Habilitar botão de login social \{#enable-social-sign-in-button}
...
Saiba mais sobre [experiência de login social](/end-user-flows/sign-up-and-sign-in/social-sign-in).

### Vincular ou desvincular uma conta social \{#link-or-unlink-a-social-account}

Use a Account API para construir um Centro de Conta personalizado em seu aplicativo que permita aos usuários autenticados vincular ou desvincular suas contas sociais. [Siga o tutorial da Account API](/end-user-flows/account-settings/by-account-api#link-a-new-social-connection)
