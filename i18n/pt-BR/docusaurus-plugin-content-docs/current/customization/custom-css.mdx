...
Se nossa interface de ajuste fino não estiver totalmente alinhada com sua marca, use CSS Personalizado para adaptar sua experiência de login. Basta adicionar seu código CSS ao nosso editor e ver as mudanças instantaneamente.

## CSS Personalizado no Console do Logto

1. Vá para <CloudLink to="/sign-in-experience/branding">Console > Experiência de login > Branding > CSS Personalizado</CloudLink>.
2. Edite o código CSS no editor à esquerda, ele será renderizado ao vivo no modal de visualização à direita imediatamente. Nota: o editor de código suporta apenas código CSS até agora. Não HTML ou Javascript.
3. Salve as alterações.
4. Não se esqueça de clicar no botão "Visualização ao Vivo" para verificar as alterações em todas as páginas.
...
![Caso de uso de CSS personalizado do Logto](./assets/logto-custom-css-use-case.png)

:::note

Como o Logto usa CSS Modules, você pode ver um valor hash na propriedade `class` dos elementos DOM (por exemplo, um `<div>` com `vUugRG_container`). Para substituir esses valores, você pode usar o seletor CSS `$=` para corresponder a elementos que terminam com um valor especificado. Nesse caso, deve ser `div[class$=container]`.

:::

## CSS Personalizado pela Management API

Você também pode usar a Management API `PATCH /api/sign-in-exp` com o corpo `{ "customCss": "string arbitrária" }` para definir CSS personalizado para a experiência de login. Você deve ver o valor de `customCss` anexado após `<title>` da página. Se o estilo tiver uma prioridade mais alta, ele deve ser capaz de substituir.
