...
Todo o projeto Logto é um [monorepo pnpm](https://pnpm.io/workspaces). Se você quiser rodar o pacote experience isoladamente (sem pnpm), substitua as dependências `workspace:` no package.json por números de versão específicos (como `^1.0.0`).
...
### Requisitos dos ativos de UI personalizada \{#custom-ui-assets-requirements}
...
Sua UI personalizada precisa interagir com a [Experience API](https://openapi.logto.io/group/endpoint-experience) para realizar várias ações como login, cadastro, redefinição de senha, vinculação de contas sociais, habilitação de MFA, entre outras. Para entender melhor os fluxos de usuário e detalhes de implementação, recomendamos consultar nosso [RFC de design da Experience API](https://github.com/logto-io/rfcs/blob/master/draft/0004-experience-api.md), que fornece especificações técnicas abrangentes e exemplos.

Você também precisará acessar outras configurações da Experiência de Login, como cores de branding, logotipo da empresa, favicon, política de senha, frases de idioma localizadas e até CSS personalizado, através do [endpoint da Experience API de login](https://openapi.logto.io/operation/operation-getsigninexperienceconfig).

### Lide com CAPTCHA quando habilitado \{#handle-captcha-when-enabled}
...
- [reCAPTCHA Enterprise com Traga sua UI](/security/captcha/recaptcha-enterprise#bring-your-ui)
- [Cloudflare Turnstile com Traga sua UI](/security/captcha/turnstile#bring-your-ui)

### Projetos de exemplo \{#sample-projects}

Disponibilizamos uma coleção de projetos de exemplo para ajudá-lo a entender e começar a implementar rapidamente uma UI personalizada. Confira o repositório [Logto Experience project](https://github.com/logto-io/experience-samples) no GitHub para mais detalhes.
