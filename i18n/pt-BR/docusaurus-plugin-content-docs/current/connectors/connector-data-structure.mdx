...
_Connectors_ desempenham um papel crítico no Logto. Com a ajuda deles, o Logto permite que os usuários finais usem registro ou login sem senha e as capacidades de login com contas sociais. Com a crescente popularidade de sites e aplicativos, logins sem senha e sociais permitem que os usuários evitem gerenciar inúmeras contas e senhas.
...
## Armazenamento local do conector: _ConnectorMetadata_

### id

_id_ é uma chave única do tipo string para identificar um conector no Logto.
...
_logo_ é uma URL ou caminho relativo do logotipo do conector.

### logoDark

_logoDark_ é uma URL ou caminho relativo do logotipo do conector em modo escuro.
...
:::note
Logto só suporta conectores sociais "padrão". Ou seja, todos os conectores de Email ou SMS do Logto NÃO são "padrão".
...
## Armazenamento remoto do conector: _Connector DB_

### id

_id_, que funciona como a chave primária do DB do conector, é uma chave do tipo string gerada aleatoriamente para identificar o conector no DB.
...
_metadata_ é um subconjunto de [ConnectorMetadata](#connectors-local-storage-connectormetadata), que contém atributos configuráveis, ou seja, [_logo_](#logo), [_logoDark_](#logodark), [_target_](#target) e [_name_](#name).

### syncProfile

_syncProfile_ é um valor booleano para determinar o esquema de atualização do perfil do usuário, padrão para FALSE.

Se _syncProfile_ for FALSE, as informações básicas do usuário do Logto (incluindo nome e avatar) serão atualizadas apenas quando o usuário se inscrever pela primeira vez no Logto através deste conector. Caso contrário, toda vez que os usuários fizerem login no Logto através do conector, o perfil da conta Logto será atualizado.

### config

_config_ pode ser um objeto não vazio arbitrário.
...
:::note
Na versão atual do Logto, apenas um conector de _Email/SMS_ pode existir ao mesmo tempo, todos os outros conectores do mesmo tipo são automaticamente excluídos.
