...
| MfaVerification          | Quando [email MFA](/end-user-flows/mfa/email-mfa) está habilitado, este modelo é usado para enviar códigos de verificação aos usuários durante o processo de autenticação multifatorial.                                                                                                                                                                                                                                                                                                              | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| BindMfa                  | Quando [email MFA](/end-user-flows/mfa/email-mfa) está habilitado, este modelo é usado para configurar o código de verificação por email para MFA. Os usuários recebem este código ao vincular ou configurar o email como fator MFA na conta.                                                                                                                                                                                                                                                         | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
...
O código de verificação que os usuários precisam inserir para concluir o processo de verificação. Disponível nos modelos `SignIn`, `Register`, `ForgotPassword`, `Generic`, `UserPermissionValidation` e `BindNewIdentifier`.
    ...
    - Um espaço reservado `{{code}}` precisa ser mantido no modelo. Ao enviar um código de verificação, um código gerado aleatoriamente substituirá este espaço reservado antes do envio do email ao usuário.
...
- Se a variável raiz `application` não for fornecida, o espaço reservado handlebars será ignorado e não será substituído.
- Se o objeto `application` fornecido não contiver os campos necessários ou o valor estiver indefinido, o espaço reservado handlebars será substituído por uma string vazia. Ex: `{{application.foo.bar}}` será substituído por ``.
      ...
      <div style="padding: 0 24px;">
        <center>
          ...
          <p style="color: #747778;">
            Se você não tentou fazer login mas recebeu este email, apenas ignore-o. O código
...
| contentType | Alguns provedores de email podem renderizar modelos de email de forma diferente com base no tipo de conteúdo. (ex: Sendgrid, Mailgun). Use este campo para especificar o tipo do modelo. |
...
Depois que os modelos de email forem criados, o Logto selecionará automaticamente o modelo de email apropriado, primeiro resolvendo a preferência de idioma do usuário e, em seguida, escolhendo o modelo mais adequado.
...
Isso permite que você gerencie a lógica dos modelos de email em seu próprio servidor.
