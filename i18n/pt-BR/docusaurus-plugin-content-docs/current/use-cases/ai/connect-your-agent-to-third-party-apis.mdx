...
sequenceDiagram
    ...
    rect rgba(200, 230, 200, 0.5)
        ...
        User->>Google: Autentica e concede permissões
        Google->>Logto: Retorna código de autorização
        Logto->>Google: Troca código por tokens
        Logto->>Logto: Armazena tokens no Cofre de Segredos
        Logto->>Agent: Retorna resultado da verificação
    end

    rect rgba(200, 200, 230, 0.5)
        Note over User, Google: Execução da tarefa pelo agente de IA
        Agent->>Logto: Recupera token de acesso do Google
        Logto->>Agent: Retorna token de acesso
        Agent->>Google: Chama API do Google Calendar
        Google->>Agent: Retorna sucesso
...
Quando seu agente de IA precisar acessar uma API de terceiros (por exemplo, Google Calendar), ele deve primeiro verificar se o usuário já autorizou o acesso. Caso contrário, solicite autorização ao usuário.
...
- **Melhor experiência do usuário**: Os usuários têm mais chances de conceder permissões quando entendem o motivo no contexto.
- **Taxas de conversão mais altas**: Menos permissões iniciais significam menos atrito.
- **Construção de confiança**: Usuários confiam em aplicativos que pedem apenas o necessário.
...
O Console do Logto oferece visibilidade sobre o status dos tokens de cada usuário:
...
- **Trate expiração de tokens de forma adequada**: Sempre trate casos em que tokens possam estar expirados ou revogados.
...
- **Audite o acesso à API**: Registre quando seu agente de IA acessa APIs de terceiros para troubleshooting e conformidade.
