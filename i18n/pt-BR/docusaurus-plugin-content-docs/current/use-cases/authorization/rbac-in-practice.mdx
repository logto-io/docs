...
description: 'Um guia prático para controle de acesso baseado em papel (RBAC): Aprenda como projetar permissões, gerenciar papéis e configurar autorização segura em seu CMS.'
---
...
- ✨ Como projetar e implementar permissões granulares que oferecem controle preciso
...
- O sistema verifica permissões de papel OU propriedade ao lidar com operações de artigos
...
Essa abordagem de duas camadas (papéis + propriedade) cria um sistema mais intuitivo e seguro. Editores e administradores ainda podem gerenciar todo o conteúdo através de suas permissões de papel, enquanto autores mantêm o controle sobre seu próprio trabalho.

## Projetando APIs seguras \{#designing-a-secure-apis}

Vamos começar projetando a funcionalidade principal do nosso CMS através de seus endpoints de API:
...
| Permissão/Papel  | 👑 Admin                                            | 📝 Publisher                                     | ✍️ Author                           |
| ---------------- | --------------------------------------------------- | ------------------------------------------------ | ----------------------------------- |
...
Cada papel é projetado com responsabilidades específicas em mente:

- **Admin**: Tem controle total sobre o CMS, incluindo todas as operações de artigos
- **Publisher**: Foca na revisão de conteúdo e gerenciamento de publicação
- **Author**: Especializa-se na criação de conteúdo

Essa estrutura de papéis cria uma separação clara de responsabilidades:

- Autores focam na criação de conteúdo
- Publishers gerenciam a qualidade e visibilidade do conteúdo
- Admins mantêm o controle geral do sistema

## Configurando RBAC no Logto \{#config-rbac-in-logto}

Antes de começar, você precisa criar uma conta no [Logto Cloud](https://cloud.logto.io), ou você também pode usar uma instância Logto auto-hospedada utilizando a [versão Logto OSS](/logto-oss).
...
:::note
Para fins de demonstração, criamos esses recursos e configurações através do Logto Console. Em projetos reais, você pode criar esses recursos e configurações programaticamente usando a [Management API](/integrate-logto/interact-with-management-api) fornecida pelo Logto.
:::
...
const logtoConfig: LogtoConfig = {
  ...
  resources: [API_RESOURCE], // O identificador do recurso de API que você criou no Logto Console, ex: https://api.cms.com
...
Primeiro, precisamos adicionar um middleware no backend para verificar as permissões do usuário, verificar se o usuário está autenticado e determinar se ele tem as permissões necessárias para acessar certas APIs.
...
Como você pode ver, neste middleware, verificamos se a requisição do frontend contém um token de acesso válido e se o público do token de acesso corresponde ao recurso de API que criamos no Logto Console.

O motivo de verificar o recurso de API é que nosso recurso de API realmente representa os recursos do backend do nosso CMS, e todas as permissões do nosso CMS estão associadas a esse recurso de API.

Como esse recurso de API representa os recursos do CMS no Logto, em nosso código frontend, incluímos o respectivo Access token ao fazer requisições de API para o backend:
...
:::note
Se você perceber que não consegue fazer login com as credenciais dos usuários criados em "User Management", será necessário habilitar primeiro o método de login apropriado. Vá em "Sign-in Experience" no Logto Console e habilite seu método de autenticação preferido (como Email + Senha ou Nome de usuário + Senha).
:::
...
Charles, com o papel de Author, só pode criar seus próprios artigos e só pode visualizar, atualizar e excluir artigos que possui.

![Painel do CMS - Charles - Lista de artigos](https://uploads.strapi.logto.io/cms_dashboard_charles_05f610066a.png)

Bob, com o papel de Publisher, pode visualizar e publicar todos os artigos, mas não pode criar, atualizar ou excluir.
...
Para cenários mais complexos, como construir aplicativos multi-inquilino, o Logto oferece suporte abrangente a organizações. Confira nosso guia [Construa um aplicativo SaaS multi-inquilino: Um guia completo do design à implementação](https://blog.logto.io/build-multi-tenant-saas) para saber mais sobre como implementar controle de acesso em toda a organização.
