...
Vou mostrar exatamente como projetar e implementar um sistema assim do zero — e você vai se surpreender com a simplicidade em 2025 com ferramentas modernas e a abordagem arquitetural certa.
...
Para habilitar uma arquitetura multi-inquilino, você precisará de uma camada de entidade chamada **organização (Organization)**. Imagine ter um único pool de usuários que podem acessar múltiplos espaços de trabalho. Cada organização representa um espaço de trabalho, e os usuários mantêm uma única identidade ao acessar diferentes espaços de trabalho (organizações) com base nos papéis atribuídos.
...
3. **Gerenciamento de conta**: Um centro de conta onde os usuários podem atualizar email, senha e outros detalhes.
4. **Login social**: Opções como Google e GitHub para login rápido.
5. **Autenticação multifatorial (MFA)**: Aumente a segurança permitindo login via aplicativos autenticadores como o Duo.
...
| --------------------------------------------------- | ------------------------------------------------------------------- |
| Nova conta                                          | Entrar pela página de login e cadastro para criar um novo inquilino |
...
### Habilitar controle de acesso em app multi-inquilino \{#enable-access-control-in-multi-tenant-app}
...
1. **Definições unificadas de permissões**: Permissões são definidas no nível do sistema e aplicadas de forma consistente em todas as organizações, garantindo um gerenciamento de permissões sustentável e consistente.
...
Este é o passo mais fácil. Só precisamos integrar o Logto ao nosso projeto. Depois podemos configurar os métodos de login/cadastro de usuários no Logto Console conforme nossa necessidade.

### Instale Logto no seu app \{#install-logto-to-your-app}

Primeiro, acesse o [Logto Cloud](https://cloud.logto.io/). Você pode se cadastrar gratuitamente se ainda não tiver uma conta. Crie um Tenant de Desenvolvimento para testes.
...
Siga o guia da página. Você pode completar a integração do Logto em cerca de 5 minutos!
...
Tudo é tão simples! Configuramos um sistema de autenticação de usuários complexo em apenas alguns minutos!
...
```mermaid
graph TD
    T[Logto tenant] --> OrgA[Organização A]
...
### Obtenha informações de organização do usuário \{#get-a-user-s-organization-information}

Para obter informações de organização do usuário no Logto, siga estes dois passos:
...
Graças ao Logto, não precisamos construir relacionamentos complexos de organização. Só precisamos criar uma organização no Logto e adicionar usuários a ela. O Logto cuida de toda a complexidade para nós. Existem duas maneiras de criar Organizações:

1. Criar organizações manualmente pelo Logto Console
2. Usar o Logto Management API para criar organizações, especialmente ao projetar um fluxo SaaS que permite aos usuários criarem suas próprias organizações (workspaces).
...
Criar uma organização no console não é suficiente. Seu app SaaS precisa de um fluxo que permita aos usuários finais criarem e gerenciarem facilmente seus próprios workspaces. Para implementar essa funcionalidade, use o Logto Management API.

Para orientação, confira a documentação [Interaja com o Management API](/integrate-logto/interact-with-management-api) para configurar a comunicação da API com o Logto.
...
1. **Proteger a API do serviço backend**:
   - O acesso do frontend à nossa API do serviço backend requer autenticação
   - Endpoints da API são protegidos validando o Logto Access Token do usuário
   - Garante que apenas usuários autenticados possam acessar nossos serviços
2. **Acessar o Logto Management API**:
   - O serviço backend precisa chamar o Logto Management API com segurança
   - Siga o guia [Interaja com o Management API](/integrate-logto/interact-with-management-api) para configuração
...
app.post('/organizations', async (req, res) => {
  // Implementação usando Logto Management API
  // ...
});
```
...
Você verá um tutorial para usar o API resource. Você pode seguir esse tutorial ou nossos passos abaixo.
...
Isso protege nosso endpoint POST /organizations. Apenas usuários com Logto access tokens válidos podem acessá-lo.
...
No código frontend, declare esse API resource no config do Logto. Adicione seu identificador ao array de resources.
...
No Dashboard, obtenha o Logto Access Token ao criar uma organização. Use esse token para acessar nossa API do serviço backend.
...
### Chamando o Logto Management API \{#calling-logto-management-api}

Vamos implementar a criação de organização usando o Logto Management API.

Assim como as requisições do frontend para o serviço backend, as requisições do serviço backend para o Logto precisam de Access tokens.

No Logto, usamos autenticação Máquina para Máquina para access tokens. Veja [Interaja com o Management API](/integrate-logto/interact-with-management-api).
...
Agora podemos obter access tokens do Logto Management API por meio dessa aplicação M2M.
...
Use esse access token para chamar o Logto Management API.
...
Agora implementamos a criação de organização via Logto Management API. Também podemos adicionar usuários às organizações.
...
O próximo passo seria convidar usuários para uma organização. Não vamos implementar esse recurso neste tutorial ainda. Você já sabe como usar o Management API. Você pode consultar este [**Tenant creation and invitation**](https://blog.logto.io/build-multi-tenant-saas-application#tenant-creation-and-invitation) como referência de design de produto e implementar facilmente esse recurso seguindo este post: [Como implementamos colaboração de usuários em um app multi-inquilino](https://blog.logto.io/implement-user-collaboration-in-your-app).
...
Há dois pontos importantes a observar nesta implementação:
...
Em seguida, criamos uma API `GET /documents` em nosso serviço backend. Assim como usamos o API resource para proteger a API `POST /organizations`, usamos indicadores de recurso específicos da organização para proteger a API `GET /documents`.
...
Alguns softwares exigem isolamento de dados entre organizações. Para discussão e implementação avançada, consulte o post: [Implementação de multi-inquilino com PostgreSQL: Aprenda com um exemplo real simples](https://blog.logto.io/implement-multi-tenancy).

### Implemente design de controle de acesso baseado em papel no nível da organização \{#implement-organization-level-role-based-access-control-design}
...
Portanto, nossa Organização precisa desses dois papéis: Admin e Colaborador.
...
Vamos ao Logto Console > Organization Templates > Organization permissions e criar duas permissões: `read:documents` e `create:documents`.
...
Em seguida, vá para a página de detalhes da Organização para atribuir os papéis apropriados aos membros.
...
Você pode realizar esses passos via Logto Management API:
...
Agora podemos implementar o controle de permissões do usuário verificando suas permissões.

No nosso código, precisamos fazer com que o token de organização do usuário carregue informações de permissão, e então verificar essas permissões no backend.

No config do Logto no frontend, precisamos declarar as permissões que os usuários precisam solicitar dentro da organização. Vamos adicionar as permissões `read:documents` e `create:documents` aos `scopes`.
...
Assim, implementamos o controle de permissões do usuário verificando suas permissões.
