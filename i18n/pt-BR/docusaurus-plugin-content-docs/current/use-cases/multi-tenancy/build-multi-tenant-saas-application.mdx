...
description: Aprenda como construir de forma eficiente um aplicativo SaaS multi-inquilino com autenticação robusta, gerenciamento de organizações e controle de acesso baseado em papel.
---
...
Quando pensei pela primeira vez em construir uma fera tão complexa, minha mente explodiu:
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Página de organização do Documind"
...
> **O código-fonte completo está disponível neste [repositório do Github](https://github.com/logto-io/multi-tenant-saas-sample). Vamos mergulhar!**

Começaremos com um produto SaaS de documentação com IA chamado DocuMind.
...
A plataforma oferece poderosos recursos de IA para gerenciamento de documentos, incluindo geração automática de resumos, extração de pontos-chave e recomendações inteligentes de conteúdo dentro das organizações.
...
Para habilitar uma arquitetura multi-inquilino, você precisará de uma camada de entidade chamada **organização**. Imagine ter um único pool de usuários que podem acessar vários espaços de trabalho. Cada organização representa um espaço de trabalho, e os usuários mantêm uma única identidade enquanto acessam diferentes espaços de trabalho (organizações) com base em seus papéis atribuídos.
...
### Design de papel e permissão \{#role-and-permission-design}

Em uma arquitetura multi-inquilino, os usuários precisam de **papéis** com **permissões** específicas para acessar os recursos do seu inquilino.
...
Aqui estão alguns cenários comuns encontrados em quase todo aplicativo SaaS. Use-os como referência para inspirar seu time de produto e design, e sinta-se à vontade para criar seus próprios fluxos conforme necessário.
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Um usuário existente entra por email"
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Um novo usuário entra por email"
...
Autenticação parece assustadora. Usuários precisam de:

- Cadastro/login com email e senha
- Login com um clique usando Google/Github
- Redefinição de senha quando esquecem
- Login em equipe para clientes corporativos
- ...
...
O sistema de organizações permite que usuários criem e entrem em várias organizações. Vamos entender os relacionamentos principais:

```mermaid
erDiagram
    Usuário ||--o{ MembroDaOrganização : "pertence a"
    Organização ||--o{ MembroDaOrganização : "possui"
...
Controle de acesso baseado em papel (RBAC) é importante para garantir segurança e escalabilidade em aplicativos SaaS multi-inquilino.
...
1. **Definições de permissões unificadas**: Permissões são definidas no nível do sistema e aplicadas de forma consistente em todas as organizações, garantindo um gerenciamento de permissões sustentável e consistente.
...
Escolhi um stack portátil e amigável para iniciantes:
...
Primeiro, faça login no [Logto Cloud](https://cloud.logto.io/). Você pode se cadastrar gratuitamente se ainda não tiver uma conta. Crie um Tenant de Desenvolvimento para testes.
...
Aqui vai uma dica útil: Nossa página de login tem botões de Entrar e Registrar. O botão Registrar leva diretamente para a página de cadastro do Logto. Isso funciona através do recurso [primeira tela](/end-user-flows/authentication-parameters/first-screen) do Logto. Ele determina qual etapa do fluxo de autenticação os usuários veem primeiro.
...
Após clicar em login, você irá para a página de login do Logto. Ao fazer login (ou cadastro) com sucesso, parabéns! Seu app tem seu primeiro usuário (você)!
...
No Logto Console, clique em "Sign-in & account" no menu à esquerda. Depois clique na aba "Sign-up and sign-in".
...
Agora temos nosso primeiro usuário! No entanto, esse usuário ainda não pertence a nenhuma organização, e ainda não criamos nenhuma organização.

O Logto oferece suporte nativo para multi-inquilino. Você pode criar qualquer número de organizações no Logto. Cada organização pode ter vários membros.
...
Cada usuário pode obter suas informações de organização a partir do Logto. Isso habilita o suporte a multi-inquilino.

### Obtenha informações de organização de um usuário \{#get-a-user-s-organization-information}

Para obter informações de organização de um usuário no Logto, siga estes dois passos:

Declare o acesso às informações da organização no Logto Config. Isso é feito definindo os `scopes` e `resources` apropriados.
...
2. Usar a Management API do Logto para criar organizações, especialmente ao projetar um fluxo SaaS que permite aos usuários criarem suas próprias organizações (workspaces).
...
## Implemente experiência de criação de organização self-service \{#implement-self-serve-organization-creation-experience}

Criar uma organização no console não é suficiente. Seu app SaaS precisa de um fluxo que permita aos usuários finais criarem e gerenciarem facilmente seus próprios workspaces. Para implementar essa funcionalidade, use a Management API do Logto.
...
1. **Proteger a API do serviço backend**:
   ...
   - Endpoints da API são protegidos validando o Token de Acesso do Logto do usuário
...
Nossa API do serviço backend só permite usuários autenticados. Precisamos usar o Logto para proteger nossa API. Também precisamos saber as informações do usuário atual (como o ID do usuário).

No conceito do Logto (e OAuth 2.0), nosso serviço backend atua como um resource server. Usuários acessam o resource server do DocuMind com um Token de Acesso vindo do frontend. O resource server verifica esse token. Se válido, retorna os recursos solicitados.

Vamos criar um Recurso de API para representar nosso serviço backend.
...
Não se preocupe com essa URL de identificador de API. É apenas um identificador único para sua API no Logto. Não está relacionado à URL real do seu serviço backend.
...
Obtenha essas variáveis no endpoint OpenID Configuration do seu tenant Logto. Visite `https://<your-tenant-id>.logto.app/oidc/.well-known/openid-configuration`. Você encontrará as informações necessárias no JSON retornado:
...
Agora use o middleware requireAuth em nosso endpoint POST /organizations.
...
Isso protege nosso endpoint POST /organizations. Apenas usuários com tokens de acesso Logto válidos podem acessá-lo.

Agora podemos obter o token do Logto no frontend. Usuários podem criar organizações através do nosso serviço backend com esse token. O middleware também nos dá o ID do usuário. Isso ajuda ao adicionar usuários às organizações.

No código do frontend, declare esse recurso de API no config do Logto. Adicione seu identificador ao array de resources.
...
const config: LogtoConfig = {
  ...
  resources: [ReservedResource.Organization, "<https://api.documind.com>"], // Identificador do novo recurso de API
};
...
No Dashboard, obtenha o Token de Acesso do Logto ao criar uma organização. Use esse token para acessar nossa API do serviço backend.

```jsx
// Obter token de acesso para "DocuMind API"
const token = await getAccessToken('<https://api.documind.com>');

// Acessar nossa API do serviço backend com o token
const response = await fetch('<http://localhost:3000/organizations>', {
  ...
  body: JSON.stringify({
    name: 'Organização A',
    description: 'Descrição da Organização A',
  }),
});
```
...
Assim como as requisições do frontend para o backend, as requisições do backend para o Logto precisam de tokens de acesso.

No Logto, usamos autenticação Máquina para Máquina para tokens de acesso. Veja [Interaja com a Management API](/integrate-logto/interact-with-management-api).

Vá para a página de aplicações no Logto Console. Crie uma aplicação Máquina para Máquina. Atribua o papel "Logto Management API access". Copie o endpoint de Token, App ID e App Secret. Usaremos esses dados para obter tokens de acesso.

![m2m-application.png](https://uploads.strapi.logto.io/m2m_application_d882b6386e.png)

Agora podemos obter tokens de acesso da Management API do Logto através dessa aplicação M2M.
...
Use esse token de acesso para chamar a Management API do Logto.

Usaremos essas APIs de gerenciamento:

- `POST /api/organizations`: Criar organização (veja: [Referência da API de criação de organização](https://openapi.logto.io/operation/operation-createorganization))
- `POST /api/organizations/{id}/users`: Adicionar usuários à organização (veja: [Referência da API de adicionar usuários](https://openapi.logto.io/operation/operation-addusers))
...
e clicar em “Criar Organização”

![dashboard-has-orgs.png](https://uploads.strapi.logto.io/dashboard_has_orgs_568a44cc1f.png)

Criação bem-sucedida!

O próximo passo seria convidar usuários para uma organização. Não implementaremos esse recurso neste tutorial ainda. Você já sabe como usar a Management API. Você pode consultar este [**Criação de inquilino e convite**](#tenant-creation-and-invitation) como referência de design de produto e implementar facilmente esse recurso seguindo este post do blog: [Como implementamos colaboração de usuários em um app multi-inquilino](https://blog.logto.io/implement-user-collaboration-in-your-app).
...
- Usuários só podem acessar recursos pertencentes às suas próprias organizações: Isso pode ser feito através do `token de organização` do Logto
...
2. Ao solicitar recursos da organização, usamos o token de organização em vez do token de acesso porque, para recursos pertencentes a uma organização, queremos usar o controle de permissão da organização e não o controle de permissão do usuário (você entenderá melhor isso ao implementarmos a API `GET /documents` mais adiante).

Em seguida, criamos uma API `GET /documents` em nosso serviço backend. Assim como usamos o recurso de API para proteger a API `POST /organizations`, usamos indicadores de recurso específicos da organização para proteger a API `GET /documents`.
...
const requireOrganizationAccess = () => {
  return async (req, res, next) => {
    try {
      ...
      // Extrai o ID da organização do claim audience
      const organizationId = extractOrganizationId(payload.aud);

      // Adiciona informações da organização à requisição
      req.user = {
        id: payload.sub,
        organizationId,
      };
...
app.get('/documents', requireOrganizationAccess(), async (req, res) => {
  // Você pode obter o id do usuário atual e o organizationId através de req.user
  console.log('userId', req.user.id);
  console.log('organizationId', req.user.organizationId);

  // Obter documentos do banco de dados pelo organizationId
  // ....
...
Alguns softwares exigem isolamento de dados entre organizações. Para discussão e implementação mais aprofundadas, consulte o post do blog: [Implementação de multi-inquilino com PostgreSQL: Aprenda com um exemplo real simples](https://blog.logto.io/implement-multi-tenancy).
...
Implementamos o uso de tokens de organização para acessar recursos da organização. Agora, vamos implementar o controle de permissão do usuário dentro das organizações usando RBAC.
...
>
> Porque escalabilidade é um dos requisitos mais importantes para produtos SaaS. Em outras palavras, o que funciona para um cliente deve funcionar para todos os clientes.
...
Assim, criamos um modelo de permissão RBAC para cada organização.

Em seguida, vamos à página de detalhes da nossa Organização para atribuir papéis apropriados aos nossos membros.
...
No nosso código, precisamos fazer com que o token de organização do usuário carregue informações de permissão e, em seguida, verificar essas permissões no backend.
...
Depois crie uma API POST /documents, e use o middleware `requireOrganizationAccess` com configuração de requiredScopes para proteger essa API e a anterior `GET /documents`.
...
Assim, implementamos o controle de permissão do usuário verificando suas permissões.
...
## Adicione mais recursos ao app multi-inquilino \{#add-more-multi-tenant-app-features}

Até agora, implementamos os recursos básicos de usuário e organização em um sistema SaaS multi-inquilino! No entanto, ainda há alguns recursos que não abordamos, como personalizar o branding da página de login para cada Organização, adicionar automaticamente usuários com emails de domínio específico a certas organizações e integrar funcionalidades de SSO corporativo.
...
- [Branding no nível da organização](/customization/match-your-brand#organization-specific-branding)
- [Autenticação multifatorial no nível da organização (MFA)](/organizations/organization-management#require-mfa-for-organization-members)
...
O código-fonte completo deste tutorial está disponível em: [Exemplo SaaS multi-inquilino](https://github.com/logto-io/multi-tenant-saas-sample).

Esse é o poder do desenvolvimento moderno em 2025 — podemos focar em construir recursos únicos de produto em vez de lutar com infraestrutura. Agora é a sua vez de construir algo incrível!
