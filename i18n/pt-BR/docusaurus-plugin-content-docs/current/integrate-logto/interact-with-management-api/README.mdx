...
Nossos desenvolvedores implementaram muitos recursos adicionais usando a Logto Management API. Acreditamos que nossa API é altamente escalável e pode suportar uma ampla gama de necessidades. Aqui estão alguns exemplos de cenários que não são possíveis com o Logto Admin Console, mas podem ser alcançados através da Logto Management API.
...
O Logto Admin Console suporta funções básicas de busca e filtragem. Para opções avançadas de busca, como busca aproximada, correspondência exata e sensibilidade a maiúsculas/minúsculas, confira nossos tutoriais e guias de [Pesquisa Avançada de Usuários](/user-management/advanced-user-search).
...
Algumas respostas de API podem incluir muitos resultados, que serão paginados. O Logto fornece 2 tipos de informações de paginação.
...
O Logto Cloud aplica limites de taxa em tempo de execução no nível do inquilino para proteger a estabilidade do sistema. Para detalhes, veja a [seção de limite de taxa do sistema](/logto-cloud/system-limit#rate-limit).
