...
Atribua permissões em nível de usuário, incluindo [permissões de perfil do usuário](#user-permissions-user-profile-scopes) (por exemplo, email, nome e avatar) e [permissões de recursos de API](#api-resource-permissions-api-resource-scopes) (por exemplo, acesso de leitura ou gravação a recursos específicos).
...
- `urn:logto:scope:organization_roles`: Escopo de organização do usuário do Logto, usado para acessar informações dos papéis do usuário na organização.
...
[Permissões de organização](/authorization/organization-template) são os escopos definidos exclusivamente para organizações Logto. Eles são usados para acessar informações e recursos da organização.
