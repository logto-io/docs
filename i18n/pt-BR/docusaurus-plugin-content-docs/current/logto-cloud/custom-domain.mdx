...
- URLs relacionadas a [Passkey](/end-user-flows/mfa/webauthn) e [login com passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) (Alterar o domínio após os usuários terem vinculado Passkeys pode bloquear tanto o MFA quanto o login com essas credenciais).
...
- Fornecer experiências de autenticação específicas por região ou marca através da [Custom UI](/customization)
...
Na página de detalhes do seu aplicativo em <CloudLink to="/applications">Console > Aplicativos</CloudLink>, role até a seção "Endpoints & Credenciais". Altere o domínio no dropdown para visualizar e copiar os endpoints correspondentes para atualizar as configurações do seu aplicativo.
...
[Conectores sociais](/connectors/social-connectors) usam o protocolo OIDC/OAuth. Quando os usuários fazem login através de um domínio personalizado, o URI de redirecionamento usará automaticamente esse domínio personalizado. Você precisa atualizar o URI de redirecionamento no console de desenvolvedor do seu provedor social.
