...
- URLs de vinculação de chave de acesso (Alterar o domínio após os usuários terem vinculado as chaves de acesso pode bloquear sua autenticação).
- URIs de callback para conectores sociais ou conectores de SSO corporativo.
- Endpoint do SDK para integrar o Logto com seu aplicativo.

:::note
Alterar o domínio após publicar seu serviço pode causar problemas porque seu código de aplicativo e integrações podem ainda referenciar o domínio antigo. Para garantir uma transição suave, **configure seu domínio personalizado no início** durante a criação de um locatário de Produção.
:::
...
<details>
<summary>Problemas com certificado SSL</summary>

Se você encontrar problemas com o certificado SSL ao configurar seu domínio personalizado, isso pode estar relacionado a registros CAA em sua configuração de DNS. Registros CAA especificam quais Autoridades Certificadoras (CAs) estão autorizadas a emitir certificados para seu domínio.
...
<details>
<summary>Erro "O nome do host está associado a uma zona retida"</summary>
