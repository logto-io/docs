...
Seu tenant Logto vem com um domínio padrão gratuito `{{tenant-id}}.app.logto`. No entanto, você pode elevar sua experiência do usuário e reconhecimento de marca usando domínios personalizados, como `auth.example.com`.
...
Uma vez salvo, o Logto serve uma correspondência exata para requisições `GET` e `HEAD` no domínio personalizado, por exemplo:
...
Depois de configurar suas definições, tanto o nome do seu domínio personalizado quanto o nome do domínio padrão do Logto estarão disponíveis para seu tenant. No entanto, certas configurações são necessárias para ativar seu nome de domínio personalizado.
...
Na página de detalhes do seu aplicativo em <CloudLink to="/applications">Console > Aplicativos</CloudLink>, role até a seção "Endpoints & Credenciais". Troque o domínio no dropdown para visualizar e copiar os endpoints correspondentes para atualizar as configurações do seu aplicativo.
...
[Conectores sociais](/connectors/social-connectors) usam o protocolo OIDC/OAuth. Quando os usuários fazem login por meio de um domínio personalizado, o URI de redirecionamento usará automaticamente esse domínio personalizado. Você precisa atualizar o URI de redirecionamento no console do desenvolvedor do seu provedor social.
...
2. Copie os URIs de redirecionamento dos detalhes do conector. O Logto lista todos os URIs de redirecionamento disponíveis para seus domínios personalizados configurados.
...
[Conectores corporativos baseados em SAML](/connectors/enterprise-connectors) usam uma URL de Serviço de Consumidor de Asserção (ACS) em vez de um URI de redirecionamento.
