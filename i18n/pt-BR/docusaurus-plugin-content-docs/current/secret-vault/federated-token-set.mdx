...
Essa capacidade é essencial para aplicativos modernos como agentes de IA, plataformas SaaS, ferramentas de produtividade e aplicativos para clientes que precisam interagir com serviços de terceiros em nome dos usuários. Aqui estão alguns exemplos práticos:

**📅 Aplicativos de gerenciamento de calendário**: Após um usuário fazer login com o Google, seu app de produtividade pode sincronizar automaticamente os eventos do calendário, criar novas reuniões e enviar convites sem pedir autenticação novamente.
...
4. Na página "Configuração", habilite a opção **Armazenar tokens para acesso persistente à API**.
...
3. Siga os tutoriais de configuração para configurar o conector, incluindo a adição dos escopos necessários para acessar APIs de terceiros específicas.
4. Na guia "Experiência SSO", habilite a opção **Armazenar tokens para acesso persistente à API**.

Certifique-se de salvar suas alterações.

## Armazenamento de tokens \{#token-storage}

Uma vez habilitado o armazenamento de tokens de terceiros, o Logto armazena automaticamente os tokens de acesso e atualização emitidos pelo provedor de identidade federado sempre que um usuário se autentica por meio de um conector social ou SSO corporativo. Isso inclui:
...
Os tokens armazenados são vinculados à identidade social ou SSO corporativa do usuário, permitindo que sejam recuperados posteriormente para acesso à API sem exigir nova autenticação.
...
4. Cada entrada de conexão mostra um rótulo de status de token indicando se os tokens estão armazenados para aquela conexão.
...
- `updatedAt`: A última vez que o conjunto de tokens foi atualizado.
  - Se nenhum token de atualização estiver disponível, esse valor será igual ao **createdAt**.
  - Se houver um token de atualização, esse valor reflete o momento mais recente em que o token de acesso foi renovado.
...
- `404` Não encontrado: Se o usuário não tiver uma identidade social ou SSO corporativa associada ao conector especificado ou se o token de acesso não estiver armazenado.
- `401` Não autorizado: Se o token de acesso estiver expirado.

Se o token de acesso estiver expirado e um token de atualização estiver disponível, o Logto tentará automaticamente renovar o token de acesso e retornará o novo token de acesso na resposta. O armazenamento do token no cofre de segredos também será atualizado com o novo token de acesso e seus metadados.
...
6. O Logto verifica o código de autorização e o troca por um novo token de acesso e token de atualização do provedor de terceiros, retornando então o ID de verificação social na resposta.
...
7. Por fim, atualize o armazenamento de tokens do usuário chamando o endpoint `PATCH /my-account/identities/:target/access-token` com o ID de verificação social:
   ...
   - **authorization header**: O token de acesso do usuário emitido pelo Logto.
   - **socialVerificationId**: O ID do registro de verificação social retornado na etapa anterior.
