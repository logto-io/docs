---
description: Guias de início rápido para a inicialização do serviço Logto open-source (OSS).
sidebar_position: 1
---
...
toda vez que você reexecutar o comando, uma nova instância do banco de dados será criada e todos os dados persistidos anteriormente serão perdidos.
:::
...
DATABASE_STATEMENT_TIMEOUT: DISABLE_TIMEOUT # (v1.36.0+) Desabilite statement_timeout para PgBouncer/RDS Proxy
```

Execute o container com as variáveis de ambiente obrigatórias acima (as opcionais estão omitidas abaixo; adicione conforme necessário):
...
- Se estiver usando PgBouncer ou RDS Proxy e ver `unsupported startup parameter: statement_timeout`, defina `DATABASE_STATEMENT_TIMEOUT=DISABLE_TIMEOUT` (v1.36.0+).
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        'Uma alternativa auto-hospedável ao Heroku/Netlify para fácil gerenciamento de apps e banco de dados.',
      customProps: {
        icon: <CoolifyIcon />,
      },
    },
...
Por favor, note que não fornecemos suporte ao cliente para provedores de serviços de terceiros. Para acessar nossos canais de suporte, faça a implantação no [Logto Cloud](https://cloud.logto.io). Obrigado!
...
:::tip
O Logto OSS (auto-hospedado) não suporta a configuração de múltiplos administradores. Para colaboração em equipe e projetos que exigem múltiplos usuários admin, recomendamos o uso do [Logto Cloud](https://cloud.logto.io), que oferece recursos completos de gerenciamento de equipes.
:::
