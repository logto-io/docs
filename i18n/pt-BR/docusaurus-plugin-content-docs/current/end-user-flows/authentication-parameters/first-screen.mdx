...
O parâmetro `login_hint`, definido na [especificação padrão do OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html#AuthorizationEndpoint), é usado para pré-preencher o formulário de login com o identificador do usuário (como um email, número de telefone ou nome de usuário). Com o Logto, ele pode ser combinado com outros parâmetros de tela de login para aprimorar a experiência do usuário. Este parâmetro é especialmente útil se você possui um formulário de pré-autenticação personalizado que coleta o identificador do usuário antecipadamente, permitindo que ele pule a etapa de digitação durante o login.
