...
Um conjunto de parâmetros de autenticação personalizados que permitem adaptar a experiência da primeira tela desejada para os usuários finais.

- `first_screen`: Especifica a primeira tela que o usuário verá.
- `identifier`: Especifica os tipos de identificadores que o formulário de login ou cadastro aceitará.
...
- `identifier:sign-in`: Exibe um formulário de login específico para identificador. O tipo de identificador pode ser especificado usando o parâmetro `identifier`. Isso é útil quando você tem vários métodos de login por identificador habilitados.
- `identifier:register`: Exibe um formulário de cadastro específico para identificador. O tipo de identificador pode ser especificado usando o parâmetro `identifier`. Isso é útil quando você tem vários métodos de cadastro por identificador habilitados.

![Parâmetro da primeira tela](./assets/first-screen-parameter.png)
...
O parâmetro `identifier` é usado para especificar os tipos de identificadores que o formulário de login ou cadastro aceitará. Este parâmetro é aplicável apenas quando o parâmetro `first_screen` está definido como `identifier:sign-in`, `identifier:register` ou `reset_password`. Os valores suportados são: `username`, `email` e `phone`. Separe múltiplos valores com um espaço vazio para permitir múltiplos tipos de identificadores.
...
Todos os tipos de identificadores especificados neste parâmetro devem estar habilitados nas suas configurações de login ou cadastro no Logto Console.
