...
O Logto utiliza o fluxo [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) para autenticação. Um fluxo de autorização OIDC válido deve ser iniciado pelo seu aplicativo, por exemplo, ao clicar no botão “Entrar” do seu app, é enviada uma solicitação de autorização OIDC para o Logto, que então configura uma sessão segura (cookie).
...
2. Defina seu endereço de e-mail de suporte
