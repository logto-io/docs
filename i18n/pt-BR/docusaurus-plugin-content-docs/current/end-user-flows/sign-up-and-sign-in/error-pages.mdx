...
O Logto utiliza o fluxo [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) para autenticação. Um fluxo de autorização OIDC válido deve ser iniciado pelo seu aplicativo, por exemplo, ao clicar no botão "Entrar" do seu app, que envia uma solicitação de autorização OIDC ao Logto, que então configura uma sessão segura (cookie).
