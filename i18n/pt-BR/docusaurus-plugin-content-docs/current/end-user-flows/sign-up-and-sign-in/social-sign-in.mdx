...
- **Enriquecimento de dados do usuário**: O login social permite que você colete informações adicionais de perfil da plataforma social, como nome, endereço de e-mail, foto de perfil e mais.
...
2. **Adicione botões de login social**:<br/>
   ...
   Navegue até <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Console > Login & conta > Cadastro e login</CloudLink>. Clique no botão “Adicionar conector social” para integrar botões de login social em suas páginas de cadastro e login. Use arrastar e soltar para organizar a ordem deles na interface.

3. **Defina opções de vinculação de conta**:<br/>
   Para novos usuários que se cadastram com [identidades sociais](/user-management/user-data#social-identities), o Logto permite vincular a conta social a uma conta existente de e-mail ou número de telefone dentro do sistema Logto. Por padrão, uma página relacionada de [vinculação de conta](#account-linking) é exibida aos usuários durante o cadastro social, permitindo que eles vinculem a conta social a uma conta existente ou criem uma nova.
...
Se o e-mail ou número de telefone for um identificador obrigatório de cadastro, o usuário será solicitado a fornecer outro e-mail ou número de telefone durante o processo de cadastro. Veja [Coletar dados adicionais de perfil do usuário](#collect-additional-user-profile-data) para mais detalhes.
:::
...
2. **Cadastro com identidade social que não fornece endereço de e-mail verificado**
...
3. **Cadastro com identidade social que fornece endereço de e-mail já cadastrado**

   Se a identidade social fornecer um endereço de e-mail que já está cadastrado no sistema Logto, o usuário será solicitado a vincular a conta social à conta existente ou criar uma nova conta. Se o usuário optar por criar uma nova conta, será solicitado a fornecer um novo endereço de e-mail e verificá-lo.
...
### Coletar outros dados de perfil do usuário \{#collect-other-user-profile}

Além dos identificadores obrigatórios de cadastro, você também pode coletar outras informações de perfil durante o processo de cadastro social. Isso pode incluir campos como nome completo, data de nascimento ou quaisquer outros campos personalizados que desejar coletar.
...
Coletar identificadores de cadastro (e-mail, telefone, nome de usuário) difere de coletar outros dados de perfil do usuário durante o login social:

- **Identificadores de cadastro**: Obrigatórios para novos e existentes usuários porque são essenciais para identificação e notificações.
- **Outros dados de perfil** (por exemplo, nome completo, aniversário): Coletados apenas durante o registro de novo usuário, pois essas informações não são críticas para identificação e podem ser coletadas depois usando a Account API.
...
Após o login social inicial, os usuários podem gerenciar suas contas sociais conectadas através das configurações de conta. Isso permite que os usuários vinculem contas sociais adicionais ao perfil existente ou removam conexões sociais que não desejam mais usar.

Os usuários podem acessar esses recursos por meio de um centro de conta self-service, que oferece flexibilidade para:
...
### Como adicionar botões de login social e permitir login direto com social no meu site? \{#how-to-add-social-login-buttons-and-directly-sign-in-with-social-on-my-website}
