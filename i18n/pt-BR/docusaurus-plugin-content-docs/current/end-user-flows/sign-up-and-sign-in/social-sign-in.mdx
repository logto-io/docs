...
2. **Adicione botões de login social**:<br/>
   ...
   Navegue até <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Console > Experiência de login > Cadastro e login</CloudLink>. Clique no botão “Adicionar conector social” para integrar botões de login social em suas páginas de cadastro e login. Use arrastar e soltar para organizar a ordem deles na interface.

3. **Defina opções de vinculação de conta**:<br/>
   Para novos usuários que se cadastram com [identidades sociais](/user-management/user-data#social-identities), o Logto permite vincular sua conta social a uma conta de email ou número de telefone existente no sistema Logto. Por padrão, uma página de [vinculação de conta](#account-linking) relacionada é mostrada aos usuários durante o cadastro social, permitindo que eles vinculem sua conta social a uma conta existente ou criem uma nova.
...
<img
  src={socialSignInEnableGoogleOneTabImage}
  alt="Habilitar Google One Tab na experiência de login"
/>

## Vinculação de conta \{#account-linking}

Como mencionado acima, o Logto permite que os usuários vinculem suas contas sociais a contas de email ou número de telefone existentes no sistema Logto. Esse processo é essencial para manter uma conta de usuário unificada em diferentes métodos de autenticação e provedores de identidade.
...
- **Vinculação de conta existente**:
  ...
  - **Vinculação manual de conta**: Se a opção “Vinculação automática de conta” estiver desabilitada, os usuários serão solicitados a vincular sua conta social à conta existente durante o processo de login. Eles podem escolher vincular as contas ou criar uma nova.
    ...
    <img src={signInSocialLinkingImage} alt="Vinculação de conta social" />
...
Durante o processo de cadastro social, dependendo das configurações de identificador de cadastro obrigatório (**endereço de email**, **número de telefone** e **nome de usuário**) que você configurou, os usuários podem ser solicitados a fornecer informações verificadas adicionais para concluir o processo de cadastro ou login após serem autenticados com o provedor social.
...
### Por que meus emails ou números de telefone não são preenchidos após o cadastro com social? \{#why-my-emails-or-phone-numbers-are-not-populated-after-signing-up-with-social}
