...
O cadastro e login é o processo central de interação para que os usuários finais possam se autenticar e autorizar o acesso a aplicativos clientes. Como uma plataforma centralizada de [CIAM](https://auth.wiki/iam) baseada em OIDC, o Logto oferece uma experiência universal de login para usuários em vários aplicativos clientes e plataformas.
...
Continue aprendendo mais sobre a [configuração da experiência de login](/end-user-flows/sign-up-and-sign-in/sign-up) e [personalização](/customization) no Logto.

## Métodos comuns de login \{#common-sign-in-methods}
...
- [Login por email / telefone / nome de usuário](/end-user-flows/sign-up-and-sign-in/sign-in): Login clássico com identificador e senha ou código de verificação.
- [Login social](/end-user-flows/sign-up-and-sign-in/social-sign-in): Login com Google, GitHub, Facebook e outros provedores sociais.
- [Login com passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in): Login sem senha com passkeys WebAuthn, incluindo botão direto, verificação de passkey após identificador e login baseado em preenchimento automático.
...
### Método de experiência de login ou personalização de marca por aplicativo \{#per-app-sign-in-experience-method-or-branding}
...
Se você precisar oferecer diferentes **métodos de login** com base no tipo de usuário ou site, basta usar [parâmetros de autenticação](/end-user-flows/authentication-parameters) (por exemplo, `first_screen` e `direct_sign_in`) para direcionar os usuários a uma página personalizada com opções de login específicas.
...
Atualmente, o Logto não fornece uma API headless para login e cadastro. No entanto, você pode criar sua própria interface de login usando o recurso [Traga sua própria UI (Bring your own UI)](/customization/bring-your-ui/) para personalizar a experiência de login e cadastro.
