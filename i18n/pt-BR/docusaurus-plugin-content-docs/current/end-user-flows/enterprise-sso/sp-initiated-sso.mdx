...
1. **Botão de autenticação única**:
   ...
   Quando o método de login SSO corporativo é habilitado, um botão "Single Sign-On" aparecerá como uma opção alternativa de login na página de login. Ao clicar neste link, os usuários são solicitados a inserir seu endereço de email corporativo para iniciar o processo de SSO.
