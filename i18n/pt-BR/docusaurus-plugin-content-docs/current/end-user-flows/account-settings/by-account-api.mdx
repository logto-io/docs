...
- Acesso direto: A Account API permite que os usuários finais acessem e gerenciem diretamente seu próprio perfil de conta sem exigir o repasse pela Management API.
...
- Autorização fluida: Autorizar nunca foi tão fácil! Basta usar `client.getAccessToken()` para obter um token opaco de acesso para OP (Logto) e anexá-lo ao cabeçalho Authorization como `Bearer <access_token>`.
...
Após verificar o código, você pode agora atualizar o email do usuário, definindo o `verificationId` no corpo da requisição como `newIdentifierVerificationRecordId`.
