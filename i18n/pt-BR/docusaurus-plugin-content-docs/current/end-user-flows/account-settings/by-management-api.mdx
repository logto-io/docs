...
```mermaid
  graph TB
    ...
    B -->|Chamada de API de configurações de conta self-hosted|C[Aplicação server-side]
...
3. **Aplicação server-side**: Aplicação server-side que fornece a API de configurações de conta para o cliente. Interage com a Management API do Logto.
...
```mermaid
  sequenceDiagram
    ...
    participant Server as Aplicação server-side
...
7. O aplicativo cliente envia a solicitação de configurações de conta para a aplicação server-side com o token de acesso do usuário.
8. A aplicação server-side verifica a identidade e permissão do solicitante a partir do token de acesso do usuário. Em seguida, solicita um token de acesso à Management API do Logto.
9. O Logto concede o token de acesso à Management API para a aplicação server-side.
10. A aplicação server-side solicita os dados do usuário ao Logto usando o token de acesso da Management API.
11. O Logto verifica a identidade do servidor e a permissão da Management API e retorna os dados do usuário.
12. A aplicação server-side processa os dados do usuário com base na permissão do solicitante e retorna os detalhes da conta do usuário para o aplicativo cliente.

### Integrar Management API à aplicação server-side \{#integrate-management-api-to-server-side-application}

Consulte a seção [Management API](/integrate-logto/interact-with-management-api/) para aprender como integrar as Management APIs com aplicações server-side.
...
:::note
Certifique-se de verificar a senha atual do usuário antes de atualizar a senha.
:::
...
| method | path                                                                                                                              | description                                                                                                          |
| ------ | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| GET    | [/api/users/\{userId\}](https://openapi.logto.io/operation/operation-getuser)                                                     | Obter detalhes do usuário pelo ID. As identidades sociais estão no campo `identities`.                               |
...
sequenceDiagram
    ...
    participant Client as Aplicativo cliente
    participant App as aplicação server
    participant Logto as Logto
    participant IdP as Provedor de identidade social

    User ->> Client: Acessa o app cliente e solicita vincular identidade social
    ...
    IdP ->> Client: Redireciona para o app cliente com código de autorização
    Client ->> Server: Solicitação de vincular identidade social, encaminha resposta do IdP
...
| method | path                                                                                                    | description                                                                                                                                                                                     |
| ------ | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | [/api/users/\{userId\}?includeSsoIdentities=true](https://openapi.logto.io/operation/operation-getuser) | Obter detalhes do usuário pelo ID. As identidades corporativas estão no campo `ssoIdentities`. Adicione o parâmetro de consulta `includeSsoIdentities=true` para incluí-las na API de detalhes. |
