...
Imagine que você está construindo um aplicativo multi-tenant (por exemplo, um aplicativo SaaS multi-tenant) que atende a vários clientes, e cada cliente possui um tenant dedicado. Quando um novo cliente chega, ele cria uma nova conta, bem como um novo tenant para seu próprio negócio.
...
Se você não está familiarizado com o Logto Management API, certifique-se de ler esses documentos primeiro.
