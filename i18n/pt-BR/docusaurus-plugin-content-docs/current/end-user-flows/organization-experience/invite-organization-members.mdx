...
sequenceDiagram
  ...
  A ->> C: Inserir email do convidado e papel
...
## Criar papéis de organização

Antes de convidar membros para sua organização, você precisa criar papéis de organização. Confira o guia [Configurar modelo de organização](/authorization/organization-template/configure-organization-template) para instruções detalhadas.
...
## Lidar com convites usando Logto Management API
...
Saiba mais sobre [emissão de um token de organização](/authorization/organization-template/protect-organization-resources/#fetch-the-organization-token).
