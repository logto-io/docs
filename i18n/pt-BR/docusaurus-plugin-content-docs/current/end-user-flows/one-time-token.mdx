...
- **Registro somente por convite**: Para ferramentas internas ou produtos de IA em fase de testes, você pode desabilitar o registro público e convidar usuários específicos via magic links.
...
e o endpoint do Logto (necessário para as Management APIs). Saiba mais sobre a [Logto Management API](/integrate-logto/interact-with-management-api).
...
Para magic links de redefinição de senha, o parâmetro de email é opcional; se você omiti-lo, o Logto pedirá ao usuário para inserir o endereço de email antes de verificar o token de uso único.
...
assim como codificar todos os parâmetros da URL.
...
Se você já tiver o email do usuário a partir da sua landing page, passe por `login_hint`; caso contrário, omita `login_hint` e o Logto pedirá ao usuário para inserir o email antes da verificação do token.
