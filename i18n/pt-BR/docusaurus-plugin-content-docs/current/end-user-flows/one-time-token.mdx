...
Por exemplo, quando você desabilita o registro público, pode enviar um magic link com um token de uso único (ex.: `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`) para o email do usuário para convidá-lo a concluir a criação da conta. Você pode personalizar o modelo de email em seu próprio serviço de envio de emails, como:
...
sequenceDiagram
  ...
  C -->> U: Compõe um magic link contendo o token de uso único e envia para o email do usuário final
...
1. O usuário já está autenticado e clica em um magic link associado à conta atual. Nesse caso, o Logto ainda irá verificar o token de uso único e provisionar o usuário para as organizações especificadas, se necessário.
2. O usuário já está autenticado e clica em um magic link associado a uma conta diferente. Nesse caso, o Logto irá solicitar ao usuário que continue como a nova conta ou volte ao aplicativo com a conta atual.
   ...
   2. Se o usuário optar por permanecer na conta atual, o Logto não irá verificar o token e retornará ao aplicativo com a conta atual.
3. Se o prompt de login estiver definido como "login" ou contiver "login", o Logto irá autenticar automaticamente a conta associada ao token de uso único sem solicitar a troca. Isso ocorre porque o prompt "login" indica uma intenção explícita de autenticação, que tem precedência sobre a sessão atual.
