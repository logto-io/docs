...
Às vezes, você pode querer convidar novos usuários para seu aplicativo / organização sem exigir que eles criem uma conta primeiro. Nesses casos, o aplicativo pode enviar um "magic link" para seu email. E você será autenticado imediatamente ao clicar no link.

Desenvolvedores de aplicativos podem usar o token de uso único para compor um magic link e enviá-lo para o endereço de email do usuário final.
...
- **Login / Cadastro**: Envie um magic link para login ou cadastro sem senha via email.
- **Redefinição de senha**: Envie um magic link de redefinição de senha a partir do seu próprio aplicativo para que o usuário possa verificar o token de uso único e definir uma nova senha no Logto.

Por exemplo, quando você desabilitou o registro público, pode enviar um magic link com um token de uso único (por exemplo, `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`) para o email do usuário para convidá-lo a concluir a criação da conta. Você pode personalizar o modelo de email no seu próprio serviço de entrega de emails, como:

<img
  src="/img/assets/email-templates-invitation-only-registration.png"
  alt="Um modelo de email para registro apenas por convite"
/>
...
sequenceDiagram
  ...
  C -->> U: Usuário é autenticado e retorna userinfo ao usuário
```
...
Para um magic link de redefinição de senha, restrinja o token ao fluxo de esqueci a senha:
...
Para magic links de login ou cadastro, o link deve conter pelo menos o token e o email do usuário como parâmetros.
Para magic links de redefinição de senha, o parâmetro de email é opcional; se você omiti-lo, o Logto pedirá ao usuário que insira o endereço de email antes de verificar o token de uso único.
O magic link deve direcionar para uma landing page no seu próprio aplicativo.
...
#### Login ou cadastro \{#sign-in-or-sign-up}

Após o usuário final clicar no magic link e ser direcionado ao seu aplicativo, você pode extrair os parâmetros `token` e `email` da URL e então chamar a função `signIn()` do Logto SDK para acionar o fluxo de autenticação.
...
Para magic links de redefinição de senha, inicie uma solicitação de autenticação com `first_screen` definido como `reset_password`.
Passe o token de uso único por `one_time_token`.
Se você já tiver o email do usuário a partir da sua landing page, passe por `login_hint`; caso contrário, omita `login_hint` e o Logto pedirá ao usuário que insira o email antes da verificação do token.
...
Se um usuário já estiver autenticado, ao chamar a função `signIn()` do SDK, todos os tokens em cache (Token de ID, Token de acesso e Token de atualização) serão automaticamente limpos do armazenamento do cliente,
o que resulta na perda do status de autenticação do usuário atual.

Portanto, você deve especificar um parâmetro adicional de login `clearTokens: false` para evitar limpar os tokens existentes.
Se isso for especificado, você também precisará limpar manualmente os tokens na página de callback do login.

Ignore isso se seus magic links não forem projetados para usuários autenticados.
...
Sim, você pode usar o magic link para convidar novos usuários para seu aplicativo, assim como para organizações.
...
Sim, o token de uso único expira após o tempo especificado em `expiresIn` (em segundos). O tempo padrão de expiração é de 10 minutos.
...
### Se eu desabilitar o registro de usuários em "Cadastro e login", ainda posso usar magic link para convidar usuários? \{#if-i-disable-user-registration-in-sign-in-experience-can-i-still-use-magic-link-to-invite-users}

</summary>

Sim, você ainda pode usar magic link para convidar usuários mesmo que desative o registro de usuários em "Cadastro e login".
...
1. O usuário já está autenticado e clica em um magic link associado à conta do usuário atual. Nesse caso, o Logto ainda verificará o token de uso único e provisionará o usuário para as organizações especificadas, se necessário.
2. O usuário já está autenticado e clica em um magic link associado a uma conta diferente. Nesse caso, o Logto solicitará ao usuário que continue como a nova conta ou volte ao aplicativo com a conta atual.
   1. Se o usuário optar por continuar como a nova conta, o Logto trocará para a nova conta após a verificação bem-sucedida do token.
...
3. Se seu prompt de login estiver definido como "login" ou contiver "login", o Logto fará automaticamente o login da conta associada ao token de uso único sem solicitar a troca. Isso ocorre porque o prompt "login" indica uma intenção explícita de autenticação, que tem precedência sobre a sessão atual.
