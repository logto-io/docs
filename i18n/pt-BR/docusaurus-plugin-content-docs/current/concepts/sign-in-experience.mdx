...
A experiência de login é o processo de autenticação de usuário no Logto. O processo pode ser simplificado da seguinte forma:
...
Embora o processo seja simples, a parte de redirecionamento pode parecer exagerada às vezes. No entanto, pode ser benéfico e seguro de várias maneiras. Explicaremos as razões nas seções seguintes.
...
Para aplicativos nativos, redirecionar para o navegador do sistema é uma maneira segura de autenticar usuários e possui suporte embutido tanto para iOS quanto para Android.
...
O Logto aplica medidas de segurança rigorosas, como [PKCE](https://tools.ietf.org/html/rfc7636), e desativa fluxos inseguros como o fluxo implícito. O redirecionamento é uma maneira segura de autenticar usuários e pode prevenir muitos ataques comuns.

## E se eu precisar mostrar alguns componentes de login no meu aplicativo?

Às vezes, sua equipe pode querer mostrar alguns componentes de login no aplicativo, como um botão "Entrar com Google". Isso pode ser alcançado usando o recurso "Direct sign-in" no Logto.
...
#### Processo de clicar em "Entrar com Google"
...
sequenceDiagram
  ...
  A->>B: Redirecionar com parâmetros de direct sign-in
...
Em resumo, o recurso de direct sign-in é uma maneira de automatizar algumas interações na experiência de login sem alterar o nível de segurança.

### Usar direct sign-in no seu aplicativo

Para usar o direct sign-in, você precisa passar o parâmetro `direct_sign_in` ao iniciar o método de login. O valor deve ser composto de um formato que o Logto reconheça. Por exemplo, para entrar com o Google, o valor deve ser `social:google`.

Em alguns SDKs oficiais do Logto, há uma opção dedicada para direct sign-in. Aqui está um exemplo de uso de direct sign-in no SDK JavaScript `@logto/client`:
...
Para mais detalhes, consulte [Direct sign-in](/end-user-flows/authentication-parameters/direct-sign-in).

:::info
Estamos implementando gradualmente esse recurso em todos os SDKs oficiais do Logto. Se você não o vê no seu SDK, sinta-se à vontade para nos contatar.
:::

## Preciso que meus usuários preencham suas credenciais no meu aplicativo
