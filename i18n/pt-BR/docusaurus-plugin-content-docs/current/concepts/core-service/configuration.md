...
| ENDPOINT                | `'protocol://localhost:$PORT'`       | `string`                                                 | Você pode especificar uma URL com seu domínio personalizado para testes online ou produção. Isso também afetará o valor do [identificador do emissor OIDC (Issuer)](https://openid.net/specs/openid-connect-core-1_0.html#IssuerIdentifier).                                                                  |
...
`HTTPS_CERT_PATH` indica o caminho para o seu certificado HTTPS, enquanto `HTTPS_KEY_PATH` indica o caminho para sua chave HTTPS.
