...
# RBAC 實務應用：為你的應用程式實作安全的授權 (Authorization)

你是否在為你的應用程式實作一個安全且可擴展的授權 (Authorization) 系統而苦惱？角色型存取控制 (RBAC, Role-based Access Control) 是管理使用者權限的業界標準，但正確實作卻充滿挑戰。本教學將以實際的內容管理系統（CMS, Content Management System）為例，帶你建立一套健全的 RBAC 系統。

跟著本指南，你將學會：

- ✨ 如何設計並實作細緻的權限，精確掌控存取行為
- 🔒 將權限有條理地組織成有意義的角色的最佳實踐
- 👤 有效處理資源擁有權的技巧
- 🚀 讓你的授權 (Authorization) 系統具備可擴展性與可維護性的方式
- 💡 以真實 CMS 範例實作的實用技巧

本教學的完整原始碼可在 [GitHub](https://github.com/logto-io/rbac-sample) 取得。

## 理解 RBAC 基礎 \{#understanding-rbac-fundamentals}

角色型存取控制 (RBAC) 不僅僅是將權限分配給使用者，更是建立一套兼顧安全與可維護性的結構化授權 (Authorization) 方法。
...
細緻的權限讓你能精確控制使用者在系統中的操作。與其僅用「admin」或「user」這類廣泛的存取層級，我們會定義使用者可對資源執行的具體動作。例如：
...
資源擁有權是我們 CMS 授權 (Authorization) 設計中的核心概念。RBAC 定義不同角色可執行的動作，而擁有權則為存取控制增添個人層面：

- 作者自動擁有他們創建的文章的存取權
- 這種自然的擁有權模型讓作者永遠可以檢視與編輯自己的內容
- 系統在處理文章操作時，同時檢查角色權限或擁有權
- 例如，即使沒有 `update:articles` 權限，作者仍可編輯自己的文章
- 這種設計減少了額外角色權限的需求，同時維持安全性

這種雙層設計（角色 + 擁有權）讓系統更直覺且安全。發佈者與管理員仍可透過角色權限管理所有內容，而作者則掌控自己的作品。

## 設計安全的 API \{#designing-a-secure-apis}

讓我們從設計 CMS 核心功能的 API 端點開始：
...
注意，這些權限僅在存取非自己擁有的資源時需要。文章擁有者可自動：
...
現在我們已定義 API 與權限，可以依邏輯將權限分組建立角色：

| 權限 / 角色      | 👑 管理員 (Admin)        | 📝 發佈者 (Publisher)        | ✍️ 作者 (Author)     |
| ---------------- | ------------------------ | ---------------------------- | -------------------- |
| **說明**         | 全系統存取，完整內容管理 | 可檢視所有文章並控管發佈狀態 | 可在系統中建立新文章 |
| list:articles    | ✅                       | ✅                           | ❌                   |
| read:articles    | ✅                       | ✅                           | ❌                   |
| create:articles  | ✅                       | ❌                           | ✅                   |
| update:articles  | ✅                       | ❌                           | ❌                   |
...
**注意**：作者自動擁有自己文章的讀 / 編輯 / 刪除權限，無論角色權限如何。

每個角色都針對特定職責設計：

- **管理員 (Admin)**：全面掌控 CMS，包括所有文章操作
- **發佈者 (Publisher)**：專注內容審核與發佈管理
- **作者 (Author)**：專責內容創作
...
- Admin
  - 具備所有權限
- Publisher
  - 具備 `read:articles`、`list:articles`、`publish:articles`
- Author
  - 具備 `create:articles`
...
在使用者詳情的「角色」分頁中，你可以指派角色給該使用者。

本例中，我們建立 3 位使用者並分配如下角色：
...
:::note
本教學為示範，透過 Logto Console 建立這些資源與設定。實際專案中，你也可以透過 Logto 提供的 [Management API](/integrate-logto/interact-with-management-api) 程式化建立這些資源與設定。
:::

## 前端整合 Logto RBAC \{#integrate-your-frontend-with-logto-rbac}

現在我們已在 Logto 設定好 RBAC，可以開始將其整合進前端。

首先，依照 [Logto 快速上手](/quick-starts) 將 Logto 整合進你的應用程式。

本例以 [React](/quick-starts/react) 示範。
...
當使用者透過 Logto 登入並請求上述 API 資源的存取權杖 (Access token) 時，Logto 會將與使用者角色相關的 scopes（權限）加入存取權杖 (Access token)。

你可以透過 `useLogto` hook 的 `getAccessTokenClaims` 取得存取權杖 (Access token) 中的 scopes。
...
你可以利用 `userScopes` 判斷使用者是否有權限存取資源。
...
接下來，將 Logto RBAC 整合進你的後端。

### 後端授權 (Authorization) 中介軟體 \{#backend-authorization-middleware}

首先，我們需要在後端新增一個中介軟體，檢查使用者權限、驗證是否登入，以及是否具備存取特定 API 的必要權限。
...
如上所示，此中介軟體會驗證前端請求是否帶有有效的存取權杖 (Access token)，並檢查該權杖的受眾 (Audience) 是否與我們在 Logto Console 建立的 API 資源一致。

驗證 API 資源的原因在於，這個 API 資源實際上代表我們 CMS 後端的資源，所有 CMS 權限都與此 API 資源綁定。

由於這個 API 資源在 Logto 代表 CMS 資源，因此前端在呼叫後端 API 時，會帶上對應的存取權杖 (Access token)：
...
:::note
如果你發現無法用「使用者管理」建立的帳號登入，請先啟用對應的登入方式。前往 Logto Console 的「登入體驗」並啟用你偏好的驗證 (Authentication) 方式（如電子郵件 + 密碼或使用者名稱 + 密碼）。
:::

首先，分別以 Alex 與 Charles 登入並建立一些文章。

Alex 具備 Admin 角色，可建立、刪除、更新、發佈並檢視所有文章。

![CMS dashboard - Alex](https://uploads.strapi.logto.io/cms_dashboard_alex_a11863a94b.png)

Charles 具備 Author 角色，只能建立自己的文章，且僅能檢視、更新、刪除自己擁有的文章。

![CMS dashboard - Charles - Article list](https://uploads.strapi.logto.io/cms_dashboard_charles_05f610066a.png)

Bob 具備 Publisher 角色，可檢視與發佈所有文章，但無法建立、更新或刪除文章。
...
若需更進階的情境（如建構多租戶應用程式），Logto 提供完整的組織 (Organization) 支援。歡迎參考我們的指南 [打造多租戶 SaaS 應用程式：從設計到實作的完整指南](https://blog.logto.io/build-multi-tenant-saas-application) 了解如何實作組織層級的存取控制。
