...
- 運作方式：官方天氣應用程式透過 Logto 連接你的服務並驗證使用者。當 Alice 登入時，她會自動取得天氣資料的存取權，不需要額外的授權頁面，因為這是你信任的應用程式。

**第三方 AI agent**

- 你希望打造一個圍繞你服務的生態系，因此另一位開發者建立了「WeatherBot」（一個能提供天氣更新與預報的 AI 助手），並將其作為第三方 AI agent 整合進來。
- 運作方式：WeatherBot（第三方 AI agent）希望透過你的服務存取使用者的天氣資料。當 Alice 使用她的帳號登入 WeatherBot 時：
  ...
  - Alice 可以允許或拒絕這個存取請求。
  - 只有她同意授權的資料會分享給 WeatherBot，WeatherBot 無法在未經明確重新授權的情況下存取其他資料。

這種存取（權限）控制確保了使用者資料的安全，即使你的服務管理所有資料，像 WeatherBot 這樣的第三方 AI agent 也只能存取使用者明確允許的內容。他們無法繞過這個流程，因為這是由你在服務中實作的存取控制所強制執行。
...
| 第三方 AI agent  | WeatherBot 助手      | 是             | 其他開發者   |

:::note
如果你想將服務整合到你自己的 AI agent 或應用程式，請參閱我們的 [快速入門指南](/quick-starts) 以獲得更多資訊。
:::
...
- **AI agent**：將存取你服務的 AI agent。
- **Logto**：作為 OpenID Connect 提供者（授權伺服器），管理使用者身分。
...
sequenceDiagram
    ...
    Agent->>Service: 請求存取資源 (Request access to a resource)
    ...
    Logto->>Logto: 使用者授權（同意）AI agent 權限範圍 (User authorizes (consents) scopes to the AI agent)
...
要讓第三方 AI agent 存取你的服務，請依照以下步驟操作：

1. **在 Logto 註冊 AI agent**：在 Logto 建立一個應用程式來代表該 AI agent。
2. **設定 AI agent**：確保 AI agent 能向你的服務發送請求，並正確處理 401 Unauthorized 回應。
3. **實作授權流程**：AI agent 應能處理 OAuth 2.0 授權流程，從 Logto 取得存取權杖 (Access token)。
4. **測試整合**：使用 AI agent 存取你的服務，並驗證其能成功驗證與授權請求。

<ConfigureThirdPartyAiAgent />

## 在你的服務中設定授權 \{#set-up-authorization-in-your-service}

要在你的服務中啟用存取控制，需實作以下內容：

1. **在 Logto 定義 API 資源**：在 Logto 建立代表 AI agent 將存取端點的 API 資源。
2. **實作存取控制邏輯**：在你的服務中，實作邏輯以驗證從 AI agent 收到的存取權杖，並檢查使用者是否有權存取請求的資源。
3. **回應 AI agent**：若存取權杖有效且使用者有權限，則回傳請求的資源；否則回傳 401 Unauthorized 回應。

想進一步瞭解如何在服務中實作存取控制，請參閱我們的 [授權指南](/authorization)。
...
4. AI agent 應能處理 401 Unauthorized 回應並將使用者導向 Logto 進行驗證。
5. 驗證成功後，AI agent 應取得存取權杖並用於向你的服務發送請求。
