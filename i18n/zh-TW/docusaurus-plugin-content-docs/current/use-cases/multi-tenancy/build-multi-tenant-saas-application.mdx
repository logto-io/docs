...
description: 學習如何高效建立具備強大驗證 (Authentication)、組織管理與角色型存取控制 (RBAC) 的多租戶 SaaS 應用程式。
---
...
Notion、Slack、Figma 這類應用程式是怎麼打造的？這些多租戶 SaaS 應用看起來簡單易用，但要自己動手做？那又是另一回事了。

當我第一次想要打造這樣一個複雜的系統時，腦袋直接炸裂：
...
「老闆，產品設計兩週後再談，我現在卡死在泥沼裡。」

但當我真的開始動手做時，**我發現其實沒那麼可怕。**

我只花了很少的力氣，就**做出了一個具備所有這些功能的系統！**
...
我會帶你從零開始設計並實作這樣的系統——你會驚訝於 2025 年用現代工具和正確架構，這一切其實很簡單。
...
平台提供強大的 AI 文件管理能力，包括自動摘要、重點提取、組織內智能內容推薦等功能。
...
要實現多租戶架構，你需要一個稱為 **組織 (Organization)** 的實體層。想像有一個使用者池，使用者可以存取多個工作區。每個組織代表一個工作區，使用者只需一個身分即可根據分配的角色存取不同工作區（組織）。

![multi-tenant-app-architecture.svg](https://uploads.strapi.logto.io/multi_tenant_app_architecture_89ed9779f8.svg)

這是驗證 (Authentication) 服務常見的功能。身分管理系統中的組織對應到 SaaS 應用的 workspace、project 或 tenant。

![organization-examples.png](https://uploads.strapi.logto.io/organization_examples_dc60ad68bb.png)

### 成員資格 \{#membership}

成員（Member）是一個暫時性概念，用來表示某個身分在組織中的成員狀態。

例如，Sarah 用她的 email **sarah@gmail.com** 註冊你的應用。她可以屬於不同的工作區。如果 Sarah 屬於 **Workspace A**，但不屬於 **Workspace B**，那她就是 **Workspace A** 的成員，而不是 **Workspace B** 的成員。

### 角色 (Role) 與權限 (Permission) 設計 \{#role-and-permission-design}

在多租戶架構下，使用者需要有**角色 (Role)** 與特定**權限 (Permission)** 來存取租戶資源。
權限 (Permission) 是細緻的存取控制，定義具體動作，例如 `read: order` 或 `write: order`，決定能對特定資源執行哪些操作。

角色 (Role) 則是在多租戶環境下分配給成員的一組權限。

你需要定義這些角色與權限，然後將角色分配給使用者，有時還會包含自動化流程。例如：

1. 加入組織的使用者自動獲得 **member** 角色。
2. 第一個建立工作區的使用者自動分配 **admin** 角色。

### 註冊與登入流程 \{#sign-up-and-login-flow}

確保註冊與驗證 (Authentication) 流程既友善又安全，包括基本登入與註冊選項：
...
在多租戶 SaaS 應用中，使用者流程的一大差異是需要支援租戶建立與成員邀請。這個流程對產品啟動與成長至關重要，需要仔細規劃與實作。
...
這些是幾乎每個 SaaS 應用都會遇到的常見情境。你可以參考這些流程啟發產品與設計團隊，也可根據需求自訂流程。
...
現代驗證 (Authentication) 服務（這次我選 [Logto](https://logto.io/)）已經幫我們包好所有功能。驗證流程非常直觀：

```mermaid
sequenceDiagram
  actor u as 使用者 (User)
  participant a as 應用程式 (App)
  participant auth as 驗證服務 (AuthProvider)
...
**從數週開發縮短到 15 分鐘設定**，Logto 幫我們處理所有複雜流程！後續會在實作章節介紹整合步驟。現在我們可以專注於 DocuMind 核心功能！

### 建立多租戶架構 \{#establish-multi-tenant-architecture}

組織 (Organization) 系統讓使用者可以建立並加入多個組織。來看看核心關係：
...
**角色型存取控制 (RBAC, Role-Based Access Control)** 對於多租戶 SaaS 應用的安全與擴展性至關重要。

在多租戶應用中，權限與角色設計通常與產品設計一致。例如多個工作區通常會有 admin 與 member 角色。Logto 作為驗證 (Authentication) 服務，提供以下組織層級 RBAC 設計：

1. **統一權限定義**：權限於系統層級定義，所有組織一致，方便維護與管理
...
由於每個使用者在每個組織中都需要自己的角色，角色與組織的關係必須能反映每個使用者的分配情況：
...
為什麼要前後端分離？因為架構清晰、易學易換技術棧。驗證 (Authentication) 服務以 Logto 為例。

接下來的教學，**這些模式適用於任何前端、任何後端、任何驗證 (Authentication) 系統。**

## 為你的應用新增基本驗證 (Authentication) 流程 \{#add-basic-authentication-flow-to-your-app}

這是最簡單的一步。我們只需將 Logto 整合進專案，然後在 Logto Console 根據需求設定登入 / 註冊方式。

### 安裝 Logto 到你的應用 \{#install-logto-to-your-app}

首先登入 [Logto Cloud](https://cloud.logto.io/)。沒有帳號可以免費註冊。建立一個 Development Tenant 進行測試。

在 Tenant Console 點選左側「應用程式」按鈕，選擇 React 開始建立應用。
...
一個實用小技巧：我們的登入頁同時有「登入」與「註冊」按鈕。註冊按鈕會直接導向 Logto 的註冊頁。這是透過 Logto 的 [first screen](/end-user-flows/authentication-parameters/first-screen) 功能實現的，可以決定驗證 (Authentication) 流程的第一步。

如果你的產品預期新用戶很多，可以預設顯示註冊頁。
...
點擊登入後會跳轉到 Logto 登入頁。登入（或註冊）成功後，恭喜！你的應用有了第一位使用者（就是你自己）！

當你想讓使用者登出時，只需呼叫 `useLogto` hook 的 `signOut` 函式即可。
...
在 Logto Console 點選左側「登入體驗 (Sign-in Experience)」，再點「註冊與登入」分頁。
...
在 Logto 啟用 MFA 很簡單。只需在 Logto Console 點選「多重要素驗證 (Multi-factor auth)」，然後在多重要素驗證頁啟用即可。
...
一切都超簡單！我們只花幾分鐘就建立了一個複雜的使用者驗證 (Authentication) 系統！
...
每個使用者都可以從 Logto 取得自己的組織資訊，這就實現了多租戶支援。

### 取得使用者的組織資訊 \{#get-a-user-s-organization-information}

要從 Logto 取得使用者的組織資訊，分兩步：

在 Logto Config 宣告組織資訊存取權，設定正確的 `scopes` 與 `resources`。
...
完成這些步驟後，你需要登出再登入一次，因為我們修改了請求的 scope 與 resource。

此時你還沒建立任何組織，使用者也沒加入任何組織，儀表板會顯示「你還沒有任何組織」。

![dashboard-no-orgs.png](https://uploads.strapi.logto.io/dashboard_no_orgs_94678fe16f.png)

接下來，我們要為使用者建立一個組織並將其加入。

多虧 Logto，我們不用自己實作複雜的組織關係，只需在 Logto 建立組織並加入使用者即可。建立組織有兩種方式：
...
點選 Logto Console 左側「組織 (Organizations)」選單，建立一個組織。
...
進入組織詳情頁，切換到「成員 (Members)」分頁，點「+ 新增成員」，從左側列表選擇你的登入使用者，點右下角「新增成員」。現在你已成功將使用者加入組織。
...
請參考 [與 Management API 互動](/integrate-logto/interact-with-management-api) 文件設置與 Logto 的 API 通訊。
...
2. **存取 Logto Management API**：
   ...
   - 依照 [與 Management API 互動](/integrate-logto/interact-with-management-api) 指南設置
...
後端服務 API 只允許已驗證 (Authentication) 使用者存取。我們需要用 Logto 保護 API，也要知道目前使用者資訊（如 user ID）。

在 Logto（以及 OAuth 2.0）概念中，後端服務是資源伺服器 (resource server)。使用者從前端帶著 Access token 存取 DocuMind 資源伺服器，資源伺服器驗證權杖，通過才回傳資源。

我們先建立一個 API Resource 來代表後端服務。

進入 Logto Console。

1. 點選右側「API 資源 (API resources)」
2. 點「建立 API 資源」，彈窗選 Express
3. API 名稱填「DocuMind API」，API 識別碼填「[https://api.documind.com](https://api.documind.com/)」
4. 點建立

不用擔心這個 API 識別碼 URL，它只是 Logto 內部唯一識別用，跟實際後端服務 URL 無關。

你會看到一份 API 資源教學，可以照著做，也可以參考下方步驟。

我們建立一個 requireAuth middleware 來保護 POST /organizations 端點。
...
到你的 Logto tenant 的 OpenID Configuration endpoint 取得。網址為 `https://<your-tenant-id>.logto.app/oidc/.well-known/openid-configuration`，回傳 JSON 內容如下：
...
我們現在可以在前端從 Logto 取得權杖，讓使用者透過這個權杖呼叫後端服務建立組織。middleware 也會給我們 user ID，方便後續加入組織。

在前端程式碼中，於 Logto config 宣告這個 API resource，將其識別碼加入 resources 陣列。
...
const config: LogtoConfig = {
  ...
  resources: [ReservedResource.Organization, "<https://api.documind.com>"], // 新增 API 資源識別碼
};
...
在 Dashboard 建立組織時，取得 Logto Access Token，並用這個權杖呼叫後端服務 API。
...
如同前端呼叫後端服務，後端服務呼叫 Logto 也需要 Access token。

在 Logto，我們用機器對機器 (Machine-to-Machine) 驗證 (Authentication) 取得 access token。詳見 [與 Management API 互動](/integrate-logto/interact-with-management-api)。

進入 Logto Console 的應用程式頁，建立一個 Machine-to-Machine 應用，分配「Logto Management API access」角色，複製 Token endpoint、App ID、App Secret，後續取得 access token 會用到。

![m2m-application.png](https://uploads.strapi.logto.io/m2m_application_d882b6386e.png)

現在我們可以用這個 M2M 應用取得 Logto Management API 的 access token。
...
我們已經用 Logto Management API 實作組織建立與成員加入。
...
下一步是邀請使用者加入組織。本教學暫不實作這個功能。你已經知道如何用 Management API，可以參考 [**租戶建立與邀請**](https://blog.logto.io/build-multi-tenant-saas-application#tenant-creation-and-invitation) 作為產品設計參考，並依照這篇部落格 [如何在多租戶應用實作使用者協作](https://blog.logto.io/implement-user-collaboration-in-your-app) 輕鬆實作。
...
- 使用者只能存取自己組織的資源：可用 Logto 的「組織權杖 (Organization token)」實現
- 使用者在組織內有特定角色（含不同權限），可執行授權操作：可用 Logto 的組織範本 (Organization template) 功能實現

來看看如何實作。

### 使用 Logto 組織權杖 (Organization token) \{#using-logto-organization-token}

類似前面提到的 Logto 存取權杖 (Access token)，Logto 會針對特定資源發放 access token，使用者用這個權杖存取後端受保護資源。對應地，Logto 也會針對特定組織發放組織權杖 (Organization token)，使用者用這個權杖存取後端受保護的組織資源。

在前端應用中，可以用 Logto 的 `getOrganizationToken` 方法取得存取特定組織的權杖。
...
1. 如果傳給 `getOrganizationToken` 的 `organizationId` 不是目前使用者所屬組織 id，這個方法無法取得權杖，確保使用者只能存取自己的組織。
2. 存取組織資源時用組織權杖而非 access token，因為組織資源要用組織權限控管，而不是單純的使用者權限（等下實作 `GET /documents` API 會更清楚）。

接下來在後端服務建立 `GET /documents` API。類似用 API resource 保護 `POST /organizations`，這裡用組織專屬資源標示符 (resource indicator) 保護 `GET /documents`。

先建立 `requireOrganizationAccess` middleware 來保護組織資源。
...
有些軟體需要組織間資料隔離，進一步討論與實作可參考：[用 PostgreSQL 實作多租戶：從簡單實例學習](https://blog.logto.io/implement-multi-tenancy)。

### 實作組織層級角色型存取控制 (RBAC) 設計 \{#implement-organization-level-role-based-access-control-design}
...
這時就要用到 Logto 的組織範本 (Organization template) 功能。

組織範本是一份每個組織都適用的存取控制藍圖：定義所有組織通用的角色與權限。

> 為什麼要組織範本？
>
> 因為 SaaS 產品最重要的需求之一就是可擴展性，也就是說，一個客戶適用的權限模型，所有客戶都適用。

進入 Logto Console > 組織範本 > 組織權限，建立兩個權限：`read:documents` 與 `create:documents`。
...
然後建立 POST /documents API，並用 `requireOrganizationAccess` middleware 加上 requiredScopes 設定來保護這個 API 及前面的 `GET /documents` API。
...
在前端，可以解碼組織權杖或用 Logto 的 `getOrganizationTokenClaims` 方法取得使用者權限資訊。
...
## 加入更多多租戶應用功能 \{#add-more-multi-tenant-app-features}

到目前為止，我們已經實作了多租戶 SaaS 系統的基本使用者與組織功能！但還有一些功能尚未涵蓋，例如每個組織自訂登入頁品牌、特定網域 email 自動加入組織、整合企業級單一登入 (Enterprise SSO) 等。
...
還記得一開始那種壓力山大的感覺嗎？使用者、組織、權限、企業功能……看起來像座爬不完的高山。

但看看我們完成了什麼：

- 完整的驗證 (Authentication) 系統，支援多種登入方式與 MFA
- 彈性的組織系統，支援多重成員資格
- 組織內角色型存取控制 (RBAC)

最棒的是？我們不用重造輪子。善用 Logto 這類現代工具，原本要花數月開發的功能，現在幾分鐘就搞定。

本教學完整原始碼請見：[Multi-tenant SaaS Sample](https://github.com/logto-io/multi-tenant-saas-sample)。

這就是 2025 年現代開發的威力——我們可以專注於打造獨特產品功能，而不是和基礎設施搏鬥。現在輪到你打造屬於自己的精彩產品！

探索 Logto 的所有功能，從 Logto Cloud 到 Logto OSS，請造訪 [Logto 官方網站](https://logto.io/)，或立即註冊 [Logto cloud](https://cloud.logto.io/?sign_up)。
