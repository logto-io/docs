---
sidebar_position: 7
sidebar_custom_props:
  sublist_label: 帳號流程 (Account flows)
---

import GearIcon from '@site/src/assets/gear.svg';

# 帳號設定 (Account settings)

Logto 提供兩組帳號設定 API，讓使用者能管理儲存在 Logto 的帳號與個人資料。

## 使用 Account API（推薦） \{#use-account-apis-recommended}

Logto 的 Account API 是現成可用的前端端點，讓終端使用者能安全地檢視與更新自己的資訊，並內建權限檢查。只需將其嵌入你的用戶端應用程式，即可打造完善的自助帳號設定頁面。

主要特色：

- **終端使用者設定**：使用者可管理自己的登入識別碼與憑證、社交帳號、MFA 方法，以及個人資料。
- **用戶端整合**：設計為可安全直接在前端使用。
- **極簡設定**：即插即用端點，無需自訂伺服器。
- **權限控制**：可透過 Management API 設定啟用哪些 Account API。

<DocCardList
  items={[
    {
      type: 'link',
      label: '使用 Logto Account API（推薦）',
      href: '/end-user-flows/account-settings/by-account-api',
      description: '深入瞭解如何使用使用者 Account API 建立你自己的帳號設定頁面。',
      customProps: {
        icon: <GearIcon />,
      },
    },
  ]}
/>

## 使用 Management API \{#use-management-apis}

Management API 是 Logto 的核心管理介面，僅限管理員或後端服務存取。它提供最大彈性與完整的 CRUD 控制權，讓你能自訂管理工具。如果你需要完全自訂的自助入口或非標準的使用者管理功能，可以在你自有的「Account API」層後方開放部分 Management API 端點，並以你的業務驗證邏輯保護它們。

主要特色：

- **僅限管理員存取**：適用於開發者與後台系統
- **完整使用者生命週期**：建立、查詢、更新、刪除、停用或還原帳號
- **進階操作**：產生個人存取權杖、模擬使用者、連接 OAuth 應用程式、自訂工作流程等

<DocCardList
  items={[
    {
      type: 'link',
      label: '使用 Logto Management API',
      href: '/end-user-flows/account-settings/by-management-api',
      description: '深入瞭解如何使用使用者 Management API 建立你自己的帳號設定頁面。',
      customProps: {
        icon: <GearIcon />,
      },
    },
  ]}
/>

## Account API vs. Management API \{#account-api-vs-management-api}

| 功能 (Feature) | Account API                                                                 | Management API                                                                            |
| -------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **目標使用者** | 終端使用者 (End users)                                                      | 管理員 / 開發者 (Admins / Developers)                                                     |
| **存取情境**   | 用戶端 / 前端 (Client-side / front-end)                                     | 伺服器端 / 後端 (Server-side / back-end)                                                  |
| **權限模型**   | 可透過 Management API 切換啟用哪些 Account API。                            | 開發者可完全自訂                                                                          |
| **支援功能**   | 檢視、更新、刪除：使用者名稱、電子郵件、電話、密碼、社交帳號、MFA、個人資料 | 所有基本設定 + 刪除 / 停用 / 還原帳號、個人存取權杖、使用者模擬、連接 OAuth 應用程式等    |
| **設定複雜度** | 非常低（即插即用）                                                          | 中至高（需自訂實作）                                                                      |
| **適用時機**   | 快速在用戶端應用程式推出安全的自助帳號設定頁，且設定極簡時。                | 當 Account API 無法滿足需求時，例如複雜的帳號刪除邏輯、高風險操作或需打造後台管理工具時。 |
