...
Logto 提供一套預建的帳號中心 UI，為終端使用者提供現成的帳號設定管理頁面。這個預建 UI 由 Logto 託管，處理常見的帳號管理任務，包括：

- 檢視與編輯使用者個人檔案（姓名、大頭貼、自訂檔案欄位）
- 更新電子郵件地址與手機號碼
- 更新使用者名稱
- 設定或更新密碼
- 管理社交連結（連結 / 解除連結社交帳號）
...
預建帳號中心 UI 設計上可無縫整合至你的應用程式，提供一致的使用體驗，無需自行開發帳號管理頁面。
...
| 路徑                                 | 說明                                             |
| ------------------------------------ | ------------------------------------------------ |
| `/account/profile`                   | 使用者個人檔案頁（姓名、大頭貼、自訂檔案欄位）   |
...
| `/account/backup-codes/generate`     | 產生新備用代碼                                   |
...
帳號中心會顯示 **個人檔案** 頁面，讓終端使用者檢視與管理個人資訊。若要控制哪些個人檔案欄位顯示於此頁：
...
在你的應用程式中新增連結，將使用者導向預建頁面。加上 `redirect` 查詢參數，讓使用者完成操作後返回你的應用程式：
...
| `show_success` | 設為 `true` 時，成功導向（如你的 `redirect` URL）會收到 `show_success` 查詢參數，方便你顯示確認訊息。                         |
| `identifier`   | 預先填入目標頁（`/account/email`、`/account/phone`、`/account/username`）的識別欄位。適合你已知使用者識別資訊時深度連結使用。 |
| `ui_locales`   | 以空格分隔的 BCP-47 語言標籤清單（如 `fr-CA fr en`），控制帳號中心 UI 語言。若未指定則回退至使用者瀏覽器語言。                |

範例——深度連結至電子郵件更新頁，預填現有電子郵件並顯示法文 UI：
...
帳號中心不直接處理帳號刪除。你可以設定一個 **帳號刪除 URL**，指向你自己的刪除流程（通常由 [Management API](/end-user-flows/account-settings/by-management-api) 支援）。設定後，帳號中心安全頁會出現 **刪除你的帳號** 項目，並連結至你的 URL。

設定方式：前往 <CloudLink to="/sign-in-experience/account-center">主控台 > 登入與帳號 > 帳號中心</CloudLink>，填寫 **帳號刪除 URL** 欄位。你也可以透過 Management API 更新：
...
其中 `{connectorId}` 為社交連接器的 ID。這就是 <CloudLink to="/connectors/social">主控台 > 連接器 > 社交連接器</CloudLink> 詳細頁顯示的 **Callback URI**，你在 [設定連接器](/connectors/social-connectors/) 時需向社交服務註冊。
...
你可以進一步透過自訂 CSS 客製帳號中心 UI 外觀。前往 <CloudLink to="/sign-in-experience/account-center">主控台 > 登入與帳號 > 帳號中心</CloudLink>，在 **自訂 CSS** 編輯器中新增你的 CSS。

預建帳號中心 UI 在主要 UI 元素（版面容器、頁首、區塊、卡片等）上提供以 `logto_ac-` 為前綴的穩定 CSS 類名，方便你精準套用樣式。這讓你能安全覆蓋預設樣式，不必擔心版本間類名變動。
...
- [MFA 信任裝置](/end-user-flows/mfa/trusted-devices) — 設定與管理時效性瀏覽器信任
