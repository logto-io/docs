...
:::note
以下設定專用的 Account API 即將推出：MFA、多重要素驗證 (MFA)、單一登入 (SSO)、自訂資料（使用者）、帳號刪除。在此之前，你可透過 Logto Management API 實作這些功能。詳情請見 [透過 Management API 管理帳號設定](/end-user-flows/account-settings/by-management-api)。
:::
...
更多驗證相關說明，請參閱 [透過 Account API 進行安全驗證](/end-user-flows/security-verification/by-account-api)。
...
:::note
使用此方式前，需先[設定電子郵件連接器](/connectors/email-connectors/)或[設定簡訊連接器](/connectors/sms-connectors/)，並確保已配置 `UserPermissionValidation` 範本。
:::
...
:::note
使用此方式前，需先[設定電子郵件連接器](/connectors/email-connectors/)，並確保已配置 `BindNewIdentifier` 範本。
:::
...
:::note
使用此方式前，需先[設定簡訊連接器](/connectors/sms-connectors/)，並確保已配置 `BindNewIdentifier` 範本。
:::
...
更多相關 origins 說明，請參閱 [Related Origin Requests](https://passkeys.dev/docs/advanced/related-origins/) 文件。
...
- `verification_record_id`：有效的驗證記錄 ID，需先驗證使用者現有因素，詳見 [取得驗證記錄 ID](#get-a-verification-record-id)。
