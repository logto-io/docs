...
SSO 帳號檢視與帳號刪除功能目前僅能透過 Logto Management API 使用。詳見 [透過 Management API 管理帳號設定](/end-user-flows/account-settings/by-management-api)。
...
1. **安全性欄位**：
   ...
   - 若要使用 WebAuthn 金鑰作為 MFA，請將前端應用程式網域加入 **WebAuthn 相關來源**，以便帳號中心與登入體驗共用金鑰。詳見 [連結新 WebAuthn 金鑰](#link-a-new-webauthn-passkey)。
2. **個人資料欄位**：
   - 包含：使用者名稱、姓名、大頭貼、[profile](/user-management/user-data#profile)（其他標準個人屬性）、[custom data](/user-management/user-data#custom-data)。
   - 終端使用者可直接編輯，無需額外驗證。
3. **密鑰保管庫 (Secret vault)**：針對 OIDC 或 OAuth 社交與企業連接器，Logto [密鑰保管庫](/secret-vault/federated-token-set) 會在驗證後安全儲存第三方存取權杖與重新整理權杖。應用程式可直接呼叫外部 API（如同步 Google Calendar 事件），無需再次提示使用者登入。啟用 Account API 後即可自動取得權杖。
...
若未使用官方 SDK，請將存取權杖請求 `/oidc/token` 的 `resource` 設為空。
...
驗證成功後，即可呼叫 [`PATCH /api/my-account/primary-email`](https://openapi.logto.io/operation/operation-updateprimaryemail) 更新使用者電子郵件，並將 `verificationId` 以 `newIdentifierVerificationRecordId` 放入請求主體。

:::info[兩組不同的驗證紀錄 ID]

此請求需兩組不同的驗證紀錄 ID：

- **`logto-verification-id`（標頭）**：證明使用者身分，取得方式為 [驗證使用者密碼](#verify-the-users-password) 或 [發送驗證碼至現有電子郵件或手機](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)。
- **`newIdentifierVerificationRecordId`（主體）**：證明新電子郵件擁有權，為上述 `POST /api/verifications/verification-code` 回傳的 `verificationRecordId`。
...
:::tip
與註冊時收集電子郵件相同，透過 Account API 連結的電子郵件必須通過你在 <CloudLink to="/security/blocklist">Console > 安全性 > 封鎖清單</CloudLink> 設定的 [封鎖清單](/security/blocklist) 驗證。若違反政策，Logto 會拒絕請求並回傳詳細錯誤。
:::
...
與更新電子郵件類似，可使用 [`PATCH /api/my-account/primary-phone`](https://openapi.logto.io/operation/operation-updateprimaryphone) 端點更新或連結新手機，並可使用 [`DELETE /api/my-account/primary-phone`](https://openapi.logto.io/operation/operation-deleteprimaryphone) 端點移除使用者手機。

### 連結新社交帳號 \{#link-a-new-social-connection}

要連結新社交帳號，需先透過 [`POST /api/verifications/social`](https://openapi.logto.io/operation/operation-createverificationbysocial) 取得授權網址。
...
- **`logto-verification-id`（標頭）**：證明使用者身分，取得方式為 [驗證使用者密碼](#verify-the-users-password) 或 [發送驗證碼至現有電子郵件或手機](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)。
- **`newIdentifierVerificationRecordId`（主體）**：標識要連結的社交身分，為上述 `POST /api/verifications/social` 回傳的 `verificationRecordId`。
...
### 連結新 WebAuthn 金鑰 \{#link-a-new-webauthn-passkey}
...
使用 [`POST /api/verifications/web-authn/registration`](https://openapi.logto.io/operation/operation-generatewebauthnregistrationoptions) 端點請求註冊新金鑰。Logto 允許每個帳號註冊多把金鑰。
...
以 [`@simplewebauthn/browser`](https://simplewebauthn.dev/) 為例，可用 `startRegistration` 函式於本地瀏覽器註冊金鑰。
...
使用 [`POST /api/verifications/web-authn/registration/verify`](https://openapi.logto.io/operation/operation-verifywebauthnregistration) 端點驗證金鑰註冊。
...
最後，可使用 [`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification) 端點將金鑰連結至使用者帳號。
...
### 管理現有 WebAuthn 金鑰 \{#manage-existing-webauthn-passkeys}

管理現有 WebAuthn 金鑰，可使用 [`GET /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-getmfaverifications) 端點取得目前金鑰與其他 MFA 驗證因子。
...
更新金鑰名稱可使用 [`PATCH /api/my-account/mfa-verifications/{verificationId}/name`](https://openapi.logto.io/operation/operation-updatemfaverificationname) 端點：
...
刪除金鑰可使用 [`DELETE /api/my-account/mfa-verifications/{verificationId}`](https://openapi.logto.io/operation/operation-deletemfaverification) 端點：
