...
或者你忘記密碼，想透過電子郵件快速驗證身分以登入 / 重設密碼。
在這些情境下，應用程式可以將「魔術連結」發送到你的電子郵件。當你點擊該連結時，將立即完成驗證 (Authentication)。

應用程式開發者可以利用一次性權杖組成魔術連結，並發送到終端使用者的電子郵件地址。
...
sequenceDiagram
  ...
  C -->> U: 使用者驗證 (Authentication) 完成並回傳使用者資訊
```
...
開始前，請確保你已有 Logto 實例，並已在應用程式伺服器與 Logto 端點間建立機器對機器連線（Management API 需要）。詳情請參閱 [Logto Management API](/integrate-logto/interact-with-management-api)。
...
魔術連結至少應包含權杖與使用者電子郵件作為參數，並導向你應用程式中的著陸頁，例如 `https://yourapp.com/landing-page`。
...
終端使用者點擊魔術連結並進入你的應用程式後，你可以從 URL 擷取 `token` 與 `email` 參數，然後呼叫 Logto SDK 的 `signIn()` 函式以觸發驗證 (Authentication) 流程。
...
導致目前使用者的驗證 (Authentication) 狀態遺失。

因此，建議額外指定 `clearTokens: false` 參數，以避免清除現有權杖。
若有指定，則需在登入回呼頁面手動清除權杖。
...
使用者驗證 (Authentication) 成功後會自動加入組織，並指派預設組織角色 (Roles)。
請參閱組織 (Organization) 詳細頁的「即時佈建 (Just-in-time provisioning)」區塊，並設定組織的預設角色 (Roles)。
...
### 我可以用魔術連結將使用者指派到特定角色 (Roles) 嗎？ \{#can-i-assign-users-to-specific-roles-using-the-magic-link}

</summary>

魔術連結驗證 (Authentication) 流程不支援直接指派角色 (Roles) 給使用者。但你可以透過 [Webhooks](/developers/webhooks) 與 [Management API](/user-management/manage-users#manage-roles-of-users) 在使用者註冊後更新其角色 (Roles)。
...
### 如果使用者已登入，然後又點擊另一個魔術連結會發生什麼事？ \{#what-will-happen-if-a-user-already-signed-in-and-then-click-another-magic-link}

</summary>

可能有以下幾種情境：

1. 使用者已登入，然後點擊與目前帳號關聯的魔術連結。此時，Logto 仍會驗證一次性權杖，並在需要時將使用者佈建到指定組織 (Organizations)。
2. 使用者已登入，然後點擊與不同帳號關聯的魔術連結。此時，Logto 會提示使用者切換為新帳號，或以目前帳號返回應用程式。
   1. 若使用者選擇切換為新帳號，Logto 會在權杖驗證成功後切換至新帳號。
   2. 若使用者選擇維持目前帳號，Logto 不會驗證權杖，並以目前帳號返回應用程式。
3. 若你的登入提示設為 "login" 或包含 "login"，Logto 會自動以一次性權杖關聯的帳號登入，且不會提示切換。
