...
有時你可能希望邀請新使用者加入你的應用程式 / 組織，而不需他們先建立帳號。
或者你忘記密碼，想透過電子郵件快速驗證身分來登入 / 重設密碼。
這些情境下，應用程式可以將「魔術連結」發送到你的電子郵件。當你點擊該連結時，將立即完成驗證 (Authentication)。
...
sequenceDiagram
  ...
  C ->> L: 請求一次性權杖
  L -->> C: 回傳一次性權杖
  C -->> U: 組成包含一次性權杖的魔術連結並發送到終端使用者電子郵件
...
魔術連結至少應包含權杖與使用者電子郵件作為參數，並導向你應用程式的著陸頁。
...
因此，你應額外指定 `clearTokens: false` 參數，以避免清除現有權杖。
...
可以，你可以用魔術連結邀請新使用者加入你的應用程式及組織。
若要邀請新使用者加入組織，只需在請求內容中指定 `jitOrganizationIds`。

使用者驗證成功後將自動加入組織，並指派預設組織角色。
請至組織詳細頁的「即時佈建 (Just-in-time provisioning)」區段，設定組織的預設角色。
...
魔術連結驗證 (Authentication) 流程不支援直接指派角色給使用者。但你可以透過 [Webhook](/developers/webhooks) 與 [Management API](/user-management/manage-users#manage-roles-of-users) 在使用者註冊後更新其角色。
...
### 如果我在「登入體驗」中停用使用者註冊，還能用魔術連結邀請使用者嗎？ \{#if-i-disable-user-registration-in-sign-in-experience-can-i-still-use-magic-link-to-invite-users}
...
2. 使用者已登入，點擊與不同帳號關聯的魔術連結。此時 Logto 會提示使用者切換到新帳號，或以目前帳號返回應用程式。
   1. 若使用者選擇切換到新帳號，Logto 會在權杖驗證成功後切換帳號。
   2. 若使用者選擇維持目前帳號，Logto 不會驗證權杖，並以目前帳號返回應用程式。
3. 若你的登入提示設為 "login" 或包含 "login"，Logto 會自動以一次性權杖關聯的帳號登入，不會提示切換。這是因為 "login" 提示表示明確的驗證 (Authentication) 意圖，優先於目前的會話。
