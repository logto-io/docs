...
Logto 採用 [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) 流程進行驗證 (Authentication)。有效的 OIDC 授權 (Authorization) 流程必須由你的應用程式啟動，例如，點擊應用程式的「登入」按鈕會向 Logto 發送 OIDC 授權請求，Logto 隨後建立安全的工作階段（Cookie）。

然而，若使用者直接存取 `/sign-in`，繞過工作階段驗證，則會在以下情況出現「未知工作階段」404 錯誤：

- 工作階段過期
- 使用者將登入連結加入書籤或分享
- 缺少授權 (Authorization) 上下文

為解決此問題，Logto 新增了 `unknownSessionRedirectUrl` 設定。當你設定此網址後，Logto 會自動將使用者重新導向到指定頁面，以重新啟動新的登入流程，而非顯示 404 錯誤。
...
若發生其他錯誤（如造訪無效路徑、社交回呼無工作階段、或開啟過期的一次性連結），Logto 會顯示預設錯誤頁面。為協助使用者，你可以在這些無法避免的錯誤頁面上顯示聯絡資訊。
