...
- **無效的一次性連結（magic link）**：[一次性權杖（magic link）](/end-user-flows/one-time-token)（如組織邀請或無密碼登入）已過期或已被使用。
...
Logto 使用 [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) 流程進行驗證 (Authentication)。有效的 OIDC 授權流程必須由你的應用程式啟動，例如，點擊應用程式的「登入」按鈕會向 Logto 發送 OIDC 授權請求，進而建立安全的工作階段（cookie）。
