...
[註冊 Alipay 開發者帳戶](https://certifyweb.alipay.com/certify/reg/guide#/)（如果你還沒有的話）。
...
6. 前往 [Alipay 客戶中心](https://b.alipay.com/index2.htm)，使用 Alipay 開發者帳戶登入。點擊頂部欄的「帳戶中心」(账号中心)，然後前往側邊欄底部的「APPID 綁定」(APPID 绑定)。輸入你在步驟 4 中創建的移動應用程式的 APPID，然後「添加綁定」(添加绑定)。
...
你可以在本地機器上使用 _openssl_ 生成密鑰對，方法是在終端中執行以下代碼片段。
...
我們假設你已經在應用程式中整合了 [Logto iOS SDK](/quick-starts/swift)。在這種情況下，事情非常簡單，你甚至不需要閱讀 Alipay SDK 文檔：
...
其中 `callbackScheme` 是可以導航到你的應用程式的 [自定義 URL Schemes](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app) 之一。

### Android \{#android}

我們假設你已經在應用程式中整合了 [Logto Android SDK](/quick-starts/android)。在這種情況下，事情非常簡單，你甚至不需要閱讀 Alipay SDK 文檔：
