...
4. 在左側導覽列選擇「基本資訊」>「憑證與基本資訊」以取得 `Client ID` 與 `Client Secret`
...
將你在前述 OAuth 應用程式詳情頁獲得的 _Client ID（原 AppKey 與 SuiteKey）_ 及 _Client Secret（原 AppKey 與 SuiteKey）_ 分別填入 `clientId` 與 `clientSecret` 欄位。

`scope` 目前支援兩個值：`openid` 與 `openid corpid`。`openid` 可在授權後取得使用者的 `userid`，而 `openid corpid` 可同時取得使用者 `id` 及登入流程中選擇的組織 `id`。多個值請以空格分隔。注意：需進行 URL 編碼。
...
就是這樣。釘釘連接器現在應該可以使用了。別忘了 [在註冊與登入中啟用連接器](https://docs.logto.io/docs/recipes/configure-connectors/social-connector/enable-social-sign-in/)。

啟用釘釘網頁連接器後，你可以再次登入你的應用程式來驗證是否運作正常。

:::note
請在開發過程中嚴格遵守釘釘開放平台的使用規範與開發指引。
:::

## 支援 \{#support}

如有任何疑問或需要進一步協助，請參閱 [釘釘開發者文件](https://open.dingtalk.com/document/orgapp/obtain-identity-credentials) 或聯繫釘釘技術支援。
