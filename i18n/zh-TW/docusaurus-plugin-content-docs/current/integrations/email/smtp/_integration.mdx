...
SMTP 是一種傳輸協議，不專屬於某些特定的電子郵件服務提供者，而是可以與所有提供者一起使用。

我們現在提供指南，說明如何使用 SMTP 連接器來發送電子郵件，以下是一些提供者的示例以便於你更好地理解：

- _Gmail_ 是全球最受歡迎的電子郵件服務供應商。
- _阿里雲郵件推送_ 和 _SendGrid 郵件_。有些人可能對這兩個電子郵件服務提供者比較熟悉，因為 Logto 團隊提供了相應的連接器；你可能對它們有大致的了解。

我們希望你能通過以下示例了解其他電子郵件供應商的設置 :rocket:
...
首先，我們假設你已經擁有一個 SendGrid 帳戶。如果沒有，請在 [SendGrid 網站](https://app.sendgrid.com/) 創建一個新帳戶。
...
### 配置阿里雲郵件推送帳戶 \{#configure-with-aliyun-direct-mail-account}
...
要檢查「發件人地址」，你可以在 [DirectMail 控制台](https://dm.console.aliyun.com/) 的左側導航窗格中找到入口。你應該能在這裡看到 `發件人地址` 和 `SMTP 密碼`。
...
你可以輸入一個電子郵件地址並點擊「發送」，以查看設置是否在「保存並完成」之前有效。
...
你還可以配置 [OAuth2 驗證選項](https://nodemailer.com/smtp/oauth2/) 和其他高級配置。詳情請參見[這裡](https://nodemailer.com/smtp/)。
