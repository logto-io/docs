...
## 先決條件 \{#test-mailgun-email-connector}

- 一個 [Mailgun](https://www.mailgun.com/) 帳戶
- 來自你的 Mailgun 帳戶的 API key，需要發送訊息（電子郵件）的權限。詳情請參閱 [Where Can I Find My API Key and SMTP Credentials?](https://help.mailgun.com/hc/en-us/articles/203380100-Where-Can-I-Find-My-API-Key-and-SMTP-Credentials-)。
...
- 在 `from` 欄位填入你想要用來發送電子郵件的電子郵件地址。此電子郵件地址必須在你的 Mailgun 帳戶中註冊。電子郵件地址格式應為 `Sender Name \<sender@example.com>`。
