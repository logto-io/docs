...
4. 你現在位於 [簡訊服務主控台頁面](https://dysms.console.aliyun.com/overview)，根據需求點選側邊欄的「國內消息」或「國際 / 港澳台消息」。
...
6. 提交簡訊簽名與範本申請後，需等待生效。此時可回到 [簡訊服務主控台頁面](https://dysms.console.aliyun.com/overview) 發送測試簡訊。若簽名與範本已可用，可直接測試；若尚未生效，阿里雲也提供測試範本。
   - 發送測試簡訊前，可能需先儲值小額金額。
...
1. 在 [簡訊服務主控台頁面](https://dysms.console.aliyun.com/overview) 右上角滑鼠移至頭像，進入「AccessKey 管理」，點擊「創建 AccessKey」。完成安全驗證後會取得一組「AccessKey ID」與「AccessKey Secret」，請妥善保存。
...
3. 填寫阿里雲簡訊連接器設定：
   ...
   - 你可以為不同情境新增多組簡訊連接器範本。以下為新增單一範本的範例：
     ...
     - `usageType` 欄位依不同用途填入 `Register`、`SignIn`、`ForgotPassword` 或 `Generic`。(`usageType` 是 Logto 用來識別用途的屬性。) 若要啟用完整使用者流程，需提供 `Register`、`SignIn`、`ForgotPassword` 與 `Generic` 四種 usageType 的範本。
4. 設定「嚴格手機區碼檢查」：
...
就是這麼簡單。別忘了 <MainSiteUrl href="/connectors/sms-connectors/#enable-phone-number-for-end-user-flows">在註冊與登入流程中啟用連接器</MainSiteUrl>。
