---
description: 快速整合 Logto，立即啟動你的身分與存取管理系統。享受驗證 (Authentication)、授權 (Authorization) 及多租戶管理於一身的完整方案。
---
...
歡迎來到 Logto 文件！Logto 是專為現代應用程式與 SaaS 產品設計的身分與存取管理（IAM, Identity and Access Management）解決方案，為你的應用程式提供安全、可擴展且可自訂的驗證 (Authentication) 與授權 (Authorization) 系統。
...
Logto 的能力建立在四大支柱之上，這些基礎讓你能輕鬆將各項功能整合進產品或業務中。
  ...
  items={[
    {
      ...
      description: '基於網頁的介面，用於設定與管理資源，快速建立登入體驗並輕鬆管理身分。',
      customProps: {
    ...
    },
    {
      ...
      href: '/end-user-flows',
      description:
        '提供完整的驗證 (Authentication) 流程與使用者介面，包含識別碼登入、社交登入、Passkey 登入、多重要素驗證 (MFA) 及透過 Logto Console 與 API 的高度自訂。',
      customProps: {
        icon: <ExperienceIcon />,
      },
    },
    ...
    },
    {
      ...
      description:
        '遠端 MCP 伺服器，將 AI 工具與 IDE 連接至 Logto Cloud，實現資源管理與驗證 (Authentication) 整合。',
      customProps: {
        icon: <McpIcon />,
      },
    },
  ...
  items={[
    {
      ...
      href: '/logto-cloud',
      description:
        '我們的雲端服務提供 Free、Pro 與 Enterprise 方案，滿足各類客戶需求。由 Logto 託管與維運，並提供 dev tenant 讓你完整測試所有功能。',
      customProps: {
        icon: <CloudIcon />,
      },
    },
    {
      ...
      href: '/logto-oss',
      description:
        '在你自己的基礎設施上自架 Logto。請參閱我們的 GitHub 頁面與開源專區以獲得更多資訊。',
      customProps: {
        icon: <OpenSourceIcon />,
      },
    },
...
歡迎加入 [Logto Discord](https://discord.com/invite/UEPaF3j5e6) 伺服器 —— 這裡是開發者與企業的活躍社群。參與討論、掌握最新動態並分享你的回饋。
