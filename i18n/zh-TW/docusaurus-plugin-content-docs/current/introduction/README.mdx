---
description: 快速整合 Logto，立即啟動你的身分與存取管理系統。一次享有驗證 (Authentication)、授權 (Authorization) 及多租戶管理。
---
...
Logto 的能力建立在四大支柱之上，這些基礎讓你能輕鬆將各項功能整合進產品或業務。
  ...
  items={[
    {
      ...
      description: '一個網頁介面，方便配置與管理資源，快速建立登入體驗並輕鬆管理身分。',
      customProps: {
    ...
    },
    {
      ...
      href: '/end-user-flows',
      description:
        '提供完整的驗證 (Authentication) 流程與使用者介面，包括識別碼登入、社交登入、Passkey 登入、多重要素驗證 (MFA) 及透過 Logto Console 與 API 的高度自訂。',
      customProps: {
        icon: <ExperienceIcon />,
      },
    },
    ...
    },
    {
      ...
      label: 'SDKs',
      href: '/quick-starts',
      description: '探索我們的端到端教學與 SDK，快速在你偏好的框架上開始使用。',
      customProps: {
        icon: <SdkIcon />,
      },
    },
    {
      ...
      description:
        '一個遠端 MCP 伺服器，將 AI 工具與 IDE 連接至 Logto Cloud，實現資源管理與驗證 (Authentication) 整合。',
      customProps: {
        icon: <McpIcon />,
      },
    },
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      label: '開源 (OSS)',
      href: '/logto-oss',
      description: '在你自己的基礎設施上自我託管 Logto。詳情請參閱我們的 GitHub 頁面與開源專區。',
      customProps: {
        icon: <OpenSourceIcon />,
      },
    },
...
加入 [Logto Discord](https://discord.com/invite/UEPaF3j5e6) 伺服器 —— 這裡是開發者與企業的活躍社群。參與討論、獲取最新資訊並分享你的回饋。
...
<Url href="/concepts/authn-vs-authz">驗證 (Authentication) & 授權 (Authorization)</Url>
<Url href="/concepts/core-service">Logto 核心服務 (Logto core service)</Url>
