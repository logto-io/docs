...
管理主控台使用 [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API)，該 API 需要 [安全環境（secure contexts）](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts)，也就是 HTTPS 或搭配 `localhost` 的 HTTP。
...
- 此外，你需要在代理伺服器設定中將 `X-Forwarded-Proto` 標頭設為 `https`。詳情請參閱 [Trusting TLS offloading proxies](https://github.com/panva/node-oidc-provider/blob/main/docs/README.md#trusting-tls-offloading-proxies)。
...
### 我使用 PgBouncer 或 RDS Proxy，Logto 連線失敗（錯誤 `unsupported startup parameter: statement_timeout`）\{#im-using-pgbouncer-or-rds-proxy-and-logto-fails-to-connect-error-unsupported-startup-parameter-statement-timeout}

</summary>

部分資料庫代理會拒絕 `statement_timeout` 啟動參數。自 Logto v1.36.0 起，你可以透過設定：
...
如果你想自訂逾時時間，請以毫秒為單位提供數字字串（例如 `5000`）。未設定或無效時，Logto 會使用預設 60000 毫秒。
...
- 若未指定 `ADMIN_ENDPOINT`，Logto 會允許 `localhost:[admin-port]` 進行跨來源資源共享（CORS）。
- 若已指定 `ADMIN_ENDPOINT`，僅允許來自 `ADMIN_ENDPOINT` 來源的請求。
...
### Logto OSS 的 Management API 端點和雲端版本有差異嗎？\{#does-the-management-api-endpoint-for-logto-oss-differ-from-the-cloud-version}
