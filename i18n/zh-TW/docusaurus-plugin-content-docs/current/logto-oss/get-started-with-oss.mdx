...
要在線上 GitPod 工作區啟動 Logto，<a href="https://gitpod.io/#https://github.com/logto-io/demo" target="_blank" rel="noopener">請點此</a>。稍等片刻，你會看到如下訊息：
...
繼續你的 Logto 之旅時，請按下 Ctrl（或 Cmd）並點擊以 `https://3002-...` 開頭的連結。祝你使用愉快！
...
:::caution
請勿在生產環境中使用我們的 docker compose 指令！目前我們在 `docker-compose.yml` 中將內建 Postgres 資料庫與 Logto 應用程式綁定在一起，
每次重新執行指令都會建立新的資料庫實例，先前儲存的所有資料都會遺失。
:::
...
將容器埠號對應到 Logto 核心與管理應用程式，例如 `3001:3001` 與 `3002:3002`；並將下列環境變數設置到容器中：

```yml
TRUST_PROXY_HEADER: 1 # 如果 Logto 前方有 HTTPS 代理（如 Nginx），請設為 1
ENDPOINT: https://<your-logto-domain> # （可選）如使用自訂網域，請替換為你的 Logto 端點 URL
ADMIN_ENDPOINT: https://<your-logto-admin-domain> # （可選）如使用自訂網域，請替換為你的 Logto 管理端點 URL
...
_如需進階控制或以程式方式存取 Logto，請參閱 [核心服務](/concepts/core-service)。_

## 主機服務供應商 \{#hosting-providers}

以下可靠的主機服務供應商提供 Logto 一鍵安裝範本。透過這些易於部署的服務，你可以在數秒內完成 Logto CIAM 系統的架設與啟動。
...
請注意，我們不提供第三方服務供應商的客戶支援。如需官方支援，請部署於 [Logto Cloud](https://cloud.logto.io)。感謝你的理解！

## 建立帳號 \{#create-an-account}

當你成功在伺服器上部署 Logto 後，請在歡迎頁點擊「建立帳號」。請注意，Logto 開源版本僅允許首次啟動時建立一個帳號，且不支援多帳號。帳號建立僅支援使用者名稱與密碼組合。
