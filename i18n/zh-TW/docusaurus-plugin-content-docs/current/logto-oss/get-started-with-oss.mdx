...
:::caution
請勿將我們的 docker compose 指令用於正式環境！目前我們在 `docker-compose.yml` 中將內建的 Postgres 資料庫與 Logto 應用程式綁定在一起，
...
準備一個 [PostgreSQL](https://www.postgresql.org/)@^14.0 實例，並使用 [Logto CLI](/logto-oss/using-cli) 初始化 Logto 專用資料庫：
...
將容器埠號對應到 Logto 核心與管理應用程式，例如 `3001:3001` 與 `3002:3002`；並設定下列環境變數：

```yml
TRUST_PROXY_HEADER: 1 # 若 Logto 前方有 HTTPS 代理（如 Nginx），請設為 1
ENDPOINT: https://<your-logto-domain> # （選填）若使用自訂網域，請替換為你的 Logto 端點網址
ADMIN_ENDPOINT: https://<your-logto-admin-domain> # （選填）若使用自訂網域，請替換為你的 Logto 管理端網址
DB_URL: postgres://username:password@your_postgres_url:port/db_name # 請替換為你的 Postgres DSN
DATABASE_STATEMENT_TIMEOUT: DISABLE_TIMEOUT #（v1.36.0+）針對 PgBouncer/RDS Proxy 停用 statement_timeout
```

以上述必要環境變數啟動容器（下方省略選填項，請視需求補充）：
...
- 若使用 Docker Hub，請將 `ghcr.io/logto-io/logto:latest` 改為 `svhd/logto:latest`。
- 在 `DB_URL` 中可用 `host.docker.internal` 或 `172.17.0.1` 指向主機 IP。
- 若使用 PgBouncer 或 RDS Proxy 並遇到 `unsupported startup parameter: statement_timeout`，請設 `DATABASE_STATEMENT_TIMEOUT=DISABLE_TIMEOUT`（v1.36.0+）。
...
若你想指定 Logto zip 檔案的下載網址，請加上 `--download-url` 參數。例如：
...
Logto 透過環境變數進行設定，亦支援 `.env` 檔案。詳細用法與完整變數列表請參閱 [設定說明](/concepts/core-service/configuration)。
...
以下可靠的主機服務商提供 Logto 一鍵安裝範本。透過這些易於部署的服務，你可以在數秒內完成 CIAM 系統的建置與啟動。
  ...
  items={[
    {
      ...
      description: '具備免費開發租戶、易於整合驗證 (Authentication) 的高性價比雲端服務。',
      customProps: {
        icon: <LogtoIcon />,
      },
    },
    ...
    },
    {
      ...
      description: '現代化控制台，透過 Docker 管理雲端伺服器。',
      customProps: {
        icon: <EasypanelIcon />,
      },
    },
...
當你成功在伺服器上部署 Logto 後，請於歡迎頁點擊「建立帳號」。請注意，Logto 開源版僅允許首次啟動時建立一個帳號，且不支援多帳號。帳號建立僅支援使用者名稱與密碼組合。
...
<Url href="https://github.com/logto-io/logto/blob/master/AWESOME.md">Logto 優質社群資源</Url>
