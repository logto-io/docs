...
_核心服務_ 是一個用於 Logto 關鍵任務的單體服務。原始碼位於 [`@logto/core`](https://github.com/logto-io/logto/tree/master/packages/core)。

:::note
_核心服務_ 和 _SDK 核心_ 是兩個不同的概念。請參閱 [SDK 規範](/developers/sdk-conventions) 了解差異。
:::
...
Logto 在底層使用了出色的認證 [OpenID Connect](https://openid.net/connect/) 實現 [node-oidc-provider](https://github.com/panva/node-oidc-provider)。提供者掛載在 `/oidc`，你可以在 [packages/core/src/oidc](https://github.com/logto-io/logto/tree/master/packages/core/src/oidc) 中查看相關配置和文件。

OIDC 的 [Userinfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo) 可用，並掛載在 `/oidc/me`。
...
_Management API_ 是一組管理和更新 Logto 資料的 API。只有擁有 `admin` 角色的使用者才能訪問。

前往 [API 參考](https://openapi.logto.io) 查看詳細資訊。

若要以程式方式訪問 API，請參閱 [與 Management API 互動](/integrate-logto/interact-with-management-api)。
...
- 查看 [開發你的自訂 UI](/customization/bring-your-ui/#develop-your-custom-ui) 指南，了解如何使用 Experience API 構建自訂體驗 UI
...
查看 [通過 Account API 的帳戶設置](/end-user-flows/account-settings/by-account-api) 以了解如何利用 Account API 構建自己的帳戶設置頁面。
