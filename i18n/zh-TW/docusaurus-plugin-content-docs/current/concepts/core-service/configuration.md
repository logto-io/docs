...
- 系統環境變數
- 專案根目錄下符合 [dotenv](https://github.com/motdotla/dotenv#readme) 格式的 `.env` 檔案
...
| Key                               | 預設值                               | Type                                                     | 說明                                                                                                                                                                                                                                                                |
| --------------------------------- | ------------------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NODE_ENV                          | `undefined`                          | <code>'production' &#124; 'test' &#124; undefined</code> | Logto 執行的環境類型。                                                                                                                                                                                                                                              |
...
此情境下，建議將 `TRUST_PROXY_HEADER` 設為 `true`，表示信任代理標頭欄位。Logto 會將該值傳遞給 [Koa app 設定](https://github.com/koajs/koa/blob/master/docs/api/index.md#settings)。
...
管理過多環境變數既沒效率又不靈活，因此大多數一般設定都儲存在資料庫表格 `logto_configs` 中。
