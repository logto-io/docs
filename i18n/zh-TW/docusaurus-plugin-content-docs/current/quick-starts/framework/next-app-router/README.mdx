...
HTTP 不允許在流式傳輸開始後設置 cookie，`getAccessTokenRSC` 無法更新 cookie 值，因此如果存取權杖被刷新，將不會在會話中持久化。建議在客戶端或路由處理器中使用 `getAccessToken` 函數。
...
HTTP 不允許在流式傳輸開始後設置 cookie，`getOrganizationTokenRSC` 無法更新 cookie 值，因此如果存取權杖被刷新，將不會在會話中持久化。建議在客戶端或路由處理器中使用 `getOrganizationToken` 函數。
