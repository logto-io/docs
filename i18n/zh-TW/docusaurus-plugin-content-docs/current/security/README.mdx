...
現代驗證 (Authentication) 安全性需對抗各種威脅，包括釣魚、憑證填充、暴力破解、勒索軟體、DDoS 以及 AI 驅動攻擊。保護使用者身分對於維護品牌信任與合規至關重要。

Logto 提供強大且安全的存取管理，專為正面迎擊這些風險而設計。我們以主動威脅預防與韌性為優先，確保你的系統在不犧牲易用性的前提下持續受到防護。對 Logto 而言，安全性不是事後考量，而是基礎，讓企業在威脅不斷演進的時代中依然能夠蓬勃發展，因為防禦進化得更快。
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: '透過允許清單與封鎖一次性或不受歡迎的信箱，控管新註冊與新綁定的電子郵件。',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    ...
    },
    {
      ...
      description: '限制每位收件人的驗證碼與訊息發送次數，防止信箱垃圾郵件與簡訊灌爆濫用。',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        '當使用者存取敏感資訊或執行高風險操作時，允許應用程式提示進階驗證 (Authentication)。',
      customProps: {
        icon: <SecurityIcon />,
      },
    },
    ...
    },
    {
      ...
      description: '啟用集中式登出，降低會話劫持與未授權存取風險。',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      ...
      href: '/end-user-flows',
      description: '僅允許受邀使用者註冊，並透過電子郵件魔術連結進行安全導入。',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
