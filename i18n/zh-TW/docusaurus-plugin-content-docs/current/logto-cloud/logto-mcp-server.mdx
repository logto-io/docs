...
:::tip
Logto MCP Server 是以 Logto 和 [MCP Auth](https://mcp-auth.dev) 建構而成。如果你想自行建立遠端 MCP 伺服器，請參考 [建立你自己的遠端 MCP 伺服器](#build-your-own-remote-mcp-server)。
:::
...
3. 新增以下 [設定](https://docs.windsurf.com/windsurf/cascade/mcp#mcp_config-json)：
...
## 建立你自己的遠端 MCP 伺服器 \{#build-your-own-remote-mcp-server}

Logto MCP Server 是以 Logto 和 [MCP Auth](https://mcp-auth.dev) 建構而成。如果你想建立具備驗證 (Authentication) 的遠端 MCP 伺服器，請參考以下資源：
...
- [加入我們的 Discord](https://discord.gg/fUQmaFyAam)：分享遠端 MCP 伺服器建置經驗與最佳實踐
- [MCP Auth](https://mcp-auth.dev)：協助你為 MCP 伺服器快速加入生產等級驗證 (Authentication) 功能，無需花數週閱讀規範或手動串接。
