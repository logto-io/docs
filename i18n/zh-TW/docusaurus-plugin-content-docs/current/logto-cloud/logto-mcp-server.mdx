...
Logto MCP Server 是一個遠端 [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) 伺服器。將其 URL 加入你的 AI 應用程式，使用 Logto Cloud 帳號登入，開始與 Logto 對話。

:::tip
Logto MCP Server 是以 Logto 與 [MCP Auth](https://mcp-auth.dev) 建構而成。如果你想自行打造遠端 MCP 伺服器，請參考[打造你自己的遠端 MCP 伺服器](#build-your-own-remote-mcp-server)。
:::

<YouTubeVideo
  id="cpjf9b6qt6U"
  title="只需一個提示詞即可啟用使用者驗證 (Authentication)：Logto MCP Server 介紹"
/>
...
更多功能即將推出，包括登入體驗設定、RBAC 與權限管理最佳實踐指引、多租戶（multi-tenancy）組織架構協助 B2B 應用開發等。
...
然後，在 Claude Code 中執行 `/mcp`，並依照瀏覽器登入流程，使用 Logto Cloud 帳號進行驗證 (Authentication)。
...
2. 選擇 **HTTP** 作為伺服器類型。
3. 輸入 URL：`https://mcp.logto.io`。你可以選擇安裝到工作區以便更好地隔離與管理。
...
Logto MCP Server 可搭配任何支援 OAuth 授權 (Authorization) 的 MCP 用戶端使用。請使用以下設定：
...
- [MCP Auth](https://mcp-auth.dev)：一站式為你的 MCP 伺服器加入生產等級驗證 (Authentication) 能力，無需花費數週閱讀規範或手動串接。
