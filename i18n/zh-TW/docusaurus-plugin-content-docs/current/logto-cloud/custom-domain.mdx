...
你的 Logto 租戶預設會有一個免費網域 `{{tenant-id}}.app.logto`。不過，你可以透過使用自訂網域（例如 `auth.example.com`）來提升使用體驗與品牌辨識度。

自訂網域會用於多個功能：

- [登入與註冊頁面](/end-user-flows/sign-up-and-sign-in) 的網址
- [通行密鑰 (Passkey)](/end-user-flows/mfa/webauthn) 連結網址（若在使用者已綁定通行密鑰後變更網域，可能會導致驗證失敗）
...
3. 複製表格中的 CNAME 值，前往你的網域 DNS 供應商新增紀錄。

   <img src="/img/assets/custom-domain-processing.jpeg" alt="自訂網域處理中" />

4. 等待驗證與 SSL 處理。
   1. 系統會每 10 秒自動驗證一次紀錄，直到自訂網域新增完成。請確保輸入的網域名稱或 DNS 紀錄正確無誤。
...
如需排查與解決 CAA 紀錄相關的 SSL 憑證問題，請參考 [Cloudflare 官方文件](https://developers.cloudflare.com/ssl/edge-certificates/caa-records/)。
...
若新增自訂網域時出現「The hostname is associated with a held zone, please contact the owner to have the hold removed」錯誤訊息，表示該網域已在 Cloudflare 區域中，且狀態為「Zone Hold」。詳情請參閱 [Cloudflare 官方文件](https://developers.cloudflare.com/fundamentals/setup/account/account-security/zone-holds/)。
