...
:::note
在服務上線後更改網域可能會導致問題，因為你的應用程式程式碼與整合可能仍然引用舊網域。為確保順利切換，建議你在建立 Production 租戶時**一開始就設定自訂網域**。
:::
...
- [Passkey](/end-user-flows/mfa/webauthn) 與 [Passkey 登入](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) 相關網址（使用者綁定 Passkey 後更改網域，可能導致 MFA 與該憑證登入失效）
...
請參考 [Cloudflare 官方文件](https://developers.cloudflare.com/ssl/edge-certificates/caa-records/) 以排查與解決 CAA 相關的 SSL 憑證問題。
...
在 <CloudLink to="/applications">Console > 應用程式</CloudLink> 的應用程式詳情頁面，捲動至「端點與憑證」區塊，切換網域下拉選單，即可查看並複製對應端點以更新你的應用程式設定。
...
[多重要素驗證 (MFA) 的 Passkey](/end-user-flows/mfa/webauthn) 與 [Passkey 登入](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) 會綁定於註冊時的網域。使用者必須透過相同網域登入，才能使用該 Passkey。
