...
func extractBearerTokenFromHeaders(r *http.Request) (string, error) {
    ...
    if authorization == "" {
        return "", NewAuthorizationError("Authorization 標頭遺失", http.StatusUnauthorized)
    }

    if !strings.HasPrefix(authorization, bearerPrefix) {
        return "", NewAuthorizationError(fmt.Sprintf("Authorization 標頭必須以 \"%s\" 開頭", bearerPrefix), http.StatusUnauthorized)
    }
