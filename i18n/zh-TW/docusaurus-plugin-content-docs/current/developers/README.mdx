  ...
  items={[
    {
      ...
      description: '使用自訂腳本為存取權杖 (Access token) 附加額外宣告 (Claims)，實現 ABAC 或拒絕權杖 (Token) 發放。',
      customProps: {
        icon: <JwtClaims />,
      }
    },
    {
      ...
      description: '依據 OIDC 規範，控制 ID 權杖 (ID token) 中包含哪些擴充宣告 (Claims)。',
      customProps: {
        icon: <JwtClaims />,
      }
    },
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      href: '/developers/',
      description: '介紹 SDK 中的資料結構、用途與方法，協助使用者自訂 SDK 以符合各種業務場景。',
      customProps: {
        icon: <Settings />,
      }
    },
