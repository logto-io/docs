...
想像一下，TechCorp 的支援工程師 Sarah 收到來自客戶 Alex 的緊急工單，Alex 無法存取關鍵資源。為了有效診斷並解決問題，Sarah 需要看到與 Alex 在系統中完全相同的畫面。這時，Logto 的使用者模擬 (User impersonation) 功能就派上用場了。

使用者模擬允許像 Sarah 這樣的授權 (Authorization) 使用者，能夠在系統內暫時以其他使用者（如 Alex）的身分行動。這項強大功能對於疑難排解、客戶支援及執行管理工作極為有用。
...
首先，Sarah 的支援應用程式需向 TechCorp 的後端伺服器請求模擬。
...
在此 API，後端應進行適當的授權 (Authorization) 檢查，確保 Sarah 具備模擬 Alex 的必要權限 (Permission)。

### 步驟 2：取得 subject token \{#step-2-obtaining-a-subject-token}

TechCorp 的伺服器驗證 Sarah 的請求後，會呼叫 Logto 的 [Management API](/integrate-logto/interact-with-management-api) 以取得 subject token。
...
現在，Sarah 的應用程式會以此 subject token 兌換代表 Alex 的存取權杖 (Access token)，並指定該權杖將用於的資源 (Resource)。

**請求（Sarah 的應用程式 → Logto 權杖端點）**

對於傳統 Web 應用程式或帶有 app secret 的機器對機器應用程式，請在 `Authorization` 標頭中帶入憑證：
...
對於單頁應用程式（SPA）或無 app secret 的原生應用程式，請在請求主體中帶入 `client_id`：
...
2. 模擬存取權杖 (Impersonation access token) 不會附帶 [重新整理權杖 (Refresh token)](https://auth.wiki/refresh-token)。若權杖過期且問題尚未解決，Sarah 需重複此流程。
3. TechCorp 的後端伺服器必須實作適當的授權 (Authorization) 檢查，確保僅有授權 (Authorization) 支援人員（如 Sarah）能請求模擬。

:::

## `act` 宣告 (Claim) \{#act-claim}

使用權杖交換流程進行模擬時，發出的存取權杖 (Access token) 可包含額外的 `act`（actor）宣告 (Claim)。此宣告代表「行動方」的身分——在本例中即執行模擬的 Sarah。

若要包含 `act` 宣告，Sarah 的應用程式需在權杖交換請求中提供 `actor_token`，此權杖需為帶有 `openid` 權限範圍 (Scope) 的有效存取權杖 (Access token)。以下為請求範例：
...
若有提供 `actor_token`，最終的存取權杖 (Access token) 會包含如下的 `act` 宣告 (Claim)：
...
此 `act` 宣告 (Claim) 明確指出 Sarah（sarah789）正以 Alex（alex123）的身分行動。`act` 宣告 (Claim) 對於稽核與追蹤模擬行為非常有用。

## 自訂權杖宣告 (Customizing token claims) \{#customizing-token-claims}

Logto 支援[自訂模擬權杖 (Impersonation token) 的宣告 (Claim)](/developers/custom-token-claims)。這對於在模擬過程中加入額外情境或中繼資料（如模擬原因或支援工單）非常有用。

當 TechCorp 的伺服器向 Logto Management API 請求 subject token 時，可帶入 `context` 物件：
...
此 [context](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) 可在 `getCustomJwtClaims()` 函式中用來為最終存取權杖 (Access token) 加入特定宣告 (Claim)。範例如下：
...
Sarah 最終收到的存取權杖 (Access token) 可能如下：
...
透過這種方式自訂存取權杖 (Access token) 宣告 (Claim)，TechCorp 可在系統中更容易稽核與理解模擬行為的情境資訊。

:::note
自訂權杖宣告 (Claim) 時請謹慎，避免加入敏感資訊，以免權杖遭攔截或洩漏時造成安全風險。JWT 雖經簽章但未加密，任何取得權杖者皆可檢視內容。
:::
