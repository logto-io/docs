...
sequenceDiagram
    ...
    Note over TechCorp,Logto: 請求 Alex 的主體 (Subject) 權杖

    Logto-->>TechCorp: 回傳主體 (Subject) 權杖
    TechCorp-->>Sarah: 回傳主體 (Subject) 權杖

    Sarah->>LogtoToken: POST /oidc/token
    Note over Sarah,LogtoToken: 交換主體 (Subject) 權杖取得存取權杖 (Access token)
...
### 步驟 2：取得主體 (Subject) 權杖 \{#step-2-obtaining-a-subject-token}

TechCorp 的伺服器在驗證 Sarah 的請求後，會呼叫 Logto 的 [Management API](/integrate-logto/interact-with-management-api) 以取得主體 (Subject) 權杖。
...
### 步驟 3：交換主體 (Subject) 權杖取得存取權杖 (Access token) \{#step-3-exchanging-the-subject-token-for-an-access-token}
...
2. 模擬存取權杖 (Impersonation access token) 不會附帶 [重新整理權杖 (Refresh token)](https://auth.wiki/refresh-token)。若權杖過期，Sarah 需重複此流程以協助 Alex。
...
Logto 允許你[自訂模擬權杖的宣告 (Token claims)](/developers/custom-token-claims)。這對於在模擬過程中加入額外情境或中繼資料（如模擬原因或支援工單）很有幫助。
...
這個 [context](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) 可在 `getCustomJwtClaims()` 函式中用來為最終存取權杖 (Access token) 加入特定宣告 (Claim)。以下為實作範例：
