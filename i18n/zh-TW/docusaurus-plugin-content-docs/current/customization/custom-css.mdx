...
如果我們的[細緻化 UI](/customization/match-your-brand)無法完全符合你的品牌需求，可以使用自訂 CSS 來調整你的登入體驗。只需將你的 CSS 程式碼加入編輯器，即可立即預覽變更。

## Logto Console 中的自訂 CSS \{#custom-css-in-logto-console}

1. 前往 <CloudLink to="/sign-in-experience/branding">Console > 登入與帳號 > 品牌設定 > 自訂 CSS</CloudLink>。
2. 在左側編輯器編輯 CSS 程式碼，右側預覽視窗會即時渲染效果。注意：目前編輯器僅支援 CSS 程式碼，不支援 HTML 或 Javascript。
3. 儲存變更。
...
例如，若你想讓登入頁呈現 _Night City_ 的網頁風格，可以嘗試以下 CSS：
...
由於 Logto 採用 CSS Modules，你可能會在 DOM 元素的 `class` 屬性中看到雜湊值（例如 `<div>` 帶有 `vUugRG_container`）。若要覆蓋這些樣式，可以使用 `$=` CSS 選擇器來匹配以指定值結尾的元素。本例應使用 `div[class$=container]`。

:::

## 透過 Management API 設定自訂 CSS \{#custom-css-by-management-api}

你也可以使用 [Management API](https://openapi.logto.io/operation/operation-uploadcustomuiassets) 的 `PATCH /api/sign-in-exp`，並在 body 傳入 `{ "customCss": "任意字串" }`，來設定登入體驗的自訂 CSS。你應該會看到 `customCss` 的內容附加在頁面的 `<title>` 之後。若樣式權重較高，即可覆蓋原有樣式。
