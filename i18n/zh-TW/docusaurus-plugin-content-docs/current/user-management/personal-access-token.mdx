...
個人存取權杖（PAT, Personal access token）為使用者提供一種安全的方式來授予 [存取權杖 (Access token)](https://auth.wiki/access-token)，而無需使用其憑證或互動式登入。這對於 CI / CD、自動化腳本或需要以程式方式存取資源的應用程式非常有用。

## 管理個人存取權杖 \{#managing-personal-access-tokens}

### 使用 Console \{#using-console}

你可以在 <CloudLink to="/users">Console > 使用者管理</CloudLink> 的使用者詳細資料頁面管理個人存取權杖。在「驗證 (Authentication)」卡片中，你可以查看個人存取權杖清單並建立新的權杖。
...
使用 PAT 取得的存取權杖與透過標準 `refresh_token` 流程取得的權杖**完全相同**。這表示：

- **組織情境**：PAT 取得的權杖支援與重新整理權杖流程相同的組織權限與權限範圍 (Scopes)
- **授權流程**：你可以將 PAT 交換取得的存取權杖用於 [組織權限](/authorization/organization-permissions) 與 [組織層級 API 資源](/authorization/organization-level-api-resources)
...
應用程式會以 HTTP POST 方法，使用特殊的 grant type，向租戶的 [權杖端點](/integrate-logto/application-data-structure#token-endpoint) 發送 [權杖交換請求](https://auth.wiki/authorization-code-flow#token-exchange-request)。HTTP 請求主體需以 `application/x-www-form-urlencoded` 格式包含以下參數：
...
若你使用 [JWT 解碼器](https://logto.io/jwt-decoder) 解碼存取權杖，會得到如下範例 payload：
