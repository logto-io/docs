...
- [基本資料](/user-management/user-data#basic-data)：來自使用者檔案的基本資訊。儲存除了社交 `identities` 和 `custom_data` 以外的所有 _user_ 屬性，例如使用者 ID、使用者名稱、電子郵件、手機號碼，以及上次登入時間。
...
_username_ 用於以 _username_ 和密碼登入。

其值來自使用者首次註冊時填寫的使用者名稱。可能為 `null`。非 null 值長度不得超過 128 字元，只能包含字母、數字與底線（`_`），且不得以數字開頭。預設區分大小寫。這些規則可依租戶透過 [使用者名稱政策](/end-user-flows/sign-up-and-sign-in/username-policy) 進一步限制。
...
只有來自社交或企業級單一登入 (SSO) 身分提供者且已驗證的電子郵件地址，才能同步並儲存為 `primary_email`。
...
只有來自社交或企業級單一登入 (SSO) 身分提供者且已驗證的手機號碼，才能同步並儲存為 `primary_phone`。
...
與其他標準宣告不同的是，`profile` 內的屬性僅在其值非空時，才會包含於 [ID 權杖 (ID token)](https://auth.wiki/id-token) 或 userinfo 端點回應中；而其他標準宣告若值為空則會回傳 `null`。

其中一個例外是 `preferred_username` 宣告：當 `profile` 中的 `preferredUsername` 為空時，該宣告會回退至使用者的 [username](/user-management/user-data#username)，確保標準相容的客戶端能直接取得可用值。若明確設定 `preferredUsername`，則以其值為主。
...
Logto 預設採用 [Argon2](https://en.wikipedia.org/wiki/Argon2) 的 [node-argon2](https://github.com/ranisalt/node-argon2) 實作作為加密方法；詳情請參考相關說明。
...
一旦使用者被停用，先前授予的重新整理權杖 (Refresh tokens) 會立即撤銷，且該使用者將無法再通過 Logto 驗證 (Authentication)。
...
使用者資訊依社交身分提供者（即社交平台）而異，通常包含以下內容：
...
_sso_identities_ 包含從 [企業級單一登入 (Enterprise SSO)](/end-user-flows/enterprise-sso)（即透過企業連接器進行單一登入 (SSO) 登入](/connectors/enterprise-connectors)）取得的使用者資訊。每個使用者的 _ssoIdentities_ 以獨立 JSON 物件儲存。

從 SSO 身分提供者同步的資料取決於企業連接器所設定請求的權限範圍 (Scopes)。以下為 TypeScript 型別定義：
...
你可以利用 _custom_data_ 做以下事情：

- 記錄使用者是否已完成特定操作，例如是否看過歡迎頁。
- 在使用者檔案中儲存應用程式專屬資料，例如每個應用程式下使用者偏好的語言與外觀。
- 維護與使用者相關的其他任意資料。
...
若你仍需將敏感資訊放入 _custom_data_，建議先加密。僅在可信任的後端服務進行加解密，避免於前端應用程式處理。如此可將 _custom_data_ 外洩時的損失降至最低。
...
請謹慎更新。更新使用者 _custom_data_ 會完全覆蓋原本儲存內容。

例如，若你呼叫更新 _custom_data_ API 時的輸入如下（假設原本 _custom_data_ 為前述範例資料）：
...
| [custom_data](/user-management/user-data#custom-data)                               | object    | 可自訂屬性的額外資訊        | ❌   | ✅   |
