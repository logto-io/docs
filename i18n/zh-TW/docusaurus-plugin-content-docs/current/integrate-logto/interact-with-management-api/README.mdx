---
description: 利用 Management API 存取 Logto 的後端服務，透過使用者管理、帳號設定、身分驗證與多租戶架構，擴展你的 CIAM 系統。
sidebar_position: 5
---
...
Logto Management API 是一套完整的 API，讓開發者能完全掌控實作細節，以滿足產品需求與技術棧。它是預先建置的，並列於 <CloudLink to="/api-resources">主控台 > API 資源 > Logto Management API</CloudLink>，且無法刪除或修改。
...
透過 Logto Management API，你可以存取 Logto 強大的後端服務，這些服務具備高度擴展性，可應用於多種情境。它的能力遠超過僅靠管理主控台低程式碼功能所能達成的範圍。
...
在角色指派模組中，你可以看到所有 M2M 角色，帶有 Logto 圖示的角色表示這些角色包含 Logto Management API 權限。
...
我們的開發者已利用 Logto Management API 實作許多進階功能。我們相信這套 API 具備高度擴展性，能支援你各種需求。以下舉例說明一些僅能透過 Logto Management API 實現、而無法在 Logto 管理主控台完成的情境。

### 自行實作使用者個人檔案 \{#implement-user-profile-on-your-own}

Logto 目前尚未提供預設的使用者個人檔案 UI 解決方案。我們理解個人檔案與業務及產品屬性密切相關。在我們持續探索最佳方案的同時，建議你利用我們的 API 自行打造解決方案。例如，你可以結合互動 API、個人檔案 API 與驗證碼 API，開發符合需求的自訂功能。
...
如果你利用 [組織](/organizations) 功能打造多租戶應用程式，可能會需要 Logto Management API 來處理組織邀請與成員管理等任務。對於你的 SaaS 產品，當租戶內同時有管理員與成員時，Logto Management API 可協助你打造專屬於業務需求的自訂管理後台。詳情請參閱 [這裡](/end-user-flows/organization-experience/)。
...
除了標準的 link 標頭外，Logto 也會新增 `Total-Number` 標頭：
...
Logto Cloud 會對每個租戶執行階段施加速率限制，以保護系統穩定性。詳情請參閱 [系統限制 - 速率限制章節](/logto-cloud/system-limit#rate-limit)。
