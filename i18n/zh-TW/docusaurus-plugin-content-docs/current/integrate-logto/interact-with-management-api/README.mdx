...
透過 Logto Management API，你可以存取 Logto 強大且高度可擴展的後端服務，並可應用於多種情境。它的能力遠超過僅靠管理主控台低程式碼功能所能達成的範圍。

常用的 API 如下：

- [使用者 (User)](https://openapi.logto.io/operation/operation-getuser)
- [應用程式 (Application)](https://openapi.logto.io/operation/operation-listapplications)
- [稽核日誌 (Audit logs)](https://openapi.logto.io/operation/operation-listlogs)
- [角色 (Roles)](https://openapi.logto.io/operation/operation-listroles)
- [資源 (Resources)](https://openapi.logto.io/operation/operation-listresources)
- [連接器 (Connectors)](https://openapi.logto.io/operation/operation-listconnectors)
- [組織 (Organizations)](https://openapi.logto.io/operation/operation-listorganizations)
...
## Logto Management API 的典型應用情境 \{#typical-scenarios-for-using-logto-management-api}

我們的開發者已利用 Logto Management API 實作許多進階功能。我們相信這套 API 具備高度擴展性，能支援你多元的需求。以下是一些僅能透過 Logto Management API 實現、無法在 Logto 管理主控台完成的應用情境範例。

### 自行實作使用者個人檔案 \{#implement-user-profile-on-your-own}

Logto 目前尚未提供預設的使用者個人檔案 UI 解決方案。我們理解個人檔案與業務及產品屬性密切相關。在我們規劃最佳方案的同時，建議你可利用我們的 API 自行開發。例如，你可以結合互動 API、個人檔案 API 與驗證碼 API，打造符合需求的自訂解決方案。
...
如果你利用 [組織 (Organizations)](/organizations) 功能打造多租戶應用，可能會需要 Logto Management API 來處理組織邀請與成員管理等任務。對於你的 SaaS 產品，當租戶中同時有管理員與成員時，Logto Management API 可協助你打造符合業務需求的自訂管理後台。詳情請參閱 [這裡](/end-user-flows/organization-experience/)。
...
Logto Cloud 會針對租戶層級執行執行階段速率限制，以保護系統穩定性。詳情請參閱 [系統限制 rate-limit 章節](/logto-cloud/system-limit#rate-limit)。
