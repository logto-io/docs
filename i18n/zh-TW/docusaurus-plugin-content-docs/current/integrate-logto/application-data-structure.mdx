---
description: 參考 OIDC 驗證 (Authentication) 整合所需的關鍵應用程式參數，包括重導 URI、端點、重新整理權杖 (Refresh token)、後台登出等。
sidebar_position: 6
---
...
應用程式也用於 Logto 的稽核日誌 (Audit logs) 中，以追蹤使用者活動並識別潛在的安全威脅或漏洞。透過將特定操作與特定應用程式關聯，Logto 能提供資料存取與使用方式的詳細洞察，協助組織更好地管理安全與合規需求。
...
- **單頁應用程式 (Single page app)**：運行於網頁瀏覽器，並可在不重新載入整個頁面的情況下從伺服器取得新資料並更新頁面的應用程式。例如 React DOM app、Vue app。
...
_應用程式密鑰_ 是用於驗證 (Authentication) 系統中應用程式身分的金鑰，專為私有用戶端（傳統網頁與 M2M 應用程式）作為私密安全屏障。

:::tip
單頁應用程式 (SPA) 與原生應用程式不提供應用程式密鑰。SPA 與原生應用程式屬於「公開用戶端」，無法安全保存密鑰（瀏覽器程式碼或 app bundle 可被檢查）。Logto 會以 PKCE、嚴格的重導 URI / CORS 驗證、短效存取權杖 (Access token) 及重新整理權杖 (Refresh token) 輪替來保護這些應用程式。
:::
...
你可以參考 [Redirection endpoint](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1.2) 以取得更多資訊。
...
重導 URI 支援萬用字元（`*`）模式，適用於如預覽部署等動態環境。萬用字元可用於 HTTP / HTTPS URI 的主機名稱與路徑部分。

**規則：**

- 僅允許在主機名稱與路徑使用萬用字元
- 不允許於協定、連接埠、查詢參數或 hash 片段中使用萬用字元
- 主機名稱萬用字元必須包含至少一個點（例如 `https://*.example.com/callback`）

**範例：**

- `https://*.example.com/callback` - 匹配任意子網域
- `https://preview-*.example.com/callback` - 匹配預覽部署
- `https://example.com/*/callback` - 匹配任意路徑段
...
你可以參考 [RP-initiated logout](https://openid.net/specs/openid-connect-rpinitiated-1_0.html#RPLogout) 以取得更多資訊。

### CORS 允許來源 \{#cors-allowed-origins}

_CORS（跨來源資源共用）允許來源_ 是允許應用程式向 Logto 服務發送請求的來源清單。不在允許清單中的來源將無法向 Logto 服務發送請求。

CORS 允許來源清單用於限制未授權網域存取 Logto 服務，並協助防止跨站請求偽造（CSRF）攻擊。透過在 Logto 中為應用程式指定允許來源，服務可確保僅授權網域能發送請求。
...
_授權端點_ 是 OIDC 術語，是用於啟動使用者驗證 (Authentication) 流程的必要端點。當使用者嘗試存取已註冊於 Logto 平台的受保護資源或應用程式時，會被重導至 _授權端點_ 以驗證身分並取得存取所請求資源的授權 (Authorization)。

你可以參考 [Authorization Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#AuthorizationEndpoint) 以取得更多資訊。

### 權杖端點 \{#token-endpoint}

_權杖端點_ 是 OIDC 術語，是 OIDC 用戶端用來向 OIDC 提供者取得存取權杖 (Access token)、ID 權杖 (ID token) 或重新整理權杖 (Refresh token) 的 Web API 端點。

當 OIDC 用戶端需要取得存取權杖或 ID 權杖時，會攜帶授權憑證（通常為授權碼或重新整理權杖）向權杖端點發送請求。權杖端點驗證授權憑證後，若有效則發放存取權杖或 ID 權杖給用戶端。

你可以參考 [Token Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#TokenEndpoint) 以取得更多資訊。
...
### 一律發放重新整理權杖 (Refresh token) \{#always-issue-refresh-token}

_適用範圍：傳統網頁、SPA_

啟用後，Logto 將一律發放重新整理權杖 (Refresh token)，無論驗證請求中是否帶有 `prompt=consent`，或權限範圍 (Scope) 中是否帶有 `offline_access`。

但除非必要（通常僅用於某些需要重新整理權杖的第三方 OAuth 整合），否則不建議這麼做，因為此做法不符合 OpenID Connect 標準，且可能導致問題。
...
:::note
對於公開用戶端，啟用此功能時，每當用戶端以重新整理權杖換取新存取權杖時，Logto 都會發放新的重新整理權杖。
雖然你仍可為這些公開用戶端關閉此功能，但強烈建議為安全起見保持啟用。
:::
...
注意：SPA（單頁應用程式）因安全考量不支援 TTL 延長。這表示 Logto 不會透過權杖請求延長 TTL。為提升使用者體驗，你可啟用「重新整理權杖輪替」功能，讓 Logto 必要時發放新的重新整理權杖。
...
除預設應用程式屬性外的額外自訂應用程式資訊，使用者可依自身需求自訂資料欄位，例如業務專屬設定與配置。
