...
<p>
  <img src={gitpodRunning} alt="Gitpod 正在运行" width="720px" />
</p>

Logto 默认使用 `3001` 作为核心服务的端口，并以 `3002` 作为管理控制台的访问端口。
按住 Ctrl (或 Cmd) 并点按以 `https://3002-...` 开头的链接即可继续你的 Logto 之旅。玩得开心！
...
映射容器端口，例如 `3001:3001` 和 `3002:3002`；并给容器赋以如下环境变量：

```yml
TRUST_PROXY_HEADER: 1 # 如果你在 Logto 前使用了一个 HTTPS 代理（例如 Nginx），设置为 1
ENDPOINT: https://<your-logto-domain> #（可选）如果你在使用自定义域名，将尖括号中的地址替换成你的 Logto URL
ADMIN_ENDPOINT: https://<your-logto-admin-domain> #（可选）如果你在使用自定义域名，将尖括号中的地址替换成你的 Logto 管理控制台 URL
...
前往 <a target="_blank" href="http://localhost:3002/" rel="noopener">http://localhost:3002/</a> 以继续你的 Logto 之旅。玩得开心！
...
<summary>我遇到了跨域（CORS）问题</summary>

- 如果 `ADMIN_ENDPOINT` 没有被指定，则来自 `localhost:[admin-port]` 的跨域请求将被允许访问 Logto。
- 如果 `ADMIN_ENDPOINT` 已被指定，则 Logto 仅允许来自 `ADMIN_ENDPOINT` 域名的请求访问。

</details>

<details>

<summary>我遇到了报错: Invalid id token</summary>

如果你没有手动修改过 ID Token 的内容，那么这通常是由于你的服务器时间和客户端时间不匹配所导致的。请同步服务器与客户端的时间之后重试。
