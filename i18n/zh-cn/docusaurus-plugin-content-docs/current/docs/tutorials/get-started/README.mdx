...
在 Logto 仓库地址的前面添加 `https://gitpod.io/#`（或 <a href="https://gitpod.io/#https://github.com/logto-io/logto" target="_blank" rel="noopener">点按此处</a>）并访问该链接。稍等片刻，你会在屏幕下方的终端里看到类似如下信息：
...
<summary>
  我在 Logto 前使用了一个 HTTPS 代理（例如 Nginx），但是我的浏览器无法加载「管理控制台」
</summary>

你需要将环境变量 `TRUST_PROXY_HEADER` 设置为 `true`。参见 [配置](../../references/core/configuration.md#使用-https-代理) 以了解详情。
