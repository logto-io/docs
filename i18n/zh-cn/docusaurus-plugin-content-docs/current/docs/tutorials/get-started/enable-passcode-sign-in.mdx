...
根据你想启用的连接器类型，点按位于「类型」列左侧的「设置」按钮。开启的对话框将询问你具体想要设置的连接器。

<Columns
  columns={[
    { title: '短信连接器', items: [{ key: 'Twilio 短信服务' }, { key: '阿里云短信服务' }] },
    {
      title: '邮件连接器',
      items: [{ key: 'Sendgrid 邮件服务' }, { key: '阿里云 DM' }, { key: 'SMTP' }],
    },
  ]}
/>
...
:::note
查看 [配置登录方式](../../recipes/customize-sie/configure-sign-in-methods.mdx) 来全面地了解如何将登录方式进行组合。
:::

## 接下来

- [启用社交登录](./enable-social-sign-in.mdx) （如果你还没看过）
- [探索更多](./further-readings.md)
