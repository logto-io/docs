...
:::note
当前版本的 Logto 只支持同时有一个 _SMS/Email 连接器_ 的 _enabled_ 为 `true`，也就意味着一旦一个其他的 _SMS_（或者 _Email_）类型连接器被启用时，其他所有的 _SMS_（或者 _Email_）类型连接器会被关闭（即 _enabled_ 被置为 `false`）。
