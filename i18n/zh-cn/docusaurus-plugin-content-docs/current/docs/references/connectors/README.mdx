...
对于 _短信连接器_ 和 _邮件连接器_ 来说，_target_ 就好像一个人类可读的 ID。

下面看一些关于 _target_ 的样例：_微信连接器网页版_ 和 _微信连接器原生版_ 的 _target_ 是 `wechat`，*阿里云邮件推送连接器*的 _target_ 是 `aliyun-dm`，_阿里云短信连接器_ 的 _target_ 是 `aliyun-sms`。
