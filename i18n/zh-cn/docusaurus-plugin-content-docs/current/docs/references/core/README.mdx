...
:::note
_核心服务_ 与 _SDK 核心_ 是两个概念。阅读 [SDK Convention](../sdk-convention/README.md) 以了解不同之处。
:::
...
<h2 id="management-api">管理 API</h2>

_管理 API_ 是一个用于管理和更新 Logto 数据的 API 集合。只有具有 `admin` 角色的用户可以访问。

它被加载于路径 `/api`。前往 [API 参考](/api) 以在浏览器中了解细节，或者通过 `GET /api/swagger.json` 来获取 JSON 格式的 Swagger 数据。
...
<BasicTokenRequirements>
  它的 <code>scope</code> 里包含 <code>&quot;all&quot;</code>。
</BasicTokenRequirements>
...
我们提供标准的 OIDC [Userinfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo)，被加载于路径 `/oidc/me`.
...
### 关于 refresh token 的延迟过期时间

你可能已注意到 Refresh Token 的刷新轮换机制存在一个为期 3 秒的过期时间。该过期时间确保了在开启 Refresh Token 刷新轮换机制的前提下，换取 Access Token 的请求在高并发场景中仍可正常工作。

在这个窗口时间（3 秒）内，已经被使用过了的 Refresh Token 仍然被视为有效。

这对于互相之间无法简单共享内存的分布式应用和 serverless 应用（如 Next.js）来说很有帮助。
