...
import AppNote from './components/AppNote.tsx';
import SignInNote from './components/SignInNote.tsx';
...
让我们在这里切换到「管理控制台」的应用详情页。添加一个值为 `logto://callback` 的 Redirect URI 并点按「保存变更」。

<img alt="「管理控制台」中的 Redirect URI" src={redirectUriFigure} width="50%" />
