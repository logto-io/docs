...
在应用中使用 [github.com/gin-contrib/sessions](https://github.com/gin-contrib/sessions) 中间件，这样在请求的路由处理函数中就可以根据用户请求的上下文获取用户的 session：
...
至此，你就可以在请求的路由处理函数中通过以下方式创建一个 session storage 给 Logto 使用了：
