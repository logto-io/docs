...
Logto SDK 使你在登录后可以通过调用 `logtoClient.FetchUserInfo()` 从 OIDC 的 [UserInfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo) 获取用户信息。

### 用户信息返回内容

| 字段名                | 类型       | 所需 Scope    | 备注                                     |
| --------------------- | ---------- | ------------- | ---------------------------------------- |
| sub                   | `string`   | `openid`      | `openid` scope 已由 Logto SDK 默认添加。 |
...
初始化 `LogtoClient` 时，你要将所需的 scope 添加到 `LogtoConfig` 中，这样在你登录之后，你可以调用 `logtoClient.FetchUserInfo()` 获取对应的用户信息。
