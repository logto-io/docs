...
在传统网页应用中，用户的认证信息将由 Logto SDK 存储在用户的 session 中。  
我们将基于 [github.com/gin-contrib/sessions](https://github.com/gin-contrib/sessions) 按照 Logto SDK 定义的 `Storage` 接口来创建 session stroage 适配器。
...
接下来在应用中使用 [github.com/gin-contrib/sessions](https://github.com/gin-contrib/sessions) 中间件，将用户的请求与用户的 session 关联起来：
