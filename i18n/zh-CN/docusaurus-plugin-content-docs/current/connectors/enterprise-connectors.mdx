...
这些连接器促进了你的服务与企业身份提供商 (IdPs) 之间的认证过程。Logto 支持 SP 发起的 SSO 和 IdP 发起的 SSO，这允许组织成员使用他们现有的公司凭据访问你的服务，从而提高安全性和生产力。了解更多关于 SSO 概念的信息，请参阅我们的专门文档：

- <Url type="inline" href="https://blog.logto.io/idp-vs-sp-initiated-sso">
    IdP 发起的 SSO 和 SP 发起的 SSO
  </Url>
...
1. 导航到：<CloudLink to="/enterprise-sso">Console > Enterprise SSO</CloudLink>。
2. 点击“添加企业连接器”按钮并选择一个连接器类型。
3. 提供一个唯一名称（例如，Acme 公司用的 Okta）。
4. 在“连接”选项卡中配置与你的 IdP 的连接。查看上面的指南以获取每种连接器类型的信息。
...
- **电子邮件域**：如果用户输入的电子邮件的域在某些企业 SSO 连接器的“企业电子邮件域”字段中，用户将被重定向到该 SSO 连接器的相应登录 URL。
  ...
  :::note
  重要的是要注意，在 SSO 连接器中配置相关电子邮件域后，使用属于这些域的电子邮件登录的用户将被强制使用 SSO 登录。
...
启用后，“单点登录”按钮将出现在你的登录页面上。具有 SSO 启用电子邮件域的企业用户可以使用他们的企业身份提供商 (IdPs) 访问你的服务。要了解更多关于 SSO 用户体验的信息，请参阅用户流程：企业 SSO。

## 即时加入组织

在 Logto 中，[即时 (JIT) 供应](https://auth.wiki/jit-provisioning) 是一种用于在用户首次登录系统时动态自动分配组织成员资格和角色的过程。

Logto 提供了一个配置 SSO 连接器 JIT 供应的入口，参见参考资料（链接到 JIT 供应 SSO 部分）。
...
- 移除 SSO：移除链接到账户的 SSO 身份，但保留用户账户，并提示用户设置替代验证方法。
