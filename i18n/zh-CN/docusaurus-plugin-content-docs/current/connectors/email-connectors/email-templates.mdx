...
2. **在服务商平台自定义模板**
   ...
   - **功能 (Capabilities)**：
     ...
     - ✅ 在服务商后台完全编辑模板（使用 Logto Management API）
...
| UserPermissionValidation | 在应用使用过程中，某些高风险操作或风险较高的操作[需要额外用户验证](/end-user-flows/account-settings/by-account-api#verify-by-sending-a-verification-code-to-the-users-email-or-phone)，如银行转账、删除正在使用的资源、注销会员等。`UserPermissionValidation` 模板可用于定义用户在这些场景下收到的邮件验证码内容。 | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
| BindNewIdentifier        | 用户修改个人资料时，可能会[为当前账号绑定邮箱地址](/end-user-flows/account-settings/by-account-api#update-or-link-new-email)。此时可使用 `BindNewIdentifier` 模板自定义验证邮件内容。                                                                                                                              | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
| MfaVerification          | 启用[邮箱 MFA](/end-user-flows/mfa/email-mfa) 时，此模板用于在多因素认证 (MFA) 过程中向用户发送验证码。                                                                                                                                                                                                            | code: string<br/>application: `ApplicationInfo`<br/>organization?: `OrganizationInfo` |
| BindMfa                  | 启用[邮箱 MFA](/end-user-flows/mfa/email-mfa) 时，此模板用于设置 MFA 邮箱验证码。用户在绑定或配置邮箱为 MFA 因子时会收到此验证码。                                                                                                                                                                                 | code: string<br/>user: `UserInfo`<br/>application?: `ApplicationInfo`                 |
...
- 对于 `SignIn`、`Register` 和 `ForgotPassword` 模板，`organization` 变量为可选项。仅在授权请求中存在 `organization_id` 参数时可用。详见[组织特定品牌](/customization/match-your-brand#organization-specific-branding)。
...
- 关于 `Profile` 类型的更多信息，请查看 [profile](/user-management/user-data#profile)。
...
在发起当前交互的 OIDC 认证 (Authentication) 请求中提供的原始 `ui_locales` 值。

- 类型：`string`（以空格分隔的 BCP 47 语言标签列表，符合 OIDC 规范），例如："fr-CA fr en"。
- 可用性：当当前登录交互通过 `ui_locales` 发起时存在。如果未提供，则该变量被省略。
- 典型用法：可在邮件内容或主题中包含，用于记录用户请求的 UI 语言以支持 i18n 或审计，例如 `Requested languages: {{uiLocales}}`。
...
由于 Logto 不同场景下使用的邮件模板非常相似，唯一的区别仅在于当前场景和操作的描述。

这里不详细展示所有模板的 HTML 代码，仅以**登录**场景为例。其他如注册、忘记密码等场景与下述示例非常类似。
...
你可以将上述 HTML 代码转义后，添加到连接器配置中的 "Template" 字段（假设使用 SendGrid 连接器）：
...
模板创建后，Logto 会自动优先解析用户的语言偏好，然后选择最匹配的邮件模板。

语言偏好解析顺序如下：

1. 如果 OIDC 认证 (Authentication) 请求包含 `ui_locales`，Logto 会选择 `ui_locales` 中第一个被租户语言库支持的标签。详见 [ui_locales](/end-user-flows/authentication-parameters/ui-locales)。
2. 否则，对于客户端 [体验 API (Experience API)](/end-user-flows/sign-up-and-sign-in) 和 [用户账号 API (Account API)](/end-user-flows/account-settings/by-account-api)，Logto 使用 `Accept-Language` 头。对于 Management API（如 [组织邀请](/end-user-flows/organization-experience/invite-organization-members)），你可以在 `messagePayload` 的 `locale` 字段指定语言。
3. 如果都未提供，Logto 会回退到登录体验中配置的租户默认语言。详见 [本地化语言](/customization/localized-languages#customization-steps-in-logto-console)。

模板选择：

4. 解析出语言后，Logto 会根据 `languageTag` 和 `templateType` 查找匹配的自定义邮件模板。如找到，则使用该模板。
...
当认证 (Authentication) 请求中存在 `ui_locales` 时，模板上下文中会同时提供 `locale` 和 `uiLocales` 变量。
`uiLocales` 变量包含认证 (Authentication) 请求中的原始 `ui_locales` 值，而 `locale` 变量则根据 `ui_locales` 解析出的第一个支持标签确定。如果未提供 `ui_locales`，`locale` 按标准解析规则（如 `Accept-Language`，再到默认语言）处理。
...
你可以在自己的 Web 服务中新增一个发送邮件的接口，然后使用 [Logto HTTP 邮件连接器](/integrations/http-email) 调用你维护的接口。
...
### 是否可以用 Logto 邮件向用户发送自定义“欢迎邮件”？ \{#is-there-a-way-to-use-logto-email-for-sending-our-users-a-customized-welcome-email}
