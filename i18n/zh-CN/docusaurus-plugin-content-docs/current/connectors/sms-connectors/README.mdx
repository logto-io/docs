...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: '腾讯短信服务的官方 Logto 连接器。',
      customProps: {
        icon: <TencentSms />,
      }
    },
...
2. 要添加新的 SMS 连接器，点击“设置”按钮并选择你想要的 SMS 服务商。
3. 阅读所选服务商的 README 文档。
4. 在“参数配置”部分填写必填项。
5. 使用 JSON 编辑器自定义短信模板。
6. 使用“通用”模板向你的手机发送验证码，测试配置是否成功。
7. 点击“保存并完成”结束配置。

## 更换 SMS 连接器 \{#change-sms-connector}
...
- [**设置注册方式**](/end-user-flows/sign-up-and-sign-in/sign-up)：进入 <CloudLink to="/sign-in-experience/sign-up-and-sign-in">登录与账号 &gt; 注册与登录</CloudLink>。在注册部分，添加“手机号”或“邮箱地址或手机号”作为标识符。注意：注册时必须进行手机号验证。
- [**设置登录方式**](/end-user-flows/sign-up-and-sign-in/sign-in)：在登录部分，添加“手机号”作为登录方式。可选择密码或短信验证码进行认证 (Authentication)。点击“添加其他”以获得更多选项。
- [**设置忘记密码**](/end-user-flows/sign-up-and-sign-in/reset-password)：在登录部分，如果启用了密码登录，将看到“忘记密码”设置。启用“手机验证码”以允许通过短信重置密码。
- [**为 MFA 设置短信**](/end-user-flows/mfa/sms-mfa)：进入 <CloudLink to="/mfa">控制台 &gt; 多因素认证 (MFA)</CloudLink>。启用“短信验证码”作为第二重安全因子。
...
### Logto 没有我想用的 SMS 连接器怎么办？ \{#logto-doesnt-have-the-sms-connector-i-want-to-use-what-should-i-do}
...
对于开源版 Logto 用户，我们提供了易于扩展的连接器创建方式，允许你根据具体场景[自定义你的连接器](/logto-oss/develop-your-connector)。我们也欢迎你向 Logto 提交 pull request，这样社区中的其他人也能受益于你的工作。
