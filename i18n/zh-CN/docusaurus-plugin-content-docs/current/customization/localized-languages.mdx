...
| 自定义值         | 自定义值是已经被用户自定义过的值。Logto 源值将被覆盖。                                                                                     |
...
假设你的网站默认语言为英语，并且已开启自动检测。一位来自日本的用户访问你的网站并决定创建账户。如果他/她的应用语言为日语，但 Logto 尚未支持该语言，则登录界面将以英语显示。
...
### 如果我只添加了少量自定义值怎么办？ \{#what-if-i-only-added-a-few-custom-values}

</summary>

最终用户看到的是两列合并的结果。
假设你只想调整 Logto 提供的部分原始内容副本。你的注册界面与 Logto 提供的界面唯一的区别就是你编辑过的键，其他内容保持不变。
...
[登录体验](/end-user-flows/sign-up-and-sign-in/sign-up)中的手机号国家代码默认取自用户浏览器的语言环境。例如，如果用户浏览器语言设置为 `fr`，则国家代码默认为法国 (+33)。

如需为特定用户或地区以编程方式控制默认国家代码，可以使用 [`ui_locales`](/end-user-flows/authentication-parameters/ui-locales) 认证参数。例如，设置 `ui_locales=ja` 时，国家代码将默认为日本 (+81)。
