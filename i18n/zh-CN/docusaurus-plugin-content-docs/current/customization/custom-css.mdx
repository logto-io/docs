...
如果我们的微调 UI 无法完全符合你的品牌，使用自定义 CSS 来定制你的登录体验。只需将你的 CSS 代码添加到我们的编辑器中，即可立即查看更改。
...
你也可以使用 Management API `PATCH /api/sign-in-exp`，在请求体中包含 `{ "customCss": "arbitrary string" }` 来为登录体验设置自定义 CSS。你应该会看到 `customCss` 的值附加在页面的 `<title>` 之后。如果样式具有更高的优先级，它应该能够覆盖。
