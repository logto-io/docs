...
日志 (Logto) 支持多种交互式和非交互式的认证 (Authentication) 方法，例如：

- **登录体验**：终端用户的认证 (Authentication) 过程。
- **机器对机器 (M2M) 认证 (Authentication)**：服务或应用程序的认证 (Authentication) 过程。

认证 (Authentication) 的最终目标非常简单：验证并获取实体的唯一标识符（在日志 (Logto) 中，是用户或应用程序）。

## 授权 (Authorization)

在日志 (Logto) 中，授权 (Authorization) 是通过基于角色的访问控制 (RBAC) 完成的。它让你可以完全控制用户或 M2M 应用程序对以下内容的访问：
...
由于单点登录 (SSO) 的特性，日志 (Logto) 目前不支持将应用程序用作资源。相反，你可以定义 API 资源和权限来控制对资源的访问。
...
这意味着在日志 (Logto) 中没有“登录到组织”的概念。一旦用户被认证 (Authentication)，他们可以根据定义的权限被授权 (Authorization) 访问所有资源（包括组织资源）。
