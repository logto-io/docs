...
像 Notion、Slack 或 Figma 这样的应用是如何构建的？这些多租户 SaaS 应用看起来很易用，但要自己动手做一个？那是另一回事。
...
我会手把手教你如何从零设计并实现这样一个系统——你会惊讶于 2025 年用现代工具和正确架构方法其实有多简单。
...
该平台为文档管理提供强大的 AI 能力，包括自动摘要生成、重点提取和组织 (Organization) 内智能内容推荐。

## SaaS 认证 (Authentication) 与授权 (Authorization) 需要哪些功能？ \{#what-features-are-required-for-saas-authentication-and-authorization}

首先，让我们梳理一下必要的需求。你需要哪些功能？

### 多租户架构 \{#multi-tenant-architecture}

要实现多租户架构，你需要一个名为 **组织 (Organization)** 的实体层。想象一下有一个用户池，用户可以访问多个工作区。每个组织 (Organization) 代表一个工作区，用户在访问不同工作区（组织 (Organization)）时保持单一身份，根据分配的角色 (Role) 获得不同权限 (Permission)。

![multi-tenant-app-architecture.svg](https://uploads.strapi.logto.io/multi_tenant_app_architecture_89ed9779f8.svg)

这是认证 (Authentication) 服务商的常见功能。身份管理系统中的组织 (Organization) 对应你的 SaaS 应用的工作区、项目或租户。
...
在多租户架构下，用户需要拥有带有特定权限 (Permission) 的**角色 (Role)**，以访问其租户资源。
...
2. 第一个创建工作区的用户自动分配 **admin** 角色 (Role)
...
4. **社交登录**：如 Google、GitHub 等一键登录。
5. **多因素认证 (MFA)**：通过如 Duo 等认证器应用提升安全性。

### 租户创建与邀请 \{#tenant-creation-and-invitation}

在多租户 SaaS 应用中，用户流程的一个关键区别是需要支持租户创建和成员邀请。这个流程对产品激活和增长至关重要，需要精心设计和实现。
...
这些是几乎每个 SaaS 应用都会遇到的常见场景。你可以参考这些流程，启发你的产品和设计团队，也可以根据需要自定义自己的流程。
...
现代认证 (Authentication) 服务商（这次我选择 [Logto](https://logto.io/)）已经为我们打包好了所有这些功能。认证 (Authentication) 流程非常简单：

```mermaid
sequenceDiagram
  actor u as 用户
  participant a as 应用
  participant auth as 认证 (Authentication) 服务商

  u->>a: 点击“登录”
  a->>auth: 跳转到认证 (Authentication) 页面
  auth->>u: 展示登录页面
  u->>auth: 输入凭证
  auth->>a: 跳转并携带令牌
  a->>u: 登录成功
...
**从数周开发到 15 分钟集成**，Logto 为我们处理了所有复杂流程！后续实现部分会介绍集成步骤。现在我们可以专注于 DocuMind 的核心功能开发！

### 搭建多租户架构 \{#establish-multi-tenant-architecture}

组织 (Organization) 系统让用户可以创建和加入多个组织 (Organization)。让我们理解核心关系：
...
### 在多租户应用中实现访问控制 \{#enable-access-control-in-multi-tenant-app}

基于角色的访问控制 (RBAC) 对于多租户 SaaS 应用的安全性和可扩展性至关重要。

在多租户应用中，权限 (Permission) 和角色 (Role) 的设计通常是一致的，这源于产品设计。例如，在多个工作区中，通常会有管理员和成员两种角色 (Role)。Logto 作为认证 (Authentication) 服务商，提供了如下组织 (Organization) 级别的基于角色的访问控制 (RBAC) 设计：

1. **统一权限 (Permission) 定义**：权限 (Permission) 在系统级别定义，并在所有组织 (Organization) 中一致应用，确保权限 (Permission) 管理可维护且一致
...
我们已经设计好了组织 (Organization) 系统和访问控制系统，现在可以开始构建产品了！
...
为什么要前后端分离？因为架构清晰，易学易换栈。认证 (Authentication) 服务商以 Logto 为例。

后续指南中的模式**适用于任何前端、任何后端和任何认证 (Authentication) 系统。**

## 为你的应用添加基础认证 (Authentication) 流程 \{#add-basic-authentication-flow-to-your-app}

这是最简单的一步。我们只需将 Logto 集成到项目中，然后在 Logto 控制台根据需求配置用户登录 / 注册方式。

### 安装 Logto 到你的应用 \{#install-logto-to-your-app}

首先，登录 [Logto Cloud](https://cloud.logto.io/)。如果没有账号可以免费注册。创建一个开发租户用于测试。

在租户控制台，点击左侧“应用”按钮，然后选择 React 开始构建应用。
...
function AppContent() {
  ...
  return (
    <Routes>
      ...
      {/* 点击 Dashboard 中的组织 (Organization) 后进入组织 (Organization) 页面 */}
...
一个实用技巧：我们的登录页面有“登录”和“注册”按钮。注册按钮直接跳转到 Logto 的注册页面。这通过 Logto 的 [first screen](/end-user-flows/authentication-parameters/first-screen) 功能实现。它决定了用户首先看到认证 (Authentication) 流程的哪一步。

如果你的产品面向大量新用户，可以默认跳转到注册页面。
...
点击登录后，你会进入 Logto 登录页面。登录（或注册）成功后，恭喜！你的应用有了第一个用户（你自己）！

当你需要时，调用 `useLogto` hook 的 `signOut` 方法即可登出用户。
...
在 Logto 控制台，点击左侧菜单的“登录体验”，再点击“注册与登录”标签页。
...
一切都如此简单！我们几分钟内就搭建好了复杂的用户认证 (Authentication) 系统！

## 增加多租户组织 (Organization) 体验 \{#adding-multi-tenant-organization-experience}

现在我们有了第一个用户！但这个用户还不属于任何组织 (Organization)，我们也还没创建组织 (Organization)。
...
要从 Logto 获取用户的组织 (Organization) 信息，分两步：
...
完成上述步骤后，你需要先登出再重新登录。因为我们修改了请求的 scope 和 resource。
...
多亏了 Logto，我们无需自己构建复杂的组织 (Organization) 关系。只需在 Logto 创建组织 (Organization) 并添加用户即可。Logto 会为我们处理所有复杂性。创建组织 (Organization) 有两种方式：

1. 通过 Logto 控制台手动创建组织 (Organization)
2. 使用 Logto Management API 创建组织 (Organization)，适用于让用户自助创建组织 (Organization)（工作区）的 SaaS 流程

### 在 Logto 控制台创建组织 (Organization) \{#create-organization-in-logto-console}

点击 Logto 控制台左侧的“组织 (Organization)”菜单按钮，创建一个组织 (Organization)。

现在你有了第一个组织 (Organization)。

![console-organizations.png](https://uploads.strapi.logto.io/console_organizations_7312c72766.png)

接下来，把用户加入该组织 (Organization)。

进入组织 (Organization) 详情页，切换到成员标签页，点击“+ 添加成员”按钮，从左侧列表选择你的登录用户，点击右下角“添加成员”按钮。现在你已成功将用户加入该组织 (Organization)。

![console-add-member-to-orgs.png](https://uploads.strapi.logto.io/console_add_member_to_orgs_ad21beb58f.png)

刷新 APP 页面，你会看到用户现在属于一个组织 (Organization)！
...
在控制台创建组织 (Organization) 还不够。你的 SaaS 应用需要让终端用户可以自助创建和管理自己的工作区。要实现该功能，使用 Logto Management API。

具体操作可参考 [与 Management API 交互](/integrate-logto/interact-with-management-api) 文档，配置与 Logto 的 API 通信。
...
后端服务 API 只允许认证 (Authentication) 用户访问。我们需要用 Logto 保护 API，并获取当前用户信息（如用户 ID）。

在 Logto（和 OAuth 2.0）中，后端服务作为资源服务器。用户通过前端携带 Access token 访问 DocuMind 资源服务器，资源服务器校验该令牌，校验通过后返回资源。
...
2. 点击“创建 API 资源 (API resource)”，弹窗中选择 Express。
...
你会看到一个关于 API 资源 (API resource) 使用的教程，可以跟着教程或参考下方步骤。

我们来创建一个 requireAuth 中间件保护 POST /organizations 端点。
...
现在在 POST /organizations 端点使用 requireAuth 中间件。
...
这样就保护了 POST /organizations 端点，只有持有有效 Logto 访问令牌 (Access token) 的用户才能访问。

我们现在可以在前端从 Logto 获取令牌。用户通过该令牌调用后端服务创建组织 (Organization)。中间件也会给我们用户 ID，便于添加用户到组织 (Organization)。
...
const config: LogtoConfig = {
  ...
  resources: [ReservedResource.Organization, "<https://api.documind.com>"], // 新建的 API 资源 (API resource) 标识符
};

```

如前所述，更新 Logto config 后用户需要重新登录。
...
我们来用 Logto Management API 实现组织 (Organization) 创建。

和前端请求后端服务一样，后端服务请求 Logto 也需要 Access token。

在 Logto 中，我们用机器对机器 (Machine-to-Machine) 认证 (Authentication) 获取 Access token。详见 [与 Management API 交互](/integrate-logto/interact-with-management-api)。
...
我们现在已经通过 Logto Management API 实现了组织 (Organization) 创建，并可添加用户到组织 (Organization)。
...
下一步是邀请用户加入组织 (Organization)。本教程暂不实现该功能。你已经掌握了 Management API 的用法。可以参考 [**租户创建与邀请**](https://blog.logto.io/build-multi-tenant-saas-application#tenant-creation-and-invitation) 作为产品设计参考，并通过这篇博客 [如何在多租户应用中实现用户协作](https://blog.logto.io/implement-user-collaboration-in-your-app) 轻松实现该功能。

## 为多租户应用实现访问控制 \{#implement-access-control-to-your-multi-tenant-app}

现在我们进入组织 (Organization) 访问控制环节。

我们希望实现：

- 用户只能访问自己组织 (Organization) 的资源：可通过 Logto 的组织令牌 (Organization token) 实现
...
和前面提到的 Logto 访问令牌 (Access token) 类似，Logto 会针对特定资源颁发访问令牌 (Access token)，用户用该令牌访问后端服务受保护资源。对应地，Logto 会针对特定组织 (Organization) 颁发组织令牌 (Organization token)，用户用该令牌访问后端服务受保护的组织 (Organization) 资源。
...
在使用 `getOrganization` 或任何组织 (Organization) 相关功能前，需确保 Logto config 中已包含 `urn:logto:scope:organizations` scope 和 `urn:logto:resource:organization` resource。我们前面已经声明过，这里不再赘述。
...
2. 请求组织 (Organization) 资源时用的是组织令牌 (Organization token) 而不是访问令牌 (Access token)，因为对于属于组织 (Organization) 的资源，我们希望用组织 (Organization) 权限 (Permission) 控制而不是用户权限 (Permission) 控制（后面实现 `GET /documents` API 时你会更明白）。

接下来，在后端服务创建 `GET /documents` API。和用 API 资源 (API resource) 保护 `POST /organizations` API 类似，我们用组织 (Organization) 专属资源指示器保护 `GET /documents` API。

首先，创建 `requireOrganizationAccess` 中间件保护组织 (Organization) 资源。
...
app.get('/documents', requireOrganizationAccess(), async (req, res) => {
  // 可通过 req.user 获取当前用户 id 和 organizationId
...
这样我们就实现了用组织令牌 (Organization token) 访问组织 (Organization) 资源。在后端服务中，你可以根据 organizationId 从数据库获取对应资源。

有些软件要求组织 (Organization) 之间数据隔离。更多讨论和实现可参考：[用 PostgreSQL 实现多租户：通过简单真实案例学习](https://blog.logto.io/implement-multi-tenancy)。
...
因此，组织 (Organization) 需要有这两个角色 (Role)：Admin 和 Collaborator。
...
组织 (Organization) 模板是每个组织 (Organization) 访问控制模型的蓝图：定义了适用于所有组织 (Organization) 的角色 (Role) 和权限 (Permission)。
...
然后创建 POST /documents API，并用 `requireOrganizationAccess` 中间件及 requiredScopes 配置保护该 API 及前面的 `GET /documents` API。
...
根据 claims 中的 scopes 控制页面元素展示。

## 增加更多多租户应用功能 \{#add-more-multi-tenant-app-features}

到目前为止，我们已经实现了多租户 SaaS 系统的基础用户和组织 (Organization) 功能！不过，还有一些功能没有涉及，比如为每个组织 (Organization) 定制登录页品牌、自动将特定域名邮箱用户加入指定组织 (Organization)、集成企业级 SSO 等。

这些都是开箱即用的功能，更多信息可在 Logto 文档中找到：
...
最棒的是？我们无需重复造轮子。借助 Logto 等现代工具，把原本可能需要数月开发的内容变成了几分钟搞定。
