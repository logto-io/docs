...
你是否在为你的应用实现一个安全且可扩展的授权 (Authorization) 系统而苦恼？基于角色的访问控制 (RBAC) 是管理用户权限的行业标准，但正确实现它并不容易。本教程将通过一个真实的内容管理系统 (CMS) 示例，教你如何构建一个健壮的 RBAC 系统。
...
基于角色的访问控制 (RBAC) 不只是给用户分配权限。它是关于创建一种结构化的授权 (Authorization) 方法，在安全性和可维护性之间取得平衡。
...
| 端点                              | 访问控制逻辑                                              |
| --------------------------------- | --------------------------------------------------------- |
| GET /api/articles                 | - 具有 `list:articles` 权限的用户，或作者可查看自己的文章 |
| GET /api/articles/:id             | - 具有 `read:articles` 权限的用户，或文章作者             |
| POST /api/articles                | - 具有 `create:articles` 权限的用户                       |
| PATCH /api/articles/:id           | - 具有 `update:articles` 权限的用户，或文章作者           |
| DELETE /api/articles/:id          | - 具有 `delete:articles` 权限的用户，或文章作者           |
| PATCH /api/articles/:id/published | - 仅具有 `publish:articles` 权限的用户                    |

### 构建可扩展的权限系统 \{#creating-a-permission-system-that-scales}

基于我们的 API 访问需求，我们可以定义如下权限：
...
注意，这些权限只在访问你不拥有的资源时需要。文章所有者可以自动：

- 查看自己的文章（无需 `read:articles` 权限）
- 编辑自己的文章（无需 `update:articles` 权限）
- 删除自己的文章（无需 `delete:articles` 权限）

## 构建高效的角色 \{#building-effective-roles}

现在我们已经定义了 API 和权限，可以创建逻辑上分组的角色：
...
在开始之前，你需要在 [Logto Cloud](https://cloud.logto.io) 创建一个账号，或者你也可以使用 [Logto OSS 版本](/logto-oss) 自建 Logto 实例。
...
现在我们已经在 Logto 中配置好 RBAC，可以开始将其集成到前端。

首先，按照 [Logto 快速开始](/quick-starts) 将 Logto 集成到你的应用中。

本示例中，我们以 [React](/quick-starts/react) 为例进行演示。

在你的应用中配置好 Logto 后，需要为 Logto 添加 RBAC 配置。
...
const logtoConfig: LogtoConfig = {
  ...
  resources: [API_RESOURCE], // 你在 Logto 控制台创建的 API 资源标识符，如 https://api.cms.com
...
如你所见，在这个中间件中，我们会校验前端请求是否包含有效的访问令牌 (Access token)，并检查访问令牌的受众 (audience) 是否与我们在 Logto 控制台创建的 API 资源一致。

校验 API 资源的原因是我们的 API 资源实际上代表了 CMS 后端的资源，所有 CMS 权限都与该 API 资源关联。

由于该 API 资源在 Logto 中代表 CMS 资源，在前端代码中，我们在请求后端 API 时会携带相应的访问令牌 (Access token)：
...
对于只允许特定权限用户访问的 API，可以直接在中间件中添加限制。例如，文章创建 API 只允许拥有 `create:articles` 权限的用户访问：
...
Alex 拥有管理员 (Admin) 角色，可以创建、删除、更新、发布和查看所有文章。
...
对于更复杂的场景，比如构建多租户应用，Logto 提供了完善的组织 (Organization) 支持。你可以查阅我们的指南 [构建多租户 SaaS 应用：从设计到实现的完整指南](https://blog.logto.io/build-multi-tenant-saas-application) 了解如何实现组织级别的访问控制。
