...
要执行这些任务，你的 AI 代理需要安全地访问用户授权的第三方 API，这意味着要正确且安全地处理 OAuth 令牌 (Token)。
...
sequenceDiagram
    ...
    rect rgba(200, 230, 200, 0.5)
        ...
        Google->>Logto: 返回授权码
        Logto->>Google: 用授权码换取令牌 (Token)
        Logto->>Logto: 将令牌 (Token) 存储到 Secret Vault
        Logto->>Agent: 返回验证结果
    end
...
2. **授权提示**：代理检测到需要第三方访问权限，并提示用户授权。
3. **令牌 (Token) 存储**：用户授权后，Logto 会将访问令牌 (Access token) 和刷新令牌 (Refresh token) 安全地存储在 Secret Vault 中。
4. **任务执行**：代理检索已存储的令牌 (Token)，调用第三方 API 完成任务。

一旦授权，用户可以多次执行任务而无需重复授权。Logto 会安全地存储令牌 (Token)，并在需要时自动刷新，确保 AI 代理持续流畅地与第三方服务交互。
...
- 一个拥有 API 访问权限的第三方服务账号（如 [Google Cloud Console](https://console.cloud.google.com)）
- 一个已集成 Logto SDK 的 AI 代理应用（用户可以登录你的 AI 代理）
...
4. 在连接器设置中：
   - 启用 **为持久 API 访问存储令牌 (Token)**，将令牌 (Token) 存储到 Secret Vault。
...
:::info 启用 Account API
在继续之前，请在 <CloudLink to="/sign-in-experience/account-center">控制台 > 登录与账户 > 账户中心</CloudLink> 启用 Account API。了解更多关于[启用 Account API](/end-user-flows/account-settings/by-account-api#how-to-enable-account-api)。
:::

### 步骤 1：检查是否已授权 \{#step-1-check-for-existing-authorization}

首先，尝试检索已存储的访问令牌 (Access token)，判断用户是否已授权：
...
如果没有令牌 (Token)、令牌已过期，或你需要扩展访问令牌 (Access token) 的权限 (Scope)，可使用 Logto 的 [Social Verification API](/secret-vault/federated-token-set#reauthentication-and-token-renewal) 发起授权流程：
...
用户授权后，Google 会跳转回你的应用。完成验证并存储令牌 (Token)：
...
async function handleAuthorizationCallback(
  ...
  // 将令牌 (Token) 存储到 Logto 的 Secret Vault
...
现在你的 AI 代理可以获取令牌 (Token) 并调用 API：
...
async function addCalendarEvent(userAccessToken: string, eventDetails: EventDetails) {
  ...
  if (!tokenData) {
    // 用户尚未授权，请求带 calendar 权限 (Scope) 的授权
...
Logto 会自动处理令牌 (Token) 刷新。如果访问令牌 (Access token) 已过期但存在刷新令牌 (Refresh token)，当你调用检索接口时，Logto 会自动获取新的访问令牌 (Access token)。

## 请求更多权限 (Scope) \{#request-additional-permissions}

随着你的 AI 代理承担更多任务，可能需要请求更多权限 (Scope)。例如，用户最初只授权了只读日历权限，现在希望创建事件，则需要写入权限。

### 为什么要渐进式授权？ \{#why-incremental-authorization}

- **更好的用户体验**：用户在理解上下文后更愿意授权。
- **更高的转化率**：前期权限 (Scope) 越少，阻力越小。
- **建立信任**：只请求所需权限 (Scope) 的应用更容易获得用户信任。
...
:::tip
请求更多权限 (Scope) 时，用户会看到只包含新增权限的用户授权页面 (Consent screen)，已有权限会被保留。
:::

## 管理令牌 (Token) 状态 \{#manage-token-status}

Logto 控制台为每个用户提供令牌 (Token) 状态可视化：

1. 进入 <CloudLink to="/users">控制台 > 用户管理</CloudLink>。
2. 点击某个用户查看详情。
3. 滚动到 **连接** 部分，查看所有已关联的社交账号。
4. 每个连接会显示令牌 (Token) 状态：
   - **活跃**：访问令牌 (Access token) 有效，可直接使用。
   - **已过期**：访问令牌 (Access token) 已过期。如果存在刷新令牌 (Refresh token)，下次检索时会自动刷新。
   - **未激活**：该连接未存储任何令牌 (Token)。
...
- **使用渐进式授权**：按需请求更多权限 (Scope)，不要一次性全部请求。
- **优雅处理令牌 (Token) 过期**：始终处理令牌 (Token) 可能过期或被撤销的情况。
- **保护用户访问令牌 (Access token)**：用户的 Logto 访问令牌 (Access token) 是检索第三方令牌 (Token) 的钥匙，请妥善保护。
