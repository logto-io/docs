...
本指南将带你完成将 Logto 集成到你的服务，并启用 AI 代理访问的流程。
...
- 工作原理：官方天气应用通过 Logto 连接到你的服务进行用户认证 (Authentication)。当 Alice 登录时，她会自动获得天气数据的访问权限，无需额外的权限页面，因为这是你信任的应用。

**第三方 AI 代理**

- 你正在围绕你的服务构建一个生态系统，因此另一位开发者创建了“WeatherBot”（一个可以提供天气更新和预报的 AI 助手），将其作为第三方 AI 代理集成。
- 工作原理：WeatherBot（第三方 AI 代理）希望通过你的服务访问用户的天气数据。当 Alice 使用她的账户登录 WeatherBot 时：
  - 她会看到一个用户授权页面 (Consent screen)，请求 WeatherBot 访问她的天气数据的权限。
  - Alice 可以允许或拒绝此访问。
  - 只有她同意的数据会被共享给 WeatherBot，WeatherBot 无法在没有明确重新授权的情况下访问任何额外数据。

这种访问（权限）控制确保了用户数据的安全，即使你的服务管理所有数据，像 WeatherBot 这样的第三方 AI 代理也只能访问用户明确允许的数据。他们无法绕过这个过程，因为这是由你在服务中实现的访问控制所强制执行的。
...
:::note
如果你想将你的服务与你自己的 AI 代理或应用集成，请参考我们的[快速开始指南](/quick-starts)获取更多信息。
:::

## 前置条件 \{#prerequisites}

- 一个 [Logto Cloud](https://cloud.logto.io)（或自托管）租户
- 一个对 AI 代理开放 API 端点的服务
...
sequenceDiagram
    ...
    Service->>Agent: 未认证 (401 Unauthorized)
    Agent->>Logto: 重定向到 Logto 进行认证 (Authentication)
    Logto->>Logto: 用户认证 (Authentication)
    Logto->>Logto: 用户授权 (Authorization)（同意）AI 代理的权限 (Scopes)
    Logto->>Agent: 重定向回服务并携带授权码
    Agent->>Logto: 使用授权码请求访问令牌 (Access token)
    Logto->>Agent: 返回访问令牌 (Access token)
    Agent->>Service: 使用访问令牌 (Access token) 请求资源
    Service->>Service: 验证访问令牌 (Access token)
...
1. **在 Logto 注册 AI 代理**：在 Logto 中创建一个应用来代表 AI 代理。
2. **配置 AI 代理**：确保 AI 代理能够向你的服务发起请求，并能处理 401 Unauthorized 响应。
3. **实现授权 (Authorization) 流程**：AI 代理应能处理 OAuth 2.0 授权 (Authorization) 流程，从 Logto 获取访问令牌 (Access token)。
4. **测试集成**：使用 AI 代理访问你的服务，验证其能成功完成认证 (Authentication) 和授权 (Authorization) 请求。
...
2. **实现访问控制逻辑**：在你的服务中，实现逻辑以验证从 AI 代理收到的访问令牌 (Access token)，并检查用户是否有权限访问所请求的资源。
3. **响应 AI 代理**：如果访问令牌 (Access token) 有效且用户有权限，则返回所请求的资源；否则返回 401 Unauthorized 响应。

要了解如何在你的服务中实现访问控制，请参考我们的[授权 (Authorization) 指南](/authorization)。
...
4. AI 代理应能处理 401 Unauthorized 响应，并将用户重定向到 Logto 进行认证 (Authentication)。
