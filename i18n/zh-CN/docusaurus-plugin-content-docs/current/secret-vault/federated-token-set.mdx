...
:::Info
此功能仅适用于支持令牌存储的连接器。目前支持的连接器包括：[GitHub](/integrations/github)、[Google](/integrations/google)、[Facebook](/integrations/facebook)、[标准 OAuth 2.0](/integrations/oauth2) 和 [标准 OIDC](/integrations/oidc)。后续将逐步支持更多连接器。
:::
...
存储的令牌会附加到用户的社交或企业单点登录 (SSO) 身份上，允许用户后续无需重新认证 (Authentication) 即可检索令牌用于 API 访问。
...
2. 点击你想要查看的用户，进入该用户详情页。
3. 滚动到 **连接** 区域。此区域列出了与用户关联的所有社交和企业单点登录 (SSO) 连接。
...
为保证数据完整性和安全性，所有令牌在存储到 Secret Vault 前都会被加密。实际令牌值仅对拥有适当授权的终端用户可见。开发者只能获取令牌集的元数据，以便了解存储令牌的状态，而不会暴露敏感内容。
...
- `hasRefreshToken`：指示是否有刷新令牌 (Refresh token) 可用。
  ...
  当访问令牌 (Access token) 过期且存在有效刷新令牌 (Refresh token) 时，用户请求访问连接的提供商时，Logto 会自动尝试使用存储的刷新令牌 (Refresh token) 获取新的访问令牌 (Access token)。
- `expiresAt`：访问令牌 (Access token) 的预计过期时间（单位：秒）。
  该值基于身份提供商的令牌端点返回的 `expires_in` 计算。（仅当提供商在令牌响应中包含 `expires_in` 字段时可用。）
...
联邦令牌存储直接绑定到每个用户的社交或企业单点登录 (SSO) 连接。这意味着在以下情况下，存储的令牌集会被自动删除：
...
撤销令牌集后，用户必须重新通过第三方提供商认证 (Authentication) 获取新的访问令牌 (Access token)，才能再次访问第三方 API。

## 重新认证 (Reauthentication) 与令牌续期 \{#reauthentication-and-token-renewal}

在存储的访问令牌 (Access token) 已过期或应用需要请求更多 API 权限范围的场景下，终端用户可以重新通过第三方提供商认证 (Authentication) 以获取新的访问令牌 (Access token)——无需再次登录 Logto。
...
1. 用户通过调用 `POST /api/verification/social` 接口发起社交验证请求。用户可以指定自定义权限范围 (scope) 以请求第三方提供商的额外权限。
   ...
   - **redirectUri**：认证 (Authentication) 完成后将用户重定向回你的应用的 URI。你需要在提供商的应用设置中注册该 URI。
   - **scope**：（可选）自定义权限范围 (scope)，用于请求第三方提供商的额外权限。如果未指定，则使用连接器中配置的默认权限范围 (scope)。

2. Logto 创建新的社交验证记录，并返回社交验证 ID 以及用于跳转到第三方提供商认证 (Authentication) 的授权 URL。
...
3. 将用户重定向到授权 URL。用户在第三方提供商处认证 (Authentication) 并授权 (Authorization)。
