...
你可以创建一个映射，将现有用户档案中的用户信息匹配到 **基本数据** 和 **自定义数据**。对于社交登录，你需要额外的步骤来导入社交身份，请参考 [链接社交身份到用户](https://openapi.logto.io/operation/operation-createuseridentity) 的 API。

## 密码哈希

Logto 使用 [Argon2](https://en.wikipedia.org/wiki/Argon2) 来哈希用户的密码，同时也支持其他算法，如 `MD5`、`SHA1`、`SHA256` 和 `Bcrypt`，以方便迁移。这些算法被认为是不安全的，相应的密码哈希将在用户首次成功登录时迁移到 Argon2。
...
4.  **导入用户数据**
    建议准备一个脚本逐个导入用户数据，我们将调用 [create user](https://openapi.logto.io/operation/operation-createuser) API 来导入用户数据。以下是脚本示例：
...
请注意，API 端点有速率限制，你应该在每个请求之间添加一个暂停以避免速率限制。请查看我们的 [速率限制](/integrate-logto/interact-with-management-api/#rate-limit) 页面了解详情。

如果你有大量的用户数据（10 万以上用户），你可以 [联系我们](https://logto.io/contact) 以增加速率限制。
