...
使用 PAT 获得的访问令牌 (Access tokens) 与通过标准 `refresh_token` 流获得的令牌**完全相同**。这意味着：

- **组织 (Organization) 上下文**：PAT 获得的令牌支持与 refresh token 流相同的组织权限 (Permissions) 和权限 (Scopes)
- **授权 (Authorization) 流程**：你可以将 PAT 交换获得的访问令牌 (Access tokens) 用于[组织权限 (Permissions)](/authorization/organization-permissions)和[组织级 API 资源](/authorization/organization-level-api-resources)
- **令牌验证**：验证逻辑相同——只有初始授权类型不同

如果你在与组织 (Organizations) 协作，无论你使用 PAT 还是 refresh token，访问模式和权限 (Permissions) 都是一样的。

:::

### 请求 \{#request}

应用程序使用 HTTP POST 方法，向租户的 [token 端点](/integrate-logto/application-data-structure#token-endpoint) 发起[令牌交换请求](https://auth.wiki/authorization-code-flow#token-exchange-request)，并使用特殊的授权类型。以下参数以 `application/x-www-form-urlencoded` 格式包含在 HTTP 请求实体主体中。
...
6. `subject_token_type`：必填。在 `subject_token` 参数中提供的安全令牌类型。该参数的值必须为 `urn:logto:token-type:personal_access_token`。

### 响应 \{#response}

如果令牌交换请求成功，租户的 token 端点会返回一个代表用户身份的访问令牌 (Access token)。响应以 `application/json` 格式在 HTTP 响应实体主体中包含以下参数。
