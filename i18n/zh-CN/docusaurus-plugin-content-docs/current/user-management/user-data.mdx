...
- [基本数据](#basic-data)：来自用户资料的基本信息。它存储所有其他 _用户_ 的属性，除了社交 `identities` 和 `custom_data`，例如用户 ID、用户名、电子邮件、电话号码以及用户上次登录的时间。
- [社交身份](#social-identities)：存储从社交登录（即使用社交连接器登录）中获取的用户信息，例如 Facebook、GitHub 和微信。
...
_avatar_ 是指向用户头像图片的 URL。其最大长度为 2048。
...
_profile_ 存储其他未包含在用户属性中的 OpenID Connect [标准声明](https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims)。
...
与其他标准声明相比的一个区别是，`profile` 中的属性只有在其值不为空时才会包含在 [ID 令牌](https://auth.wiki/id-token) 或用户信息端点响应中，而其他标准声明如果值为空将返回 `null`。
...
_last_sign_in_at_ 是用户上次登录时带有时区的时间戳。
...
_is_suspended_ 是一个布尔值，指示用户是否被暂停。可以通过调用 [Logto Management API](https://openapi.logto.io/operation/operation-updateuserissuspended) 或使用 Logto Console 来管理该值。

一旦用户被暂停，预先授予的刷新令牌将立即被撤销，用户将无法再通过 Logto 进行认证 (Authentication)。

## 属性参考 \{#property-reference}

以下属性（除 _password_encrypted_ 和 _password_encryption_method_ 外）在用户资料上可见，这意味着你可以使用 [Management API](https://openapi.logto.io/operation/operation-getuser) 查询它们。
...
| [name](#name)                                             | string    | 全名                       | ❌   | ❌   |
| [avatar](#avatar)                                         | string    | 指向用户头像图片的 URL     | ❌   | ❌   |
...
| [last_sign_in_at](#last_sign_in_at)                       | date time | 用户上次登录的时间戳       | ❌   | ✅   |
...
注意：不要在 _custom_data_ 中放置敏感数据。

你可以使用 [Management API](https://openapi.logto.io/operation/operation-listusercustomdata) 获取包含 _custom_data_ 的用户资料，并将其发送到前端应用程序或外部后端服务。因此，将敏感信息放在 _custom_data_ 中可能会导致数据泄露。

如果你仍然想将敏感信息放在 _custom_data_ 中，我们建议先加密它。仅在受信任的方（如你的后端服务）中进行加密 / 解密，避免在前端应用程序中进行。这将最大限度地减少如果用户的 _custom_data_ 被错误泄露时的损失。

你可以使用 [Logto Console](/user-management/manage-users/#view-and-update-the-user-profile) 或 [Logto Management API](/user-management/manage-users/#manage-via-logto-management-api) 更新用户的 _custom_data_，例如 [`PATCH /api/users/:userId`](https://openapi.logto.io/operation/operation-getuser)。
