...
2. 配置 OAuth 设置：
   - 打开你想用于登录的应用，点击“回调地址”（如果你还没有编辑回调地址，它会显示为“已启用”）
   - 添加授权回调地址：`${your_logto_origin}/callback/${connector_id}`
   - `connector_id` 可以在 Logto 控制台连接器详情页顶部找到
3. 在应用详情页获取 `AppID` 和 `AppSecret`
4. 在 Logto 控制台中将第 3 步获取的值填写到 `clientId` 和 `clientSecret` 字段
5. 可选配置：
   - `skipConfirm`：当用户已登录小米账号时，是否跳过小米授权确认页面，默认为 false
...
就是这样。别忘了在注册和登录中[启用社交登录](https://docs.logto.io/connectors/social-connectors#enable-social-sign-in)。
