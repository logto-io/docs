...
Google 连接器支持 OAuth 2.0 集成，让你的应用可以：

- 添加“使用 Google 登录”认证 (Authentication)
- 将用户账户与 Google 身份关联
- 从 Google 同步用户资料信息
- 通过 Logto Secret Vault 安全存储令牌访问 Google API，实现自动化任务（如编辑 Google Docs、在你的应用中管理日历事件等）

要设置这些认证 (Authentication) 功能，请先在 Logto 中创建一个 Google 连接器：

1. 前往 <CloudLink to="/connectors/social">Logto 控制台 > 连接器 > 社交连接器</CloudLink>。
2. 点击 **添加社交连接器**，选择 **Google**，点击 **下一步**，并按照分步教程完成集成。

<Integration />

## 使用 Google 连接器 \{#utilize-the-google-connector}

创建并连接 Google 连接器后，你可以将其集成到终端用户流程中。根据你的需求选择相应选项：
...
2. 在 **社交登录** 部分添加 Google 连接器，让用户可以通过 Google 认证 (Authentication)。
...
:::tip
你可以仅为账户关联和 API 访问启用 Google 连接器，而不启用社交登录。
:::

### 访问 Google API 并执行操作 \{#access-google-apis-and-perform-actions}

你的应用可以从 Secret Vault 获取存储的 Google 访问令牌，用于调用 Google API 并自动化后端任务（例如管理 Google Drive 文件、创建日历事件或通过 Gmail 发送邮件）。请参考关于检索存储令牌以访问 API 的指南。
...
2. 在 **社交连接** 下找到 Google 项，点击 **管理**。
3. 在此页面，管理员可以管理用户的 Google 连接，查看所有从 Google 账户授权并同步的资料信息，以及检查访问令牌状态。
