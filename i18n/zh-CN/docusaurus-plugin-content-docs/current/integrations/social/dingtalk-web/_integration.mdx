...
将你在上文 OAuth 应用详情页获得的 _Client ID（原 AppKey 和 SuiteKey）_ 和 _Client Secret（原 AppKey 和 SuiteKey）_ 填写到 `clientId` 和 `clientSecret` 字段中。
...
就是这样。现在钉钉连接器应该可以使用了。别忘了 [在注册和登录中启用连接器](https://docs.logto.io/docs/recipes/configure-connectors/social-connector/enable-social-sign-in/)。

启用钉钉 Web 连接器后，你可以再次登录你的应用，检查是否正常工作。
...
如果你有任何问题或需要进一步的帮助，请访问 [钉钉开发者文档](https://open.dingtalk.com/document/orgapp/obtain-identity-credentials) 或联系钉钉技术支持。
