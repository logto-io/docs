...
1. 主体令牌是短期的并且仅供一次使用。
2. 模拟访问令牌不附带 [刷新令牌](https://auth.wiki/refresh-token)。如果令牌在 Sarah 解决 Alex 的问题之前过期，Sarah 将需要重复此过程。
...
这个 [context](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) 然后可以在 `getCustomJwtClaims()` 函数中使用，以向最终访问令牌添加特定声明。以下是如何实现的示例：
