...
在某些 API 架构中，后端服务会收到已登录用户的请求，并需要在保留用户身份的情况下调用另一个后端服务。
...
使用 Logto 的令牌交换授权 (token exchange grant)，可以将用户的访问令牌 (access token) 交换为以下游 API 资源为受众 (audience) 的新访问令牌 (access token)。这遵循 OAuth 2.0 令牌交换模式，避免将原始用户令牌转发到下游服务。
...
- API A 需要代表同一用户调用 API B。
- API B 应该验证一个以自身 API 资源为受众 (audience) 的访问令牌 (access token)。

不要在没有用户的纯机器对机器 (machine-to-machine) 访问场景下使用此流程。在这种情况下，请使用 [客户端凭据流程](/quick-starts/m2m)。对于支持、管理员或代理等场景，即一个用户临时以另一个用户身份操作，请使用 [用户模拟 (User impersonation)](/developers/user-impersonation)。
...
对于带有应用密钥的传统 Web 应用或机器对机器 (machine-to-machine) 应用，在 `Authorization` 头中包含凭据：
...
5. `scope`：API A 为本次委托调用请求的下游权限 (Scopes)。Logto 仅会根据 RBAC 设置，为原始用户在该资源上可用的权限 (Scopes) 签发请求的权限。
...
5. 检查所需权限 (Scopes)。
6. 使用 `sub` 作为原始用户 ID。
7. 如果只允许特定上游服务进行委托调用，可选地检查 `client_id`。

实现指导参见 [在 API 中验证访问令牌 (access token)](/authorization/validate-access-tokens)。

## 相关资源 \{#related-resources}

<Url href="/authorization/global-api-resources">保护全局 API 资源</Url>

<Url href="/authorization/validate-access-tokens">在 API 中验证访问令牌 (access token)</Url>
