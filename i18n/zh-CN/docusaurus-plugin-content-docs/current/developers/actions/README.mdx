...
- 调用外部服务并将其结果应用到 Logto 用户。

:::note
Actions 仅在 Logto Cloud 企业版计划中可用。
:::

:::warning
Action 脚本可以影响认证 (Authentication) 并修改用户数据。只有受信任的管理员才应被允许查看、创建、编辑、测试、启用或删除它们。

在自托管部署中，Action 脚本在 Logto 服务器进程内以其权限运行。授予脚本编辑或测试权限等同于授予 Logto 主机上的代码执行权限，因此不应将管理控制台与不受信任的用户共享。将脚本视为受信任的服务端代码；运行时会限制脚本的时间和内存，但这不是针对不受信任代码的安全边界。
:::

## Actions 在登录流程中的作用 \{#how-actions-fit-into-sign-in}
...
编辑器提供类型信息，但保存的脚本会作为 JavaScript 执行。脚本可以是异步的，并且在 Logto Cloud 和自托管 Logto 中都可以使用以下标准 Web API：
...
脚本不能导入包。请避免使用 Node.js 特有的全局变量和模块，因为它们在自托管 Logto 与 Logto Cloud 之间不可移植，也不属于支持的脚本约定。
...
|                                        | Actions                              | Webhook                                       |
| -------------------------------------- | ------------------------------------ | --------------------------------------------- |
| 执行方式                               | 同步，嵌入认证 (Authentication) 流程 | 异步，认证 (Authentication) 请求之外          |
...
异步操作请放在 Webhook 中。只有当 Logto 需要在认证 (Authentication) 继续前获得结果时，才使用 Action。
