...
- **签名密钥**： Logto 为每个 Webhook 生成一个唯一的哈希签名，称为签名密钥。你可以在端点实现中将此密钥作为请求头包含在内。验证签名密钥可确保 Webhook 负载来自 日志 (Logto)，并且未被未经授权的来源篡改。阅读 [保护你的 Webhook](/developers/webhooks/secure-webhooks/) 以了解更多关于代码的信息。
