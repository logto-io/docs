...
| Role.Scopes.Updated | 分配给角色 (Role) 的权限被添加或移除。                       |

### 权限 (Scope) \{#permission-scope}

| 事件类型           | 描述                                          |
| ------------------ | --------------------------------------------- |
| Scope.Created      | 创建了一个新的 API 权限 (Scope)。             |
| Scope.Deleted      | 删除了一个 API 权限 (Scope)。                 |
| Scope.Data.Updated | API 权限 (Scope) 的数据被更新，例如权限描述。 |
...
| Organization.Membership.Updated | 用户或应用被添加到组织 (Organization) 或从组织移除。负载包含 [成员变更字段](/developers/webhooks/webhooks-request#organizationmembershipupdated-payload)。 |
...
| OrganizationRole.Scopes.Updated | 分配给组织角色 (Organization role) 的权限被添加或移除。             |
...
### Experience API 触发事件 \{#experience-api-triggered-events}
...
| Message.RateLimited | 终端用户超过了每个收件人的 [验证码发送速率限制](/security/send-rate-limit)。仅在终端用户发送路径（Experience API 和 Account API）触发，负载为 `{ action, recipient }`。 |
