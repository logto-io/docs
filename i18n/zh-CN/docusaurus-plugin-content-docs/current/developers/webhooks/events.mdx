...
| Organization.Membership.Updated | 用户或应用程序被添加到组织 (Organization) 或从组织中移除。负载包含 [成员变更字段](/developers/webhooks/webhooks-request#organizationmembershipupdated-payload)。 |
...
| Grant.LimitExceeded | 一次成功的授权 (Authorization) 使用户超过了应用的 [最大并发认证 (Authentication) 设备数](/sessions/session-configs#max-concurrent-authenticated-devices-per-app)（`maxAllowedGrants`），Logto 会撤销其最早的授权。 |
