...
当一个 webhook 事件触发时，Logto 会向每个订阅了该事件的端点发送一个 `POST` 请求。完整的事件目录在 [Webhooks 事件](/developers/webhooks/events)中；本页面记录了 Logto 发送的请求的**结构**。
...
**事件：** `User.*`, `Role.*`, `Scope.*`, `Organization.*`, `OrganizationRole.*`, `OrganizationScope.*` 下的每个事件——参见 [Webhooks 事件 → 数据变更 hook 事件](/developers/webhooks/events#data-mutation-hook-events) 以获取完整目录。
