...
Logto Management API 是一套全面的 API，开发者可以完全控制实现方式，以满足产品需求和技术栈。它是预置的，列在 <CloudLink to="/api-resources">控制台 > API 资源 > Logto Management API</CloudLink>，且无法删除或修改。
...
通过 Logto Management API，你可以访问 Logto 强大的后端服务，这些服务具备高度可扩展性，可用于多种场景。它超越了管理控制台低代码能力的限制。
...
我们的开发者已经通过 Logto Management API 实现了许多额外功能。我们相信我们的 API 具备高度可扩展性，可以满足你广泛的需求。以下是一些无法通过 Logto 管理控制台实现，但可以通过 Logto Management API 实现的场景示例。

### 自主实现用户资料页 \{#implement-user-profile-on-your-own}

Logto 目前尚未提供用户资料的预置 UI 方案。我们认识到用户资料与业务和产品属性密切相关。在我们探索最佳方案的同时，建议你使用我们的 API 自行实现。例如，你可以利用我们的交互 API、资料 API 和验证码 API，开发满足你需求的自定义方案。

### 高级用户搜索 \{#advanced-user-search}

Logto 管理控制台支持基础的搜索和筛选功能。对于模糊搜索、精确匹配、区分大小写等高级搜索选项，请参考我们的 [高级用户搜索](/user-management/advanced-user-search) 教程和指南。

### 自主实现组织管理 \{#implement-organization-management-on-your-own}

如果你正在使用 [组织 (Organizations)](/organizations) 功能构建多租户应用，可能需要通过 Logto Management API 完成如组织邀请、成员管理等任务。对于你的 SaaS 产品，如果租户中既有管理员又有成员，Logto Management API 可以帮助你打造符合业务需求的自定义管理后台。详情请查看 [这里](/end-user-flows/organization-experience/)。

## 使用 Logto Management API 的小贴士 \{#tips-for-using-logto-management-api}

### 管理分页 API 响应 \{#managing-paginated-api-responses}

部分 API 响应可能包含大量结果，结果会被分页。Logto 提供两种分页信息。
...
除了标准的 link headers，Logto 还会添加一个 `Total-Number` 头：
...
### 速率限制 (Rate limit) \{#rate-limit}

:::note
仅适用于 Logto Cloud。
:::

Logto Cloud 对租户级运行时请求速率进行限制，以保护系统稳定性。详情请参见 [系统限制速率限制部分](/logto-cloud/system-limit#rate-limit)。
