...
通过启用特定权限 (Scopes)，你可以决定第三方应用能够访问哪些用户信息。用户将在用户授权页面 (Consent screen) 上查看并批准这些权限后，才会授予访问权限。
...
分配用户级别的权限，包括[用户资料权限](#user-permissions-user-profile-scopes)（如邮箱、姓名和头像）和 [API 资源权限](#api-resource-permissions-api-resource-scopes)（如对特定资源的读或写权限）。

所请求资源的名称（如个人用户数据、API 名称）和具体权限描述（如你的邮箱地址）会显示在用户授权页面 (Consent screen) 上，供用户审核。
...
分配组织 (Organization) 级别的权限，包括[组织 (Organization) 权限](#organization-permissions-organization-scopes)和 [API 资源权限](#api-resource-permissions-api-resource-scopes)。Logto 支持将 API 资源分配给特定的组织 (Organization) 角色。

在用户授权页面 (Consent screen) 上，组织 (Organization) 数据会与用户数据分开展示。在授权 (Authorization) 流程中，用户必须选择一个具体的组织 (Organization) 进行授权。用户可以在确认前切换组织 (Organization)。第三方应用只会获得所选组织 (Organization) 的数据及相关权限的访问权。
...
- `urn:logto:scope:organizations`：Logto 用户组织 (Organization) 权限 (Scope)，用于访问用户组织 (Organization) 信息。如果启用并被第三方应用请求，用户授权页面 (Consent screen) 上会显示组织 (Organization) 选择器。用户可以审核并选择希望授权访问的组织 (Organization)。详见[组织 (Organizations)](/organizations)。
- `urn:logto:scope:organization_roles`：Logto 用户组织 (Organization) 权限 (Scope)，用于访问用户组织 (Organization) 角色信息。
...
Logto 为 API 资源提供基于角色的访问控制 (RBAC)。API 资源是你的服务所拥有并由 Logto 保护的资源。你可以为第三方应用分配自定义 API 权限 (Scopes) 以访问你的 API 资源。详情请参考[授权 (Authorization)](/authorization)。

你可以在 <CloudLink to="/api-resources">控制台 > API 资源</CloudLink> 下创建和管理你的 API 资源权限 (Scopes)。

:::warning
未为第三方应用启用的 API 资源权限 (Scopes) 在发送授权 (Authorization) 请求时会被忽略。它不会显示在用户授权页面 (Consent screen) 上，也不会被 Logto 授权 (Authorization)。
:::

### 组织 (Organization) 权限（组织权限 Scopes） \{#organization-permissions-organization-scopes}

[组织 (Organization) 权限](/authorization/organization-template) 是专为 Logto 组织 (Organization) 定义的权限 (Scopes)。它们用于访问组织 (Organization) 信息和资源。
...
你可以在组织 (Organization) 模板设置页面自定义你的组织 (Organization) 权限 (Scopes)。详见[组织模板](/authorization/organization-template)。

:::warning
未为第三方应用启用的组织 (Organization) 权限 (Scopes) 在发送授权 (Authorization) 请求时会被忽略。它不会显示在用户授权页面 (Consent screen) 上，也不会被 Logto 授权 (Authorization)。
:::
