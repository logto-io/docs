...
第三方应用（非你服务自有的应用）作为身份提供商 (IdP) 集成到 Logto，用于认证 (Authentication) 用户。这些应用通常来自外部服务提供商，因此需要谨慎管理权限，以保护用户数据。
...
通过启用特定权限 (Scopes)，你可以决定第三方应用能够访问哪些用户信息。用户将在用户授权页面 (Consent screen) 上查看并批准这些权限后才会授予访问权限。

## 管理你的 OIDC 第三方应用的权限 \{#manage-the-permissions-of-your-oidc-third-party-applications}

前往你的 OIDC 第三方应用的 <CloudLink to="/applications">控制台 > 应用程序 > 应用详情页</CloudLink>，切换到 **权限** 标签页并点击 **添加权限** 按钮，即可管理第三方应用的权限。
...
分配组织 (Organization) 级别的权限，包括[组织 (Organization) 权限](#organization-permissions-organization-scopes)和 [API 资源权限](#api-resource-permissions-api-resource-scopes)。Logto 支持将 API 资源分配给特定组织角色。

在用户授权页面 (Consent screen) 上，组织 (Organization) 数据会与用户数据分开展示。在授权 (Authorization) 流程中，用户必须选择一个具体的组织 (Organization) 进行授权。用户可以在确认前切换组织 (Organization)。第三方应用仅能访问所选组织 (Organization) 的数据及相关权限。
...
- `urn:logto:scope:organizations`：Logto 用户组织 (Organization) 权限 (Scope)，用于访问用户组织 (Organization) 信息。如果启用并被第三方应用请求，用户授权页面 (Consent screen) 上会显示组织 (Organization) 选择器，允许用户审核并选择要授权的组织 (Organization)。详见[组织 (Organizations)](/organizations)。
...
Logto 为 API 资源提供基于角色的访问控制 (RBAC)。API 资源是你服务所拥有并由 Logto 保护的资源。你可以为第三方应用分配自定义 API 权限 (Scopes) 以访问你的 API 资源。详情请参考[授权 (Authorization)](/authorization)。
...
[组织 (Organization) 权限](/authorization/organization-template) 是专为 Logto 组织 (Organizations) 定义的权限 (Scopes)，用于访问组织 (Organization) 信息和资源。

:::note
要使用 Logto 组织 (Organization) 权限，需先启用 `urn:logto:scope:organizations` 用户权限 (Scope)。否则，在发送授权 (Authorization) 请求时，组织 (Organization) 权限会被忽略。
:::
...
1. `openid`：OIDC 认证 (Authentication) 必需（纯 OAuth 可选）。授予 ID 令牌 (ID token) 及访问 `userinfo_endpoint`。
