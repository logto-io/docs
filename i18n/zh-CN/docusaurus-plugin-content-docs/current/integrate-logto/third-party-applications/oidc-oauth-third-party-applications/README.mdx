---
description: 集成第三方 OIDC / OAuth 应用程序，并通过用户授权页面提供 OAuth 授权。
---
...
Logto 是一个基于 [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) 协议的身份服务，提供 [认证 (Authentication)](https://auth.wiki/authentication) 和 [授权 (Authorization)](https://auth.wiki/authorization) 功能。这使得集成 OIDC 第三方应用程序与传统 Web 应用程序一样简单。
...
这些请求的权限只有在用户点击“授权 (Authorize)”按钮后才会授予第三方应用程序。
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: '个性化用户授权页面的外观，以匹配你的品牌标识，并提供一致的用户体验。',
      customProps: {
