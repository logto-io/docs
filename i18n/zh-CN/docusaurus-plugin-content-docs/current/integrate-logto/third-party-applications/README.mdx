...
如需限制哪些用户可以登录第三方应用，请在应用的 **规则** 标签页配置[应用级访问控制](/integrate-logto/app-level-access-control)。
...
如果第三方应用需要以编程方式将 Logto 作为 IdP 集成，应实现标准的 [授权码流程](https://auth.wiki/authorization-code-flow)。我们建议使用适合你编程语言的 OAuth 2.0 / OIDC 客户端库来实现。
