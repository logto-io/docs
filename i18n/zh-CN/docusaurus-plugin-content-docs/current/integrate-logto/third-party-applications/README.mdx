...
Logto 的第三方应用集成让你能够将 Logto 作为外部应用的 [身份提供商 (IdP)](https://auth.wiki/identity-provider)。
...
这种集成方式非常适合常见的业务场景。你可以让用户使用他们的 Logto 账户访问合作伙伴应用，就像企业用户用 Google Workspace 登录 Slack 一样。你也可以构建一个开放平台，让第三方应用添加“使用 Logto 登录”功能，类似于“使用 Google 登录”。
...
3. 根据第三方应用的类型选择应用类型：
   ...
   - **单页应用 (SPA)**：完全在浏览器中运行、无法安全存储密钥的客户端渲染应用（如 React、Vue、Angular）。
...
在应用详情页，你可以找到集成所需的 [**Client ID**](/integrate-logto/application-data-structure#application-id)、[**Client secret**](/integrate-logto/application-data-structure#application-secret)（仅适用于传统 Web 应用）以及 OIDC 端点。
...
4. 从服务的配置页面复制 **Redirect URI**，并将其添加到 Logto 应用的允许重定向 URI 列表中。
5. 如服务允许，配置 **scopes**。由于 Logto 是 OIDC 提供商，如果需要认证 (Authentication) 用户，请包含 `openid` scope（可获取 ID 令牌和 UserInfo 端点）。如果只需要 OAuth 资源访问，`openid` scope 可选。
...
传统 Web 应用属于**机密客户端**，可以在后端服务器安全存储客户端密钥。完整实现细节见 [授权请求 (Authorization Code Flow)](https://auth.wiki/authorization-code-flow)。
...
单页应用和原生应用属于**公开客户端**，无法安全存储密钥。这类应用必须使用 [PKCE（Proof Key for Code Exchange）](https://auth.wiki/pkce) 进行安全授权。完整实现细节见 [授权请求 (Authorization Code Flow)](https://auth.wiki/authorization-code-flow) 和 [PKCE](https://auth.wiki/pkce)。
...
对于运行在输入受限设备（如智能电视、游戏主机、CLI 工具）上的原生第三方应用，标准的基于重定向的授权请求 (Authorization Code Flow) 可能不可行。在这种情况下，应用可以使用 [OAuth 2.0 设备授权 (Device Authorization Grant)](https://auth.wiki/device-flow)。

使用设备流时，设备会显示一个用户代码和一个验证 URL。用户在另一台设备（手机、笔记本）上访问该 URL，输入代码并完成认证 (Authentication)。设备会轮询 Logto 的令牌端点，直到授权完成。

:::note
在实现设备流之前，请确保在 Logto 控制台为你的第三方应用配置了所需的 [权限](/integrate-logto/third-party-applications/permission-management)。请求未启用 scopes 的第三方应用将被拒绝访问。
:::

完整实现细节见 [设备流快速上手](/quick-starts/device-flow)。
...
只有当用户点击“授权”按钮后，这些请求的权限才会被授予第三方应用。
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: '个性化用户授权页面外观，使其与你的品牌形象保持一致，提供一致的用户体验。',
      customProps: {
