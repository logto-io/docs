...
在 Logto 的登录体验中使用应用程序，可以让用户从一个位置轻松访问和管理他们已授权的应用程序，并享受一致且安全的认证 (Authentication) 流程。这有助于简化用户体验，并确保只有被授权的个人才能访问敏感信息或代表组织执行操作。

应用程序还用于 Logto 的审计日志中，用于追踪用户活动并识别潜在的安全威胁或漏洞。通过将特定操作与某个应用程序关联，Logto 能够提供有关数据如何被访问和使用的详细洞察，帮助组织更好地管理安全和合规性需求。
...
- **单页应用 (Single page app)**：运行在 Web 浏览器中的应用，通过从服务器获取新数据来更新页面，而无需加载整个新页面。例如，React DOM 应用、Vue 应用。
...
:::note
需要注意的是，_应用程序名称_ 应该谨慎选择，因为它对所有有权访问管理控制台的用户可见。它应准确反映应用程序的用途和功能，同时易于理解和识别。
:::
...
允许的 URI 列表用于验证应用程序在认证 (Authentication) 过程中向 Logto 发送的授权 (Authorization) 请求中包含的重定向 URI。如果授权 (Authorization) 请求中指定的重定向 URI 与应用程序设置中的某个允许 URI 匹配，用户将在认证 (Authentication) 成功后被重定向到该 URI。如果重定向 URI 不在允许列表中，用户将不会被重定向，认证 (Authentication) 过程将失败。
...
_登出后重定向 URI_ 是为应用程序预先配置的一组有效 URI，用于在用户从 Logto 登出后重定向用户。

允许的 _登出后重定向 URI_ 用于 OIDC 中的 RP 发起（Relying Party Initiated）注销规范。该规范为应用程序发起用户注销请求提供了标准化方法，包括在用户登出后将其重定向到预先配置的端点。

当用户从 Logto 登出时，他们的会话被终止，并被重定向到应用程序设置中指定的某个允许 URI。这确保用户仅被引导到授权且有效的端点，防止因重定向到未知或未验证端点而导致的未授权访问和安全风险。
...
_CORS（跨域资源共享）允许来源_ 是允许应用程序向 Logto 服务发起请求的来源列表。不在允许列表中的来源将无法向 Logto 服务发起请求。

CORS 允许来源列表用于限制来自未授权域名对 Logto 服务的访问，并有助于防止跨站请求伪造（CSRF）攻击。通过在 Logto 中为应用程序指定允许的来源，服务可以确保只有被授权的域名能够向服务发起请求。

:::note
允许来源列表应包含应用程序实际部署的来源。这确保来自应用程序的请求被允许，而来自未授权来源的请求会被阻止。
:::
...
_授权端点_ 是 OIDC 术语，是用于启动用户认证 (Authentication) 流程的必需端点。当用户尝试访问已在 Logto 平台注册的受保护资源或应用程序时，他们将被重定向到 _授权端点_ 以认证 (Authentication) 身份并获取访问所请求资源的授权 (Authorization)。
...
:::note
对于公共客户端，启用此功能后，每当客户端使用刷新令牌 (Refresh token) 换取新的访问令牌 (Access token) 时，都会颁发新的刷新令牌 (Refresh token)。
...
刷新令牌 (Refresh token) 可用于请求新访问令牌 (Access token) 的有效期，超过该时间后将失效。每次令牌请求会将刷新令牌 (Refresh token) 的 TTL 延长到该值。
...
当刷新令牌 (Refresh token) 在授权 (Authorization) 请求中**未包含** `offline_access` 权限 (Scope) 时，将绑定到用户会话。会话的固定 TTL 为 **14 天**。会话过期后，刷新令牌 (Refresh token) 无论自身 TTL 设置如何都将失效。

如需刷新令牌 (Refresh token) TTL 设置完全生效，请确保在授权 (Authorization) 请求中包含 `offline_access` 权限 (Scope)。
:::
