...
Logto 默认使用端口 `3001` 作为核心服务端口，端口 `3002` 用于交互式管理控制台。
...
:::caution
不要在生产环境中使用我们的 docker compose 命令！由于我们目前在 `docker-compose.yml` 中将内置的 Postgres 数据库与 Logto 应用打包在一起，
每次重新执行命令都会创建一个新的数据库实例，之前持久化的数据将会丢失。
:::
...
DATABASE_STATEMENT_TIMEOUT: DISABLE_TIMEOUT # (v1.36.0+) 为 PgBouncer/RDS Proxy 禁用 statement_timeout
```

使用上述必需的环境变量运行容器（下面省略了可选项；如有需要请自行添加）：
...
- 在 `DB_URL` 中使用 `host.docker.internal` 或 `172.17.0.1` 可指向主机 IP。
- 如果你使用 PgBouncer 或 RDS Proxy 并看到 `unsupported startup parameter: statement_timeout`，请设置 `DATABASE_STATEMENT_TIMEOUT=DISABLE_TIMEOUT`（v1.36.0+）。
...
我们建议为 Logto 使用一个全新且空的数据库，虽然这不是硬性要求。
...
### 使用备用下载链接 \{#using-an-alternative-url-for-downloading}

</summary>

如果你想为 Logto zip 文件指定下载链接，可以使用 `--download-url` 选项。例如：
...
这些可靠的托管服务商为 Logto 提供了一键安装模板。通过便捷的部署服务，你可以在几秒钟内使用 Logto 搭建并启动你的 CIAM 系统。
  ...
  items={[
    {
      ...
      description: '高性价比的云服务，提供免费开发租户，轻松集成认证 (Authentication)。',
      customProps: {
        icon: <LogtoIcon />,
      },
    },
    {
      ...
      description: '可自托管的 Heroku/Netlify 替代方案，便于应用和数据库管理。',
      customProps: {
        icon: <CoolifyIcon />,
      },
    },
    {
      ...
      description: '轻量级工具，可在自有基础设施上部署应用。',
      customProps: {
        icon: <DokployIcon />,
      },
    },
    {
      ...
      description: '现代化控制面板，基于 Docker 管理云服务器。',
      customProps: {
        icon: <EasypanelIcon />,
      },
    },
...
当你成功在服务器上部署 Logto 后，在欢迎页面点击“创建账户”。请注意，Logto 开源版仅允许在首次启动时创建一个账户，不支持多账户。账户创建过程仅支持用户名和密码组合。
...
<Url href="https://blog.logto.io/local-https">本地 HTTPS 开发指南</Url>

<Url href="https://github.com/logto-io/logto/blob/master/AWESOME.md">Logto 优秀社区资源</Url>
