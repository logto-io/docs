...
要启动 Logto 的在线 GitPod 工作区，<a href="https://gitpod.io/#https://github.com/logto-io/demo" target="_blank" rel="noopener">点击这里</a>。稍等片刻，你会看到类似这样的消息：
...
我们建议使用一个新的空数据库专用于 Logto，虽然这不是硬性要求。
...
一旦你完成初始化过程并启动 Logto，你会看到类似这样的消息：
...
Logto 使用环境变量进行配置，并支持 `.env` 文件。查看 [Configuration](/concepts/core-service/configuration) 以获取详细用法和完整变量列表。

</details>

_查看 [Core service](/concepts/core-service) 如果你想要更高级的控制或以编程方式访问 Logto。_

## 创建账户

一旦你成功在服务器上托管了 Logto，点击欢迎页面上的“创建账户”。请记住，Logto 的开源版本仅允许在初次启动时创建一个账户，并且不支持多个账户。账户创建过程仅限于用户名和密码组合。
