...
要在线启动 Logto 的 GitPod 工作区，<a href="https://gitpod.io/#https://github.com/logto-io/demo" target="_blank" rel="noopener">点击这里</a>。稍等片刻，你会看到如下信息：
...
要继续你的 Logto 之旅，请按下 Ctrl（或 Cmd）并点击以 `https://3002-...` 开头的链接。祝你玩得开心！
...
每次你重新执行命令时都会创建一个新的数据库实例，之前持久化的数据将会丢失。
:::
...
### 使用备用下载地址 \{#using-an-alternative-url-for-downloading}

</summary>

如果你想为 Logto zip 文件指定下载地址，请使用 `--download-url` 选项。例如：
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: '一个现代化的控制面板，用于通过 Docker 管理云服务器。',
      customProps: {
        icon: <EasypanelIcon />,
      },
    },
    {
      ...
      description: '全托管 DevOps 平台，助你部署代码和开源软件。',
      customProps: {
        icon: <ElestioIcon />,
      },
    },
