...
在上一篇文章中，我们介绍了如何快速开始使用 Logto 的基础知识。本文将深入探讨，将重点放在生产环境中部署 Logto 的最佳实践和详细配置步骤。

## 环境变量

我们在演示中使用了一组生成的环境变量预设（`docker-compose.yml`），你应该用自己的变量替换它们，并在多个 Logto 实例之间保持一致性。

你可以直接设置环境变量，或者将 `.env` 文件放在 Logto 项目的根目录中。如果你正在使用 Docker 进行测试，请查看镜像在 `/etc/logto` 中生成的 `.env`。
...
你可以使用 Node.js 直接提供 HTTPS 服务，或者在 Node.js 前面设置一个 HTTPS 代理 / 平衡器。有关详细信息，请参阅 [启用 HTTPS](/concepts/core-service/configuration/#enabling-https)。

### 反向代理

如果你想在服务器上使用反向代理，例如 Nginx 或 Apache，你需要在代理传递设置中分别映射 3001 和 3002 端口。假设你正在使用 Nginx，Logto 认证端点运行在端口 3001，Logto 管理控制台运行在 3002，请在 nginx.conf 中放置以下配置：
...
对于生产环境，你可以使用 Docker 将 Logto 容器化。你可以在项目的根目录中找到 Dockerfile。如果你想运行多个 Logto 实例，例如，在 Kubernetes 集群中部署 Logto，需要采取一些额外的步骤。
...
这是一个 Kubernetes 的最小示例 `deployment`：
...
在这个示例中，我们创建了一个空目录作为卷并将其挂载到容器中。然后我们在初始化容器中运行 `npm run cli connector add -- --official` 来下载连接器。最后，每个容器将共享同一个连接器文件夹，其中已经包含我们的官方连接器。
...
对于生产环境，你可以将 "empty dir" 卷替换为持久卷，并以自己的方式完成 "init" 工作，你知道自己在做什么！

### 数据库变更

与连接器类似，数据库变更需要在单个实例中运行。你可以使用一个作业来运行变更脚本。

当变更在非交互模式下运行时，`CI=true` 环境变量是必要的。
