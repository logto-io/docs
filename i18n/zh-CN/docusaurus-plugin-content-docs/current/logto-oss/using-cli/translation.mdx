...
:::note
从 v1.20.0 开始，translate 命令已从 `@logto/cli` 中移除，并作为独立的 `@logto/translate` 包在日志 (logto) monorepo 中维护。请参考 [translate CLI](/logto-oss/translate-cli/) 获取更多详情。
:::

使用 CLI 管理 i18n 翻译。这些命令适用于想要为 日志 (Logto) 做出贡献的开发者。如果你只是想使用自定义翻译的 日志 (Logto)，请参考 [本指南](/customization/localized-languages/)。
