...
| [使用组织级 RBAC（组织模板）保护你的组织资源。](/authorization/organization-template/protect-organization-resources) | 将每个组织视为资源，使用组织级 RBAC（组织模板）在 **多租户** 架构中进行访问控制。 | Logto 授予包含 `组织权限 (Organization permission)` 的 `组织令牌 (Organization token)` 以保护组织级资源。 |
