...
- **你的应用服务于不同的部门或团队**：在不同的组需要对资源的不同访问级别的场景中，它是有用的。
...
角色 (Roles) 是可以分配给用户的权限 (Permissions) 的一种分组。它们还提供了一种聚合为不同 API 定义的权限的方法，使得添加、删除或调整权限比单独分配给用户更高效。
...
可以将 `order_admin` 和 `product_admin` 都分配给一个用户，那么他们将拥有你刚定义的所有六个权限。
