...
除了将 API 作为资源的 [保护你的 API](/authorization/api-resources/protect-your-api) 之外，组织也可以作为资源，并以相同的方式保护你的组织资源。在本文中，我们将重点介绍如何以类似的方式保护你的组织资源。
...
sequenceDiagram
    ...
    Note right of Client: 资源为 "urn:logto:scope:organizations"<br/>并请求的权限包括<br/>"urn:logto:scope:organizations"
...
## 步骤 1：从 OIDC 流程中获取组织 ID

Logto 扩展了标准的 [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) 协议，允许你的应用从用户那里获取组织信息。有两种方法可以做到这一点：

- 如果你使用支持组织的 Logto SDK，可以将 `urn:logto:scope:organizations` 权限添加到配置对象的 `scopes` 参数中。通常，SDK 会为此权限提供一个枚举，例如在 [Logto JS SDKs](https://github.com/logto-io/js) 中的 `UserScope.Organizations`。
...
如果你还没有设置组织角色，请参考[本节](/authorization/organization-template/configure-organization-template/#create-organization-role)。

要获取当前用户的所有组织角色：

- 如果你使用支持组织的 Logto SDK，可以将 `urn:logto:scope:organization_roles` 权限添加到配置对象的 `scopes` 参数中。通常，SDK 会为此权限提供一个枚举，例如在 [Logto JS SDKs](https://github.com/logto-io/js) 中的 `UserScope.OrganizationRoles`。
...
数组中的每个字符串格式为 `organization_id:role_id`，例如 `org_123:admin` 表示用户在 ID 为 `org_123` 的组织中拥有 `admin` 角色。

`organization_roles` 字段（声明）也会包含在 [UserInfo 端点](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo) 的响应中。

## 步骤 2：获取组织令牌

要在组织的上下文中执行操作，用户需要为该组织授予一个访问令牌（组织令牌）。组织令牌是一个 JWT 令牌，包含组织 ID 和用户在组织中的权限（权限）。
...
- 对于其他情况，你需要将 `offline_access` 和 `urn:logto:scope:organizations` 权限添加到 `scope` 参数中，并将 `urn:logto:resource:organizations` 资源添加到 SDK 配置（或认证请求）的 `resource` 参数中。
  - 注意：`offline_access` 是获取可以用于获取组织令牌的 `refresh_token` 所必需的。
...
const App = () => {
  ...
  return <button onClick={getToken}>获取组织令牌</button>;
};
```
...
- 对于 `org_1`，用户拥有 `admin` 角色，因此组织令牌应具有所有可用权限（权限）。
- 对于 `org_2`，用户拥有 `member` 角色，因此组织令牌应具有 `read:logs` 和 `read:users` 权限（权限）。

由于我们在认证流程中仅请求了 `read:logs` 和 `write:logs` 权限，因此组织令牌已相应地“降级”，导致请求的权限与可用权限的交集。

### 为机器对机器应用获取组织令牌

与为用户获取组织令牌类似，你也可以为机器对机器应用获取组织令牌。唯一的区别是你需要使用 `client_credentials` 授权类型而不是 `refresh_token` 授权类型。
...
- 与 API 资源的访问令牌不同，如果用户不是该组织的成员，则无法获取组织令牌。
