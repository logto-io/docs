...
sequenceDiagram
    ...
    Note over Client,API: 1. 认证请求
    Client->>Logto: GET /oidc/auth
    Note right of Client: 带有 resource=https://resource-server.com/api<br/>和可选的 scope=read write
    Logto-->>Client: 返回 authorization_code

    Note over Client,API: 2. 令牌请求
    Client->>Logto: POST /oidc/token
    Note right of Client: 带有 authorization_code<br/>resource=https://resource-server.com/api<br/>和可选的 scope=read write
...
在[认证请求](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest)中提供资源指示参数列表。它将指示用户可能请求的所有受保护资源。
...
当资源参数与上面授予的 `authorization_code` 一起出现在[令牌请求](https://openid.net/specs/openid-connect-core-1_0.html#TokenRequest)中时，它将指定请求的访问令牌的目标 API 资源受众。
...
Logto 将授予一个受限于此请求资源的受众的加密访问令牌。令牌携带所有你需要的数据来表示请求的授权状态。例如，请求用户的身份和角色，令牌的受众和过期时间。
...
Logto 为每个授权的 API 请求颁发一个标准的 [JWT](https://datatracker.ietf.org/doc/html/rfc7519) 格式的授权令牌。令牌被加密并签名为 [JWS](https://datatracker.ietf.org/doc/html/rfc7515) 令牌。
...
- JWS Signature：使用 [JWK](https://datatracker.ietf.org/doc/html/rfc7517) 签名的签名
...
对于开发，为了直观地检查 JWT 令牌，你可以访问 [jwt.io](https://jwt.io/) 来解码和检查你收到的令牌。请小心或永远不要使用来自生产环境的令牌。由于这是第三方提供的公共在线服务，你的令牌可能会被暴露。
