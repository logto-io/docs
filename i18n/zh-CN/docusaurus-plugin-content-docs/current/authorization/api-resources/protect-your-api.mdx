...
## 对 API 资源的请求 \{#request-to-the-api-resource}
...
Logto 为每个授权的 API 请求发布一个标准的 [JWT](https://datatracker.ietf.org/doc/html/rfc7519) 格式的授权令牌。令牌被加密并签名为 [JWS](https://datatracker.ietf.org/doc/html/rfc7515) 令牌。
...
- JWS Payload：包含令牌的所有声明
- JWS Signature：使用 [JWK](https://datatracker.ietf.org/doc/html/rfc7517) 签名的签名

Logto 发布的 JWS Payload 的标准模式：（声明可能会有所不同，基于你的自定义 OIDC 配置）
...
在开发过程中，为了直观地检查 JWT 令牌，你可以访问 [JWT 解码工具](https://www.jstoolset.com/jwt) 来解码和检查你收到的令牌。请小心或永远不要使用来自生产环境的令牌。由于这是第三方提供的公共在线服务，你的令牌可能会被暴露。
