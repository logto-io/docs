...
一个已授权的请求应该包含一个 `Authorization` 头，其内容为 `Bearer <access_token>`。从请求头中提取认证令牌：
...
为了演示，我们使用 Flask 应用和 [jose](https://github.com/mpdavis/python-jose) 包来创建 require_auth 装饰器，以验证令牌的签名、过期状态和所需的声明 (Claims)。

### 安装 `python-jose` 作为你的依赖

选择你在 Logto 中使用的加密技术。（默认是 `ecdsa`）
...
如果你使用 [基于角色的访问控制](/authorization/role-based-access-control/protect-api-resources-with-rbac)，还需要进行权限 (Scope) 验证。
