...
API 资源，也称为[资源指示器](https://www.rfc-editor.org/rfc/rfc8707.html)，表示要请求的目标服务或资源，通常是表示资源身份的 URI 格式变量。这是 Logto 授权 (Authorization) 系统中的一个关键概念，因为它定义了授权 (Authorization) 策略旨在保护的端点。
...
在实践中，客户端可能知道要与之交互的应用程序或资源的基本 URI。使用它作为资源参数的值是合适的。
...
| 当指定了默认 API 时，可以在授权请求中省略资源参数。随后的令牌交换将默认使用该 API 作为受众，从而发放 JWT。 | false                                                                                   |

:::note

应限制使用过期的 `访问令牌` 的请求。这是为了保护你的 API 不被滥用的令牌攻击。设置的值越大，发放的 `访问令牌` 存活时间越长，你的 API 就越容易受到攻击。
...
在 Logto 的实现中，每个用户定义的权限 (Scope) 必须与一个 API 资源相关联。否则，它将被视为 OpenID Connect（或 OAuth）权限。通常，这不会影响你的授权 (Authorization) 过程。然而，当与不支持 [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html) 的第三方应用集成时（例如 ChatGPT 插件），可能会带来挑战，因为初始授权请求可能不包含 `resource` 参数。因此，Logto 将始终发放不透明访问令牌。
...
要从 Logto 获取受众限制的 `访问令牌`，首先需要注册你的后端 API 以启用 Logto 的受众限制访问令牌。当收到授权请求时，Logto 将识别注册的 API 资源并相应地授予访问权限。

要注册你的 API，请导航到 <CloudLink to="/api-resources">控制台 > API 资源</CloudLink> 你将看到一个内置资源，其 API 标识符显示为 `https://[your-tenant-id].logto.app/api`。此资源涵盖了 Logto 的所有 Management API，并确保它们仅对 Logto 的授权用户可访问。

点击 **创建 API 资源** 按钮，并按照交互式表单创建你自己的 API 资源：

- 一个易于阅读的 **API 名称**，可以更好地帮助你识别此实体。
- 一个 URI 格式的唯一 **API 标识符**。它表示 API 资源的身份。
