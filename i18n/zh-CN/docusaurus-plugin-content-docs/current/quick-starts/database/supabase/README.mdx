...
sidebar_custom_props:
  description: 将 Logto 集成到 Supabase 中。
  logoFilename: 'supabase.svg'
---

# 为你的 Supabase 应用添加认证

## Supabase 基础 \{#supabase-basics}
...
由于 Supabase 支持 JWT，当我们的应用与 Supabase 交互时，我们可以使用 Supabase 提供的 JWT 密钥生成一个包含用户数据的 JWT。然后，我们将此 JWT 作为认证头在请求中使用。在接收到请求后，Supabase 会自动验证 JWT 的有效性，并允许在后续过程中访问其中包含的数据。

首先，我们可以从 Supabase 仪表板的“项目设置”中获取 Supabase 提供的 JWT 密钥：

![Supabase API settings page](./supabase-api-settings.webp)

然后，当我们使用 Supabase SDK 向 Supabase 发出请求时，我们利用这个密钥生成我们的 JWT，并将其附加为请求的认证头。（请注意，此过程发生在你的应用的后端服务中，JWT 密钥不应暴露给第三方）。
...
/*
 * 注意：
 * 你可以在找到 JWT 密钥的同一位置找到 SUPABASE_URL 和 SUPABASE_ANON_KEY。
 */
...
接下来，导航到 Supabase 仪表板中的 SQL 编辑器，并创建一个函数来检索请求中携带的 userId：
...
通过这个函数，Supabase 可以确定当前访问数据库的用户。
...
通过利用这些策略，可以在 Supabase 中实现数据访问控制。
...
### Logto 的基本集成过程 \{#basic-integration-process-with-logto}

如前所述，由于 Supabase 使用 RLS 进行访问控制，与 Logto（或任何其他认证服务）的集成关键在于获取授权用户的用户 ID 并将其发送到 Supabase。整个过程在下图中说明：
...
sequenceDiagram
	...
	Logto-->>U: `访问令牌`
	U->>B: 使用 `访问令牌` 请求数据
	B->>B: 从 `访问令牌` 中检索 `用户 ID`
	B->>B: 通过 `用户 ID` 和 `Supabase JWT 密钥` 生成 jwt
	B->>Supabase: 使用 jwt 请求数据
	Supabase-->>B: 返回数据
	B-->>U: 返回数据
```

接下来，我们将根据此流程图解释如何将 Logto 集成到 Supabase 中。

## Logto 集成 \{#logto-integration}

Logto 提供了针对各种框架和编程语言的集成指南。

通常，使用这些框架和语言构建的应用分为原生应用、SPA（单页应用）、传统 Web 应用和 M2M（机器对机器）应用等类别。你可以访问 [Logto 快速入门](/quick-starts) 页面，根据你使用的技术栈将 Logto 集成到你的应用中。之后，根据你的应用类型，按照以下说明将 Logto 集成到你的项目中。

### 原生应用或 SPA \{#native-app-or-spa}

原生应用和 SPA 都在你的设备上运行，登录后获得的凭证（访问令牌）存储在你的设备上。

因此，当将你的应用与 Supabase 集成时，你需要通过后端服务与 Supabase 交互，因为你不能在每个用户的设备上暴露敏感信息（如 Supabase JWT 密钥）。

假设你使用 React 和 Express 构建你的 SPA。你已经按照 [Logto React SDK 指南](/quick-starts/react) 成功将 Logto 集成到你的应用中（你可以参考我们的 [react 示例](https://github.com/logto-io/js/tree/master/packages/react-sample) 中的代码）。此外，你已经根据 [保护你的 API 在 Node (Express)](/authorization/api-resources/node-express) 文档在后端服务器中添加了 Logto 访问令牌验证。

接下来，你将使用从 Logto 获得的访问令牌向你的后端服务器请求用户数据：
...
在你的后端服务器中，你已经使用中间件从访问令牌中提取了登录用户的 ID：
...
在此代码中，Supabase 将根据之前设置的策略仅返回属于当前用户的帖子数据。

### 传统 Web 应用 \{#traditional-web-app}

传统 Web 应用与原生应用或 SPA 的主要区别在于，传统 Web 应用仅在 Web 服务器上渲染和更新页面。因此，用户凭证由 Web 服务器直接管理，而在原生应用和 SPA 中，它们位于用户设备上。

在 Supabase 中将 Logto 与传统 Web 应用集成时，你可以直接从后端检索登录用户的 ID。

以 Next.js 项目为例，在你按照 [Next.js SDK 指南](/quick-starts/next-app-router) 将 Logto 集成到你的项目后，你可以使用 Logto SDK 检索用户信息并构建与 Supabase 交互的相应 JWT。
...
机器对机器 (M2M) 通常用于你的应用需要直接与资源服务器通信的情况，例如拉取每日帖子的静态服务等。

你可以使用 [机器对机器：使用 Logto 认证](/quick-starts/m2m) 指南进行机器对机器应用认证。Supabase 与机器对机器应用的集成类似于原生应用和 SPA（如 "[原生应用或 SPA](#native-app-or-spa)" 部分所述）。它涉及从 Logto 获取访问令牌，然后通过受保护的后端 API 验证它。
...
此外，如果你希望特定的 M2M 应用代表整个服务访问 Supabase 以绕过 RLS 限制，你可以使用 Supabase 的 `service_role` 密钥创建一个 Supabase 客户端。当你希望执行一些需要访问所有数据的管理或自动化任务而不受为单个用户设置的行级安全策略限制时，这很有用。

`service_role` 密钥可以在与 JWT 密钥相同的页面上找到：

![Service role secret](./service-role-secret.webp)

创建 Supabase 客户端时，使用 `service_role` 密钥，这样该客户端可以访问数据库中的所有数据：
