...
[Hasura](https://hasura.io/) 是一个可以快速为你的数据提供相应 GraphQL 和 REST API 的工具。考虑到数据安全，Hasura 还提供了为每个不同 API 进行细粒度访问控制的能力。
...
Hasura 可以通过 Webhook 或 JWT 支持访问控制。我们之前的 [博客文章](https://blog.logto.io/logto-x-hasura) 介绍了如何使用 Webhook，接下来的部分中，我们将解释如何利用 Hasura 的 JWT 模式进行访问控制。

## 开始 \{#get-started}

让我们从一个简单的例子开始。假设用户已经在 Hasura 中有两个 API，`GET /user` 和 `PATCH /user`，分别对应两个角色：`user:reader` 和 `user:maintainer`。
...
我们需要为步骤 1 中提到的 Hasura API 资源创建两个 `权限 (scope)`，即 `read:user` 和 `maintain:user`，然后创建两个角色：`user:reader`（包含 `read:user` 权限）和 `user:maintainer`（包含 `maintain:user` 权限），以一对一对应 Hasura 的角色。并根据需要将这些角色分配给 Logto 用户。
...
### 4. 自定义用户访问令牌额外声明 \{#4-customize-user-access-token-extra-claims}
...
通过比较 Hasura 的 [Webhook](https://hasura.io/docs/latest/auth/authentication/webhook/) 和 [JWT](https://hasura.io/docs/latest/auth/authentication/jwt/) 访问控制的过程，我们可以看到 Webhook 方法在每个 Hasura 请求时都会发送一个 Webhook 到 Logto 并等待响应；而基于 JWT 的方法可以持续使用，直到 JWT 过期。
