...
<p>
  Thus, in the above case, scopes can be simplified from the definition in Logto, both of the API
  resources can have <code>read</code> and <code>write</code> scopes without the prefix. Then, in
  the Logto config:
</p>
...
<p>
  For every API resource, it will request for both <code>read</code> and <code>write</code> scopes.
</p>
