...
对于 [gradle](https://spring.io/guides/gs/gradle) 用户，请在你的 `build.gradle` 文件中添加以下依赖项：
...
对于 [maven](https://spring.io/guides/gs/maven) 用户，请在你的 `pom.xml` 文件中添加以下依赖项：
...
在 Logto 控制台中注册一个新的 `Java Spring Boot` 应用，并获取你的 Web 应用的客户端凭证和 IdP 配置。
