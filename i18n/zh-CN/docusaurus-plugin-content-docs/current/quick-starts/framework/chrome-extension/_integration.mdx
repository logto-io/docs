...
```mermaid
sequenceDiagram
  participant A as 扩展程序弹出窗口
  participant B as 扩展程序服务工作者
  participant C as Logto 登录体验 (Experience)
...
你可能注意到上面的代码中使用了两个重定向 URI。它们都是由 `chrome.identity.getRedirectURL` 创建的，这是一个 [内置 Chrome API](https://developer.chrome.com/docs/extensions/reference/api/identity#method-getRedirectURL)，用于为认证 (Authentication) 流程生成重定向 URL。这两个 URI 将是：
...
查看我们的 [示例项目](https://github.com/logto-io/js/tree/HEAD/packages/chrome-extension-sample)，了解使用 TypeScript、Rollup 和其他配置的完整示例。
