...
1. 将 `wellKnown` URL 替换为你的 Logto 应用的“OpenID 提供商配置端点”。
2. 将 `clientId` 和 `clientSecret` 替换为你的 Logto 应用的 ID 和密钥。
3. 自定义 `profile` 函数以将用户资料映射到 Next Auth 用户对象，示例中显示了默认映射。
...
现在，你可以测试你的应用程序以查看认证 (Authentication) 是否按预期工作。
