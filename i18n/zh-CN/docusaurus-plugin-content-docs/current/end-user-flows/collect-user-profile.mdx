...
- **新用户注册时**：使用 [收集用户资料](#quick-start) 功能，在注册流程中添加一个开箱即用的“告诉我们你的信息”步骤，收集额外的用户资料信息。新用户必须填写所有必填项后，注册才算完成。本篇文档重点介绍此方式。

- **用户注册后**：通过 [Account API](/end-user-flows/account-settings/by-account-api) 在引导流程、账户中心或产品使用过程中实现自助体验，更新用户资料信息。

## 主要优势 \{#key-benefits}

**收集用户资料**功能让你能够在终端用户注册体验中收集额外的用户信息。我们建议只收集产品所需的关键信息，避免冗长的注册流程影响用户转化率。

此功能可以帮助你：

- **获取全面的用户数据**：可收集任何 [用户数据](/user-management/user-data) 以满足业务或合规需求，包括 [OIDC 标准用户属性](#basic-user-data-fields)和[自定义数据](#custom-user-data-fields)。

- **灵活的字段自定义**：可选择多种[字段类型](#field-types)，如文本、数字、日期、复选框、下拉框（选择）、URL 和正则校验，以满足你的具体数据需求。

- **优化用户体验**：可自定义显示标签、描述、占位符和校验规则。根据业务需求将字段设置为必填或选填。

- **内置字段配置**：为常见用户属性提供预配置的基础数据字段，开箱即用。利用复合字段（地址、全名）可高效地在单步中收集结构化数据。
...
用户数据可在每个[用户资料](/user-management/manage-users)中查看。关于所有用户数据类别的完整概览，请参阅 [用户数据结构](/user-management/user-data) 文档。收集用户资料功能允许你在注册时收集基础用户数据和自定义用户数据。
...
| 描述       | 全部（复选框除外） | 字段下方的辅助说明，用于更长的指引或补充说明。                   |
| 必填       | 全部（复选框除外） | 启用后，字段为空时禁止提交。禁用时，标签后会追加“（可选）”标记。 |
...
| 日期           | 日期格式（预设 / 自定义） | 可选择预设格式（MM/dd/yyyy、dd/MM/yyyy、yyyy-MM-dd）。更改占位符会覆盖默认交互。也可使用完全自定义的 <a href="https://date-fns.org/v2.30.0/docs/format" target="_blank" rel="noopener noreferrer">date-fns</a> 格式。 |
| 复选框         | 默认值                    | 选中（True）/ 未选中（False）。                                                                                                                                                                                       |
| 下拉框（选择） | 选项列表（≥1）            | 每行使用 <code>value:label</code>。若省略 label，则显示 value。                                                                                                                                                       |
| URL            | N/A                       | N/A                                                                                                                                                                                                                   |
| 正则表达式     | 正则模式                  | 校验模式（无需首尾斜杠）。如 <code>^\d{3}-\d{2}-\d{4}$</code> 用于社保号。                                                                                                                                            |
...
你可以在 [Logto > 登录体验 > 内容](https://cloud.logto.io/to/sign-in-experience/content) 中通过自定义翻译覆盖这些标签。
