...
4. 使用专用用途类型设置 MFA 的短信模板
   ...
   - `BindMFA` 用于绑定 MFA
5. 测试连接器功能，确保消息能正常送达
6. 参考 [短信连接器](/connectors/sms-connectors) 获取特定服务商的设置说明
...
1. **登录方式限制**：短信验证码不能同时作为 [登录方式 (1FA)](/end-user-flows/sign-up-and-sign-in/sign-in) 和 MFA 因子 (2FA) 使用。每个短信实现请选择一种认证 (Authentication) 流程。

2. **注册方式兼容性**：短信验证码可以同时用于注册方式和 MFA。Logto 会根据你选择的 MFA 策略优化终端用户注册流程，避免对同一手机号重复要求短信验证。

3. **找回密码兼容性**：虽然短信验证码可以同时用于 [忘记密码](/end-user-flows/sign-up-and-sign-in/reset-password) 和 MFA，但**不推荐**这种组合。该配置会降低 MFA 的安全性，因为用户可能通过忘记密码短信验证重置密码，然后用新密码进行主认证 (1FA)，再用相同的短信方式进行 MFA 验证，从而绕过 MFA。

:::

## 短信 MFA 绑定流程 \{#sms-mfa-setup-flows}

MFA 设置提示可能出现在用户注册时或登录后，具体取决于你配置的 [MFA 策略](/end-user-flows/mfa/configure-mfa#global-mfa-configuration)。

短信 MFA 设置流程受以下因素影响：

- **主 MFA 因子数量**：如有多个主因子，用户需选择一个进行配置。主因子指除备用码外的 MFA 方法。
- **启用备用码**：启用后，主 MFA 因子配置完成后会自动生成备用码，并提示用户保存。
- **注册标识符配置**：如果手机号作为 [注册标识符](/end-user-flows/sign-up-and-sign-in/sign-up#set-up-the-sign-up-identifier) 并在注册时已通过短信验证码验证，系统会自动将该号码绑定为 MFA 因子，无需再次验证。如果存在其他主因子，界面会显示“添加另一个两步验证”选项（用户可跳过），同时明确提示已启用 MFA。
...
### 场景 3：手机号已验证但有多个主因子 \{#scenario-3-phone-verified-but-multiple-primary-factors-available}
...
如有多个 MFA 因子，用户可从已配置的因子中选择。系统会根据 [配置 MFA](/end-user-flows/mfa/configure-mfa#mfa-verification-flow) 中指定的优先级顺序决定优先提示哪个 MFA 因子。
