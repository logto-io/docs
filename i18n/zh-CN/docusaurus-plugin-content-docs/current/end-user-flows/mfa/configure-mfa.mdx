...
2. 为你的用户启用支持的验证因子。
   1. 主验证因子：
      ...
      - [邮件验证](/end-user-flows/mfa/email-mfa)：一种广泛可用的方法，将一次性验证码发送到用户注册的邮箱，适用于所有平台和设备的用户。
   2. 备份验证因子：
      - [备份码](/end-user-flows/mfa/backup-codes)：当用户无法验证上述主验证因子时，作为备选方案。启用此选项可降低用户访问的阻力。
3. 选择是否启用 **强制 MFA**：
   - **启用**：用户在登录过程中会被要求设置 MFA，且无法跳过。如果用户未能设置 MFA 或删除了 MFA 设置，他们将无法访问账户，直到重新设置 MFA。
   - **禁用**：用户可以在注册流程中跳过 MFA 设置。之后可通过自助账户设置页面设置 MFA。[了解更多](/end-user-flows/account-settings/) 关于实现用户账户设置页面。并继续选择 MFA 设置提示的策略：
     ...
     - **在注册后首次登录时提示用户设置 MFA**：新用户在注册后第二次登录时会被提示设置 MFA，现有用户将在下次登录时看到提示。用户可以跳过此步骤，且不会再次出现。

:::tip
如果你需要在用户跳过 MFA 注册后再次提示，请重置他们的跳过状态，这样下次登录时会显示设置界面。管理员可以使用 Management API（`PATCH /api/users/{userId}/logto-configs`），开发者在自助流程中可调用 Account API（`PATCH /api/my-account/logto-configs`）。[Management API 参考](https://openapi.logto.io/operation/operation-updateuserlogtoconfig) · [Account API 参考](https://openapi.logto.io/operation/operation-updatelogtoconfig)
:::
...
3. **设置 MFA 主验证因子**：用户被提示设置主 MFA 验证因子（如 passkey、身份验证器应用 OTP、短信验证码或邮件验证码）。
   - 如果启用了多个主验证因子，用户可选择自己喜欢的方式。
   - 如果主验证因子与注册标识符相同（如短信或邮件），则会自动预验证，用户可跳过验证步骤，直接进入下一步（如“添加另一个两步验证”或“保存你的备份因子”）。
...
- 如果用户为两步验证 (2FA) 设置了多个因子，系统会根据以下优先级规则展示验证选项：
  - **Passkey 优先**：如果用户配置了 passkey，将作为默认验证方式展示。
  - **上次使用优先**：如果没有 passkey，系统会优先展示用户上次成功使用的验证方式。
  - **选择列表**：如果以上优先级均不适用，两步验证页面会展示所有可用的已绑定验证方式供用户选择。
  - 用户可随时点击“尝试其他验证方式”在不同验证选项间切换。
- 如果所有启用的主验证因子用户都无法使用，且已启用备份码，用户可使用一次性备份码验证身份。
...
当管理员移除用户所有主 MFA 因子（passkey、身份验证器应用 OTP、短信或邮件）后，用户下次登录时会出现以下场景：
