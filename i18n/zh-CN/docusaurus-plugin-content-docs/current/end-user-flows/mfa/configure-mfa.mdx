...
2. 为你的用户启用支持的验证因子。
   1. 主验证因子：
      - [通行密钥 (WebAuthn)](/end-user-flows/mfa/webauthn)：适用于支持设备生物识别或安全密钥等的 Web 产品的高安全性选项，确保强大的保护。
...
对于支持 [组织 (Organizations)](/organizations) 的多租户架构产品，大多数情况下你无需对所有用户强制要求 MFA。你可以按组织 (Organization) 启用 MFA，根据每个客户的需求定制要求。开始使用请参考 [为组织成员强制 MFA](/organizations/organization-management#require-mfa-for-organization-members)。
...
4. **设置 MFA 备份因子**：如果启用了**备份码**，用户在成功配置主认证 (Authentication) 因子后会被提示保存备份码。系统会自动生成备份码并展示给用户，用户可下载并安全保存。用户必须手动确认备份码以完成 MFA 设置流程。
...
了解更多关于如何通过 [Account API](/end-user-flows/account-settings/by-account-api) 实现这些模式。
