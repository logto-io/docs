...
![重置密码流程](./assets/reset-password.png)

## 更新已认证用户的密码

对于已认证的用户，他们可以通过访问账户设置页面来更新密码。查看 [账户设置](/end-user-flows/account-settings/) 以了解有关实现用户账户设置页面的更多信息。
...
<details>
  <summary> 如何在成功重置密码后注销用户？</summary>

订阅 `PostResetPassword` [Webhook 事件](/developers/webhooks/webhooks-events#user-interaction-hook-events) 以在用户成功重置密码时接收通知。然后你可以触发 [注销](/end-user-flows/sign-out) 操作以使用户的当前会话失效，并将其重定向到登录页面。

</details>

<details>
  <summary> 如何在我的自定义 UI 上实现密码重置流程？</summary>
...
<details>
  <summary> 如何将密码重置链接发送到用户的电子邮件？</summary>
