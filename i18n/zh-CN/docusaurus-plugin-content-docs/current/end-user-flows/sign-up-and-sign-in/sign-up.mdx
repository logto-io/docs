...
用户注册是用户参与你的应用的第一步。Logto 支持多种注册方式，包括用户名密码、邮箱或手机号验证、[社交注册](/end-user-flows/sign-up-and-sign-in/social-sign-in) 和 [企业单点登录 (SSO)](/end-user-flows/enterprise-sso)。你可以根据应用的需求设置最合适的注册方式。

访问 <CloudLink to="/sign-in-experience/sign-up-and-sign-in">控制台 > 登录与账户 > 注册与登录</CloudLink> 开始配置标识符注册流程。
...
- **用户名**：用户可用于登录应用的唯一 [用户名](/user-management/user-data#username)。可通过 [用户名策略](/end-user-flows/sign-up-and-sign-in/username-policy) 自定义用户名格式。
...
你可以调整注册标识符的顺序，以优先让用户在注册时首先填写你希望的标识符。此顺序会体现在注册流程中，第一个标识符会出现在初始注册界面，其余的在后续步骤收集。

:::tip
如需在注册时屏蔽特定类型的邮箱地址（如一次性邮箱、带加号 (+) 的子地址、特定邮箱或整个域名），请在安全设置中使用 **黑名单** 功能。详见 [黑名单](/security/blocklist)。
:::

:::tip
手机号**国家区号**默认根据用户浏览器的语言环境自动选择。例如，若用户浏览器语言为 `fr`，则国家区号默认法国 (+33)。

你也可以通过 [`ui_locales`](/end-user-flows/authentication-parameters/ui-locales) 认证参数设置登录体验的语言，这也会影响默认国家区号。
:::

## 设置注册验证选项 \{#set-up-the-sign-up-verification-settings}

为确保用户注册及后续登录流程的安全，你还需要为注册过程中收集的标识符配置验证选项。可用设置包括：

- **创建密码：** 要求用户在注册时创建一个符合安全设置中密码策略的密码。该密码与用户标识符一起作为登录应用的凭证。如果你将**用户名**设为注册标识符，则此项会自动启用，因为**用户名**只能与密码配合使用以有效验证用户身份。[密码策略](/security/password-policy) 可根据你的安全需求自定义。
...
## 注册时收集额外用户信息 \{#collect-additional-user-info-on-sign-up}

如需在注册时收集额外的用户资料信息（如姓名、生日、公司名等），你有两种灵活选择：

**方式一：收集用户资料**

将 Logto 预设的“告诉我们你的信息”步骤直接加入注册流程。用户需填写所有必填项后才能完成注册。此方式无需编写代码，开箱即用。

通过 <CloudLink to="/sign-in-experience/collect-user-profile">控制台 > 登录与账户 > 收集用户资料</CloudLink> 配置，选择预设基础数据字段或自定义字段并灵活校验。了解更多：[收集用户资料](/end-user-flows/collect-user-profile)

**方式二：自托管引导流程**

注册成功后将用户重定向到你自定义的引导流程，实现完全自定义的数据收集体验。此方式让你完全掌控用户体验，支持复杂的多步骤引导流程。

可通过 [Account API](/end-user-flows/account-settings/by-account-api) 以编程方式管理用户资料数据。
...
订阅 `User.Created` webhook 事件以向新用户发送欢迎邮件。详见 [webhook 事件](/developers/webhooks/webhooks-events/#data-mutation-hook-events)。
