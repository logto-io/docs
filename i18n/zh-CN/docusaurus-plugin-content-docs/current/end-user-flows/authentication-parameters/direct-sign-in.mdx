...
直接登录是 Logto 特有的[认证 (Authentication) 参数](/end-user-flows/authentication-parameters)，它可以让你直接发起社交登录或企业单点登录 (SSO)，跳过默认的 Logto 通用登录页面。
...
sequenceDiagram
    ...
    logto->>idp: 跳过登录页面，直接发起社交或企业 SSO 登录
    idp->>logto: 发送认证 (Authentication) 令牌或断言
    logto->>app: 将用户带着认证 (Authentication) 数据重定向回客户端应用程序
```
...
2. 点击你想要使用的[社交连接器](/connectors/social-connectors)。
3. 在连接器设置页面顶部找到身份提供商 (IdP) 名称。
...
### 在哪里找到企业 SSO 连接器 ID \{#where-to-find-the-enterprise-sso-connector-id}

1. 前往 <CloudLink to="/enterprise-sso">控制台 > 企业单点登录 (SSO)</CloudLink>
2. 点击你想要使用的[企业连接器](/connectors/enterprise-connectors)。
3. 在连接器设置页面顶部找到连接器 ID。

<img src="/img/assets/direct-sign-in-enterprise-sso.png" alt="企业 SSO 连接器 ID" />
...
### 直接登录和 API 认证 (Authentication) 是一样的吗？\{#is-direct-sign-in-the-same-as-api-authentication}
