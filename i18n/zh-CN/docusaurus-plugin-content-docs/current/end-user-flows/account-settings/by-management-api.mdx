...
1. 用户访问客户端应用。
2. 客户端应用向 Logto 发送认证请求，并将用户重定向到 Logto 登录页面。
3. 用户登录到日志 (Logto)。
...
### 用户数据模式 \{#user-data-schema}

查看 [用户数据和自定义数据](/user-management/user-data/) 部分，了解 Logto 中的用户模式。
...
### 邮箱和电话号码验证 \{#email-and-phone-number-verification}

在 Logto 系统中，邮箱地址和电话号码都可以作为用户标识符，因此它们的验证至关重要。为此，我们提供了一组验证码 API 来帮助验证提供的邮箱或电话号码。
...
12. IdP 将用户信息返回给日志 (Logto)，Logto 将社交身份链接到用户。
