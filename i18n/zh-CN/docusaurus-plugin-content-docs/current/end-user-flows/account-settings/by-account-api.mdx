...
想了解更多可用的 API，请访问 [Logto Account API Reference](https://openapi.logto.io/group/endpoint-my-account) 和 [Logto Verification API Reference](https://openapi.logto.io/group/endpoint-verifications)。

:::note
以下设置的专用 Account API 即将上线：MFA、SSO、自定义数据（用户）、账户删除。在此期间，你可以使用 Logto Management API 实现这些功能。详见 [通过 Management API 进行账户设置](/end-user-flows/account-settings/by-management-api)。
:::
...
更多 API 详情请参见 [Logto Management API Reference](https://openapi.logto.io/group/endpoint-account-center)。
