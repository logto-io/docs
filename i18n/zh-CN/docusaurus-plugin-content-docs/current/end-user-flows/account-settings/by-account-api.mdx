...
:::note
以下设置的专用 Account API 即将上线：MFA、SSO、自定义数据（用户）、账户删除。在此期间，你可以通过 Logto Management API 实现这些功能。详见 [通过 Management API 进行账户设置](/end-user-flows/account-settings/by-management-api)。
:::
...
更多 API 详情请见 [Logto Management API 参考文档](https://openapi.logto.io/group/endpoint-account-center)。
...
想了解更多关于验证的信息，请参考 [通过 Account API 进行安全验证](/end-user-flows/security-verification/by-account-api)。
...
你会在响应中获得 `verificationId`，并在邮箱中收到验证码，使用它来验证邮箱。
...
验证码验证通过后，就可以更新用户邮箱，将 `verificationId` 作为 `newIdentifierVerificationRecordId` 放到请求体中。
...
- `state`：用户授权 (Authorization) 应用后返回的 state，是一个用于防止 CSRF 攻击的随机字符串。

响应中会有一个 `verificationRecordId`，请保存以备后用。
