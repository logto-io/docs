...
- 用户资料和身份管理：用户可以完全管理他们的资料和安全设置，包括更新身份信息如电子邮件、电话和密码，以及管理社交连接。MFA 和 SSO 支持即将推出。
...
- `email`：电子邮件字段。
- `phone`：电话字段。
- `password`：密码字段，获取时，如果用户已设置密码，将返回 `true`，否则返回 `false`。
- `social`：社交连接。

在 [Logto Management API 参考](https://openapi.logto.io/group/endpoint-account-center) 中了解更多关于 API 的详细信息。
...
:::note
要使用此方法，你需要[配置电子邮件连接器](/connectors/email-connectors/)或[短信连接器](/connectors/sms-connectors/)，并确保 `UserPermissionValidation` 模板已配置。
:::
...
在发送请求更新用户标识符时，你需要将验证记录 ID 附加到请求头的 `logto-verification-id` 字段中。

### 更新或链接新电子邮件 \{#update-or-link-new-email}

:::note
要使用此方法，你需要[配置电子邮件连接器](/connectors/email-connectors/)，并确保 `BindNewIdentifier` 模板已配置。
:::

要更新或链接新电子邮件，你应首先证明电子邮件的所有权。
...
:::note
要使用此方法，你需要[配置短信连接器](/connectors/sms-connectors/)，并确保 `BindNewIdentifier` 模板已配置。
:::
