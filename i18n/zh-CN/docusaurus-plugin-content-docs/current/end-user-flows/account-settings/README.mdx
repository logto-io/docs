...
- **终端用户设置**：用户可管理自己的登录标识和凭证、社交账户、多因素认证 (MFA) 方法、个人资料数据和会话。
- **客户端集成**：专为前端安全直接使用而设计。
- **完全自定义**：在利用 Logto 安全 API 的同时，构建你自己的界面。
...
Management API 构成了 Logto 的核心管理接口，仅限管理员或后端服务访问。它们提供最大灵活性和对每个用户账户的完整 CRUD 控制，允许你构建自定义管理工具。如果你需要一个完全自定义的自助门户或非标准的用户管理功能，可以在你自己的“Account API”层后暴露选定的 Management API 端点，并用你的业务认证 (Authentication) 逻辑保护它们。
...
| **权限模型**   | 通过账户中心设置切换哪些字段可用                                                                  | 通过 Management API 设置切换哪些 Account API 被启用                                      | 开发者可完全自定义                                                                        |
| **支持的功能** | 更新：邮箱、手机号、用户名、密码、多因素认证 (TOTP、通行密钥、备份码)、社交账户、会话、已授权应用 | 查看、更新和删除：用户名、邮箱、手机号、密码、社交账户、多因素认证 (MFA)、个人资料、会话 | 所有基础设置 + 删除 / 暂停 / 恢复账户、个人访问令牌、用户模拟、连接 OAuth 应用、会话等    |
| **界面自定义** | 继承登录体验品牌；支持自定义 CSS                                                                  | 完全自定义（自建界面）                                                                   | 完全自定义（自建界面）                                                                    |
| **集成复杂度** | 无（只需链接到预构建页面）                                                                        | 低（结合你的界面使用 API）                                                               | 中到高（需要自定义实现）                                                                  |
| **适用场景**   | 需要最快速添加账户管理且无需自定义页面时                                                          | 需要自定义界面但希望利用 Logto 安全 API 时                                               | 当 Account API 不能满足你的需求时。例如复杂的账户删除逻辑、高风险操作或构建后台管理工具等 |
