...
在这些情况下，应用程序可以向你的邮箱发送一个“魔法链接”。当你点击该链接时，将立即完成认证 (Authentication)。
...
sequenceDiagram
  ...
  U ->> C: （可选）用户向你的应用程序请求魔法链接
  C ->> L: 请求一次性令牌
  L -->> C: 返回一次性令牌
  C -->> U: 生成包含一次性令牌的魔法链接并发送到终端用户邮箱
  U ->> C: 用户点击魔法链接并跳转到你的应用程序的落地页
...
魔法链接至少应包含令牌和用户邮箱作为参数，并跳转到你自己应用程序的落地页。
...
可以，你可以使用魔法链接邀请新用户加入你的应用程序以及组织。
如果你希望邀请新用户加入组织，只需在请求体中指定 `jitOrganizationIds`。

用户成功验证后会自动加入组织，并分配默认组织角色。
...
魔法链接认证 (Authentication) 流程不支持为用户分配角色。但你始终可以通过 [Webhooks](/developers/webhooks) 和 [Management API](/user-management/manage-users#manage-roles-of-users) 在用户注册后更新用户角色。
...
可以，即使你在“登录体验”中禁用了用户注册，仍然可以使用魔法链接邀请用户。
...
3. 如果你的 sign-in prompt 设置为 "login" 或包含 "login"，Logto 会自动使用一次性令牌关联的账户登录，而不会提示切换。这是因为 "login" prompt 表示明确的认证 (Authentication) 意图，其优先级高于当前会话。
