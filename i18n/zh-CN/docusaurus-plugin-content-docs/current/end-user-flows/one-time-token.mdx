...
有时你可能希望邀请新用户加入你的应用程序 / 组织，而无需他们先创建账户。在这种情况下，应用程序可以向你的邮箱发送一个“魔法链接”。当你点击该链接时，你会立即完成认证 (Authentication)。
...
例如，当你禁用公开注册时，可以将带有一次性令牌的魔法链接（如 `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`）发送到用户邮箱，邀请其完成账户创建。你可以在自己的邮件发送服务中自定义邮件模板，例如：
...
在开始之前，请确保你已经准备好 Logto 实例，并已在你的应用服务器与 Logto 端点之间建立了机器对机器连接（Management API 需要）。了解更多 [Logto Management API](/integrate-logto/interact-with-management-api)。
...
魔法链接至少应包含令牌和用户邮箱作为参数，并跳转到你应用中的某个着陆页。
...
因此，你应额外指定登录参数 `clearTokens: false`，以避免清除现有令牌。
...
### 步骤 4：（可选）在登录回调页面清除缓存令牌 \{#step-4-clear-cached-tokens-in-sign-in-callback-page}

如果你在登录方法中指定了 `clearTokens: false`，则需要在登录回调页面手动清除令牌。
...
可以，你可以使用魔法链接邀请新用户加入你的应用，也可以加入组织。
...
3. 如果你的登录提示设置为 "login" 或包含 "login"，Logto 会自动登录与一次性令牌关联的账户，不会提示切换。这是因为 "login" 提示表示明确的认证 (Authentication) 意图，会优先于当前会话。
