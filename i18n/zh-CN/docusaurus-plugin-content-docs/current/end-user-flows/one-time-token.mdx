...
在这些情况下，应用程序可以向你的邮箱发送“魔法链接”。你点击链接后会立即完成认证 (Authentication)。
...
- **仅限邀请注册**：对于内部工具或处于测试阶段的 AI 产品，你可以关闭公开注册，通过魔法链接邀请特定用户。
- **组织成员邀请**：对于 SaaS 产品，使用魔法链接邀请新成员加入组织，简化入职流程。
- **登录 / 注册**：通过电子邮件发送魔法链接，实现无密码登录或注册。

例如，当你关闭了公开注册时，可以将带有一次性令牌的魔法链接（如 `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`）发送到用户邮箱，邀请其完成账户创建。你可以在自己的邮件服务中自定义邮件模板，例如：
...
### 我可以用魔法链接为用户分配特定角色吗？ \{#can-i-assign-users-to-specific-roles-using-the-magic-link}
