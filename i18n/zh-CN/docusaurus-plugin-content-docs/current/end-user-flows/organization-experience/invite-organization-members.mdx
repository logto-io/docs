...
你可以在 [Logto Console](https://cloud.logto.io/) 中轻松完成这些操作。你也可以使用 [Logto Management API](https://openapi.logto.io/operation/operation-createorganizationrole) 以编程方式创建组织角色。
...
- `POST /api/one-time-tokens`：为被邀请人在接受邀请时认证 (Authentication) 创建一次性令牌。[了解更多](/end-user-flows/one-time-token)
