...
sequenceDiagram
  ...
  Participant A as 组织 (Organization) 管理员
  Participant C as 你的多组织应用
	Participant L as Logto

  A ->> C: 输入被邀请者邮箱和角色
  C ->> L: 使用 Management API 创建组织 (Organization) 邀请
  L -->> C: 返回邀请 ID
  C ->> C: 通过邀请 ID 生成邀请链接
  C ->> L: 请求发送包含邀请链接的邀请邮件
  L -->> U: 发送包含邀请链接的邀请邮件
...
## 创建组织 (Organization) 角色 \{#create-organization-roles}

在邀请成员之前，需要创建组织 (Organization) 角色。参见 [组织模板](/authorization/organization-template) 了解更多关于角色和权限的信息。
...
你可以在 [Logto 控制台](https://cloud.logto.io/) 中轻松完成这些操作。你也可以使用 [Logto Management API](https://openapi.logto.io/operation/operation-createorganizationrole) 以编程方式创建组织 (Organization) 角色。

## 配置你的邮件连接器 (Connector) \{#configure-your-email-connector}

由于邀请是通过邮件发送的，请确保你的 [邮件连接器 (Connector)](/connectors/email-connectors) 已正确配置。要发送邀请，请配置一个用法类型为 `OrganizationInvitation` 的 [邮件模板](/connectors/email-connectors/email-templates#email-template-types)。你可以在内容中包含组织 (Organization)（如名称、Logo）和邀请人（如邮箱、姓名）的 [变量](/connectors/email-connectors/email-templates#email-template-variables)，并根据需要自定义 [本地化模板](/connectors/email-connectors/email-templates#email-template-localization)。
...
如果你还没有配置 Logto Management API，请参见 [与 Management API 交互](/integrate-logto/interact-with-management-api) 获取详细信息。

:::

### 使用 Logto Management API 创建组织 (Organization) 邀请 \{#create-an-organization-invitation-with-logto-management-api}
