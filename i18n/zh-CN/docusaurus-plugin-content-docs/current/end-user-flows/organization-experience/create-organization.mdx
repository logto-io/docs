...
想象一下，你正在构建一个 [多租户应用](https://auth.wiki/multi-tenancy)（例如，多租户 SaaS 应用），它服务于众多客户，每个客户拥有一个专用租户。当一个新客户到来时，他们创建一个新账户，以及一个用于其自身业务的新租户。
...
有两种方式为你的应用创建组织 (Organizations)。

### 通过 Logto 控制台创建

通过 Logto 控制台 UI 手动创建你的组织 (Organizations)。前往 <CloudLink to="/organizations">控制台 > 组织 (Organizations)</CloudLink> 创建组织 (Organization)，分配成员和角色 (Role)，并自定义组织 (Organization) 登录体验 UI。
...
在控制台 UI 上点击按钮效果很好，但我们真正希望终端用户能够自助创建组织 (Organizations)，并在**你的应用**中自行管理组织 (Organizations)。因此，你需要在你的应用中实现这些功能，借助 Logto Management API。
...
在你的应用中，当用户在组织 (Organization) 的上下文中执行操作时，他们需要使用组织令牌 (Organization token) 而不是常规访问令牌 (Access token)。组织令牌 (Organization token) 是一种特殊的 [JWT](https://auth.wiki/jwt)，其中包含组织 (Organization) 权限。就像任何 JWT [访问令牌 (Access tokens)](https://auth.wiki/access-token) 一样，你可以解码令牌声明并验证“权限 (Scope)”声明以进行权限检查。
