...
在开发多租户应用时，一个常见的场景是为你的组织成员提供一个平台，以便他们自主管理其组织资源。

我们仍然以包含管理员角色和普通成员角色的基本组织设置为例。

| 角色   | 权限                                                                                                                     |
| ------ | ------------------------------------------------------------------------------------------------------------------------ |
| Admin  | 管理组织中的组织成员，例如邀请、更新成员角色、删除成员等。<br />管理组织信息，例如更新组织名称和自定义数据、删除组织等。 |
| Member | 只能邀请用户加入组织。                                                                                                   |

因此，我们可以定义以下组织模板：

<center>
  <img src={organizationTemplateImage} width="100%" alt="组织模板" />
</center>

设置组织模板后，你还可以在你的服务中创建一系列 API，这些 API 在后台调用 Management API 以在特定组织中执行管理操作。你的服务中的 API 应验证组织令牌，以确保用户具有所需的权限。
...
你可以让管理员和成员用户邀请其他人加入组织。查看 [此文档](/end-user-flows/organization-experience/invite-organization-members) 了解有关邀请成员的详细信息。
...
你还可以实现允许管理员用户向特定组织添加机器人的功能。这可以通过首先创建机器对机器 (M2M) 应用，然后将这些 M2M 应用作为机器人添加到组织中来完成。
...
<Url href="https://openapi.logto.io/operation/operation-listorganizationscopes">获取组织权限</Url>
