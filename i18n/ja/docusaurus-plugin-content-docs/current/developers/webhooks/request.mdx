...
Webhook イベントが発生すると、Logto はそれに登録されたすべてのエンドポイントに `POST` リクエストを送信します。完全なイベントカタログは [Webhooks イベント](/developers/webhooks/events) にあります。このページでは、Logto が配信するリクエストの**形状**を記載しています。
...
**イベント:** `User.*`, `Role.*`, `Scope.*`, `Organization.*`, `OrganizationRole.*`, `OrganizationScope.*` — 完全なカタログについては [Webhooks イベント → データ変更フックイベント](/developers/webhooks/events#data-mutation-hook-events) を参照してください。
...
標準のリクエストボディフィールドと Management API コンテキストフィールドに加えて、`Organization.Membership.Updated` イベントは `organizationId` とペイロードのトップレベルにオプションのデルタ配列を持ちます（`event`、`createdAt` などの隣に — このイベントでは常に `null` である `data` 内ではありません）。
