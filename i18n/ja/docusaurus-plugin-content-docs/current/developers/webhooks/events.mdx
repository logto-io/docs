...
| User.Data.Updated             | ユーザープロファイルデータが更新されました（例：メール、アバター、custom.data、ソーシャルIDなど）。 |
...
| ユーザー登録                                                                                                                                                            | User.Created                    |
| [ジャストインタイムプロビジョニング](/organizations/just-in-time-provisioning)（メールドメインまたはエンタープライズ SSO コネクターの一致）で組織に自動プロビジョニング | Organization.Membership.Updated |
...
| イベントタイプ      | 説明                                                                                                                                                                                                                                            |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identifier.Lockout  | 連続したアイデンティティ確認失敗によりユーザーアカウントがロックされました。次のフローでトリガーされます：<br /><ul><li>パスワード認証失敗</li><li>コード認証失敗</li><li>ワンタイムトークン認証失敗</li></ul>                                  |
| Message.RateLimited | エンドユーザーが認証コードの [送信レート制限](/security/send-rate-limit) を超えました。エンドユーザー送信パス（Experience API および Account API）でのみ発生し、ペイロードは `{ action, recipient }` です。                                     |
| Grant.LimitExceeded | 認可 (Authorization) が成功し、ユーザーがアプリの [最大同時認証デバイス数](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) 制限 (`maxAllowedGrants`) を超えた場合、Logto はそのアプリの最も古いグラントを取り消します。 |
