...
Logto では、サードパーティアプリケーションに付与する特定の権限を制御できます。これには、[ユーザープロファイル](#user-permissions-user-profile-scopes)、[API リソース](#api-resource-permissions-api-resource-scopes)、[組織スコープ](#organization-permissions-organization-scopes) の管理が含まれます。ファーストパーティアプリとは異なり、サードパーティアプリが許可されていないスコープをリクエストした場合はアクセスが拒否されます。
...
[ユーザープロファイル権限](#user-permissions-user-profile-scopes)（例：メール、名前、アバター）や [API リソース権限](#api-resource-permissions-api-resource-scopes)（例：特定リソースへの読み取り / 書き込みアクセス）など、ユーザーレベルの権限を割り当てます。

リクエストされたリソース名（例：個人ユーザーデータ、API 名）や具体的な権限の説明（例：メールアドレス）が、ユーザーが確認できるよう同意画面に表示されます。
...
- `custom_data`: Logto ユーザープロファイルスコープ。[ユーザーカスタムデータ](/user-management/user-data/#custom-data) へのアクセスに使用。
...
- `urn:logto:scope:organizations`: Logto ユーザー組織スコープ。ユーザーの組織情報へのアクセスに使用。サードパーティアプリケーションで有効化・リクエストされた場合、同意画面に組織セレクターが表示されます。これにより、ユーザーはアクセスを許可する組織を確認・選択できます。詳細は [組織](/organizations) を参照してください。
...
Logto は API リソースに対してロールベースのアクセス制御 (RBAC) を提供します。API リソースはサービスが所有し、Logto によって保護されるリソースです。独自に定義した API スコープをサードパーティアプリケーションに割り当てて、API リソースへアクセスさせることができます。詳細は [認可 (Authorization)](/authorization) を参照してください。
