...
- `role`: Logto ユーザープロファイルスコープ。ユーザーの [ロール](/authorization/role-based-access-control/#roles) 情報へのアクセスに使用。
...
Logto は API リソースに対してロールベースのアクセス制御 (RBAC) を提供します。API リソースは自サービスが所有し、Logto によって保護されるリソースです。サードパーティアプリケーションに独自定義の API スコープを割り当てて、API リソースへのアクセスを許可できます。詳細は [RBAC](/authorization/role-based-access-control)、組織テンプレート、および [API 保護](/authorization/api-resources/protect-your-api/) を参照してください。
...
[組織権限](/authorization/organization-template/#organization-permission) は、Logto 組織専用に定義されたスコープです。組織情報やリソースへのアクセスに使用されます。
