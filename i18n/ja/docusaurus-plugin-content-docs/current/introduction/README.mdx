---
description: Logto を統合することで、アイデンティティおよびアクセス管理システムを素早く立ち上げることができます。認証 (Authentication)、認可 (Authorization)、マルチテナント管理をすべて一つで実現します。
---
...
Logto ドキュメントへようこそ！Logto は、モダンなアプリや SaaS プロダクト向けに設計されたアイデンティティおよびアクセス管理 (IAM) ソリューションです。アプリケーションに対して、安全でスケーラブルかつカスタマイズ可能な認証 (Authentication)・認可 (Authorization) システムを提供します。
...
Logto の機能は 4 つの主要な柱の上に構築されており、これらが製品やビジネスに簡単に統合できる機能の基盤となっています。
  ...
  items={[
    {
      ...
      description:
        'リソースの設定や管理を行うための Web ベースのインターフェースです。サインイン体験のクイックセットアップや簡単なアイデンティティ管理を提供します。',
      customProps: {
    ...
    },
    {
      ...
      href: '/end-user-flows',
      description:
        '認証 (Authentication) フローとユーザーインターフェースを一式提供します。識別子サインイン、ソーシャルサインイン、パスキーサインイン、多要素認証 (MFA)、Logto Console や API を通じた幅広いカスタマイズが可能です。',
      customProps: {
        icon: <ExperienceIcon />,
      },
    },
    {
      ...
      description:
        'Logto のバックエンドは、さまざまな認証 (Authentication)・認可 (Authorization) 機能を実現するための API 群を提供します。Logto Management API、Experience API、Account API などが含まれます。',
      customProps: {
        icon: <LogtoApiIcon />,
      },
    },
    ...
    },
    {
      ...
      description:
        'AI ツールや IDE を Logto Cloud に接続するリモート MCP サーバーです。リソース管理や認証 (Authentication) 連携を実現します。',
      customProps: {
        icon: <McpIcon />,
      },
    },
  ...
  items={[
    {
      ...
      href: '/logto-cloud',
      description:
        'クラウドサービスは Free、Pro、Enterprise プランを用意し、さまざまなニーズに対応します。Logto がホスティング・管理を行います。開発用テナントでは Logto の全機能をテストできます。',
      customProps: {
        icon: <CloudIcon />,
      },
    },
