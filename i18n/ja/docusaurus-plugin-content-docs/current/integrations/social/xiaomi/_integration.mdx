...
2. [Xiaomi Account Service](https://dev.mi.com/passport/oauth2/applist) にアクセスします
3. まだアプリケーションがない場合は、新しいアプリケーションを作成します

## Xiaomi OAuth アプリケーションの設定 \{#configure-xiaomi-oauth-application}

1. [Xiaomi Account Service](https://dev.mi.com/passport/oauth2/applist) にアクセスします
2. OAuth 設定を構成します：
   - ログインに使用したいアプリケーションを開き、「Callback URL」をクリックします（まだコールバック URL を編集していない場合、「Enabled」と表示されます）
...
## スコープの説明 \{#scopes-description}

デフォルトでは、コネクターは次のスコープをリクエストします：

- `1`：ユーザープロファイルの読み取り

利用可能なスコープ：

| Scope Value | 説明                                                       | API インターフェース |
| ----------- | ---------------------------------------------------------- | -------------------- |
...
| 6002        | サードパーティーデバイスを Mi Home へ追加                  | Mi Home              |
| 6003        | Alexa で Xiaomi デバイスを操作                             | Mi Home              |
| 6004        | サードパーティーサービスによる Xiaomi デバイスへのアクセス | Mi Home              |
...
複数のスコープはスペースで区切って設定できます。例：`1 3 6000`

## Xiaomi コネクターのテスト \{#test-xiaomi-connector}

以上です。サインアップおよびサインインで [ソーシャルサインインを有効化](https://docs.logto.io/connectors/social-connectors#enable-social-sign-in) するのを忘れないでください。
