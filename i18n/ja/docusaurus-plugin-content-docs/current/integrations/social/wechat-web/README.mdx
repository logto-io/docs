...
提出ごとにレビュアーがランダムに割り当てられると考えられ、基準が変動することがあります。最初は拒否されるかもしれませんが、諦めないでください！現状を説明し、どのように修正すればよいかをレビュアーに尋ねてください。
...
`scope` フィールドには 'snsapi_userinfo' または 'snsapi_base' のいずれかを入力します。このフィールドはオプションであり、空白のままにすることもできます。デフォルト値は 'snsapi_userinfo' に設定されています。異なる値の [違い](https://developers.weixin.qq.com/doc/offiaccount/OA_Web_Apps/Wechat_webpage_authorization.html) を確認できます。
