...
ほとんどは非常に簡単ですが、いくつかのヒントがあります：

- WeChat サインインをテストしたいだけで、アプリが App Store にない場合、「アプリが利用可能」セクションで「いいえ」を選択して「アプリダウンロードリンク」をスキップします。
- 「アプリ操作フローチャート」は難しそうに見えます。私たちの経験では、レビューを通過する可能性を高めるために、シンプルなフローチャートといくつかのアプリのスクリーンショットを用意する必要があります。
...
App Store の可用性で「いいえ」を選択した場合、「AppStore ダウンロードアドレス」の入力をスキップできます。
...
WeChat はネイティブサインインにユニバーサルリンクを必要とします。設定していない場合や知らない場合は、 [Apple の公式ドキュメント](https://developer.apple.com/ios/universal-links/) を参照してください。
...
_アプリケーション署名署名_ (应用签名) と _アプリケーションパッケージ名_ (应用包名) を入力します。

:::note

アプリに署名して署名を取得する必要があります。詳細については、 [アプリに署名する](https://developer.android.com/studio/publish/app-signing) を参照してください。
...
対応するビルドバリアントのレポートの `MD5` 値が _アプリケーション署名署名_ (应用签名) になりますが、値からすべてのセミコロンを削除し、小文字にすることを忘れないでください。
...
各提出時にレビュアーがランダムに割り当てられると推測されます。基準が変動しているため、最初は拒否されるかもしれませんが、諦めないでください！現状を説明し、どのように修正すればよいかをレビュアーに尋ねてください。
...
これらのアクションはあまり合理的ではないことは承知していますが、これが見つけた最小限の実行可能なソリューションです。詳細については、 [公式ガイド](https://developers.weixin.qq.com/doc/oplatform/en/Mobile_App/Access_Guide/iOS.html) を参照してください。
...
プロジェクト内の `WXEntryActivity` の最終的な位置は次のようになります（例として Kotlin を使用）：
...
次の行を `AndroidManifest.xml` に追加します：
