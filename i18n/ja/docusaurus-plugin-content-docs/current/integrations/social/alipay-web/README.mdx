...
## Alipay 開発者アカウントを登録する \{#test-alipay-web-connector}
...
> ℹ️ **注意**
>
> _openssl_ を使用して、次のコードスニペットをターミナルで実行することで、ローカルマシンで鍵ペアを生成できます。
>
> ```bash
...
## Logto Alipay Web コネクター設定をセットアップする \{#set-up-the-logto-alipay-web-connector-settings}

1. [Alipay コンソールワークスペース](https://open.alipay.com/dev/workspace) の「マイアプリケーション」(我的应用) パネルに移動し、「Web & Mobile Apps」(网页&移动应用) タブをクリックすると、すべてのアプリケーションの APPID を見つけることができます。
