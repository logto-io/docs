...
このガイドでは、あなたの Python ウェブアプリケーションに Logto を統合する方法を示します。
...
ご覧のとおり、`@authenticated` デコレーターを使用して、Flask アプリケーション API にユーザー情報コンテキストを提供しています。
...
例えば、API でユーザーの情報を表示するには、次のコードを使用できます：
...
例えば、サインイン時に `email` スコープを要求しなかった場合、`email` フィールドは JSON 出力から除外されます。ただし、`email` スコープを要求したが、ユーザーにメールアドレスがない場合、`email` フィールドは `null` 値で JSON 出力に含まれます。
