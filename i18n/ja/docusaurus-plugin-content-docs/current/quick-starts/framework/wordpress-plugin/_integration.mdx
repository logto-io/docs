...
:::note
プラグインを設定する前に、Logto コンソールで **従来のウェブ** アプリケーションを作成しておく必要があります。まだ作成していない場合は、[アプリケーションへの Logto の統合](/integrate-logto/integrate-logto-into-your-application) を参照してください。
:::
