...
[細かな UI 調整](/customization/match-your-brand) だけではブランドに完全に合わない場合、カスタム CSS を使ってサインイン体験を自由にカスタマイズできます。エディターに CSS コードを追加するだけで、すぐに変更内容を確認できます。
...
2. 左側のエディターで CSS コードを編集すると、右側のプレビューモーダルに即座に反映されます。注意：コードエディターは現在 CSS コードのみサポートしています。HTML や Javascript には対応していません。
3. 変更を保存します。
4. すべてのページで変更を確認するには、「[ライブプレビュー](/customization/live-preview)」ボタンをクリックするのを忘れずに。
