...
Logto をホスティングするための最小推奨ハードウェア要件は次の通りです：
...
- Docker Hub を使用する場合は、`ghcr.io/logto-io/logto:latest` の代わりに `svhd/logto:latest` を使用してください。
...
Logto の zip ファイルの URL を指定したい場合は、`--download-url` オプションを使用します。例：
...
Logto は設定に環境変数を使用し、`.env` ファイルもサポートしています。詳細な使い方や全変数リストは [設定](/concepts/core-service/configuration) をご覧ください。
...
これらの信頼できるホスティングプロバイダーは、Logto のワンクリックインストールテンプレートを提供しています。簡単にデプロイできるサービスで、Logto を使った CIAM システムを数秒でセットアップし、開始できます。
  ...
  items={[
    {
      ...
      description:
        '無料の開発テナント付きで、簡単に認証 (Authentication) 統合ができるコスト効率の高いクラウドサービス。',
      customProps: {
        icon: <LogtoIcon />,
      },
    },
    ...
    },
    {
      ...
      description: 'アプリのデプロイ、スケーリング、モニタリングを開発者向けに簡素化します。',
      customProps: {
        icon: <ZeaburIcon />,
      },
    },
