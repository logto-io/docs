...
description: 'ロールベースのアクセス制御 (RBAC) の実践ガイド：権限設計、ロール管理、CMS での安全な認可 (Authorization) の設定方法を学びます。'
---
...
| ---------------- | -------------------------------------------- | ---------------------------------- | -------------------------------- |
| **説明**         | 全コンテンツ管理のためのシステム全体アクセス | すべての記事の閲覧と公開管理が可能 | システム内で新規記事の作成が可能 |
| list:articles    | ✅                                           | ✅                                 | ❌                               |
| read:articles    | ✅                                           | ✅                                 | ❌                               |
| create:articles  | ✅                                           | ❌                                 | ✅                               |
| update:articles  | ✅                                           | ❌                                 | ❌                               |
...
:::note
デモのため、これらのリソースや設定は Logto コンソールから作成しています。実際のプロジェクトでは、Logto が提供する [Management API](/integrate-logto/interact-with-management-api) を使ってプログラム的にリソースや設定を作成できます。
:::
...
より複雑なシナリオ、たとえばマルチテナントアプリケーションの構築などには、Logto の包括的な組織 (Organization) サポートが役立ちます。組織単位でのアクセス制御の実装については、 [マルチテナント SaaS アプリケーション構築：設計から実装までの完全ガイド](/use-cases/multi-tenancy/build-multi-tenant-saas-application) をご覧ください。
