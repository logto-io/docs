...
description: 'ロールベースのアクセス制御 (RBAC) の実践ガイド：権限設計、ロール管理、安全な認可 (Authorization) のセットアップ方法を CMS 例で学ぶ。'
---
...
アプリケーションのために安全かつスケーラブルな認可 (Authorization) システムの実装に悩んでいませんか？ロールベースのアクセス制御 (RBAC) はユーザー権限管理の業界標準ですが、正しく実装するのは難しい場合があります。このチュートリアルでは、実際のコンテンツ管理システム (CMS) を例に、堅牢な RBAC システムの構築方法を紹介します。

このガイドを通じて、次のことが学べます：

- ✨ 細かな権限設計と実装方法、精密なコントロールの実現
- 🔒 権限を意味のあるロールに整理するベストプラクティス
- 👤 リソース所有権を効果的に扱うテクニック
...
ロールベースのアクセス制御 (RBAC) は、単にユーザーに権限を割り当てるだけではありません。セキュリティと保守性のバランスを取るための体系的な認可 (Authorization) アプローチを構築することです。
...
細かな権限設計により、ユーザーがシステム内で何ができるかを精密にコントロールできます。「admin」や「user」といった大まかなアクセスレベルではなく、リソースごとにユーザーが実行できる具体的なアクションを定義します。例：
...
リソース所有権は、CMS の認可 (Authorization) 設計における基本概念です。RBAC でロールごとのアクションを定義する一方、所有権はアクセス制御に個人の側面を加えます：

- 著者は自分が作成した記事へのアクセス権を自動的に持ちます
- この自然な所有権モデルにより、著者は常に自分のコンテンツを閲覧・編集できます
...
| ---------------- | -------------------------------------------- | ------------------------------------------ | ------------------------------ |
| **説明**         | 全コンテンツ管理のためのシステム全体アクセス | すべての記事閲覧と公開ステータス管理が可能 | システム内で新規記事作成が可能 |
| list:articles    | ✅                                           | ✅                                         | ❌                             |
| read:articles    | ✅                                           | ✅                                         | ❌                             |
| create:articles  | ✅                                           | ❌                                         | ✅                             |
| update:articles  | ✅                                           | ❌                                         | ❌                             |
...
- **管理者 (Admin)**：CMS 全体の完全なコントロール、すべての記事操作が可能
- **パブリッシャー (Publisher)**：コンテンツレビューと公開管理に特化
- **著者 (Author)**：コンテンツ作成に特化

このロール構造により、役割ごとの責任分担が明確になります：

- 著者はコンテンツ作成に集中
- パブリッシャーはコンテンツ品質と公開を管理
- 管理者はシステム全体を維持
...
:::note
デモのため、これらのリソースや設定は Logto コンソールから作成しています。実際のプロジェクトでは、Logto が提供する [Management API](/integrate-logto/interact-with-management-api) を使ってプログラム的に作成・設定できます。
:::

## フロントエンドと Logto RBAC の統合 \{#integrate-your-frontend-with-logto-rbac}

Logto で RBAC を設定できたので、フロントエンドに統合していきます。

まず [Logto クイックスタート](/quick-starts) に従ってアプリケーションへ Logto を統合します。

本例では [React](/quick-starts/react) を使って説明します。

アプリケーションに Logto をセットアップしたら、Logto が機能するよう RBAC 設定を追加します。
...
const logtoConfig: LogtoConfig = {
  ...
  // フロントエンドで API リソースからリクエストしたい全スコープ
  scopes: [
    'list:articles',
    'create:articles',
    'read:articles',
...
すでにサインイン済みの場合は、一度サインアウトして再度サインインし直すことで設定が反映されます。

ユーザーが Logto でサインインし、上記で指定した API リソースのアクセス トークンをリクエストすると、Logto はユーザーのロールに関連するスコープ（権限）をアクセス トークンに追加します。
...
## バックエンドと Logto RBAC の統合 \{#integrate-your-backend-with-logto-rbac}

次は、Logto RBAC をバックエンドに統合します。

### バックエンド認可 (Authorization) ミドルウェア \{#backend-authorization-middleware}

まず、バックエンドでユーザー権限をチェックし、ログイン済みかどうか、必要な権限を持っているかを判定するミドルウェアを追加します。
...
このミドルウェアでは、フロントエンドリクエストが有効なアクセス トークンを含んでいるかを検証し、アクセス トークンのオーディエンスが Logto コンソールで作成した API リソースと一致するかをチェックしています。

API リソースを検証する理由は、この API リソースが実際に CMS バックエンドのリソースを表し、すべての CMS 権限がこの API リソースに紐付いているためです。

この API リソースが Logto で CMS リソースを表しているので、フロントエンドのコードでは、バックエンド API リクエスト時に対応するアクセス トークンを付与します：
...
権限とリソース所有権の両方をチェックする必要がある API では、`hasScopes` 関数を利用できます。たとえば記事一覧 API では、`list:articles` 権限を持つユーザーは全記事にアクセスでき、著者は自分が作成した記事のみアクセスできます：
...
これで RBAC 実装が完了しました。[完全なソースコード](https://github.com/logto-io/rbac-sample) で全体像を確認できます。
...
:::note
「ユーザー管理」で作成したユーザーの認証情報でサインインできない場合は、まず適切なサインイン方法を有効化する必要があります。Logto コンソールの「サインイン体験」で、Email + パスワードやユーザー名 + パスワードなど、希望する認証方法を有効にしてください。
:::

まず、Alex と Charles でそれぞれサインインし、記事を作成してみます。

Alex は管理者 (Admin) ロールなので、記事の作成・削除・編集・公開・全記事閲覧が可能です。

![CMS dashboard - Alex](https://uploads.strapi.logto.io/cms_dashboard_alex_a11863a94b.png)

Charles は著者 (Author) ロールなので、自分の記事のみ作成・閲覧・編集・削除できます。

![CMS dashboard - Charles - Article list](https://uploads.strapi.logto.io/cms_dashboard_charles_05f610066a.png)

Bob はパブリッシャー (Publisher) ロールなので、すべての記事を閲覧・公開できますが、作成・編集・削除はできません。
...
より複雑なシナリオ、たとえばマルチテナントアプリケーションの構築などには、Logto の包括的な組織 (Organization) サポートが役立ちます。組織単位のアクセス制御実装については、 [マルチテナント SaaS アプリケーション構築：設計から実装までの完全ガイド](https://blog.logto.io/build-multi-tenant-saas) をご覧ください。
