...
標準コネクターで要件を満たせない場合は、[お問い合わせ](https://logto.productlane.com/roadmap) ください。OSS ユーザーの場合、要件が緊急の場合は [コネクターの実装 (OSS)](/logto-oss/develop-your-connector/implement-connectors#build-a-passwordless-connector) も可能です。[コントリビューション](/logto-oss/contribution) も常に歓迎しています。あなたの取り組みが、同じニーズを持つ他のコミュニティメンバーの助けになるかもしれません。
