...
## メールテンプレートの種類 \{#email-template-customization-options}
...
## メールテンプレートの変数 \{#email-template-types}
...
## メールテンプレートのローカライズ \{#email-template-examples}

### 異なる言語のカスタムメールテンプレート \{#email-template-localization}
...
### プロバイダー側のメールテンプレートのローカライズ \{#custom-email-templates-for-different-languages}
...
## よくある質問 \{#provider-side-email-template-localization}

<details>
<summary>

### Logto にテンプレートが設定されていない場合、サードパーティのメールテンプレートサービスを使用する方法はありますか？ \{#faqs}
...
### Logto メールを使用して、ユーザーにカスタマイズされた「ウェルカムメール」を送信する方法はありますか？ \{#how-to-use-third-party-email-template-services-if-templates-are-not-configured-in-logto}
...
## 関連リソース \{#is-there-a-way-to-use-logto-email-for-sending-our-users-a-customized-welcome-email}
