...
標準コネクターで要件を満たせない場合は、[お問い合わせ](https://feedback.logto.io/roadmap) ください。OSS ユーザーの場合、要件が緊急の場合は [独自コネクターの実装 (OSS)](/logto-oss/develop-your-connector/implement-connectors#build-a-passwordless-connector) も可能です。常に [コントリビューション](/logto-oss/contribution) を歓迎しています。あなたの取り組みが同じニーズを持つ他のコミュニティメンバーの助けになるかもしれません。
