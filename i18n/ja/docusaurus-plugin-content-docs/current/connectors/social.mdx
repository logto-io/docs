...
標準コネクターが要件を満たさない場合は、[お問い合わせ](https://feedback.logto.io/roadmap) ください。OSS ユーザーの場合、要件が急ぎの場合は [コネクターの実装 (OSS)](/logto-oss/develop-your-connector/implement-connectors#build-a-social-connector) も可能です。常に [コントリビューション](/logto-oss/contribution) を歓迎しています。あなたの取り組みが同じニーズを持つ他のコミュニティメンバーの助けになるかもしれません。
