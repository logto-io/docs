...
[ロールベースのアクセス制御 (RBAC)](https://auth.wiki/rbac) は、ユーザーに対してロールに基づいて権限を割り当てる方法です。次のシナリオで RBAC の使用を検討してください：
...
- **_注文管理者_** は注文を管理し、製品を見ることができます（注文は製品で構成されているため）が、製品を管理することはできません。
- **_製品管理者_** は製品を管理でき、注文については知らないはずです。
...
`order_admin` と `product_admin` の両方をユーザーに割り当てることも可能で、その場合、定義した 6 つの権限すべてを持つことになります。
