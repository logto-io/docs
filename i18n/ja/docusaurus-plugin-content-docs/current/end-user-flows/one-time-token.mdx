...
このトークンは一定期間のみ有効で、エンドユーザーのメールアドレスに紐付けられています。

新しいユーザーにアカウント作成を求めずにアプリケーション / 組織へ招待したい場合があります。このような場合、アプリケーションからメールに「マジックリンク」を送信できます。リンクをクリックすると、即座に認証 (Authentication) されます。
...
- **招待制登録**：社内ツールやテスト段階の AI プロダクトなどで、一般公開登録を無効にし、特定のユーザーにマジックリンクで招待できます。
- **組織メンバー招待**：SaaS プロダクトで、マジックリンクを使って新しいメンバーを組織に招待し、オンボーディングを効率化できます。
- **サインイン / サインアップ**：メール経由でパスワードレスのサインインまたはサインアップ用マジックリンクを送信します。
- **パスワードリセット**：独自アプリケーションからパスワードリセット用マジックリンクを送信し、ユーザーがワンタイムトークンを検証して Logto で新しいパスワードを設定できるようにします。

例えば、一般公開登録を無効にしている場合、ワンタイムトークン付きのマジックリンク（例： `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com` ）をユーザーのメールに送信し、アカウント作成を完了するよう招待できます。メールテンプレートは独自のメール配信サービスでカスタマイズ可能です。例：
...
sequenceDiagram
  ...
  U ->> C: （任意）アプリケーションからマジックリンクをリクエスト
  C ->> L: ワンタイムトークンをリクエスト
  L -->> C: ワンタイムトークンを返却
...
Logto はマジックリンク実装を容易にするため、Management API および Experience API を提供しています。

開始する前に、Logto インスタンスが準備できていること、およびアプリケーションサーバーと Logto エンドポイント間でマシン間通信 (M2M) 接続が確立されていることを確認してください（Management API に必要）。 [Logto Management API](/integrate-logto/interact-with-management-api) について詳しくはこちら。
...
```json
{
  ...
  // 任意。デフォルトは 600（10 分）。
  "expiresIn": 3600,
  // 任意。検証成功時に指定した組織へユーザーをプロビジョニングします。
  "context": {
...
パスワードリセット用マジックリンクの場合、トークンをパスワード忘れフローに限定します：
...
サインインまたはサインアップ用マジックリンクの場合、リンクには少なくともトークンとユーザーのメールアドレスをパラメータとして含めてください。
パスワードリセット用マジックリンクの場合、メールパラメータは任意です。省略した場合、Logto はトークン検証前にユーザーにメールアドレスの入力を求めます。
マジックリンクは独自アプリケーションのランディングページへ遷移するようにしてください。
...
パスワードリセット用マジックリンクの場合、`first_screen` を `reset_password` に設定して認証 (Authentication) リクエストを開始します。
ワンタイムトークンは `one_time_token` で渡します。
ランディングページでユーザーのメールアドレスが既に分かっている場合は `login_hint` で渡してください。分からない場合は省略し、Logto がトークン検証前にメールアドレスの入力を求めます。
...
そのため、既存トークンのクリアを防ぐために追加のサインインパラメータ `clearTokens: false` を指定してください。
この場合、サインインコールバックページで手動でトークンをクリアする必要があります。

マジックリンクが認証済みユーザー向けでない場合は、この注意事項は無視してください。
...
検証が成功すると、ユーザーは自動的に組織に参加し、デフォルトの組織ロールが割り当てられます。
...
### 「サインアップとサインイン」でユーザー登録を無効にした場合でも、マジックリンクでユーザーを招待できますか？ \{#if-i-disable-user-registration-in-sign-in-experience-can-i-still-use-magic-link-to-invite-users}
...
### すでにサインインしているユーザーが別のマジックリンクをクリックした場合、どうなりますか？ \{#what-will-happen-if-a-user-already-signed-in-and-then-click-another-magic-link}

</summary>

いくつかのシナリオが考えられます：

1. ユーザーがすでにサインインしていて、現在のアカウントに紐付いたマジックリンクをクリックした場合。この場合、Logto はワンタイムトークンを検証し、必要に応じて組織へのプロビジョニングを行います。
2. ユーザーがすでにサインインしていて、別のアカウントに紐付いたマジックリンクをクリックした場合。この場合、Logto は新しいアカウントで続行するか、現在のアカウントでアプリケーションに戻るかをユーザーに促します。
...
3. サインインプロンプトが "login" または "login" を含む場合、Logto は切り替えを促さず、ワンタイムトークンに紐付いたアカウントで自動的にサインインします。これは "login" プロンプトが明示的な認証 (Authentication) 意図を示し、現在のセッションより優先されるためです。
