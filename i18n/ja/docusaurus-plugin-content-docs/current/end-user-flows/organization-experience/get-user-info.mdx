...
### 1. ID トークン (ID token) をデコードする \{#1-decode-the-id-token}
...
最新の情報が必要な場合は、以下の 2 番目の方法を利用するか、`clearAllTokens()` を呼び出して認証 (Authentication) フローを再実行し、新しい ID トークン (ID token) を取得してください。
...
### 2. `/oidc/me` エンドポイントからユーザー情報を取得する \{#2-fetch-user-info-from-the-oidc-me-endpoint}
