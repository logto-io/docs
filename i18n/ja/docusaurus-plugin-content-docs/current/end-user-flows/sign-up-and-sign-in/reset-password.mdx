...
- [カスタムトークンクレーム](/developers/custom-token-claims)：`hasPassword` を ID / アクセストークンに注入（フロントエンドで追加の API コールなしに UI 分岐が可能）
...
メール / 電話 / ユーザー名登録時に「パスワード設定」が必須でも、**ソーシャルサインイン**のみで作成されたユーザーは、初期パスワード作成をスキップして摩擦を減らします。これらのユーザーは明示的にパスワードを設定するまで `hasPassword = false` となります。セキュリティモデルで必要な場合を除き、ソーシャルサインアップ直後に即座にパスワード設定を強制しないでください。遅延かつコンテキストに応じたプロンプトの方が通常はコンバージョンが高くなります。

:::

## カスタムパスワードポリシー \{#custom-password-policy}

パスワードの長さ、文字要件、単語制限などをカスタマイズして、ビジネスのセキュリティ要件とユーザー体験の両立を図れます。これらの設定は <CloudLink to="/security/password-policy">セキュリティ > パスワードポリシー</CloudLink> セクションで設定できます。詳細は [パスワードポリシー](/security/password-policy) ドキュメントを参照してください。
...
セルフホスト型のパスワードリセットエンドポイントを作成し、Logto SDK を利用して [`first_screen`](/end-user-flows/authentication-parameters/first-screen) を `reset-password` に設定したサインインリクエストを開始できます。これにより、ユーザーはシームレスにパスワードリセットページへリダイレクトされます。
