...
_コアサービス_ は、重要な Logto の業務を担当するモノリスサービスです。ソースコードは [`@logto/core`](https://github.com/logto-io/logto/tree/master/packages/core) にあります。
...
Logto は、素晴らしい認定済み [OpenID Connect](https://openid.net/connect/) 実装 [node-oidc-provider](https://github.com/panva/node-oidc-provider) を内部で使用しています。プロバイダーは `/oidc` にマウントされており、関連する設定とファイルは [packages/core/src/oidc](https://github.com/logto-io/logto/tree/master/packages/core/src/oidc) で確認できます。
...
- [OAuth 2.0 トークン取り消し](https://www.rfc-editor.org/rfc/rfc7009.html)
- [OAuth 2.0 リソースインジケーター](https://www.rfc-editor.org/rfc/rfc8707.html)
- [コード交換のためのプルーフキー (PKCE)](https://www.rfc-editor.org/rfc/rfc7636.html)
...
プログラムで API にアクセスするには、[Management API と連携する](/integrate-logto/interact-with-management-api) を参照してください。
