...
Si nuestra [personalización de la interfaz](/customization/match-your-brand) no se ajusta completamente a tu marca, utiliza CSS personalizado para adaptar tu experiencia de inicio de sesión. Simplemente añade tu código CSS en nuestro editor y verás los cambios al instante.

## CSS personalizado en Logto Console \{#custom-css-in-logto-console}

1. Ve a <CloudLink to="/sign-in-experience/branding">Consola > Inicio de sesión y cuenta > Marca > CSS personalizado</CloudLink>.
2. Edita el código CSS en el editor de la izquierda; se mostrará en vivo en la vista previa de la derecha de inmediato. Nota: el editor de código solo admite código CSS por ahora. No HTML ni Javascript.
3. Guarda los cambios.
4. No olvides hacer clic en el botón “[Vista previa en vivo](/customization/live-preview)” para comprobar los cambios en todas las páginas.
...
Para ver el diseño de la interfaz por componentes de Logto, puedes navegar a [Logto/packages/experience/src](https://github.com/logto-io/logto/tree/master/packages/experience/src) en GitHub o abrir la herramienta "Inspeccionar elementos" del navegador en la página de vista previa en vivo de Logto.

## Ejemplos \{#examples}

Por ejemplo, si quieres darle a tu página de inicio de sesión un estilo _Night City_ para la vista web, prueba este CSS:
...
<img
  ...
  alt="Ejemplo de uso de CSS personalizado en Logto"
/>

:::note

Dado que Logto utiliza CSS Modules, puedes ver un valor hash en la propiedad `class` de los elementos DOM (por ejemplo, un `<div>` con `vUugRG_container`). Para sobrescribir estos estilos, puedes usar el selector CSS `$=` para seleccionar elementos cuyo valor de clase termina con un valor específico. En este caso, debe ser `div[class$=container]`.

:::

## CSS personalizado por Management API \{#custom-css-by-management-api}
