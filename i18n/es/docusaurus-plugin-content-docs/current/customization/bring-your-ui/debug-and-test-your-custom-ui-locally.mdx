...
sequenceDiagram
    ...
    B -->> C: Responde la solicitud de inicio de sesión y<br/>le dice al cliente que redirija a tu aplicación
...
Pero ahora, dado que estás desarrollando tu propia UI de inicio de sesión personalizada, necesitas una forma de navegar a las páginas de UI de inicio de sesión personalizadas que se ejecutan en tu máquina local en su lugar. Esto requiere un servicio de túnel local para interceptar las solicitudes salientes de tu aplicación y redirigirlas a tus páginas de UI de inicio de sesión personalizadas.

Además, necesitas interactuar con la [API de Experiencia de Logto](https://openapi.logto.io/group/endpoint-experience) para autenticar usuarios y gestionar sesiones. Este servicio también ayudará a reenviar estas solicitudes de la API de Experiencia a Logto Cloud para evitar problemas de CORS.
...
sequenceDiagram
    ...
    C ->> D: Reenvía las solicitudes de la API de Experiencia a Logto Cloud
    D -->> C: Autentica la solicitud de inicio de sesión y<br/>le dice al cliente que redirija a tu aplicación
    C -->> A: Redirige a tu aplicación
    A --> A: Maneja la devolución de llamada de inicio de sesión y<br/>el usuario ahora está autenticado
```

Con el servicio de túnel en su lugar, ahora puedes desarrollar y probar tu UI de inicio de sesión personalizada localmente, sin necesidad de cargar los activos en Logto Cloud cada vez que realices un cambio.
...
Si todo está configurado correctamente, cuando hagas clic en el botón "iniciar sesión" en tu aplicación, deberías ser dirigido a tu página de inicio de sesión personalizada en lugar de la UI integrada de Logto, junto con una sesión válida (cookies) que te permita interactuar más con la API de Experiencia de Logto.
