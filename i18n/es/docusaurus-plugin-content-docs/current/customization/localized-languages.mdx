...
1. **Navega a**: <CloudLink to="/sign-in-experience/content">Console > Sign-in & account > Content > Languages</CloudLink>.
2. **Gestionar idioma**: Haz clic en el botón “Manage language” para acceder a tu biblioteca de idiomas.
   ...
   - **Agregar un nuevo idioma**: Haz clic en el botón “Add language”, selecciona una etiqueta de idioma, proporciona tus traducciones y guarda los cambios para agregar un nuevo idioma.
...
Aquí tienes algunos términos clave para entender al gestionar idiomas:

| Definición                     | Descripción                                                                                                                                                                                                                                     |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Etiqueta de idioma             | La etiqueta de idioma identifica el idioma del contenido. Una etiqueta de idioma está compuesta por un código de idioma (por ejemplo, en, fr, zh) y un código de país / región (por ejemplo, US, UK, KR) separados por guiones. Ejemplo: en-US. |
...
| Valores fuente de Logto        | Los valores fuente de Logto son valores proporcionados por Logto que no han sido personalizados por los usuarios.                                                                                                                               |
...
También puedes usar el API [PATCH /api/sign-in-exp](https://openapi.logto.io/group/endpoint-sign-in-experience) para controlar las [políticas de detección de idioma](https://openapi.logto.io/operation/operation-getsigninexp#operation-getsigninexp-200-body-application-json-languageinfo).
...
1. Parámetro OIDC `ui_locales` de la solicitud de autenticación actual (se usa la primera etiqueta compatible). Ver [ui_locales](/end-user-flows/authentication-parameters/ui-locales).
...
3. De lo contrario, el idioma predeterminado del tenant en la Experiencia de Inicio de Sesión.
...
Supongamos que tienes un sitio web donde el idioma predeterminado es inglés y la detección automática está activada. Un usuario de Japón llega a tu sitio y decide crear una cuenta. Si usa japonés como idioma de su aplicación pero Logto aún no admite ese idioma, la pantalla de inicio de sesión aparecerá en inglés.
...
### ¿Cómo puedo establecer un código de país predeterminado para el número de teléfono en la experiencia de inicio de sesión? \{#how-can-i-set-a-default-phone-number-country-code-for-the-sign-in-experience}
