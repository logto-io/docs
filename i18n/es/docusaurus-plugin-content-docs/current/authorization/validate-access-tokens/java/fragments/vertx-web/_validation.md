...
public class JwtAuthHandler implements Handler<RoutingContext> {
        ...
        jwtAuth.authenticate(new JsonObject().put("jwt", token))
            .onSuccess(user -> {
                try {
                    ...
                    context.response()
                        .setStatusCode(e.getStatusCode())  // Usa el código de estado de la excepción
                        .putHeader("Content-Type", "application/json")
    ...
    private void verifyPayload(JsonObject principal) {
        ...
        // Usa los métodos auxiliares de abajo para la extracción de reclamos
    }

    // Métodos auxiliares para JWT en Vert.x
