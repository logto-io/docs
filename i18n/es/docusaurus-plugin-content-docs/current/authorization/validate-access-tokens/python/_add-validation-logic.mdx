...
def require_access_token(view_func):
    def wrapper(request, *args, **kwargs):
        try:
            ...
            # Adjunta la información de autenticación a la request para uso general
...
class AccessTokenAuthentication(TokenAuthentication):
    ...
    def authenticate_credentials(self, key):
        ...
        try:
            ...
            # Crea un objeto tipo usuario que contiene la información de autenticación para uso general
            user = type('User', (), {
                'auth': auth_info,
...
De acuerdo con tu modelo de permisos, implementa la lógica de verificación adecuada en `jwt_validator.py`:
...
def verify_payload(payload: Dict[str, Any]) -> None:
    ...
    # Verifica que el ID de la organización coincida con el contexto (puede que necesites extraerlo del contexto de la request)
