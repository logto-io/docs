...
[Autorización (Authorization)](https://auth.wiki/authorization) define qué acciones puede realizar un usuario o a qué recursos puede acceder después de ser autenticado, determinando qué puede hacer en tu aplicación.
...
Hay dos tipos de [tokens de acceso](https://auth.wiki/access-token):

1. **Tokens de acceso**: Usados para acceder a recursos de API de usuario / sistema.
2. **Tokens de organización**: Tokens de acceso especiales para recursos de organización, utilizados en una arquitectura multi-tenant cuando la función de organización está habilitada.
...
## Familiarízate con el control de acceso basado en roles \{#get-familiar-with-role-based-access-control}
...
## Características para la autorización \{#features-for-authorization}
