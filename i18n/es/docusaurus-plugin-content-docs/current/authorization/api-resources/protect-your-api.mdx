...
sequenceDiagram
    ...
    Note over Cliente,API: 1. Solicitud de autenticación
    Cliente->>Logto: GET /oidc/auth
    Note right of Cliente: con resource=https://resource-server.com/api<br/>y alcance opcional=read write
    Logto-->>Cliente: Devuelve authorization_code

    Note over Cliente,API: 2. Solicitud de token
    Cliente->>Logto: POST /oidc/token
    Note right of Cliente: con authorization_code<br/>resource=https://resource-server.com/api<br/>y alcance opcional=read write
    ...
    Note over Cliente,API: 3. Solicitud de API
...
Logto sigue el protocolo estándar de autorización basado en tokens para proteger tus recursos de API. Para aprender más sobre OAuth 2.0, por favor consulta el [documento oficial](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.1) de OAuth 2.0.
...
Para desarrollo, para inspeccionar visualmente un token JWT, puedes visitar [jwt.io](https://jwt.io/) para decodificar y verificar los tokens que recibiste. Ten cuidado o nunca uses los tokens de un entorno de producción. Como este es un servicio público en línea proporcionado por un tercero, tu token puede estar expuesto.
...
Existen varias bibliotecas y paquetes de código abierto que pueden ayudarte a validar y decodificar un token JWT fácilmente. Puedes elegir uno e integrarlo con tu aplicación backend según el lenguaje y el framework que estés utilizando. Por favor, consulta algunos de los ejemplos que tenemos:
...
Logto utiliza el Protocolo de Autorización OAuth 2.0 basado en código para hacer que tu solicitud de API sea segura. Si estás interesado en la estrategia detrás de esto, consulta la [especificación](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.1) de OAuth 2.0 para más detalles.
