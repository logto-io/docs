...
# Protege los recursos de API con RBAC

Además de [Proteger tu API](/authorization/api-resources/protect-your-api), que asegura los recursos garantizando que un JWT válido esté presente, también se puede aplicar el Control de Acceso Basado en Roles (RBAC).
...
sequenceDiagram
    ...
    Note left of Logto: El token contiene:<br/>- Reclamos estándar de JWT<br/>- Alcances concedidos (filtrados por RBAC)

    Note over Client,API: Solicitud de API
    Client->>API: Solicitud con token Bearer

    Note over API: Validar token JWT:<br/>- Validación estándar de JWT<br/>- Verificar alcances concedidos

    alt Token Inválido
        API-->>Client: 401 No autorizado
    else Token Válido
        alt Alcances requeridos no en alcances concedidos
...
Los tokens de acceso de corta duración aseguran actualizaciones más frecuentes de los permisos de usuario. Configura los ajustes de expiración de tokens en la configuración del [Recurso de API](/authorization/api-resources/#logto-api-resource-schema) de la Consola. La desventaja es que aumentará el uso de tus tokens.

**Opción 2: Verificar dinámicamente los permisos**

Llama a los endpoints de [Logto Management API](/integrate-logto/interact-with-management-api) periódicamente (por ejemplo, SWR) o utiliza técnicas como WebSocket para implementar conexiones de larga duración para obtener dinámicamente los permisos de usuario. Al detectar cambios, borra el token de acceso existente y los tokens recién emitidos reflejarán automáticamente los cambios en el alcance de permisos. - API: User-role - API: Role-permission

    Cuando se detecten cambios en los permisos, primero borra el token de acceso del almacenamiento y luego llama al método del SDK `getAccessToken()` para adquirir uno nuevo. El nuevo token de acceso emitido debería reflejar los cambios en los permisos.
