...
La [solución de inicio de sesión único (SSO)](/end-user-flows/enterprise-sso) de Logto simplifica la gestión de acceso para tus clientes empresariales. Los conectores empresariales SSO son cruciales para habilitar el SSO para tus diferentes clientes empresariales.
...
Logto proporciona conectores preconstruidos para proveedores de identidad empresariales populares, ofreciendo una integración rápida. Para necesidades personalizadas, admitimos la integración mediante los protocolos [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) y [SAML](https://auth.wiki/saml).
...
- **Dominios de correo electrónico**: Si el dominio del correo electrónico ingresado por el usuario está en el campo "Dominios de correo electrónico empresariales" de algún conector SSO empresarial, el usuario será redirigido a la URL de inicio de sesión correspondiente de ese conector SSO.
...
Una vez habilitado, aparecerá un botón de "Inicio de sesión único" en tu página de inicio de sesión. Los usuarios empresariales con dominios de correo electrónico habilitados para SSO pueden acceder a tus servicios utilizando sus proveedores de identidad empresariales (IdP). Para obtener más información sobre la experiencia de usuario SSO, consulta Flujos de usuario: [SSO empresarial](/end-user-flows/enterprise-sso).
