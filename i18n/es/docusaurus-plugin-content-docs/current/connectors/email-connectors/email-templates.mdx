...
- Si la variable raíz `application` no se proporciona, el marcador de posición handlebars será ignorado y no se reemplazará.
- Si el objeto `application` proporcionado no contiene los campos requeridos o el valor es indefinido, el marcador de posición handlebars será reemplazado por una cadena vacía. Ejemplo: `{{application.foo.bar}}` será reemplazado por ``.
...
### ¿Existe una forma de usar el correo electrónico de Logto para enviar a nuestros usuarios un "correo de bienvenida" personalizado? \{#is-there-a-way-to-use-logto-email-for-sending-our-users-a-customized-welcome-email}
