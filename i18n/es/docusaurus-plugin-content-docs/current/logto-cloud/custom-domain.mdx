...
- **Plan Pro**: Incluye 1 dominio personalizado, con la posibilidad de añadir hasta 10 dominios personalizados en total mediante complementos
...
Después de que un dominio personalizado esté **Activo**, abre la tarjeta del dominio en <CloudLink to="/tenant-settings/domains">Consola > Configuración > Dominios</CloudLink> y utiliza la sección **Archivos de verificación de dominio** para añadir, actualizar o eliminar archivos.
...
Si encuentras problemas con el certificado SSL al configurar tu dominio personalizado, puede estar relacionado con los registros CAA en tu configuración DNS. Los registros CAA especifican qué Autoridades Certificadoras (CAs) están autorizadas para emitir certificados para tu dominio. Si usas registros CAA, deberás autorizar tanto a "letsencrypt.org" como a "pki.goog" para que Logto pueda emitir certificados SSL.
...
Si obtienes un error "redirect URI does not match" después de añadir tu dominio personalizado, necesitas actualizar la configuración de tu SDK para usar el dominio personalizado como endpoint.

**Sobre el "dominio principal":**

No hay una configuración separada de "dominio principal" en Logto. Después de añadir un dominio personalizado, tanto tu dominio personalizado como el dominio predeterminado `{tenant-id}.logto.app` siguen siendo válidos. El dominio que configures en el parámetro `endpoint` de tu SDK determina qué dominio se usará para los flujos de autenticación.
...
3. Añade este URI de redirección en la consola de desarrollador de tu proveedor social (por ejemplo, Google, GitHub, Facebook).

**Para múltiples dominios personalizados:**

- Añade todos los URIs de redirección para cada dominio personalizado que hayas configurado. Esto asegura que el inicio de sesión social funcione sin importar desde qué dominio accedan los usuarios.
...
**Para múltiples dominios personalizados:** Añade todos los URIs de redirección correspondientes a tu IdP para asegurar que el SSO empresarial funcione en todos los dominios.
...
[Las passkeys para autenticación multifactor (MFA)](/end-user-flows/mfa/webauthn) y [el inicio de sesión con passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) están vinculados al dominio donde se registraron. Los usuarios deben iniciar sesión a través del mismo dominio para usar sus passkeys.

Limitación actual: Logto aún no admite la verificación de passkey entre dominios. Si un usuario registra una passkey en `auth.us.example.com`, debe iniciar sesión a través de `auth.us.example.com` para usar esa passkey tanto para la verificación MFA como para el inicio de sesión con passkey. Una passkey registrada en un dominio no puede usarse al iniciar sesión a través de un dominio personalizado diferente.
