...
:::note
Cambiar el dominio después de publicar tu servicio puede causar problemas porque el código de tu aplicación y las integraciones podrían seguir haciendo referencia al dominio antiguo. Para asegurar una transición sin problemas, **configura tu dominio personalizado al principio** durante la creación de un tenant de Producción.
:::
...
Si encuentras el mensaje de error "El nombre de host está asociado con una zona retenida, por favor contacta al propietario para que elimine la retención" al añadir un dominio personalizado, significa que el dominio ya está en la zona de Cloudflare y está configurado con el estado "Zone Hold". Consulta esta [documentación de Cloudflare](https://developers.cloudflare.com/fundamentals/setup/account/account-security/zone-holds/) para más información.
...
Puedes localizar los endpoints de autenticación en la URL well-known:
