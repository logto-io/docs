...
| ADMIN_DISABLE_LOCALHOST | N/A                                  | <code>string &#124; boolean &#124; number</code>         | Establécelo en `1` o `true` para deshabilitar el puerto de la Consola de Administración. Si `ADMIN_ENDPOINT` no está definido, deshabilitará completamente la Consola de Administración.                                                                                                                                        |
...
En este caso, probablemente querrás establecer `TRUST_PROXY_HEADER` en `true`, lo que indica si se deben confiar los campos de encabezado del proxy. Logto pasará el valor a la [configuración de la app Koa](https://github.com/koajs/koa/blob/master/docs/api/index.md#settings).
