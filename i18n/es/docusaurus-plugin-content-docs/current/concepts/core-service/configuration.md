...
| ADMIN_DISABLE_LOCALHOST    | N/A                                  | <code>string &#124; boolean &#124; number</code>         | Establécelo en `1` o `true` para deshabilitar el puerto de la Consola de Administración. Si `ADMIN_ENDPOINT` no está establecido, deshabilitará completamente la Consola de Administración.                                                                                                                                     |
...
Otra práctica común es tener un proxy HTTPS delante de Node (por ejemplo, Nginx).

En este caso, probablemente quieras establecer `TRUST_PROXY_HEADER` en `true`, lo que indica si se deben confiar los campos de encabezado del proxy. Logto pasará el valor a [Koa app settings](https://github.com/koajs/koa/blob/master/docs/api/index.md#settings).
