...
- 🚀 Formas de hacer que tu sistema de autorización sea escalable y fácil de mantener
...
- Este diseño reduce la necesidad de permisos adicionales de rol mientras mantiene la seguridad
...
| --------------------------------- | ------------------------------------------------------------------------------------------- |
| GET /api/articles                 | - Cualquiera con el permiso `list:articles`, O los autores pueden ver sus propios artículos |
| GET /api/articles/:id             | - Cualquiera con el permiso `read:articles`, O el autor del artículo                        |
| POST /api/articles                | - Cualquiera con el permiso `create:articles`                                               |
| PATCH /api/articles/:id           | - Cualquiera con el permiso `update:articles`, O el autor del artículo                      |
| DELETE /api/articles/:id          | - Cualquiera con el permiso `delete:articles`, O el autor del artículo                      |
...
- **Publisher**: Se enfoca en la revisión de contenido y la gestión de publicaciones
...
:::note
Para fines demostrativos, creamos estos recursos y configuraciones a través de Logto Console. En proyectos reales, puedes crear estos recursos y configuraciones programáticamente usando la [Management API](/integrate-logto/interact-with-management-api) proporcionada por Logto.
:::
...
Primero, necesitamos añadir un middleware en el backend para verificar los permisos del usuario, comprobar si el usuario ha iniciado sesión y determinar si tiene los permisos necesarios para acceder a ciertas APIs.
...
Como puedes ver, en este middleware, verificamos si la solicitud del frontend contiene un token de acceso válido y comprobamos si la audiencia del token de acceso coincide con el recurso de API que creamos en Logto Console.
...
Dado que este recurso de API representa los recursos del CMS en Logto, en nuestro código frontend, incluimos el correspondiente token de acceso al hacer solicitudes de API al backend:
        ...
        try {
          ...
          const response = await fetch(`${API_BASE_URL}${endpoint}`, {
            ...
            headers: {
              ...
              // Añadir el token de acceso a los headers de la solicitud
...
## Probar la implementación de RBAC en el CMS \{#test-the-cms-rbac-implementation}

Ahora, probemos nuestra implementación de RBAC en el CMS usando los tres usuarios que acabamos de crear.

:::note
Si ves que no puedes iniciar sesión con las credenciales de los usuarios creados en "User Management", primero deberás habilitar el método de inicio de sesión adecuado. Ve a "Sign-in Experience" en Logto Console y habilita tu método de autenticación preferido (como Email + Password o Username + Password).
:::
...
Para escenarios más complejos, como construir aplicaciones multi-tenant, Logto proporciona soporte completo para organizaciones. Consulta nuestra guía [Construye una aplicación SaaS multi-tenant: Una guía completa desde el diseño hasta la implementación](https://blog.logto.io/build-multi-tenant-saas) para aprender más sobre cómo implementar control de acceso a nivel organizacional.
