...
¿Cómo se construyen aplicaciones como Notion, Slack o Figma? Estas aplicaciones SaaS multi-inquilino parecen sencillas de usar, pero ¿construir una tú mismo? Eso es otra historia.
...
¡Simplemente **construí un sistema con todas estas características con sorprendentemente poco esfuerzo!**
...
DocuMind es un producto SaaS de documentación con IA diseñado con un modelo multi-inquilino para soportar usuarios individuales, pequeñas empresas y grandes corporaciones.
...
En una arquitectura multi-inquilino, los usuarios necesitan **roles (Roles)** con **permisos (Permissions)** específicos para acceder a los recursos de su inquilino.
...
1. **Inicio de sesión con correo electrónico y contraseña**: Método tradicional con correo electrónico y contraseña.
2. **Inicio de sesión sin contraseña**: Usa códigos de verificación por correo electrónico para un acceso fácil y seguro.
3. **Gestión de cuenta**: Un centro de cuentas donde los usuarios pueden actualizar su correo, contraseña y otros detalles.
4. **Inicio de sesión social**: Opciones como Google y GitHub para un acceso rápido.
5. **Autenticación multifactor (MFA)**: Mejora la seguridad permitiendo el inicio de sesión mediante apps autenticadoras como Duo.

### Creación de inquilinos e invitaciones \{#tenant-creation-and-invitation}
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Nueva cuenta                                          | Entra desde la página de inicio de sesión / registro para crear un nuevo inquilino |
| Cuenta existente                                      | Crea otro inquilino dentro del producto                                            |
| Cuenta existente recibió invitación a nuevo inquilino | Entra desde la página de inicio de sesión / registro                               |
| Cuenta existente recibió invitación a nuevo inquilino | Entra desde el correo de invitación                                                |
| Nueva cuenta recibió invitación a nuevo inquilino     | Entra desde la página de inicio de sesión / registro                               |
| Nueva cuenta recibió invitación a nuevo inquilino     | Entra desde el correo de invitación                                                |

Estos son escenarios comunes que se encuentran en casi todas las aplicaciones SaaS. Úsalos como referencia para inspirar a tu equipo de producto y diseño, y siéntete libre de crear tus propios flujos según sea necesario.
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Un usuario existente se une a través del correo"
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Un nuevo usuario se une a través del correo"
...
La autenticación parece intimidante. Los usuarios necesitan:
...
- Inicio de sesión para todo el equipo en clientes empresariales
- ...

Implementar solo estas características básicas podría tomar semanas de desarrollo.

Pero ahora, **¡no necesitamos construir NADA de esto nosotros mismos!**

Los proveedores de autenticación modernos (esta vez elegiré [Logto](https://logto.io/)) ya han empaquetado todas estas funciones para nosotros. El flujo de autenticación es sencillo:

```mermaid
sequenceDiagram
  actor u as Usuario
  participant a as App
  participant auth as ProveedorAuth

  u->>a: Clic en "Iniciar sesión"
  a->>auth: Redirige a la página de autenticación
  auth->>u: Muestra la página de inicio de sesión
  u->>auth: Ingresa credenciales
  auth->>a: Redirige con token
  a->>u: Inicio de sesión exitoso
```

**¡De semanas de desarrollo a 15 minutos de configuración!** Logto maneja todos los flujos complejos por nosotros. Cubriremos los pasos de integración en la sección de implementación más adelante. ¡Ahora podemos enfocarnos en construir las funciones principales de DocuMind!

### Establecer la arquitectura multi-inquilino \{#establish-multi-tenant-architecture}

El sistema de organizaciones permite a los usuarios crear y unirse a múltiples organizaciones. Entendamos las relaciones principales:

```mermaid
erDiagram
    Usuario ||--o{ MiembroOrganizacion : "pertenece a"
    Organizacion ||--o{ MiembroOrganizacion : "tiene"
...
### Habilitar control de acceso en la app multi-inquilino \{#enable-access-control-in-multi-tenant-app}
...
2. **Plantillas de organización**: Combinaciones predefinidas de roles y permisos mediante plantillas de organización, simplificando la inicialización de organizaciones.

La relación de permisos se ve así:

```mermaid
graph TD
    subgraph plantilla_organizacion[PlantillaOrganizacion]
        subgraph rol_2[Rol2]
            permiso_c[Permiso C]
            permiso_d[Permiso D]
        end
    ...
    style plantilla_organizacion fill:#e6ffe6
...
Elegí un stack amigable para principiantes y portable:
...
Primero, inicia sesión en [Logto Cloud](https://cloud.logto.io/). Puedes registrarte gratis si no tienes una cuenta. Crea un Tenant de Desarrollo para pruebas.

En la Consola del Tenant, haz clic en el botón "Application" a la izquierda. Luego selecciona React para comenzar a construir nuestra aplicación.
...
function AppContent() {
  ...
  if (!isAuthenticated) {
    // Muestra la página de bienvenida para usuarios no autenticados
    return <Landing />;
  }

  // Muestra la app principal para usuarios autenticados
  return (
    <Routes>
      {/* El panel muestra todas las organizaciones disponibles */}
      <Route path="/" element={<Dashboard />} />

      {/* Página de organización tras hacer clic en una organización en el panel */}
...
Puedes establecer por defecto la página de registro cuando tu producto espera muchos usuarios nuevos.
...
Y llama a la función `signOut` del hook `useLogto` para cerrar sesión cuando lo desees.
...
Con Logto, habilitar MFA es sencillo. Solo haz clic en el botón "Multi-factor auth" en la Consola de Logto. Luego actívalo en la página de autenticación multifactor.
...
<div className="twoColumn">
  ...
  />
  <img
    alt="Escanear código QR en app autenticadora"
...
## Añadiendo experiencia multi-inquilino de organización \{#adding-multi-tenant-organization-experience}
...
```mermaid
graph TD
    T[Logto tenant] --> OrgA[Organización A]
...
Declara el acceso a la información de organización en el Logto Config. Esto se hace estableciendo los `scopes` y `resources` apropiados.
...
Después de completar estos pasos, necesitas cerrar sesión e iniciar sesión de nuevo. Esto es necesario porque modificamos el scope y resource solicitados.
...
Gracias a Logto, no necesitamos construir relaciones complejas de organizaciones. Solo necesitamos crear una organización en Logto y añadir usuarios a ella. Logto se encarga de toda la complejidad por nosotros. Hay dos formas de crear Organizaciones:

1. Crear organizaciones manualmente a través de la Consola de Logto
2. Usar la Management API de Logto para crear organizaciones, especialmente cuando diseñas un flujo SaaS que permite a los usuarios crear sus propias organizaciones (espacios de trabajo).
...
Ve a la página de detalles de la organización. Cambia a la pestaña Members. Haz clic en el botón "+ Add member". Selecciona tu usuario de inicio de sesión de la lista de la izquierda. Haz clic en el botón "Add members" en la parte inferior derecha. Ahora has añadido correctamente el usuario a esta organización.
...
Crear una organización en la consola no es suficiente. Tu app SaaS necesita un flujo que permita a los usuarios finales crear y gestionar fácilmente sus propios espacios de trabajo. Para implementar esta funcionalidad, usa la Management API de Logto.

Para orientación, consulta la documentación [Interactúa con la Management API](/integrate-logto/interact-with-management-api) para configurar la comunicación API con Logto.

### Entender el flujo de interacción de autenticación de organización \{#understand-organization-auth-interaction-flow}

Tomemos como ejemplo el flujo de creación de organización. Así es como funciona el proceso de creación de organización:
...
sequenceDiagram
  ...
  C-->>B: Respuesta de éxito
  B-->>A: Muestra mensaje de éxito
```
...
1. **Proteger la API del servicio backend**:
   ...
   - Garantiza que solo usuarios autenticados puedan acceder a nuestros servicios
...
En el concepto de Logto (y OAuth 2.0), nuestro servicio backend actúa como un resource server. Los usuarios acceden al resource server de DocuMind con un Access token desde el frontend. El resource server verifica este token. Si es válido, devuelve los recursos solicitados.

Vamos a crear un API Resource para representar nuestro servicio backend.
...
3. Rellena "DocuMind API" como nombre del API. Usa "[https://api.documind.com](https://api.documind.com/)" como identificador del API.
4. Haz clic en crear.
...
Verás un tutorial para usar el API resource. Puedes seguir ese tutorial o nuestros pasos a continuación.

Vamos a crear un middleware requireAuth para proteger nuestro endpoint POST /organizations.
...
En el código frontend, declara este API resource en el config de Logto. Añade su identificador al array de resources.
...
const config: LogtoConfig = {
  ...
  resources: [ReservedResource.Organization, "<https://api.documind.com>"], // Identificador del nuevo API resource
};
...
Al igual que las peticiones frontend al servicio backend, las peticiones del servicio backend a Logto necesitan Access tokens.

En Logto, usamos autenticación máquina a máquina para los access tokens. Consulta [Interactúa con la Management API](/integrate-logto/interact-with-management-api).

Ve a la página de aplicaciones en la Consola de Logto. Crea una aplicación máquina a máquina. Asigna el rol "Logto Management API access". Copia el endpoint de Token, App ID y App Secret. Los usaremos para los access tokens.
...
- `POST /api/organizations`: Crear organización (ver: [Referencia API de crear organización](https://openapi.logto.io/operation/operation-createorganization))
- `POST /api/organizations/{id}/users`: Añadir usuarios a la organización (ver: [Referencia API de añadir usuarios a organización](https://openapi.logto.io/operation/operation-addusers))
...
El siguiente paso sería invitar usuarios a una organización. No implementaremos esta función en nuestro tutorial aún. Ya sabes cómo usar la Management API. Puedes consultar este [**Tenant creation and invitation**](https://blog.logto.io/build-multi-tenant-saas-application#tenant-creation-and-invitation) como referencia de diseño de producto y fácilmente implementar esta función siguiendo este post: [Cómo implementamos colaboración de usuarios en una app multi-inquilino](https://blog.logto.io/implement-user-collaboration-in-your-app).

## Implementa control de acceso en tu app multi-inquilino \{#implement-access-control-to-your-multi-tenant-app}

Ahora pasemos al control de acceso de organizaciones.

Queremos lograr:

- Los usuarios solo pueden acceder a recursos que pertenecen a sus propias organizaciones: Esto se puede hacer mediante el `token de organización` de Logto
...
Antes de usar `getOrganization` o cualquier función de organización, debemos asegurarnos de que el scope `urn:logto:scope:organizations` y el resource `urn:logto:resource:organization` estén incluidos en el config de Logto. Como ya los declaramos antes, no lo repetiremos.
...
A continuación, creamos una API `GET /documents` en nuestro servicio backend. Similar a cómo usamos el API resource para proteger la API `POST /organizations`, usamos indicadores de recurso específicos de organización para proteger la API `GET /documents`.
...
Ya hemos implementado el uso de tokens de organización para acceder a recursos de organización. Ahora, implementaremos el control de permisos de usuario dentro de las organizaciones usando RBAC.
...
Admin tiene ambos permisos `read:documents` y `create:documents`, mientras que Colaborador solo tiene el permiso `read:documents`.
...
Una plantilla de organización es un modelo del sistema de control de acceso para cada organización: define los roles y permisos que se aplican a todas las organizaciones.
...
Vamos a la Consola de Logto > Organization Templates > Organization permissions y creamos dos permisos: `read:documents` y `create:documents`.
...
Así, hemos creado un modelo de permisos RBAC para cada organización.
...
Puedes lograr estos pasos vía la Management API de Logto:
...
Así, hemos implementado el control de permisos de usuario comprobando los permisos del usuario.
...
## Añade más funciones multi-inquilino a tu app \{#add-more-multi-tenant-app-features}

Hasta ahora, hemos implementado las funciones básicas de usuario y organización en un sistema SaaS multi-inquilino. Sin embargo, aún hay algunas funciones que no hemos cubierto, como personalizar la marca de la página de inicio de sesión para cada Organización, añadir automáticamente usuarios con correos de dominio específico a ciertas organizaciones, e integrar funcionalidad SSO a nivel empresarial.
...
- [Branding a nivel de organización](/customization/match-your-brand#organization-specific-branding)
- [MFA a nivel de organización](/organizations/organization-management#require-mfa-for-organization-members)
- [Gestión a nivel de organización](/end-user-flows/organization-experience/organization-management)

## Resumen \{#summary}

¿Recuerdas lo abrumador que parecía al principio? Usuarios, organizaciones, permisos, funciones empresariales... parecía una montaña interminable.
...
El código fuente completo de este tutorial está disponible en: [Multi-tenant SaaS Sample](https://github.com/logto-io/multi-tenant-saas-sample).

Este es el poder del desarrollo moderno en 2025: podemos enfocarnos en construir funciones únicas de producto en vez de luchar con la infraestructura. ¡Ahora es tu turno de construir algo increíble!
