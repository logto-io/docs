...
¿Cómo se construyen apps como Notion, Slack o Figma? Estas aplicaciones SaaS multi-inquilino parecen simples de usar, pero ¿construir una tú mismo? Eso es otra historia.
...
Es una característica ampliamente utilizada en los proveedores de autenticación. Una organización en un sistema de gestión de identidad corresponde al espacio de trabajo, proyecto o inquilino de tu app SaaS.
...
Por ejemplo, Sarah se registra en tu app usando su correo, **sarah@gmail.com**. Ella puede pertenecer a diferentes espacios de trabajo. Si Sarah es parte de **Workspace A** pero no de **Workspace B**, se considera miembro de **Workspace A** pero no de **Workspace B**.
...
2. El primer usuario que crea un espacio de trabajo recibe automáticamente el rol de **admin**.
...
1. **Inicio de sesión con correo electrónico y contraseña**: Método tradicional con correo y contraseña.
2. **Inicio de sesión sin contraseña**: Usa códigos de verificación por correo para un acceso fácil y seguro.
3. **Gestión de cuenta**: Un centro de cuenta donde los usuarios pueden actualizar su correo, contraseña y otros detalles.
...
### Creación de inquilino e invitación \{#tenant-creation-and-invitation}

En una app SaaS multi-inquilino, una diferencia clave en el flujo de usuario es la necesidad de soportar la creación de inquilinos e invitaciones de miembros. Este proceso requiere una planificación y ejecución cuidadosa, ya que juega un papel clave en la activación y el crecimiento del producto.
...
Aquí tienes algunos escenarios comunes que se encuentran en casi todas las apps SaaS. Úsalos como referencia para inspirar a tu equipo de producto y diseño, y siéntete libre de crear tus propios flujos según sea necesario.
...
El sistema de organizaciones permite a los usuarios crear y unirse a múltiples organizaciones. Veamos las relaciones principales:
...
En una app multi-inquilino, el diseño de permisos y roles suele ser consistente, ya que proviene del diseño del producto. Por ejemplo, en múltiples espacios de trabajo, normalmente hay un rol de admin y un rol de miembro. Logto como proveedor de autenticación tiene el siguiente diseño de control de acceso basado en roles a nivel de organización:
...
2. **Plantillas de organización**: Combinaciones predefinidas de roles y permisos a través de plantillas de organización, simplificando la inicialización de la organización.

La relación de permisos se ve así:

```mermaid
graph TD
    subgraph organization_template[PlantillaOrganizacion]
        subgraph role_2[Rol2]
            permission_c[Permiso C]
            permission_d[Permiso D]
        end

        subgraph role_1[Rol1]
            permission_a[Permiso A]
            permission_b[Permiso B]
        end
    end
...
En la Consola de Logto, haz clic en "Sign-in & account" en el menú de la izquierda. Luego haz clic en la pestaña "Sign-up and sign-in".
...
```mermaid
graph TD
    T[Tenant Logto] --> OrgA[Organización A]
...
Después de completar estos pasos, necesitas cerrar sesión e iniciar sesión de nuevo. Esto es necesario porque modificamos el alcance y recurso solicitados.

Ahora mismo, no has creado ninguna organización. El usuario tampoco se ha unido a ninguna organización. El panel mostrará "Aún no tienes ninguna organización".
...
Ve a la página de detalles de la organización. Cambia a la pestaña Members. Haz clic en el botón "+ Add member". Selecciona tu usuario de inicio de sesión de la lista de la izquierda. Haz clic en el botón "Add members" en la esquina inferior derecha. Ahora has añadido exitosamente el usuario a esta organización.
...
## Implementar experiencia de creación de organización self-service \{#implement-self-serve-organization-creation-experience}

Crear una organización en la consola no es suficiente. Tu app SaaS necesita un flujo que permita a los usuarios finales crear y gestionar fácilmente sus propios espacios de trabajo. Para implementar esta funcionalidad, utiliza la Management API de Logto.
...
app.post('/organizations', async (req, res) => {
  // Implementación usando Logto Management API
  // ...
});
```
...
const requireAuth = (resource) => {
  if (!resource) {
    throw new Error('El parámetro Resource es requerido para la autenticación');
  }
  ...
  return async (req, res, next) => {
    try {
      ...
      // Añadir info de usuario a la request
      req.user = {
        id: payload.sub,
      };
...
Obtén estas variables del endpoint OpenID Configuration de tu tenant de Logto. Visita `https://<your-tenant-id>.logto.app/oidc/.well-known/openid-configuration`. Encontrarás la información necesaria en el JSON devuelto:
...
Esto protege nuestro endpoint POST /organizations. Solo los usuarios con access tokens válidos de Logto pueden acceder.
...
Al igual que las peticiones frontend al servicio backend, las peticiones del backend a Logto necesitan Access tokens.
...
- `POST /api/organizations/{id}/users`: Añadir usuarios a la organización (ver: [Referencia API Añadir usuarios](https://openapi.logto.io/operation/operation-addusers))
...
Similar al access token de Logto mencionado antes, Logto emite un access token correspondiente a un recurso específico, y los usuarios usan este token para acceder a recursos protegidos en el servicio backend. Correspondientemente, Logto emite un token de organización correspondiente a una organización específica, y los usuarios usan este token para acceder a recursos protegidos de la organización en el servicio backend.
...
2. Al solicitar recursos de organización, usamos el token de organización en lugar del access token porque para recursos que pertenecen a una organización, queremos usar el control de permisos de organización en vez del control de permisos de usuario (entenderás mejor esto cuando implementemos la API `GET /documents` más adelante).
...
const requireOrganizationAccess = () => {
  return async (req, res, next) => {
    try {
      ...
      if (!aud) {
        throw new Error('Falta audiencia en el token');
      }
      ...
      // Extraer organization ID del claim de audiencia
      const organizationId = extractOrganizationId(payload.aud);

      // Añadir info de organización a la request
      req.user = {
        id: payload.sub,
        organizationId,
      };
...
app.get('/documents', requireOrganizationAccess(), async (req, res) => {
  // Puedes obtener el id de usuario y organizationId actual a través de req.user
...
Algunos softwares requieren aislamiento de datos entre organizaciones. Para más discusión e implementación, puedes consultar el post: [Implementación de multi-inquilinato con PostgreSQL: Aprende con un ejemplo real sencillo](https://blog.logto.io/implement-multi-tenancy).
...
Los Admin pueden crear y acceder a documentos, mientras que los Colaboradores solo pueden acceder a documentos.
...
Luego crea una API POST /documents, y usa el middleware `requireOrganizationAccess` con la configuración requiredScopes para proteger esta API y la anterior `GET /documents`.
...
Así, hemos implementado el control de permisos de usuario verificando los permisos del usuario.
...
¿Y lo mejor? No tuvimos que reinventar la rueda. Aprovechando herramientas modernas como Logto, transformamos lo que podrían haber sido meses de desarrollo en cuestión de minutos.
...
Este es el poder del desarrollo moderno en 2025: podemos centrarnos en construir funciones únicas de producto en vez de luchar con la infraestructura. ¡Ahora es tu turno de construir algo increíble!
