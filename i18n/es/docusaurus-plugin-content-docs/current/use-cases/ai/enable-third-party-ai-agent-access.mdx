...
Esta guía te guía paso a paso para integrar Logto con tu servicio y habilitar que agentes de IA accedan a él.
...
- Cómo funciona: WeatherBot (agente de IA de terceros) quiere acceder a los datos meteorológicos del usuario a través de tu servicio. Cuando Alice inicia sesión en WeatherBot usando su cuenta:
  ...
  - Solo los datos a los que ella consiente son compartidos con WeatherBot, y WeatherBot no puede acceder a ningún dato adicional sin un nuevo consentimiento explícito.

Este control de acceso (permiso) garantiza la seguridad de los datos del usuario; aunque tu servicio gestione todos los datos, los agentes de IA de terceros como WeatherBot solo pueden acceder a lo que el usuario ha permitido explícitamente. No pueden saltarse este proceso, ya que está reforzado por tu implementación de control de acceso en el servicio.
...
- Un tenant de [Logto Cloud](https://cloud.logto.io) (o autoalojado)
...
- **Servicio**: El servicio que quieres exponer al agente de IA.
- **Agente de IA**: El agente de IA que accederá a tu servicio.
- **Logto**: Funciona como el proveedor OpenID Connect (servidor de autorización) y gestiona las identidades de usuario.
...
## Configurar agente de IA de terceros \{#set-up-third-party-ai-agent}
...
3. **Implementar el flujo de autorización**: El agente de IA debe poder manejar el flujo de autorización OAuth 2.0 para obtener un token de acceso (Access token) de Logto.
...
## Configurar la autorización en tu servicio \{#set-up-authorization-in-your-service}
...
2. **Implementar lógica de control de acceso**: En tu servicio, implementa la lógica para validar el token de acceso recibido del agente de IA y verifica si el usuario tiene los permisos necesarios para acceder al recurso solicitado.
...
3. En el agente de IA, invoca el endpoint de la API para acceder a tu servicio.
