...
El token es válido por un período de tiempo limitado y está asociado a una dirección de correo electrónico del usuario final.

A veces puedes querer invitar a nuevos usuarios a tu aplicación / organización sin requerir que creen una cuenta primero. En tales casos, la aplicación puede enviar un "magic link" a tu correo electrónico. Y serás autenticado inmediatamente al hacer clic en el enlace.

Los desarrolladores de aplicaciones pueden usar el token de un solo uso para componer un magic link y enviarlo al correo electrónico del usuario final.

## Casos de uso \{#use-cases}

Logto admite los siguientes escenarios con magic links:

- **Registro solo por invitación**: Para herramientas internas o productos de IA en fase de pruebas, puedes desactivar el registro público e invitar a usuarios específicos mediante magic links.
...
- **Restablecimiento de contraseña**: Envía un magic link de restablecimiento de contraseña desde tu propia aplicación para que el usuario pueda verificar el token de un solo uso y establecer una nueva contraseña en Logto.

Por ejemplo, cuando hayas desactivado el registro público, puedes enviar un magic link con un token de un solo uso (por ejemplo, `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`) al correo electrónico del usuario para invitarlo a completar la creación de la cuenta. Puedes personalizar la plantilla de correo electrónico en tu propio servicio de envío de correos, como:
...
- Usar número de teléfono o nombre de usuario como identificador.
...
sequenceDiagram
  ...
  U ->> C: El usuario hace clic en el magic link y es redirigido a una landing page de tu aplicación
...
Logto ofrece un conjunto de Management APIs y Experience APIs para facilitar la implementación de tu magic link.

Antes de comenzar, asegúrate de tener una instancia de Logto lista y haber establecido la conexión Máquina a Máquina entre el servidor de tu aplicación
y el endpoint de Logto (requerido para las Management APIs). Aprende más sobre [Logto Management API](/integrate-logto/interact-with-management-api).
...
Para un magic link de restablecimiento de contraseña, restringe el token al flujo de "olvidé mi contraseña":
...
Para magic links de inicio de sesión o registro, el enlace debe contener al menos el token y el correo electrónico del usuario como parámetros.
Para magic links de restablecimiento de contraseña, el parámetro de correo electrónico es opcional; si lo omites, Logto pedirá al usuario que ingrese su correo electrónico antes de verificar el token de un solo uso.
El magic link debe dirigir a una landing page en tu propia aplicación.
...
### Paso 3: Disparar el flujo de autenticación vía Logto SDK \{#step-3-trigger-the-authentication-flow-via-logto-sdk}

#### Inicio de sesión o registro \{#sign-in-or-sign-up}

Después de que el usuario final haga clic en el magic link y navegue a tu aplicación, puedes extraer los parámetros `token` y `email` de la URL, y luego llamar a la función `signIn()` de Logto SDK para disparar el flujo de autenticación.
...
Para magic links de restablecimiento de contraseña, inicia una solicitud de autenticación con `first_screen` establecido en `reset_password`.
Pasa el token de un solo uso a través de `one_time_token`.
Si ya tienes el correo electrónico del usuario desde tu landing page, pásalo a través de `login_hint`; de lo contrario, omite `login_hint` y Logto pedirá al usuario que ingrese su correo electrónico antes de verificar el token.
...
Si un usuario ya ha iniciado sesión, llamar a la función `signIn()` del SDK limpiará automáticamente todos los tokens en caché (Token de ID, Token de acceso y Token de actualización) del almacenamiento del cliente,
...
El flujo de autenticación con magic link no admite la asignación de roles a los usuarios. Pero siempre puedes usar los [Webhooks](/developers/webhooks) y la [Management API](/user-management/manage-users#manage-roles-of-users) para actualizar los roles del usuario después de que se registre.
...
### Si desactivo el registro de usuarios en "Registro e inicio de sesión", ¿puedo seguir usando magic link para invitar usuarios? \{#if-i-disable-user-registration-in-sign-in-experience-can-i-still-use-magic-link-to-invite-users}

</summary>

Sí, puedes seguir usando magic link para invitar usuarios incluso si desactivas el registro de usuarios en "Registro e inicio de sesión".
...
2. El usuario ya ha iniciado sesión y luego hace clic en un magic link que está asociado con una cuenta diferente. En este caso, Logto le pedirá al usuario continuar como la nueva cuenta o regresar a la aplicación con la cuenta actual.
   ...
   2. Si el usuario elige permanecer en la cuenta actual, Logto no verificará el token y regresará a la aplicación con la cuenta actual.
