...
A veces puedes querer invitar a nuevos usuarios a tu aplicación / organización sin requerir que creen una cuenta primero. En tales casos, la aplicación puede enviar un "magic link" a tu correo electrónico. Y serás autenticado inmediatamente cuando hagas clic en el enlace.
...
El magic link debe dirigir a una página de destino en tu propia aplicación.
...
Si ya tienes el correo electrónico del usuario desde tu página de destino, pásalo a través de `login_hint`; de lo contrario, omite `login_hint` y Logto pedirá al usuario que ingrese su correo electrónico antes de verificar el token.
...
3. Si tu prompt de inicio de sesión está configurado como "login" o contiene "login", Logto iniciará sesión automáticamente con la cuenta asociada al token de un solo uso sin solicitar cambio. Esto se debe a que el prompt "login" indica una intención explícita de autenticación, lo que tiene prioridad sobre la sesión actual.
