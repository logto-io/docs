...
- **Después del registro del usuario**: Utiliza la [Account API](/end-user-flows/account-settings/by-account-api) para implementar experiencias de autoservicio durante los flujos de onboarding, centros de cuentas o a lo largo del uso del producto para actualizar la información del perfil de usuario.

## Beneficios clave \{#key-benefits}

La función **Recopilar perfil de usuario** te permite reunir información adicional del usuario durante la experiencia de registro del usuario final. Recomendamos recopilar solo la información esencial requerida para tu producto para evitar flujos de registro largos que puedan afectar las tasas de conversión de usuarios.
...
- **Personalización flexible de campos**: Elige entre varios [tipos de campo](#field-types) incluyendo texto, número, fecha, casilla de verificación, lista desplegable (select), URL y validación por expresión regular para adaptarse a tus necesidades específicas de datos.
...
- **Configuraciones de campos integradas**: Utiliza campos de datos básicos preconfigurados para propiedades comunes de usuario con configuración plug-and-play. Aprovecha los campos compuestos (dirección, nombre completo) para recopilar datos estructurados de manera eficiente en un solo paso.
...
5. Prueba la experiencia de usuario con [Logto live preview](/customization/live-preview) o tu app de prueba. Ya sea que los usuarios creen una nueva cuenta mediante [identificador (correo electrónico / número de teléfono / nombre de usuario)](/end-user-flows/sign-up-and-sign-in/sign-up), [inicio de sesión social](/end-user-flows/sign-up-and-sign-in/social-sign-in) o [SSO empresarial](/end-user-flows/enterprise-sso), todos verán la página de "Cuéntanos sobre ti" durante el registro.
...
Los datos de usuario pueden verse en cada [perfil de usuario](/user-management/manage-users). Para una visión general de todas las categorías de datos de usuario, consulta la documentación de [Estructura de datos de usuario](/user-management/user-data). La función Recopilar perfil de usuario te permite recopilar tanto datos básicos de usuario como datos personalizados durante el registro.
...
| Nombre completo        | <code>user.profile.givenName</code><br /><code>user.profile.middleName</code><br /><code>user.profile.familyName</code>                                                                                                                                                                         | El nombre legal completo del usuario. Combina de forma flexible familyName, givenName y middleName según la configuración.                                                                                                          |
| Apodo                  | <code>user.profile.nickname</code>                                                                                                                                                                                                                                                              | Nombre casual o familiar para el usuario, que puede diferir de su nombre legal.                                                                                                                                                     |
| Fecha de nacimiento    | <code>user.profile.birthdate</code>                                                                                                                                                                                                                                                             | La fecha de nacimiento del usuario en un formato especificado (ej., "MM-dd-aaaa").                                                                                                                                                  |
| Género                 | <code>user.profile.gender</code>                                                                                                                                                                                                                                                                | El género autoidentificado del usuario (ej., "Femenino", "Masculino", "Prefiero no decirlo").                                                                                                                                       |
| Perfil                 | <code>user.profile.profile</code>                                                                                                                                                                                                                                                               | URL de la página de perfil legible del usuario (ej., perfil de red social).                                                                                                                                                         |
| Sitio web              | <code>user.profile.website</code>                                                                                                                                                                                                                                                               | URL del sitio web personal o blog del usuario.                                                                                                                                                                                      |
| Dirección              | <code>user.profile.address.formatted</code><br /><code>user.profile.address.streetAddress</code><br /><code>user.profile.address.region</code><br /><code>user.profile.address.zoneinfo</code><br /><code>user.profile.address.postalCode</code><br /><code>user.profile.address.country</code> | La dirección completa del usuario (ej., "123 Main St, Anytown, USA 12345"). Elige entre una sola línea (<code>address.formatted</code>) o varias líneas con componentes configurables (calle, ciudad, estado, código postal, país). |
...
Al crear campos personalizados, utiliza solo caracteres alfanuméricos para la clave de dato de usuario (ej., `customData.companySize`). Luego puedes configurar el [tipo de campo](#field-types) y personalizar propiedades adicionales para satisfacer tus necesidades específicas.
...
| Fecha                      | Formato de fecha (predefinido / personalizado) | Elige formatos predefinidos (MM/dd/aaaa, dd/MM/aaaa, aaaa-MM-dd). Cambiar el marcador de posición sobrescribe las interacciones predeterminadas. O usa patrones totalmente personalizables de <a href="https://date-fns.org/v2.30.0/docs/format" target="_blank" rel="noopener noreferrer">date-fns</a>. |
| Casilla de verificación    | Valor predeterminado                           | Marcado (Verdadero) / Desmarcado (Falso).                                                                                                                                                                                                                                                                |
| Lista desplegable (Select) | Lista de opciones (≥1)                         | Usa <code>valor:etiqueta</code> por línea. Si se omite la etiqueta, se muestra el valor.                                                                                                                                                                                                                 |
| URL                        | N/A                                            | N/A                                                                                                                                                                                                                                                                                                      |
...
- Coincidencia de patrón (Expresión regular, Fecha con formato, URL, pertenencia a opción Select).
- Estructura de URL (validación sintáctica básica, no comprobación de accesibilidad).

También aplica a los subcomponentes de los dos campos compuestos: `address` y `fullname`. Los componentes individuales también deben cumplir los mismos criterios de validación.
...
| PUT          | <code>/api/custom-profile-fields/:name</code>                | Actualizar campo por nombre y dataset completo.                          |
...
- Usa campos compuestos para datos estructurados (dirección, nombre completo) para consistencia y localización.
