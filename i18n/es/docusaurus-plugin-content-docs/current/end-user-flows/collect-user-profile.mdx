...
- **Después del registro del usuario**: Utiliza la [Account API](/end-user-flows/account-settings/by-account-api) para implementar experiencias de autoservicio durante los flujos de incorporación, centros de cuentas o a lo largo del recorrido de uso del producto para actualizar la información del perfil de usuario.
...
- **Experiencia de usuario optimizada**: Personaliza la visualización con etiquetas, descripciones, textos de ayuda y reglas de validación. Configura los campos como obligatorios u opcionales según las necesidades de tu negocio.
...
| Apodo                  | <code>user.profile.nickname</code>                                                                                                                                                                                                                                                              | Nombre casual o familiar del usuario, que puede diferir de su nombre legal.                                                                                                                                                          |
...
| Texto de ayuda  | Todos (excepto Checkbox) | Pista en línea visible cuando la etiqueta está flotando (texto de ejemplo / guía de formato).                                       |
...
| Fecha                      | Formato de fecha (predefinido / personalizado) | Elige formatos predefinidos (MM/dd/yyyy, dd/MM/yyyy, yyyy-MM-dd). Cambiar el texto de ayuda sobrescribe las interacciones predeterminadas. O usa patrones totalmente personalizables de <a href="https://date-fns.org/v2.30.0/docs/format" target="_blank" rel="noopener noreferrer">date-fns</a>. |
...
La validación de campos se realiza al enviar el usuario el formulario en el paso final del registro. Se aplican las siguientes comprobaciones:
...
Los campos personalizados dependen completamente de las etiquetas, textos de ayuda y descripciones que proporciones. Para la localización de campos personalizados, puedes gestionar las traducciones usando [Bring your UI](/customization/bring-your-ui) o [contactarnos](https://logto.io/contact) con tus comentarios.
...
| POST         | <code>/api/custom-profile-fields/properties/sie-order</code> | Actualizar el orden de los campos en Experiencia de inicio de sesión. |
...
- Proporciona ejemplos claros / textos de ayuda para campos sensibles al formato (expresión regular, fecha, rangos numéricos).
...
Sí, puedes cambiar el componente "País" a un campo "Lista desplegable (Selección única)" con opciones estandarizadas.
