...
- **Después del registro del usuario**: Usa la [Account API](/end-user-flows/account-settings/by-account-api) para implementar experiencias de autoservicio durante los flujos de incorporación, centros de cuentas o a lo largo del recorrido de uso del producto para actualizar la información del perfil de usuario.
...
- **Capturar datos de usuario completos**: Recopila cualquier [dato de usuario](/user-management/user-data) para fines comerciales o de cumplimiento, incluidos [propiedades estándar de usuario OIDC](#basic-user-data-fields) y [datos personalizados](#custom-user-data-fields).

- **Personalización flexible de campos**: Elige entre varios [tipos de campo](#field-types) incluyendo texto, número, fecha, casilla de verificación, lista desplegable (select), URL y validación por expresión regular para adaptarse a tus requisitos de datos específicos.
...
- **Configuraciones de campo integradas**: Usa campos de datos básicos preconfigurados para propiedades comunes de usuario con configuración plug-and-play. Aprovecha los campos compuestos (dirección, nombre completo) para recopilar datos estructurados de manera eficiente en un solo paso.
...
Los datos de usuario pueden verse en cada [perfil de usuario](/user-management/manage-users). Para una visión completa de todas las categorías de datos de usuario, consulta la documentación de [Estructura de datos de usuario](/user-management/user-data). La función Recopilar perfil de usuario te permite recopilar tanto datos básicos de usuario como datos personalizados durante el registro.

### Campos básicos de datos de usuario \{#basic-user-data-fields}

Los campos básicos de datos de usuario proporcionados por Logto se almacenan directamente en la propiedad `user` o `user.profile`. Estos campos vienen con valores de configuración predeterminados y traducciones i18n, listos para usarse inmediatamente después de su creación. También puedes personalizar los tipos de campo y parámetros según tus necesidades específicas.
...
| Fecha de nacimiento    | <code>user.profile.birthdate</code>                                                                                                                                                                                                                                                             | La fecha de nacimiento del usuario en un formato especificado (ejemplo: "MM-dd-yyyy").                                                                                                                                                  |
| Género                 | <code>user.profile.gender</code>                                                                                                                                                                                                                                                                | El género autoidentificado del usuario (ejemplo: "Femenino", "Masculino", "Prefiero no decirlo").                                                                                                                                       |
...
La validación de campos se realiza al enviar el usuario la información en el paso final del registro. Se aplican las siguientes comprobaciones:

- Presencia obligatoria (excepto Casilla de verificación).
- Límites de longitud (Texto).
- Rango numérico (Número).
- Coincidencia de patrón (Expresión regular, Fecha con formato, URL, pertenencia a opción seleccionada).
...
- Recopila solo los datos esenciales en el registro; pospone la información adicional para la posterior finalización del perfil.
...
No, no se eliminarán los datos existentes del usuario. Solo se eliminará el campo del formulario de registro en la experiencia del usuario final.
