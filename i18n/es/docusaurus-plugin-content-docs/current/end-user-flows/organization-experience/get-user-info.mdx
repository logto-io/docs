...
### 1. Decodificar el token de ID (ID token) \{#1-decode-the-id-token}
...
Para obtener la información más actualizada, utiliza el segundo enfoque a continuación, o llama a `clearAllTokens()` y reinicia un flujo de autenticación para obtener un nuevo token de ID (ID token).
...
### 2. Obtener información del usuario desde el endpoint `/oidc/me` \{#2-fetch-user-info-from-the-oidc-me-endpoint}
