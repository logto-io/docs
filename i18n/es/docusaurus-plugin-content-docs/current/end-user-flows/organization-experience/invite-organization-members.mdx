...
Antes de invitar a miembros a tu organización, necesitas crear roles de organización. Consulta la guía [Configurar organizaciones](https://docs.logto.io/docs/recipes/organizations/configuration/) para obtener instrucciones detalladas.
...
Esto se puede hacer fácilmente en la Consola de Administración de Logto. También puedes usar el Management API de Logto para crear roles de organización programáticamente.

## Configura tu conector de correo electrónico

Dado que las invitaciones se envían por correo electrónico, asegúrate de que tu conector de correo electrónico esté configurado correctamente. Para enviar invitaciones, necesitas configurar un nuevo tipo de uso de plantilla de correo electrónico introducido - `OrganizationInvitation`.
...
```json
{
  ...
  "content": "<p>Únete a mi organización a través de este <a href=\"{{link}}\" target=\"_blank\">enlace</a>.</p>",
...
Puedes consultar la guía [Configurar servicio de correo electrónico popular](https://docs.logto.io/docs/recipes/configure-connectors/email-connector/configure-popular-email-service/) para obtener más detalles sobre la configuración de conectores de correo electrónico.

## Manejar invitaciones con Logto Management API

nota

Si aún no has configurado el Management API de Logto, consulta [Interactuar con Management API](https://docs.logto.io/docs/recipes/interact-with-management-api/) para obtener detalles.
...
- `POST /api/organization-invitations/{id}/message` enviar la invitación de organización al invitado por correo electrónico.
  Nota: Esta carga útil de API admite una propiedad `link`, puedes componer tu enlace de invitación basado en el ID de invitación. Por ejemplo:
  ```json
  {
...
- `GET /api/organization-invitations` & `GET /api/organization-invitations/{id}` obtener todas tus invitaciones o una específica por ID.
  En tu página de destino, usa estas APIs para listar todas las invitaciones o detalles de una invitación que un usuario ha recibido.
...
Ten en cuenta que todas las APIs listadas arriba requieren un "token de organización" válido. Consulta esta [guía](https://docs.logto.io/docs/recipes/organizations/integration/#step-2-fetch-the-organization-token) para aprender cómo obtener el token de organización.

## Usa el control de acceso basado en roles de organización (RBAC) para gestionar permisos de usuario
...
#### Otorgar nuevos alcances que son recién introducidos en tu sistema de autenticación
...
- Si es degradado, puedes simplemente borrar el token de organización en caché y el SDK emitirá automáticamente uno nuevo con los alcances actualizados.
  ...
  Esto no requiere un proceso de re-inicio de sesión o re-consentimiento. Los nuevos tokens de organización se emitirán automáticamente por el SDK de Logto.
...
- Si se introduce un nuevo alcance en tu sistema de autenticación, activa un proceso de re-inicio de sesión o re-consentimiento para actualizar el token de organización del usuario. Tomemos el SDK de React como ejemplo:
  ...
  El código anterior activará una navegación de página a la pantalla de consentimiento y redirigirá automáticamente de vuelta a tu aplicación, con los alcances actualizados en el token de organización del usuario.
