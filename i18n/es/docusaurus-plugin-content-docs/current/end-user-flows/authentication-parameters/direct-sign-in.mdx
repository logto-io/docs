...
El inicio de sesión directo es un parámetro de autenticación específico de Logto que te permite iniciar sesión social o SSO empresarial directamente, omitiendo la página de inicio de sesión universal predeterminada de Logto.
...
sequenceDiagram
    ...
    app->>logto: Enviar solicitud de autenticación con el parámetro de inicio de sesión directo
    logto->>idp: Omitir la página de inicio de sesión e iniciar directamente el inicio de sesión Social o SSO Empresarial
    idp->>logto: Enviar el token de autenticación o aserción
    logto->>app: Redirigir al usuario de vuelta a la aplicación cliente con los datos de autenticación
```
...
2. Haz clic en el conector que deseas usar.
3. Ubica el nombre del proveedor de identidad en la parte superior de la página de configuración del conector.

![Nombre del IdP del conector social](./assets/direct-sign-in-social-name.png)
...
![ID del conector SSO empresarial](./assets/direct-sign-in-enterprise-sso.png)
