...
- `login_hint`: Rellena el campo de identificador con la dirección de correo electrónico o nombre de usuario del usuario. (Este es un parámetro estándar de OIDC)
...
- `identifier:sign-in`: Muestra un formulario de inicio de sesión específico por identificador. El tipo de identificador se puede especificar usando el parámetro `identifier` (opcional). Esto es útil cuando tienes varios métodos de inicio de sesión por identificador habilitados.
- `identifier:register`: Muestra un formulario de registro específico por identificador. El tipo de identificador se puede especificar usando el parámetro `identifier` (opcional). Esto es útil cuando tienes varios métodos de registro por identificador habilitados.
...
:::tip
La primera pantalla seguirá la configuración establecida en <CloudLink to="/sign-in-experience/branding">Consola > Inicio de sesión y cuenta</CloudLink>. Debes habilitar primero los métodos de autenticación requeridos y configurar la personalización de marca, los términos y políticas de privacidad, y la internacionalización (i18n). Ten en cuenta que solo las páginas de `sign-in` y `register` muestran el logo por defecto.
:::
