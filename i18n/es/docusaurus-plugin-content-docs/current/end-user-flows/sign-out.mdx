...
El proceso de cierre de sesión en Logto (como proveedor de identidad basado en OIDC) es un concepto multifacético debido a la participación tanto de la sesión centralizada de inicio de sesión gestionada por Logto como del estado de autenticación distribuido gestionado por las aplicaciones cliente.
...
```mermaid
  sequenceDiagram
    ...
    Usuario ->> Cliente: Accede a la aplicación web
    Cliente ->> OIDC: Redirige al usuario a OIDC para autenticación
    ...
    OIDC ->> Cliente: Redirige al usuario de vuelta a la aplicación web con el código de autenticación (Flujo de código de autorización)
    Cliente ->> OIDC: Intercambia el código por tokens
    OIDC -->> Cliente: Devuelve los tokens
```

1. El usuario accede a la aplicación web (RP).
2. La aplicación cliente redirige al usuario a Logto (IdP) para [autenticación](https://auth.wiki/authentication).
...
### Sesión centralizada de inicio de sesión gestionada por Logto \{#centralized-sign-in-session-managed-by-logto}

En el flujo anterior, la sesión centralizada de inicio de sesión es gestionada por Logto. La sesión se crea cuando el usuario inicia sesión con éxito y se destruye cuando el usuario cierra sesión. La sesión también se destruye cuando la sesión del usuario expira.
...
1. Cookie de sesión compartida de Logto  
   ...
   ```mermaid
    flowchart TD
    subgraph Usuario [Agente de usuario A]
      U[Usuario]

      subgraph Capa1 [Dominio cliente A]
        A[Aplicación cliente A]
      end

      subgraph Capa1 [Dominio cliente B]
        B[Aplicación cliente B]
      end

      subgraph Capa2 [Dominio Logto]
...
2. Cookie de sesión aislada de Logto  
   ...
   ```mermaid
    ...
    subgraph DispositivoA [Agente de usuario A]
      subgraph Capa1 [Dominio cliente A]
        A[Aplicación cliente A]
      end
    ...
    subgraph DispositivoB [Agente de usuario B]
      subgraph Capa3 [Dominio cliente B]
        B[Aplicación cliente B]
      end

      subgraph Capa4 [Dominio Logto]
...
El **endpoint de fin de sesión** es un endpoint estándar de OIDC que permite a la aplicación cliente notificar al servidor de autorización que el usuario ha cerrado sesión. El endpoint limpiará la sesión centralizada de inicio de sesión en Logto.
...
Si se proporciona un **post-logout redirect URI**, el usuario será redirigido a la URI especificada después de limpiar la sesión. De lo contrario, el usuario será redirigido a la página de cierre de sesión predeterminada alojada por Logto.
...
:::note
Para aplicaciones nativas que no admiten webview sin sesión o no reconocen la configuración `emphasized` (aplicación Android usando el SDK de **React Native** o **Flutter**), puedes forzar que se solicite al usuario iniciar sesión nuevamente pasando el parámetro `prompt=login` en la solicitud de autorización.
:::

## Forzar la re-autenticación en cada acceso \{#enforce-re-authentication-on-every-access}

En escenarios de alta seguridad—como verificar la identidad de un usuario antes de acciones sensibles—puedes requerir que el usuario se re-autentique en cada acceso. Para forzar este comportamiento, incluye `prompt=login` en tu solicitud de autenticación.

Configurar `prompt=login` obliga a Logto a mostrar siempre la página de inicio de sesión, independientemente de si el usuario tiene una sesión activa o ha iniciado sesión recientemente. Esto omite el comportamiento de inicio de sesión único (SSO) y asegura que se solicite al usuario ingresar sus credenciales cada vez.
...
En la mayoría de los casos, para forzar la re-autenticación y asegurar la emisión del Token de actualización, configura:
