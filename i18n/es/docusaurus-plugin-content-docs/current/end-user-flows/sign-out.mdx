...
3. El proveedor OIDC verifica el estado de la sesión de inicio de sesión del usuario. Si no existe sesión o la sesión ha expirado, se solicita al usuario que inicie sesión.
...
2. Cookie de sesión aislada de Logto
   Para un usuario que inicia sesión en diferentes aplicaciones cliente desde diferentes dispositivos o navegadores, el usuario tendrá cookies de sesión aisladas bajo el dominio de Logto. Esto significa que el usuario necesita iniciar sesión por separado para cada aplicación cliente.
...
- Aplicaciones Web:
  Las aplicaciones web, como aquellas construidas con frameworks como Next.js, a menudo gestionan su propia sesión para usuarios autenticados junto con los tokens emitidos por Logto. Una vez que el usuario inicia sesión y la aplicación web recibe los tokens de Logto, puede almacenar los tokens en el lado del cliente al igual que las aplicaciones SPA, o puede almacenar los tokens en el lado del servidor y gestionar la sesión utilizando cookies u otros mecanismos.
...
### Borrar sesión de inicio de sesión en Logto
...
## Cierre de sesión federado: Cierre de sesión por canal posterior

Para una gestión de cierre de sesión más consistente, Logto admite el [cierre de sesión por canal posterior](https://openid.net/specs/openid-connect-backchannel-1_0-final.html). El cierre de sesión por canal posterior es un mecanismo que permite a Logto notificar a todas las aplicaciones cliente bajo la misma sesión de inicio de sesión cuando el usuario cierra sesión.
...
Para habilitar el cierre de sesión por canal posterior para tus aplicaciones cliente, ve a la página de detalles de la aplicación en el panel de Logto y registra un URI de cierre de sesión por canal posterior. Logto enviará un token de cierre de sesión a todos los URI registrados cuando el usuario inicie una solicitud de cierre de sesión desde cualquier aplicación cliente.

Si tu aplicación cliente requiere que la sesión de inicio de sesión se incluya en el token de cierre de sesión, activa la configuración `¿Es necesaria la sesión?` en la configuración de cierre de sesión por canal posterior. Se incluirá un reclamo `sid` en el token de cierre de sesión para identificar la sesión de inicio de sesión del usuario en Logto.

1. El usuario inicia una solicitud de cierre de sesión desde una aplicación cliente.
2. Logto recibe la solicitud de fin de sesión, genera un token de cierre de sesión y envía el token de cierre de sesión a todos los URI de cierre de sesión por canal posterior registrados.
...
Si estás integrando Logto con tu aplicación cliente utilizando los SDKs de Logto:
...
<details>
  <summary>No estoy recibiendo las notificaciones de cierre de sesión por canal posterior.</summary>

- Asegúrate de que el URI de cierre de sesión por canal posterior esté correctamente registrado en el panel de Logto.
...
<Url href="https://blog.logto.io/oidc-back-channel-logout/">
  Comprendiendo el cierre de sesión por canal posterior de OIDC.
</Url>
