...
- **Vinculación de cuenta existente**:
  ...
  - **Vinculación manual de cuentas**: Si la opción “Vinculación automática de cuentas” está deshabilitada, se le pedirá al usuario que vincule su cuenta social a la cuenta existente durante el proceso de inicio de sesión. Puede elegir vincular las cuentas o crear una nueva.
    ...
    E -- Sí --> G[Vincula la identidad social a la cuenta existente]
    G --> C
...
3. **Registro con identidad social que proporciona una dirección de correo electrónico ya registrada**
   ...
   ```mermaid
      flowchart TD
       ...
       C -- Sí --> D[Vincula la identidad social a la cuenta existente]
...
Configura la recopilación de perfil a través de <CloudLink to="/sign-in-experience/collect-user-profile">Consola > Experiencia de inicio de sesión > Recopilar perfil de usuario</CloudLink> para elegir entre campos básicos preconfigurados o crear campos personalizados con validación flexible. Más información: [Recopilar perfil de usuario](/end-user-flows/collect-user-profile)
...
actor user as Usuario
    ...
    experience->>Logto: Solicita iniciar sesión usando la autenticación de Google
    Logto->>app: Autentica al usuario y redirige de vuelta a la aplicación
```
