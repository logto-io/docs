...
- **Enriquecimiento de datos de usuario**: El inicio de sesión social te permite recopilar información adicional del perfil de la plataforma social, como nombre, dirección de correo electrónico, foto de perfil y más.
...
sequenceDiagram
    ...
    social-->>experience: Autenticar usuario y redirigir de nuevo a la página de inicio de sesión de Logto
    experience->>Logto: Validar datos de autenticación social e identidad del usuario
    Logto->>app: Autenticar usuario y redirigir de nuevo a la aplicación
```
...
3. **Autenticación social**: El usuario ingresa sus credenciales sociales y se autentica con el proveedor social. Si el usuario ya ha iniciado sesión en el proveedor social, puede ser autenticado automáticamente. Si se detectan múltiples sesiones de inicio de sesión, se le puede pedir al usuario que elija la cuenta correcta (por ejemplo, múltiples cuentas de Google).
   ...
   :::
4. **Regreso a Logto**: Después de la autenticación exitosa, el proveedor social redirige al usuario de nuevo a la página de inicio de sesión de Logto con los datos de autenticación.
...
6. **Autenticación del usuario**: Logto autentica al usuario y lo redirige de nuevo a la aplicación cliente para completar el proceso de autenticación.
...
![habilitar google one tab en experiencia de inicio de sesión](./assets/social-sign-in-enable-google-one-tab.png)
...
### ¿Por qué mis correos electrónicos o números de teléfono no se completan después de registrarse con social? \{#why-my-emails-or-phone-numbers-are-not-populated-after-signing-up-with-social}
