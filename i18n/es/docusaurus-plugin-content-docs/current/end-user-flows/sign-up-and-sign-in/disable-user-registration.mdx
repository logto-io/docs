...
En algunas circunstancias, es posible que desees restringir el registro de usuarios para tu aplicación. Logto te permite desactivar el registro anónimo de usuarios y restringir el inicio de sesión solo a los usuarios invitados por un administrador.
...
2. En la sección **Opciones avanzadas**, desactiva el interruptor **Habilitar registro de usuarios** para desactivar el registro de usuarios. (Habilitado por defecto)

Una vez que el registro de usuarios esté desactivado:
...
- La función de vinculación de cuentas sociales sigue estando disponible para los usuarios que tengan una cuenta con la misma dirección de correo electrónico o número de teléfono en el sistema.
...
4. Crea y envía el **enlace mágico de invitación** [siguiendo la guía](/end-user-flows/one-time-token#implementation-guide) (solicita el token de un solo uso, compón el enlace mágico, activa la autenticación mediante el SDK de Logto).<br/>
   **Nota**: Establece un tiempo de expiración para el enlace de invitación. Se recomienda que el enlace sea válido por al menos un día. Utiliza el siguiente cuerpo de solicitud para generar el token de un solo uso:

   ```json
   {
     "email": "user@example.com",
     "expiresIn": 172800 // Opcional. Por defecto es 600 (10 minutos)
   }
   ```
...
2. Crea manualmente cuentas de usuario en la <CloudLink to="/users">Consola > Gestión de usuarios</CloudLink> o mediante la [Management API](https://openapi.logto.io/operation/operation-createuser).
