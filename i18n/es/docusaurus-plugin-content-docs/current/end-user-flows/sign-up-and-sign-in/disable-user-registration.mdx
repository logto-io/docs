...
- La función de vinculación de cuentas sociales sigue disponible para los usuarios que tengan una cuenta con la misma dirección de correo electrónico o número de teléfono en el sistema.

## Implementar un flujo de registro solo por invitación \{#implement-an-invitation-only-sign-up-flow}

Logto no proporciona un flujo de invitación integrado. Sin embargo, puedes implementar fácilmente un flujo de registro solo por invitación utilizando la Management API de usuarios en Logto.
...
3. Recoge las direcciones de correo electrónico de los usuarios que deseas invitar (por ejemplo, a través de tu sitio web o recomendaciones de usuarios existentes).
4. Crea y envía el **enlace mágico de invitación** [siguiendo la guía](/end-user-flows/one-time-token#implementation-guide) (solicita el token de un solo uso, compón el enlace mágico, activa la autenticación mediante Logto SDK).<br/>
...
2. Crea manualmente cuentas de usuario en la página <CloudLink to="/users">Consola > Gestión de usuarios</CloudLink> o mediante la [Management API](https://openapi.logto.io/operation/operation-createuser).
