...
2. **Clic en el enlace de Olvidé mi contraseña**: El usuario hace clic en el enlace "¿Olvidaste tu contraseña?".
...
Puedes crear un endpoint de restablecimiento de contraseña autohospedado y usar el SDK de Logto para iniciar una solicitud de inicio de sesión con [`first_screen`](/end-user-flows/authentication-parameters/first-screen) configurado como `reset_password`. Esto redirige al usuario a la página de restablecimiento de contraseña.

Para verificar al usuario con un enlace mágico, crea un [token de un solo uso](/end-user-flows/one-time-token) para el correo electrónico del usuario y pásalo a través del parámetro de autenticación `one_time_token`. Si incluyes `login_hint`, Logto verificará el token inmediatamente; de lo contrario, Logto pedirá al usuario que ingrese su correo electrónico antes de la verificación del token.
