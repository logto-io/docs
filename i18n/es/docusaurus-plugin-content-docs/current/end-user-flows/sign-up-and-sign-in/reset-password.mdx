...
## Olvidé la contraseña para recuperación de cuenta \{#forgot-password-for-account-recovery}

### Configuración \{#configuration}

Para habilitar la funcionalidad de "olvidé mi contraseña":

1. **Configura conectores**: Configura los [conectores de correo electrónico](/connectors/email-connectors) o [SMS](/connectors/sms-connectors) en <CloudLink to="/connectors/passwordless">Consola > Conectores > Conectores de correo electrónico y SMS</CloudLink>

2. **Recopila información de contacto del usuario**: Asegúrate de que los usuarios tengan correo electrónico / teléfono registrado durante el [registro](/end-user-flows/sign-up-and-sign-in/sign-up) o mediante [configuración de cuenta](/end-user-flows/account-settings/by-account-api#update-or-link-new-email)

3. **Habilita métodos de verificación**:

   - Ve a <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Consola > Inicio de sesión y cuenta > Registro e inicio de sesión</CloudLink>
...
4. **Guarda y prueba**: Guarda los cambios y prueba usando la [Vista previa en vivo](/customization/live-preview)

### Flujo de experiencia del usuario \{#user-experience-flow}

Una vez habilitada la función de restablecimiento de contraseña, se mostrará un botón de enlace "¿Olvidaste tu contraseña?" debajo del formulario de inicio de sesión. Los usuarios pueden hacer clic en el enlace "¿Olvidaste tu contraseña?" para iniciar el proceso de restablecimiento de contraseña.

1. **Visitar la página de inicio de sesión**: El usuario visita la página de inicio de sesión.
2. **Clic en el enlace Olvidé mi contraseña**: El usuario hace clic en el enlace "¿Olvidaste tu contraseña?".
3. **Ingresar correo electrónico / teléfono**: Después de hacer clic en el enlace, el usuario será redirigido a una nueva página donde podrá ingresar su dirección de correo electrónico o número de teléfono registrado.
...
6. **Ingresar nueva contraseña**: Se le pedirá al usuario que ingrese una nueva contraseña una vez que el código de verificación haya sido verificado correctamente.
...
Luego llama al endpoint de la Account API para establecer o actualizar la contraseña (consulta la guía de configuración de cuenta para detalles de la solicitud). Para los usuarios que nunca han tenido una contraseña, NO necesitas (y no debes requerir) el campo de contraseña anterior.
...
Suscríbete al evento de webhook `PostResetPassword` ([eventos de webhooks](/developers/webhooks/webhooks-events#user-interaction-hook-events)) para recibir una notificación cuando un usuario restablezca su contraseña con éxito. Luego puedes desencadenar una acción de [cerrar sesión](/end-user-flows/sign-out) para invalidar la sesión actual del usuario y redirigirlo a la página de inicio de sesión.
...
Puedes crear un endpoint de restablecimiento de contraseña autoalojado y usar el SDK de Logto para iniciar una solicitud de inicio de sesión con [`first_screen`](/end-user-flows/authentication-parameters/first-screen) establecido en `reset_password`. Esto redirige al usuario a la página de restablecimiento de contraseña.

Para verificar al usuario con un enlace mágico, crea un [token de un solo uso](/end-user-flows/one-time-token) para el correo electrónico del usuario y pásalo mediante el parámetro de autenticación `one_time_token`. Si incluyes `login_hint`, Logto verificará el token inmediatamente; de lo contrario, Logto pedirá al usuario que ingrese su dirección de correo electrónico antes de la verificación del token.

Aprende más en la [guía de restablecimiento de contraseña con token de un solo uso](/end-user-flows/one-time-token#reset-password).
