...
```mermaid
  graph TB
    ...
    B -->|Llamada API de configuración de cuenta autoalojada|C[Aplicación del lado del servidor]
...
Consulta la sección de [datos del usuario y datos personalizados](/user-management/user-data/) para aprender más sobre el esquema de usuario en Logto.
...
| PATCH  | [/api/users/\{userId\}/profile](https://openapi.logto.io/operation/operation-updateuserprofile)          | Actualizar campos del perfil del usuario por ID de usuario.    |
...
### Gestión de identidades sociales del usuario \{#user-social-identities-management}
...
### Gestión de identidades empresariales del usuario \{#user-enterprise-identities-management}
...
### Gestión de configuraciones de MFA del usuario \{#user-mfa-settings-management}
