...
Logto proporciona varias Management API para gestionar cuentas de usuario. Puedes usar estas APIs para construir una página de configuración de cuenta de autoservicio para los usuarios finales.
...
```mermaid
  graph TB
    ...
    B -->|Llamada a la API de configuración de cuenta autoalojada|C[Aplicación del lado del servidor]
...
5. La aplicación cliente solicita el token de acceso a Logto para acceder a la API de configuración de cuenta autoalojada.
...
8. La aplicación del lado del servidor verifica la identidad y permisos del solicitante a partir del token de acceso del usuario. Luego solicita un token de acceso de Management API a Logto.
...
| PATCH  | [/api/users/\{userId\}/profile](https://openapi.logto.io/operation/operation-updateuserprofile)          | Actualizar campos del perfil de usuario por ID.     |
...
| POST   | [/api/verifications/verification-code/verify](https://openapi.logto.io/operation/operation-verifyverificationbyverificationcode) | Verificar correo o teléfono mediante código de verificación. |
...
- Management API no tiene ningún contexto de sesión, cualquier conector social que requiera una sesión activa para mantener de forma segura el estado de autenticación social no puede vincularse mediante Management API. Los conectores no soportados incluyen apple, OIDC estándar y conector OAuth 2.0 estándar.
...
| method | path                                                                                                    | description                                                                                                                                                                                                       |
| ------ | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | [/api/users/\{userId\}?includeSsoIdentities=true](https://openapi.logto.io/operation/operation-getuser) | Obtener detalles del usuario por ID. Las identidades empresariales están en el campo `ssoIdentities`. Añade el parámetro de consulta `includeSsoIdentities=true` a la API de detalles de usuario para incluirlas. |
...
| POST   | [/api/users/\{userId\}/mfa-verifications](https://openapi.logto.io/operation/operation-createusermfaverification)                    | Configurar una verificación MFA para el usuario por ID. |
| DELETE | [/api/users/\{userId\}/mfa-verifications/\{verificationId\}](https://openapi.logto.io/operation/operation-deleteusermfaverification) | Eliminar una verificación MFA del usuario por ID.       |
