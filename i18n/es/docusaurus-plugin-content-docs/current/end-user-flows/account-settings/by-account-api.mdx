...
## ¿Qué es el Logto Account API? \{#what-is-logto-account-api}

El Logto Account API es un conjunto completo de APIs que brinda a los usuarios finales acceso directo a la API sin necesidad de pasar por el Management API. Aquí tienes los aspectos destacados:

- Acceso directo: El Account API permite a los usuarios finales acceder y gestionar directamente su propio perfil de cuenta sin requerir el uso del Management API.
...
Con el Logto Account API, puedes construir un sistema personalizado de gestión de cuentas, como una página de perfil totalmente integrada con Logto.
...
El endpoint de la API `/api/account-center` puede usarse para recuperar y actualizar la configuración del centro de cuentas; puedes usarlo para habilitar o deshabilitar el Account API y personalizar los campos.
...
El campo `enabled` se usa para habilitar o deshabilitar el Account API, y el campo `fields` se usa para personalizar los campos; el valor puede ser `Off`, `Edit`, `ReadOnly`. El valor predeterminado es `Off`. Lista de campos:
...
Después de configurar el SDK en tu aplicación, puedes usar el método `client.getAccessToken()` para obtener un token de acceso. Este token es un token opaco (opaque token) que puede usarse para acceder al Account API.
