...
La Account API de Logto es un conjunto completo de APIs que otorga a los usuarios finales acceso directo a la API sin necesidad de pasar por la Management API. Aquí tienes los aspectos más destacados:
...
Para obtener más información sobre las APIs disponibles, visita [Referencia de la Account API de Logto](https://openapi.logto.io/group/endpoint-my-account) y [Referencia de la Verification API de Logto](https://openapi.logto.io/group/endpoint-verifications).
...
Debes incluir el token de acceso en el campo `Authorization` de los encabezados HTTP con el formato Bearer (`Bearer TU_TOKEN`) al interactuar con la Account API.
...
Después de verificar el código, ahora puedes llamar a [`PATCH /api/my-account/primary-email`](https://openapi.logto.io/operation/operation-updateprimaryemail) para actualizar el correo del usuario, estableciendo el `verificationId` en el cuerpo de la solicitud como `newIdentifierVerificationRecordId`.
...
:::tip
Al igual que los correos electrónicos recogidos durante el registro, cualquier correo vinculado mediante la Account API debe pasar la verificación de [lista de bloqueo](/security/blocklist) que configuraste en <CloudLink to="/security/blocklist">Consola > Seguridad > Lista de bloqueo</CloudLink>. Logto rechazará la solicitud y devolverá un error detallado si el correo viola la política.
:::
...
El `connectorData` son los datos devueltos por el conector social después de que el usuario autoriza la aplicación; necesitas analizar y obtener los parámetros de consulta del `redirectUri` en tu página de callback y envolverlos como JSON como valor del campo `connectorData`.
...
Usa el secreto para generar un código QR o muéstralo directamente al usuario. El usuario debe añadirlo a su aplicación autenticadora (como Google Authenticator, Microsoft Authenticator o Authy).
...
Después de que el usuario haya añadido el secreto a su aplicación autenticadora, debe verificarlo y vincularlo a su cuenta. Usa el endpoint [`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification) para vincular el factor TOTP.
