...
Las funciones de visualización de cuentas SSO y eliminación de cuentas están disponibles actualmente a través de las Management APIs de Logto. Consulta [Configuración de cuenta mediante Management API](/end-user-flows/account-settings/by-management-api) para detalles de implementación.
...
Si no estás usando el SDK oficial, debes establecer el `resource` como vacío en la solicitud de concesión de token de acceso a `/oidc/token`.
...
- Un usuario solo puede tener un conjunto de códigos de respaldo a la vez. Si todos los códigos han sido usados, el usuario debe generar y vincular nuevos códigos.
