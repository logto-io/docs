...
- Autorización fluida: ¡La autorización es más fácil que nunca! Simplemente usa `client.getAccessToken()` para obtener un token de acceso opaco para OP (Logto), y adjúntalo al encabezado Authorization como `Bearer <access_token>`.
...
const config: LogtoConfig = {
  // ...otras opciones
  // Añade los alcances adecuados según tus casos de uso.
  scopes: [
...
Primero, necesitas obtener un **ID de registro de verificación** con una expiración de 10 minutos (TTL). Esto puede usarse para verificar la identidad del usuario antes de actualizar información sensible. Esto significa que una vez que un usuario verifica exitosamente su identidad mediante contraseña, código de verificación por correo electrónico o código de verificación por SMS, tiene 10 minutos para actualizar sus datos relacionados con la autenticación, incluyendo identificadores, credenciales, vinculación de cuentas sociales y MFA.
...
Para saber más sobre las verificaciones, consulta [Verificación de seguridad mediante Account API](/end-user-flows/security-verification).
...
Dado que tu aplicación front-end llama a la Account API desde un dominio diferente al de las páginas de autenticación de Logto, necesitas configurar **Orígenes Relacionados** para permitir operaciones de claves de acceso entre orígenes.
...
**Configura Orígenes Relacionados:**
