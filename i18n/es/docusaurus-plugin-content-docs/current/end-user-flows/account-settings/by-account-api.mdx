...
- Autorización sin complicaciones: ¡La autorización es más fácil que nunca! Simplemente usa `client.getAccessToken()` para obtener un token opaco de acceso para OP (Logto), y adjúntalo en el encabezado Authorization como `Bearer <access_token>`.
...
- Gestionar aplicaciones autorizadas del usuario (grants)
...
3. **Bóveda secreta**:
   - Para conectores sociales y empresariales OIDC u OAuth, la [bóveda secreta](/secret-vault/federated-token-set) de Logto almacena de forma segura los tokens de acceso y actualización de terceros después de la autenticación. Las aplicaciones pueden entonces llamar a APIs externas, como sincronizar eventos de Google Calendar, sin pedir a los usuarios que inicien sesión de nuevo. La recuperación de tokens estará disponible automáticamente una vez que la Account API esté habilitada.
4. **Gestión de sesiones**:
   - Cuando está habilitada, los usuarios pueden ver y gestionar sus sesiones activas, incluyendo información del dispositivo y la última hora de inicio de sesión. Los usuarios también pueden revocar sesiones para cerrar sesión en dispositivos específicos.
   - Este mismo permiso de campo `Sessions` también controla las APIs de aplicaciones autorizadas por el usuario (grants) (ver y revocar grants).
   - Antes de que los usuarios finales accedan a la gestión de sesiones, deben verificar su identidad mediante contraseña, correo electrónico o SMS para obtener un ID de registro de verificación válido por 10 minutos. Consulta [Obtener un ID de registro de verificación](#get-a-verification-record-id).
...
const config: LogtoConfig = {
  ...
  scopes: [
    ...
    UserScope.Sessions, // Para gestionar sesiones de usuario y grants de aplicaciones
  ],
};
```
...
:::tip
Al igual que las contraseñas creadas durante el registro, las contraseñas establecidas mediante la Account API deben cumplir con la [política de contraseñas](/security/password-policy) que configuraste en <CloudLink to="/security/password-policy">Consola > Seguridad > Política de contraseñas</CloudLink>. Logto devuelve resultados de validación detallados y mensajes de error si la contraseña no cumple la política.
:::
...
:::info[Dos IDs de registro de verificación diferentes]

Esta solicitud requiere dos IDs de registro de verificación separados:

- **`logto-verification-id` (header)**: Demuestra la identidad del usuario antes de realizar cambios sensibles. Obtén esto [verificando la contraseña del usuario](#verify-the-users-password) o [enviando un código de verificación al correo electrónico o teléfono existente del usuario](#verify-by-sending-a-verification-code-to-the-users-email-or-phone).
- **`newIdentifierVerificationRecordId` (body)**: Demuestra la propiedad del nuevo correo electrónico. Este es el `verificationRecordId` devuelto por la llamada `POST /api/verifications/verification-code` anterior.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/primary-email \
  -H 'authorization: Bearer <access_token>' \
  # Verifica la identidad del usuario (por contraseña o verificación de correo/teléfono existente)
  -H 'logto-verification-id: <verification_record_id_from_existing_identifier>' \
  -H 'content-type: application/json' \
  # El "newIdentifierVerificationRecordId" demuestra la propiedad del nuevo correo (del flujo de verificación anterior)
  --data-raw '{"email":"...","newIdentifierVerificationRecordId":"<verification_record_id_from_new_email>"}'
```

:::tip
Al igual que los correos electrónicos recogidos durante el registro, cualquier correo vinculado mediante la Account API debe pasar la verificación de la [lista de bloqueo](/security/blocklist) que configuraste en <CloudLink to="/security/blocklist">Consola > Seguridad > Lista de bloqueo</CloudLink>. Logto rechazará la solicitud y devolverá un error detallado si el correo viola la política.
:::
...
- **`logto-verification-id` (header)**: Demuestra la identidad del usuario antes de realizar cambios sensibles. Obtén esto [verificando la contraseña del usuario](#verify-the-users-password) o [enviando un código de verificación al correo electrónico o teléfono existente del usuario](#verify-by-sending-a-verification-code-to-the-users-email-or-phone).
- **`newIdentifierVerificationRecordId` (body)**: Identifica la identidad social que se está vinculando. Este es el `verificationRecordId` devuelto por la llamada `POST /api/verifications/social` anterior.
...
**Paso 1: Añade el origen de tu aplicación front-end a los orígenes relacionados**

Las claves de acceso WebAuthn están vinculadas a un hostname específico llamado **Relying Party ID (RP ID)**. Solo las aplicaciones alojadas en el origen del RP ID pueden registrar o autenticar con esas claves.

Dado que tu aplicación front-end llama a la Account API desde un dominio diferente al de las páginas de autenticación de Logto, necesitas configurar **Related Origins** para permitir operaciones de claves de acceso entre dominios.
...
- **Dominio personalizado**: Si has configurado un [dominio personalizado](/logto-cloud/custom-domain) como `https://auth.example.com`, el RP ID pasa a ser `auth.example.com`
...
WebAuthn admite hasta 5 etiquetas eTLD+1 únicas para Related Origins. El eTLD+1 (dominio de nivel superior efectivo más una etiqueta) es la parte registrable del dominio. Por ejemplo:
...
Si necesitas admitir más de 5 dominios diferentes como Related Origins, consulta la documentación de [Related Origin Requests](https://passkeys.dev/docs/advanced/related-origins/) para más detalles.

:::

**Paso 2: Solicita nuevas opciones de registro**

Usa el endpoint [`POST /api/verifications/web-authn/registration`](https://openapi.logto.io/operation/operation-generatewebauthnregistrationoptions) para solicitar el registro de una nueva clave de acceso. Logto permite que cada cuenta de usuario registre múltiples claves.
...
Tomando [`@simplewebauthn/browser`](https://simplewebauthn.dev/) como ejemplo, puedes usar la función `startRegistration` para registrar la clave en el navegador local.
...
Usa el endpoint [`POST /api/verifications/web-authn/registration/verify`](https://openapi.logto.io/operation/operation-verifywebauthnregistration) para verificar el registro de la clave.
...
Finalmente, puedes vincular la clave a la cuenta del usuario usando el endpoint [`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification).
...
- `verification_record_id`: un ID de registro de verificación válido, concedido al verificar el factor existente del usuario; puedes consultar la sección [Obtener un ID de registro de verificación](#get-a-verification-record-id) para más detalles.
...
Actualiza el nombre de la clave usando el endpoint [`PATCH /api/my-account/mfa-verifications/{verificationId}/name`](https://openapi.logto.io/operation/operation-updatemfaverificationname):
...
Elimina la clave usando el endpoint [`DELETE /api/my-account/mfa-verifications/{verificationId}`](https://openapi.logto.io/operation/operation-deletemfaverification):
...
### Reemplazar un TOTP existente \{#replace-an-existing-totp}

Si el usuario ya tiene un factor TOTP configurado, puedes reemplazarlo por uno nuevo usando el endpoint [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-createorreplacetotpmfaverification). Esta es una operación idempotente que elimina el TOTP existente y vincula uno nuevo.

:::note
Para usar este método, necesitas habilitar el campo `mfa` en la [configuración del centro de cuentas](#how-to-enable-account-api).
:::

**Paso 1: Genera un nuevo secreto TOTP**

Usa el endpoint [`POST /api/my-account/mfa-verifications/totp-secret/generate`](https://openapi.logto.io/operation/operation-generatetotpsecret) para generar un nuevo secreto TOTP, luego muéstralo al usuario como se describe en [Vincular un nuevo TOTP](#link-a-new-totp).

**Paso 2: Reemplaza el factor TOTP**
...
- `verification_record_id`: un ID de registro de verificación válido, concedido al verificar el factor existente del usuario. Puedes consultar la sección [Obtener un ID de registro de verificación](#get-a-verification-record-id) para más detalles.
- `secret`: el nuevo secreto TOTP generado en el paso 1.
- `code`: el código TOTP generado a partir del nuevo secreto para confirmar la vinculación.
...
Antes de vincular los códigos de respaldo a la cuenta del usuario, debes mostrárselos e indicarle que:
...
### Gestionar la configuración de verificación en dos pasos \{#manage-2-step-verification-settings}

:::note
Para usar estos métodos, necesitas habilitar el campo `mfa` en la [configuración del centro de cuentas](#how-to-enable-account-api).
:::

Los usuarios finales pueden activar o desactivar la verificación en dos pasos para su cuenta. Un caso común es cuando un usuario ha configurado factores MFA (como claves de acceso) pero solo quiere usarlos para iniciar sesión y no como un paso adicional de verificación; desactivar la verificación en dos pasos le permite omitir el aviso MFA durante el inicio de sesión mientras mantiene sus factores MFA intactos. Esto se controla mediante la configuración `skipMfaOnSignIn` por usuario.

**Obtener la configuración actual de MFA**

Usa el endpoint [`GET /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-getmfasettings) para recuperar la configuración actual de verificación en dos pasos del usuario.
...
**Actualizar la configuración de MFA**

Para habilitar o deshabilitar la verificación en dos pasos, usa el endpoint [`PATCH /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-updatemfasettings).
...
- `verification_record_id`: un ID de registro de verificación válido. Puedes consultar la sección [Obtener un ID de registro de verificación](#get-a-verification-record-id) para más detalles.
- `skipMfaOnSignIn`: establece en `true` para deshabilitar la verificación en dos pasos para el usuario, o en `false` para habilitarla.

:::note
Esta configuración solo tiene efecto cuando la política de MFA está establecida en **Opcional**. Si MFA es **Obligatorio** o **Adaptativo**, la configuración `skipMfaOnSignIn` será ignorada y MFA será obligatorio.
:::
...
- El alcance `UserScope.Sessions` es necesario para acceder a este endpoint.
- El campo `Sessions` en la configuración del centro de cuentas debe estar en `ReadOnly` o `Edit`.
...
- El alcance `UserScope.Sessions` es necesario para acceder a este endpoint.
- El campo `Sessions` en la configuración del centro de cuentas debe estar en `Edit`.
  :::
...
- `revokeGrantsTarget`: Opcionalmente especifica el objetivo de los grants a revocar junto con la sesión. Valores posibles:
  - `all`: Revoca todos los grants asociados con la sesión.
  - `firstParty`: Revoca solo los grants de aplicaciones de primera parte asociados con la sesión. (Recomendado para la mayoría de los casos, ya que revoca el acceso para tu propia app mientras mantiene los grants de apps de terceros intactos, proporcionando una mejor experiencia de usuario.)
  - sin especificar: El comportamiento predeterminado revoca grants que no tienen el alcance `offline_access`, lo que normalmente significa revocar grants sin refresh token para la sesión.

### Gestionar aplicaciones autorizadas del usuario (grants) \{#manage-user-authorized-apps-grants}

Usa las APIs de aplicaciones autorizadas del usuario (grants) cuando los usuarios necesiten revisar y revocar aplicaciones autorizadas desde su página de configuración de cuenta.

:::note

- Las APIs de grants de aplicaciones comparten el mismo modelo de permisos que las APIs de sesiones.
...
- El campo `Sessions` en la configuración del centro de cuentas debe estar habilitado:
  - `ReadOnly` o `Edit` para listar grants.
  - `Edit` para revocar grants.

:::

**Listar grants de aplicaciones activas**

Para listar los grants de aplicaciones activas del usuario actual, usa el endpoint [`GET /api/my-account/grants`](https://openapi.logto.io/operation/operation-getgrants).
...
- `appType=firstParty`: Devuelve solo grants de aplicaciones de primera parte.
- `appType=thirdParty`: Devuelve solo grants de aplicaciones de terceros.
- Omitir `appType`: Devuelve todos los grants activos.
...
**Revocar grant de aplicación por ID de grant**

Para revocar un grant de aplicación específico, usa el endpoint [`DELETE /api/my-account/grants/{grantId}`](https://openapi.logto.io/operation/operation-deletegrantbyid).
...
Cuando se revoca un grant, los tokens de acceso opacos y los tokens de actualización emitidos previamente para ese grant quedan invalidados.
