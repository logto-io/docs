...
# Configuración de cuenta con la interfaz preconstruida del Centro de Cuenta
...
- Gestionar la configuración de MFA (aplicación autenticadora TOTP, llaves de acceso, códigos de respaldo)
...
- **Esfuerzo de desarrollo cero**: Páginas listas para usar que funcionan desde el primer momento
...
| `/account/passkey/add`               | Añadir una nueva llave de acceso (WebAuthn)                                                  |
| `/account/passkey/manage`            | Ver y gestionar llaves de acceso existentes                                                  |
...
2. En la tarjeta **Integrar interfaz preconstruida**, añade los campos de perfil que deseas exponer desde el selector de campos de perfil. Estos campos provienen del mismo catálogo utilizado por [Recopilar perfil de usuario](/end-user-flows/collect-user-profile): cualquier campo definido allí también puede mostrarse en el Centro de Cuenta.
...
Puedes integrar las páginas preconstruidas en tu flujo existente de configuración de cuenta:
...
| `ui_locales`   | Lista separada por espacios de etiquetas de idioma BCP-47 (por ejemplo, `fr-CA fr en`) que controla el idioma de la interfaz del Centro de Cuenta. Si se omite, se utiliza el idioma del navegador del usuario.           |
...
Donde `{connectorId}` es el ID del conector social. Este es el **Callback URI** que se muestra en la página de detalles del conector en <CloudLink to="/connectors/social">Consola > Conectores > Conectores sociales</CloudLink>, que registras con tu proveedor social al [configurar el conector](/connectors/social-connectors/).
...
- [Configuración de cuenta con Account API](/end-user-flows/account-settings/by-account-api) - Crea gestión de cuentas personalizada con control total de la API
- [Configuración de cuenta con Management API](/end-user-flows/account-settings/by-management-api) - Gestión de cuentas a nivel de administrador
