...
- Ver y editar el perfil de usuario (nombre, avatar y campos de perfil personalizados)
...
| `/account/password`                  | Establecer o actualizar la contraseña                                                        |
...
| `/account/authenticator-app`         | Configurar la aplicación autenticadora TOTP                                                  |
...
Por ejemplo, si el endpoint de tu tenant es `https://example.logto.app`, la página de actualización de correo electrónico estaría disponible en `https://example.logto.app/account/email`.
...
1. Asegúrate de que el permiso del campo **Name**, **Avatar**, **Profile** o **Custom data** esté establecido en `Edit` o `ReadOnly` en las secciones de seguridad de la cuenta / perfil de usuario.
...
Solo los campos que estén tanto listados en los campos de perfil **como** cuyo permiso correspondiente no esté en `Off` aparecerán en la página de Perfil.
...
Puedes integrar las páginas preconstruidas en el flujo de configuración de cuenta existente de tu aplicación:
...
Después de que los usuarios completen una acción, serán redirigidos a la URL que especifiques con un parámetro de consulta opcional `show_success`. Puedes usar esto para mostrar un mensaje de éxito:
...
| `ui_locales`   | Lista separada por espacios de etiquetas de idioma BCP-47 (por ejemplo, `fr-CA fr en`) que controla el idioma de la interfaz del Centro de Cuenta. Si se omite, se usará el idioma del navegador del usuario.             |

Ejemplo — enlace profundo a la página de actualización de correo electrónico con el correo actual prellenado y la interfaz en francés:
...
Para configurarlo, navega a <CloudLink to="/sign-in-experience/account-center">Consola > Inicio de sesión y cuenta > Centro de cuenta</CloudLink> y establece el campo **URL de eliminación de cuenta**. También puedes actualizarlo mediante la Management API:
...
## URL de callback de conexión social \{#social-connection-callback-url}

Vincular una cuenta social desde el Centro de Cuenta utiliza la URL de callback del conector:

```
https://[your-tenant-endpoint]/callback/{connectorId}
```

Donde `{connectorId}` es el ID del conector social. Esta es la **Callback URI** que se muestra en la página de detalles del conector en <CloudLink to="/connectors/social">Consola > Conectores > Conectores sociales</CloudLink>, que registras con tu proveedor social al [configurar el conector](/connectors/social-connectors/).
...
La interfaz preconstruida del Centro de Cuenta hereda la marca de tu configuración de experiencia de inicio de sesión, incluyendo:
...
Si necesitas más personalización de la que ofrece la interfaz preconstruida y el CSS personalizado, considera usar la [Account API](/end-user-flows/account-settings/by-account-api) para construir tus propias páginas personalizadas de gestión de cuentas.
...
- [Configuración de MFA](/end-user-flows/mfa) - Configura la autenticación multifactor
- [Dispositivos de confianza para MFA](/end-user-flows/mfa/trusted-devices) - Configura y gestiona la confianza temporal en el navegador
