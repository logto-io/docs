...
:::note
La actualización del TTL no está disponible en aplicaciones de una sola página (SPA) por razones de seguridad. Para SPA, esta configuración controla la vida útil fija del token de actualización desde el momento de emisión original. Logto no extenderá el TTL mediante solicitudes de token, y la rotación de tokens de actualización no evita que los tokens de actualización de SPA expiren.
:::
