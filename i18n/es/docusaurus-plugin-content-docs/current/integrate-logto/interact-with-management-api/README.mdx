...
Con la Logto Management API, puedes acceder a los robustos servicios backend de Logto, que son altamente escalables y pueden ser utilizados en multitud de escenarios. Va más allá de lo que es posible con las capacidades low-code de la Consola de Administración.
...
Algunas respuestas de la API pueden incluir muchos resultados, los resultados estarán paginados. Logto proporciona 2 tipos de información de paginación.

#### Usando encabezados link \{#using-link-headers}
...
#### Usando el encabezado total-number \{#using-total-number-header}
...
Logto Cloud aplica límites de tasa de ejecución a nivel de inquilino para proteger la estabilidad del sistema. Para más detalles, consulta la [sección de límite de tasa del sistema](/logto-cloud/system-limit#rate-limit).
