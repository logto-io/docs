...
Este enfoque de integración es ideal para escenarios comerciales comunes. Puedes permitir que los usuarios accedan a aplicaciones de socios utilizando sus cuentas de Logto, de la misma manera que los usuarios empresariales inician sesión en Slack con Google Workspace. También puedes construir una plataforma abierta donde las aplicaciones de terceros puedan añadir la funcionalidad "Iniciar sesión con Logto", similar a "Iniciar sesión con Google".
...
Así, dado que OIDC se basa en [OAuth 2.0](https://auth.wiki/oauth-2.0) añadiendo una capa de autenticación, también puedes integrar aplicaciones de terceros utilizando el protocolo OAuth.
...
<Url href="https://blog.logto.io/logto-as-idp">
  Usar Logto como proveedor de identidad de terceros (IdP)
</Url>
