...
En la página de detalles de la aplicación, puedes encontrar el [**ID de cliente**](/integrate-logto/application-data-structure#application-id), [**Secreto de cliente**](/integrate-logto/application-data-structure#application-secret) (solo para aplicaciones web tradicionales) y los endpoints OIDC necesarios para la integración.
...
### Integración vía protocolo OAuth / OIDC \{#integrate-via-oauth-protocol}
...
### Integración vía device flow \{#integrate-via-device-flow}

Para aplicaciones de terceros nativas que se ejecutan en dispositivos con entrada limitada (por ejemplo, smart TVs, consolas de juegos, herramientas CLI), el flujo estándar de código de autorización basado en redirección puede no ser factible. En estos casos, la aplicación puede usar el [Device Authorization Grant de OAuth 2.0](https://auth.wiki/device-flow).

Con el device flow, el dispositivo muestra un código de usuario y una URL de verificación. El usuario visita la URL en un dispositivo separado (teléfono, portátil), ingresa el código y completa la autenticación allí. El dispositivo consulta el endpoint de token de Logto hasta que la autorización se complete.
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        'Personaliza la apariencia de la pantalla de consentimiento para que coincida con la identidad de tu marca y proporciona una experiencia de usuario consistente.',
      customProps: {
