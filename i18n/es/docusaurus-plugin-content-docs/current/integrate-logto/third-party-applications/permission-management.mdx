...
Al habilitar alcances específicos, determinas a qué información de usuario pueden acceder las aplicaciones de terceros. Los usuarios revisarán y aprobarán estos permisos en la pantalla de consentimiento antes de otorgar acceso.
...
2. `offline_access`: Opcional. Recupera [tokens de actualización](/integrate-logto/application-data-structure#rotate-refresh-token) para acceso prolongado o tareas en segundo plano.
