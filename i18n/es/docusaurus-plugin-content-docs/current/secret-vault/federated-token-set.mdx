...
Los tokens almacenados se adjuntan a la identidad social o de SSO empresarial del usuario, permitiéndole recuperarlos posteriormente para acceder a la API sin requerir una nueva autenticación.

### Comprobación del estado de almacenamiento de tokens \{#checking-token-storage-status}

Puedes comprobar el estado de almacenamiento de tokens federados de un usuario en la Consola de Logto:
...
También puedes comprobar las identidades de terceros del usuario y el estado de almacenamiento de tokens a través del Management API:
...
- **Inactivo**: No hay token de acceso almacenado para esta conexión. Esto puede ocurrir si el usuario no se ha autenticado a través de esta conexión o si el almacenamiento del token ha sido eliminado.
- **No aplicable**: El conector no admite almacenamiento de tokens.

### Metadatos del token \{#token-metadata}

Por integridad y seguridad de los datos, todos los tokens se cifran antes de almacenarse en el Secret Vault. Los valores reales de los tokens solo son accesibles para el usuario final con la autorización adecuada. Los desarrolladores, por su parte, solo pueden recuperar los metadatos del conjunto de tokens para comprender el estado de los tokens almacenados sin exponer contenido sensible.
...
- `404` No encontrado: Si el usuario no tiene una identidad social o de SSO empresarial asociada con el conector o ID especificado, o si el token de acceso no está almacenado.
- `401` No autorizado: Si el token de acceso ha expirado.

Si el token de acceso ha expirado y hay un token de actualización disponible, Logto intentará automáticamente refrescar el token de acceso y devolverá el nuevo token de acceso en la respuesta. El almacenamiento de tokens en el Secret Vault también se actualizará con el nuevo token de acceso y sus metadatos.
...
Revocar el conjunto de tokens obligará al usuario a autenticarse nuevamente con el proveedor de terceros para obtener un nuevo token de acceso antes de poder acceder nuevamente a las APIs de terceros.

## Reautenticación y renovación de tokens \{#reauthentication-and-token-renewal}

En escenarios donde un token de acceso almacenado ha expirado o cuando una aplicación necesita solicitar alcances (scopes) adicionales de la API, los usuarios finales pueden reautenticarse con el proveedor de terceros para obtener un nuevo token de acceso, sin necesidad de iniciar sesión nuevamente en Logto.
...
1. El usuario inicia una solicitud de verificación social llamando al endpoint `POST /api/verification/social`. El usuario puede especificar scopes personalizados para solicitar permisos adicionales al proveedor de terceros.
   ...
   - **redirectUri**: El URI al que redirigir al usuario de vuelta a tu aplicación tras la autenticación. Deberás registrar este URI en la configuración de la aplicación del proveedor.
...
7. Finalmente, actualiza el almacenamiento de tokens del usuario llamando al endpoint `PATCH /my-account/identities/:target/access-token` con el ID de verificación social:
   ...
   Esto actualizará el almacenamiento del conjunto de tokens del usuario en el Secret Vault de Logto con el nuevo token de acceso y token de actualización, permitiendo al usuario acceder a APIs de terceros sin necesidad de iniciar sesión nuevamente en Logto.
