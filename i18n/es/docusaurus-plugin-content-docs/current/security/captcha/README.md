...
:::note
El CAPTCHA se aplica a las acciones de identificador, contraseña, código de verificación, registro y recuperación de contraseña. No se aplica a [enlace mágico](/end-user-flows/one-time-token) o [inicio de sesión con passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in), por lo que los usuarios que completen el inicio de sesión con un enlace mágico o passkey no necesitan resolver un desafío CAPTCHA adicional.
:::

:::note
Si utilizas [Bring your UI](/customization/bring-your-ui/), la integración CAPTCHA incorporada no se ejecuta en tu frontend personalizado. Tu UI personalizada debe cargar el script del proveedor de CAPTCHA, ejecutar el desafío y enviar el token como `captchaToken` en `PUT /api/experience`.
:::

## Habilitar la protección contra bots con CAPTCHA {#enabling-captcha-bot-protection}
...
5. **(Opcional) Habilitar CAPTCHA**: El CAPTCHA se habilitará automáticamente en la página de seguridad una vez que se configure un proveedor. Sin embargo, puedes verificar o ajustar la configuración manualmente si es necesario.
...
1. **Usa tu aplicación**: Navega a las páginas de inicio de sesión, registro o recuperación de contraseña de tu aplicación e intenta las acciones de usuario correspondientes.
