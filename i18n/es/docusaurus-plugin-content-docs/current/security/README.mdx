import BlockUserIcon from '@site/src/assets/block-user.svg';
import CheckboxIcon from '@site/src/assets/checkbox.svg';
import HideIcon from '@site/src/assets/hide.svg';
import KeyIcon from '@site/src/assets/key.svg';
import LockIcon from '@site/src/assets/lock.svg';
import LockoutIcon from '@site/src/assets/lockout.svg';
import SecurityIcon from '@site/src/assets/security.svg';
import SendEmailIcon from '@site/src/assets/send-mail.svg';
import SuspendIcon from '@site/src/assets/suspend.svg';

# Seguridad

La seguridad moderna de la autenticación combate amenazas que van desde phishing, relleno de credenciales, ataques de fuerza bruta, ransomware, DDoS, hasta ataques impulsados por IA. Proteger las identidades de los usuarios es fundamental para salvaguardar la confianza en la marca y el cumplimiento normativo.

Logto ofrece una gestión de acceso segura y robusta diseñada para contrarrestar estos riesgos de manera directa. Al priorizar la prevención proactiva de amenazas y la resiliencia, aseguramos que tus sistemas permanezcan protegidos sin comprometer la usabilidad. Con Logto, la seguridad no es una ocurrencia tardía: es la base, empoderando a las empresas para prosperar en una era donde las amenazas evolucionan, pero las defensas evolucionan aún más rápido.

## Configura protección de seguridad avanzada \{#set-up-advanced-security-protection}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Política de contraseñas',
      href: '/security/password-policy',
      description:
        'Mejora los requisitos de contraseñas para defenderte contra ataques de relleno de credenciales y contraseñas débiles.',
      customProps: {
        icon: <KeyIcon />,
      },
    },
    {
      type: 'link',
      label: 'CAPTCHA',
      href: '/security/captcha',
      description:
        'Añade CAPTCHA (por ejemplo, Google reCAPTCHA, Cloudflare Turnstile) a tu experiencia de inicio de sesión para prevenir ataques automatizados de bots.',
      customProps: {
        icon: <CheckboxIcon />,
      },
    },
    {
      type: 'link',
      label: 'Lista de bloqueo',
      href: '/security/blocklist',
      description:
        'Controla los nuevos registros y los correos electrónicos recién vinculados con una lista de permitidos y bloquea direcciones desechables o no deseadas.',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    {
      type: 'link',
      label: 'Bloqueo de identificador',
      href: '/security/identifier-lockout',
      description:
        'Bloquea temporalmente un identificador tras varios intentos fallidos de autenticación para prevenir el acceso por fuerza bruta.',
      customProps: {
        icon: <LockoutIcon />,
      },
    },
    {
      type: 'link',
      label: 'Límite de envío',
      href: '/security/send-rate-limit',
      description:
        'Limita el envío de códigos de verificación y mensajes por destinatario para evitar spam en la bandeja de entrada y abuso de SMS pumping.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'Ocultar existencia de cuenta',
      href: '/end-user-flows/sign-up-and-sign-in',
      description:
        'Oculta el estado de la cuenta para bloquear ataques de enumeración de cuentas y evitar divulgar información sensible sobre el estado de la cuenta.',
      customProps: {
        icon: <HideIcon />,
      },
    },
  ]}
/>

## Descubre más formas de proteger tus aplicaciones \{#discover-more-ways-to-protect-your-apps}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Autenticación multifactor (MFA)',
      href: '/end-user-flows/mfa',
      description:
        'Agrega una capa extra de protección al proceso de inicio de sesión con soporte para OTP de aplicaciones autenticadoras, passkeys (WebAuthn) y códigos de respaldo.',
      customProps: {
        icon: <LockIcon />,
      },
    },
    {
      type: 'link',
      label: 'Autenticación escalonada',
      href: '/end-user-flows/security-verification',
      description:
        'Permite que la aplicación solicite autenticación escalonada cuando los usuarios acceden a información sensible o realizan acciones de alto riesgo.',
      customProps: {
        icon: <SecurityIcon />,
      },
    },
    {
      type: 'link',
      label: 'Suspender usuarios',
      href: '/user-management/manage-users#suspend-user',
      description:
        'Desactiva temporalmente cuentas de usuario para bloquear el acceso sin eliminar datos ni registros de usuario.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Rotar claves de firma',
      href: '/developers/signing-keys',
      description:
        'Rota periódicamente las claves de firma para protegerte contra fugas de claves y falsificación de tokens.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'Cierre de sesión back-channel OIDC',
      href: '/end-user-flows/sign-out#federated-sign-out-back-channel-logout',
      description:
        'Habilita el cierre de sesión centralizado para reducir el riesgo de secuestro de sesión y acceso no autorizado.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Registro solo por invitación',
      href: '/end-user-flows',
      description:
        'Restringe los registros solo a usuarios invitados, utilizando enlaces mágicos por correo electrónico para una incorporación segura.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
  ]}
/>
