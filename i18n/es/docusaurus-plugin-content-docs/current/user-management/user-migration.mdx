...
Logto admite específicamente [PBKDF2](https://en.wikipedia.org/wiki/PBKDF2).
...
3.  **Configura la conexión con la Management API**  
    Usaremos la Management API (Management API) para importar los datos de usuario. Puedes consultar [Management API](/integrate-logto/interact-with-management-api) para aprender cómo configurar la conexión en tu entorno de desarrollo.
...
Ten en cuenta que el endpoint de la API tiene límite de velocidad; debes añadir una pausa entre cada solicitud para evitar el límite. Consulta nuestra página de [límites de velocidad](/integrate-logto/interact-with-management-api/#rate-limit) para más detalles.
