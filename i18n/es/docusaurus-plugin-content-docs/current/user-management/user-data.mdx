...
Su valor proviene del nombre de usuario con el que el usuario se registró por primera vez. Puede ser `null`. Su valor no nulo no debe tener más de 128 caracteres, solo contener letras, números y guiones bajos (`_`), y NO comenzar con un número.

### primary_email

_primary_email_ es la dirección de correo electrónico del usuario, utilizada para iniciar sesión con el correo electrónico y el código de acceso.
...
_primary_phone_ es el número de teléfono del usuario, utilizado para iniciar sesión con el número de teléfono y el código de acceso de SMS.
...
Una diferencia en comparación con los otros reclamos estándar es que las propiedades en `profile` solo se incluirán en el Token de ID o en la respuesta del endpoint de información del usuario cuando sus valores no estén vacíos, mientras que otros reclamos estándar devolverán `null` si los valores están vacíos.
...
_last_signed_in_at_ es la marca de tiempo con la zona horaria cuando el usuario inició sesión por última vez.
...
Logto utiliza la implementación de [Argon2](https://en.wikipedia.org/wiki/Argon2) [node-argon2](https://github.com/ranisalt/node-argon2) como método de encriptación por defecto; consulta la referencia para más detalles si estás interesado.
...
_is_suspended_ es un valor booleano que indica si un usuario está suspendido o no. El valor se puede gestionar llamando a la [Logto Management API](https://openapi.logto.io/operation/operation-updateuserissuspended) o usando Admin Console.
...
| [last_sign_in_at](./#last_signed_in_at)                     | date time | Marca de tiempo cuando el usuario inició sesión por última vez       | ❌    | ✅        |
...
_identities_ contiene la información del usuario obtenida del inicio de sesión social (es decir, inicio de sesión con un conector social). Cada _identities_ de usuario se almacena en un objeto JSON individual.
...
Puedes obtener un perfil de usuario que contenga _custom_data_ usando Management API y enviarlo a las aplicaciones frontend o servicios backend externos. Por lo tanto, poner información sensible en _custom_data_ puede causar filtraciones de datos.
...
Puedes actualizar el _custom_data_ del usuario usando [Admin Console](/user-management/manage-users/#view-and-update-the-user-profile) o [Logto Management API](/user-management/manage-users/#manage-via-logto-management-api), como [`PATCH /api/users/:userId`](https://openapi.logto.io/operation/operation-patch-api-users-parameter).
