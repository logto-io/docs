...
En la página de detalles del usuario, haz clic en "Tres puntos" -> botón "Eliminar". La eliminación del usuario no se puede deshacer.
...
[Management API](/concepts/core-service/#management-api) es una colección de APIs que proporcionan acceso al servicio backend de Logto. Como se mencionó anteriormente, la API de usuario es un componente crítico de este servicio y puede admitir una amplia gama de escenarios.
...
Sin embargo, aún puedes diseñar roles y permisos específicos de la aplicación para proteger tus recursos de API, y validar permisos en el acceso a la API tras el inicio de sesión exitoso del usuario.
Consulta Autorización: [Control de acceso basado en roles](/authorization/role-based-access-control) para obtener más información.
