...
Después de configurar la [Management API](/integrate-logto/interact-with-management-api/), puedes usar los [endpoints de API](https://openapi.logto.io/operation/operation-listuserpersonalaccesstokens) para crear, listar y eliminar tokens de acceso personal.
...
Para aplicaciones web tradicionales o aplicaciones máquina a máquina con secreto de aplicación, incluye las credenciales en el encabezado `Authorization` usando autenticación HTTP Basic:
...
Luego, si decodificas el token de acceso con [JWT decoder](https://logto.io/jwt-decoder), obtendrás el siguiente ejemplo de payload de token de acceso:
