...
## Crea y configura una app de Alipay \{#create-and-configure-alipay-app}
...
5. Después de crear la aplicación, llegarás a la página de resumen, donde debes hacer clic en "add ability" (添加能力) para añadir "Third-party application authorization" (第三方应用授权), "Get member information" (获取会员信息) y "App Alipay login" (App 支付宝登录) antes de habilitar el inicio de sesión con Alipay.
...
9. Configura el "Authorization Redirect URI" (授权回调地址) haciendo clic en el botón "set up" (设置) en la parte inferior de la página de la consola de Alipay. `${your_logto_origin}/callback/${connector_id}` es el URI de redirección predeterminado utilizado en Logto core. El `connector_id` se puede encontrar en la barra superior de la página de detalles del conector en la Consola de Administración de Logto.
...
3. Completa los ajustes del conector de Logto:
   ...
   - Rellena el campo `privateKey` con el contenido del archivo de clave privada mencionado en el paso 2. ASEGÚRATE de usar '\n' para reemplazar todos los saltos de línea. No necesitas eliminar el encabezado y pie en el archivo de clave privada.
   - Rellena el campo `signType` con 'RSA2' debido al modo de firma `Public key` que elegimos en el paso 7 de "Crear y configurar apps de Alipay".
...
Suponemos que ya has integrado el [SDK de Logto para iOS](/quick-starts/swift) en tu app. En este caso, todo es bastante sencillo, y ni siquiera necesitas leer la documentación del SDK de Alipay:
...
Suponemos que ya has integrado el [SDK de Logto para Android](/quick-starts/android) en tu app. En este caso, todo es bastante sencillo, y ni siquiera necesitas leer la documentación del SDK de Alipay:
...
### Prueba el conector nativo de Alipay \{#test-alipay-native-connector}
