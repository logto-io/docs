## Iniciar sesión con cuenta de GitLab \{#sign-in-with-gitlab-account}
...
## Crear y configurar la app OAuth \{#create-and-configure-oauth-app}
...
Ve a la [página de Aplicaciones](https://gitlab.com/-/profile/applications) en GitLab, donde puedes agregar, editar o eliminar apps OAuth existentes. También puedes encontrar el `Application ID` y generar el `Secret` en las páginas de detalles de la app OAuth.
...
`scope` es una lista separada por espacios de [alcances](https://docs.gitlab.com/ee/integration/oauth_provider.html#authorized-applications). Si no se proporciona, el alcance por defecto será `openid`. Para el conector de GitLab, los alcances que puedes querer usar son `openid`, `profile` y `email`. El alcance `profile` es necesario para obtener la información del perfil del usuario, y el alcance `email` es necesario para obtener la dirección de correo electrónico del usuario. Asegúrate de haber permitido estos alcances en tu app OAuth de GitLab (configurado en la sección [Crear y configurar la app OAuth](#create-and-configure-oauth-app)).
...
## Probar el conector de GitLab \{#test-gitlab-connector}

Eso es todo. El conector de GitLab debería estar disponible ahora. No olvides [habilitar el conector en el registro e inicio de sesión](https://docs.logto.io/docs/recipes/configure-connectors/social-connector/enable-social-sign-in/).
