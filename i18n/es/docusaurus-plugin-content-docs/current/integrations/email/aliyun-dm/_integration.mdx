...
Ve a la [página de la consola del servicio DM](https://www.aliyun.com/product/directmail) e inicia sesión. Habilita el servicio de Direct Mail haciendo clic en el botón "Apply to enable" (申请开通) en la parte superior izquierda de la página y comienza el proceso de configuración.
...
1. Desde la [página de la consola de administración de DM](https://dm.console.aliyun.com/), pasa el cursor sobre tu avatar en la esquina superior derecha y ve a "AccessKey Management" (AccessKey 管理), y haz clic en "Create AccessKey" (创建 AccessKey). Obtendrás un par de "AccessKey ID" y "AccessKey Secret" después de completar la verificación de seguridad. Por favor, guárdalos adecuadamente.
2. Ve a la pestaña "Sender Addresses" (发信地址) o "Email Tags" (邮件标签) que acabas de visitar desde la [página de la consola de administración de DM](https://dm.console.aliyun.com/), puedes encontrar fácilmente _Sender Address_ o _Email Tag_.
3. Completa la configuración del Aliyun DM Connector:
