...
Las [claves de firma OIDC](https://auth.wiki/signing-key) de Logto, también conocidas como "claves privadas OIDC" y "claves de cookie OIDC", son las claves de firma utilizadas para firmar JWTs ([tokens de acceso (Access tokens)](https://auth.wiki/access-token) e [ID tokens](https://auth.wiki/id-token)) y cookies del navegador en las [sesiones de inicio de sesión](/end-user-flows/sign-out#sign-in-session) de Logto. Estas claves de firma se generan al inicializar la base de datos de Logto ([open-source](/logto-oss)) o al crear un nuevo tenant ([Cloud](/logto-cloud)) y pueden ser gestionadas a través de la [CLI](/logto-oss/using-cli) (open-source), Management APIs o la interfaz de la Consola.
...
- **Clave privada OIDC**
  Al inicializar una instancia de Logto, se genera automáticamente un par de clave pública y clave privada, que se registran en el proveedor OIDC subyacente. Así, cuando Logto emite un nuevo JWT (token de acceso o ID token), el token se firma con la clave privada. Mientras tanto, cualquier aplicación cliente que reciba un JWT puede usar la clave pública emparejada para verificar la firma del token, con el fin de asegurar que el token no haya sido manipulado por terceros. La clave privada está protegida en el servidor de Logto. La clave pública, como su nombre indica, es pública para todos y puede accederse a través de la interfaz `/oidc/jwks` del endpoint OIDC. Se puede especificar un algoritmo de clave de firma al generar la clave privada, y Logto utiliza el algoritmo EC (Curva Elíptica) por defecto. Los usuarios administradores pueden cambiar el algoritmo predeterminado a RSA (Rivest-Shamir-Adleman) rotando las claves privadas.
...
3. Encontrarás una tabla que enumera todas las claves de firma en uso. Nota: Puedes eliminar la clave anterior, pero no puedes eliminar la actual.
   ...
   | Anterior | Se refiere a una clave que se usó previamente pero que ha sido rotada. Los tokens existentes con esta clave de firma siguen siendo válidos. |
