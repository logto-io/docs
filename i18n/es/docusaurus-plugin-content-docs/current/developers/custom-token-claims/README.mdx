...
- **Token opaco (Opaque token):** Un [token opaco](http://localhost:3000/concepts/opaque-token) no es autocontenible y siempre requiere un paso de validación adicional a través del endpoint de [introspección de token](https://auth.wiki/token-introspection). A pesar de su formato no transparente, los tokens opacos pueden ayudar a obtener reclamos y ser transmitidos de forma segura entre las partes. Los reclamos de token se almacenan de forma segura en el servidor Logto y son accedidos por las aplicaciones cliente mediante el endpoint de introspección de token. Los tokens de acceso se emiten en formato opaco cuando no se incluye un recurso u organización específica en la solicitud de autorización. Estos tokens se utilizan principalmente para acceder al endpoint OIDC `userinfo` y otros propósitos generales.
...
sequenceDiagram
  ...
  IdP-->>IdP: Fusionar carga útil bruta del token de acceso y reclamos adicionales de token
...
:::warning
Nota de seguridad: En implementaciones autogestionadas, los scripts personalizados de JWT se ejecutan con los mismos privilegios que el proceso del servidor Logto, por lo que otorgar acceso para editar o probar scripts equivale a otorgar ejecución de código en el host de Logto. Esta función está destinada solo para administradores de confianza: no permitas que usuarios no confiables o de menor privilegio creen, modifiquen o prueben estos scripts, y no compartas la Consola de Administración con ellos.
:::
