...
Utiliza el editor de código en el lado izquierdo para modificar el script. Se proporciona una función `getCustomJwtClaims` predeterminada con un valor de retorno de objeto vacío para que puedas comenzar. Puedes modificar la función para que retorne un objeto con tus propios reclamos personalizados.
...
El objeto de contexto contiene los datos de usuario y datos de concesión relevantes para el proceso actual de autorización (Authorization).
...
- Maneja los errores y el tiempo de espera correctamente en tu script para evitar que el proceso de emisión del token se bloquee.
...
:::note
El resultado de la prueba es la salida de la función `getCustomJwtClaims` con los datos simulados que configures ("reclamos extra del token" obtenidos tras completar el paso 3 en [el diagrama de secuencia](/developers/custom-token-claims/#how-do-custom-token-claims-work)). La carga útil real del token y el contexto de datos de usuario serán diferentes cuando el script se ejecute en el proceso de emisión del token.
:::
