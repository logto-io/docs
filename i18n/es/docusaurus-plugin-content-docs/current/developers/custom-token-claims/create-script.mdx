...
Para agregar reclamos personalizados al token de acceso, necesitas proporcionar un script que devuelva un objeto que contenga esos reclamos. El script debe escribirse como una función de `JavaScript` que devuelva un objeto con los reclamos personalizados.

1. Navega a <CloudLink to="/customize-jwt">Consola > Custom JWT</CloudLink>.
2. Hay dos tipos diferentes de tokens de acceso para los cuales puedes personalizar los reclamos del token de acceso:
   ...
   Elige cualquier tipo de token de acceso para el cual desees personalizar los reclamos del token y haz clic en el botón **Agregar reclamos personalizados** para crear un nuevo script.

:::note
La función de reclamos personalizados de token solo está disponible para:

- Usuarios de OSS
- Inquilinos de desarrollo
- Inquilinos de pago (incluidos inquilinos Pro y Enterprise)
  :::

## Implementar la función `getCustomJwtClaims()`

En la página de detalles de **Custom JWT**, puedes encontrar el editor de scripts para escribir tu script de reclamos personalizados de token. El script debe ser una función de `JavaScript` que devuelva un objeto de reclamos personalizados.

![Página de detalles de reclamos personalizados de token](../assets/custom-jwt-detail-page.png)

## Paso 1: Editar el script

Usa el editor de código en el lado izquierdo para modificar el script. Se proporciona un `getCustomJwtClaims` predeterminado con un valor de retorno de objeto vacío para que comiences. Puedes modificar la función para devolver un objeto con tus propios reclamos personalizados.
...
El objeto de contexto contiene los datos del usuario y los datos de concesión relevantes para el proceso de autorización actual.
...
Usa la sección **Configurar variables de entorno** a la derecha para configurar las variables de entorno para tu script. Puedes usar estas variables para almacenar información sensible o datos de configuración que no deseas codificar directamente en el script. Por ejemplo, claves de API, secretos o URLs.
...
Puedes usar la función `fetch` incorporada de node para obtener datos externos en tu script. La función `fetch` es una función basada en promesas que te permite realizar solicitudes HTTP a APIs externas.
...
- Maneja adecuadamente los errores y el tiempo de espera en tu script para evitar que el proceso de emisión de tokens se bloquee.
- Usa encabezados de autorización adecuados para proteger tu API externa de accesos no autorizados.
  :::

## Paso 4: Probar el script

Asegúrate de probar tu script antes de guardarlo. Haz clic en la pestaña **Contexto de prueba** en el lado derecho de la página para modificar la carga útil del token simulado y el contexto de datos del usuario para las pruebas.
...
![Probar script de JWT personalizado](../assets/test-custom-jwt-script.png)
...
Haz clic en el botón **Crear** para guardar el script. El script de reclamos personalizados de token se guardará y aplicará al proceso de emisión de tokens de acceso.
