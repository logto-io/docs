...
La suplantación de usuario permite a usuarios autorizados como Sarah actuar temporalmente en nombre de otros usuarios como Alex dentro del sistema. Esta potente función es invaluable para la resolución de problemas, la atención al cliente y la realización de tareas administrativas.
...
async function impersonateUser(
  ...
  try {
    ...
    // highlight-start
    // Para aplicaciones web tradicionales o M2M, usa autenticación básica con client secret
...
async function performImpersonation(): Promise<void> {
  try {
    // highlight-start
    // Para aplicaciones web tradicionales o M2M, pasa el client secret
...
2. El token de acceso de suplantación no incluye un [refresh token (Token de actualización)](https://auth.wiki/refresh-token). Sarah tendrá que repetir este proceso si el token expira antes de resolver el problema de Alex.
...
Para incluir el reclamo `act`, la aplicación de Sarah debe proporcionar un `actor_token` en la solicitud de intercambio de tokens. Este token debe ser un token de acceso válido para Sarah con el alcance `openid`. Así es como se incluye en la solicitud de intercambio de tokens:
