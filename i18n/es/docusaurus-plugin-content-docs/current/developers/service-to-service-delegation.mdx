...
Utiliza la concesión de intercambio de tokens de Logto para intercambiar el token de acceso del usuario por un nuevo token de acceso cuya audiencia sea el recurso de API downstream. Esto sigue el patrón de intercambio de tokens de OAuth 2.0 y evita reenviar el token original del usuario a los servicios downstream.
...
sequenceDiagram
    ...
    User->>ApiA: Authorization: Bearer token de acceso de usuario
    Note over ApiA: Validar el token de usuario para API A

    ApiA->>Logto: POST /oidc/token con concesión de intercambio de token
...
Cuando API A necesita llamar a API B, realiza una solicitud de intercambio de token al [endpoint de token](/integrate-logto/application-data-structure#token-endpoint) de Logto.
...
Cuando se decodifica, el token de acceso incluye reclamos similares a:
...
Consulta [Validar tokens de acceso en API](/authorization/validate-access-tokens) para orientación de implementación.
