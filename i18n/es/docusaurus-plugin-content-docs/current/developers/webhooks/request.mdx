...
Se dispara cuando un usuario completa un flujo de registro, inicio de sesión o restablecimiento de contraseña gestionado por la Experience API. Además de los [campos comunes](#common-fields), el cuerpo incluye:
...
#### Formatos de entidad \{#user-flow-entity-shapes}
...
Cada evento de excepción lleva los [campos comunes](#common-fields) y un campo `ip` (misma estructura que los eventos de mutación de datos). Los campos restantes dependen del evento.

### Identifier.Lockout \{#identifierlockout-payload}

Se origina en un flujo orientado al usuario, por lo que el cuerpo también incluye los [campos de contexto de Experience API](#experience-api-context-fields), además de:
...
| recipient | `string` |          | El correo electrónico o número de teléfono que alcanzó el [límite de envío](/security/send-rate-limit). |

### Grant.LimitExceeded \{#grantlimitexceeded-payload}

Se dispara cuando una autorización exitosa hace que un usuario supere el [límite máximo de dispositivos autenticados concurrentes](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) de una aplicación (`maxAllowedGrants`) y Logto revoca sus concesiones más antiguas para esa aplicación.
