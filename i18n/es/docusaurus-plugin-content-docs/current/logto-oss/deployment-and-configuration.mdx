...
Usamos un conjunto predefinido de variables de entorno generado en nuestra demo (`docker-compose.yml`), que deberías reemplazar por las tuyas propias y mantener la coherencia entre múltiples instancias de Logto.
...
- `DATABASE_STATEMENT_TIMEOUT` (v1.36.0+) `statement_timeout` de PostgreSQL en milisegundos. Usa una cadena numérica (por ejemplo, `5000`) o `DISABLE_TIMEOUT` para PgBouncer / RDS Proxy. Consulta [Configuración](/concepts/core-service/configuration/) para más detalles.
...
**Habilitar Consola de Administración**
...
**Deshabilitar Consola de Administración**
...
- Para usar el [Secret Vault](/secret-vault), necesitas establecer `SECRET_VAULT_KEK` con una cadena codificada en base64 de tu Key Encryption Key (KEK). Esto se usa para cifrar las Data Encryption Keys (DEK) en el Secret Vault. Se recomienda AES-256 (32 bytes). Ejemplo: `crypto.randomBytes(32).toString('base64')`.
...
Si deseas usar un proxy inverso en tu servidor, por ejemplo Nginx o Apache, necesitas mapear los puertos 3001 y 3002 por separado en la configuración de tu proxy pass. Suponiendo que usas Nginx, tu endpoint de autenticación de Logto está en el puerto 3001 y tu consola de administración de Logto está en el 3002, coloca la siguiente configuración en nginx.conf:

Establece `TRUST_PROXY_HEADER=1` para que Logto confíe en los encabezados reenviados. Usa `$http_host` tanto para `Host` como para `X-Forwarded-Host` para preservar los puertos no estándar en la URL pública.

```
server {
  listen 443 ssl;
  server_name <your_endpoint_url>; // por ejemplo, auth.tu-dominio.com
  ...
  ...
  ssl_certificate_key <ruta-a-tu-clave-para-auth-endpoint>
  ...
}
```
...
server {
  listen 443 ssl;
  server_name <your_admin_endpoint_url>; // por ejemplo, admin.tu-dominio.com
  ...
  ...
  ssl_certificate_key <ruta-a-tu-clave-para-admin-endpoint>
  ...
}
```
...
### Configuración de la base de datos \{#containerization}

Prepara una base de datos Postgres para Logto. Se recomienda encarecidamente usar Logto CLI para inicializar la base de datos.

```bash
npm run cli db seed -- --swe
```

Consulta la [Logto CLI](/logto-oss/using-cli) para más detalles.

:::tip Instalaciones en entornos aislados o sin conexión

Si tu entorno de despliegue no puede acceder a `api.pwnedpasswords.com`, añade `--disable-admin-pwned-password-check` a `logto init` o `npm run cli db seed` para que el primer registro de administrador no se quede esperando la comprobación de brechas de Have I Been Pwned. Consulta [Seed para despliegues aislados o sin conexión](/logto-oss/using-cli#seed-for-air-gapped-or-offline-deployments) para más detalles.

:::

### Carpeta compartida de conectores \{#database-setup}
...
Para producción, puedes reemplazar el volumen "empty dir" por un volumen persistente y hacer el trabajo de "init" a tu manera, ¡tú sabes lo que haces!

### Alteración de la base de datos \{#shared-connectors-folder}
