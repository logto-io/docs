...
[gradle](https://spring.io/guides/gs/gradle) 사용자라면, 다음 종속성을 `build.gradle` 파일에 추가하세요:
...
[maven](https://spring.io/guides/gs/maven) 사용자라면, 다음 종속성을 `pom.xml` 파일에 추가하세요:
...
Logto Console에서 새로운 `Java Spring Boot` 애플리케이션을 등록하고 웹 애플리케이션에 대한 클라이언트 자격 증명 및 IdP 구성을 받으세요.
