...
- **v2**: 네이티브 소셜 커넥터에 필요한 임베디드 WebView에서 로그인 환경을 엽니다. 단, [패스키 로그인](/end-user-flows/sign-up-and-sign-in/passkey-sign-in)은 지원하지 않습니다 (WebView는 패스키의 기반 표준인 WebAuthn을 지원하지 않습니다).
- **v3 (beta)**: [Chrome Custom Tabs](https://developer.android.com/develop/ui/views/layout/webapps/overview-of-android-custom-tabs) (시스템 브라우저)에서 로그인 환경을 열어 패스키 로그인을 지원하고 브라우저 세션을 공유합니다. v3은 [WeChat (Native)](/integrations/wechat-native) 및 [Alipay (Native)](/integrations/alipay-native) 커넥터 지원을 제거합니다. 대신 브라우저를 통해 작동하는 [WeChat (Web)](/integrations/wechat-web) 및 [Alipay (Web)](/integrations/alipay-web)을 사용할 수 있습니다. 네이티브 커넥터에 의존하는 경우 v2를 유지하세요.
