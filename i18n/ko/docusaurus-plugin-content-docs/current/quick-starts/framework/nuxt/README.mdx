---
slug: /quick-starts/nuxt
sidebar_label: Nuxt
sidebar_custom_props:
  description: Nuxt는 웹 개발을 직관적이고 강력하게 만들어 주는 오픈 소스 프레임워크입니다.
language: js
official_link: https://nuxt.com
app_type: 전통적인 웹
framework: Nuxt
---

import FurtherReadings from '../../fragments/_further-readings.md';

import GetUserInformation from './_get-user-information.mdx';
import GuideTip from './_guide-tip.md';
import Installation from './_installation.mdx';
import Integration from './_integration.mdx';
import ApiResources from './api-resources/_index.mdx';

# Nuxt 3 애플리케이션에 인증 (Authentication)을 추가하세요

<GuideTip />

## 사전 준비 사항 \{#prerequisites}

- [Logto Cloud](https://cloud.logto.io) 계정 또는 [셀프 호스팅 Logto](/introduction/set-up-logto-oss).
- Logto 전통적인 웹 애플리케이션이 생성되어 있어야 합니다.

## 설치 \{#installation}

<Installation />

## 통합 \{#integration}

<Integration />

## 사용자 정보 가져오기 \{#get-user-information}

<GetUserInformation />

## API 리소스 및 조직 \{#api-resources-and-organizations}

<ApiResources />

## 미들웨어 또는 API 라우트에서 사용하기 \{#use-in-middleware-or-api-routes}

컴포저블 `useLogtoClient()` 및 `useLogtoUser()`는 미들웨어 또는 API 라우트에서는 사용할 수 없습니다. 대신 `logtoEventHandler()` 함수를 사용하여 Logto 클라이언트 및 기타 컨텍스트를 얻을 수 있습니다:

```ts title="/server/api/auth/get-access-token.ts"
import { logtoEventHandler } from '#logto';

export default defineEventHandler(async (event) => {
  const config = useRuntimeConfig(event);
  await logtoEventHandler(event, config);
  const accessToken = await event.context.logtoClient.getAccessToken();
  return { accessToken };
});
```

## 추가 자료 \{#further-readings}

<FurtherReadings />
