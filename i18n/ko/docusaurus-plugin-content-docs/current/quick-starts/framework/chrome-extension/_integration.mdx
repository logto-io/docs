...
Chrome 확장의 팝업에 "로그인" 버튼을 넣었다고 가정하면, 인증 흐름은 다음과 같이 보일 것입니다:
...
확장의 다른 상호작용 페이지에 대해서는 `확장 팝업` 참가자를 페이지 이름으로 교체하기만 하면 됩니다. 이 튜토리얼에서는 팝업 페이지에 집중할 것입니다.
...
팝업이나 옵션 페이지와 같은 일반 확장 페이지는 백그라운드에서 실행될 수 없으며, 인증 과정 중에 닫힐 가능성이 있습니다. 백그라운드 스크립트는 인증 과정이 적절히 처리될 수 있도록 보장합니다.
:::
...
위 코드에서 두 개의 리디렉션 URI가 사용된 것을 알 수 있습니다. 이들은 모두 `chrome.identity.getRedirectURL`에 의해 생성된 것으로, 인증 흐름을 위한 리디렉션 URL을 생성하는 [Chrome의 내장 API](https://developer.chrome.com/docs/extensions/reference/api/identity#method-getRedirectURL)입니다. 두 URI는 다음과 같습니다:
...
`popup.js` 파일에서 (`popup.html`에 `popup.js`가 포함되어 있다고 가정):
...
`popup.js`에서 백그라운드 스크립트에서 생성한 `LogtoClient` 인스턴스를 재사용하거나 동일한 구성으로 새 인스턴스를 생성할 수 있습니다:
...
- **서비스 워커 번들링**: Webpack 또는 Rollup과 같은 번들러를 사용하는 경우, Node.js 모듈의 불필요한 번들링을 피하기 위해 명시적으로 타겟을 `browser` 또는 유사한 것으로 설정해야 합니다.
- **모듈 해상도**: Logto Chrome 확장 SDK는 ESM 전용 모듈입니다.

TypeScript, Rollup 및 기타 설정이 포함된 완전한 예제를 보려면 [샘플 프로젝트](https://github.com/logto-io/js/tree/HEAD/packages/chrome-extension-sample)를 참조하세요.
