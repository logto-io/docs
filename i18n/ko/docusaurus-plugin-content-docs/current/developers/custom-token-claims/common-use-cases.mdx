...
이 섹션에서는 [커스텀 액세스 토큰 클레임](/developers/custom-token-claims)이 유용하게 사용될 수 있는 몇 가지 시나리오 예시를 제공하여 참고할 수 있도록 도와드립니다. 이를 통해 접근 관리에 어려움을 겪을 때 커스텀 액세스 토큰 클레임이 편리함을 제공할 수 있는지 평가할 수 있습니다.
...
예를 들어, 앱을 개발 중이고 앱 출시가 공개 베타와 공식 출시의 두 단계로 나뉘어 있다고 가정해봅시다. 앱이 공식적으로 출시된 후에도, 공개 베타에 참여했던 기존 사용자들이 유료 기능을 계속 사용할 수 있도록 하고 싶습니다.
...
1. RBAC 컨텍스트에서 유료 리소스에 접근할 수 있는 스코프를 가지고 있는 경우.
2. `createdAt`이 공개 베타 종료 시점보다 이전인 경우.
...
비슷한 시나리오로, 사용자의 생일이 다가오면 로그인 페이지에 생일 축하 메시지를 표시하고 싶다고 가정해봅시다. 커스텀 토큰 클레임을 사용하여 [토큰 페이로드](/user-management/personal-access-token#example-token-exchange)에 생일 필드를 추가하고, 이를 통해 특정 메시지를 표시할지 여부를 판단할 수 있습니다.
...
이 게임은 게임 시간을 구매하기 위해 충전이 필요하다고 가정합니다. Joe는 각 사용자의 잔액을 자신의 게임 서비스에 기록하고, 게임 시간이 누적될수록 잔액을 계속 차감합니다. Joe는 계정 잔액이 소진되면 플레이어가 강제로 로그아웃되어 재충전을 유도하고 싶어합니다.

이때, Joe는 Logto가 제공하는 커스텀 토큰 클레임 기능을 활용할 수 있습니다:
