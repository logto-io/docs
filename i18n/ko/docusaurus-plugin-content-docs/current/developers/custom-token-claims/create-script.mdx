...
[커스텀 클레임 (Claim)](/developers/custom-token-claims)을 [액세스 토큰 (Access token)](https://auth.wiki/access-token)에 추가하려면, 해당 클레임을 포함하는 객체를 반환하는 스크립트를 제공해야 합니다. 이 스크립트는 커스텀 클레임이 포함된 객체를 반환하는 `JavaScript` 함수로 작성되어야 합니다.

1. <CloudLink to="/customize-jwt">콘솔 > 커스텀 JWT</CloudLink>로 이동하세요.
2. 커스텀 액세스 토큰 클레임을 커스터마이즈할 수 있는 액세스 토큰에는 두 가지 유형이 있습니다:
...
- 프로덕션 환경이 적용된 Logto Cloud 유료 테넌트 ( [Pro 테넌트 및 엔터프라이즈 테넌트](https://logto.io/pricing) 포함)
  :::
...
이 에디터는 JavaScript 언어 서버를 사용하여 기본 구문 강조, 코드 완성, 오류 검사를 제공합니다. 입력 파라미터는 잘 타입 지정되어 있으며 jsDoc 스타일로 문서화되어 있습니다. 에디터의 IntelliSense를 활용하여 입력 객체의 속성에 올바르게 접근할 수 있습니다. 상세 파라미터 정의는 페이지 우측에서 확인할 수 있습니다.
...
토큰 페이로드 객체 및 사용자 데이터 객체의 상세 타입 정의는 페이지 우측에서 확인할 수 있습니다. 에디터의 IntelliSense를 통해 입력 객체의 속성에 올바르게 접근할 수 있습니다.
...
- **사용자 데이터 객체**
  사용자 액세스 토큰의 경우, Logto는 추가적인 사용자 데이터 컨텍스트를 제공합니다. 사용자 데이터 객체에는 커스텀 클레임 설정에 필요한 모든 사용자 프로필 데이터와 조직 멤버십 데이터가 포함되어 있습니다. 자세한 내용은 [사용자](/user-management/user-data) 및 [조직](/organizations/organization-management#organization-data-structure)을 참고하세요.
- **조직 데이터 객체**
  [조직 액세스 토큰](/authorization/organization-template) (특정 조직을 위해 요청된 액세스 토큰)의 경우, Logto는 `context.organization`을 통해 대상 조직을 제공합니다. 이를 통해 조직별 클레임을 추가할 수 있습니다. 예를 들어, Logto 조직 ID를 조직의 커스텀 데이터에 저장된 내부 ID로 매핑할 수 있습니다. 이 데이터 객체는 토큰이 특정 조직을 위해 발급될 때만 존재하며, 다음을 포함합니다:
  ...
  자세한 내용은 [조직](/organizations/organization-management#organization-data-structure)을 참고하세요.

- **Grant 데이터 객체**
  사용자 가장 (User impersonation) 토큰 교환으로 부여된 사용자 액세스 토큰의 경우, Logto는 추가적인 grant 데이터 컨텍스트를 제공합니다. grant 데이터 객체에는 subject 토큰의 커스텀 컨텍스트가 포함되어 있습니다. 자세한 내용은 [사용자 가장](/developers/user-impersonation)을 참고하세요.
- **사용자 상호작용 데이터 객체**
  특정 사용자 액세스 토큰의 경우, 현재 인가 (Authorization) 세션에 대한 사용자의 상호작용 세부 정보를 접근해야 할 때가 있습니다. 예를 들어, 사용자가 로그인에 사용한 엔터프라이즈 SSO 아이덴티티를 조회해야 할 수 있습니다. 이 사용자 상호작용 데이터 객체에는 최근 사용자가 제출한 상호작용 데이터가 포함되어 있습니다. 주요 속성은 다음과 같습니다:

  | 속성                  | 설명                                                                            | 타입                     |
  | --------------------- | ------------------------------------------------------------------------------- | ------------------------ |
  | `interactionEvent`    | 현재 사용자 상호작용의 이벤트                                                   | `SignIn` 또는 `Register` |
  | `userId`              | 현재 사용자 상호작용의 사용자 ID                                                | `string`                 |
  | `verificationRecords` | 상호작용 중 사용자가 아이덴티티를 식별 및 검증하기 위해 제출한 검증 레코드 목록 | `VerificationRecord[]`   |
  ...
  :::note
  사용자 상호작용 데이터 객체에는 여러 개의 검증 레코드가 존재할 수 있습니다. 특히 사용자가 여러 번 로그인 또는 회원가입 과정을 거친 경우에 그렇습니다.

  예를 들어, 사용자가 `Social` 검증 레코드로 로그인한 후, `EmailVerificationCode` 검증 레코드로 새 이메일 주소를 바인딩하고, `Totp` 검증 레코드로 MFA 상태를 검증한 경우가 있습니다. 이 경우, 스크립트에서 모든 검증 레코드를 적절히 처리해야 할 수 있습니다.

  각 검증 레코드 타입은 사용자 상호작용 데이터 객체에 한 번만 존재합니다.
  :::

### environmentVariables \{#environmentvariables}

우측의 **환경 변수 설정** 섹션을 사용하여 스크립트에 사용할 환경 변수를 설정하세요. 이 변수들은 스크립트 내에서 하드코딩하지 않고 민감한 정보나 설정 데이터를 저장하는 데 사용할 수 있습니다. 예: API 키, 시크릿, URL 등.
...
:::note
테스트 결과는, 여러분이 설정한 모의 데이터로 `getCustomJwtClaims` 함수가 실행된 출력입니다 ("추가 토큰 클레임"은 [시퀀스 다이어그램](/developers/custom-token-claims/#how-do-custom-token-claims-work) 3단계 완료 후 얻은 값). 실제 토큰 페이로드 및 사용자 데이터 컨텍스트는 토큰 발급 과정에서 실행될 때와 다를 수 있습니다.
:::
