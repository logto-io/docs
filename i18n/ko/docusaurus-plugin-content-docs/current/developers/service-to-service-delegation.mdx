...
일부 API 아키텍처에서는 백엔드 서비스가 로그인한 사용자로부터 요청을 받고, 사용자의 아이덴티티를 유지한 채로 또 다른 백엔드 서비스를 호출해야 할 때가 있습니다.
...
sequenceDiagram
    ...
    Note over ApiA: API A용 사용자 토큰 검증

    ApiA->>Logto: POST /oidc/token (토큰 교환 그랜트)
    Note over ApiA,Logto: subject_token = 사용자 액세스 토큰<br/>resource = API B 리소스 지표

    Logto-->>ApiA: API B용 액세스 토큰
...
4. API A의 애플리케이션에서 토큰 교환을 활성화하세요.
...
2. `subject_token`: API A가 받은 원래 Logto 발급 사용자 액세스 토큰
3. `subject_token_type`: `urn:ietf:params:oauth:token-type:access_token` 사용
4. `resource`: API B의 리소스 지표
5. `scope`: 이 위임 호출에서 API A가 요청하는 다운스트림 권한. Logto는 RBAC 설정에 따라 원래 사용자가 이 리소스에 대해 가질 수 있는 요청된 스코프만 발급합니다.
...
이제 API A는 교환된 토큰으로 API B를 호출합니다:
