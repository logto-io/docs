...
| Organization.Membership.Updated | 사용자 또는 애플리케이션이 조직에 추가되거나 제거되었습니다. 페이로드에는 [멤버십 변경 필드](/developers/webhooks/webhooks-request#organizationmembershipupdated-payload)가 포함됩니다. |
...
| 사용자 등록                                                                                                                                             | User.Created                    |
| [Just-in-time 프로비저닝](/organizations/just-in-time-provisioning) (이메일 도메인 또는 엔터프라이즈 SSO 커넥터 일치)로 조직에 자동 프로비저닝된 사용자 | Organization.Membership.Updated |
