...
reCAPTCHA Enterprise는 고급 봇 감지 기술을 사용하여 사용자 경험을 방해하지 않으면서 웹사이트를 사기 및 남용으로부터 보호하는 Google 서비스입니다. 이 가이드에서는 Logto와 함께 reCAPTCHA Enterprise를 설정하는 과정을 안내합니다.
...
3. 다음 세부 정보로 양식을 작성하세요:
   ...
   - **Domain list**: Logto의 엔드포인트 도메인 추가
   - **Verification type**: **Score-based (invisible)** 또는 **Checkbox challenge** 중에서 선택하세요. 이는 reCAPTCHA가 사용자에게 어떻게 표시될지 결정합니다. 자세한 내용은 [Verification mode](#verification-mode)를 참조하세요.
...
5. "Application restrictions"가 무엇인지 모를 경우 **None**으로 두세요.
...
[Bring your UI](/customization/bring-your-ui/)를 사용하는 경우, Logto는 reCAPTCHA를 자동으로 커스텀 프론트엔드에 삽입하거나 실행할 수 없습니다. Logto Console에서 CAPTCHA를 활성화한 후, 커스텀 UI에서 reCAPTCHA Enterprise 스크립트를 로드하고, CAPTCHA 토큰을 받아 Experience API로 전송해야 합니다.

**Invisible** 모드의 경우, 사이트 키로 스크립트를 로드하세요:

```html
<script src="https://www.google.com/recaptcha/enterprise.js?render=<siteKey>" async defer></script>
```

Logto에서 커스텀 도메인을 구성했다면, `www.google.com`을 해당 도메인(예: `recaptcha.net`)으로 교체하세요.

상호작용을 시작하기 전에, 고정된 action 값 `interaction`으로 reCAPTCHA를 실행하고, 반환된 토큰을 `PUT /api/experience`의 `captchaToken`으로 전달하세요:
...
**Checkbox** 모드의 경우, `render=explicit`로 스크립트를 로드하고, 페이지에 위젯을 렌더링한 뒤, 콜백 토큰을 `PUT /api/experience` 호출 시 `captchaToken`으로 사용하세요.
...
CAPTCHA 공급자를 설정한 후, 반드시 CAPTCHA 봇 보호 기능을 활성화해야 합니다.
