...
:::note
CAPTCHA는 식별자, 비밀번호, 인증 코드, 회원가입, 비밀번호 복구 작업에 적용됩니다. [매직 링크](/end-user-flows/one-time-token) 또는 [패스키 로그인](/end-user-flows/sign-up-and-sign-in/passkey-sign-in)에는 적용되지 않으므로, 매직 링크나 패스키로 로그인을 완료한 사용자는 추가적인 CAPTCHA 챌린지를 해결할 필요가 없습니다.
:::

:::note
[Bring your UI](/customization/bring-your-ui/)를 사용하는 경우, 내장된 CAPTCHA 통합은 사용자 정의 프론트엔드에서 실행되지 않습니다. 사용자 정의 UI는 CAPTCHA 제공업체 스크립트를 로드하고, 챌린지를 실행하며, 토큰을 `PUT /api/experience`의 `captchaToken`으로 전송해야 합니다.
:::
...
5. **(선택 사항) CAPTCHA 활성화**: 제공업체가 구성되면 보안 페이지에서 CAPTCHA가 자동으로 활성화됩니다. 필요에 따라 수동으로 설정을 확인하거나 조정할 수 있습니다.
