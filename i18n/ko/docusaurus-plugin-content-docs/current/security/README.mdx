...
현대의 인증 (Authentication) 보안은 피싱, 크리덴셜 스터핑, 무차별 대입 공격, 랜섬웨어, DDoS, 그리고 AI 기반 공격 등 다양한 위협과 싸우고 있습니다. 사용자 아이덴티티를 보호하는 것은 브랜드 신뢰와 규정 준수를 지키는 데 매우 중요합니다.

Logto는 이러한 위험에 정면으로 대응할 수 있도록 설계된 강력한 보안 접근 관리 기능을 제공합니다. 사전 예방적 위협 방지와 복원력을 우선시함으로써, 사용성을 해치지 않으면서도 시스템을 안전하게 보호합니다. Logto와 함께라면 보안은 사후 고려 사항이 아니라, 비즈니스가 번창할 수 있는 기반이 됩니다. 위협이 진화하는 시대에 방어도 더 빠르게 진화할 수 있도록 지원합니다.
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        '허용 목록과 블록리스트를 사용하여 신규 가입 및 새로 연결된 이메일을 제어하고, 일회용 또는 원치 않는 주소를 차단하세요.',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    ...
    },
    {
      ...
      description:
        '수신자별 인증 코드 및 메시지 발송 횟수를 제한하여 받은 편지함 스팸 및 SMS 펌핑 남용을 방지하세요.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      ...
      description:
        '계정 상태를 숨겨 계정 열거 공격을 차단하고 민감한 계정 상태 정보를 노출하지 않도록 하세요.',
      customProps: {
        icon: <HideIcon />,
      },
    },
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      href: '/end-user-flows',
      description:
        '초대된 사용자만 가입할 수 있도록 제한하고, 이메일 매직 링크로 안전하게 온보딩하세요.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
