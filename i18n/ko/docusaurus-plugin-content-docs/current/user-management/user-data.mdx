...
다음은 Facebook에 로그인하여 가져온 사용자의 데이터 샘플입니다:
...
사용자 프로필은 <CloudLink to="/users">Logto Console</CloudLink> 또는 Logto Management API를 사용하여 조회할 수 있습니다. 예를 들어 [`GET /api/users/:userId`](https://openapi.logto.io/operation/operation-getuser).
...
*id*는 Logto에서 사용자를 식별하기 위한 고유하게 자동 생성된 키입니다.

### username \{#username}

*username*은 *username*과 비밀번호로 로그인하는 데 사용됩니다.

이 값은 사용자가 처음 등록할 때 입력한 사용자 이름에서 가져옵니다. `null`일 수 있습니다. null이 아닌 값은 128자를 초과하지 않아야 하며, 문자, 숫자, 밑줄 (`_`)만 포함해야 하며 숫자로 시작해서는 안 됩니다. 대소문자를 구분합니다.
...
이 값은 사용자가 처음 등록할 때 입력한 전화번호에서 가져옵니다. `null`일 수 있습니다. null이 아닌 값은 [국가 전화 코드](https://en.wikipedia.org/wiki/List_of_country_calling_codes)로 시작하는 숫자를 포함해야 합니다 (플러스 기호 `+` 제외).
...
*avatar*는 사용자의 아바타 이미지로 가리키는 URL입니다. 최대 길이는 2048입니다.
...
*has_password*는 사용자가 비밀번호를 가지고 있는지를 나타내는 불리언 값입니다. 이 상태는 <CloudLink to="/users">Console > 사용자 관리</CloudLink>의 세부 페이지에서 새 비밀번호 설정 또는 비밀번호 재설정을 포함하여 볼 수 있고 관리할 수 있습니다.
...
*is_suspended*는 사용자가 정지되었는지를 나타내는 불리언 값입니다. 이 값은 [Logto Management API](https://openapi.logto.io/operation/operation-updateuserissuspended)를 호출하거나 Logto Console을 사용하여 관리할 수 있습니다.
...
*mfa_verification_factors*는 사용자의 계정과 연관된 [다단계 인증 (MFA)](/end-user-flows/mfa) 방법을 나열하는 배열입니다. 가능한 값은 _Totp_ (인증 앱 OTP), _WebAuthn_ (패스키), *BackupCode*를 포함합니다.
...
사용자의 계정은 소셜 로그인으로 여러 소셜 아이덴티티 제공자와 연결될 수 있으며, 이러한 제공자로부터 가져온 해당 사용자 정보는 _identities_ 객체에 저장됩니다.
...
*sso_identities*는 [엔터프라이즈 SSO](/end-user-flows/enterprise-sso) (즉, [엔터프라이즈 커넥터](/connectors/enterprise-connectors)로 싱글 사인온 (SSO) 로그인)에서 가져온 사용자 정보를 포함합니다. 각 사용자의 *ssoIdentities*는 개별 JSON 객체에 저장됩니다.
...
- 사용자가 특정 작업을 수행했는지 여부를 기록합니다. 예: 환영 페이지를 본 적이 있는지.
- 사용자 프로필에 애플리케이션별 데이터를 저장합니다. 예: 애플리케이션별 사용자의 선호 언어 및 외관.
...
여전히 *custom_data*에 민감한 정보를 넣고 싶다면, 먼저 암호화할 것을 권장합니다. 신뢰할 수 있는 파티 (예: 백엔드 서비스)에서만 암호화 / 복호화하고, 프론트엔드 앱에서는 피하세요. 이렇게 하면 사용자의 *custom_data*가 실수로 유출되었을 때 손실을 최소화할 수 있습니다.
...
사용자의 *custom_data*를 업데이트하면 저장소의 원래 내용이 완전히 덮어쓰여집니다.

예를 들어, _custom_data_ API 업데이트 호출의 입력이 다음과 같다면 (원래 *custom_data*가 이전에 표시된 샘플 데이터라고 가정):
...
다음 DB 사용자 테이블 열 ( _password_encrypted_ 및 _password_encryption_method_ 제외)은 사용자 프로필에 표시됩니다. 이는 [Management API](https://openapi.logto.io/operation/operation-getuser)를 사용하여 조회할 수 있음을 의미합니다.
...
| [avatar](/user-management/user-data#avatar)                                         | string    | 사용자의 아바타 이미지를 가리키는 URL   | ❌   | ❌   |
