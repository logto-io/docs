...
개인 액세스 토큰 (PAT, Personal access tokens)은 사용자가 자격 증명과 상호작용 로그인 없이 [액세스 토큰 (Access token)](https://auth.wiki/access-token)을 안전하게 부여할 수 있는 방법을 제공합니다. 이는 CI/CD, 스크립트, 또는 리소스에 프로그래밍 방식으로 접근해야 하는 애플리케이션에 유용합니다.
...
PAT를 사용하여 획득한 액세스 토큰은 표준 `refresh_token` 플로우로 획득한 토큰과 **동일하게** 동작합니다. 즉:

- **조직 컨텍스트**: PAT로 획득한 토큰은 refresh token 플로우와 동일한 조직 권한 및 스코프를 지원합니다.
...
- **토큰 검증**: 동일한 검증 로직이 적용됩니다. 초기 grant type만 다릅니다.

조직과 함께 작업할 때, PAT 또는 리프레시 토큰 (Refresh token)을 사용하더라도 접근 패턴과 권한은 동일합니다.
...
전통적인 웹 애플리케이션 또는 앱 시크릿이 있는 기계 간 애플리케이션의 경우, HTTP Basic 인증을 사용하여 `Authorization` 헤더에 자격 증명을 포함하세요:
...
이후 [JWT 디코더](https://logto.io/jwt-decoder)로 액세스 토큰을 디코딩하면 다음과 같은 액세스 토큰 페이로드 예시를 얻을 수 있습니다:
...
<Url href="https://blog.logto.io/pat-and-api-keys">
  개인 액세스 토큰, 기계 간 인증 (Machine-to-Machine authentication), 그리고 API 키의 정의 및 실제
  시나리오
</Url>
