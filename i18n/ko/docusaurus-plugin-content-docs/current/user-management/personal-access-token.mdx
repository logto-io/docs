...
개인 액세스 토큰 (PAT, Personal access token)은 사용자가 자신의 자격 증명과 상호작용 로그인 없이 [액세스 토큰 (Access token)](https://auth.wiki/access-token)을 안전하게 부여할 수 있는 방법을 제공합니다. 이는 CI/CD, 스크립트, 또는 리소스에 프로그래밍 방식으로 접근해야 하는 애플리케이션에 유용합니다.
...
6. `subject_token_type`: 필수. `subject_token` 파라미터에 제공된 보안 토큰의 타입. 이 파라미터의 값은 `urn:logto:token-type:personal_access_token`이어야 합니다.
...
2. `issued_token_type`: 필수. 발급된 토큰의 타입. 이 파라미터의 값은 `urn:ietf:params:oauth:token-type:access_token`이어야 합니다.
3. `token_type`: 필수. 토큰의 타입. 이 파라미터의 값은 `Bearer`이어야 합니다.
