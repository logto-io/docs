...
PAT를 사용해 획득한 액세스 토큰은 표준 `refresh_token` 플로우로 획득한 토큰과 **동일하게** 동작합니다. 즉:

- **조직 컨텍스트**: PAT로 획득한 토큰은 refresh token 플로우와 동일한 조직 권한 및 스코프를 지원합니다.
- **인가 (Authorization) 플로우**: PAT로 교환한 액세스 토큰을 [조직 권한](/authorization/organization-permissions) 및 [조직 수준 API 리소스](/authorization/organization-level-api-resources)에 사용할 수 있습니다.
- **토큰 검증**: 동일한 검증 로직이 적용됩니다. 단, 최초 grant type만 다릅니다.
...
앱 시크릿이 없는 싱글 페이지 또는 네이티브 애플리케이션의 경우:
...
그런 다음 [JWT 디코더](https://logto.io/jwt-decoder)로 액세스 토큰을 디코딩하면 다음과 같은 액세스 토큰 페이로드 예시를 얻을 수 있습니다:
...
<Url href="https://blog.logto.io/pat-and-api-keys">
  개인 액세스 토큰, 기계 간 (Machine-to-Machine) 인증 (Authentication), 그리고 API 키의 정의와 실제
  시나리오
</Url>
