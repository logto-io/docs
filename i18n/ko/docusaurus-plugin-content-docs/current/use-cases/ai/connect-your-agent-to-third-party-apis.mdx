...
이 가이드는 AI 에이전트가 사용자를 대신하여 타사 API(예: Google Calendar, GitHub 등)에 접근할 수 있도록 설정하는 방법을 안내합니다. Logto의 소셜 커넥터와 Secret Vault를 활용하면 액세스 토큰을 안전하게 저장 및 관리할 수 있어, 에이전트가 반복적으로 사용자에게 재인증을 요청하지 않고 자동화된 작업을 수행할 수 있습니다.
...
이러한 작업을 수행하려면, AI 에이전트가 사용자로부터 권한을 부여받은 타사 API에 안전하게 접근해야 하며, 이를 위해 OAuth 토큰을 올바르고 안전하게 다루는 것이 중요합니다.

## 작동 방식 \{#how-it-works}

다음은 전체 흐름의 간단한 개요입니다:
...
AI 에이전트가 타사 API에 접근할 수 있도록, 토큰 저장이 활성화된 소셜 커넥터를 구성해야 합니다. 이를 통해 사용자가 AI 에이전트와 상호작용 중 타사 서비스를 인증할 때 Logto가 액세스 토큰을 저장 및 관리할 수 있습니다.
...
4. 커넥터 설정에서:
   ...
   - **Prompts**에 `consent`를 포함시켜 사용자가 권한 요청 화면을 볼 수 있도록 합니다.
   - **오프라인 접근**을 활성화하여 장기 API 접근을 위한 리프레시 토큰을 받습니다.
5. 변경 사항을 저장합니다.

:::info
이 커넥터를 로그인 경험에 추가할 필요는 없습니다. 이 커넥터는 사용자가 로그인할 때가 아니라, AI 에이전트가 타사 API에 접근해야 할 때 온디맨드로 사용됩니다.
:::

## 권한 요청 및 타사 API 접근하기 \{#request-authorization-and-access-third-party-apis}

AI 에이전트가 타사 API(예: Google Calendar)에 접근해야 할 때, 먼저 사용자가 이미 접근 권한을 부여했는지 확인해야 합니다. 권한이 없다면 사용자에게 권한 부여를 요청하세요.

:::info Account API 활성화
진행하기 전에 <CloudLink to="/sign-in-experience/account-center">콘솔 > 로그인 & 계정 > 계정 센터</CloudLink>에서 Account API를 활성화하세요. [Account API 활성화 방법](/end-user-flows/account-settings/by-account-api#how-to-enable-account-api)에 대해 자세히 알아보세요.
:::

### 1단계: 기존 권한 확인하기 \{#step-1-check-for-existing-authorization}

먼저, 저장된 액세스 토큰을 조회하여 사용자가 이미 권한을 부여했는지 확인합니다:
...
### 2단계: 권한이 없으면 요청하기 \{#step-2-request-authorization-if-needed}
...
Logto는 토큰 갱신을 자동으로 처리합니다. 액세스 토큰이 만료되었지만 리프레시 토큰이 존재한다면, 조회 엔드포인트 호출 시 Logto가 새 액세스 토큰을 투명하게 발급합니다.
...
### 점진적 권한 부여가 필요한 이유 \{#why-incremental-authorization}

- **더 나은 사용자 경험**: 사용자는 맥락에 맞게 권한이 필요한 이유를 이해할 때 더 쉽게 권한을 부여합니다.
- **높은 전환율**: 처음부터 많은 권한을 요청하지 않으면 마찰이 줄어듭니다.
- **신뢰 구축**: 필요한 권한만 요청하는 애플리케이션에 사용자는 더 많은 신뢰를 보냅니다.

### 예시: 읽기에서 쓰기로 권한 업그레이드 \{#example-upgrading-from-read-to-write-access}
...
AI 에이전트가 타사 API에 접근할 때 다음 보안 모범 사례를 지키세요:

- **최소 scope 요청**: 에이전트가 실제로 필요한 권한만 요청하세요.
- **점진적 권한 부여 사용**: 모든 권한을 한 번에 요청하지 말고, 맥락에 맞게 추가 권한을 요청하세요.
- **토큰 만료 처리**: 토큰이 만료되거나 철회된 경우를 항상 처리하세요.
