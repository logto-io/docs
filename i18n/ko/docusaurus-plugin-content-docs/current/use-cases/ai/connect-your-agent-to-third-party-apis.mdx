...
- 저장된 토큰을 조회 및 활용하여 타사 API에 접근하기
...
- **📅 캘린더 관리**: AI 에이전트가 Google Calendar에서 자동으로 미팅을 예약하거나, 이벤트를 추가하거나, 일정을 조정할 수 있습니다.
- **📧 이메일 자동화**: Gmail API를 사용해 후속 이메일을 보내거나, 받은 편지함을 정리하거나, 답장을 초안 작성할 수 있습니다.
- **💻 코드 관리**: GitHub 이슈 생성, 풀 리퀘스트 리뷰, 저장소 관리 등.
- **📁 파일 관리**: Google Drive 또는 Dropbox에서 파일 업로드, 정리, 공유 등.

이러한 작업을 수행하려면, AI 에이전트가 사용자가 승인한 타사 API에 안전하게 접근해야 하며, 이를 위해 OAuth 토큰을 올바르고 안전하게 처리해야 합니다.
...
- API 접근이 가능한 타사 제공자 계정 (예: [Google Cloud Console](https://console.cloud.google.com))
...
AI 에이전트가 타사 API에 접근할 수 있도록, 토큰 저장이 활성화된 소셜 커넥터를 구성해야 합니다. 이를 통해 사용자가 AI 에이전트와 상호작용 중 타사 서비스를 승인할 때 Logto가 액세스 토큰을 저장 및 관리할 수 있습니다.

Google을 예시로 들어 설명합니다:

1. <CloudLink to="/connectors/social">콘솔 > 커넥터 > 소셜 커넥터</CloudLink>로 이동합니다.
2. **소셜 커넥터 추가**를 클릭하고 **Google**을 선택합니다.
3. [Google 커넥터 설정 가이드](/integrations/google)를 참고하여 OAuth 클라이언트 자격 증명을 구성합니다.
4. 커넥터 설정에서:
   - **지속적인 API 접근을 위한 토큰 저장**을 활성화하여 Secret Vault에 토큰을 저장합니다.
   - **프롬프트**에 `consent`를 포함시켜 사용자가 권한 요청 화면을 볼 수 있도록 합니다.
   - **오프라인 접근**을 활성화하여 장기 API 접근을 위한 리프레시 토큰을 받습니다.
5. 변경 사항을 저장합니다.

:::info
이 커넥터를 로그인 경험에 추가할 필요는 없습니다. 이 커넥터는 사용자가 로그인할 때가 아니라, AI 에이전트가 타사 API 접근이 필요할 때 온디맨드로 인가 요청에 사용됩니다.
:::

## 인가 요청 및 타사 API 접근하기 \{#request-authorization-and-access-third-party-apis}

AI 에이전트가 타사 API(예: Google Calendar)에 접근해야 할 때, 먼저 사용자가 이미 접근을 승인했는지 확인해야 합니다. 승인되지 않았다면, 사용자에게 인가를 요청하세요.

:::info Account API 활성화
진행하기 전에 <CloudLink to="/sign-in-experience/account-center">콘솔 > 로그인 & 계정 > 계정 센터</CloudLink>에서 Account API를 활성화하세요. [Account API 활성화 방법](/end-user-flows/account-settings/by-account-api#how-to-enable-account-api)도 참고하세요.
:::

### 1단계: 기존 인가 여부 확인 \{#step-1-check-for-existing-authorization}

먼저, 저장된 액세스 토큰을 조회하여 사용자가 이미 인가했는지 확인합니다:
...
### 2단계: 필요 시 인가 요청 \{#step-2-request-authorization-if-needed}

토큰이 없거나, 만료되었거나, 액세스 토큰의 스코프를 확장해야 하는 경우 Logto의 [Social Verification API](/secret-vault/federated-token-set#reauthentication-and-token-renewal)를 사용해 인가 플로우를 시작하세요:
...
async function requestGoogleAuthorization(userAccessToken: string, scopes: string) {
  ...
  // verificationRecordId를 나중에 사용하기 위해 저장
  sessionStorage.setItem('verificationRecordId', verificationRecordId);

  // 사용자를 Google 인가 화면으로 리디렉션
  window.location.href = authorizationUri;
}
```

### 3단계: 인가 콜백 처리 \{#step-3-handle-the-authorization-callback}
...
### 4단계: 타사 API 호출 \{#step-4-call-the-third-party-api}
...
async function addCalendarEvent(userAccessToken: string, eventDetails: EventDetails) {
  ...
  if (!tokenData) {
    // 사용자가 인가하지 않은 경우, calendar scope로 인가 요청
...
Logto는 토큰 갱신을 자동으로 처리합니다. 액세스 토큰이 만료되었지만 리프레시 토큰이 존재하는 경우, 조회 엔드포인트를 호출하면 Logto가 자동으로 새 액세스 토큰을 발급합니다.

## 추가 권한 요청하기 \{#request-additional-permissions}

AI 에이전트가 더 많은 작업을 수행하게 되면, 추가 권한이 필요할 수 있습니다. 예를 들어, 사용자가 처음에는 읽기 전용 캘린더 접근만 승인했지만, 이제 이벤트 생성을 원한다면 쓰기 권한이 필요합니다.

### 점진적 인가가 필요한 이유 \{#why-incremental-authorization}

- **더 나은 사용자 경험**: 사용자는 맥락에 맞게 권한이 필요한 이유를 이해할 때 권한을 더 잘 부여합니다.
- **높은 전환율**: 처음부터 많은 권한을 요구하지 않으면 마찰이 줄어듭니다.
- **신뢰 구축**: 필요한 권한만 요청하는 애플리케이션에 사용자는 더 신뢰를 가집니다.

### 예시: 읽기에서 쓰기 권한으로 업그레이드 \{#example-upgrading-from-read-to-write-access}
...
async function createCalendarEvent(userAccessToken: string, eventDetails: EventDetails) {
  ...
  if (!tokenData) {
    // 아직 인가되지 않은 경우, 캘린더 쓰기 권한을 바로 요청
...
:::tip
추가 scope를 요청할 때, 사용자는 새로 요청되는 권한만 표시된 동의 화면을 보게 됩니다. 기존 권한은 그대로 유지됩니다.
:::

## 토큰 상태 관리하기 \{#manage-token-status}

Logto 콘솔에서 각 사용자의 토큰 상태를 확인할 수 있습니다:

1. <CloudLink to="/users">콘솔 > 사용자 관리</CloudLink>로 이동합니다.
2. 사용자를 클릭하여 상세 정보를 확인합니다.
3. **연결** 섹션으로 스크롤하여 연결된 모든 소셜 계정을 확인합니다.
...
- **최소 scope 요청**: 에이전트가 실제로 필요한 권한만 요청하세요.
- **점진적 인가 사용**: 모든 권한을 한 번에 요청하지 말고, 맥락에 맞게 추가 권한을 요청하세요.
- **토큰 만료 처리**: 토큰이 만료되거나 취소된 경우를 항상 처리하세요.
- **사용자 액세스 토큰 보호**: 사용자의 Logto 액세스 토큰은 타사 토큰을 조회하는 열쇠입니다. 반드시 안전하게 보호하세요.
