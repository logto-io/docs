...
description: 강력한 인증 (Authentication), 조직 관리, 역할 기반 접근 제어 (RBAC)로 효율적인 멀티 테넌트 SaaS 애플리케이션을 구축하는 방법을 알아보세요.
---
...
Notion, Slack, Figma 같은 앱은 어떻게 만들어질까요? 이런 멀티 테넌트 SaaS 애플리케이션은 사용하기는 간단해 보이지만, 직접 만들려고 하면 이야기가 달라집니다.

저도 처음에 이런 복잡한 시스템을 만들 생각을 했을 때 머리가 터질 것 같았습니다:

- 사용자는 여러 로그인 옵션이 필요함 (이메일, Google, GitHub)
- 각 사용자는 여러 조직에 가입 및 소속될 수 있음
- 각 조직 내에서 다른 권한 수준 필요
...
하지만 실제로 작업을 시작해보니, **생각보다 그렇게 어렵지 않다는 걸 알게 되었습니다.**
...
이제 이런 시스템을 처음부터 어떻게 설계하고 구현하는지 정확히 보여드리겠습니다. 2025년의 최신 도구와 올바른 아키텍처 접근법으로 얼마나 간단한지 놀라실 겁니다.

> **전체 소스 코드는 이 [Github Repo](https://github.com/logto-io/multi-tenant-saas-sample)에서 확인할 수 있습니다. 바로 시작해봅시다!**

우리는 DocuMind라는 AI 문서화 SaaS 제품을 예시로 시작합니다.
...
멀티 테넌트 아키텍처를 구현하려면 **조직**이라는 엔티티 계층이 필요합니다. 하나의 사용자 풀에서 여러 워크스페이스에 접근할 수 있다고 상상해보세요. 각 조직은 하나의 워크스페이스를 의미하며, 사용자는 단일 아이덴티티로 여러 워크스페이스(조직)에 역할에 따라 접근할 수 있습니다.

![multi-tenant-app-architecture.svg](https://uploads.strapi.logto.io/multi_tenant_app_architecture_89ed9779f8.svg)

이는 인증 (Authentication) 제공자에서 널리 사용되는 기능입니다. 아이덴티티 관리 시스템의 조직은 SaaS 앱의 워크스페이스, 프로젝트, 테넌트에 해당합니다.
...
예를 들어, Sarah가 이메일 **sarah@gmail.com**으로 앱에 가입했다고 가정합시다. 그녀는 여러 워크스페이스에 소속될 수 있습니다. Sarah가 **워크스페이스 A**에는 속하지만 **워크스페이스 B**에는 속하지 않는다면, 그녀는 **워크스페이스 A**의 멤버이고 **워크스페이스 B**의 멤버는 아닙니다.

### 역할 및 권한 설계 \{#role-and-permission-design}

멀티 테넌트 아키텍처에서는 사용자가 테넌트 리소스에 접근하기 위해 특정 **역할 (Role)**과 **권한 (Permission)**이 필요합니다.
권한 (Permission)은 `read: order` 또는 `write: order`와 같이 특정 리소스에 대해 수행할 수 있는 작업을 정의하는 세부적인 접근 제어입니다.

역할 (Role)은 멀티 테넌트 환경에서 멤버에게 할당되는 권한 (Permission) 집합입니다.

이 역할과 권한을 정의한 후, 사용자의 역할을 할당해야 하며, 때로는 자동화된 프로세스가 포함될 수 있습니다. 예를 들어:

1. 조직에 가입한 사용자는 자동으로 **멤버** 역할을 받습니다.
2. 워크스페이스를 처음 생성한 사용자는 자동으로 **관리자** 역할을 받습니다.
...
2. **비밀번호 없는 로그인**: 이메일 인증 코드로 간편하고 안전하게 접근
3. **계정 관리**: 사용자가 이메일, 비밀번호, 기타 정보를 업데이트할 수 있는 계정 센터
4. **소셜 로그인**: Google, GitHub 등 빠른 로그인 옵션
5. **다단계 인증 (MFA)**: Duo와 같은 인증 앱을 통한 로그인으로 보안 강화

### 테넌트 생성 및 초대 \{#tenant-creation-and-invitation}

멀티 테넌트 SaaS 앱에서는 사용자 플로우의 주요 차이점 중 하나가 테넌트 생성 및 멤버 초대를 지원해야 한다는 점입니다. 이 과정은 제품 활성화와 성장에 중요한 역할을 하므로 신중한 기획과 실행이 필요합니다.

다음은 고려해야 할 일반적인 사용자 플로우입니다:

| 사용자 유형                            | 진입 경로                                    |
| -------------------------------------- | -------------------------------------------- |
| 신규 계정                              | 로그인 및 회원가입 페이지에서 새 테넌트 생성 |
| 기존 계정                              | 제품 내에서 또 다른 테넌트 생성              |
| 기존 계정이 새 테넌트 초대를 받은 경우 | 로그인 및 회원가입 페이지에서 진입           |
| 기존 계정이 새 테넌트 초대를 받은 경우 | 초대 이메일에서 진입                         |
| 신규 계정이 새 테넌트 초대를 받은 경우 | 로그인 및 회원가입 페이지에서 진입           |
| 신규 계정이 새 테넌트 초대를 받은 경우 | 초대 이메일에서 진입                         |

이 시나리오들은 거의 모든 SaaS 앱에서 볼 수 있는 일반적인 예시입니다. 제품 및 디자인 팀에 영감을 주는 참고 자료로 활용하고, 필요에 따라 자체 플로우를 자유롭게 설계하세요.
...
모든 제품 요구 사항을 이해했다면, 이제 구현 단계로 넘어가겠습니다.
...
조직 시스템을 통해 사용자는 여러 조직을 생성하고 가입할 수 있습니다. 핵심 관계를 이해해봅시다:

```mermaid
erDiagram
    User ||--o{ OrganizationMember : "소속됨"
    Organization ||--o{ OrganizationMember : "가짐"
...
멀티 테넌트 앱에서는 권한 (Permission)과 역할 (Role) 설계가 제품 설계에서 비롯되어 일관된 경우가 많습니다. 예를 들어, 여러 워크스페이스에는 일반적으로 관리자 역할과 멤버 역할이 있습니다. Logto는 다음과 같은 조직 수준 역할 기반 접근 제어 (RBAC) 설계를 제공합니다:

1. **통합 권한 정의**: 권한 (Permission)은 시스템 레벨에서 정의되어 모든 조직에 일관되게 적용되어 유지보수와 관리가 용이합니다.
...
각 사용자는 각 조직 내에서 자신의 역할 (Role)이 필요하므로, 역할과 조직 간의 관계는 각 사용자에게 할당된 역할을 반영해야 합니다:
...
이제 조직 시스템과 접근 제어 시스템을 설계했으니, 본격적으로 제품을 만들어봅시다!

## 기술 스택 \{#tech-stack}

초보자도 쉽게 사용할 수 있고, 이식성이 좋은 스택을 선택했습니다:

1. **프론트엔드**: React (Vue/Angular/Svelte로도 쉽게 전환 가능)
2. **백엔드**: Express (간단하고 직관적인 API)

프론트엔드와 백엔드를 분리한 이유는 명확한 아키텍처로 배우기 쉽고, 스택 전환도 간단하기 때문입니다. 인증 (Authentication) 제공자는 예시로 Logto를 사용합니다.

이 가이드의 패턴은 **어떤 프론트엔드, 어떤 백엔드, 어떤 인증 시스템에도 적용 가능합니다.**

## 앱에 기본 인증 (Authentication) 플로우 추가 \{#add-basic-authentication-flow-to-your-app}

가장 쉬운 단계입니다. Logto를 프로젝트에 통합하기만 하면 됩니다. 이후 Logto 콘솔에서 원하는 로그인/회원가입 방식을 설정할 수 있습니다.
...
테넌트 콘솔에서 왼쪽의 "Application" 버튼을 클릭하세요. React를 선택해 애플리케이션을 시작합니다.

페이지의 안내를 따라 진행하면 약 5분 만에 Logto 통합을 완료할 수 있습니다!
...
꿀팁: 로그인 페이지에는 Sign in과 Register 버튼이 모두 있습니다. Register 버튼은 Logto의 회원가입 페이지로 바로 이동합니다. 이는 Logto의 [첫 화면](/end-user-flows/authentication-parameters/first-screen) 기능을 활용한 것입니다. 사용자가 인증 플로우에서 어떤 단계를 먼저 볼지 결정할 수 있습니다.

신규 사용자가 많을 것으로 예상된다면 기본값을 회원가입 페이지로 설정할 수 있습니다.
...
로그인 버튼을 클릭하면 Logto 로그인 페이지로 이동합니다. 로그인(또는 회원가입)에 성공하면 축하합니다! 앱의 첫 번째 사용자가 탄생했습니다(바로 당신!).
...
Logto 콘솔에서 왼쪽 메뉴의 "Sign-in & account"를 클릭하세요. "Sign-up and sign-in" 탭을 클릭합니다.
...
Logto에서는 MFA 활성화가 매우 간단합니다. Logto 콘솔에서 "Multi-factor auth" 버튼을 클릭한 후, 다단계 인증 페이지에서 활성화하세요.
...
## 멀티 테넌트 조직 경험 추가 \{#adding-multi-tenant-organization-experience}

이제 첫 번째 사용자가 생겼습니다! 하지만 이 사용자는 아직 어떤 조직에도 소속되어 있지 않고, 조직도 생성하지 않았습니다.

Logto는 멀티 테넌시를 기본적으로 지원합니다. Logto에서 원하는 만큼 조직을 생성할 수 있습니다. 각 조직은 여러 멤버를 가질 수 있습니다.
...
Logto에서 사용자의 조직 정보를 얻으려면 두 단계를 따르세요:
...
function Dashboard() {
  ...
  if (organizations.length === 0) {
    return <div>아직 어떤 조직의 멤버도 아닙니다</div>;
  }
...
아직 조직을 생성하지 않았고, 사용자가 어떤 조직에도 가입하지 않았으므로 대시보드에는 "아직 조직이 없습니다"가 표시됩니다.

![dashboard-no-orgs.png](https://uploads.strapi.logto.io/dashboard_no_orgs_94678fe16f.png)

이제 사용자를 위한 조직을 생성하고, 사용자를 조직에 추가해봅시다.

Logto 덕분에 복잡한 조직 관계를 직접 구축할 필요가 없습니다. Logto에서 조직을 생성하고 사용자를 추가하기만 하면 됩니다. Logto가 모든 복잡성을 처리해줍니다. 조직을 생성하는 방법은 두 가지입니다:

1. Logto 콘솔에서 수동으로 조직 생성
2. Logto Management API를 사용해 조직 생성(특히 사용자가 직접 조직(워크스페이스)을 만들 수 있는 SaaS 플로우 설계 시)

### Logto 콘솔에서 조직 생성하기 \{#create-organization-in-logto-console}

Logto 콘솔 왼쪽의 "Organizations" 메뉴 버튼을 클릭하세요. 조직을 생성합니다.
...
조직 상세 페이지로 이동하세요. Members 탭으로 전환합니다. "+ Add member" 버튼을 클릭하세요. 왼쪽 목록에서 로그인한 사용자를 선택합니다. 오른쪽 하단의 "Add members" 버튼을 클릭하세요. 이제 사용자가 조직에 성공적으로 추가되었습니다.

![console-add-member-to-orgs.png](https://uploads.strapi.logto.io/console_add_member_to_orgs_ad21beb58f.png)

앱 페이지를 새로고침하면 사용자가 조직에 소속된 것을 볼 수 있습니다!

![dashboard-has-orgs.png](https://uploads.strapi.logto.io/dashboard_has_orgs_568a44cc1f.png)

## 셀프 서비스 조직 생성 경험 구현 \{#implement-self-serve-organization-creation-experience}
...
자세한 내용은 [Management API와 상호작용하기](/integrate-logto/interact-with-management-api) 문서를 참고해 Logto와의 API 통신을 설정하세요.

### 조직 인증 상호작용 플로우 이해하기 \{#understand-organization-auth-interaction-flow}

조직 생성 플로우를 예로 들어보겠습니다. 조직 생성 과정은 다음과 같이 진행됩니다:
...
이 플로우에는 두 가지 주요 인증 (Authentication) 요구 사항이 있습니다:

1. **백엔드 서비스 API 보호**:
   - 프론트엔드에서 백엔드 서비스 API 접근 시 인증 필요
   - API 엔드포인트는 사용자의 Logto 액세스 토큰(Access token)으로 보호
   - 인증된 사용자만 서비스에 접근 가능
2. **Logto Management API 접근**:
   - 백엔드 서비스가 Logto Management API를 안전하게 호출해야 함
   - [Management API와 상호작용하기](/integrate-logto/interact-with-management-api) 가이드 참고
   - 기계 간 (Machine-to-machine) 인증으로 접근 자격 획득

### 백엔드 API 보호하기 \{#protect-your-backend-api}

먼저, 백엔드 서비스에 조직 생성용 API 엔드포인트를 만듭니다.

```jsx
app.post('/organizations', async (req, res) => {
  // Logto Management API 사용 구현
  // ...
});
```

백엔드 서비스 API는 인증된 사용자만 허용합니다. Logto를 사용해 API를 보호해야 하며, 현재 사용자 정보(예: user ID)도 알아야 합니다.

Logto(및 OAuth 2.0) 개념에서 백엔드 서비스는 리소스 서버 역할을 합니다. 사용자는 프론트엔드에서 Access token으로 DocuMind 리소스 서버에 접근합니다. 리소스 서버는 이 토큰을 검증하고, 유효하면 요청된 리소스를 반환합니다.

API 리소스를 생성해 백엔드 서비스를 대표하게 합시다.

Logto 콘솔에서:

1. 오른쪽의 "API resources" 버튼 클릭
2. "Create API resource" 클릭, 팝업에서 Express 선택
3. API 이름은 "DocuMind API", API 식별자는 "[https://api.documind.com](https://api.documind.com/)"로 입력
4. 생성 클릭

API 식별자 URL은 Logto 내에서 API를 고유하게 식별하는 용도일 뿐, 실제 백엔드 서비스 URL과는 무관합니다.

API 리소스 사용 튜토리얼이 표시됩니다. 해당 튜토리얼이나 아래 단계를 따라 진행하세요.

`requireAuth` 미들웨어를 만들어 POST /organizations 엔드포인트를 보호합시다.
...
Logto 테넌트의 OpenID Configuration 엔드포인트에서 이 값을 얻으세요. `https://<your-tenant-id>.logto.app/oidc/.well-known/openid-configuration`에 접속하면 반환된 JSON에서 정보를 확인할 수 있습니다:
...
프론트엔드에서 Logto에서 토큰을 받아올 수 있습니다. 사용자는 이 토큰으로 백엔드 서비스 API를 통해 조직을 생성할 수 있습니다. 미들웨어는 user ID도 제공합니다. 이는 사용자를 조직에 추가할 때 유용합니다.
...
대시보드에서 조직 생성 시 Logto 액세스 토큰을 받아와서 백엔드 서비스 API에 전달하세요.
...
프론트엔드에서 백엔드로 요청하는 것과 마찬가지로, 백엔드에서 Logto로 요청할 때도 액세스 토큰이 필요합니다.
...
Logto 콘솔의 애플리케이션 페이지로 이동해 Machine-to-Machine 애플리케이션을 생성하세요. "Logto Management API access" 역할을 할당합니다. Token endpoint, App ID, App Secret을 복사해두세요. 이 값으로 액세스 토큰을 발급받습니다.

![m2m-application.png](https://uploads.strapi.logto.io/m2m_application_d882b6386e.png)

이제 이 M2M 애플리케이션으로 Logto Management API 액세스 토큰을 받아올 수 있습니다.
...
사용할 Management API는 다음과 같습니다:

- `POST /api/organizations`: 조직 생성 ([Create organization API reference](https://openapi.logto.io/operation/operation-createorganization) 참고)
- `POST /api/organizations/{id}/users`: 조직에 사용자 추가 ([Add users to organization API reference](https://openapi.logto.io/operation/operation-addusers) 참고)
...
대시보드에서 이 기능을 테스트해봅시다.

![dashboard-create-org.png](https://uploads.strapi.logto.io/dashboard_create_org_b703f1125b.png)

"Create Organization"을 클릭하세요.

![dashboard-has-orgs.png](https://uploads.strapi.logto.io/dashboard_has_orgs_568a44cc1f.png)

생성 성공!

다음 단계는 조직에 사용자 초대 기능입니다. 이 튜토리얼에서는 구현하지 않지만, 이미 Management API 사용법을 익혔으니 [**테넌트 생성 및 초대**](#tenant-creation-and-invitation) 섹션을 참고해 쉽게 구현할 수 있습니다. 자세한 내용은 [멀티 테넌트 앱에서 사용자 협업 구현 방법](https://blog.logto.io/implement-user-collaboration-in-your-app) 블로그를 참고하세요.

## 멀티 테넌트 앱에 접근 제어 구현 \{#implement-access-control-to-your-multi-tenant-app}

이제 조직 접근 제어로 넘어가겠습니다.

우리가 달성하고자 하는 것은:

- 사용자는 자신의 조직에 속한 리소스만 접근 가능: Logto의 `조직 토큰`으로 구현
- 사용자는 조직 내에서 특정 역할 (Role)을 가지고, 각 역할에 따라 다른 권한 (Permission)으로 인가된 작업 수행: Logto의 조직 템플릿 기능으로 구현
...
프론트엔드 애플리케이션에서는 Logto의 `getOrganizationToken` 메서드로 특정 조직에 접근할 수 있는 토큰을 얻을 수 있습니다.
...
`getOrganization` 또는 조직 관련 기능을 사용하기 전에 Logto config에 `urn:logto:scope:organizations` 스코프와 `urn:logto:resource:organization` 리소스가 포함되어야 합니다. 이미 선언했으므로 반복하지 않겠습니다.

조직 페이지에서는 조직 토큰으로 해당 조직의 문서를 가져옵니다.
...
이 구현에서 주의할 점은 두 가지입니다:

1. `getOrganizationToken`에 전달된 `organizationId`가 현재 사용자가 소속된 조직 id가 아니라면, 이 메서드는 토큰을 발급하지 않습니다. 즉, 사용자는 자신의 조직에만 접근할 수 있습니다.
2. 조직 리소스 요청 시 액세스 토큰이 아닌 조직 토큰을 사용합니다. 조직에 속한 리소스는 사용자 권한이 아니라 조직 권한으로 접근 제어를 해야 하기 때문입니다(이후 `GET /documents` API 구현에서 더 잘 이해할 수 있습니다).

다음으로 백엔드 서비스에 `GET /documents` API를 만듭니다. 앞서 API 리소스로 `POST /organizations` API를 보호한 것처럼, 조직별 리소스 지표로 `GET /documents` API를 보호합니다.

먼저, 조직 리소스를 보호하는 `requireOrganizationAccess` 미들웨어를 만듭니다.
...
app.get('/documents', requireOrganizationAccess(), async (req, res) => {
  // req.user로 현재 사용자 id와 organizationId를 얻을 수 있습니다
...
이렇게 하면 조직 토큰으로 조직 리소스에 접근할 수 있습니다. 백엔드 서비스에서는 organization id로 데이터베이스에서 해당 리소스를 조회하면 됩니다.

조직 간 데이터 격리가 필요한 소프트웨어의 경우, 자세한 논의와 구현은 [PostgreSQL로 멀티 테넌시 구현: 실전 예제로 배우기](https://blog.logto.io/implement-multi-tenancy) 블로그를 참고하세요.

### 조직 수준 역할 기반 접근 제어 (RBAC) 설계 구현 \{#implement-organization-level-role-based-access-control-design}

조직 토큰으로 조직 리소스에 접근하는 기능을 구현했습니다. 이제 RBAC로 조직 내 사용자 권한 제어를 구현해봅시다.

DocuMind에는 Admin과 Collaborator 두 가지 역할이 있다고 가정합시다.

Admin은 문서를 생성 및 조회할 수 있고, Collaborator는 문서 조회만 가능합니다.
...
이제 사용자 권한을 확인해 인가를 구현할 수 있습니다.

코드에서는 사용자의 조직 토큰에 권한 정보를 포함시키고, 백엔드에서 이 권한을 검증해야 합니다.

프론트엔드 코드의 Logto config에서 조직 내에서 요청할 권한을 선언해야 합니다. `read:documents`와 `create:documents` 권한을 `scopes`에 추가하세요.
...
이제 백엔드의 `requireOrganizationAccess` 미들웨어에서 사용자 권한 검증을 추가합니다.
...
이렇게 하면 사용자 권한을 확인해 인가를 구현할 수 있습니다.

프론트엔드에서는 조직 토큰을 디코딩하거나 Logto의 `getOrganizationTokenClaims` 메서드로 사용자 권한 정보를 얻을 수 있습니다.
...
지금까지 멀티 테넌트 SaaS 시스템의 기본 사용자 및 조직 기능을 구현했습니다! 하지만 각 조직별 로그인 페이지 브랜딩, 특정 도메인 이메일 사용자를 자동으로 조직에 추가, 엔터프라이즈 SSO 통합 등 아직 다루지 않은 기능도 있습니다.
...
처음에는 얼마나 막막했는지 기억나시나요? 사용자, 조직, 권한, 엔터프라이즈 기능... 끝없는 산처럼 느껴졌을 겁니다.

하지만 우리가 해낸 것을 보세요:

- 다양한 로그인 옵션과 MFA 지원이 포함된 완벽한 인증 (Authentication) 시스템
...
이 튜토리얼의 전체 소스 코드는 [Multi-tenant SaaS Sample](https://github.com/logto-io/multi-tenant-saas-sample)에서 확인할 수 있습니다.

이것이 바로 2025년 현대 개발의 힘입니다. 인프라와 씨름하지 않고, 고유한 제품 기능 개발에 집중할 수 있습니다. 이제 여러분 차례입니다. 멋진 것을 만들어보세요!
