...
Notion, Slack, Figma 같은 앱은 어떻게 만들어질까요? 이런 멀티 테넌트 SaaS 애플리케이션은 사용하기엔 간단해 보이지만, 직접 만들려고 하면 얘기가 달라집니다.
...
하지만 실제로 작업을 시작하니, **생각보다 그렇게 두렵지 않다는 걸 알게 됐습니다.**
...
> **전체 소스 코드는 이 [Github 저장소](https://github.com/logto-io/multi-tenant-saas-sample)에서 확인할 수 있습니다. 바로 시작해 볼까요!**
...
먼저, 필요한 요구 사항을 정리해봅시다. 어떤 기능이 필요할까요?

### 멀티 테넌트 아키텍처 \{#multi-tenant-architecture}

멀티 테넌트 아키텍처를 구현하려면 **조직**이라는 엔티티 계층이 필요합니다. 하나의 사용자 풀에서 여러 워크스페이스에 접근할 수 있다고 상상해보세요. 각 조직은 하나의 워크스페이스를 의미하며, 사용자는 역할에 따라 다양한 워크스페이스(조직)에 단일 아이덴티티로 접근할 수 있습니다.
...
### 역할 (Role) 및 권한 (Permission) 설계 \{#role-and-permission-design}

멀티 테넌트 아키텍처에서는 사용자가 테넌트 리소스에 접근할 수 있도록 특정 **역할 (Role)**과 **권한 (Permission)**이 필요합니다.
권한 (Permission)은 `read: order` 또는 `write: order`와 같이 특정 작업을 정의하는 세부적인 접근 제어입니다. 이는 특정 리소스에 대해 어떤 작업을 수행할 수 있는지를 결정합니다.

역할 (Role)은 멀티 테넌트 환경에서 멤버에게 할당되는 권한 (Permission) 집합입니다.

이러한 역할과 권한을 정의하고, 사용자를 역할에 할당해야 하며, 때로는 자동화된 프로세스가 포함될 수 있습니다. 예를 들어:
...
사용자 친화적이고 안전한 회원가입 및 인증 (Authentication) 프로세스를 보장해야 합니다. 기본 로그인 및 회원가입 옵션은 다음과 같습니다:
...
멀티 테넌트 SaaS 앱에서는 사용자 플로우의 중요한 차이점이 테넌트 생성 및 멤버 초대 지원입니다. 이 과정은 제품 활성화와 성장에 핵심적인 역할을 하므로 신중한 기획과 실행이 필요합니다.
...
이러한 시나리오는 거의 모든 SaaS 앱에서 볼 수 있습니다. 제품 및 디자인 팀에 참고 자료로 활용하고, 필요에 따라 자체 플로우를 자유롭게 만드세요.
...
최신 인증 (Authentication) 제공자(이번에는 [Logto](https://logto.io/)를 선택합니다)가 이 모든 기능을 패키지로 제공합니다. 인증 (Authentication) 플로우는 매우 간단합니다:
...
**몇 주 걸릴 개발이 15분 만에 설정 완료!** Logto가 모든 복잡한 플로우를 처리해줍니다. 통합 단계는 뒤에서 다루겠습니다. 이제 DocuMind의 핵심 기능 개발에 집중할 수 있습니다!
...
멀티 테넌트 앱에서는 권한 (Permission)과 역할 (Role) 설계가 제품 설계에서 비롯되므로 대체로 일관적입니다. 여러 워크스페이스에서는 일반적으로 관리자 역할과 멤버 역할이 있습니다. Logto는 다음과 같은 조직 레벨 역할 기반 접근 제어 설계를 제공합니다:

1. **통합 권한 정의**: 권한은 시스템 레벨에서 정의되어 모든 조직에 일관되게 적용되어 유지 관리가 쉽고 일관된 권한 관리가 가능합니다.
2. **조직 템플릿**: 미리 정의된 역할과 권한 조합을 조직 템플릿으로 제공하여 조직 초기화를 간소화합니다.
...
각 사용자는 각 조직 내에서 자신만의 역할이 필요하므로, 역할과 조직 간의 관계는 각 사용자에게 할당된 역할을 반영해야 합니다:
...
초보자도 쉽게 접근할 수 있는 이식성 높은 스택을 선택했습니다:

1. **프론트엔드**: React (Vue/Angular/Svelte로도 쉽게 전환 가능)
2. **백엔드**: Express (간단하고 직관적인 API)

프론트엔드와 백엔드를 분리한 이유는? 명확한 아키텍처로 배우기 쉽고 스택 전환도 간단하기 때문입니다. 인증 (Authentication) 제공자로는 Logto를 예시로 사용합니다.

이후 가이드의 패턴은 **어떤 프론트엔드, 어떤 백엔드, 어떤 인증 시스템에도 적용됩니다.**

## 앱에 기본 인증 (Authentication) 플로우 추가 \{#add-basic-authentication-flow-to-your-app}

이 단계는 가장 쉽습니다. Logto를 프로젝트에 통합하기만 하면 됩니다. 그 후 Logto 콘솔에서 필요에 따라 사용자 로그인/회원가입 방식을 설정할 수 있습니다.

### 앱에 Logto 설치 \{#install-logto-to-your-app}
...
페이지의 안내를 따라 진행하면 Logto 통합을 약 5분 만에 완료할 수 있습니다!
...
꿀팁: 로그인 페이지에는 Sign in과 Register 버튼이 모두 있습니다. Register 버튼은 Logto의 회원가입 페이지로 바로 이동합니다. 이는 Logto의 [first screen](/end-user-flows/authentication-parameters/first-screen) 기능을 활용한 것입니다. 사용자가 인증 (Authentication) 플로우에서 처음 보게 될 단계를 결정합니다.
...
Logto 콘솔에서 왼쪽 메뉴의 "Sign-in Experience"를 클릭하세요. "Sign-up and sign-in" 탭을 클릭합니다.
...
Logto에서는 MFA 활성화가 매우 간단합니다. Logto 콘솔에서 "Multi-factor auth" 버튼을 클릭한 뒤, 다단계 인증 (MFA) 페이지에서 활성화하세요.
...
모든 것이 정말 간단합니다! 몇 분 만에 복잡한 사용자 인증 (Authentication) 시스템을 구축했습니다!
...
각 사용자는 Logto에서 자신의 조직 정보를 가져올 수 있습니다. 이를 통해 멀티 테넌시 지원이 가능합니다.

### 사용자의 조직 정보 가져오기 \{#get-a-user-s-organization-information}

Logto에서 사용자의 조직 정보를 가져오려면 두 가지 단계를 따르세요:
...
이 단계를 완료한 후에는 로그아웃 후 다시 로그인해야 합니다. 요청하는 scope와 resource를 수정했기 때문입니다.

아직 조직을 생성하지 않았으므로, 사용자는 어떤 조직에도 소속되어 있지 않습니다. 대시보드에는 "아직 조직이 없습니다"가 표시됩니다.
...
2. Logto Management API를 사용해 조직 생성(사용자가 직접 조직을 생성하는 SaaS 플로우 설계 시)

### Logto 콘솔에서 조직 생성 \{#create-organization-in-logto-console}
...
조직 상세 페이지로 이동하세요. Members 탭으로 전환합니다. "+ 멤버 추가" 버튼을 클릭하세요. 왼쪽 목록에서 로그인한 사용자를 선택합니다. 오른쪽 하단의 "Add members" 버튼을 클릭하세요. 이제 사용자가 조직에 성공적으로 추가되었습니다.

![console-add-member-to-orgs.png](https://uploads.strapi.logto.io/console_add_member_to_orgs_ad21beb58f.png)

앱 페이지를 새로고침하면 사용자가 이제 조직에 소속된 것을 볼 수 있습니다!
...
설정 방법은 [Management API와 상호작용하기](/integrate-logto/interact-with-management-api) 문서를 참고하세요.

### 조직 인증 (Authentication) 상호작용 플로우 이해 \{#understand-organization-auth-interaction-flow}
...
2. **Logto Management API 접근**:
   ...
   - 기계 간 (Machine-to-Machine) 인증 (Authentication)으로 접근 자격 획득

### 백엔드 API 보호 \{#protect-your-backend-api}
...
Logto(및 OAuth 2.0) 개념에서 백엔드 서비스는 리소스 서버 역할을 합니다. 사용자는 프론트엔드에서 받은 액세스 토큰(Access token)으로 DocuMind 리소스 서버에 접근합니다. 리소스 서버는 이 토큰을 검증하고, 유효하면 요청한 리소스를 반환합니다.

백엔드 서비스를 대표하는 API 리소스를 Logto 콘솔에서 생성합시다.

1. 오른쪽의 "API resources" 버튼 클릭
2. "Create API resource" 클릭, 팝업에서 Express 선택
3. API 이름은 "DocuMind API", API 식별자는 "[https://api.documind.com](https://api.documind.com/)" 입력
4. 생성 클릭
...
POST /organizations 엔드포인트를 보호하는 requireAuth 미들웨어를 만듭니다.
...
프론트엔드에서 Logto 토큰을 받아 백엔드 서비스 API에 전달할 수 있습니다. 미들웨어는 사용자 ID도 제공합니다. 이는 사용자를 조직에 추가할 때 유용합니다.
...
### Logto Management API 호출 \{#calling-logto-management-api}

Logto Management API를 사용해 조직 생성을 구현해봅시다.

프론트엔드에서 백엔드 서비스로 요청하는 것과 마찬가지로, 백엔드 서비스에서 Logto로 요청할 때도 액세스 토큰이 필요합니다.

Logto에서는 기계 간 (Machine-to-Machine) 인증 (Authentication)으로 액세스 토큰을 사용합니다. 자세한 내용은 [Management API와 상호작용하기](/integrate-logto/interact-with-management-api)를 참고하세요.

Logto 콘솔의 애플리케이션 페이지로 이동해 Machine-to-Machine 애플리케이션을 생성하세요. "Logto Management API access" 역할을 할당합니다. Token endpoint, App ID, App Secret을 복사해두세요. 액세스 토큰 발급에 사용합니다.
...
다음 단계는 조직에 사용자 초대 기능입니다. 이 튜토리얼에서는 구현하지 않지만, 이미 Management API 사용법을 익혔으니 [**테넌트 생성 및 초대**](https://blog.logto.io/build-multi-tenant-saas-application#tenant-creation-and-invitation)와 [멀티 테넌트 앱에서 사용자 협업 구현 방법](https://blog.logto.io/implement-user-collaboration-in-your-app) 블로그를 참고해 쉽게 구현할 수 있습니다.

## 멀티 테넌트 앱에 접근 제어 구현 \{#implement-access-control-to-your-multi-tenant-app}

이제 조직 접근 제어로 넘어가 봅시다.

우리가 달성하고자 하는 것은:

- 사용자는 자신이 소속된 조직의 리소스만 접근 가능: Logto의 `조직 토큰 (Organization token)`으로 구현
- 사용자는 조직 내에서 특정 역할(다른 권한 포함)을 가져야 하며, 인가된 작업만 수행 가능: Logto의 조직 템플릿 기능으로 구현

이 기능들을 어떻게 구현하는지 살펴봅시다.

### Logto 조직 토큰 (Organization token) 사용 \{#using-logto-organization-token}

앞서 언급한 Logto 액세스 토큰(Access token)과 유사하게, Logto는 특정 리소스에 대응하는 액세스 토큰을 발급하며, 사용자는 이 토큰으로 백엔드 서비스의 보호된 리소스에 접근합니다. 마찬가지로, Logto는 특정 조직에 대응하는 조직 토큰(Organization token)을 발급하며, 사용자는 이 토큰으로 백엔드 서비스의 보호된 조직 리소스에 접근합니다.
...
`getOrganization` 또는 조직 관련 기능을 사용하기 전에 Logto config에 `urn:logto:scope:organizations` scope와 `urn:logto:resource:organization` resource가 포함되어야 합니다. 이미 선언했으므로 반복하지 않겠습니다.
...
1. `getOrganizationToken`에 전달된 `organizationId`가 현재 사용자가 소속된 조직 id가 아니면, 토큰을 얻을 수 없습니다. 즉, 사용자는 자신의 조직에만 접근할 수 있습니다.
2. 조직 리소스 요청 시 액세스 토큰이 아닌 조직 토큰을 사용합니다. 조직에 속한 리소스는 사용자 권한이 아니라 조직 권한으로 접근 제어하기 때문입니다(이후 `GET /documents` API 구현에서 더 잘 이해할 수 있습니다).

이제 백엔드 서비스에 `GET /documents` API를 만듭니다. 앞서 API 리소스로 `POST /organizations` API를 보호한 것처럼, 조직별 리소스 지표로 `GET /documents` API를 보호합니다.
...
이제 requireOrganizationAccess 미들웨어로 `GET /documents` API를 보호합니다.
...
app.get('/documents', requireOrganizationAccess(), async (req, res) => {
  ...
  // organizationId로 데이터베이스에서 문서 조회
  // ....
...
조직 간 데이터 격리가 필요한 소프트웨어의 경우, [PostgreSQL로 멀티 테넌시 구현: 실전 예제로 배우기](https://blog.logto.io/implement-multi-tenancy) 블로그를 참고하세요.

### 조직 레벨 역할 기반 접근 제어 (RBAC) 설계 구현 \{#implement-organization-level-role-based-access-control-design}

조직 토큰으로 조직 리소스에 접근하는 기능을 구현했습니다. 이제 RBAC로 조직 내 사용자 권한 제어를 구현해봅시다.

DocuMind에는 Admin과 Collaborator 두 가지 역할이 있다고 가정합시다.

Admin은 문서 생성 및 접근이 가능하고, Collaborator는 문서 접근만 가능합니다.
...
그 다음 조직 역할 탭에서 Admin과 Collaborator 역할을 생성하고, 각 역할에 해당 권한을 할당하세요.
...
그 다음 백엔드의 requireOrganizationAccess 미들웨어에서 사용자 권한 검증을 추가합니다.
...
이렇게 하면 사용자 권한을 확인해 권한 제어를 구현할 수 있습니다.
...
지금까지 멀티 테넌트 SaaS 시스템의 기본 사용자 및 조직 기능을 구현했습니다! 하지만 조직별 로그인 페이지 브랜딩, 특정 도메인 이메일 사용자를 자동으로 조직에 추가, 엔터프라이즈 SSO 연동 등 아직 다루지 않은 기능도 있습니다.

이 모든 기능은 바로 사용할 수 있으며, 자세한 내용은 Logto 문서에서 확인할 수 있습니다:

- [엔터프라이즈 SSO 연동](/end-user-flows/enterprise-sso)
...
그리고 가장 중요한 점? 우리는 바퀴를 다시 만들 필요가 없었습니다. Logto 같은 최신 도구를 활용해 몇 달 걸릴 개발을 단 몇 분 만에 끝냈습니다.
