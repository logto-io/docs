...
Logto는 기본적으로 코어 서비스에 포트 `3001`을, 인터랙티브 Admin Console에 포트 `3002`를 사용합니다.
...
:::caution
프로덕션 환경에서는 우리의 docker compose 명령어를 사용하지 마세요! 현재 `docker-compose.yml`에서 Logto 앱과 함께 내장된 Postgres 데이터베이스가 번들로 제공되기 때문에,
