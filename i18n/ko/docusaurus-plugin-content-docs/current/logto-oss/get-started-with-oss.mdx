---
description: Logto 오픈소스 서비스 (OSS) 초기화에 대한 빠른 시작 가이드입니다.
sidebar_position: 1
---
...
명령을 다시 실행할 때마다 새로운 데이터베이스 인스턴스가 생성되며, 이전에 저장된 모든 데이터가 손실됩니다.
:::
...
컨테이너 포트를 Logto core 및 admin 앱에 매핑하세요. 예: `3001:3001` 및 `3002:3002`; 그리고 다음 환경 변수를 컨테이너에 설정하세요:
...
ADMIN_ENDPOINT: https://<your-logto-admin-domain> # (선택 사항) 커스텀 도메인을 사용하는 경우 Logto admin URL로 교체
DB_URL: postgres://username:password@your_postgres_url:port/db_name # Postgres DSN으로 교체
DATABASE_STATEMENT_TIMEOUT: DISABLE_TIMEOUT # (v1.36.0+) PgBouncer/RDS Proxy용 statement_timeout 비활성화
```

위 필수 환경 변수(선택 사항은 아래에 생략, 필요시 추가)를 사용하여 컨테이너를 실행하세요:
...
- PgBouncer 또는 RDS Proxy를 사용 중이고 `unsupported startup parameter: statement_timeout` 오류가 발생하면 `DATABASE_STATEMENT_TIMEOUT=DISABLE_TIMEOUT` (v1.36.0+)을 설정하세요.
...
Logto 전용의 새로운 빈 데이터베이스 사용을 권장하지만, 필수는 아닙니다.
...
Logto는 환경 변수로 설정을 관리하며, `.env` 파일도 지원합니다. 자세한 사용법과 전체 변수 목록은 [구성](/concepts/core-service/configuration)을 참고하세요.

</details>

_더 고급 제어나 프로그래밍 방식 접근이 필요하다면 [Core service](/concepts/core-service)를 확인하세요._
  ...
  items={[
    {
      ...
      description:
        '무료 개발 테넌트로 손쉽게 인증 (Authentication) 통합이 가능한 비용 효율적인 클라우드 서비스입니다.',
      customProps: {
        icon: <LogtoIcon />,
      },
    },
    {
      ...
      description:
        '앱 및 데이터베이스 관리를 쉽게 할 수 있는 자체 호스팅 Heroku/Netlify 대안입니다.',
      customProps: {
        icon: <CoolifyIcon />,
      },
    },
    ...
    },
    {
      ...
      description: '코드 및 오픈소스 소프트웨어 배포를 위한 완전 관리형 DevOps 플랫폼입니다.',
      customProps: {
        icon: <ElestioIcon />,
      },
    },
...
서버에 Logto를 성공적으로 호스팅한 후, 환영 페이지에서 "계정 생성"을 클릭하세요. Logto 오픈소스 버전은 최초 실행 시 한 번만 계정 생성을 허용하며, 여러 계정 생성을 지원하지 않습니다. 계정 생성은 사용자 이름과 비밀번호 조합으로만 제한됩니다.
