...
Logto Management API를 통해 Logto의 강력한 백엔드 서비스에 접근할 수 있습니다. 이 서비스는 매우 확장 가능하며 다양한 시나리오에서 활용할 수 있습니다. Admin Console의 로우코드 기능으로는 불가능한 것들도 가능합니다.
...
## Logto Management API 접근 방법 \{#how-to-access-logto-management-api}

### M2M 앱 생성하기 \{#create-an-m2m-app}

:::note
기계 간 (M2M) 인증 (Authentication) 플로우에 익숙하지 않다면, 먼저 [인증 (Authentication) 플로우 이해하기](/integrate-logto/integrate-logto-into-your-application/understand-authentication-flow/#machine-to-machine-authentication-flow)를 읽고 기본 개념을 이해하는 것을 추천합니다.
:::

<CloudLink to="/applications">콘솔 > 애플리케이션</CloudLink>으로 이동하여 "Machine-to-machine"
애플리케이션 유형을 선택하고 생성 과정을 시작하세요.
...
#### Logto Management API용 액세스 토큰 (Access token) 가져오기 \{#fetch-access-token-for-logto-management-api}
...
우리 개발자들은 Logto Management API를 활용해 다양한 추가 기능을 구현했습니다. 이 API는 매우 확장 가능하며 다양한 요구 사항을 지원할 수 있다고 믿습니다. 아래는 Logto Admin Console로는 불가능하지만 Logto Management API로는 가능한 몇 가지 시나리오 예시입니다.

### 사용자 프로필 직접 구현하기 \{#implement-user-profile-on-your-own}

Logto는 현재 사용자 프로필에 대한 미리 구축된 UI 솔루션을 제공하지 않습니다. 사용자 프로필은 비즈니스 및 제품 속성과 밀접하게 연관되어 있음을 인식하고 있습니다. 최적의 방안을 고민하는 동안, API를 활용해 직접 솔루션을 구축하는 것을 권장합니다. 예를 들어, 인터랙션 API, 프로필 API, 인증 코드 API를 활용해 맞춤형 솔루션을 개발할 수 있습니다.

### 고급 사용자 검색 \{#advanced-user-search}

Logto Admin Console은 기본적인 검색 및 필터링 기능을 지원합니다. 퍼지 검색, 정확한 일치, 대소문자 구분 등 고급 검색 옵션이 필요하다면 [고급 사용자 검색](/user-management/advanced-user-search) 튜토리얼과 가이드를 참고하세요.

### 조직 관리 직접 구현하기 \{#implement-organization-management-on-your-own}

[조직](/organizations) 기능을 활용해 다중 테넌트 앱을 구축하는 경우, 조직 초대 및 멤버 관리와 같은 작업에 Logto Management API가 필요할 수 있습니다. 테넌트 내에 관리자와 멤버가 모두 존재하는 SaaS 제품이라면, Logto Management API를 활용해 비즈니스 요구에 맞는 맞춤형 관리자 포털을 만들 수 있습니다. 자세한 내용은 [여기](/end-user-flows/organization-experience/)를 참고하세요.
...
#### 링크 헤더 (Link headers) 사용하기 \{#using-link-headers}
...
<Url href="https://blog.logto.io/use-postman-to-obtain-m2m-access-token">
  Postman으로 M2M 액세스 토큰 (Access token) 몇 분 만에 얻기
</Url>
