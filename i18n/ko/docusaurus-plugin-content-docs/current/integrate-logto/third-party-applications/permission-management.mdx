...
- `role`: Logto 사용자 프로필 스코프, 사용자 [역할](/authorization/role-based-access-control/#roles) 정보 접근에 사용.
- `urn:logto:scope:organizations`: Logto 사용자 조직 스코프, 사용자 조직 정보 접근에 사용. 활성화되어 서드파티 애플리케이션이 요청할 경우, 동의 화면에 조직 선택기가 표시됩니다. 사용자는 접근을 허용할 조직을 검토 및 선택할 수 있습니다. 자세한 내용은 [조직](/organizations)을 참고하세요.
...
Logto는 API 리소스에 대해 역할 기반 접근 제어 (RBAC)를 제공합니다. API 리소스는 귀하의 서비스가 소유하며 Logto가 보호하는 리소스입니다. 직접 정의한 API 스코프를 서드파티 애플리케이션에 할당하여 API 리소스에 접근할 수 있습니다. 자세한 내용은 [RBAC](/authorization/role-based-access-control), 조직 템플릿, [API 보호하기](/authorization/api-resources/protect-your-api/)를 참고하세요.

API 리소스 스코프는 <CloudLink to="/api-resources">콘솔 > API 리소스</CloudLink>에서 생성 및 관리할 수 있습니다.
...
[조직 권한](/authorization/organization-template/#organization-permission)은 Logto 조직을 위해 독점적으로 정의된 스코프입니다. 조직 정보 및 리소스 접근에 사용됩니다.
...
조직 템플릿 설정 페이지에서 직접 조직 스코프를 정의할 수 있습니다. 자세한 내용은 [조직 템플릿 구성](/authorization/organization-template/configure-organization-template/)을 참고하세요.
