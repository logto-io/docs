---
description: Logto를 사용하여 나만의 아이덴티티 제공자 (IdP)를 만들고, 타사 애플리케이션에 싱글 사인온 (SSO)을 활성화하세요. OIDC / OAuth 애플리케이션을 손쉽게 통합할 수 있습니다.
sidebar_position: 4
---
...
Logto는 [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) 프로토콜을 기반으로 한 아이덴티티 서비스로, [인증 (Authentication)](https://auth.wiki/authentication)과 [인가 (Authorization)](https://auth.wiki/authorization) 기능을 모두 제공합니다. 이로 인해 OIDC 타사 앱 통합은 기존 웹 애플리케이션만큼 간단하게 진행할 수 있습니다.
...
타사 서비스가 OIDC 디스커버리를 지원한다면, **디스커버리 엔드포인트**만 제공하면 됩니다. 그렇지 않은 경우, **엔드포인트 세부 정보 보기**를 클릭하여 [인가 엔드포인트](/integrate-logto/application-data-structure#authorization-endpoint) 및 [토큰 엔드포인트](/integrate-logto/application-data-structure#token-endpoint)를 포함한 모든 엔드포인트를 확인하세요.

### 타사 IdP를 지원하는 서비스와 통합하기 \{#integrate-with-services-that-support-third-party-idp}

외부 아이덴티티 제공자 구성을 기본적으로 지원하는 서비스나 제품(예: 엔터프라이즈 SaaS 플랫폼, 협업 도구)에 연결하는 경우, 설정은 매우 간단합니다:

1. 서비스의 IdP 또는 SSO 구성 페이지를 엽니다.
2. Logto에서 **클라이언트 ID**(필요한 경우 **클라이언트 시크릿**도)를 복사하여 서비스의 구성에 붙여넣습니다.
...
2. **콜백 처리**: 리디렉션에서 인가 `code`를 수신합니다.
...
3. **콜백 처리**: 리디렉션에서 인가 `code`를 수신합니다.
...
디바이스 플로우에서는 기기가 사용자 코드와 인증 (Authentication) URL을 표시합니다. 사용자는 별도의 기기(휴대폰, 노트북 등)에서 해당 URL에 접속하여 코드를 입력하고 인증 (Authentication)을 완료합니다. 기기는 Logto의 토큰 엔드포인트를 폴링하여 인가가 완료될 때까지 대기합니다.
...
자세한 구현 방법은 [디바이스 플로우 빠른 시작](/quick-starts/device-flow)을 참고하세요.
...
Logto는 역할 기반 접근 제어 (RBAC)를 사용하여 사용자 권한을 관리합니다. 동의 화면에는 사용자가 이미 역할을 통해 할당받은 스코프(권한)만 표시됩니다. 타사 앱이 사용자가 갖고 있지 않은 스코프를 요청하면, 해당 항목은 제외되어 비인가 동의가 방지됩니다.
