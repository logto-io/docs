...
## 이메일 템플릿 유형 \{#email-template-customization-options}
...
## 이메일 템플릿 변수 \{#email-template-types}

### 코드 \{#email-template-variables}
...
## 이메일 템플릿 현지화 \{#email-template-examples}

### 다양한 언어에 대한 사용자 정의 이메일 템플릿 \{#email-template-localization}
...
### 제공자 측 이메일 템플릿 현지화 \{#custom-email-templates-for-different-languages}
...
## 자주 묻는 질문 \{#provider-side-email-template-localization}

<details>
<summary>

### Logto에 템플릿이 구성되어 있지 않은 경우 타사 이메일 템플릿 서비스를 사용하는 방법은 무엇인가요? \{#faqs}
...
### Logto 이메일을 사용하여 사용자에게 맞춤형 "환영 이메일"을 보낼 수 있는 방법이 있나요? \{#how-to-use-third-party-email-template-services-if-templates-are-not-configured-in-logto}
...
## 관련 리소스 \{#is-there-a-way-to-use-logto-email-for-sending-our-users-a-customized-welcome-email}
