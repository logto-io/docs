...
| ENDPOINT                | `'protocol://localhost:$PORT'`       | `string`                                                 | 온라인 테스트 또는 프로덕션을 위해 사용자 지정 도메인으로 URL을 지정할 수 있습니다. 이는 [OIDC 발급자 식별자](https://openid.net/specs/openid-connect-core-1_0.html#IssuerIdentifier)의 값에도 영향을 줍니다.                                                                    |
| ADMIN_ENDPOINT          | `'protocol://localhost:$ADMIN_PORT'` | `string`                                                 | 프로덕션을 위해 사용자 지정 도메인으로 URL을 지정할 수 있습니다 (예: `ADMIN_ENDPOINT=https://admin.domain.com`). 이는 Admin Console 리디렉션 URI의 값에도 영향을 줍니다.                                                                                                         |
