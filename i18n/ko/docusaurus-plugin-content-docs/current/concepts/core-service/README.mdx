...
Logto는 놀라운 인증된 [OpenID Connect](https://openid.net/connect/) 구현체인 [node-oidc-provider](https://github.com/panva/node-oidc-provider)를 내부적으로 사용합니다. 이 제공자는 `/oidc`에 마운트되어 있으며, 관련 구성 및 파일은 [packages/core/src/oidc](https://github.com/logto-io/logto/tree/master/packages/core/src/oidc)에서 확인할 수 있습니다.

OIDC [Userinfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo)는 `/oidc/me`에 사용 가능하고 마운트되어 있습니다.
...
- 심층 기술 사양 및 예제는 [Experience API 디자인 RFC](https://github.com/logto-io/rfcs/blob/master/draft/0004-experience-api.md)를 읽어보세요.
...
[Account API에 의한 계정 설정](/end-user-flows/account-settings/by-account-api)을 확인하여 Account API를 활용하여 자체 계정 설정 페이지를 구축하는 방법을 배우세요.
