...
_핵심 서비스_는 중요한 Logto 업무를 위한 모놀리식 서비스입니다. 소스 코드는 [`@logto/core`](https://github.com/logto-io/logto/tree/master/packages/core)에 있습니다.

:::note
_핵심 서비스_와 _SDK core_는 두 개의 별도 개념입니다. 차이점은 [SDK 규칙](/developers/sdk-conventions)을 참조하세요.
:::
...
_Management API_는 Logto 데이터를 관리하고 업데이트하는 API 세트입니다. `admin` 역할을 가진 사용자만 접근할 수 있습니다.
...
_프론트엔드 프록시_는 환경 관련 방식으로 프론트엔드 프로젝트를 제공하는 미들웨어 함수입니다:
