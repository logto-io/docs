...
- **ACS URL**: Assertion Consumer Service (ACS) URL은 POST 요청과 함께 SAML 어설션이 전송되는 위치입니다. 이 URL은 IdP가 SAML 어설션을 Logto로 전송하는 데 사용됩니다. 이는 Logto가 사용자의 아이덴티티 정보를 포함하는 SAML 응답을 수신하고 처리할 것으로 기대하는 콜백 URL로 작동합니다.
