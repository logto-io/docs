...
- 다음 필드로 Tencent Cloud SMS 커넥터를 구성하세요:
  ...
  - `signName`: 2단계에서 얻은 서명 이름입니다. 이는 모든 템플릿에 적용됩니다.
  - 필요에 따라 다양한 시나리오에 대한 여러 템플릿을 추가하세요. 단일 템플릿의 예:
    - `templateCode`: 2단계에서 가져온 "템플릿 코드".
    - `usageType`: 다양한 시나리오에 대해 `Register`, `SignIn`, `ForgotPassword`, 또는 `Generic` 중 하나를 지정하세요 (Logto 전용 속성).
...
전화번호를 입력하고 "전송"을 클릭하여 설정이 작동하는지 확인한 후 "저장 및 완료"를 클릭하세요.

이제 끝났습니다. [로그인 경험에서 커넥터 활성화](/connectors/sms-connectors#enable-phone-number-sign-up-or-sign-in)를 잊지 마세요.
