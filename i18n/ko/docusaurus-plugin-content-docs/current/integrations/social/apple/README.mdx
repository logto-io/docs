...
Apple로 이미 로그인한 후에 사용자의 이메일 주소를 요청하도록 앱을 구성한 경우, Apple ID를 사용하여 다시 로그인하더라도 기존 사용자의 이메일 주소를 가져올 수 없습니다. 이를 해결하려면 사용자가 [Apple ID 계정 관리 페이지](https://appleid.apple.com/account/manage)를 방문하여 "Apple로 로그인" 섹션에서 애플리케이션을 제거하도록 안내해야 합니다. 이는 앱의 세부 정보 페이지에서 "Apple Sign In 사용 중지"를 선택하여 수행할 수 있습니다.

예를 들어, 앱이 사용자의 이메일과 이름 (`email name` 스코프)을 모두 요청하는 경우, 새로운 사용자가 처음 로그인할 때 보는 동의 화면은 다음과 유사해야 합니다:

![Apple로 로그인 동의 화면](./assets/sign-in-with-apple-consent-screen.png)
