...
Alipay Native 커넥터는 모바일 플랫폼에서 Logto SDK와 긴밀하게 작동합니다. Alipay의 OAuth 2.0 인증 워크플로우를 활용하여 Alipay 사용자가 번거로운 등록 과정을 거치지 않고 공용 Alipay 사용자 프로필을 사용하여 다른 앱에 로그인할 수 있도록 합니다.
...
4. "애플리케이션 이름" (应用名称)에 명명 규칙을 따라 애플리케이션 이름을 지정하고 "애플리케이션 아이콘" (应用图标)을 업로드하세요. "앱 유형" (应用类型)으로 "모바일 애플리케이션" (移动应用)을 선택해야 합니다. iOS 앱을 빌드하려면 고유한 "번들 ID"가 필요합니다. 또한, Android 앱에는 "애플리케이션 서명" (应用签名)과 "애플리케이션 패키지 이름" (应用包名)이 필요합니다.
...
3. Logto 커넥터 설정을 작성하세요:
   ...
   - 2단계에서 언급한 개인 키 파일의 내용으로 `privateKey` 필드를 작성하세요. 모든 줄 바꿈 문자를 '\n'으로 대체하는 것을 반드시 확인하세요. 개인 키 파일의 헤더와 푸터를 제거할 필요는 없습니다.
   - "Alipay 앱 생성 및 구성하기"의 7단계에서 선택한 `공개 키` 서명 모드에 따라 `signType` 필드를 'RSA2'로 작성하세요.
...
<img src={addFrameworkImage} alt="프레임워크 추가" />

:::note

플러그인은 Alipay "극단적 SDK" (极简版 SDK)를 포함합니다. 플러그인을 가져온 후 `import AFServiceSDK`를 직접 사용할 수 있습니다.
...
**1. Alipay "극단적 SDK"를 다운로드하여 프로젝트에 추가하세요**

[Logto 3rd-party Social SDKs](https://github.com/logto-io/social-sdks/blob/master/alipay/android/alipaySdk-15.7.9-20200727142846.aar)에서 Alipay "극단적 SDK" (极简版 SDK)를 다운로드하여 프로젝트의 `app/libs` 폴더에 추가하세요:

```bash
project-path/app/libs/alipaySdk-15.7.9-20200727142846.aar
```

**2. Alipay "극단적 SDK"를 종속성으로 추가하세요**
