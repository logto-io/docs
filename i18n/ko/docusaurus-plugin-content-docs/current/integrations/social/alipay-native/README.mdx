...
[Alipay 개발자 계정을 등록하세요](https://certifyweb.alipay.com/certify/reg/guide#/) (아직 계정이 없는 경우).
...
4. "애플리케이션 이름" (应用名称)을 명명 규칙에 따라 지정하고 "애플리케이션 아이콘" (应用图标)을 업로드하세요. "앱 유형" (应用类型)으로 "모바일 애플리케이션" (移动应用)을 선택해야 합니다. iOS 앱을 빌드하려면 고유한 "번들 ID"가 필요합니다. 또한, Android 앱에는 "애플리케이션 서명" (应用签名)과 "애플리케이션 패키지 이름" (应用包名)이 필요합니다.
5. 애플리케이션 생성을 완료한 후, 개요 페이지로 이동하여 "능력 추가" (添加能力)를 클릭하여 Alipay 로그인 활성화 전에 "제3자 애플리케이션 인가" (第三方应用授权), "회원 정보 가져오기" (获取会员信息), "앱 Alipay 로그인" (App 支付宝登录)을 추가하세요.
...
7. "앱 Alipay 로그인"의 "서명" 버튼을 클릭하고 안내에 따라 서명 과정을 완료하세요. 이 단계를 완료하면 5단계에서 추가한 능력이 작동하는 것을 볼 수 있습니다.
...
3. Logto 커넥터 설정을 작성하세요:
   ...
   - "Alipay 앱 생성 및 구성"의 7단계에서 선택한 `Public key` 서명 모드에 따라 `signType` 필드를 'RSA2'로 작성하세요.
...
플러그인은 Alipay "극소형 SDK" (极简版 SDK)를 포함합니다. 플러그인을 가져온 후 `import AFServiceSDK`를 직접 사용할 수 있습니다.
...
**1. Alipay "극소형 SDK"를 다운로드하여 프로젝트에 추가하세요**

[Logto 3rd-party Social SDKs](https://github.com/logto-io/social-sdks/blob/master/alipay/android/alipaySdk-15.7.9-20200727142846.aar)에서 Alipay "극소형 SDK" (极简版 SDK)를 다운로드하여 프로젝트의 `app/libs` 폴더에 추가하세요:

```bash
project-path/app/libs/alipaySdk-15.7.9-20200727142846.aar
```

**2. Alipay "극소형 SDK"를 종속성으로 추가하세요**
