...
## WeChat Open Platform에서 모바일 앱 생성하기 \{#test-wechat-native-connector}
...
해당 빌드 변형의 보고서에서 `MD5` 값이 _Application Signing Signature_ (应用签名)가 되지만, 값에서 모든 세미콜론을 제거하고 소문자로 변경하세요.
