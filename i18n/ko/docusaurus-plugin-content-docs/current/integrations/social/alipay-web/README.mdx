...
9. Alipay 콘솔 페이지 하단의 "설정" (设置) 버튼을 클릭하여 "인가 리디렉션 URI" (授权回调地址)를 설정하세요. `${your_logto_origin}/callback/${connector_id}`는 Logto 코어에서 사용되는 기본 리디렉션 URI입니다. `connector_id`는 Logto 관리자 콘솔 커넥터 세부 정보 페이지의 상단 바에서 찾을 수 있습니다.
