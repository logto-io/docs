...
3. Aliyun DM Connector 설정을 작성하세요:
   ...
   - 다양한 경우에 대해 여러 DM 커넥터 템플릿을 추가할 수 있습니다. 단일 템플릿을 추가하는 예시는 다음과 같습니다:
     ...
     - `usageType` 필드를 `Register`, `SignIn`, `ForgotPassword` 또는 `Generic` 중 하나로 작성하여 다양한 사용 사례에 맞추세요. (`usageType`은 적절한 사용 사례를 식별하기 위한 Logto 속성입니다.) 전체 사용자 흐름을 활성화하려면 `Register`, `SignIn`, `ForgotPassword` 및 `Generic` 사용 사례의 템플릿이 필요합니다.

### Aliyun DM 커넥터 테스트 \{#test-aliyun-dm-connector}
