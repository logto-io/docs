---
description: Logto를 통합하여 신속하게 아이덴티티 및 접근 관리 시스템을 시작하세요. 인증 (Authentication), 인가 (Authorization), 다중 테넌트 관리까지 한 번에 경험할 수 있습니다.
---
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      href: '/end-user-flows',
      description:
        '식별자 로그인, 소셜 로그인, 패스키 로그인, MFA, 그리고 Logto Console 및 API를 통한 광범위한 커스터마이징 등 완전한 인증 (Authentication) 플로우와 사용자 인터페이스를 제공합니다.',
      customProps: {
        icon: <ExperienceIcon />,
      },
    },
    ...
    },
    {
      ...
      description:
        'AI 도구와 IDE를 Logto Cloud에 연결하는 원격 MCP 서버로, 리소스 관리 및 인증 (Authentication) 통합을 지원합니다.',
      customProps: {
        icon: <McpIcon />,
      },
    },
  ...
  items={[
    {
      ...
      href: '/logto-cloud',
      description:
        '클라우드 서비스는 Free, Pro, Enterprise 플랜을 제공하여 다양한 고객 요구를 충족합니다. Logto에서 호스팅 및 관리하며, 모든 기능을 테스트할 수 있는 dev 테넌트도 제공합니다.',
      customProps: {
        icon: <CloudIcon />,
      },
    },
    {
      ...
      href: '/logto-oss',
      description:
        'Logto를 직접 인프라에 셀프 호스팅하세요. 자세한 내용은 GitHub 페이지와 오픈 소스 섹션을 참고하세요.',
      customProps: {
        icon: <OpenSourceIcon />,
      },
    },
...
아이덴티티 시스템이 처음이라면, 시작에 앞서 아래 필수 개념을 읽어보세요:
