...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        'Logto의 백엔드는 다양한 인증 (Authentication) 및 인가 (Authorization) 기능을 지원하는 API 모음을 제공합니다. 여기에는 Logto Management API, Experience API, Account API가 포함됩니다.',
      customProps: {
        icon: <LogtoApiIcon />,
      },
    },
  ...
  items={[
    {
      ...
      href: '/logto-cloud',
      description:
        '클라우드 서비스는 Free, Pro, Enterprise 요금제를 제공하여 다양한 고객 요구를 충족합니다. Logto가 호스팅 및 관리하며, 모든 기능을 테스트할 수 있는 dev 테넌트도 제공합니다.',
      customProps: {
        icon: <CloudIcon />,
      },
    },
...
아이덴티티 시스템이 처음이라면, 시작에 앞서 아래의 필수 개념을 읽어보세요:
