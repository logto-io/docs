...
`direct_sign_in` 매개변수에 `social:<idp-name>` 값을 전달하여 소셜 로그인 프로세스를 직접 시작하세요.
...
`direct_sign_in` 매개변수를 사용하여 인증 요청 URL을 만드세요:
...
`direct_sign_in` 매개변수에 `sso:<connector-id>` 값을 전달하여 엔터프라이즈 SSO 로그인 프로세스를 직접 시작하세요.
...
:::note
우리는 모든 Logto SDK에 direct_sign_in 매개변수 지원을 점진적으로 추가하고 있습니다. SDK에서 이를 찾을 수 없다면, 이슈를 열거나 저희에게 연락해 주세요.
:::
...
아니요, 직접 로그인은 기본 Logto 로그인 페이지를 건너뛰고 사용자를 소셜 또는 엔터프라이즈 SSO 제공자의 로그인 페이지로 직접 리디렉션할 수 있게 해주는 사용자 흐름 매개변수입니다. API 기반 인증과 달리, 사용자는 로그인 프로세스를 시작하기 위해 먼저 Logto 인증 엔드포인트로 리디렉션되어야 합니다.
