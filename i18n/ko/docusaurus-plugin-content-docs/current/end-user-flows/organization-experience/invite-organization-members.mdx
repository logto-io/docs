...
sequenceDiagram
  ...
  U ->> C: 초대 링크 클릭 후 랜딩 페이지로 이동,<br /> 초대 수락 또는 거부
...
초대는 이메일을 통해 전송되므로, [이메일 커넥터](/connectors/email-connectors)가 올바르게 구성되어 있는지 확인하세요. 초대를 보내려면 새로 도입된 [이메일 템플릿](/connectors/email-connectors/email-templates#email-template-types) 사용 유형 - `OrganizationInvitation`을 구성해야 합니다.
...
- `PUT /api/organization-invitations/{id}/status` 초대 상태를 업데이트하여 초대를 수락하거나 거부.
  이 API를 사용하여 초대에 대한 사용자의 응답을 처리하세요.
...
### 실시간 권한 확인 및 조직 토큰 업데이트 구현 \{#implement-real-time-permission-check-and-update-organization-token}
