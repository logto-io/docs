...
- 직접 접근: Account API는 최종 사용자가 Management API를 거치지 않고 자신의 계정 프로필을 직접 접근하고 관리할 수 있도록 합니다.
- 사용자 프로필 및 아이덴티티 관리: 사용자는 이메일, 전화번호, 비밀번호 등 아이덴티티 정보 업데이트와 소셜 연결 관리 등 프로필과 보안 설정을 완전히 관리할 수 있습니다. MFA 및 SSO 지원은 곧 제공될 예정입니다.
...
1. **보안 필드**:
   ...
   - 최종 사용자가 이 필드를 수정하기 전, 비밀번호, 이메일 또는 SMS를 통해 본인 인증을 거쳐 10분간 유효한 인증 레코드 ID를 받아야 합니다. [인증 레코드 ID 받기](#get-a-verification-record-id)를 참고하세요.
...
공식 SDK를 사용하지 않는 경우, 액세스 토큰 발급 요청 시 `resource`를 비워서 `/oidc/token` 엔드포인트에 요청해야 합니다.
...
### 인증 레코드 ID 받기 \{#get-a-verification-record-id}

먼저, 10분간 유효한 **인증 레코드 ID**를 받아야 합니다. 이는 민감 정보 업데이트 전 사용자의 신원을 검증하는 데 사용됩니다. 즉, 사용자가 비밀번호, 이메일 인증 코드, SMS 인증 코드로 본인 인증에 성공하면, 10분 동안 인증 관련 데이터(식별자, 자격 증명, 소셜 계정 연결, MFA 등)를 업데이트할 수 있습니다.

인증 레코드 ID를 받으려면 [사용자 비밀번호 인증](#verify-the-users-password) 또는 [사용자 이메일/전화로 인증 코드 전송](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)을 사용할 수 있습니다.
...
이메일을 예로 들어, 새 인증 코드를 요청하고 인증 레코드 ID를 받으세요:
...
코드 인증 후, 이제 인증 레코드 ID를 사용하여 사용자의 식별자를 업데이트할 수 있습니다.

인증에 대해 더 알아보려면 [Account API로 보안 인증](/end-user-flows/security-verification)을 참고하세요.

### 인증 레코드 ID와 함께 요청 보내기 \{#send-request-with-verification-record-id}

사용자 식별자를 업데이트하는 요청을 보낼 때, 요청 헤더의 `logto-verification-id` 필드에 인증 레코드 ID를 포함해야 합니다.
...
코드 인증 후, [`PATCH /api/my-account/primary-email`](https://openapi.logto.io/operation/operation-updateprimaryemail)로 사용자의 이메일을 업데이트할 수 있으며, `verificationId`를 요청 본문의 `newIdentifierVerificationRecordId`로 설정하세요.

:::info[서로 다른 두 인증 레코드 ID]

이 요청에는 두 개의 별도 인증 레코드 ID가 필요합니다:

- **`logto-verification-id` (헤더)**: 민감한 변경 전 사용자의 신원을 증명합니다. [사용자 비밀번호 인증](#verify-the-users-password) 또는 [기존 이메일/전화로 인증 코드 전송](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)로 획득합니다.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/primary-email \
  ...
  -H 'content-type: application/json' \
  # "newIdentifierVerificationRecordId"는 새 이메일 소유권 증명 (위 인증 코드 플로우에서 획득)
...
- `redirectUri`: 사용자가 애플리케이션을 인증한 후 리디렉션되는 URI. 이 URL에 웹 페이지를 호스팅하고 콜백을 받아야 합니다.
- `state`: CSRF 공격 방지를 위한 임의 문자열로, 인증 후 반환됩니다.

응답에서 `verificationRecordId`를 확인하고, 이후에 사용할 수 있도록 보관하세요.

사용자가 애플리케이션을 인증하면, `redirectUri`로 `state` 파라미터와 함께 콜백을 받게 됩니다. 그런 다음 [`POST /api/verifications/social/verify`](https://openapi.logto.io/operation/operation-verifyverificationbysocial) 엔드포인트로 소셜 연결을 인증할 수 있습니다.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/identities \
  ...
  -H 'logto-verification-id: <verification_record_id_from_existing_identifier>' \
  -H 'content-type: application/json' \
  # "newIdentifierVerificationRecordId"는 연결할 소셜 아이덴티티 식별 (위 소셜 인증 플로우에서 획득)
...
- **기본 설정**: Logto 기본 도메인 `https://[tenant-id].logto.app`만 사용하는 경우, RP ID는 `[tenant-id].logto.app`입니다.
...
- `verificationRecordId`: 1단계에서 서버가 반환한 인증 레코드 ID
...
- `verification_record_id`: 기존 요소 인증을 통해 부여받은 유효한 인증 레코드 ID. 자세한 내용은 [인증 레코드 ID 받기](#get-a-verification-record-id) 섹션 참고.
- `type`: MFA 요소 유형, 현재는 `WebAuthn`만 지원.
- `newIdentifierVerificationRecordId`: 1단계에서 서버가 반환한 인증 레코드 ID.
...
- 즉시 다운로드하거나 기록해 둘 것
- 안전한 장소에 보관할 것
- 각 코드는 한 번만 사용할 수 있음을 인지할 것
- 주 MFA 수단을 잃었을 때 최후의 수단임을 인지할 것

코드는 복사하기 쉽고 명확한 형식으로 표시하고, 다운로드 옵션(예: 텍스트 파일, PDF 등)도 제공하는 것이 좋습니다.
