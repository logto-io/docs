...
- 사용자 프로필 및 아이덴티티 관리: 사용자는 이메일, 전화번호, 비밀번호와 같은 아이덴티티 정보 업데이트 및 소셜 연결 관리 등 프로필과 보안 설정을 완전히 관리할 수 있습니다. MFA 및 SSO 지원도 곧 제공될 예정입니다.
...
보안상의 이유로, Account API에서 식별자 및 기타 민감 정보와 관련된 작업을 수행할 때는 추가적인 인가 (Authorization) 절차가 필요합니다.

### 인증 기록 ID(verification record id) 얻기 \{#get-a-verification-record-id}

먼저, 인증 기록 ID를 얻어야 하며, 이는 식별자 업데이트 시 사용자의 신원을 검증하는 데 사용됩니다.
...
:::note
이 방법을 사용하려면 [이메일 커넥터](/connectors/email-connectors/) 또는 [SMS 커넥터](/connectors/sms-connectors/)를 설정하고, `UserPermissionValidation` 템플릿이 구성되어 있어야 합니다.
:::

이메일을 예시로, 새 인증 코드를 요청하고 인증 기록 ID를 얻으세요:
...
사용자의 식별자를 업데이트하는 요청을 보낼 때는, 요청 헤더에 `logto-verification-id` 필드로 인증 기록 ID를 첨부해야 합니다.

### 새 이메일 업데이트 또는 연결 \{#update-or-link-new-email}

:::note
이 방법을 사용하려면 [이메일 커넥터](/connectors/email-connectors/)를 설정하고, `BindNewIdentifier` 템플릿이 구성되어 있어야 합니다.
:::
...
:::note
이 방법을 사용하려면 [SMS 커넥터](/connectors/sms-connectors/)를 설정하고, `BindNewIdentifier` 템플릿이 구성되어 있어야 합니다.
:::
...
- `redirectUri`: 사용자가 애플리케이션을 인가한 후 리디렉션되는 URI. 이 URL에 웹 페이지를 호스팅하고 콜백을 받아야 합니다.
- `state`: 사용자가 애플리케이션을 인가한 후 반환되는 상태값. CSRF 공격을 방지하기 위한 임의의 문자열입니다.

응답에서 `verificationRecordId`를 확인하고, 이후에 사용할 수 있도록 보관하세요.

사용자가 애플리케이션을 인가하면, `redirectUri`로 `state` 파라미터와 함께 콜백을 받게 됩니다. 이후 `POST /api/verifications/social/verify` 엔드포인트를 사용하여 소셜 연결을 검증할 수 있습니다.
