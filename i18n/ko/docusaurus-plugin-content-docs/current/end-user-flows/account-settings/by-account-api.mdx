...
- 사용자 프로필 및 아이덴티티 관리: 사용자는 이메일, 전화번호, 비밀번호 등 아이덴티티 정보를 업데이트하고, 소셜 연결을 관리하는 등 프로필과 보안 설정을 완전히 관리할 수 있습니다. MFA와 SSO 지원도 곧 제공될 예정입니다.
...
활성화 후, 식별자, 프로필 데이터, 서드파티 토큰 접근에 대한 필드별 권한을 설정할 수 있습니다. 각 필드는 `Off`, `ReadOnly`, `Edit` 중 하나를 지원하며, 기본값은 `Off`입니다.

1. **보안 필드**:
   - 필드: 기본 이메일, 기본 전화번호, 소셜 아이덴티티, 비밀번호, MFA.
   - 최종 사용자가 이 필드를 수정하기 전에 비밀번호, 이메일, SMS를 통해 본인 인증을 해야 하며, 10분 유효 기간의 인증 기록 ID를 받아야 합니다. [인증 기록 ID 받기](#get-a-verification-record-id)를 참고하세요.
...
2. **프로필 필드**:
   - 필드: 사용자명, 이름, 아바타, [프로필](/user-management/user-data#profile) (기타 표준 프로필 속성), [커스텀 데이터](/user-management/user-data#custom-data).
   - 최종 사용자는 추가 인증 없이 이 필드를 수정할 수 있습니다.
3. **시크릿 볼트**:
   - OIDC 또는 OAuth 소셜 및 엔터프라이즈 커넥터의 경우, Logto [시크릿 볼트](/secret-vault/federated-token-set)는 인증 후 서드파티 액세스 토큰 및 리프레시 토큰을 안전하게 저장합니다. 앱은 사용자가 다시 로그인하지 않아도 외부 API(예: Google Calendar 이벤트 동기화 등)를 호출할 수 있습니다. Account API가 활성화되면 토큰 조회가 자동으로 가능합니다.
4. **세션 관리**:
   - 활성화 시, 사용자는 자신의 활성 세션(디바이스 정보, 마지막 로그인 시간 포함)을 조회 및 관리할 수 있습니다. 특정 디바이스에서 로그아웃하려면 세션을 해제할 수도 있습니다.
   - 동일한 `Sessions` 필드 권한은 사용자 승인 앱(권한 부여) API(조회 및 해제)에도 적용됩니다.
   - 최종 사용자가 세션 관리에 접근하기 전에 비밀번호, 이메일, SMS를 통해 본인 인증을 해야 하며, 10분 유효 기간의 인증 기록 ID를 받아야 합니다. [인증 기록 ID 받기](#get-a-verification-record-id)를 참고하세요.

## Account API 접근 방법 \{#how-to-access-account-api}

:::note
액세스 토큰에 적절한 권한이 포함되어 있는지 확인하려면 Logto 설정에서 해당 스코프를 올바르게 구성해야 합니다.
...
먼저, 10분 유효 기간(TTL)의 **인증 기록 ID**를 받아야 합니다. 이는 민감 정보 업데이트 전에 사용자의 신원을 확인하는 데 사용됩니다. 즉, 사용자가 비밀번호, 이메일 인증 코드, SMS 인증 코드로 본인 인증에 성공하면, 10분 동안 인증 관련 데이터(식별자, 자격 증명, 소셜 계정 연결, MFA 등)를 업데이트할 수 있습니다.

인증 기록 ID를 받으려면 [사용자 비밀번호 인증](#verify-the-users-password) 또는 [사용자 이메일/전화로 인증 코드 전송](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)을 사용할 수 있습니다.
...
#### 사용자 이메일/전화로 인증 코드 전송 \{#verify-by-sending-a-verification-code-to-the-users-email-or-phone}
...
코드 인증이 완료되면, 이제 인증 기록 ID를 사용하여 사용자의 식별자를 업데이트할 수 있습니다.
...
사용자 비밀번호를 업데이트하려면 [`POST /api/my-account/password`](https://openapi.logto.io/operation/operation-updatepassword) 엔드포인트를 사용할 수 있습니다.
...
코드 인증이 완료되면, [`PATCH /api/my-account/primary-email`](https://openapi.logto.io/operation/operation-updateprimaryemail)로 사용자의 이메일을 업데이트할 수 있습니다. 이때 `verificationId`를 요청 본문의 `newIdentifierVerificationRecordId`로 설정하세요.

:::info[서로 다른 두 개의 인증 기록 ID]

이 요청에는 두 개의 별도 인증 기록 ID가 필요합니다:

- **`logto-verification-id` (헤더)**: 민감한 변경 전에 사용자의 신원을 증명합니다. [사용자 비밀번호 인증](#verify-the-users-password) 또는 [기존 이메일/전화로 인증 코드 전송](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)으로 획득할 수 있습니다.
- **`newIdentifierVerificationRecordId` (본문)**: 새 이메일 주소의 소유권을 증명합니다. 위의 `POST /api/verifications/verification-code` 호출에서 반환된 `verificationRecordId`입니다.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/primary-email \
  -H 'authorization: Bearer <access_token>' \
  # 사용자 신원 인증(비밀번호 또는 기존 이메일/전화 인증)
  -H 'logto-verification-id: <verification_record_id_from_existing_identifier>' \
  -H 'content-type: application/json' \
  # "newIdentifierVerificationRecordId"는 새 이메일 소유권을 증명(위 인증 코드 플로우에서 획득)
  --data-raw '{"email":"...","newIdentifierVerificationRecordId":"<verification_record_id_from_new_email>"}'
```

:::tip
회원가입 시 수집된 이메일과 마찬가지로, Account API를 통해 연결된 모든 이메일은 <CloudLink to="/security/blocklist">콘솔 > 보안 > 블록리스트</CloudLink>에서 구성한 [블록리스트](/security/blocklist) 검증을 통과해야 합니다. 정책 위반 시 Logto는 요청을 거부하고 상세 오류를 반환합니다.
:::
...
이메일 업데이트와 유사하게, [`PATCH /api/my-account/primary-phone`](https://openapi.logto.io/operation/operation-updateprimaryphone) 엔드포인트로 새 전화번호를 업데이트/연결할 수 있습니다. [`DELETE /api/my-account/primary-phone`](https://openapi.logto.io/operation/operation-deleteprimaryphone) 엔드포인트로 전화번호를 삭제할 수 있습니다.

### 새 소셜 연결 추가 \{#link-a-new-social-connection}

새 소셜 연결을 추가하려면, 먼저 [`POST /api/verifications/social`](https://openapi.logto.io/operation/operation-createverificationbysocial)로 인증 URL을 요청해야 합니다.
...
- `redirectUri`: 사용자가 애플리케이션을 승인한 후 리디렉션되는 URI. 이 URL에 웹 페이지를 호스팅하고 콜백을 수신해야 합니다.
- `state`: CSRF 공격 방지를 위한 임의 문자열

응답에서 `verificationRecordId`를 확인하고, 이후 단계에서 사용하세요.

사용자가 애플리케이션을 승인하면, `redirectUri`로 `state` 파라미터와 함께 콜백을 받게 됩니다. 이후 [`POST /api/verifications/social/verify`](https://openapi.logto.io/operation/operation-verifyverificationbysocial) 엔드포인트로 소셜 연결을 인증할 수 있습니다.
...
`connectorData`는 사용자가 애플리케이션을 승인한 후 소셜 커넥터에서 반환된 데이터입니다. 콜백 페이지에서 `redirectUri`의 쿼리 파라미터를 파싱하여 JSON으로 감싸 `connectorData` 필드 값으로 전달해야 합니다.
...
- **`logto-verification-id` (헤더)**: 민감한 변경 전에 사용자의 신원을 증명합니다. [사용자 비밀번호 인증](#verify-the-users-password) 또는 [기존 이메일/전화로 인증 코드 전송](#verify-by-sending-a-verification-code-to-the-users-email-or-phone)으로 획득할 수 있습니다.
- **`newIdentifierVerificationRecordId` (본문)**: 연결할 소셜 아이덴티티를 식별합니다. 위의 `POST /api/verifications/social` 호출에서 반환된 `verificationRecordId`입니다.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/identities \
  ...
  -H 'logto-verification-id: <verification_record_id_from_existing_identifier>' \
  -H 'content-type: application/json' \
  # "newIdentifierVerificationRecordId"는 연결할 소셜 아이덴티티를 식별(위 소셜 인증 플로우에서 획득)
...
WebAuthn 패스키는 **Relying Party ID (RP ID)**라는 특정 호스트네임에 바인딩됩니다. 해당 Origin에서 호스팅되는 애플리케이션만 해당 패스키로 등록/인증할 수 있습니다.

프론트엔드 애플리케이션이 Logto 인증 페이지와 다른 도메인에서 Account API를 호출하는 경우, **Related Origins**를 구성하여 교차 Origin 패스키 작업을 허용해야 합니다.

**Logto가 RP ID를 결정하는 방법:**

- **기본 설정**: Logto의 기본 도메인 `https://[tenant-id].logto.app`만 사용하는 경우, RP ID는 `[tenant-id].logto.app`
- **커스텀 도메인**: [커스텀 도메인](/logto-cloud/custom-domain) `https://auth.example.com`을 구성한 경우, RP ID는 `auth.example.com`

**Related Origins 구성:**

[`PATCH /api/account-center`](https://openapi.logto.io/operation/operation-updateaccountcentersettings) 엔드포인트를 사용하여 프론트엔드 앱의 Origin을 추가하세요. 예를 들어, 계정 센터가 `https://account.example.com`에서 동작한다면:
...
WebAuthn은 Related Origins로 최대 5개의 고유 eTLD+1 레이블을 지원합니다. eTLD+1(유효 최상위 도메인 + 1)은 등록 가능한 도메인 부분입니다. 예시:

- `https://example.com`, `https://app.example.com`, `https://auth.example.com`은 **하나**의 레이블(`example.com`)로 간주
- `https://shopping.com`, `https://shopping.co.uk`, `https://shopping.co.jp`도 **하나**의 레이블(`shopping`)
- `https://example.com`과 `https://another.com`은 **두 개**의 레이블

Related Origins로 5개 이상의 도메인을 지원해야 한다면 [Related Origin Requests](https://passkeys.dev/docs/advanced/related-origins/) 문서를 참고하세요.
...
- `verification_record_id`: 기존 인증 요소를 인증하여 획득한 유효한 인증 기록 ID. 자세한 내용은 [인증 기록 ID 받기](#get-a-verification-record-id) 섹션 참고.
...
사용자가 인증 앱에 시크릿을 추가한 후, 이를 인증하고 계정에 바인딩해야 합니다. [`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification) 엔드포인트를 사용하세요.
...
- `verification_record_id`: 기존 인증 요소를 인증하여 획득한 유효한 인증 기록 ID. 자세한 내용은 [인증 기록 ID 받기](#get-a-verification-record-id) 참고.
- `type`: 반드시 `Totp`여야 함
- `secret`: 1단계에서 생성한 TOTP 시크릿

:::note
사용자는 한 번에 하나의 TOTP 요소만 가질 수 있습니다. 이미 TOTP 요소가 있다면 추가 시 422 오류가 발생합니다.
:::

### 기존 TOTP 교체 \{#replace-an-existing-totp}

사용자가 이미 TOTP 요소를 구성한 경우, [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-createorreplacetotpmfaverification) 엔드포인트로 새 요소로 교체할 수 있습니다. 이 작업은 멱등적이며 기존 TOTP를 제거하고 새 요소를 바인딩합니다.
...
[`POST /api/my-account/mfa-verifications/totp-secret/generate`](https://openapi.logto.io/operation/operation-generatetotpsecret) 엔드포인트로 새 TOTP 시크릿을 생성한 후, [새 TOTP 연결](#link-a-new-totp)에서 설명한 대로 사용자에게 표시하세요.
...
- `verification_record_id`: 기존 인증 요소를 인증하여 획득한 유효한 인증 기록 ID. 자세한 내용은 [인증 기록 ID 받기](#get-a-verification-record-id) 참고.
- `secret`: 1단계에서 생성한 새 TOTP 시크릿
- `code`: 바인딩 확인을 위해 새 시크릿에서 생성한 TOTP 코드
...
- 안전한 장소에 보관할 것
- 각 코드는 한 번만 사용할 수 있음
- 기본 MFA 수단을 잃어버렸을 때 마지막 수단임을 인지할 것

코드를 명확하고 복사하기 쉬운 형식으로 표시하고, 다운로드 옵션(예: 텍스트 파일, PDF 등)도 제공하는 것이 좋습니다.

**3단계: 백업 코드를 사용자 계정에 바인딩**

[`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification) 엔드포인트로 백업 코드를 계정에 바인딩하세요.
...
### 2단계 인증 설정 관리 \{#manage-2-step-verification-settings}

:::note
이 방법을 사용하려면 [계정 센터 설정](#how-to-enable-account-api)에서 `mfa` 필드를 활성화해야 합니다.
:::

최종 사용자는 자신의 계정에 대해 2단계 인증을 켜거나 끌 수 있습니다. 일반적인 사례는 사용자가 MFA 요소(예: 패스키)를 구성했지만 로그인 시 추가 인증 단계로 사용하고 싶지 않은 경우입니다. 2단계 인증을 비활성화하면 MFA 프롬프트를 건너뛰고 MFA 요소는 그대로 유지됩니다. 이는 사용자별 `skipMfaOnSignIn` 설정으로 제어됩니다.

**현재 MFA 설정 조회**

[`GET /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-getmfasettings) 엔드포인트로 사용자의 현재 2단계 인증 설정을 조회할 수 있습니다.
...
2단계 인증을 활성화/비활성화하려면 [`PATCH /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-updatemfasettings) 엔드포인트를 사용하세요.
...
- `verification_record_id`: 유효한 인증 기록 ID. 자세한 내용은 [인증 기록 ID 받기](#get-a-verification-record-id) 참고.
- `skipMfaOnSignIn`: 사용자의 2단계 인증을 비활성화하려면 `true`, 활성화하려면 `false`로 설정

:::note
이 설정은 MFA 정책이 **선택적(Optional)**일 때만 적용됩니다. MFA가 **필수(Mandatory)** 또는 **적응형(Adaptive)**인 경우, `skipMfaOnSignIn` 설정은 무시되고 MFA가 강제 적용됩니다.
:::
...
사용자의 활성 세션을 조회하려면 [`GET /api/my-account/sessions`](https://openapi.logto.io/operation/operation-getsessions) 엔드포인트를 사용할 수 있습니다.

:::note

- 이 엔드포인트에 접근하려면 `UserScope.Sessions` 스코프가 필요합니다.
- 계정 센터 설정의 `Sessions` 필드는 `ReadOnly` 또는 `Edit`이어야 합니다.
...
특정 세션을 해제하려면 [`DELETE /api/my-account/sessions/{sessionId}`](https://openapi.logto.io/operation/operation-deletesessionbyid) 엔드포인트를 사용하세요.

:::note

- 이 엔드포인트에 접근하려면 `UserScope.Sessions` 스코프가 필요합니다.
- 계정 센터 설정의 `Sessions` 필드는 `Edit`이어야 합니다.
  :::
...
- `revokeGrantsTarget`: 세션과 함께 해제할 권한의 대상을 지정할 수 있습니다. 가능한 값:
  - `all`: 세션과 연결된 모든 권한 해제
  - `firstParty`: 세션과 연결된 1st-party 앱 권한만 해제(대부분의 경우 권장, 3rd-party 앱 권한은 유지되어 더 나은 사용자 경험 제공)
  - 미지정: 기본 동작은 `offline_access` 스코프가 없는 권한만 해제(일반적으로 리프레시 토큰이 없는 권한만 해제)

### 사용자 승인 앱(권한 부여) 관리 \{#manage-user-authorized-apps-grants}

사용자가 계정 설정 페이지에서 승인된 앱을 검토 및 해제해야 할 때, 사용자 승인 앱(권한 부여) API를 사용할 수 있습니다.

:::note

- 앱 권한 API는 세션 API와 동일한 권한 모델을 공유합니다.
- `UserScope.Sessions` 스코프가 필요합니다.
- 계정 센터 설정의 `Sessions` 필드가 활성화되어야 합니다:
  - `ReadOnly` 또는 `Edit`이면 권한 목록 조회 가능
  - `Edit`이면 권한 해제 가능
...
현재 사용자의 활성 앱 권한을 조회하려면 [`GET /api/my-account/grants`](https://openapi.logto.io/operation/operation-getgrants) 엔드포인트를 사용하세요.
...
- `appType=firstParty`: 1st-party 앱 권한만 반환
- `appType=thirdParty`: 3rd-party 앱 권한만 반환
- `appType` 생략: 모든 활성 권한 반환
...
특정 앱 권한을 해제하려면 [`DELETE /api/my-account/grants/{grantId}`](https://openapi.logto.io/operation/operation-deletegrantbyid) 엔드포인트를 사용하세요.
...
권한이 해제되면, 해당 권한으로 발급된 불투명 액세스 토큰 (Opaque token) 및 리프레시 토큰 (Refresh token)은 모두 무효화됩니다.
