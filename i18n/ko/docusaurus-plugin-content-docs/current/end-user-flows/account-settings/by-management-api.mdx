...
1. **사용자**: 자신의 계정 설정에 접근하고 관리해야 하는 인증된 최종 사용자.
...
5. 클라이언트 애플리케이션이 셀프 호스팅 계정 설정 API 접근을 위해 Logto에서 액세스 토큰을 요청합니다.
...
| ------ | -------------------------------------------------------------------------------------------------------- | ------------------------------- |
| GET    | [/api/users/\{userId\}](https://openapi.logto.io/operation/operation-getuser)                            | 사용자 ID로 사용자 정보 조회    |
| PATCH  | [/api/users/\{userId\}](https://openapi.logto.io/operation/operation-updateuser)                         | 사용자 정보 수정                |
| PATCH  | [/api/users/\{userId\}/profile](https://openapi.logto.io/operation/operation-updateuserprofile)          | 사용자 ID로 프로필 필드 수정    |
| GET    | [/api/users/\{userId\}/custom-data](https://openapi.logto.io/operation/operation-listusercustomdata)     | 사용자 ID로 커스텀 데이터 조회  |
| PATCH  | [/api/users/\{userId\}/custom-data](https://openapi.logto.io/operation/operation-updateusercustomdata)   | 사용자 ID로 커스텀 데이터 수정  |
| PATCH  | [/api/users/\{userId\}/is-suspended](https://openapi.logto.io/operation/operation-updateuserissuspended) | 사용자 ID로 계정 정지 상태 수정 |

### 이메일 및 전화번호 인증 (Email and phone number verification) \{#email-and-phone-number-verification}

Logto 시스템에서 이메일 주소와 전화번호 모두 사용자 식별자로 사용될 수 있으므로, 이들의 인증은 필수적입니다. 이를 지원하기 위해, 제공된 이메일 또는 전화번호를 인증할 수 있도록 인증 코드 API 세트를 제공합니다.

:::note
새 이메일 또는 전화번호로 사용자 프로필을 업데이트하기 전에 반드시 해당 이메일 또는 전화번호를 인증하세요.
:::
...
| PATCH  | [/api/users/\{userId\}/password](https://openapi.logto.io/operation/operation-updateuserpassword)        | 사용자 ID로 비밀번호 변경           |
| GET    | [/api/users/\{userId\}/has-password](https://openapi.logto.io/operation/operation-getuserhaspassword)    | 사용자 ID로 비밀번호 존재 여부 확인 |

:::note
사용자 비밀번호를 변경하기 전에 반드시 현재 비밀번호를 인증하세요.
:::
...
| method | path                                                                                                                              | description                                                                       |
| ------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| GET    | [/api/users/\{userId\}](https://openapi.logto.io/operation/operation-getuser)                                                     | 사용자 ID로 사용자 정보 조회. 소셜 아이덴티티는 `identities` 필드에서 확인 가능.  |
...
| PUT    | [/api/users/\{userId\}/identities](https://openapi.logto.io/operation/operation-replaceuseridentity)                              | 사용자 ID로 연결된 소셜 아이덴티티 직접 수정                                      |
| POST   | [/api/connectors/\{connectorId\}/authorization-uri](https://openapi.logto.io/operation/operation-createconnectorauthorizationuri) | 소셜 아이덴티티 제공자의 인가 URI 조회. 이 URI로 새로운 소셜 아이덴티티 연결 시작 |
...
2. 클라이언트 애플리케이션이 서버에 소셜 아이덴티티 바인딩 요청을 전송합니다.
3. 서버가 Logto에 소셜 아이덴티티 제공자의 인가 URI를 요청합니다. 이때 `state` 파라미터와 `redirect_uri`를 직접 제공해야 합니다. `redirect_uri`는 소셜 아이덴티티 제공자에 미리 등록해야 합니다.
4. Logto가 서버에 인가 URI를 반환합니다.
5. 서버가 클라이언트 애플리케이션에 인가 URI를 반환합니다.
...
11. Logto가 인가 코드를 사용해 IdP에서 사용자 정보를 조회합니다.
12. IdP가 Logto에 사용자 정보를 반환하고, Logto가 소셜 아이덴티티를 사용자에 연결합니다.
...
- 같은 이유로, Logto는 인가 응답의 `state` 파라미터를 검증할 수 없습니다. 클라이언트 앱에서 `state` 파라미터를 저장하고 인가 응답을 받을 때 검증해야 합니다.
...
| method | path                                                                                                    | description                                                                                                                                                                  |
| ------ | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | [/api/users/\{userId\}?includeSsoIdentities=true](https://openapi.logto.io/operation/operation-getuser) | 사용자 ID로 사용자 정보 조회. 엔터프라이즈 아이덴티티는 `ssoIdentities` 필드에서 확인 가능. API에 `includeSsoIdentities=true` 쿼리 파라미터를 추가하여 포함시킬 수 있습니다. |

현재 Management API는 사용자에 엔터프라이즈 아이덴티티를 연결하거나 연결 해제하는 기능을 지원하지 않습니다. 연결된 엔터프라이즈 아이덴티티만 표시할 수 있습니다.

### 개인 액세스 토큰 (Personal access token) \{#personal-access-token}

| method | path                                                                                                                                 | description                           |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| GET    | [/api/users/\{userId\}/personal-access-tokens](https://openapi.logto.io/operation/operation-listuserpersonalaccesstokens)            | 사용자의 모든 개인 액세스 토큰 조회   |
| POST   | [/api/users/\{userId\}/personal-access-tokens](https://openapi.logto.io/operation/operation-createuserpersonalaccesstoken)           | 사용자에 새로운 개인 액세스 토큰 추가 |
...
| PATCH  | [/api/users/\{userId\s}/personal-access-tokens/\{name\}](https://openapi.logto.io/operation/operation-updateuserpersonalaccesstoken) | 이름으로 사용자의 토큰 수정           |
...
| method | path                                                                                                                                 | description               |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------- |
| GET    | [/api/users/\{userId\}/mfa-verifications](https://openapi.logto.io/operation/operation-listusermfaverifications)                     | 사용자 ID로 MFA 설정 조회 |
| POST   | [/api/users/\{userId\}/mfa-verifications](https://openapi.logto.io/operation/operation-createusermfaverification)                    | 사용자 ID로 MFA 인증 설정 |
