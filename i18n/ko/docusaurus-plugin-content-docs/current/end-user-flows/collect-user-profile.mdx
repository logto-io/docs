...
- **신규 사용자 등록 시**: [사용자 프로필 수집](#quick-start) 기능을 사용하여, 회원가입 과정에서 추가 사용자 프로필 정보를 수집하는 "자기소개" 단계를 기본 제공으로 추가할 수 있습니다. 신규 사용자는 모든 필수 항목을 완료해야만 등록이 완료됩니다. 이 문서는 이 접근 방식에 중점을 둡니다.

- **사용자 등록 후**: [Account API](/end-user-flows/account-settings/by-account-api)를 사용하여 온보딩 플로우, 계정 센터, 또는 제품 사용 여정 전반에 걸쳐 셀프 서비스 방식으로 사용자 프로필 정보를 업데이트할 수 있습니다.

## 주요 이점 \{#key-benefits}

**사용자 프로필 수집** 기능을 통해 최종 사용자 등록 과정에서 추가 사용자 정보를 수집할 수 있습니다. 제품에 꼭 필요한 정보만 수집하여, 사용자 전환율에 영향을 줄 수 있는 긴 등록 과정을 피하는 것을 권장합니다.
...
- **유연한 필드 커스터마이징**: 텍스트, 숫자, 날짜, 체크박스, 드롭다운(선택), URL, 정규식 검증 등 다양한 [필드 유형](#field-types) 중에서 선택하여 데이터 요구 사항에 맞게 설정할 수 있습니다.
...
4. 로그인 & 계정 > 사용자 프로필 수집 화면으로 돌아가서, 필드를 드래그 앤 드롭하여 순서를 변경하면 변경 사항이 자동으로 적용됩니다.
...
Logto에서 제공하는 기본 사용자 데이터 필드는 `user` 또는 `user.profile` 속성에 직접 저장됩니다. 이 필드들은 기본 구성 값과 i18n 번역이 제공되어 생성 즉시 바로 사용할 수 있습니다. 또한, 필요에 따라 필드 유형과 파라미터를 커스터마이즈할 수 있습니다.

| 필드명 (13%) | 사용자 데이터 키 (44%)                                                                                                                                                                                                                                                                          | 설명                                                                                                                                                                               |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 이름         | <code>user.name</code>                                                                                                                                                                                                                                                                          | 모든 이름 구성 요소를 포함한 사용자의 전체 표시 이름 (예: "Jane Doe").                                                                                                             |
| 전체 이름    | <code>user.profile.givenName</code><br /><code>user.profile.middleName</code><br /><code>user.profile.familyName</code>                                                                                                                                                                         | 사용자의 완전한 법적 이름. 구성에 따라 familyName, givenName, middleName을 유연하게 조합합니다.                                                                                    |
...
사용자 생성 후 맞춤 사용자 데이터를 조회 및 업데이트하는 방법은 [사용자 데이터 구조](/user-management/user-data#custom-data)를 참고하세요.
...
| 날짜           | 날짜 형식(프리셋/사용자 지정) | 프리셋 형식(MM/dd/yyyy, dd/MM/yyyy, yyyy-MM-dd) 선택. 플레이스홀더 변경 시 기본 상호작용이 덮어써집니다. 또는 <a href="https://date-fns.org/v2.30.0/docs/format" target="_blank" rel="noopener noreferrer">date-fns</a> 패턴 완전 지원. |
| 체크박스       | 기본 값                       | 체크(참) / 해제(거짓).                                                                                                                                                                                                                  |
| 드롭다운(선택) | 옵션 목록(1개 이상)           | 각 줄마다 <code>값:라벨</code> 형식 사용. 라벨 생략 시 값이 표시됩니다.                                                                                                                                                                 |
| URL            | 해당 없음                     | 해당 없음                                                                                                                                                                                                                               |
...
필드 검증은 회원가입 마지막 단계에서 사용자 제출 시 수행됩니다. 다음과 같은 검증이 적용됩니다:

- 필수 입력 여부(체크박스 제외).
- 길이 제한(텍스트).
- 숫자 범위(숫자).
- 패턴 일치(정규식, 날짜 형식, URL, 선택 옵션 포함 여부).
- URL 구조(기본 구문 검증, 접근성 검증 아님).
...
맞춤 필드는 사용자가 입력한 라벨, 플레이스홀더, 설명에 전적으로 의존합니다. 맞춤 필드 현지화는 [UI 직접 구현](/customization/bring-your-ui) 또는 [문의하기](https://logto.io/contact)를 통해 피드백을 남겨주시면 지원받을 수 있습니다.
...
- 회원가입 시 꼭 필요한 데이터만 수집하고, 추가 정보는 이후 프로필 완성 단계로 미루세요.
