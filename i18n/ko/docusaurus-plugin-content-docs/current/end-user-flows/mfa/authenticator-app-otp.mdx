...
인증자 앱, 소프트웨어 토큰이라고도 불리는 것은 가장 널리 채택된 [MFA](https://auth.wiki/mfa) 방법 중 하나입니다. 이는 온라인 서비스 인증의 보안을 강화하기 위해 일회용 [비밀번호 (OTP)](https://auth.wiki/otp)를 생성합니다. 물리적 하드웨어 토큰과 달리, 소프트웨어 토큰은 일반적으로 사용자가 스마트폰이나 컴퓨터 브라우저에 설치하는 애플리케이션이나 플러그인입니다. 소프트웨어 토큰은 인증자의 기능과 개별 사용자 설정에 따라 단일 장치에서 로컬로 작동하거나 다양한 장치 간에 동기화될 수 있습니다.
...
<img src={otpSetUpFlowImage} alt="OTP 설정 흐름" />
...
<img src={otpVerificationFlowImage} alt="OTP 검증 흐름" />
