...
## MFA용 이메일 인증 설정 \{#configure-email-verification-for-mfa}
...
4. MFA 전용 사용 유형으로 [이메일 템플릿](/connectors/email-connectors/email-templates)을 설정하세요.
   ...
   - 팁: [Logto 이메일 서비스](/connectors/email-connectors/built-in-email-service)는 내장 이메일 템플릿을 제공합니다.
...
1. **로그인 방식 제한**: 이메일 인증 코드는 [로그인 방식(1FA)](/end-user-flows/sign-up-and-sign-in/sign-in)과 MFA 요소(2FA)로 동시에 사용할 수 없습니다. 이메일 구현당 하나의 인증 흐름만 선택하세요.
...
3. **비밀번호 재설정 호환성**: 이메일 인증 코드는 [비밀번호 찾기](/end-user-flows/sign-up-and-sign-in/reset-password)와 MFA에 동시에 사용할 수 있지만, 이 조합은 **권장하지 않습니다**. 이 구성은 사용자가 비밀번호 찾기 이메일 인증을 통해 비밀번호를 재설정한 후, 새 비밀번호로 1차 인증(1FA)을 진행하고 동일한 이메일 방식으로 MFA 인증을 할 수 있으므로 MFA 보안 효과가 저하됩니다.

:::

## 이메일 MFA 설정 흐름 \{#email-mfa-setup-flows}

MFA 설정 프롬프트는 사용자 등록 시 또는 로그인 후에 표시될 수 있으며, 이는 구성한 [MFA 정책](/end-user-flows/mfa/configure-mfa#global-mfa-configuration)에 따라 다릅니다. 사용자는 [계정 설정 페이지](/end-user-flows/account-settings/by-account-api#update-or-link-new-email)에서 이메일 MFA를 직접 활성화할 수도 있습니다.
...
- **회원가입 식별자 구성**: 이메일 주소가 [회원가입 식별자](/end-user-flows/sign-up-and-sign-in/sign-up#set-up-the-sign-up-identifier)로 사용되었고, 사용자가 회원가입 시 이미 이메일 인증 코드를 통해 인증을 마쳤다면, 시스템이 해당 이메일을 자동으로 MFA 요소로 바인딩하며 추가 인증이 필요하지 않습니다. 다른 주요 요소가 있다면 UI에 "다른 2단계 인증 추가" 옵션이 표시됩니다(사용자는 건너뛸 수 있음). 이로써 MFA가 활성화되었음을 명확히 안내합니다.
...
### 시나리오 1: MFA에만 사용하는 이메일 주소 (일반적인 흐름) \{#scenario-1-email-address-only-used-for-mfa-typical-flow}
...
### 시나리오 2: 회원가입 식별자로 인증된 이메일 \{#scenario-2-email-verified-as-the-sign-up-identifier}
...
### 시나리오 3: 이메일 인증 완료, 주요 요소가 여러 개 \{#scenario-3-email-verified-but-multiple-primary-factors-available}
...
여러 MFA 요소가 설정되어 있다면, 사용자는 구성된 요소 중에서 선택할 수 있습니다. 시스템은 [MFA 구성](/end-user-flows/mfa/configure-mfa#mfa-verification-flow)에서 지정한 우선순위에 따라 먼저 안내할 MFA 요소를 결정합니다.
