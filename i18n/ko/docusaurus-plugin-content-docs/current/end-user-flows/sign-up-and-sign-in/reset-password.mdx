...
비밀번호 복구 기능은 유효한 [Email 커넥터](/connectors/email-connectors) 또는 [SMS 커넥터](/connectors/sms-connectors)를 구성하면 자동으로 활성화됩니다. 사용자는 등록된 이메일 주소 또는 전화번호를 제공하여 비밀번호를 재설정할 수 있습니다.
...
5. **인증 코드 입력**: 사용자는 이메일 또는 전화로 받은 인증 코드를 입력합니다. Logto는 코드와 이메일 주소 또는 전화번호와 연결된 사용자의 아이덴티티를 확인합니다.
...
새 사용자 또는 비밀번호를 업데이트하는 사용자를 위해 비밀번호 강도 요구 사항을 적용하는 비밀번호 정책을 설정할 수 있습니다. <CloudLink to="/sign-in-experience/password-policy">Console > 로그인 경험 > 비밀번호 정책</CloudLink>을 방문하여 비밀번호 정책 설정을 구성하세요.
...
6. **사용자 정의 단어**: 비밀번호에서 거부하고자 하는 사용자 정의 단어 목록 (대소문자 구분 없음)을 제공합니다.
...
사용자가 비밀번호를 성공적으로 재설정할 때 알림을 받기 위해 `PostResetPassword` [웹훅 이벤트](/developers/webhooks/webhooks-events#user-interaction-hook-events)를 구독하세요. 그런 다음 사용자의 현재 세션을 무효화하고 로그인 페이지로 리디렉션하는 [로그아웃](/end-user-flows/sign-out) 작업을 트리거할 수 있습니다.
