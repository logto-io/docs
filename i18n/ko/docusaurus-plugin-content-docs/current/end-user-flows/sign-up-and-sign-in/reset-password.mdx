...
5. **인증 코드 입력**: 사용자는 이메일 또는 전화로 받은 인증 코드를 입력합니다. Logto는 이메일 주소 또는 전화번호와 연결된 사용자의 코드를 확인합니다.
...
### 비밀번호 재설정 후 사용자를 어떻게 로그아웃시키나요? \{#how-to-sign-out-a-user-after-a-successful-password-reset}

</summary>

사용자가 비밀번호를 성공적으로 재설정하면 알림을 받기 위해 `PostResetPassword` [웹훅 이벤트](/developers/webhooks/webhooks-events#user-interaction-hook-events)를 구독하세요. 그런 다음 사용자의 현재 세션을 무효화하고 로그인 페이지로 리디렉션하기 위해 [로그아웃](/end-user-flows/sign-out) 동작을 트리거할 수 있습니다.
...
### 내 커스텀 UI에서 비밀번호 재설정 흐름을 어떻게 구현하나요? \{#how-to-implement-the-password-reset-flow-on-my-custom-ui}
