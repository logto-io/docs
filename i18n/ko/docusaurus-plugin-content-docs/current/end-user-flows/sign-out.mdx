...
4. 사용자가 로그인 페이지와 상호작용하여 인증을 완료합니다.
...
8. 클라이언트가 인증 코드를 받아 토큰으로 교환하여 사용자 정보를 얻습니다.
...
위 플로우에서 중앙화된 로그인 세션은 Logto가 관리합니다. 사용자가 성공적으로 로그인하면 세션이 생성되고, 로그아웃 시 세션이 파기됩니다. 사용자의 세션이 만료되어도 세션이 파기됩니다.

Logto 로그인 세션은 세션 쿠키로 관리됩니다. 사용자가 로그인할 때 세션 쿠키가 설정됩니다. 모든 인증 요청은 세션 쿠키를 기준으로 검증됩니다. 세션 쿠키가 존재하고 유효하다면 사용자는 자동으로 인증되어 인가 코드와 함께 클라이언트 애플리케이션으로 바로 리디렉션됩니다. 그렇지 않으면 사용자는 로그인하라는 안내를 받게 됩니다.
...
각 클라이언트 애플리케이션은 자체적으로 인증 상태를 관리합니다. Native, SPA, Web 애플리케이션 모두 사용자 인증 상태를 관리하는 고유한 방식을 가집니다.

로그인에 성공하면, 클라이언트 애플리케이션은 [ID 토큰](https://auth.wiki/id-token) 및 [액세스 토큰](https://auth.wiki/access-token)을 받을 수 있습니다. 클라이언트 애플리케이션은 ID 토큰으로 사용자의 아이덴티티를 확인하고, 액세스 토큰으로 사용자 리소스에 접근할 수 있습니다. 사용자의 인증 상태는 액세스 토큰의 만료 시간으로 표현됩니다.

- Native 및 SPA 애플리케이션:  
  클라이언트 애플리케이션은 사용자의 인증 상태 유지를 위해 이러한 토큰을 안전하게 저장 및 관리해야 합니다. 예를 들어, 토큰을 local storage 또는 session storage에 저장하고, 사용자가 로그아웃할 때 토큰을 삭제합니다.
- Web 애플리케이션:  
  Next.js와 같은 프레임워크로 구축된 웹 앱은 Logto에서 발급한 토큰과 별도로 자체 세션을 관리하는 경우가 많습니다. 사용자가 로그인하고 Logto에서 토큰을 받으면, SPA처럼 클라이언트 측에 토큰을 저장하거나, 서버 측에 저장하여 쿠키 등으로 세션을 관리할 수 있습니다.

## 로그아웃 메커니즘 \{#sign-out-mechanisms}

### 클라이언트 측에서 토큰 및 로컬 세션 삭제 \{#clear-tokens-and-local-session-at-the-client-side}

클라이언트 측에서 간단한 로그아웃은 로컬 세션을 삭제하고 local storage 또는 session storage에서 토큰 (ID 토큰, 액세스 토큰, 리프레시 토큰)을 제거하는 것입니다. 이 경우 중앙화된 세션은 그대로 남아 있으므로, 사용자는 중앙화된 세션이 만료되거나 명시적으로 파기되기 전까지 동일 인가 서버 하의 다른 애플리케이션에 계속 접근할 수 있습니다.
...
**end session endpoint**는 클라이언트 애플리케이션이 인가 서버에 사용자가 로그아웃했음을 알릴 수 있는 표준 OIDC 엔드포인트입니다. 이 엔드포인트는 Logto에서 중앙화된 로그인 세션을 삭제합니다.

세션이 삭제되면, 이후의 모든 인가 요청은 사용자가 다시 로그인해야 합니다.

**post-logout redirect URI**가 제공되면, 세션 삭제 후 사용자는 해당 URI로 리디렉션됩니다. 그렇지 않으면, Logto가 호스팅하는 기본 로그아웃 후 페이지로 리디렉션됩니다.
...
클라이언트 애플리케이션에서 로그아웃 토큰에 로그인 세션이 포함되길 원한다면, 백채널 로그아웃 설정에서 `Is session required` 옵션을 켜세요. 이 경우 로그아웃 토큰에 사용자의 Logto 로그인 세션을 식별하는 `sid` 클레임이 포함됩니다.
...
클라이언트 애플리케이션이 로그아웃 토큰을 받았을 때의 로그아웃 작업:
...
`prompt=login`을 설정하면 사용자가 활성 세션이 있거나 최근에 로그인했더라도 Logto가 항상 로그인 페이지를 표시합니다. 이는 싱글 사인온 (SSO) 동작을 우회하고, 매번 사용자에게 자격 증명을 입력하도록 보장합니다.
