...
IdP-initiated SSO는 아이덴티티 제공자 (IdP)가 주로 인증 흐름을 제어하는 싱글 사인온 프로세스입니다. 이 프로세스는 사용자가 회사 포털이나 중앙 아이덴티티 대시보드와 같은 IdP의 플랫폼에 로그인할 때 시작됩니다. 인증이 완료되면, IdP는 SAML 어설션을 생성하고 사용자를 서비스 제공자 (SP)로 안내하여 애플리케이션이나 서비스를 이용할 수 있도록 합니다.
...
사용자 주도의 인증이 부족하면 적절한 보호 장치가 없을 경우 무단 접근으로 이어질 수 있습니다. 또한, 단일 인증 지점에 의존하는 것은 보안 침해의 위험을 증가시킵니다. IdP가 손상되면 연결된 모든 애플리케이션이 노출될 수 있기 때문입니다.
...
SAML 커넥터가 설정되면, <CloudLink to="/sign-in-experience">로그인 경험</CloudLink> 섹션에서 SSO 로그인 방법을 활성화하고, 설정이 올바른지 확인하기 위해 SP-initiated SSO 흐름을 테스트하세요. IdP-initiated SSO를 진행하기 전에 SP-initiated SSO가 예상대로 작동하는지 확인하세요.
...
2. IdP-initiated SSO 인증 흐름을 시작한 SAML 커넥터를 식별하기 위해 `ssoConnectorId` 쿼리 매개변수를 사용하여 클라이언트 측에서 OIDC 인증 요청을 처리합니다.
...
:::note
이 방법은 **전통적인 웹 앱**과 **싱글 페이지 앱** 애플리케이션 모두에 사용할 수 있습니다. 모든 사용 사례에 권장됩니다.
:::
...
:::warning
이 방법은 **싱글 페이지 앱** 애플리케이션에는 사용할 수 없습니다. 클라이언트 앱이 `PKCE` 매개변수를 사용하여 인증 요청을 안전하게 처리해야 하기 때문입니다. SPA 애플리케이션에 대해 IdP-initiated SSO를 구현해야 하는 경우, 위의 옵션을 사용하세요.
:::
...
1. 성공적인 인증 후 사용자를 클라이언트 애플리케이션으로 다시 리디렉션할 **로그인 후 리디렉션 URI**를 선택합니다. 이 URL은 OIDC 인증 요청에서 `redirect_uri`로 사용됩니다. URI는 클라이언트 애플리케이션에 등록된 허용된 리디렉션 URI 중 하나여야 합니다.
   ...
   :::note
   IdP-initiated SSO에 대해 전용 **리디렉션 URI**를 사용하는 것이 강력히 권장됩니다. 인증 요청이 비요청된 것이기 때문에, 클라이언트 애플리케이션은 표준 SP-initiated 인증 흐름과 별도로 응답을 독립적으로 관리해야 합니다.
   :::
