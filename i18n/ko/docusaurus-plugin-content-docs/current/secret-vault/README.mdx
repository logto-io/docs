...
<DocCardList
  items={[
    {
      type: 'link',
      label: '페더레이티드 토큰 세트 (Federated token set)',
...
:::info
현재, 페더레이티드 토큰 세트 (Federated token set)가 기본적으로 지원되는 유일한 시크릿 유형입니다. 그러나 시크릿 볼트 (Secret Vault)는 모든 종류의 민감한 정보를 저장할 수 있도록 설계되었습니다. 추가 시크릿 유형 지원이 필요하다면, 언제든 [문의해 주세요](https://logto.io/contact) — 기꺼이 도와드리겠습니다.
:::
