...
tags: [인증, apple, dotnet-core-blazor-webassembly, c#, 소셜 로그인, 로그인, 로그인]
title: Apple 로그인 을 .NET Core (Blazor WebAssembly)로 구축하는 방법
...
import ConnectorGuide from '@site/i18n/ko/docusaurus-plugin-content-docs/current/integrations/social/apple/_integration.mdx';
...
## Apple 커넥터 추가하기 \{#add-connector-connector}

<AddSocialConnector connector={connector} framework=".NET Core (Blazor WebAssembly)" />

## Apple Sign-in 설정하기 \{#set-up-connectorconfigname}
...
## 로그인 경험에서 Apple 커넥터 활성화하기 \{#enable-connector-connector-in-sign-in-experience}
