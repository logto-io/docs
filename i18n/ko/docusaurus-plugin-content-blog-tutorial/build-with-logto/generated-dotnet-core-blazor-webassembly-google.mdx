...
tags: [인증, google, dotnet-core-blazor-webassembly, c#, 소셜 로그인, 로그인, 로그인]
title: Google 로그인 을 .NET Core (Blazor WebAssembly)로 구축하는 방법
...
import ConnectorGuide from '@site/i18n/ko/docusaurus-plugin-content-docs/current/integrations/social/google/_integration.mdx';
...
## Google 커넥터 추가하기 \{#add-connector-connector}

<AddSocialConnector connector={connector} framework=".NET Core (Blazor WebAssembly)" />

## Google OAuth 앱 설정하기 \{#set-up-connectorconfigname}
...
## 로그인 경험에서 Google 커넥터 활성화하기 \{#enable-connector-connector-in-sign-in-experience}
