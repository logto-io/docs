---
slug: how-to-build-microsoft-entra-id-oidc-enterprise-sso-sign-in-with-next-and-logto
tags: [SSO providers, microsoft-entra-id-oidc-enterprise-sso, next, js, SAML authentication, enterprise login, SSO login]
title: Microsoft Entra ID OIDC enterprise SSO를 Next.js (Pages Router)로 구축하는 방법
sdk: Next.js (Pages Router)
connector: Microsoft Entra ID OIDC enterprise SSO
---

{/*
  목차에 표시되지 않기 때문에 템플릿을 섹션으로 나누어야 합니다.
  목차를 사용자 정의할 방법도 없습니다. :-)

  @see https://github.com/facebook/docusaurus/issues/3915
*/}

import Intro from './fragments/_intro.mdx';
import CreateApplication from './fragments/_create-application.mdx';
import AddSsoConnector from './fragments/_add-sso-connector.mdx';
import SaveYourConfiguration from './fragments/_save-your-configuration.mdx';
import EnableSsoConnector from './fragments/_enable-sso-connector.mdx';
import TestingAndValidation from './fragments/_testing-and-validation.mdx';
import IntegrateSdk from '@site/i18n/ko/docusaurus-plugin-content-docs/current/quick-starts/framework/next/_for-tutorial.mdx';
import ConnectorGuide from '@site/i18n/ko/docusaurus-plugin-content-docs/current/integrations/sso/entra-id-oidc/_integration.mdx';

import FurtherReadings from './fragments/_further-readings.mdx';

export const connector = 'Microsoft Entra ID OIDC enterprise SSO';

{/* truncate */}

<Intro connector={connector} sdk="Next.js (Pages Router)" link="https://nextjs.org" />

## Logto에 애플리케이션 생성하기 \{#create-an-application-in-logto}

<CreateApplication type="Traditional web" framework="Next.js" />

## Next.js SDK 통합하기 \{#integrate-framework-sdk}

<IntegrateSdk />

## Microsoft Entra ID OIDC enterprise SSO 커넥터 추가하기 \{#add-connector-connector}

<AddSsoConnector framework="Next.js" />

## Azure AD SSO 애플리케이션 설정하기 \{#set-up-connectorconfigname}

<ConnectorGuide />

### 구성 저장하기 \{#save-your-configuration}

<SaveYourConfiguration connector={connector} />

## 로그인 경험에서 Microsoft Entra ID OIDC enterprise SSO 커넥터 활성화하기 \{#enable-connector-connector-in-sign-in-experience}

<EnableSsoConnector />

## 테스트 및 검증 \{#testing-and-validation}

<TestingAndValidation connector={connector} sdk="Next.js (Pages Router)" />

## 추가 읽을거리 \{#further-readings}

<FurtherReadings />