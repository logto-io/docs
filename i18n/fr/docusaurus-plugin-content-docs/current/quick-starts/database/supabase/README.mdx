...
![Page des paramètres de l'API Supabase](./supabase-api-settings.webp)

Ensuite, lorsque nous utilisons le SDK Supabase pour faire des requêtes à Supabase, nous utilisons ce secret pour générer notre JWT et l'attacher comme en-tête d'Authentification à la requête. (Veuillez noter que ce processus se déroule dans le service backend de votre application, et le secret JWT ne doit jamais être exposé à des tiers).
...
![Créer une fonction pour obtenir l'ID utilisateur](./create-get-user-id-function.webp)
...
![Créer une politique RLS](./create-rls-policy.webp)

En tirant parti de telles politiques, le contrôle d'accès aux données au sein de Supabase est réalisé.
...
Dans ce code, Supabase ne retournera que les données de post appartenant à l'utilisateur actuel en fonction des politiques précédemment définies.
...
Prenons un projet Next.js comme exemple, après avoir intégré Logto à votre projet en suivant le [Guide du SDK Next.js](/quick-starts/next-app-router), vous pouvez utiliser le SDK Logto pour récupérer les informations utilisateur et construire le JWT correspondant pour interagir avec Supabase.
...
De plus, si vous souhaitez qu'une application M2M spécifique accède à Supabase au nom de l'ensemble du service pour contourner les restrictions RLS, vous pouvez utiliser le secret `service_role` de Supabase pour créer un client Supabase. C'est utile lorsque vous souhaitez effectuer certaines tâches administratives ou automatisées qui nécessitent un accès à toutes les données sans être restreint par les politiques de sécurité au niveau des lignes définies pour les utilisateurs individuels.

Le secret `service_role` peut être trouvé sur la même page que le secret JWT :

![Secret du rôle de service](./service-role-secret.webp)
