...
En supposant que vous traitiez `io.logto.android` comme le schéma personnalisé `LOGTO_REDIRECT_SCHEME`, et `io.logto.sample` comme le nom de votre package d'application, l'URI de redirection devrait être `io.logto.android://io.logto.sample/callback`.
