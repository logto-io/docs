---
slug: /quick-starts/webflow
sidebar_label: Webflow
sidebar_custom_props:
  description: Webflow est une plateforme SaaS pour la création et l’hébergement de sites web.
language: webflow
official_link: https://webflow.com
app_type: Application monopage
framework: Webflow
---

import FurtherReadings from '../../fragments/_further-readings.md';

import Checkpoint from './_checkpoint-test-your-webflow-site.md';
import GetUserInformation from './_get-user-information.mdx';
import GuideTip from './_guide-tip.mdx';
import HandleAuthenticationStatus from './_handle-authentication-status.mdx';
import ImplementSignIn from './_implement-sign-in.mdx';
import ImplementSignOut from './_implement-sign-out.mdx';
import InitClient from './_init-client.mdx';
import ApiResources from './api-resources/_index.mdx';

# Ajoutez l’authentification à votre application Webflow (Add authentication to your Webflow application)

Ce guide vous montrera comment intégrer Logto à vos sites Webflow.

<GuideTip />

## Prérequis \{#prerequisits}

1. L’intégration de Logto avec Webflow nécessite la fonctionnalité "Custom code" de Webflow, qui requiert au minimum l’offre "Basic".
2. Un site Webflow, soit en utilisant un site existant, soit en en créant un nouveau.

## Intégration \{#integration}

### Initialiser le fournisseur Logto \{#init-logto-provider}

<InitClient />

### Implémenter la connexion \{#implement-sign-in}

<ImplementSignIn />

### Implémenter la déconnexion \{#implement-sign-out}

<ImplementSignOut />

### Gérer le statut d’authentification \{#handle-authentication-status}

<HandleAuthenticationStatus />

<Checkpoint />

## Obtenir les informations de l’utilisateur \{#get-user-information}

<GetUserInformation />

## Ressources API \{#api-resources}

<ApiResources />

## Pour aller plus loin \{#further-readings}

<FurtherReadings />
