...
### Étape 1 : Activer Account API \{#step-1-enable-account-api}

L’interface préconstruite du Centre de compte repose sur Account API. Rendez-vous sur <CloudLink to="/sign-in-experience/account-center">Console > Connexion & compte > Centre de compte</CloudLink> et activez Account API.
...
1. Assurez-vous que la permission du champ **Name**, **Avatar**, **Profile** ou **Custom data** est définie sur `Edit` ou `ReadOnly` dans les sections Sécurité du compte / Profil utilisateur.
...
Seuls les champs à la fois listés dans les champs de profil **et** dont la permission correspondante n’est pas `Off` apparaîtront sur la page Profil.
...
3. Après l’action, l’utilisateur est redirigé vers votre application
...
Vous pouvez ajouter les paramètres de requête suivants à toute URL du Centre de compte pour personnaliser l’expérience :

| Paramètre      | Description                                                                                                                                                                                                                      |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `redirect`     | L’URL absolue vers laquelle renvoyer l’utilisateur après l’action. Seules les URLs `http(s)` sont acceptées.                                                                                                                     |
| `show_success` | Si défini à `true`, la destination de succès (par exemple, votre URL `redirect`) reçoit un paramètre de requête `show_success` pour vous permettre d’afficher un message de confirmation.                                        |
| `identifier`   | Pré-remplit le champ d’identifiant sur la page cible (`/account/email`, `/account/phone`, `/account/username`). Utile pour un lien profond depuis votre application si vous connaissez déjà l’identifiant de l’utilisateur.      |
...
Le Centre de compte ne gère pas directement la suppression de compte. À la place, vous pouvez configurer une **URL de suppression de compte** qui pointe vers votre propre flux de suppression (généralement basé sur [Management API](/end-user-flows/account-settings/by-management-api)). Lorsqu’elle est configurée, une entrée **Supprimer votre compte** apparaît sur la page de sécurité du Centre de compte et redirige l’utilisateur vers votre URL.

Pour la configurer, rendez-vous sur <CloudLink to="/sign-in-experience/account-center">Console > Connexion & compte > Centre de compte</CloudLink> et renseignez le champ **URL de suppression de compte**. Vous pouvez également la mettre à jour via Management API :
...
Si vous avez besoin de plus de personnalisation que ce que l’interface préconstruite et le CSS personnalisé offrent, envisagez d’utiliser [Account API](/end-user-flows/account-settings/by-account-api) pour construire vos propres pages de gestion de compte.
...
- [Appareils de confiance MFA](/end-user-flows/mfa/trusted-devices) - Configurer et gérer la confiance limitée dans le temps pour les navigateurs
