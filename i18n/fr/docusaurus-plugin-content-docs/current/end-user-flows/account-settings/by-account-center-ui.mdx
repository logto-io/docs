...
| `show_success` | Si défini à `true`, la destination de succès (par exemple, votre URL `redirect`) reçoit un paramètre de requête `show_success` pour que vous puissiez afficher un message de confirmation.                                |
...
| `ui_locales`   | Liste d’étiquettes de langue BCP-47 séparées par des espaces (ex. `fr-CA fr en`) qui contrôle la langue de l’interface du Centre de compte. Par défaut, la langue du navigateur de l’utilisateur est utilisée si omise.   |
...
## URL de rappel pour connexion sociale \{#social-connection-callback-url}
...
Où `{connectorId}` est l’identifiant du connecteur social (vous pouvez le trouver dans <CloudLink to="/connectors/social">Console > Connecteurs > Connecteurs sociaux</CloudLink>).
