...
- **Fournisseur de services (SP)** : Un système ou une application qui nécessite l'authentification des utilisateurs et s'appuie sur le Fournisseur d'Identité (IdP) pour l'authentification. Le SP reçoit des jetons ou assertions d'authentification de l'IdP, accordant l'accès à ses ressources sans nécessiter d'identifiants de connexion séparés. Exemples : Slack, Shopify, Dropbox, Figma, Notion, etc. et votre service. [En savoir plus sur SP](https://auth.wiki/service-provider).
...
- [**SSO initié par IdP**](/end-user-flows/enterprise-sso/idp-initiated-sso) : Dans le SSO initié par IdP, le Fournisseur d'Identité (IdP) contrôle principalement le processus d'authentification unique. Ce processus commence lorsqu'un utilisateur se connecte à la plateforme de l'IdP, telle qu'un portail d'entreprise ou un tableau de bord d'identité centralisé. Une fois authentifié, l'IdP génère un jeton ou une assertion d'authentification, qui est ensuite utilisé pour accorder sans effort à l'utilisateur l'accès à plusieurs services ou applications connectés (SP) sans nécessiter de connexions supplémentaires.
  ![SSO initié par IdP](./assets/idp-initiated-sso.png)
- [**SSO initié par SP**](/end-user-flows/enterprise-sso/sp-initiated-sso) : Dans le SSO initié par SP, le Fournisseur de Services (SP) prend l'initiative d'initier et de gérer le processus d'authentification unique, souvent préféré dans les scénarios B2B. Ce scénario se produit lorsqu'un utilisateur tente d'accéder à un service ou une application spécifique (le SP) et est redirigé vers son IdP pour l'authentification. Après une connexion réussie à l'IdP, un jeton d'authentification est renvoyé au SP, accordant l'accès à l'utilisateur. Logto prend en charge le SSO initié par SP pour vos services B2B.
  ![SSO initié par SP](./assets/sp-initiated-sso.png)
