...
Parfois, vous pouvez souhaiter inviter de nouveaux utilisateurs à votre application / organisation sans leur demander de créer un compte au préalable. Dans de tels cas, l'application peut envoyer un "lien magique" à votre adresse e-mail. Vous serez alors authentifié immédiatement lorsque vous cliquerez sur le lien.
...
sequenceDiagram
  ...
  C -->> U: L'utilisateur est authentifié et reçoit les informations utilisateur
```

## Guide de mise en œuvre \{#implementation-guide}

Logto propose un ensemble d'APIs de gestion et d'APIs d'expérience pour faciliter la mise en œuvre de votre lien magique.
...
et le point de terminaison Logto (requis pour les APIs de gestion). En savoir plus sur [Logto Management API](/integrate-logto/interact-with-management-api).
...
Les noms des paramètres dans le lien magique peuvent être entièrement personnalisés.
...
const TokenLandingPage = () => {
  ...
  useEffect(() => {
    ...
    // Supposons qu'il s'agit de votre URI de redirection de connexion
...
Si ce paramètre est spécifié, vous devrez également effacer manuellement les jetons dans la page de rappel de connexion.
