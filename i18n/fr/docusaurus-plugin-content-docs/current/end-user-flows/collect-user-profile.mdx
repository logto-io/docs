...
- **Expérience utilisateur optimisée** : Personnalisez l'affichage avec des libellés, descriptions, exemples et règles de validation. Configurez les champs comme obligatoires ou optionnels selon vos besoins métier.
...
5. Testez l'expérience utilisateur avec [aperçu en direct Logto](/customization/live-preview) ou votre application de test. Que les utilisateurs créent un nouveau compte via [identifiant (e-mail / numéro de téléphone / nom d'utilisateur)](/end-user-flows/sign-up-and-sign-in/sign-up), [connexion sociale](/end-user-flows/sign-up-and-sign-in/social-sign-in) ou [SSO d’entreprise](/end-user-flows/enterprise-sso), ils verront tous la page "Parlez-nous de vous" lors de l'inscription.
...
Lors de la création de champs personnalisés, utilisez uniquement des caractères alphanumériques pour la clé de donnée utilisateur (ex : `customData.companySize`). Vous pouvez ensuite configurer le [type de champ](#field-types) et personnaliser d'autres propriétés selon vos besoins.
...
| --------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Libellé         | Tous                      | Nom affiché aux utilisateurs. Flotte en haut lorsque le champ a une valeur ou est sélectionné. Champ obligatoire. |
...
| Obligatoire     | Tous (sauf case à cocher) | Si activé, la soumission est bloquée si vide. Si désactivé, une mention "(Optionnel)" est ajoutée au libellé.     |

### Configurations spécifiques au type \{#type-specific-configurations}

| Type (14%)                   | Aspects configurables (36%)      | Remarques                                                                                                                                                                                                                                                                                       |
| ---------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Texte                        | min/max longueur                 | Limite la longueur minimale et maximale du texte saisi par l'utilisateur.                                                                                                                                                                                                                       |
| Nombre                       | min/max valeur                   | Limite la plage numérique minimale et maximale de la valeur saisie.                                                                                                                                                                                                                             |
| Date                         | Format de date (prédéfini/perso) | Choisissez des formats prédéfinis (MM/jj/aaaa, jj/MM/aaaa, aaaa-MM-jj). Modifier l'exemple remplace les interactions par défaut. Ou utilisez des motifs <a href="https://date-fns.org/v2.30.0/docs/format" target="_blank" rel="noopener noreferrer">date-fns</a> entièrement personnalisables. |
...
| URL                          | N/A                              | N/A                                                                                                                                                                                                                                                                                             |
| Regex                        | Expression régulière             | Motif de validation (omettre les barres de début / fin). Ex : <code>^\d{3}-\d{2}-\d{4}$</code> pour un numéro de sécurité sociale.                                                                                                                                                              |
...
- Fournissez des exemples clairs / des indices pour les champs sensibles au format (regex, date, plages numériques).
