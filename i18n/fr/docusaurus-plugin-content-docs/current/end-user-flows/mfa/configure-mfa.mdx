...
3. Choisissez la politique **Exiger MFA** dans le menu déroulant. Cette politique contrôle quand les utilisateurs doivent compléter la MFA lors de la connexion :
   - **MFA optionnelle** : Laissez les utilisateurs décider d’activer ou non la MFA pour la sécurité de leur propre compte. Les utilisateurs peuvent ignorer la configuration MFA lors de la connexion et la configurer plus tard via votre page de paramètres de compte en libre-service. [En savoir plus](/end-user-flows/account-settings/) sur la mise en œuvre d’une page de paramètres de compte utilisateur.
   - **MFA adaptative** : Appliquez la MFA uniquement lorsqu’une connexion semble inhabituelle, afin que les connexions à faible risque restent fluides tandis que les connexions suspectes nécessitent une vérification supplémentaire. Pour prendre cette décision, Logto évalue des signaux contextuels de haut niveau tels que le niveau d’inactivité de l’utilisateur, la cohérence du contexte de connexion avec les habitudes de déplacement récentes, l’état de la connexion réseau et d’autres indicateurs de session ou d’environnement. Ce mode dépend de votre configuration MFA existante (au moins un facteur MFA doit être activé), s’applique aux flux de connexion des utilisateurs finaux et ne repose pas sur l’empreinte de l’appareil ou le suivi précis de la localisation.
   - **MFA obligatoire** : Exigez que tous les utilisateurs complètent la MFA à chaque connexion. Les utilisateurs qui n’ont pas configuré la MFA doivent terminer la configuration avant de pouvoir continuer.
   - Lorsque **MFA optionnelle** ou **MFA adaptative** est sélectionnée, configurez la politique d’invite de configuration MFA :
     ...
     - **Demander aux utilisateurs de configurer la MFA lors de l’inscription** : Les nouveaux utilisateurs seront invités à configurer la MFA lors de l’inscription, et les utilisateurs existants verront l’invite lors de leur prochaine connexion. En mode **MFA optionnelle**, les utilisateurs peuvent ignorer cette étape et elle ne s’affichera plus. En mode **MFA adaptative**, une fois invités, les utilisateurs doivent terminer la configuration MFA avant de terminer le flux de connexion ou d’inscription en cours.
     - **Demander aux utilisateurs de configurer la MFA lors de leur connexion après inscription** : Les nouveaux utilisateurs seront invités à configurer la MFA lors de leur deuxième connexion après inscription, et les utilisateurs existants verront l’invite lors de leur prochaine connexion. En mode **MFA optionnelle**, les utilisateurs peuvent ignorer cette étape et elle ne s’affichera plus. En mode **MFA adaptative**, une fois invités, les utilisateurs doivent terminer la configuration MFA avant de terminer le flux de connexion ou d’inscription en cours.

Lorsque **MFA optionnelle** ou **MFA adaptative** est sélectionnée et que Logto décide d’inviter l’utilisateur à s’inscrire, le flux affiche d’abord une page de confirmation d’activation MFA intitulée `Activer la vérification en 2 étapes`. En mode **MFA optionnelle**, les utilisateurs peuvent choisir `Activer la vérification en 2 étapes` pour continuer à l’association MFA, ou ignorer l’invite et terminer le flux de connexion / inscription en cours sans activer la MFA. En mode **MFA adaptative**, une fois invités, les utilisateurs doivent continuer à l’association MFA avant de terminer le flux de connexion ou d’inscription en cours.
...
2. **Compléter la connexion ou l’inscription** : L’utilisateur termine le processus de vérification d’identité dans le flux de connexion ou d’inscription.
   - Si **MFA optionnelle** ou **MFA adaptative** est sélectionnée et que votre politique d’invite est activée, Logto peut d’abord afficher la page `Activer la vérification en 2 étapes` avant le début de l’association d’un facteur MFA.
   - En mode **MFA optionnelle**, les utilisateurs peuvent ignorer cette page ou cliquer sur `Activer la vérification en 2 étapes` pour continuer. En mode **MFA adaptative**, une fois invités, les utilisateurs doivent continuer à l’association MFA.
...
- **Associer de nouveaux facteurs MFA** : Ajouter des applications d’authentification supplémentaires, des passkeys ou régénérer des codes de secours
...
- **Supprimer la MFA de l’utilisateur** : Supprimer tous les facteurs MFA d’un utilisateur, l’obligeant à configurer à nouveau la MFA.
...
- Cette approche empêche les utilisateurs d’être bloqués hors de leur compte lorsque plus aucun facteur principal n’est disponible.
