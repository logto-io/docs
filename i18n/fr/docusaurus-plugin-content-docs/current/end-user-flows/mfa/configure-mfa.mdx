...
2. Activez les facteurs de vérification pris en charge pour vos utilisateurs.
   1. Facteurs principaux :
      ...
      - [OTP via application d’authentification](/end-user-flows/mfa/authenticator-app-otp) : La méthode la plus courante et largement acceptée. Utilisez un mot de passe à usage unique basé sur le temps (TOTP) généré par une application d’authentification comme Google Authenticator ou Authy.
      - [Vérification par SMS](/end-user-flows/mfa/sms-mfa) : Une méthode pratique qui envoie des codes de vérification à usage unique par SMS au numéro de téléphone enregistré de l’utilisateur, idéale pour les utilisateurs préférant l’authentification mobile sans applications supplémentaires.
...
3. **Configurer le facteur principal MFA** : L’utilisateur est invité à configurer son facteur principal MFA (soit passkey, OTP via application d’authentification, code SMS ou code e-mail).
