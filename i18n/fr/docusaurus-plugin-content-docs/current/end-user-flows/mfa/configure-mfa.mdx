...
2. Activez les facteurs de vérification pris en charge pour vos utilisateurs.
   1. Facteurs principaux :
      ...
      - [Vérification par e-mail](/end-user-flows/mfa/email-mfa) : Une méthode largement accessible qui envoie des codes de vérification à usage unique à l’adresse e-mail enregistrée de l’utilisateur, adaptée à tous les utilisateurs sur toutes les plateformes et appareils.
...
3. Choisissez si vous souhaitez activer **Exiger MFA** :
   ...
   - **Désactiver** : Les utilisateurs peuvent ignorer le processus de configuration MFA lors de l’inscription. Ils pourront configurer MFA plus tard via la page de paramètres de compte en libre-service. [En savoir plus](/end-user-flows/account-settings/) sur la mise en œuvre d’une page de paramètres de compte utilisateur. Puis continuez à choisir la politique pour l’invite de configuration MFA :
...
4. **Configurer le facteur de secours MFA** : Si **Codes de secours** est activé, l’utilisateur est invité à enregistrer les codes de secours après avoir configuré avec succès son facteur principal d’authentification. Des codes de secours générés automatiquement seront affichés à l’utilisateur, qu’il pourra télécharger et stocker en toute sécurité. L’utilisateur doit confirmer manuellement les codes de secours pour terminer le processus de configuration MFA.
...
- Si un utilisateur a configuré plusieurs facteurs pour la 2FA, le système présentera les options de vérification selon les règles de priorité suivantes :
  ...
  - **Préférence du dernier utilisé** : Si aucune passkey n’est disponible, le système privilégiera la méthode de vérification que l’utilisateur a utilisée avec succès en dernier.
...
- **Voir l’état actuel du MFA** : Voir quels facteurs MFA sont actuellement configurés
...
- Après une vérification réussie du code de secours, l’utilisateur sera invité à configurer un nouveau facteur principal MFA.
