...
4. Configurez le [modèle d’e-mail](/connectors/email-connectors/email-templates) pour MFA avec les types d’utilisation dédiés :
   ...
   - Astuce : [Service e-mail Logto](/connectors/email-connectors/built-in-email-service) fournit des modèles d’e-mail intégrés.
...
### Scénario 1 : Adresse e-mail utilisée uniquement pour la MFA (Flux typique) \{#scenario-1-email-address-only-used-for-mfa-typical-flow}
...
Si plusieurs facteurs MFA sont disponibles, les utilisateurs peuvent choisir parmi leurs facteurs configurés. Le système détermine quel facteur MFA afficher en premier selon l’ordre de priorité défini dans [Configurer la MFA](/end-user-flows/mfa/configure-mfa#mfa-verification-flow).
