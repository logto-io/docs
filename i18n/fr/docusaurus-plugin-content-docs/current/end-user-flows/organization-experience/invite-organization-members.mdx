...
Avant d'inviter des membres dans votre organisation, vous devez créer des rôles d'organisation. Consultez le guide [Configurer les organisations](https://docs.logto.io/docs/recipes/organizations/configuration/) pour des instructions détaillées.
...
Le "service de messagerie Logto" intégré de Logto Cloud ne prend pas en charge le type d'utilisation `OrganizationInvitation` pour le moment. Au lieu de cela, vous devez configurer votre connecteur de messagerie (par exemple, Sendgrid) et configurer le modèle `OrganizationInvitation`.

Un exemple de modèle d'e-mail pour le type d'utilisation `OrganizationInvitation` est montré ci-dessous :
...
Vous pouvez consulter le guide [Configurer un service de messagerie populaire](https://docs.logto.io/docs/recipes/configure-connectors/email-connector/configure-popular-email-service/) pour plus de détails sur la configuration des connecteurs de messagerie.

## Gérer les invitations avec Logto Management API

note

Si vous n'avez pas encore configuré le Logto Management API, consultez [Interagir avec Management API](https://docs.logto.io/docs/recipes/interact-with-management-api/) pour plus de détails.
...
- `POST /api/organization-invitations/{id}/message` envoyer l'invitation d'organisation à l'invité par e-mail.
  ...
  ```
  En conséquence, vous devez implémenter une page d'accueil lorsque votre invité navigue à travers le lien d'invitation vers votre application.
...
Veuillez noter que toutes les APIs listées ci-dessus nécessitent un "jeton d’organisation" valide. Consultez ce [guide](https://docs.logto.io/docs/recipes/organizations/integration/#step-2-fetch-the-organization-token) pour apprendre comment obtenir le jeton d’organisation.
...
Dans ce cas, vous devez déclencher un processus de reconnexion ou de re-consentement pour mettre à jour le jeton d’organisation de l'utilisateur. Par exemple, en appelant la méthode `signIn` dans Logto SDK.
