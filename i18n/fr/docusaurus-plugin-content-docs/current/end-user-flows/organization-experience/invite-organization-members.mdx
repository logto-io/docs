...
- `POST /api/organization-invitations/{id}/message` envoyer l'invitation d'organisation à l'invité par e-mail.
  Note : Ce payload API prend en charge une propriété `link`, vous pouvez composer votre lien d'invitation basé sur l'ID de l'invitation. Par exemple :
  ```json
  {
...
## Utiliser le contrôle d’accès basé sur les rôles (RBAC) pour gérer les permissions des utilisateurs

Avec les configurations ci-dessus, vous pouvez maintenant envoyer des invitations par e-mail, et les invités peuvent rejoindre l'organisation avec le rôle attribué.
...
- Si une nouvelle portée est introduite dans votre système d'authentification, déclenchez un processus de reconnexion ou de reconsentement pour mettre à jour le jeton d’organisation de l'utilisateur. Prenons l'exemple du SDK React :
  ...
  ...
  // Si les portées en temps réel récupérées ont des portées nouvellement attribuées que les portées du jeton d’organisation
