...
En supposant que vous ayez déjà connecté votre serveur backend API au point de terminaison Logto Management API via le mécanisme Machine à Machine (M2M), et acquis le jeton d’accès M2M.
