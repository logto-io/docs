...
- `login_hint` : Prérenseigne le champ identifiant avec l'adresse e-mail ou le nom d'utilisateur de l'utilisateur. (Il s'agit d'un paramètre standard OIDC)
...
:::tip
Le premier écran suivra les paramètres configurés dans <CloudLink to="/sign-in-experience/branding">Console > Connexion & compte</CloudLink>. Vous devez d'abord activer les méthodes d'authentification requises, puis configurer la personnalisation, les conditions générales et politiques de confidentialité, ainsi que l'internationalisation (i18n). Notez que seules les pages `sign-in` et `register` affichent le logo par défaut.
:::
