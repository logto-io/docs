...
1. Désactivez l’inscription des utilisateurs dans la page <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Console > Expérience de connexion</CloudLink>.
...
5. Envoyez le lien magique à l’adresse e-mail de l’utilisateur (par exemple, `https://yourapp.com/landing-page?type=registrationInvitation&token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`). Personnalisez le modèle d’e-mail, par exemple :
   ...
   Lorsque les utilisateurs cliquent sur "Accepter l’invitation", ils seront automatiquement inscrits à votre service, même si l’inscription publique est désactivée. Ceci est appelé "Invitation ciblée d’utilisateur".
...
2. Créez manuellement les comptes utilisateurs dans la page <CloudLink to="/users">Console > Gestion des utilisateurs</CloudLink> ou via l’[API de gestion](https://openapi.logto.io/operation/operation-createuser).
...
- **Nom d’utilisateur / E-mail / Numéro de téléphone avec mot de passe** : Créez un compte utilisateur avec un **Nom d’utilisateur**, une **Adresse e-mail** ou un **Numéro de téléphone**, et activez l’identifiant correspondant pour la connexion par mot de passe.
  - Avec un mot de passe prédéfini : Définissez un mot de passe lors de la création du compte, puis partagez-le de manière sécurisée avec l’utilisateur. L’utilisateur pourra alors se connecter directement avec son nom d’utilisateur, e-mail ou numéro de téléphone et le mot de passe prédéfini, puis le réinitialiser s’il le souhaite.
  - Sans mot de passe prédéfini : Créez le compte sans mot de passe, et invitez l’utilisateur à en définir un lors de l’activation du compte. (Une **Adresse e-mail** ou un **Numéro de téléphone** est requis pour le flux de réinitialisation du mot de passe.) Après la création de l’utilisateur, redirigez-le vers la page de réinitialisation du mot de passe avec son e-mail ou numéro de téléphone pré-rempli. Pour savoir comment rediriger les utilisateurs vers la page de réinitialisation du mot de passe avec les informations pré-remplies, consultez les [Paramètres d’authentification](/end-user-flows/authentication-parameters/). Une fois le nouveau mot de passe défini, l’utilisateur pourra se connecter directement avec son nom d’utilisateur, e-mail ou numéro de téléphone et le nouveau mot de passe.
