...
2. Dans la section **Options avancées**, désactivez l’interrupteur **Activer l’inscription des utilisateurs** pour désactiver l’inscription des utilisateurs. (Activé par défaut)
...
- Le flux d’inscription via SSO social et d’entreprise est également désactivé. Les utilisateurs avec des comptes sociaux non enregistrés recevront une erreur "compte non trouvé" après l’authentification sociale.
...
1. Désactivez l’inscription des utilisateurs dans la <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Console > Expérience de connexion</CloudLink>.
...
5. Envoyez le lien magique à l’e-mail de l’utilisateur (par exemple, `https://yourapp.com/landing-page?type=registrationInvitation&token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`). Personnalisez le modèle d’e-mail, tel que :
...
### Option 3 : Notifier les utilisateurs lors de l’inscription manuelle \{#option-3-notify-users-on-manual-registration}

1. Désactivez l’inscription des utilisateurs dans la Console Logto.
2. Créez manuellement les comptes utilisateurs dans la <CloudLink to="/users">Console > Gestion des utilisateurs</CloudLink> ou via l’[API de gestion](https://openapi.logto.io/operation/operation-createuser).
