...
## Mettre en œuvre la connexion sociale

1. **Configurez vos connecteurs sociaux**
   Accédez à <CloudLink to="/connectors/social">Console > Connecteurs > Connecteurs sociaux</CloudLink>. Cliquez sur le bouton “Ajouter un connecteur social” et localisez le connecteur social que vous souhaitez ajouter (par exemple, Google ou Github). Consultez notre documentation pour des instructions détaillées sur l'ajout d'un [connecteur social](/connectors/social-connectors/).
2. **Ajoutez des boutons de connexion sociale** :
   ...
   Accédez à <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Console > Expérience de connexion > Inscription et connexion</CloudLink>. Cliquez sur le bouton “Ajouter un connecteur social” pour intégrer des boutons de connexion sociale sur vos pages d'inscription et de connexion. Utilisez le glisser-déposer pour organiser leur ordre sur l'interface utilisateur.

3. **Définissez les options de liaison de compte** :
   Pour les nouveaux utilisateurs s'inscrivant avec des identités sociales, Logto permet de lier leur compte social à un compte e-mail ou numéro de téléphone existant dans le système Logto. Par défaut, une page de liaison de compte associée est affichée aux utilisateurs lors de l'inscription sociale, leur permettant de lier leur compte social à un compte existant ou d'en créer un nouveau.

   Pour simplifier ce processus, vous pouvez activer l'option **Liaison de compte automatique** dans les paramètres de l' expérience de connexion. Cela liera automatiquement le compte social à un compte existant si l'e-mail ou le numéro de téléphone correspond.

4. **Enregistrez les modifications** : Examinez attentivement vos modifications et enregistrez-les pour appliquer la configuration.

## Expérience utilisateur de la connexion sociale
...
sequenceDiagram
    ...
    user->>app: Visiter l'application
    app->>experience: Rediriger vers la page de connexion
    user->>experience: Cliquer sur le bouton de connexion sociale
    experience->>Logto: Demande de connexion avec le connecteur social
    Logto-->>experience: Retourner l'URL d'authentification du fournisseur social
    experience->>social: Rediriger vers la page d'authentification du fournisseur social
    user->>social: Entrer les identifiants sociaux
    social-->>experience: Authentifier l'utilisateur et rediriger vers la page de connexion Logto
    experience->>Logto: Valider les données d'authentification sociale et l'identité de l'utilisateur
    Logto->>app: Authentifier l'utilisateur et rediriger vers l'application
```
...
3. **Authentification sociale** : L'utilisateur entre ses identifiants sociaux et s'authentifie auprès du fournisseur social. Si l'utilisateur est déjà connecté au fournisseur social, il peut être automatiquement authentifié. Si plusieurs sessions de connexion sont détectées, l'utilisateur peut être invité à choisir le bon compte (par exemple, plusieurs comptes Google).
   :::note
   Les paramètres de “prompt” de Google peuvent être définis dans le connecteur Google, vous permettant de personnaliser l' expérience utilisateur de sélection de compte et l' écran de consentement pour la connexion Google.
   :::
...
actor user as Utilisateur
    ...
    user->>app: Visiter l'application
    app->>experience: Rediriger vers la page de connexion
    user->>experience: Cliquer sur le bouton "Se connecter avec Google"
...
- **Création de nouveau compte** :
  Lorsqu'un utilisateur se connecte avec une identité sociale qui n'existe pas dans le système Logto, et que l'e-mail ou le numéro de téléphone fourni ne correspond à aucun compte utilisateur existant, un nouveau compte sera créé directement dans Logto.
- **Liaison de compte existant** :
  ...
  - **Liaison de compte automatique :** Si l'option “Liaison de compte automatique” est activée dans vos paramètres d' expérience de connexion, Logto liera automatiquement le compte social au compte existant en fonction d'un e-mail ou d'un numéro de téléphone correspondant. Les utilisateurs ne seront pas invités à lier les comptes et seront instantanément connectés à leur compte existant. Le compte social sera lié, permettant à l'utilisateur de se connecter en utilisant l'une ou l'autre méthode à l'avenir.
  - **Liaison de compte manuelle** : Si l'option “Liaison de compte automatique“ est désactivée, les utilisateurs seront invités à lier leur compte social au compte existant lors du processus de connexion. Ils peuvent choisir de lier les comptes ou d'en créer un nouveau.

    ![liaison de compte social](./assets/sign-in-social-linking.png)
...
## Collecter des informations de profil utilisateur supplémentaires
...
Par exemple, **Adresse e-mail** a été définie comme identifiant d'inscription :
...
2. Inscription avec une identité sociale qui ne fournit pas d'adresse e-mail vérifiée.
   ...
   flowchart TD
      ...
      B -- Oui --> C[Entrer l'adresse e-mail]
      C --> D[Entrer le code de vérification]
...
3. Inscription avec une identité sociale qui fournit une adresse e-mail enregistrée
   ...
   ```mermaid
      flowchart TD
       ...
       C -- Non --> F[Entrer une nouvelle adresse e-mail]
       F --> G[Entrer le code de vérification]
       G --> E
       B -- Non --> E
   ```
...
<details>
  <summary> Comment ajouter des boutons de connexion sociale et se connecter directement avec le social sur mon site web ?</summary>
...
<details>
  <summary> Pourquoi mes e-mails ou numéros de téléphone ne sont-ils pas renseignés après l'inscription avec le social ?</summary>
