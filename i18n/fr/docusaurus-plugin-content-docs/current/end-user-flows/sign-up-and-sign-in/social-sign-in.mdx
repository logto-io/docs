...
2. **Inscription avec une identité sociale ne fournissant pas d'adresse e-mail vérifiée**
...
3. **Inscription avec une identité sociale fournissant une adresse e-mail déjà enregistrée**
   ...
   ```mermaid
      flowchart TD
       A[Authentification avec une identité sociale] --> B{{Adresse e-mail correspond à un compte existant ?}}
...
Après une première connexion sociale, les utilisateurs peuvent gérer leurs comptes sociaux connectés via les paramètres de leur compte. Cela leur permet de lier d'autres comptes sociaux à leur profil existant ou de supprimer des connexions sociales qu'ils ne souhaitent plus utiliser.

Les utilisateurs peuvent accéder à ces fonctionnalités via un centre de compte en libre-service, offrant la flexibilité de :
...
- **Voir les comptes connectés** : Visualiser toutes les identités sociales actuellement liées
...
Vous pouvez activer en option le stockage de jetons tiers pour les connecteurs sociaux pris en charge afin qu'après l'authentification de l'utilisateur et l'octroi des permissions, les jetons d’accès (Access token) et de rafraîchissement (Refresh token) émis par le fournisseur soient stockés en toute sécurité dans le coffre-fort secret de Logto. Votre application (via Account API) pourra ensuite les récupérer et les utiliser pour appeler les API du fournisseur (par exemple, permettre à votre application d'ajouter des événements à Google Agenda) au nom de l'utilisateur sans imposer une nouvelle authentification sociale. Voir [stockage de jetons tiers](/secret-vault/federated-token-set) pour les détails de configuration.
...
### Comment ajouter des boutons de connexion sociale et permettre la connexion directe avec un compte social sur mon site web ? \{#how-to-add-social-login-buttons-and-directly-sign-in-with-social-on-my-website}
