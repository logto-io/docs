...
Par défaut, Logto affichera une page d’erreur générique (par exemple, une page 404 “session inconnue”) dans ces scénarios. Pour améliorer l’Expérience (Experience) utilisateur lors de la rencontre d’erreurs, vous devez configurer les paramètres suivants :
...
Logto utilise le flux [OpenID Connect (OIDC)](https://auth.wiki/openid-connect) pour l’Authentification (Authentication). Un flux d’autorisation OIDC valide doit être initié par votre application, par exemple, cliquer sur le bouton “Se connecter” de votre application envoie une requête d’autorisation OIDC à Logto, qui met alors en place une session sécurisée (cookie).
