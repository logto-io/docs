...
Une **session d'interaction** est créée pour chaque interaction utilisateur lorsqu'une application cliente initie une requête d'autorisation. Cette session centralise le statut d'interaction utilisateur à travers plusieurs applications clientes, permettant à Logto de fournir une expérience de connexion cohérente. Lorsque les utilisateurs passent d'une application cliente à une autre, la session d'interaction reste cohérente, maintenant le statut d'authentification de l'utilisateur et réduisant le besoin de connexions répétées sur les plateformes. Une fois la **session d'interaction** établie, l'utilisateur est invité à se connecter à Logto.
...
<details>
  <summary>Méthode d'expérience de connexion ou de marque par application</summary>
...
<details>
  <summary>Limiter le domaine Email / l'adresse IP / la région</summary>
