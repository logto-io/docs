...
flowchart TD
  ...
  subgraph Layer2 [Couche de gestion d’interaction]
      B[Fournisseur OIDC]
...
## Méthodes de connexion courantes \{#common-sign-in-methods}
...
- [Connexion par e-mail / téléphone / nom d’utilisateur](/end-user-flows/sign-up-and-sign-in/sign-in) : Connexion classique "identifiant d’abord" avec mot de passe ou code de vérification.
- [Connexion sociale](/end-user-flows/sign-up-and-sign-in/social-sign-in) : Connexion avec Google, GitHub, Facebook et d’autres fournisseurs sociaux.
- [Connexion par passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) : Connexion sans mot de passe avec passkeys WebAuthn, incluant bouton direct, vérification passkey "identifiant d’abord" et connexion par saisie automatique.
...
### Méthode d’expérience de connexion ou personnalisation par application \{#per-app-sign-in-experience-method-or-branding}
...
Pour le contrôle d’accès basé sur les attributs, par exemple limiter la connexion selon le domaine e-mail, l’adresse IP ou la région, vous pouvez utiliser la fonctionnalité [Revendications de jeton personnalisées (Custom token claims)](/developers/custom-token-claims/) dans Logto pour refuser ou autoriser les requêtes d’autorisation selon les attributs de l’utilisateur.
