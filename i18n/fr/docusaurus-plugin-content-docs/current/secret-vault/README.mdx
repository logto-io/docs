...
Pour protéger les données sensibles, le coffre-fort de secrets utilise un schéma de chiffrement robuste basé sur les **clés de chiffrement des données (DEK)** et les **clés de chiffrement des clés (KEK)**.
...
<DocCardList
  items={[
    {
      type: 'link',
      label: 'Stockage de jetons tiers & accès API',
      href: 'secret-vault/federated-token-set',
      description:
        'Stockez et gérez en toute sécurité les jetons d’accès (Access tokens) et de rafraîchissement (Refresh tokens) émis par des fournisseurs d’identité (IdPs) fédérés tiers pour une utilisation dans les appels API en aval.',
      customProps: {
        icon: <Connectors />,
      },
    },
...
:::info
Actuellement, l’ensemble de jetons fédérés (federated token set) est le seul type de secret pris en charge nativement. Cependant, le coffre-fort de secrets est conçu pour accueillir tout type d’information sensible. Si vous avez besoin de la prise en charge d’autres types de secrets, veuillez [nous contacter](https://logto.io/contact) — nous serons ravis de vous aider.
:::
