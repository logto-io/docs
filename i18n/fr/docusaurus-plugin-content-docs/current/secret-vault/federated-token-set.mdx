...
### Métadonnées des jetons \{#token-metadata}
...
Pour les connecteurs SSO d’entreprise, la réauthentification et le renouvellement des jetons nécessitent que l'utilisateur initie à nouveau un flux d’authentification complet Logto, car la réautorisation directe auprès du fournisseur SSO d’entreprise n’est pas prise en charge après la connexion.
:::
