...
Mais maintenant que vous développez votre propre interface utilisateur de connexion personnalisée, vous avez besoin d'un moyen de naviguer vers les pages de votre interface utilisateur personnalisée exécutées sur votre machine locale à la place. Cela nécessite un service de tunnel local pour intercepter les requêtes sortantes de votre application et les rediriger vers vos pages d'interface utilisateur de connexion personnalisée.

De plus, vous devez interagir avec [l'Experience API de Logto](https://openapi.logto.io/group/endpoint-experience) pour authentifier les utilisateurs et gérer les sessions. Ce service aidera également à transférer ces requêtes Experience API vers Logto Cloud afin d'éviter les problèmes de CORS.
