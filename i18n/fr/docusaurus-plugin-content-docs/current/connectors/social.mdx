...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description: 'Le connecteur Discord offre un moyen pour votre application d\'utiliser Discord comme système d\'autorisation.',
      customProps: {
        icon: <Discord />,
      }
    },
...
- **Nom du fournisseur d'identité** : Chaque connecteur social a un nom de fournisseur d'identité (IdP) unique pour différencier les identités des utilisateurs. Alors que les connecteurs communs utilisent un nom IdP fixe, les connecteurs personnalisés nécessitent une valeur unique. Apprenez-en plus sur les [noms IdP](/connectors/connector-data-structure#target-identity-provider-name) pour plus de détails.
