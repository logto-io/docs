...
- **Activer le stockage des jetons** : Pour les connecteurs sociaux pris en charge, vous pouvez activer le stockage des jetons pour enregistrer en toute sécurité les jetons d’accès et de rafraîchissement dans le [Secret Vault](/secret-vault) de Logto lorsque les utilisateurs se connectent avec un fournisseur social. Cela permet à votre application d’accéder aux API tierces au nom de l’utilisateur sans qu’il ait à se ré-authentifier. En savoir plus sur le [stockage fédéré des jetons](/secret-vault/federated-token-set).

## Activer la connexion sociale \{#enable-social-sign-in}

Une fois que vous avez créé un connecteur social avec succès, vous pouvez l’activer comme bouton de connexion sociale (par exemple, Continuer avec Google) lors de l’inscription et de la connexion.
