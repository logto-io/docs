...
Ces connecteurs facilitent le processus d’authentification entre votre service et les fournisseurs d’identité (IdP) d’entreprise. Logto prend en charge à la fois le [SSO initié par le fournisseur de services (SP)](/end-user-flows/enterprise-sso/sp-initiated-sso) et le [SSO initié par le fournisseur d’identité (IdP)](/end-user-flows/enterprise-sso/idp-initiated-sso), ce qui permet aux membres d’une organisation d’accéder à vos services en utilisant leurs identifiants d’entreprise existants, renforçant ainsi la sécurité et la productivité.
