  ...
  items={[
    {
      ...
      description: 'Connecteur officiel pour le Service Email Logto.',
      customProps: {
...
<details>
<summary>Comment puis-je restreindre les adresses IP pour les demandes d'email dans les connecteurs email Logto ?</summary>

Une solution de contournement consiste à utiliser le connecteur email HTTP de Logto.

Implémentez un endpoint API sur votre serveur qui appelle le service email pertinent, et déclenchez cet endpoint API via le [connecteur email HTTP](/integrations/http-email) de Logto. De cette manière, vous aurez un contrôle total sur l'adresse IP de votre endpoint API et pourrez ajouter les adresses IP correspondantes à la liste blanche dans la configuration du fournisseur de service email.
