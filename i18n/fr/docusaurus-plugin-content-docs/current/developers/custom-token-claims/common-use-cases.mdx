...
S’il n’existe pas de fonctionnalité de revendications personnalisées dans les jetons, lors de la vérification des permissions pour l’[autorisation](/authorization), il est nécessaire d’appeler le Management API de Logto pour vérifier si l’utilisateur possédant le jeton d’accès actuel dispose des permissions correspondant au rôle requis par une certaine ressource API.

Dans un scénario similaire, supposons que votre application affiche des vœux d’anniversaire sur la page de connexion si l’anniversaire de l’utilisateur approche. Vous pouvez utiliser les revendications personnalisées dans les jetons pour ajouter un champ anniversaire à la [charge utile du jeton](/user-management/personal-access-token#example-token-exchange), qui pourra servir à déterminer s’il faut afficher un message spécifique.
