...
:::warning
Note de sécurité : Dans les déploiements auto-hébergés, les scripts JWT personnalisés sont exécutés avec les mêmes privilèges que le processus serveur Logto, donc accorder l’accès à l’édition ou au test de script équivaut à accorder l’exécution de code sur l’hôte Logto. Cette fonctionnalité est destinée uniquement aux administrateurs de confiance : n’autorisez pas les utilisateurs non fiables ou à privilèges réduits à créer, modifier ou tester ces scripts, et ne partagez pas la Console d’administration avec eux.
:::

## Ressources associées \{#related-resources}

<Url href="https://blog.logto.io/glance-on-custom-jwt-access-token-claims">
  Ajoutez des revendications personnalisées pour les jetons d’accès JWT avec Logto afin de renforcer
