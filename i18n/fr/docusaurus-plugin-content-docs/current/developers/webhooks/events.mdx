...
| Message.RateLimited | Un utilisateur final dépasse la [limite d'envoi](/security/send-rate-limit) par destinataire pour les codes de vérification. Déclenché uniquement sur les chemins d'envoi utilisateur final (Experience API et Account API), avec un payload `{ action, recipient }`.                                                        |
