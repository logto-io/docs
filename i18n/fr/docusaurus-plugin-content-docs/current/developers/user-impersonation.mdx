...
Dans cette API, le backend doit effectuer des vérifications d’autorisation appropriées pour s’assurer que Sarah dispose des permissions nécessaires pour usurper l’identité d’Alex.

### Étape 2 : Obtention d’un jeton de sujet \{#step-2-obtaining-a-subject-token}

Après avoir validé la demande de Sarah, le serveur de TechCorp appellera ensuite le [Management API](/integrate-logto/interact-with-management-api) de Logto pour obtenir un jeton de sujet.
...
async function impersonateUser(
  ...
  resource: string,
  // highlight-next-line
  clientSecret?: string // Requis pour les applications web traditionnelles ou M2M
): Promise<string> {
  try {
    ...
    if (!impersonationResponse.ok) {
      throw new Error(`Une erreur HTTP est survenue. Statut : ${impersonationResponse.status}`);
    }
...
async function performImpersonation(): Promise<void> {
  try {
    ...
    const accessToken = await impersonateUser(
      ...
      'your-client-secret' // Omettre ceci pour les SPA ou applications natives
    );
    // highlight-end
...
Pour les SPA ou applications natives, incluez `client_id` dans le corps de la requête à la place :
