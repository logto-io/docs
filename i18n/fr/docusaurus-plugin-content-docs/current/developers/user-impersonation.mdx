...
2. Le serveur de TechCorp obtient un jeton de sujet de Logto Management API
...
### Étape 1 : Demande d'usurpation \{#step-1-requesting-impersonation}
...
Dans cette API, le backend doit effectuer des vérifications d'autorisation appropriées pour s'assurer que Sarah a les permissions nécessaires pour usurper Alex.

### Étape 2 : Obtention d'un jeton de sujet \{#step-2-obtaining-a-subject-token}

Le serveur de TechCorp, après avoir validé la demande de Sarah, appellera ensuite le [Management API](/integrate-logto/interact-with-management-api) de Logto pour obtenir un jeton de sujet.
...
### Étape 3 : Échange du jeton de sujet contre un jeton d’accès \{#step-3-exchanging-the-subject-token-for-an-access-token}
...
**Demande (application de Sarah au point de terminaison de jeton Logto)**
...
**Note** : Pour les applications web traditionnelles, incluez `client_id` et `client_secret` dans l'en-tête de la demande de jeton pour éviter une erreur 401 invalid_client.
...
Logto vous permet de [personnaliser les revendications de jeton](/developers/custom-token-claims) pour les jetons d'usurpation. Cela peut être utile pour ajouter un contexte ou des métadonnées supplémentaires au processus d'usurpation, comme la raison de l'usurpation ou le ticket de support associé.

Lorsque le serveur de TechCorp demande un jeton de sujet à Logto Management API, il peut inclure un objet `context` :
...
En personnalisant les revendications de jeton d’accès de cette manière, TechCorp peut inclure des informations précieuses sur le contexte d'usurpation, ce qui facilite l'audit et la compréhension des activités d'usurpation dans leur système.
