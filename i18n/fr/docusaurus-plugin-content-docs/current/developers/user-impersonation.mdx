...
async function impersonateUser(
  ...
  try {
    ...
    if (clientSecret) {
      // Client confidentiel : utilisez l’authentification Basic
      headers['Authorization'] =
        `Basic ${Buffer.from(`${clientId}:${clientSecret}`).toString('base64')}`;
    } else {
      // Client public : incluez client_id dans le corps
...
2. Le jeton d’accès d’usurpation ne comporte pas de [jeton de rafraîchissement (refresh token)](https://auth.wiki/refresh-token). Sarah devra répéter ce processus si le jeton expire avant qu’elle ne résolve le problème d’Alex.
