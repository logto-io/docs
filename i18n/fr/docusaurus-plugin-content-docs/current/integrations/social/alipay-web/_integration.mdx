...
7. Cliquez sur le bouton "Sign" de "App Alipay login", et terminez le processus de signature en suivant le guide. Après avoir terminé cette étape, vous devriez constater que les capacités que vous avez ajoutées à l'étape 5 prennent effet.
...
## Configurer les paramètres du connecteur Web Alipay de Logto \{#set-up-the-logto-alipay-web-connector-settings}
...
3. Remplissez les paramètres du connecteur Logto :
   ...
   - Remplissez le champ `charset` avec 'gbk' ou 'utf8'. Vous pouvez laisser ce champ vide car il est OPTIONNEL. La valeur par défaut est définie sur 'utf8'.
   - Remplissez le champ `scope` avec 'auth_base' ou 'auth_user'. Vous pouvez laisser ce champ vide car il est OPTIONNEL. La valeur par défaut est définie sur 'auth_user'. Vous pouvez consulter la [différence](https://opendocs.alipay.com/fw/api/105942) entre les différentes valeurs.
