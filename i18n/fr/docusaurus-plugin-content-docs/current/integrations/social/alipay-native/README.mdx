...
Nous supposons que vous avez intégré le [Logto iOS SDK](/quick-starts/swift) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK Alipay :
...
Où `callbackScheme` est l'un des [custom URL Schemes](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app) qui peut naviguer vers votre application.

### Android \{#android}

Nous supposons que vous avez intégré le [Logto Android SDK](/quick-starts/android) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK Alipay :
