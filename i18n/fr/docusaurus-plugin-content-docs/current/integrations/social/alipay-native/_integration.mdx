...
Lorsque vous remplissez la clé publique sur le site de configuration de l'application Alipay, vous devez supprimer l'en-tête et le pied de page de `public.pem`, supprimer tous les caractères de nouvelle ligne et coller le reste du contenu dans la zone de saisie de texte pour la "clé publique".
:::

## Configurer les paramètres du connecteur Alipay Native de Logto \{#set-up-the-logto-alipay-native-connector-settings}
...
Nous supposons que vous avez intégré le [SDK iOS Logto](/quick-starts/swift) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK Alipay :
...
Nous supposons que vous avez intégré le [SDK Android Logto](/quick-starts/android) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK Alipay :
