...
3. Cliquez sur **Nouvelle application OAuth** pour enregistrer une nouvelle application :
   ...
   - **URL de rappel d'autorisation** : Copiez l'**URI de rappel** depuis votre connecteur GitHub Logto et collez-la ici. Après la connexion des utilisateurs avec GitHub, ils seront redirigés ici avec un code d'autorisation que Logto utilise pour finaliser l'authentification.
...
:::note
Nous vous recommandons de ne pas cocher la case **Activer le Device Flow**, car les utilisateurs qui se connectent avec GitHub sur des appareils mobiles devront confirmer l'action de connexion initiale dans l'application mobile GitHub. Beaucoup d'utilisateurs GitHub n'installent pas l'application mobile GitHub sur leur téléphone, ce qui pourrait bloquer le flux de connexion. Activez cette option uniquement si vous attendez que les utilisateurs finaux confirment leur connexion via l'application mobile GitHub. Voir les détails sur le [Device Flow](https://docs.github.com/en/developers/apps/building-oauth-apps/authorizing-oauth-apps#device-flow).
...
:::warning
Gardez votre secret client en sécurité et ne l'exposez jamais dans du code côté client. Les secrets clients GitHub ne peuvent pas être récupérés s'ils sont perdus — vous devrez en générer un nouveau.
:::
...
- Exemples de portées supplémentaires courantes :
...
2. Activez **Stocker les jetons pour un accès API persistant** dans le connecteur GitHub Logto. Logto stockera alors en toute sécurité les jetons d’accès GitHub dans le Secret Vault.

:::note
Lorsque vous utilisez une **application OAuth GitHub** comme décrit dans ce tutoriel, vous ne pouvez pas obtenir de jeton de rafraîchissement de GitHub car son jeton d’accès n’expire pas, sauf si l’utilisateur le révoque manuellement. Par conséquent, il n’est pas nécessaire d’ajouter `offline_access` dans le champ `Scopes` — cela pourrait même provoquer une erreur.
...
4. Testez les appels API si vous stockez les jetons.
