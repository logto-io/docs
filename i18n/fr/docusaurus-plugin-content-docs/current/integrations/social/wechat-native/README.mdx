...
Connectez-vous avec le compte que vous venez de créer. Dans l'onglet "Mobile Application" (移动应用), cliquez sur le grand bouton vert "Create a mobile app" (创建移动应用).

![Onglets d'application](./assets/app-tabs.png)

Remplissons les informations requises dans le formulaire de demande.

![Créer une application mobile](./assets/create-mobile-app.png)
...
Après avoir complété les informations de la plateforme, cliquez sur "Submit Review" pour continuer. En général, la révision est rapide, elle se termine en 1 à 2 jours.
...
![Schéma d'URL personnalisé](./assets/custom-url-scheme.png)
...
![Indicateurs de l'éditeur de liens](./assets/linker-flags.png)
...
Créez un package `wxapi` sous votre racine de package et ajoutez le `WXEntryActivity` dans le package `wxapi` (Prenons `com.sample.app` comme exemple) :
...
> ⚠️ **Attention**
>
> WeChat n'a pas de plan pour les appareils sans l'application WeChat installée. Logto masquera ce connecteur lors de la connexion si c'est le cas (ce qui est la méthode recommandée par le [guide de développement officiel](https://developers.weixin.qq.com/doc/oplatform/en/Mobile_App/WeChat_Login/Development_Guide.html)).
