## Créer une application mobile dans la plateforme ouverte WeChat \{#create-a-mobile-app-in-the-wechat-open-platform}
...
Connectez-vous avec le compte que vous venez de créer. Dans l'onglet "Mobile Application" (移动应用), cliquez sur le grand bouton vert "Créer une application mobile" (创建移动应用).
...
- Si vous souhaitez simplement tester la connexion WeChat et que l'application n'est pas sur l'App Store, dans la section "App is available", choisissez "No" pour ignorer le "Lien de téléchargement de l'application".
- Le "Diagramme de flux de fonctionnement de l'application" semble compliqué. D'après notre expérience, vous devez préparer un simple organigramme et plusieurs captures d'écran de l'application pour améliorer la possibilité de passer la révision.
...
Si vous avez choisi "No" pour la disponibilité sur l'App Store, vous pouvez ignorer le remplissage de l'"Adresse de téléchargement AppStore" ici.
...
Après avoir complété les informations de la plateforme, cliquez sur "Submit Review" pour continuer. En général, la révision est rapide et se termine en 1-2 jours.
...
Nous supposons que vous avez intégré [Logto iOS SDK](/quick-starts/swift) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK WeChat :

**1. Configurez le lien universel et le schéma URL dans votre projet Xcode**
...
Ensuite, allez dans l'onglet "Info", ajoutez un [schéma URL personnalisé](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app) avec l'ID d'application WeChat.

<img src="/img/assets/custom-url-scheme.png" alt="Schéma URL personnalisé" />
...
Nous supposons que vous avez intégré [Logto Android SDK](/quick-starts/android) dans votre application. Dans ce cas, les choses sont assez simples, et vous n'avez même pas besoin de lire la documentation du SDK WeChat :
