...
1. Accédez à la page de l'[écran de consentement OAuth](https://console.cloud.google.com/apis/credentials/consent) et sélectionnez le type d'utilisateur `Interne`. Cela rendra l'application OAuth disponible uniquement pour les utilisateurs au sein de votre organisation.

<img
  src={googleWorkspaceConsentScreenUserTypeImage}
  alt="Type d'utilisateur de l'écran de consentement Google Workspace"
/>
...
<img
  ...
  alt="Paramètres de l'écran de consentement Google Workspace"
/>

3. Définissez les `Portées` pour votre application. Afin de récupérer correctement les informations d'identité et l'adresse e-mail de l'utilisateur depuis le fournisseur d’identité (IdP), les connecteurs SSO de Logto doivent accorder les portées suivantes depuis le IdP :

<img
  src={googleWorkspaceConsentScreenScopesImage}
  alt="Portées de l'écran de consentement Google Workspace"
/>
