...
- Remplissez le champ `from` avec l'adresse e-mail à partir de laquelle vous souhaitez envoyer les e-mails. Cette adresse e-mail doit être enregistrée dans votre compte Mailgun. L'adresse e-mail doit être au format `Nom de l'expéditeur <sender@example.com>`.
...
- `subject` : L'objet de l'e-mail à envoyer.
...
- `variables` : Les variables à transmettre au modèle Mailgun. Doit être une carte clé-valeur JSON car elle sera convertie en chaîne avant l'envoi à Mailgun. Notez qu'il n'est pas nécessaire d'inclure la variable `code` car elle sera automatiquement ajoutée par le connecteur.
