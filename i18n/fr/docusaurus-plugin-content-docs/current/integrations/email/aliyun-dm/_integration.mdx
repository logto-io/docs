...
3. Remplissez les paramètres du connecteur Aliyun DM :
   ...
   - Vous pouvez ajouter plusieurs modèles de connecteurs DM pour différents cas. Voici un exemple d'ajout d'un modèle unique :
     ...
     - Remplissez le champ `usageType` avec `Register`, `SignIn`, `ForgotPassword` ou `Generic` pour différents cas d'utilisation. (`usageType` est une propriété Logto pour identifier le cas d'utilisation approprié.) Pour activer les flux utilisateur complets, les modèles avec usageType `Register`, `SignIn`, `ForgotPassword` et `Generic` sont requis.

### Tester le connecteur Aliyun DM \{#test-aliyun-dm-connector}

Vous pouvez saisir une adresse e-mail et cliquer sur "Send" pour voir si les paramètres fonctionnent avant de "Save and Done".
