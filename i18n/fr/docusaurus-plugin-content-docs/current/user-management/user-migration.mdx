...
2. **Données personnalisées** : stocke des informations utilisateur supplémentaires, vous pouvez utiliser cela pour stocker des fichiers qui ne correspondent pas aux données de base.
...
- **`salt`** : La valeur de sel utilisée dans le hachage d'origine
...
- **`expected_hashed_value`** : Le résultat de hachage attendu sous forme de chaîne hexadécimale
...
Veuillez noter que le point d’API est soumis à une limitation de débit, vous devez donc ajouter une pause entre chaque requête pour éviter d’atteindre la limite. Veuillez consulter notre page sur les [limitations de débit](/integrate-logto/interact-with-management-api/#rate-limit) pour plus de détails.

Si vous avez une grande quantité de données utilisateur (plus de 100 000 utilisateurs), vous pouvez [nous contacter](https://logto.io/contact) pour augmenter la limite de débit.
