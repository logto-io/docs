...
Sa valeur provient du nom d'utilisateur avec lequel l'utilisateur s'est inscrit pour la première fois. Elle peut être `null`. Sa valeur non nulle ne doit pas dépasser 128 caractères, ne contenir que des lettres, des chiffres et des underscores (`_`), et NE PAS commencer par un chiffre. Elle est sensible à la casse par défaut. Ces règles peuvent être restreintes par locataire avec la [politique de nom d'utilisateur](/end-user-flows/sign-up-and-sign-in/username-policy).
...
Une exception concerne la revendication `preferred_username` : lorsque `preferredUsername` dans `profile` est vide, la revendication revient au [username](/user-management/user-data#username) de l'utilisateur, afin que les clients conformes aux standards reçoivent une valeur utilisable par défaut. La valeur explicite de `preferredUsername` a toujours la priorité lorsqu'elle est définie.
...
_has_password_ est une valeur booléenne indiquant si l'utilisateur possède un mot de passe. Vous pouvez consulter et gérer ce statut, y compris définir ou réinitialiser le mot de passe sur la page de détail de <CloudLink to="/users">Console > Gestion des utilisateurs</CloudLink>.
...
Logto utilise par défaut l’implémentation [node-argon2](https://github.com/ranisalt/node-argon2) de [Argon2](https://en.wikipedia.org/wiki/Argon2) comme méthode de chiffrement ; voir la référence pour plus de détails si cela vous intéresse.
...
_is_suspended_ est une valeur booléenne indiquant si un utilisateur est suspendu ou non. La valeur peut être gérée en appelant le [Logto Management API](https://openapi.logto.io/operation/operation-updateuserissuspended) ou via Logto Console.

Une fois un utilisateur suspendu, les jetons de rafraîchissement préalablement accordés seront immédiatement révoqués et l'utilisateur ne pourra plus être authentifié par Logto.
...
Les informations utilisateur varient selon le fournisseur d'identité sociale (plateforme de réseau social), et incluent généralement :
...
_sso_identities_ contient les informations utilisateur récupérées lors du [SSO d’entreprise](/end-user-flows/enterprise-sso) (c’est-à-dire la connexion Single Sign-On avec un [connecteur d’entreprise](/connectors/enterprise-connectors)). Les _ssoIdentities_ de chaque utilisateur sont stockées dans un objet JSON individuel.
...
NE PAS mettre de données sensibles dans _custom_data_.
...
- Utilisez le [Management API](/user-management/manage-users/#manage-via-logto-management-api) pour la gestion des utilisateurs ou des flux personnalisés avancés :
...
Mettez à jour avec précaution. La mise à jour des _custom_data_ d’un utilisateur écrasera complètement son contenu d’origine dans le stockage.

Par exemple, si votre entrée lors de l’appel à l’API de mise à jour _custom_data_ ressemble à ceci (en supposant que les _custom_data_ d’origine sont les données d’exemple précédentes) :
...
C’est-à-dire que la valeur du champ mis à jour n’a aucun lien avec la valeur précédente.

## Référence des propriétés \{#property-reference}

Les colonnes suivantes de la table utilisateur en base de données (sauf _password_encrypted_ et _password_encryption_method_) sont visibles sur le profil utilisateur, ce qui signifie que vous pouvez les interroger via le [Management API](https://openapi.logto.io/operation/operation-getuser).
...
| [application_id](/user-management/user-data#application_id)                         | string    | ID de l’application de première inscription                | ❌     | ✅     |
