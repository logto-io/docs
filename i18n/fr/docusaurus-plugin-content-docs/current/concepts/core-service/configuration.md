...
| ADMIN_DISABLE_LOCALHOST           | N/A                                  | <code>string &#124; boolean &#124; number</code>         | Définissez à `1` ou `true` pour désactiver le port de la Console d'administration. Si `ADMIN_ENDPOINT` n'est pas défini, cela désactivera complètement la Console d'administration.                                                                                                                                                  |
...
| DATABASE_STATEMENT_TIMEOUT        | N/A                                  | `string`                                                 | (v1.36.0+) `statement_timeout` PostgreSQL en millisecondes. Utilisez une chaîne numérique (ex : `5000`) pour le définir, ou `DISABLE_TIMEOUT` pour omettre le paramètre de démarrage (recommandé pour PgBouncer/RDS Proxy). Si non défini ou invalide, la valeur par défaut est 60000 ms.                                            |
...
| PRIVATE_KEY_ROTATION_GRACE_PERIOD | `0`                                  | `number`                                                 | Délai de grâce en secondes pour la rotation progressive de la clé privée OIDC. Lorsqu'une valeur positive est définie, la nouvelle clé privée est d'abord créée comme `Next` et ne devient effective qu'après le délai de grâce.                                                                                                     |
...
| ---------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| oidc.cookieKeys  | <code>string[]</code> | Le tableau de chaînes des [clés de signature des cookies](https://github.com/panva/node-oidc-provider/blob/main/docs/README.md#cookieskeys).   |
| oidc.privateKeys | <code>string[]</code> | Le tableau de chaînes du contenu des clés privées pour la [signature JWT OIDC](https://openid.net/specs/openid-connect-core-1_0.html#Signing). |
