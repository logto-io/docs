...
- `DATABASE_STATEMENT_TIMEOUT` (v1.36.0+) `statement_timeout` PostgreSQL en millisecondes. Utilisez une chaîne numérique (par exemple, `5000`) ou `DISABLE_TIMEOUT` pour PgBouncer / RDS Proxy. Voir [Configuration](/concepts/core-service/configuration/) pour plus de détails.
...
- Pour utiliser le [Secret Vault](/secret-vault), vous devez définir `SECRET_VAULT_KEK` sur une chaîne encodée en base64 de votre Key Encryption Key (KEK). Ceci est utilisé pour chiffrer les Data Encryption Keys (DEK) dans le Secret Vault. AES-256 (32 octets) est recommandé. Exemple : `crypto.randomBytes(32).toString('base64')`.
...
Définissez `TRUST_PROXY_HEADER=1` pour que Logto fasse confiance aux en-têtes transférés. Utilisez `$http_host` pour `Host` et `X-Forwarded-Host` afin de préserver les ports non standards dans l'URL publique.

```
server {
  listen 443 ssl;
  server_name <your_endpoint_url>; // par exemple auth.votre-domaine.com
  ...
...
server {
  listen 443 ssl;
  server_name <your_admin_endpoint_url>; // par exemple admin.votre-domaine.com
  ...
...
## Conteneurisation \{#reverse-proxy}
...
### Configuration de la base de données \{#containerization}
...
:::tip Installations isolées ou hors ligne

Si votre environnement de déploiement ne peut pas atteindre `api.pwnedpasswords.com`, ajoutez `--disable-admin-pwned-password-check` à `logto init` ou `npm run cli db seed` pour que la première inscription admin ne soit pas bloquée sur la vérification Have I Been Pwned. Voir [Seed pour les déploiements isolés ou hors ligne](/logto-oss/using-cli#seed-for-air-gapped-or-offline-deployments) pour plus de détails.

:::

### Dossier partagé des connecteurs \{#database-setup}
...
Pour la production, vous pouvez remplacer le volume "empty dir" par un volume persistant, et effectuer le travail "init" à votre manière, vous savez ce que vous faites !

### Modification de la base de données \{#shared-connectors-folder}
...
La variable d'environnement `CI=true` est nécessaire lorsque la modification est lancée en mode non interactif.
...
Voir [Modification de la base de données](/logto-oss/using-cli/database-alteration/) pour plus de détails sur la commande d'altération.
