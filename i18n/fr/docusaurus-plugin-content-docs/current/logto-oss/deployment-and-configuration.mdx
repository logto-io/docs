...
Dans l'article précédent, nous avons couvert les bases de [démarrer rapidement avec Logto](/logto-oss/get-started-with-oss). Cet article va plus loin, en se concentrant sur les bonnes pratiques et les étapes de configuration détaillées pour déployer Logto dans un environnement de production.
...
- `DATABASE_STATEMENT_TIMEOUT` (v1.36.0+) `statement_timeout` PostgreSQL en millisecondes. Utilisez une chaîne numérique (ex : `5000`) ou `DISABLE_TIMEOUT` pour PgBouncer / RDS Proxy. Voir [Configuration](/concepts/core-service/configuration/) pour plus de détails.
...
Tout est prêt. Ouvrez le navigateur et visitez `https://admin.votre-domaine.com`, vous devriez voir la page de bienvenue Logto.
...
La variable d'environnement `CI=true` est nécessaire lorsque la modification est exécutée en mode non interactif.
