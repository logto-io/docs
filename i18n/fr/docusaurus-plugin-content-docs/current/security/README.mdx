import BlockUserIcon from '@site/src/assets/block-user.svg';
import CheckboxIcon from '@site/src/assets/checkbox.svg';
import HideIcon from '@site/src/assets/hide.svg';
import KeyIcon from '@site/src/assets/key.svg';
import LockIcon from '@site/src/assets/lock.svg';
import LockoutIcon from '@site/src/assets/lockout.svg';
import SecurityIcon from '@site/src/assets/security.svg';
import SendEmailIcon from '@site/src/assets/send-mail.svg';
import SuspendIcon from '@site/src/assets/suspend.svg';

# Sécurité

La sécurité moderne de l'authentification doit faire face à des menaces allant du phishing, du bourrage d'identifiants, des attaques par force brute, des ransomwares, des attaques DDoS, jusqu'aux attaques pilotées par l'IA. Protéger les identités des utilisateurs est essentiel pour préserver la confiance envers la marque et la conformité.

Logto offre une gestion robuste et sécurisée des accès, conçue pour contrer ces risques de front. En donnant la priorité à la prévention proactive des menaces et à la résilience, nous veillons à ce que vos systèmes restent protégés sans compromettre l'expérience utilisateur. Avec Logto, la sécurité n'est pas une réflexion après coup — c'est le socle, permettant aux entreprises de prospérer à une époque où les menaces évoluent, mais où les défenses évoluent encore plus vite.

## Mettre en place une protection de sécurité avancée \{#set-up-advanced-security-protection}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Politique de mot de passe',
      href: '/security/password-policy',
      description:
        'Renforcez les exigences de mot de passe pour vous défendre contre le bourrage d’identifiants et les attaques par mots de passe faibles.',
      customProps: {
        icon: <KeyIcon />,
      },
    },
    {
      type: 'link',
      label: 'CAPTCHA',
      href: '/security/captcha',
      description:
        'Ajoutez un CAPTCHA (par exemple, Google reCAPTCHA, Cloudflare Turnstile) à votre expérience de connexion pour empêcher les attaques automatisées par bots.',
      customProps: {
        icon: <CheckboxIcon />,
      },
    },
    {
      type: 'link',
      label: 'Liste de blocage',
      href: '/security/blocklist',
      description:
        'Prenez le contrôle de votre base d’utilisateurs en bloquant les domaines ou adresses e-mail jetables ou indésirables.',
      customProps: {
        icon: <BlockUserIcon />,
      },
    },
    {
      type: 'link',
      label: 'Verrouillage d’identifiant',
      href: '/security/identifier-lockout',
      description:
        'Verrouillez temporairement un identifiant après plusieurs tentatives d’authentification échouées pour empêcher l’accès par force brute.',
      customProps: {
        icon: <LockoutIcon />,
      },
    },
    {
      type: 'link',
      label: 'Masquer l’existence du compte',
      href: '/end-user-flows/sign-up-and-sign-in',
      description:
        'Masquez le statut du compte pour bloquer les attaques d’énumération de comptes et éviter de divulguer des informations sensibles sur le statut du compte.',
      customProps: {
        icon: <HideIcon />,
      },
    },
  ]}
/>

## Découvrez d’autres moyens de protéger vos applications \{#discover-more-ways-to-protect-your-apps}

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Authentification multi-facteurs (MFA)',
      href: '/end-user-flows/mfa',
      description:
        'Ajoutez une couche de protection supplémentaire au processus de connexion avec la prise en charge des OTP d’application d’authentification, des passkeys (WebAuthn) et des codes de secours.',
      customProps: {
        icon: <LockIcon />,
      },
    },
    {
      type: 'link',
      label: 'Authentification renforcée',
      href: '/end-user-flows/security-verification',
      description:
        'Permettez à l’application de demander une authentification renforcée lorsque les utilisateurs accèdent à des informations sensibles ou effectuent des actions à haut risque.',
      customProps: {
        icon: <SecurityIcon />,
      },
    },
    {
      type: 'link',
      label: 'Suspendre des utilisateurs',
      href: '/user-management/manage-users#suspend-user',
      description:
        'Désactivez temporairement les comptes utilisateurs pour bloquer l’accès sans supprimer les données ou les enregistrements utilisateurs.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Rotation des clés de signature',
      href: '/developers/signing-keys',
      description:
        'Faites tourner périodiquement les clés de signature pour vous protéger contre la fuite de clés et la falsification de jetons.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
    {
      type: 'link',
      label: 'Déconnexion back-channel OIDC',
      href: '/end-user-flows/sign-out#federated-sign-out-back-channel-logout',
      description:
        'Activez la déconnexion centralisée pour réduire le risque de détournement de session et d’accès non autorisé.',
      customProps: {
        icon: <SuspendIcon />,
      },
    },
    {
      type: 'link',
      label: 'Inscription sur invitation uniquement',
      href: '/end-user-flows',
      description:
        'Limitez les inscriptions aux seuls utilisateurs invités, en utilisant des liens magiques par e-mail pour un onboarding sécurisé.',
      customProps: {
        icon: <SendEmailIcon />,
      },
    },
  ]}
/>
