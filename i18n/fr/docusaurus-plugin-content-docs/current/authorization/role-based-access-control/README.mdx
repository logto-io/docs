...
- **Vous avez plusieurs utilisateurs avec des besoins d'accès différents** : Le RBAC est idéal lorsque les utilisateurs ont besoin de permissions variées en fonction des rôles, tels qu'administrateur, éditeur ou spectateur.
- **Vous devez simplifier la gestion des permissions** : C'est efficace pour gérer de grands groupes d'utilisateurs en attribuant des rôles plutôt qu'en définissant des permissions individuellement.
- **Votre application dessert différents départements ou équipes** : C'est utile dans les scénarios où différents groupes nécessitent des niveaux d'accès distincts aux ressources.
...
La permission fait référence à l'autorisation d'accéder à une [ressource API](/authorization/api-resources). Dans le monde réel, des entités telles que les commandes, les produits et les documents peuvent être désignées comme ressources, et diverses actions peuvent être attribuées.
...
Si aucune ressource API n'est fournie, la permission sera considérée comme "pour OIDC". Habituellement, ce n'est pas ce que vous souhaitez dans le RBAC.

Apprenez comment [configurer les permissions API](/authorization/role-based-access-control/configure-permissions) dans Logto.
...
<img src={roleOrderAdminImage} width="60%" alt="Rôle d'administrateur de commande" />

Il est acceptable d'avoir un chevauchement de permissions entre les rôles.

Apprenez comment [configurer les rôles](/authorization/role-based-access-control/configure-roles) dans Logto.
...
<img src={bookstoreSetupImage} width="60%" alt="API et Permissions de la librairie" />
...
<img src={bookstoreRolesImage} width="60%" alt="Rôles de la librairie" />
