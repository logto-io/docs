...
Voici un exemple utilisant Notion, un outil de collaboration populaire, et une application [multi-locataire](https://auth.wiki/multi-tenancy) typique. Il prend en charge diverses fonctionnalités pour l'[authentification](https://auth.wiki/authentication) et l'[autorisation](https://auth.wiki/authorization) :
...
Un rôle d'organisation est un ensemble de [permissions d'organisation](#organization-permission) ou de [permissions API](/authorization/role-based-access-control/configure-permissions) (permissions API définies directement dans les <CloudLink to="/api-resources">ressources API dans Logto Console</CloudLink>) qui peuvent être attribuées aux utilisateurs.
...
**John**, **Sarah** sont dans différentes organisations avec des rôles différents dans le contexte de différentes organisations.
