...
| Lorsqu'une API par défaut est désignée, le paramètre de ressource peut être omis dans la requête d'authentification. Les échanges de jetons ultérieurs utiliseront cette API comme audience par défaut, entraînant l'émission de JWT. | false                                                                                                                                           |

:::note

Les requêtes avec un `jeton d’accès` expiré doivent être restreintes. Cela est mis en œuvre pour protéger votre API d'un jeton abusé. Plus la valeur est grande, plus le `jeton d’accès` émis survivra longtemps, et plus votre API sera vulnérable.
