...
sequenceDiagram
    ...
    Note right of Client: avec resource=https://resource-server.com/api<br/>et scope optionnel=read write
    Logto-->>Client: Retourne authorization_code

    Note over Client,API: 2. Requête de jeton
    Client->>Logto: POST /oidc/token
    Note right of Client: avec authorization_code<br/>resource=https://resource-server.com/api<br/>et scope optionnel=read write
...
Lorsque le paramètre de ressource est présent sur une [Requête de jeton](https://openid.net/specs/openid-connect-core-1_0.html#TokenRequest) avec le `authorization_code` accordé ci-dessus, il spécifiera l'audience de la ressource API cible du jeton d’accès demandé.
...
Un jeton [JWS](https://datatracker.ietf.org/doc/html/rfc7515) encodé est construit en trois parties :
...
Logto utilise le protocole d’autorisation OAuth 2.0 basé sur le code pour sécuriser votre requête API. Si vous êtes intéressé par la stratégie qui le sous-tend, référez-vous à la [spécification](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.1) d'OAuth 2.0 pour plus de détails.
