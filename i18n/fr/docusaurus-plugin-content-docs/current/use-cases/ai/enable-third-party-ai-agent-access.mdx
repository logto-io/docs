...
- Obtenir un jeton d’accès (Access token) pour que l’agent IA accède à votre service.
...
Prenons un exemple. Imaginez que vous êtes un développeur qui exploite un service fournissant des données météorologiques.
...
- Vous développez un écosystème autour de votre service, et un autre développeur crée "WeatherBot" (un assistant IA capable de fournir des mises à jour et des prévisions météo) en l’intégrant comme agent IA tiers.
- Fonctionnement : WeatherBot (agent IA tiers) souhaite accéder aux données météo utilisateur via votre service. Lorsque Alice se connecte à WeatherBot avec son compte :
...
:::note
Si vous souhaitez intégrer votre service avec votre propre agent IA ou application, veuillez consulter nos [guides de démarrage rapide](/quick-starts) pour plus d’informations.
:::
...
- **Logto** : Sert de fournisseur OpenID Connect (serveur d’autorisation) et gère les identités utilisateur.
...
sequenceDiagram
    ...
    Service->>Logto: Demande de configuration OpenID Connect
    ...
    Agent->>Logto: Redirection vers Logto pour authentification
    Logto->>Logto: Authentification de l’utilisateur
    Logto->>Logto: L’utilisateur autorise (consentement) les portées à l’agent IA
    Logto->>Agent: Redirection vers le service avec le code d’autorisation
    Agent->>Logto: Demande de jeton d’accès avec le code d’autorisation
    Logto->>Agent: Retourne le jeton d’accès
    Agent->>Service: Demande de ressource avec le jeton d’accès
    Service->>Service: Validation du jeton d’accès
...
1. **Enregistrer l’agent IA dans Logto** : Créez une application dans Logto pour représenter l’agent IA.
2. **Configurer l’agent IA** : Assurez-vous que l’agent IA peut effectuer des requêtes vers votre service et gérer la réponse 401 Non autorisé.
3. **Implémenter le flux d’autorisation** : L’agent IA doit pouvoir gérer le flux d’autorisation OAuth 2.0 pour obtenir un jeton d’accès (Access token) depuis Logto.
4. **Tester l’intégration** : Utilisez l’agent IA pour accéder à votre service et vérifiez qu’il peut authentifier et autoriser les requêtes avec succès.

<ConfigureThirdPartyAiAgent />

## Configurer l’autorisation dans votre service \{#set-up-authorization-in-your-service}

Pour activer le contrôle d’accès dans votre service, vous devez mettre en œuvre les éléments suivants :
...
3. Dans l’agent IA, invoquez le point de terminaison API pour accéder à votre service.
4. L’agent IA doit gérer la réponse 401 Non autorisé et rediriger l’utilisateur vers Logto pour l’authentification.
5. Après une authentification réussie, l’agent IA doit recevoir un jeton d’accès et l’utiliser pour effectuer des requêtes vers votre service.
6. L’agent IA doit pouvoir récupérer la ressource depuis votre service à l’aide du jeton d’accès.
