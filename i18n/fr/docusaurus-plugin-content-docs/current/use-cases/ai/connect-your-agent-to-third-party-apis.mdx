...
- **💻 Gestion de code** : Créer des issues GitHub, relire des pull requests ou gérer des dépôts.
...
sequenceDiagram
    ...
    rect rgba(200, 230, 200, 0.5)
        Note over Utilisateur, Google: L'utilisateur accorde des permissions au fournisseur tiers
        Utilisateur->>Agent: "Ajoute une réunion à mon agenda demain à 15h"
        Agent->>Utilisateur: Besoin d’un accès Google Calendar, veuillez autoriser
        Utilisateur->>Agent: Initier l’autorisation Google
        Agent->>Logto: Initier la vérification sociale
        Logto->>Google: Rediriger vers Google
        Utilisateur->>Google: S’authentifier et accorder les permissions
    ...
    rect rgba(200, 200, 230, 0.5)
        Note over Utilisateur, Google: Exécution de la tâche par l’agent IA
        ...
        Agent->>Utilisateur: "C’est fait ! J’ai ajouté la réunion à votre agenda."
    end
```
...
Une fois autorisé, l’utilisateur peut effectuer plusieurs tâches sans devoir réautoriser. Logto stocke les jetons en toute sécurité et les renouvelle automatiquement si besoin, offrant une expérience fluide pour les interactions continues avec l’agent IA.
...
3. Suivez le [guide de configuration du connecteur Google](/integrations/google) pour configurer vos identifiants client OAuth.
...
Si aucun jeton n’existe, que le jeton a expiré ou que vous devez étendre la portée du jeton d’accès, utilisez l’[API de vérification sociale](/secret-vault/federated-token-set#reauthentication-and-token-renewal) de Logto pour initier le flux d’autorisation :
...
### Exemple : Passer de l’accès en lecture à l’écriture \{#example-upgrading-from-read-to-write-access}
...
Lorsque vous développez des agents IA qui accèdent à des API tierces, gardez ces pratiques de sécurité à l’esprit :
...
- **Auditez l’accès aux API** : Journalisez quand votre agent IA accède aux API tierces pour le dépannage et la conformité.
