...
Vous pouvez en apprendre plus sur [Qu’est-ce que le RBAC](https://auth-wiki.logto.io/rbac) dans l’Auth Wiki.
...
La propriété des ressources est un concept fondamental dans la conception de l’autorisation de notre CMS. Alors que le RBAC définit quelles actions les différents rôles peuvent effectuer, la propriété ajoute une dimension personnelle au contrôle d’accès :
...
- Le système vérifie à la fois les permissions de rôle OU la propriété lors des opérations sur les articles
...
Cette approche à double niveau (rôles + propriété) crée un système plus intuitif et sécurisé. Les éditeurs et administrateurs peuvent toujours gérer tout le contenu via leurs permissions de rôle, tandis que les auteurs gardent le contrôle sur leur propre travail.
...
Sur la base de nos besoins d’accès à l’API, nous pouvons définir ces permissions :
...
Maintenant que nous avons défini notre API et nos permissions, nous pouvons créer des rôles qui regroupent ces permissions de façon logique :
...
- **Admin** : A un contrôle complet sur le CMS, y compris toutes les opérations sur les articles
- **Éditeur** : Se concentre sur la relecture du contenu et la gestion de la publication
...
### Création des rôles \{#creating-roles}
...
### Attribution des rôles aux utilisateurs \{#assigning-roles-to-users}
...
Tout d’abord, nous devons ajouter un middleware dans le backend pour vérifier les permissions utilisateur, vérifier si l’utilisateur est connecté et déterminer s’il dispose des permissions nécessaires pour accéder à certaines APIs.
...
Testons maintenant notre mise en œuvre RBAC du CMS avec les trois utilisateurs que nous venons de créer.

:::note
Si vous constatez que vous ne pouvez pas vous connecter avec les identifiants des utilisateurs créés dans "Gestion des utilisateurs", vous devrez d’abord activer la méthode de connexion appropriée. Allez dans "Expérience de connexion" dans la Console Logto et activez votre méthode d’authentification préférée (comme Email + Mot de passe ou Nom d’utilisateur + Mot de passe).
:::
...
Bob, avec le rôle Éditeur, peut voir et publier tous les articles mais ne peut pas en créer, mettre à jour ou supprimer.
...
Pour des scénarios plus complexes, comme la création d’applications multi-locataires, Logto offre un support complet des organisations. Consultez notre guide [Construire une application SaaS multi-locataire : Guide complet de la conception à la mise en œuvre](https://blog.logto.io/build-multi-tenant-saas-application) pour en savoir plus sur la mise en œuvre du contrôle d’accès à l’échelle de l’organisation.
