...
:::note
Changer le domaine après la mise en ligne de votre service peut causer des problèmes, car le code de votre application et les intégrations pourraient encore référencer l'ancien domaine. Pour assurer une transition en douceur, nous recommandons **de configurer vos domaines personnalisés dès le début** lors de la création du tenant en Production.
:::
...
- URLs liées à [Passkey](/end-user-flows/mfa/webauthn) et à la [connexion par passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) (Changer le domaine après que les utilisateurs aient lié des Passkeys peut bloquer à la fois la MFA et la connexion avec ces identifiants).
...
### Délai d’attente dépassé (Erreur 522) pour un domaine hébergé chez Cloudflare \{#cloudflare-522-timeout}
...
Sur la page de détails de votre application dans <CloudLink to="/applications">Console > Applications</CloudLink>, faites défiler jusqu’à la section "Points de terminaison & Identifiants". Utilisez le menu déroulant des domaines pour afficher et copier les points de terminaison correspondants afin de mettre à jour les paramètres de votre application.
...
2. Dans la section "Configurer dans l’IdP", utilisez le menu déroulant des domaines pour basculer entre vos domaines personnalisés.
...
### Passkey pour la MFA et la connexion \{#passkey-for-mfa}
