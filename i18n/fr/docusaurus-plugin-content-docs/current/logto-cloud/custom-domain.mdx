...
- URI de rappel pour les [connecteurs sociaux](/connectors/social-connectors) ou [connecteurs SSO d’entreprise](/connectors/enterprise-connectors).
- URL liées à la [clé d’accès](/end-user-flows/mfa/webauthn) et à la [connexion par clé d’accès](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) (Changer de domaine après que les utilisateurs ont lié des clés d’accès peut bloquer à la fois la MFA et la connexion avec ces identifiants).
...
## Plusieurs domaines personnalisés \{#multiple-custom-domains}
...
- Offrir des expériences d’authentification spécifiques à une région ou à une marque via la [personnalisation de l’interface](/customization)
...
Pour ajouter plusieurs domaines personnalisés, répétez simplement les étapes ci-dessus pour chaque domaine que vous souhaitez configurer.

## Fichiers de vérification de domaine \{#domain-verification-files}

Certaines plateformes tierces exigent que vous hébergiez un petit fichier texte ou JSON sur votre domaine pour prouver la propriété avant de pouvoir utiliser leur OAuth, SSO ou d’autres intégrations. Comme un domaine personnalisé Logto pointe vers Logto Cloud, vous ne pouvez pas téléverser ce fichier sur votre propre origine. Logto peut héberger ces fichiers pour vous.

Après qu’un domaine personnalisé soit **Actif**, ouvrez la carte du domaine dans <CloudLink to="/tenant-settings/domains">Console > Paramètres > Domaines</CloudLink> et utilisez la section **Fichiers de vérification de domaine** pour ajouter, mettre à jour ou supprimer des fichiers.

**Chemins pris en charge**

- Noms de fichiers à la racine avec une extension, comme `/MP_verify_xxx.txt` ou `/apple-developer-domain-association.txt`
- Chemins sous `/.well-known/`, comme `/.well-known/apple-developer-domain-association.txt`
...
- Type de contenu : texte brut (`text/plain`) ou JSON (`application/json`)
...
Les routes Logto existantes sont toujours prioritaires. Les fichiers de vérification ne sont servis que lorsqu’aucun autre gestionnaire Logto ne correspond au chemin.
...
Pour résoudre ce problème, vous devrez lever le blocage de la zone. Suivez le lien ci-dessus pour savoir comment lever le blocage dans Cloudflare.
...
Sur la page de détails de votre application dans <CloudLink to="/applications">Console > Applications</CloudLink>, faites défiler jusqu’à la section "Points de terminaison & Identifiants". Utilisez le menu déroulant de domaine pour afficher et copier les points de terminaison correspondants afin de mettre à jour les paramètres de votre application.
...
### Mise à jour des URI de redirection des connecteurs sociaux \{#updating-social-connector-redirect-uris}
...
2. Copiez l’URI de redirection affichée dans les détails du connecteur. Logto liste toutes les URI de redirection disponibles pour vos domaines personnalisés configurés.
...
- Ajoutez toutes les URI de redirection pour chaque domaine personnalisé que vous avez configuré. Cela garantit que la connexion sociale fonctionne quel que soit le domaine utilisé par les utilisateurs.
- Le domaine Logto par défaut (`*.logto.app`) reste valide. Incluez-le uniquement si vous souhaitez également prendre en charge les connexions via le domaine par défaut.
- Pour le connecteur GitHub, utilisez les GitHub Apps au lieu des applications OAuth configurées dans le tableau de bord GitHub, car les GitHub Apps prennent en charge plusieurs URI de redirection. Les applications OAuth ne prennent en charge qu’une seule URI de redirection.

### Mise à jour des URI de redirection des connecteurs SSO d’entreprise basés sur OIDC \{#updating-oidc-enterprise-sso-redirect-uris}
...
2. Copiez les URI de redirection dans les détails du connecteur. Logto liste toutes les URI de redirection disponibles pour vos domaines personnalisés configurés.
...
### Mise à jour des URL ACS des connecteurs SSO d’entreprise basés sur SAML \{#updating-saml-enterprise-sso-acs-urls}

Les [connecteurs d’entreprise basés sur SAML](/connectors/enterprise-connectors) utilisent une URL de service consommateur d’assertion (ACS) au lieu d’une URI de redirection.

**Étapes :**

1. Accédez à <CloudLink to="/enterprise-sso">Console > SSO d’entreprise</CloudLink> et sélectionnez votre connecteur SAML.
2. Dans la section "Configurer dans l’IdP", utilisez le menu déroulant de domaine pour basculer entre vos domaines personnalisés.
3. Copiez l’URL ACS pour le domaine que vous souhaitez prendre en charge.
4. Ajoutez ces URL ACS à la configuration de votre fournisseur d’identité SAML.
...
### Clé d’accès pour MFA et connexion \{#passkey-for-mfa}

Les [clés d’accès pour l’authentification multi-facteurs (MFA)](/end-user-flows/mfa/webauthn) et la [connexion par clé d’accès](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) sont liées au domaine sur lequel elles ont été enregistrées. Les utilisateurs doivent se connecter via le même domaine pour utiliser leurs clés d’accès.

Limitation actuelle : Logto ne prend pas encore en charge la vérification de clé d’accès inter-domaine. Si un utilisateur enregistre une clé d’accès sur `auth.us.example.com`, il doit se connecter via `auth.us.example.com` pour utiliser cette clé d’accès, que ce soit pour la vérification MFA ou la connexion par clé d’accès. Une clé d’accès enregistrée sur un domaine ne peut pas être utilisée lors d’une connexion via un autre domaine personnalisé.
