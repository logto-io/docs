...
- URLs liées à [Passkey](/end-user-flows/mfa/webauthn) et à la [connexion par passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) (Changer de domaine après que les utilisateurs aient lié des Passkeys peut bloquer à la fois la MFA et la connexion avec ces identifiants).
...
- Fournir des expériences d'authentification spécifiques à une région ou à une marque via la [personnalisation de l’interface](/customization)
...
1. Rendez-vous sur <CloudLink to="/tenant-settings/domains">Console > Paramètres > Domaines</CloudLink>.
2. Dans la section "Ajouter un domaine personnalisé", saisissez votre sous-domaine (ex : `auth.example.com`, `auth.us.example.com`) et cliquez sur "ajouter le domaine".

   <img src="/img/assets/custom-domain-section.png" alt="Ajouter un domaine" />

3. Copiez la valeur **CNAME** `domains.logto.app` dans le tableau, puis allez chez votre fournisseur DNS pour ajouter l'enregistrement.
...
4. Attendez la vérification et le processus SSL.
   ...
   2. La vérification prend généralement quelques minutes mais peut aller jusqu'à 24 heures selon le fournisseur DNS. Vous pouvez quitter la page pendant ce temps.
...
- Noms de fichiers à la racine avec extension, comme `/MP_verify_xxx.txt` ou `/apple-developer-domain-association.txt`
...
### Délai d’attente dépassé (Erreur code 522) pour un domaine hébergé chez Cloudflare \{#cloudflare-522-timeout}
...
Vérifiez également que les URI de redirection enregistrés dans <CloudLink to="/applications">Console → Applications</CloudLink> correspondent au domaine que vous utilisez.
...
1. Rendez-vous sur <CloudLink to="/connectors/social">Console > Connecteurs > Connecteurs sociaux</CloudLink> et sélectionnez votre connecteur.
...
3. Ajoutez cette URI de redirection dans la console développeur de votre fournisseur social (ex : Google, GitHub, Facebook).
...
1. Rendez-vous sur <CloudLink to="/enterprise-sso">Console > SSO d’entreprise</CloudLink> et sélectionnez votre connecteur OIDC.
...
### Mise à jour des URLs ACS des connecteurs SSO d’entreprise basés sur SAML \{#updating-saml-enterprise-sso-acs-urls}
...
1. Rendez-vous sur <CloudLink to="/enterprise-sso">Console > SSO d’entreprise</CloudLink> et sélectionnez votre connecteur SAML.
...
### Passkey pour MFA et connexion \{#passkey-for-mfa}

Les [Passkeys pour l’authentification multi-facteurs (MFA)](/end-user-flows/mfa/webauthn) et la [connexion par passkey](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) sont liées au domaine sur lequel elles ont été enregistrées. Les utilisateurs doivent se connecter via le même domaine pour utiliser leurs passkeys.
