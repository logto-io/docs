...
![Informations sur la prochaine facturation](./assets/next-billing-info.png)

## Détail des éléments

Gardez à l'esprit que seules quelques fonctionnalités font partie de la structure des suppléments.
...
|                                     | Quota gratuit | Coût supplémentaire              |
| ----------------------------------- | ------------- | -------------------------------- |
| **Jetons**                          | 1 million     | 80 $ **_par million_** de jetons |
...
| **Organisation**                    | _N/A_         | 48 $ par mois                    |
...
1. **prix unitaire supplémentaire** : Le prix unitaire de ce supplément
2. **quantité ajoutée** : La quantité de supplément ajoutée par l'utilisateur dans le cycle de facturation **_actuel_**
3. **quantité supprimée** : La quantité de supplément supprimée par l'utilisateur dans le cycle de facturation **_actuel_**
...
### Exemple 1 : Facturation cohérente sans changements
...
### Exemple 2 : Ajouter des suppléments ou effectuer des changements affectera vos factures à venir
...
**Cas 1 : Vous avez souscrit au plan Pro sans aucun supplément et avez seulement testé le SSO d’entreprise pendant 10 jours.**

Si votre abonnement se renouvelle le 5 de chaque mois et que vous ajoutez un SSO d’entreprise le 20, vous l'utiliserez pendant 15 jours ce mois-là. Si vous supprimez le SSO d’entreprise le 30, cela signifie que vous l'avez utilisé seulement pendant 10 jours.

```math
\$16 + \$48 \times (1 \times 15 / 30) - \$48 \times (1 \times 5 / 30) = \$32
```

**Cas 2 : Vous avez souscrit au plan Pro sans aucun supplément. Après avoir ajouté et supprimé des ressources API, vous avez fini par utiliser 5 ressources API**
...
### Comment est déterminé le temps d'utilisation des suppléments ?

Nous utilisons un modèle de paiement à l'utilisation, et vous vous demandez peut-être ce qui se passe si vous ajoutez ou supprimez fréquemment des ressources. Le supplément est considéré comme utilisé pour la durée exacte pendant laquelle il est actif. Ne vous inquiétez pas, notre système, alimenté par Stripe, calcule votre utilisation à la seconde près, garantissant une facturation précise basée sur la durée exacte d'utilisation de chaque ressource.

Si vous vous désabonnez du plan Pro, nous vous rembourserons le coût des suppléments non utilisés, et le montant sera déduit de votre facture finale.

## Mise à niveau ou rétrogradation

### Mise à niveau du plan gratuit vers le plan Pro

Les mises à niveau et rétrogradations s'appliquent uniquement au locataire de production. Voici des situations où vous pourriez envisager de passer du plan gratuit au plan Pro :
...
### Mise à niveau du plan Pro vers le plan Enterprise
...
### Rétrogradation du plan Pro vers le plan gratuit
...
## Changement futur de plan tarifaire
