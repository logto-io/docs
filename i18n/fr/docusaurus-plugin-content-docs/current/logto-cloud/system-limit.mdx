...
Les limites du système permettent de garder Logto stable et équitable pour tous. Elles sont distinctes de la facturation.

**TL;DR**

- **Tenants Dev :** plafonds fixes (limites strictes)
- **Tenants Pro :** plafonds de départ (limites souples). Nous pouvons les augmenter pour un usage légitime.
- **Tenants Enterprise :** configurables, définis dans le contrat

Sur la page de tarification, **illimité** signifie _aucun plafond de facturation prédéfini dans le cadre d'une utilisation équitable_. Pour protéger la stabilité de la plateforme, nous appliquons des **garde-fous initiaux** (comme des plafonds d'entités et des limites de débit) qui peuvent être augmentés pour des charges de travail légitimes.
...
Par défaut : **200 requêtes par 10 secondes par tenant** (token-bucket). Cela correspond à environ **20 RPS soutenus**, avec des pics brefs autorisés jusqu'à la capacité du bucket.
...
- endpoints concernés (par exemple, connexion, jeton, JWKS, userinfo, Management API)
- délai (quand vous avez besoin de l'augmentation)

[Contactez-nous](https://logto.io/contact)

## Protection contre les limites au niveau du tenant \{#tenant-level-rate-limit-protection}
...
Les limites des tenants Dev sont des **limites strictes**, dimensionnées pour des charges de travail de développement. Si vous avez besoin d'un environnement proche de la production, vous pouvez [convertir le tenant en Pro](/logto-cloud/tenant-settings#tenant-types-dev-vs-prod).

Les tenants Dev suivent également une [politique de conservation des données distincte](/logto-cloud/dev-tenant-data-retention).
...
Les tenants Pro bénéficient de plafonds plus élevés. Les plafonds ci-dessous sont des **garde-fous initiaux**, et peuvent être augmentés pour des charges de travail légitimes (y compris des limites de débit plus élevées).
