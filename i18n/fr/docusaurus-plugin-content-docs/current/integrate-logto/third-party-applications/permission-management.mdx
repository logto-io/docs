...
Ces permissions sont des portées standard OIDC et des portées essentielles de profil utilisateur Logto utilisées pour accéder aux revendications utilisateur. Les revendications utilisateur seront retournées dans le jeton d’identifiant (ID token) et via l’endpoint userinfo en conséquence.
