...
Logto vous permet de contrôler les permissions spécifiques accordées aux applications tierces. Cela inclut la gestion du profil utilisateur, des ressources API et des portées d’organisation. Contrairement aux applications de première partie, les applications tierces demandant des portées non autorisées se verront refuser l'accès.
...
Accédez à la page **Détails de l'application** de votre application tierce OIDC, naviguez vers l'onglet **Permissions** et cliquez sur le bouton **Ajouter des permissions** pour gérer les permissions de vos applications tierces.
...
Attribuez des permissions au niveau utilisateur, y compris les [permissions de profil utilisateur](#user-permissions-user-profile-scopes) (par exemple, email, nom et avatar) et les [permissions de ressources API](#api-resource-permissions-api-resource-scopes) (par exemple, accès en lecture ou écriture à des ressources spécifiques).

Les noms des ressources demandées (par exemple, Données utilisateur personnelles, Nom de l'API) et les descriptions spécifiques des permissions (par exemple, Votre adresse email) apparaîtront sur l'écran de consentement pour que les utilisateurs les examinent.
...
- `email` : Portée standard OIDC, utilisée pour accéder à l'email de l'utilisateur.
