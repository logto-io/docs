...
- **URL du service consommateur d'assertions (URL de réponse)** (Requis) : Entrez l'URL où Logto doit envoyer l'assertion SAML après une authentification réussie. Cette URL doit correspondre à l'URL ACS fournie dans votre application Fournisseur de Services (SP).

- **ID d'entité du Fournisseur de Services (SP)** (Requis) : Entrez l'identifiant unique de votre Fournisseur de Services. Cette valeur doit correspondre à l'ID d'entité trouvé dans votre application SP. L'ID d'entité SP est une entrée de chaîne qui suit généralement un format URI (mais pas nécessairement).
  - Les formats courants incluent :
    - `urn:your-domain.com:sp:saml:{serviceProviderId}`
    - `https://your-domain/saml/{serviceProviderId}`

## Métadonnées SAML IdP \{#saml-idp-metadata}
...
L'identifiant unique pour le Fournisseur d'Identité.
...
Logto utilise ce certificat pour signer les assertions SAML. Vous devrez le configurer dans votre SP pour vérifier les signatures :
...
![Certificats d'application SAML](./assets/certificates.png)
...
![Format de l'ID de nom SAML](./assets/name-id-format.png)
...
- **Transitoire** (Utiliser un ID utilisateur unique comme ID de nom) : Génère un identifiant temporaire, unique qui change à chaque requête d'authentification. Cela offre une confidentialité accrue et convient aux applications où le suivi utilisateur persistant n'est pas souhaité.
...
![Chiffrement de l'assertion SAML](./assets/encryption.png)
