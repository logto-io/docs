...
`maxAllowedGrants` est un champ optionnel au niveau de l’application sous `customClientMetadata` qui contrôle le nombre maximal de grants actifs simultanés par utilisateur pour l’application courante.

- **Défaut** : `undefined` (aucune limite)
- **Si configuré** : À chaque autorisation réussie, Logto vérifie le nombre total de grants actifs pour l’utilisateur dans l’application courante (tous navigateurs et appareils confondus). Si la limite est dépassée, Logto révoque les grants les plus anciens.
