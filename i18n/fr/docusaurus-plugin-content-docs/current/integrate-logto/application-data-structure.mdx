...
Le point de terminaison de déconnexion backchannel OpenID Connect. Voir [Déconnexion fédérée : Back-channel logout](#) pour plus d’informations.

### Nombre maximal de grants autorisés (`maxAllowedGrants`) \{#max-allowed-grants-maxallowedgrants}

`maxAllowedGrants` est un champ optionnel au niveau de l’application sous `customClientMetadata` qui contrôle le nombre maximal de grants actifs simultanés par utilisateur pour l’application actuelle.

- **Défaut** : `undefined` (aucune limite)
- **Si configuré** : À chaque autorisation réussie, Logto vérifie le nombre total de grants actifs pour l’utilisateur dans l’application actuelle (tous navigateurs et appareils confondus). Si la limite est dépassée, Logto révoque les grants les plus anciens.
...
Informations supplémentaires personnalisées sur l’application non listées dans les propriétés prédéfinies de l’application ; les utilisateurs peuvent définir leurs propres champs de données personnalisées selon leurs besoins spécifiques, tels que des paramètres et configurations métier.
