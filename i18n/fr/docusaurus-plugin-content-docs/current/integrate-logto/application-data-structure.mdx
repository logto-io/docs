...
- Les jokers dans le hostname doivent inclure au moins un point (par exemple, `https://*.example.com/callback`)
...
:::caution TTL du jeton de rafraîchissement et TTL de la subvention
...
### Nombre maximal de subventions autorisées (`maxAllowedGrants`) \{#max-allowed-grants-maxallowedgrants}
...
- **Si configuré** : À chaque autorisation réussie, Logto vérifie le nombre total de subventions actives pour l’utilisateur dans l’application actuelle (tous navigateurs et appareils confondus). Si la limite est dépassée, Logto révoque les subventions les plus anciennes.
