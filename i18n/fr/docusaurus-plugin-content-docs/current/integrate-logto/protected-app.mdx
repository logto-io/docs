...
graph LR
  ...
  B --> Condition{{La route<br/>correspond-elle ?}}
  Condition -->|Oui| Matched{{Est-il authentifié ?}}
...
Pour vérifier les en-têtes au sein de votre application, consultez l'[exemple d'authentification de base HTTP](https://developers.cloudflare.com/workers/examples/basic-auth/) fourni par Cloudflare pour apprendre à restreindre l'accès en utilisant le schéma d'authentification de base HTTP.
...
Si vous avez besoin d'obtenir l'authentification et les informations utilisateur pour votre application, vous pouvez également utiliser l'en-tête `Logto-ID-Token`.
