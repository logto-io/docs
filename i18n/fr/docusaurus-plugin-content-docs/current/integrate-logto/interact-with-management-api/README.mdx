...
La Management API de Logto est un ensemble complet d’API qui donne aux développeurs un contrôle total sur leur implémentation afin de répondre aux besoins de leur produit et de leur stack technique. Elle est préconfigurée, listée dans <CloudLink to="/api-resources">Console > Ressources API > Logto Management API</CloudLink>, et ne peut pas être supprimée ou modifiée.
...
Avec la Logto Management API, vous pouvez accéder aux puissants services backend de Logto, qui sont hautement évolutifs et peuvent être utilisés dans de nombreux scénarios. Elle va au-delà de ce qui est possible avec les capacités low-code de la Console Admin.
...
#### Principes de base sur la demande de jeton d’accès \{#basics-about-access-token-request}
...
### Gérer les réponses paginées de l’API \{#managing-paginated-api-responses}
...
Logto Cloud applique des limites de débit au niveau du locataire à l’exécution pour protéger la stabilité du système. Pour plus de détails, voir la [section limite de débit du système](/logto-cloud/system-limit#rate-limit).
