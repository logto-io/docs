...
In dieser API sollte das Backend ordnungsgemäße Autorisierungsprüfungen durchführen, um sicherzustellen, dass Sarah die notwendigen Berechtigungen hat, um Alex zu imitieren.
...
Nun tauscht Sarahs Anwendung dieses Subjekt-Token gegen ein Zugangstoken aus, das Alex repräsentiert, und gibt die Ressource an, für die das Token verwendet wird.
...
Das zurückgegebene `access_token` ist an die angegebene Ressource gebunden und kann nur mit der Customer Data API von TechCorp verwendet werden.
...
Um den `act`-Anspruch einzuschließen, muss Sarahs Anwendung ein `actor_token` im Token-Austausch-Request bereitstellen. Dieses Token sollte ein gültiges Zugangstoken für Sarah mit dem `openid`-Scope sein. So wird es im Token-Austausch-Request eingebunden:
...
Für SPA oder native Anwendungen gib `client_id` stattdessen im Request-Body an:
...
Wenn der Server von TechCorp ein Subjekt-Token von Logtos Management API anfordert, kann er ein `context`-Objekt übergeben:
...
Dieser [Kontext](/developers/custom-token-claims/create-script#context-only-available-for-user-access-token) kann dann in einer `getCustomJwtClaims()`-Funktion verwendet werden, um spezifische Claims zum finalen Zugangstoken hinzuzufügen. Hier ein Beispiel, wie das implementiert werden könnte:
