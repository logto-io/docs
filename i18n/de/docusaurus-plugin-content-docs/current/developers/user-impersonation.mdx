...
Der Server von TechCorp ruft nach Validierung von Sarahs Anfrage die [Management API](/integrate-logto/interact-with-management-api) von Logto auf, um ein Subjekt-Token zu erhalten.
...
Das zurückgegebene `access_token` wird an die angegebene Ressource gebunden, wodurch sichergestellt wird, dass es nur mit der Kundendaten-API von TechCorp verwendet werden kann.
...
3. Der Backend-Server von TechCorp muss ordnungsgemäße Autorisierungsprüfungen implementieren, um sicherzustellen, dass nur autorisierte Support-Mitarbeiter wie Sarah Mimikry anfordern können.
   :::

## `act` Anspruch

Beim Verwenden des Token-Austausch-Flows für Mimikry kann das ausgegebene Zugangstoken einen zusätzlichen `act` (Akteur) Anspruch enthalten. Dieser Anspruch repräsentiert die Identität der "handelnden Partei" - in unserem Beispiel Sarah, die die Mimikry durchführt.
...
## Anpassung von Token-Ansprüchen
