...
### Vertrauenswürdiges Gerät \{#trusted-device}
...
| Benutzer wird automatisch über [Just-in-Time-Bereitstellung](/organizations/just-in-time-provisioning) (passende E-Mail-Domain oder Enterprise SSO Connector) in eine Organisation aufgenommen | Organization.Membership.Updated |

## Ausnahme-Webhooks \{#exception-hook-events}
...
| Grant.LimitExceeded | Eine erfolgreiche Autorisierung (Authorization) überschreitet das [maximale Limit gleichzeitiger authentifizierter Geräte](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) (`maxAllowedGrants`) einer App, sodass Logto die ältesten Berechtigungen für diese App widerruft.         |
