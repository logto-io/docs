...
## Webhook-Ereignisse bei Benutzerinteraktionen \{#user-interaction-hook-events}
...
## Webhook-Ereignisse bei Datenänderungen \{#data-mutation-hook-events}
...
| Benutzer-Sozial/SSO-Verknüpfung                                                                                                                                                             | User.Data.Updated               |
| Benutzer-Passwort zurücksetzen                                                                                                                                                              | User.Data.Updated               |
| Benutzerregistrierung                                                                                                                                                                       | User.Created                    |
| Benutzer wird automatisch über [Just-in-Time-Bereitstellung](/organizations/just-in-time-provisioning) (passende E-Mail-Domain oder Enterprise SSO Connector) einer Organisation zugeordnet | Organization.Membership.Updated |

## Ausnahme-Webhooks-Ereignisse \{#exception-hook-events}
...
| Message.RateLimited | Ein Endbenutzer überschreitet das [Sendelimit](/security/send-rate-limit) pro Empfänger für Verifizierungscodes. Wird nur auf den Endbenutzer-Sendewegen (Experience API und Account API) ausgelöst, mit einer Nutzlast von `{ action, recipient }`.                                                         |
| Grant.LimitExceeded | Eine erfolgreiche Autorisierung überschreitet das [maximale Limit gleichzeitiger authentifizierter Geräte](/sessions/session-configs#max-concurrent-authenticated-devices-per-app) (`maxAllowedGrants`) einer App, sodass Logto die ältesten Berechtigungen für diese App widerruft.                         |
