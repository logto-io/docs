...
- Logto Cloud bezahlte Mandanten mit Produktionsumgebung (einschließlich [Pro-Mandanten und Enterprise-Mandanten](https://logto.io/pricing))
  :::

## Implementiere die Funktion `getCustomJwtClaims()` \{#implement-getcustomjwtclaims-function}

Auf der **Benutzerdefiniertes JWT**-Detailseite findest du den Skripteditor, um dein benutzerdefiniertes Token-Ansprüche-Skript zu schreiben. Das Skript sollte eine `JavaScript`-Funktion sein, die ein Objekt mit benutzerdefinierten Ansprüchen zurückgibt.
...
Die Funktion `api.denyAccess()` ermöglicht es dir, den Token-Ausgabeprozess mit einer benutzerdefinierten Nachricht zu verweigern. Du kannst diese Funktion verwenden, um zusätzliche Zugangsvalidierungen während des Token-Ausgabeprozesses durchzusetzen.
...
:::note
Beachte, dass das Abrufen externer Daten zu einer Latenz beim Token-Ausgabeprozess führen kann. Stelle sicher, dass die externe API zuverlässig und schnell genug ist, um deine Anforderungen zu erfüllen.
