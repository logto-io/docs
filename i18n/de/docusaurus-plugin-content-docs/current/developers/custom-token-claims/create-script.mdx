...
Nutze den Code-Editor auf der linken Seite, um das Skript zu bearbeiten. Eine Standardfunktion `getCustomJwtClaims` mit einem leeren Objekt als Rückgabewert ist bereits für dich vorbereitet. Du kannst die Funktion anpassen, um ein Objekt mit deinen eigenen benutzerdefinierten Ansprüchen zurückzugeben.
...
- **Benutzerdaten-Objekt**
  Für Benutzer-Zugangstoken stellt Logto zusätzlichen Benutzerdaten-Kontext zur Verfügung. Das Benutzerdaten-Objekt enthält alle Benutzerdaten und Organisationsmitgliedschaften, die du benötigst, um die benutzerdefinierten Ansprüche einzurichten. Siehe [Benutzer](/user-management/user-data) und [Organisationen](/organizations/organization-management#organization-data-structure) für weitere Details.
- **Organisationsdaten-Objekt**
  Für [Organisations-Zugangstokens](/authorization/organization-template) (ein Zugangstoken, das für eine bestimmte Organisation angefordert wurde), stellt Logto die Zielorganisation über `context.organization` bereit, sodass du organisationsspezifische Ansprüche anhängen kannst – zum Beispiel die Zuordnung der Logto-Organisations-ID zu einer internen ID, die in den benutzerdefinierten Daten der Organisation gespeichert ist. Dieses Datenobjekt ist nur vorhanden, wenn das Token für eine bestimmte Organisation ausgestellt wird, und enthält:

  | Property      | Beschreibung                                   | Typ                       |
  | ------------- | ---------------------------------------------- | ------------------------- |
  | `id`          | Die eindeutige Organisations-ID                | `string`                  |
  | `name`        | Der Name der Organisation                      | `string`                  |
  | `description` | Die Beschreibung der Organisation              | `string \| null`          |
  | `customData`  | Die benutzerdefinierten Daten der Organisation | `Record<string, unknown>` |

  Siehe [Organisationen](/organizations/organization-management#organization-data-structure) für weitere Details.

- **Grant-Daten-Objekt**
  Für Benutzer-Zugangstoken, die durch Benutzermimikry-Token-Austausch gewährt wurden, stellt Logto zusätzlichen Grant-Daten-Kontext zur Verfügung. Das Grant-Daten-Objekt enthält den benutzerdefinierten Kontext aus dem Subjekt-Token. Siehe [Benutzermimikry](/developers/user-impersonation) für weitere Details.
...
Das `api`-Objekt stellt eine Reihe von Hilfsfunktionen bereit, die du in deinem Skript für zusätzliche Zugangskontrolle während des Token-Ausgabeprozesses verwenden kannst. Das `api`-Objekt enthält die folgende Funktion:
...
Benutzerdefinierte Zugangstoken-Skripte können diese Standard-Web-APIs sowohl in Logto Cloud als auch im selbst gehosteten Logto verwenden:
...
Zum Beispiel kannst du SHA-256- und HMAC-SHA-256-Werte mit Web Crypto berechnen:
...
Skripte können keine Pakete importieren. Vermeide Node.js-spezifische globale Variablen und Module, da sie nicht zwischen selbst gehostetem Logto und Logto Cloud portierbar sind und nicht Teil des unterstützten Skriptvertrags sind.

## Schritt 3: Externe Daten abrufen \{#step-3-fetch-external-data}

Du kannst die Standardfunktion `fetch` verwenden, um in deinem Skript externe Daten abzurufen. Die Funktion `fetch` ist eine auf Promises basierende Funktion, mit der du HTTP-Anfragen an externe APIs stellen kannst.
...
:::note
Beachte, dass das Abrufen externer Daten zu einer Verzögerung im Token-Ausgabeprozess führen kann. Stelle sicher, dass die externe API zuverlässig und schnell genug ist, um deine Anforderungen zu erfüllen.
...
Stelle sicher, dass du dein Skript testest, bevor du es speicherst. Klicke auf den Tab **Testkontext** auf der rechten Seite der Seite, um die simulierte Token-Payload und den Benutzerdaten-Kontext für Tests zu bearbeiten.

Klicke auf **Test ausführen** in der rechten oberen Ecke des Editors, um das Skript mit den Testdaten auszuführen. Die Ausgabe des Skripts wird im **Testergebnis**-Drawer angezeigt.

<img src="/img/assets/test-custom-jwt-script.png" alt="Benutzerdefiniertes JWT-Skript testen" />

:::note
Das Testergebnis ist die Ausgabe der Funktion `getCustomJwtClaims` mit den von dir festgelegten Testdaten ("zusätzliche Token-Ansprüche", die nach Abschluss von Schritt 3 im [Ablaufdiagramm](/developers/custom-token-claims/#how-do-custom-token-claims-work) erhalten wurden). Die tatsächliche Token-Payload und der Benutzerdaten-Kontext werden unterschiedlich sein, wenn das Skript im Token-Ausgabeprozess ausgeführt wird.
:::
