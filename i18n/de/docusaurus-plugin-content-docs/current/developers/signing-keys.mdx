...
1. Navigiere zu <CloudLink to="/tenant-settings/oidc-configs">Console > Tenant settings > OIDC configs</CloudLink>. Dort kannst du sowohl OIDC-Private Keys als auch OIDC-Cookie-Keys verwalten.
2. Um den Signaturschlüssel zu rotieren, klicke auf die Schaltfläche "Rotate private keys" oder "Rotate cookie keys". Beim Rotieren der privaten Schlüssel hast du die Möglichkeit, den Signaturalgorithmus zu ändern.
3. Du findest dort eine Tabelle, die alle verwendeten Signaturschlüssel auflistet. Für OIDC-Private Keys kannst du den vorherigen Schlüssel löschen, aber nicht den aktuellen oder nächsten Schlüssel. Für OIDC-Cookie-Keys kannst du den vorherigen Schlüssel löschen, aber nicht den aktuellen Schlüssel.
   ...
   | Status   | Beschreibung                                                                                                                                                                                                                |
   | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | Next     | Dieser Status wird für gestaffelte OIDC-Private-Key-Rotation verwendet. Der Schlüssel wurde erstellt, aber Logto verwendet ihn erst zum Signieren neuer JWTs, wenn die Übergangsfrist endet und der Schlüssel wirksam wird. |
   | Current  | Dies zeigt an, dass dieser Schlüssel derzeit von Logto zum Signieren neu ausgestellter JWTs oder Cookies verwendet wird.                                                                                                    |
...
3. **Entfernen des ältesten vorherigen Schlüssels**: Logto behält maximal einen "Previous"-Private-Key. Wenn bereits ein vorheriger Private Key existiert, wird beim Wirksamwerden des neuen Schlüssels der ältere vorherige Schlüssel entfernt.

Für Logto Cloud tritt die Rotation des OIDC-Private Keys nach einer Übergangsfrist von 4 Stunden in Kraft. Während dieser Zeit wird der neue Schlüssel vorbereitet und über JWKS bereitgestellt, bevor Logto beginnt, neu ausgestellte JWTs damit zu signieren. In der Console-Tabelle zeigt die Spalte **Effective at**, wann ein gestaffelter "Next"-Schlüssel zum "Current" wird.

Für selbstgehostete OSS-Deployments beträgt die Standard-Übergangsfrist für die Rotation privater Schlüssel `0` Sekunden, was bedeutet, dass die Rotation sofort erfolgt. Um eine gestaffelte Rotation zu nutzen, setze die Umgebungsvariable `PRIVATE_KEY_ROTATION_GRACE_PERIOD` im Logto-Service oder übergebe einen expliziten Wert für `rotationGracePeriod` im Request-Body beim Aufruf des Management API-Endpunkts `POST /api/configs/oidc/private-keys/rotate`.
