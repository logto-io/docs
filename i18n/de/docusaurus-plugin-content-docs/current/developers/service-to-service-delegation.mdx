...
Nutze das Token Exchange Grant von Logto, um das Zugangstoken des Benutzers gegen ein neues Zugangstoken auszutauschen, dessen Zielgruppe die nachgelagerte API-Ressource ist. Dies folgt dem OAuth 2.0 Token Exchange Muster und vermeidet das Weiterleiten des ursprünglichen Benutzertokens an nachgelagerte Dienste.

## Wann dieser Ablauf verwendet werden sollte \{#when-to-use-this-flow}

Verwende Service-to-service delegation, wenn:

- API A ein Backend-Service ist, der sich sicher am Token-Endpunkt von Logto authentifizieren kann.
...
sequenceDiagram
    ...
    User->>ApiA: Authorization: Bearer Benutzer-Zugangstoken
    Note over ApiA: Validierung des Benutzertokens für API A
    ...
    Note over ApiB: Überprüfe Aussteller, Zielgruppe, Ablauf und Berechtigungen
```
...
2. Überprüfe den Aussteller (`iss`).
3. Überprüfe, dass die Zielgruppe (`aud`) mit dem Ressourcenindikator von API B übereinstimmt.
4. Überprüfe das Ablaufdatum (`exp`).
5. Überprüfe die erforderlichen Berechtigungen.
