...
Die JIT-Bereitstellung ist eine nützliche Funktion für [B2B](/introduction/plan-your-architecture/b2b) und Multi-Tenancy-Produkte. Sie macht das Onboarding von Mandantenmitgliedern reibungslos und erfordert keine administrative Beteiligung.
...
| Verzeichnissynchronisation                     | Neu und bestehend | ✅            | Verwende die Verzeichnissynchronisationsfunktion des IdP, um Benutzer im Voraus in der App bereitzustellen.                                                                                                |
...
Wir haben die JIT-Funktionen auf ihrem skalierbarsten und sichersten Niveau implementiert, um den Bereitstellungsprozess für dich zu vereinfachen und zu beschleunigen. Da Bereitstellungssysteme jedoch komplex sein können und auf die spezifischen Bedürfnisse deiner Kunden zugeschnitten sein müssen, ist es wichtig, die vorgefertigten JIT-Funktionen von Logto, dein sorgfältiges Systemdesign und die Logto Management API zu kombinieren. Dieser integrierte Ansatz hilft dir, ein robustes und effizientes Bereitstellungssystem zu erstellen.
...
- **JIT-Bereitstellung** erzwingt nicht die Mitgliedschaft oder Rollenzuweisung, während **Verzeichnissynchronisation** diese erzwingen kann.
...
Die Adressübereinstimmung kann die verifizierte E-Mail-Adresse aus allen nicht-Enterprise-SSO-Identitätsquellen erkennen, einschließlich:
...
**Warum gilt die E-Mail-Domain-Bereitstellung nicht für den bestehenden Benutzeranmeldeprozess?**

Die Anmeldung bestehender Benutzer erfordert eine weitere Kontrolle, um festzustellen, ob sie einer bestimmten Organisation zugewiesen oder eine Rolle erhalten können. Dieser Prozess ist dynamisch und hängt von spezifischen Anwendungsfällen und geschäftlichen Anforderungen ab, wie Anmeldefrequenz und organisationsspezifischen Richtlinien.
...
| Benutzerstatus                                                                                                                                    | Beschreibung                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Benutzer existiert nicht, meldet sich mit einem sozialen Konto mit einer verifizierten E-Mail an                                                  | Benutzer wird erstellt und tritt automatisch der entsprechenden Organisation mit passenden Rollen bei. |
...
Bei der Bereitstellung von Benutzern in einer Organisation kannst du ihre Standard-Organisationsrollen festlegen. Die Rollenliste stammt aus der [Organisationstemplate](/authorization/organization-template#organization-role), und du kannst eine Rolle auswählen oder sie leer lassen.
