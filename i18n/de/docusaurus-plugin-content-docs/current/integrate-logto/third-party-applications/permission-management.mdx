---
description: Wähle App-Berechtigungen (Scopes) und stelle sicher, dass sie auf dem OAuth-Zustimmungsbildschirm klar angezeigt werden.
...
Durch das Aktivieren bestimmter Scopes bestimmst du, auf welche Benutzerinformationen Drittanbieter-Apps zugreifen können. Die Benutzer überprüfen und genehmigen diese Berechtigungen auf dem Zustimmungsbildschirm, bevor sie den Zugriff gewähren.
...
Grundlegende Benutzerdaten sind für Anfragen von Drittanbieter-Apps immer erforderlich. Zusätzlich unterstützt Logto die Zuweisung von Organisationsressourcen, was es ideal für B2B-Services macht.
...
- `custom_data`: Logto-Benutzerprofil-Scopes, verwendet für den Zugriff auf [benutzerdefinierte Benutzerdaten](/user-management/user-data/#custom-data).
...
### API-Ressourcen-Berechtigungen (API-Ressourcen-Scopes) \{#api-resource-permissions-api-resource-scopes}

Logto bietet rollenbasierte Zugangskontrolle (RBAC) für API-Ressourcen. API-Ressourcen sind die Ressourcen, die deinem Dienst gehören und von Logto geschützt werden. Du kannst selbstdefinierte API-Scopes Drittanbieteranwendungen zuweisen, damit sie auf deine API-Ressourcen zugreifen können. Weitere Informationen findest du unter [Autorisierung](/authorization).
...
Du kannst eigene Organisations-Scopes auf der Seite mit den Organisationstemplate-Einstellungen definieren. Siehe [Organisationstemplate](/authorization/organization-template) für weitere Details.
...
Kern-OIDC-Berechtigungen werden automatisch für deine App konfiguriert. Diese Scopes sind für die OIDC-Authentifizierung erforderlich und werden **nicht** auf dem Zustimmungsbildschirm angezeigt. OAuth-Apps können darauf verzichten, sie anzufordern, wenn keine OIDC-Authentifizierung benötigt wird.
