...
Passe das Aussehen und die Benutzererfahrung deiner Anmeldeseite an, indem du zu <CloudLink to="/sign-in-experience/branding">Konsole > Anmeldung & Konto > Branding</CloudLink> navigierst. In diesem Bereich kannst du wichtige Branding-Elemente ganz einfach anpassen.

### Markenfarbe \{#brand-color}

Definiere die Primärfarbe, die in der gesamten Anmeldeerfahrung und im sofort einsatzbereiten Account Center verwendet wird, einschließlich primärer Schaltflächen, Links und anderer Komponenten. Ersetze das standardmäßige Logto-Lila durch die Farbe deiner Marke. Für den Dunkelmodus kann eine separate Markenfarbe angegeben werden.
...
- Wenn du das Logofeld leer lässt, wird das Logo in der Oberfläche nicht angezeigt.
...
- Außerdem wird jetzt der Browser-Titel für verschiedene Abläufe (Anmelden / Registrieren / Passwort vergessen usw.) verwendet, anstelle eines benutzerdefinierten Titels.
...
Wenn du dann eine Anmeldung auslöst, kannst du die Organisations-ID im Parameter `organization_id` übergeben, um Logto mitzuteilen, welches Organisationslogo angezeigt werden soll. Um beispielsweise das Organisationslogo für die Organisations-ID `123456` anzuzeigen:
...
:::note
Wenn sowohl App-spezifisches Branding als auch organisationsspezifisches Branding aktiviert sind, hat das organisationsspezifische Branding Vorrang. Die vollständige Reihenfolge ist:
