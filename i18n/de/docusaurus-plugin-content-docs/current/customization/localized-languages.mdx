...
1. **Navigiere zu**: <CloudLink to="/sign-in-experience/content">Console > Anmeldung & Konto > Inhalt > Sprachen</CloudLink>.
...
4. **Standardsprache festlegen:** Du kannst eine Standardsprache aus deiner Sprachbibliothek auswählen. Sie wird verwendet, wenn die erkannte Benutzersprache nicht in der aktuellen Sprachbibliothek enthalten ist.
...
2. Andernfalls, wenn „Auto-Erkennung“ aktiviert ist, die erkannte Benutzersprache des Clients (z. B. aus dem HTTP-Header `Accept-Language`).
3. Andernfalls die Standard-Sprache des Tenants in der Anmeldungserfahrung.

Diese Auflösung beeinflusst auch die E-Mail-Lokalisierung für Nachrichten, die durch die Interaktion ausgelöst werden. Erfahre mehr: [E-Mail-Template-Lokalisierung](/connectors/email-connectors/email-templates#email-template-localization).
...
So kann der Benutzer, der deine Seite aus Japan besucht, Inhalte auf Japanisch lesen, die du gerade aus dem Englischen übersetzt hast.
...
Der Ländercode für Telefonnummern in der [Anmeldeerfahrung](/end-user-flows/sign-up-and-sign-in/sign-up) richtet sich standardmäßig nach der Browser-Lokalisierung des Benutzers. Wenn beispielsweise die Browsersprache eines Benutzers auf `fr` eingestellt ist, wird der Ländercode standardmäßig auf Frankreich (+33) gesetzt.
