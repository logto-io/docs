...
sequenceDiagram
    ...
    A --> A: Anmelderückruf bearbeiten und<br/>der Benutzer ist jetzt authentifiziert
```
...
Zusätzlich musst du mit Logtos Experience API interagieren, um Benutzer zu authentifizieren und Sitzungen zu verwalten.
Dieser Dienst wird auch helfen, diese Experience API-Anfragen an Logto Cloud weiterzuleiten, um CORS-Probleme zu vermeiden.

Das folgende Sequenzdiagramm veranschaulicht, wie ein erfolgreicher "Anmelden"-Fluss mit deiner benutzerdefinierten UI und dem Tunneldienst funktioniert:
