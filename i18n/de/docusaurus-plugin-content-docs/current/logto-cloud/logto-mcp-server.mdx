...
:::tip
Logto MCP Server wurde mit Logto und [MCP Auth](https://mcp-auth.dev) entwickelt. Wenn du deinen eigenen entfernten MCP-Server bauen möchtest, siehe [Baue deinen eigenen entfernten MCP-Server](#build-your-own-remote-mcp-server).
:::
...
3. Gib die URL ein: `https://mcp.logto.io`. Du kannst wählen, ob du sie für deinen Workspace installieren möchtest, um bessere Isolierung und Verwaltung zu erhalten.
...
2. Klicke im Abschnitt **Installed MCPs** auf das Zahnradsymbol rechts, um die Datei `~/.codeium/windsurf/mcp_config.json` zu erstellen und zu öffnen.
...
- **Ressource:** `https://mcp.logto.io/` (falls von deinem Client benötigt)
- **Scope:** `mcp:all` (falls von deinem Client benötigt)

Wenn du Entwickler einer KI-Anwendung bist und das Verbinden mit Logto MCP Server unterstützen möchtest, aber Logto MCP Server deine Anwendungs-Redirect-URI noch nicht unterstützt, [kontaktiere uns](https://logto.io/contact) mit den Details zu deiner Anwendung.

## Baue deinen eigenen entfernten MCP-Server \{#build-your-own-remote-mcp-server}

Logto MCP Server wurde mit Logto und [MCP Auth](https://mcp-auth.dev) entwickelt. Wenn du deinen eigenen entfernten MCP-Server mit Authentifizierung bauen möchtest, schaue dir die folgenden Ressourcen an:
