...
## Grundverständnis von RBAC \{#understanding-rbac-fundamentals}

Rollenbasierte Zugangskontrolle ist mehr als nur das Zuweisen von Berechtigungen an Benutzer. Es geht darum, einen strukturierten Ansatz für die Autorisierung zu schaffen, der Sicherheit und Wartbarkeit in Einklang bringt.

Mehr dazu findest du unter [Was ist RBAC](https://auth-wiki.logto.io/rbac) im Auth Wiki.

Hier sind die wichtigsten Prinzipien, denen wir in unserer Umsetzung folgen:

### Fein abgestufte Berechtigungs-Entwürfe \{#fine-grained-permission-design}

Fein abgestufte Berechtigungen geben dir präzise Kontrolle darüber, was Benutzer in deinem System tun können. Anstatt allgemeiner Zugriffsebenen wie „admin“ oder „user“ definieren wir spezifische Aktionen, die Benutzer auf Ressourcen ausführen dürfen. Zum Beispiel:
...
Ressourcenbesitz ist ein grundlegendes Konzept im Autorisierungsdesign unseres CMS. Während RBAC festlegt, welche Aktionen verschiedene Rollen ausführen dürfen, fügt der Besitz eine persönliche Dimension zur Zugangskontrolle hinzu:
...
Beachte, dass diese Berechtigungen nur benötigt werden, wenn auf Ressourcen zugegriffen wird, die man nicht selbst besitzt. Artikelbesitzer können automatisch:
...
| ------------------ | ---------------------------------------------- | -------------------------------------------------------- | ------------------------------------- |
| **Beschreibung**   | Vollzugriff für vollständige Inhaltsverwaltung | Kann alle Artikel ansehen und Veröffentlichungen steuern | Kann neue Artikel im System erstellen |
| list:articles      | ✅                                             | ✅                                                       | ❌                                    |
| read:articles      | ✅                                             | ✅                                                       | ❌                                    |
| create:articles    | ✅                                             | ❌                                                       | ✅                                    |
| update:articles    | ✅                                             | ❌                                                       | ❌                                    |
...
Folge zunächst den [Logto Quick Starts](/quick-starts), um Logto in deine Anwendung zu integrieren.
...
const logtoConfig: LogtoConfig = {
  ...
  resources: [API_RESOURCE], // Der API-Resource-Identifier, z. B. https://api.cms.com
  // Alle Scopes, die du von der API-Ressource im Frontend anfordern möchtest
  scopes: [
    'list:articles',
    'create:articles',
    'read:articles',
...
Wenn sich der Benutzer mit Logto anmeldet und ein Zugangstoken für die oben angegebenen API-Ressourcen anfordert, fügt Logto dem Zugangstoken Berechtigungen (Scopes) hinzu, die zur Rolle des Benutzers gehören.
...
Jetzt ist es Zeit, Logto RBAC in dein Backend zu integrieren.
...
Für APIs, die sowohl Berechtigungen als auch Ressourcenbesitz prüfen müssen, können wir die Funktion `hasScopes` verwenden. Zum Beispiel können Benutzer mit der Berechtigung `list:articles` in der Artikelliste-API alle Artikel abrufen, während Autoren nur ihre eigenen Artikel sehen:
...
Charles, mit der Autor-Rolle, kann nur eigene Artikel erstellen und nur Artikel sehen, bearbeiten und löschen, die er selbst besitzt.
