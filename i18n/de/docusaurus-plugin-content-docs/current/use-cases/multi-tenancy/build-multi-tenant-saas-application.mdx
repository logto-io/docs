---
sidebar_position: 2
sidebar_label: Baue eine Multi-Tenant-SaaS-Anwendung
...
> **Der vollständige Quellcode ist in diesem [Github Repo](https://github.com/logto-io/multi-tenant-saas-sample) verfügbar. Lass uns eintauchen!**
...
## Welche Funktionen sind für SaaS Authentifizierung und Autorisierung erforderlich? \{#what-features-are-required-for-saas-authentication-and-authorization}
...
### Registrierungs- und Anmeldefluss \{#sign-up-and-login-flow}
...
### Authentifizierungsstrategie definieren \{#define-authentication-strategy}
...
Moderne Auth-Anbieter (ich wähle diesmal [Logto](https://logto.io/)) haben all diese Funktionen für uns gebündelt. Der Authentifizierungsfluss ist unkompliziert:
...
sequenceDiagram
  ...
  auth->>u: Zeige Anmeldeseite
  u->>auth: Eingabe der Zugangsdaten
...
### Zugangskontrolle in Multi-Tenant-App aktivieren \{#enable-access-control-in-multi-tenant-app}
...
## Füge deiner App einen grundlegenden Authentifizierungsfluss hinzu \{#add-basic-authentication-flow-to-your-app}
...
Im Logto Console klicke im linken Menü auf "Sign-in & account". Dann auf den Tab "Sign-up and sign-in".
Auf dieser Seite folge den Anweisungen, um die Login-/Registrierungsmethoden von Logto zu konfigurieren.
...
## Multi-Tenant-Organisationserfahrung hinzufügen \{#adding-multi-tenant-organization-experience}
...
2. Verwendung der Logto Management API zum Erstellen von Organisationen, insbesondere wenn du einen SaaS-Flow entwirfst, der es Benutzern ermöglicht, eigene Organisationen (Workspaces) zu erstellen.
...
Für Anleitungen siehe die [Interaktion mit Management API](/integrate-logto/interact-with-management-api)-Dokumentation, um die API-Kommunikation mit Logto einzurichten.
...
2. **Zugriff auf Logto Management API**:
   ...
   - Folge der Anleitung [Interaktion mit Management API](/integrate-logto/interact-with-management-api) für die Einrichtung
...
In Logto verwenden wir Maschine-zu-Maschine-Authentifizierung für Zugangstokens. Siehe [Interaktion mit Management API](/integrate-logto/interact-with-management-api).
...
Der nächste Schritt wäre, Benutzer zu einer Organisation einzuladen. Diese Funktion implementieren wir in unserem Tutorial noch nicht. Du weißt bereits, wie du die Management API verwendest. Du kannst dich an diesem [**Tenant-Erstellung und Einladung**](#tenant-creation-and-invitation) als Produktdesign-Referenz orientieren und diese Funktion leicht umsetzen, indem du diesem Blogpost folgst: [Wie wir Benutzerzusammenarbeit in einer Multi-Tenant-App implementieren](https://blog.logto.io/implement-user-collaboration-in-your-app).

## Zugangskontrolle für deine Multi-Tenant-App implementieren \{#implement-access-control-to-your-multi-tenant-app}

Kommen wir nun zur Zugangskontrolle für Organisationen.

Wir wollen erreichen:

- Benutzer können nur auf Ressourcen zugreifen, die zu ihren eigenen Organisationen gehören: Das kann über Logtos `Organisationstoken` erfolgen
...
1. Wenn die an `getOrganizationToken` übergebene `organizationId` keine Organisation ist, der der aktuelle Benutzer angehört, kann diese Methode kein Token erhalten. So wird sichergestellt, dass Benutzer nur auf ihre eigenen Organisationen zugreifen können.
2. Beim Anfordern von Organisationsressourcen verwenden wir das Organisationstoken anstelle des Zugangstokens, da wir für Ressourcen, die zu einer Organisation gehören, die Organisationsberechtigungssteuerung und nicht die Benutzerberechtigungssteuerung verwenden möchten (du wirst das besser verstehen, wenn wir später die API `GET /documents` implementieren).
...
- [Organisationsebene Branding](/customization/match-your-brand#organization-specific-branding)
- [Organisationsebene Multi-Faktor-Authentifizierung (MFA)](/organizations/organization-management#require-mfa-for-organization-members)
...
Der vollständige Quellcode für dieses Tutorial ist verfügbar unter: [Multi-Tenant-SaaS-Beispiel](https://github.com/logto-io/multi-tenant-saas-sample).
