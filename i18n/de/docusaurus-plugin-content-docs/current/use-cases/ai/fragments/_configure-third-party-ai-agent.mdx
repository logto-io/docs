...
- [Einen Registrierungsdienst auf der Management API aufbauen](#allow-developers-to-create-third-party-apps-in-logto), wenn du kontrollieren möchtest, wer Zugangsdaten erhält.
...
In einem offenen MCP-Ökosystem kennst du die Agents in der Regel nicht im Voraus. [Dynamische App](/integrate-logto/third-party-applications/dynamic-apps) entfernt den Registrierungsschritt: Der Agent verwendet eine öffentliche HTTPS-URL, die sein eigenes Client-Metadaten-Dokument bereitstellt, als `client_id`, und Logto löst diese auf, wenn die Autorisierungsanfrage eintrifft.
...
sequenceDiagram
  ...
  Logto->>Metadata: Abruf des Client-ID-Metadaten-Dokuments
...
4. Klicke auf den Tab **Berechtigungen**, um der App [Berechtigungen](/integrate-logto/third-party-applications/permission-management) zu gewähren:

   - **Benutzer**-Abschnitt: Benutzer-Datenberechtigungen wie **`profile`** und **`email`**, für grundlegende Identitätsansprüche (Claims).
   - **API-Ressourcen**-Abschnitt: Berechtigungen (Berechtigungen) der [API-Ressourcen](/authorization/global-api-resources), die du in Logto definiert hast, z. B. die API-Ressource, die dein {props.serviceName ?? 'Dienst'} repräsentiert.
