...
- Ein Zugangstoken (Access token) für den AI-Agenten erhältst, damit dieser auf deinen Dienst zugreifen kann.
...
Schauen wir uns ein Beispiel an. Stell dir vor, du bist ein Entwickler, der einen Dienst bereitstellt, der Wetterdaten liefert.

**Offizielle Wetter-App (Dein eigener AI-Agent / App)**

- Du stellst eine offizielle Wetter-App bereit, mit der Benutzer das Wetter prüfen können.
- So funktioniert es: Die offizielle Wetter-App verbindet sich mit deinem Dienst und nutzt Logto, um Benutzer zu authentifizieren. Wenn Alice sich anmeldet, erhält sie automatisch Zugriff auf die Wetterdaten, ohne dass zusätzliche Zustimmungsbildschirme (Consent screens) erforderlich sind, da es sich um deine vertrauenswürdige App handelt.

**Drittanbieter-AI-Agent**

- Du baust ein Ökosystem rund um deinen Dienst auf, daher erstellt ein anderer Entwickler "WeatherBot" (einen AI-Assistenten, der Wetteraktualisierungen und Vorhersagen liefern kann) und integriert ihn als Drittanbieter-AI-Agenten.
- So funktioniert es: WeatherBot (Drittanbieter-AI-Agent) möchte über deinen Dienst auf die Wetterdaten der Benutzer zugreifen. Wenn Alice sich mit ihrem Konto bei WeatherBot anmeldet:
  ...
  - Nur die Daten, denen sie zustimmt, werden mit WeatherBot geteilt, und WeatherBot kann ohne ausdrückliche erneute Zustimmung auf keine weiteren Daten zugreifen.

Diese Zugriffskontrolle (Berechtigung) stellt die Sicherheit der Benutzerdaten sicher: Auch wenn dein Dienst alle Daten verwaltet, können Drittanbieter-AI-Agenten wie WeatherBot nur auf das zugreifen, was der Benutzer ausdrücklich erlaubt hat. Sie können diesen Prozess nicht umgehen, da er durch deine Zugriffskontroll-Implementierung im Dienst erzwungen wird.
...
- **Dienst**: Der Dienst, den du dem AI-Agenten zur Verfügung stellen möchtest.
...
Ein nicht-normativer Sequenzdiagramm illustriert den gesamten Ablauf des Prozesses:
...
sequenceDiagram
    ...
    Service->>Logto: Anfrage OpenID Connect-Konfiguration
    Logto->>Service: Konfiguration zurückgeben
    Note over Service: Konfiguration zwischenspeichern
    Agent->>Service: Anfrage Zugriff auf eine Ressource
    ...
    Agent->>Logto: Anfrage Zugangstoken mit Autorisierungscode
    Logto->>Agent: Zugangstoken zurückgeben
    Agent->>Service: Anfrage Ressource mit Zugangstoken
    Service->>Service: Zugangstoken validieren
    Service->>Agent: Ressourcenantwort zurückgeben
```
...
3. **Implementiere den Autorisierungsablauf**: Der AI-Agent sollte in der Lage sein, den OAuth 2.0-Autorisierungsablauf zu durchlaufen, um ein Zugangstoken von Logto zu erhalten.
4. **Teste die Integration**: Verwende den AI-Agenten, um auf deinen Dienst zuzugreifen, und überprüfe, ob er Anfragen erfolgreich authentifizieren und autorisieren kann.

<ConfigureThirdPartyAiAgent />

## Autorisierung in deinem Dienst einrichten \{#set-up-authorization-in-your-service}

Um Zugriffskontrolle in deinem Dienst zu ermöglichen, musst du Folgendes implementieren:
...
2. **Implementiere Zugriffskontroll-Logik**: Implementiere in deinem Dienst eine Logik, um das vom AI-Agenten erhaltene Zugangstoken zu validieren und zu prüfen, ob der Benutzer die erforderlichen Berechtigungen zum Zugriff auf die angeforderte Ressource hat.
3. **Antworte dem AI-Agenten**: Wenn das Zugangstoken gültig ist und der Benutzer die Berechtigung hat, gib die angeforderte Ressource zurück. Andernfalls gib eine 401 Nicht autorisiert-Antwort zurück.
...
4. Der AI-Agent sollte die 401 Nicht autorisiert-Antwort behandeln und den Benutzer zu Logto zur Authentifizierung weiterleiten.
5. Nach erfolgreicher Authentifizierung sollte der AI-Agent ein Zugangstoken erhalten und dieses für Anfragen an deinen Dienst verwenden.
6. Der AI-Agent sollte in der Lage sein, die Ressource mit dem Zugangstoken von deinem Dienst abzurufen.
