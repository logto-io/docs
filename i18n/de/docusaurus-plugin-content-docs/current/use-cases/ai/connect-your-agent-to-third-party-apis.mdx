...
sequenceDiagram
    ...
    rect rgba(200, 200, 230, 0.5)
        Note over User, Google: KI-Agent-Aufgabenausführung
        Agent->>Logto: Google-Zugangstoken abrufen
        Logto->>Agent: Zugangstoken zurückgeben
        Agent->>Google: Google Kalender API aufrufen
        Google->>Agent: Erfolg zurückgeben
        Agent->>User: "Erledigt! Ich habe das Meeting zu deinem Kalender hinzugefügt."
    end
```
...
2. **Autorisierungsaufforderung**: Der Agent erkennt den Bedarf an Drittanbieterzugriff und fordert den Benutzer zur Autorisierung auf.
...
4. In den Connector-Einstellungen:
   ...
   - Aktiviere **Offline-Zugriff**, um Auffrischungstokens für langlebigen API-Zugriff zu erhalten.
...
Bevor du fortfährst, aktiviere die Account API unter <CloudLink to="/sign-in-experience/account-center">Konsole > Anmeldung & Konto > Account Center</CloudLink>. Erfahre mehr über das [Aktivieren der Account API](/end-user-flows/account-settings/by-account-api#how-to-enable-account-api).
:::
...
async function handleAuthorizationCallback(
  ...
  // Tokens im Logto Secret Vault speichern
