...
- Du **SOLLTES** die spezifischste URI angeben, die es für die vollständige API oder den Satz von Ressourcen, auf die zugegriffen werden soll, geben kann.
...
| Standard-API                                                                                                                                                                                                                             | Pro Mandant kann nur eine Standard-API festgelegt werden.                                                                              |
...
- Logto wird die Standard-API-Ressource verwenden, wenn kein `Ressourcen`-Parameter in der [Authentifizierungsanfrage](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest) vorhanden ist.
- Wenn die `openid`-Berechtigung enthalten ist, wird ein Opakes Zugangstoken für den [Userinfo-Endpunkt](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo) ausgegeben, wenn kein `Ressourcen`-Parameter in nachfolgenden [Token-Anfragen](https://openid.net/specs/openid-connect-core-1_0.html#TokenRequest) vorhanden ist.
- Wenn die `openid`-Berechtigung nicht enthalten ist, wird ein JWT-Zugangstoken für die Standard-API-Ressource ausgegeben, wenn kein `Ressourcen`-Parameter in nachfolgenden [Token-Anfragen](https://openid.net/specs/openid-connect-core-1_0.html#TokenRequest) vorhanden ist.
...
Um ein zielgruppenbeschränktes `Zugangstoken` von Logto zu erhalten, registriere zuerst deine Backend-APIs, um Logtos zielgruppenbeschränkte Zugangstoken zu aktivieren. Wenn eine Autorisierungsanfrage eingeht, wird Logto die registrierten API-Ressourcen identifizieren und den Zugang entsprechend gewähren.

Um deine APIs zu registrieren, navigiere zu <CloudLink to="/api-resources">Konsole > API-Ressourcen</CloudLink>. Du wirst eine eingebaute Ressource mit dem API-Identifier angezeigt sehen, der als `https://[your-tenant-id].logto.app/api` angezeigt wird. Diese Ressource umfasst alle Management-APIs von Logto und stellt sicher, dass sie geschützt und nur für autorisierte Benutzer von Logto zugänglich sind.
