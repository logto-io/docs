...
Implementiere gemäß deinem Berechtigungsmodell die passende Überprüfungslogik in `jwt_validator.py`:
...
def verify_payload(payload: Dict[str, Any]) -> None:
    ...
    # Überprüfe, ob die Organisations-ID mit dem Kontext übereinstimmt (möglicherweise musst du dies aus dem Anfragekontext extrahieren)
