...
Ein **Organisationstemplate** ist ein Zugangskontroll-Design für Multi-Tenant-Apps. Es baut auf den Grundlagen der [RBAC (Rollenbasierte Zugangskontrolle)](/authorization/role-based-access-control) auf, ist jedoch für Multi-Tenant-Umgebungen angepasst, in denen Organisationen Unternehmen, Gruppen oder Benutzer repräsentieren.
...
Eine Organisationsrolle ist eine Sammlung von [Organisationsberechtigungen](#organization-permission) oder [API-Berechtigungen](/authorization/role-based-access-control/configure-permissions) (API-Berechtigungen, die direkt in <CloudLink to="/api-resources">API-Ressourcen in Logto Console</CloudLink> definiert sind), die Benutzern zugewiesen werden können.
...
Ja, du kannst [API-Berechtigungen](/authorization/role-based-access-control#permissionsscopes) Organisationsrollen zuweisen, nicht nur [Organisationsberechtigungen](/authorization/organization-template#organization-permission).
...
Organisationstemplate bezieht sich auf eine Sammlung von Organisationsberechtigungen und Rollen, die für jede [Organisation](/organizations) gelten. Es wird als organisationsbezogene rollenbasierte Zugangskontrolle betrachtet.
...
Du kannst sowohl das [Organisationstemplate](/authorization/organization-template) als auch das [Benutzer-/System-RBAC](/authorization/role-based-access-control/) in Logto verwenden, um einen robusteren Ansatz zu verfolgen, der deinen spezifischen Geschäfts- und Produktanforderungen entspricht.
