...
- **`salt`**: Der beim ursprünglichen Hashing verwendete Salt-Wert
...
1.  **Benutzerdaten vorbereiten**  
    Du solltest zuerst die Benutzerdaten aus deiner bestehenden Plattform exportieren und dann die Benutzerinformationen dem Logto-Benutzerschema zuordnen. Wir empfehlen, die zugeordneten Daten im JSON-Format vorzubereiten. Hier ist ein Beispiel für Benutzerdaten:
...
4.  **Benutzerdaten importieren**  
    Es wird empfohlen, ein Skript vorzubereiten, um die Benutzerdaten einzeln zu importieren. Wir rufen die [create user](https://openapi.logto.io/operation/operation-createuser) API auf, um die Benutzerdaten zu importieren. Hier ist ein Beispiel für ein Skript:
