...
Verwende `GET /api/users`, um Benutzer zu suchen. Beachte, dass es sich um eine Management API handelt, die wie andere Authentifizierung erfordert. Siehe [Interaktion mit der Management API](/integrate-logto/interact-with-management-api) für das Interaktionsrezept.
...
Lass uns einige Beispiele durchgehen. Alle Suchparameter werden als Konstruktor von `URLSearchParams` formatiert.
...
Denke daran, dass verschachtelte Felder nicht unterstützt werden, z.B. `search.name.first` wird zu einem Fehler führen.
...
PostgreSQL unterstützt zwei Arten von regulären Ausdrücken, [similar to](https://www.postgresql.org/docs/current/functions-matching.html#FUNCTIONS-SIMILARTO-REGEXP) und [posix](https://www.postgresql.org/docs/current/functions-matching.html#FUNCTIONS-POSIX-REGEXP). Setze `mode` auf `similar_to` oder `posix`, um nach regulären Ausdrücken zu suchen:
