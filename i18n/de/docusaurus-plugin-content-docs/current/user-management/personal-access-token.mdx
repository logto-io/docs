...
- **Autorisierungsflow**: Du kannst PAT-ausgetauschte Zugangstokens für [Organisationsberechtigungen](/authorization/organization-permissions) und [organisationsbezogene API-Ressourcen](/authorization/organization-level-api-resources) verwenden
...
Wenn die Token-Austauschanfrage erfolgreich ist, gibt der Token-Endpunkt des Tenants einen Zugangstoken zurück, der die Identität des Benutzers repräsentiert. Die Antwort enthält die folgenden Parameter im HTTP-Response-Entity-Body im Format `application/json`.
