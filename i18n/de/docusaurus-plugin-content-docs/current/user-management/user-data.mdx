...
- [Basisdaten](/user-management/user-data#basic-data): sind die Basisinformationen aus dem Benutzerprofil. Sie speichern alle anderen _Benutzer_-Eigenschaften außer den sozialen `identities` und `custom_data`, wie Benutzer-ID, Benutzername, E-Mail, Telefonnummer und wann sich der Benutzer zuletzt angemeldet hat.
- [Soziale Identitäten](/user-management/user-data#social-identities): speichert die Benutzerinformationen, die durch die soziale Anmeldung (d. h. Anmeldung mit einem sozialen Connector) abgerufen wurden, wie Facebook, GitHub und WeChat.
...
_id_ ist ein eindeutig automatisch generierter Schlüssel zur Identifizierung des Benutzers in Logto.

### username \{#username}

_username_ wird für die Anmeldung mit _Benutzername_ und Passwort verwendet.

Der Wert stammt vom Benutzernamen, mit dem sich der Benutzer erstmals registriert hat. Er kann `null` sein. Ein nicht-null Wert darf maximal 128 Zeichen lang sein, nur Buchstaben, Zahlen und Unterstriche (`_`) enthalten und NICHT mit einer Zahl beginnen. Standardmäßig wird die Groß- und Kleinschreibung beachtet. Diese Regeln können pro Mandant mit der [Benutzernamenrichtlinie](/end-user-flows/sign-up-and-sign-in/username-policy) weiter eingeschränkt werden.
...
Wenn sich der Benutzer mit einem sozialen Connector wie Google oder Facebook registriert, kann der Wert aus den sozialen Benutzerinformationen abgerufen werden.
...
Die Typdefinition findest du in [dieser Datei](https://github.com/logto-io/logto/blob/HEAD/packages/schemas/src/foundations/jsonb-types/users.ts#L6). Hier ist eine Kopie der Typdefinition:
...
Ein Unterschied zu den anderen Standardansprüchen besteht darin, dass die Eigenschaften in `profile` nur dann im [ID-Token](https://auth.wiki/id-token) oder in der Antwort des userinfo-Endpunkts enthalten sind, wenn ihre Werte nicht leer sind, während andere Standardansprüche `null` zurückgeben, wenn die Werte leer sind.

Eine Ausnahme ist der Anspruch `preferred_username`: Wenn `preferredUsername` in `profile` leer ist, wird auf den [Benutzernamen](/user-management/user-data#username) des Benutzers zurückgegriffen, sodass standardkonforme Clients sofort einen verwendbaren Wert erhalten. Der explizite Wert von `preferredUsername` hat immer Vorrang, wenn er gesetzt ist.
...
_has_password_ ist ein boolescher Wert, der angibt, ob der Benutzer ein Passwort hat. Du kannst diesen Status anzeigen und verwalten, einschließlich des Setzens eines neuen oder Zurücksetzens des Passworts auf der Detailseite von <CloudLink to="/users">Console > Benutzerverwaltung</CloudLink>.
...
_password_encryption_method_ wird verwendet, um das Passwort des Benutzers zu verschlüsseln. Der Wert wird beim Registrieren mit Benutzername und Passwort initialisiert. Er kann `null` sein.
...
_identities_ enthält die Benutzerinformationen, die durch [soziale Anmeldung](/end-user-flows/sign-up-and-sign-in/social-sign-in) (d. h. Anmeldung mit einem [sozialen Connector](/connectors/social-connectors)) abgerufen wurden. Die _identities_ jedes Benutzers werden in einem eigenen JSON-Objekt gespeichert.

Die Benutzerinformationen variieren je nach sozialem Identitätsanbieter (d. h. sozialem Netzwerk), und sie umfassen typischerweise Folgendes:
...
Das Benutzerkonto kann über die soziale Anmeldung mit mehreren sozialen Identitätsanbietern verknüpft sein; die entsprechenden Benutzerinformationen, die von diesen Anbietern abgerufen werden, werden im _identities_-Objekt gespeichert.
...
_sso_identities_ enthält die Benutzerinformationen, die durch [Enterprise SSO](/end-user-flows/enterprise-sso) (d. h. Single Sign-On-Anmeldung mit einem Enterprise-Connector](/connectors/enterprise-connectors)) abgerufen wurden. Die _ssoIdentities_ jedes Benutzers werden in einem eigenen JSON-Objekt gespeichert.
...
Wenn du dennoch sensible Informationen in _custom_data_ ablegen möchtest, empfehlen wir, diese vorher zu verschlüsseln. Verschlüssele/entschlüssele sie nur in einer vertrauenswürdigen Partei wie deinen Backend-Diensten und vermeide dies in den Frontend-Apps. Dadurch wird der Schaden minimiert, falls die _custom_data_ deiner Benutzer versehentlich offengelegt wird.
...
- Verwende die [Management API](/user-management/manage-users/#manage-via-logto-management-api) für Benutzerverwaltung oder fortgeschrittene benutzerdefinierte Abläufe:
...
dann sieht der neue _custom_data_-Wert nach dem Aktualisieren so aus:
...
| [identities](/user-management/user-data#social-identities)                          | object    | Benutzerinfo aus sozialer Anmeldung                             | ❌        | ✅           |
| [custom_data](/user-management/user-data#custom-data)                               | object    | Zusätzliche Infos in anpassbaren Eigenschaften                  | ❌        | ✅           |
...
<Url href="https://blog.logto.io/secure-hub-for-user-data/">
  Sicherer Hub für Benutzerdaten unterwegs
</Url>
