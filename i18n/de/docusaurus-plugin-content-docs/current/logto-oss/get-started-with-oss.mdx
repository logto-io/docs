...
Nach erfolgreichem Aufbau siehst du eine Nachricht wie:
...
DATABASE_STATEMENT_TIMEOUT: DISABLE_TIMEOUT # (v1.36.0+) Deaktiviert statement_timeout für PgBouncer/RDS Proxy
```

Starte den Container mit den oben genannten erforderlichen Umgebungsvariablen (optionale sind unten weggelassen; füge sie bei Bedarf hinzu):
...
- Wenn du Docker Hub verwendest, nutze `svhd/logto:latest` statt `ghcr.io/logto-io/logto:latest`.
- Verwende `host.docker.internal` oder `172.17.0.1` in `DB_URL`, um auf die Host-IP zu verweisen.
- Wenn du PgBouncer oder RDS Proxy verwendest und `unsupported startup parameter: statement_timeout` siehst, setze `DATABASE_STATEMENT_TIMEOUT=DISABLE_TIMEOUT` (v1.36.0+).
...
Logto verwendet Umgebungsvariablen für die Konfiguration und unterstützt zusätzlich `.env`-Dateien. Siehe [Konfiguration](/concepts/core-service/configuration) für eine detaillierte Nutzung und die vollständige Variablenliste.

</details>

_Schau dir [Core Service](/concepts/core-service) an, wenn du erweiterte Steuerungsmöglichkeiten oder programmatischen Zugriff auf Logto möchtest._

## Hosting-Anbieter \{#hosting-providers}

Diese zuverlässigen Hosting-Anbieter bieten One-Click-Installationsvorlagen für Logto. Mit einfach bereitzustellenden Diensten kannst du dein CIAM-System mit Logto in Sekundenschnelle einrichten und starten.
...
Sobald du Logto erfolgreich auf deinem Server gehostet hast, klicke auf der Willkommensseite auf „Konto erstellen“. Beachte, dass die Open-Source-Version von Logto nur die Erstellung eines Kontos beim ersten Start erlaubt und keine Mehrfachkonten unterstützt. Der Kontoerstellungsprozess ist auf die Kombination aus Benutzername und Passwort beschränkt.

:::tip
Logto OSS (self-hosted) unterstützt nicht die Konfiguration mehrerer Administratoren. Für Teamarbeit und Projekte, die mehrere Admin-Benutzer erfordern, empfehlen wir [Logto Cloud](https://cloud.logto.io), das vollständige Teamverwaltungsfunktionen bietet.
:::
