...
- Die Registrierungsseite ist deaktiviert, und Benutzer, die sie besuchen, werden zur Anmeldeseite weitergeleitet.
- Der Social- und Enterprise-SSO-Registrierungsablauf ist ebenfalls deaktiviert. Benutzer mit nicht registrierten Social-Accounts erhalten nach der Social-Authentifizierung einen „Konto nicht gefunden“-Fehler.
- Die Funktion zum Verknüpfen von Social-Accounts ist weiterhin für Benutzer verfügbar, die eine passende E-Mail-Adresse oder Telefonnummer im System haben.

## Einen Anmeldeablauf nur auf Einladung implementieren \{#implement-an-invitation-only-sign-up-flow}
...
1. Deaktiviere die Benutzerregistrierung in der <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Konsole > Anmeldungserlebnis</CloudLink>.
2. Gehe zu <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Konsole > Anmeldungserlebnis</CloudLink> und schalte „Benutzerregistrierung aktivieren“ aus, um die öffentliche Registrierung zu schließen.
3. Sammle die E-Mail-Adressen der Benutzer, die du einladen möchtest (z. B. über deine Website oder Empfehlungen von bestehenden Benutzern).
4. Erstelle und sende den **magischen Einladungslink** [gemäß Anleitung](/end-user-flows/one-time-token#implementation-guide) (Fordere das Einmal-Token an, erstelle den Magic Link, löse die Authentifizierung über das Logto SDK aus).<br/>
   **Hinweis**: Setze eine Ablaufzeit für den Einladungslink. Es wird empfohlen, den Link mindestens einen Tag gültig zu machen. Verwende folgenden Request-Body, um das Einmal-Token zu generieren:
   ...
   ```json
   {
     ...
     "expiresIn": 172800 // Optional. Standardwert ist 600 (10 Minuten)
   }
   ```
...
5. Sende den Magic Link an die E-Mail des Benutzers (z. B. `https://yourapp.com/landing-page?type=registrationInvitation&token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`). Passe die E-Mail-Vorlage an, zum Beispiel:
   ...
   Wenn Benutzer auf „Einladung annehmen“ klicken, registrieren sie sich automatisch für deinen Dienst, selbst wenn die öffentliche Registrierung deaktiviert ist. Dies wird als „Gezielte Benutzereinladung“ bezeichnet.

### Option 2: Benutzer über Management API einladen und erstellen \{#option-2-invite-and-create-users-via-management-api}

1. Deaktiviere die Benutzerregistrierung in der <CloudLink to="/sign-in-experience/sign-up-and-sign-in">Konsole > Anmeldungserlebnis</CloudLink>.
2. Erstelle ein benutzerdefiniertes, einladungsbasiertes Registrierungsformular in deiner Anwendung.
3. Sende dem Benutzer eine Einladung per E-Mail mit einem eindeutigen Einladungslink.
...
Wenn die Benutzerregistrierung deaktiviert ist, werden Benutzer, die die Registrierungsseite besuchen, zur Anmeldeseite weitergeleitet. Wenn ein Benutzerkonto von einem Administrator oder über die Management API (wie in Option 2 oder 3) erstellt wurde, kann das Anmeldeerlebnis des Benutzers je nach den von dir konfigurierten Anmelde- und Registrierungsverfahren variieren:

- **Anmeldung mit E-Mail oder Telefonnummer per Bestätigungscode**: Erstelle das Benutzerkonto mit einer **E-Mail-Adresse** oder **Telefonnummer**. Aktiviere die Anmeldungsmethode per E-Mail- oder Telefonnummer-Bestätigungscode. Sobald der Benutzer erstellt ist, kann er sich direkt mit seiner E-Mail oder Telefonnummer über die Bestätigungscode-Authentifizierungsmethode anmelden.
- **Soziale Anmeldung mit bestimmter E-Mail oder Telefonnummer**: Erstelle das Benutzerkonto mit einer **E-Mail-Adresse** oder **Telefonnummer**, die mit dem Social-Account des Benutzers verknüpft ist. Erstelle und aktiviere die Social-Verbindung für den jeweiligen Social-Anbieter. Aktiviere die [**Automatische Kontoverknüpfung**](/end-user-flows/sign-up-and-sign-in/social-sign-in#account-linking), um die soziale Identität automatisch mit dem Benutzerkonto anhand der passenden E-Mail oder Telefonnummer zu verknüpfen. Sobald der Benutzer erstellt ist, kann er sich direkt mit seinem Social-Account anmelden, der mit der E-Mail oder Telefonnummer übereinstimmt.
