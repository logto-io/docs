...
1. **Konfiguriere deine sozialen Connectors**
...
2. **Soziale Login-Buttons hinzufügen**:
   Standardmäßig ist dein neu hinzugefügter sozialer Connector auf der Anmeldeseite für Endbenutzer nicht sichtbar. Um soziale Anmelde-Buttons zur Anmeldeseite hinzuzufügen, musst du den sozialen Connector in den Einstellungen der Anmeldeerfahrung aktivieren.
...
3. **Kontoverknüpfungsoptionen festlegen**:
   Für neue Benutzer, die sich mit sozialen Identitäten registrieren, ermöglicht Logto die Verknüpfung ihres sozialen Kontos mit einem bestehenden E-Mail- oder Telefonnummernkonto im Logto-System. Standardmäßig wird den Benutzern während der sozialen Registrierung eine zugehörige Kontoverknüpfungsseite angezeigt, die es ihnen ermöglicht, ihr soziales Konto mit einem bestehenden Konto zu verknüpfen oder ein neues zu erstellen.
...
4. **Änderungen speichern**: Überprüfe deine Änderungen sorgfältig und speichere sie, um die Konfiguration anzuwenden.
...
sequenceDiagram
    ...
    user->>experience: Klicke auf den sozialen Anmelde-Button
    ...
    user->>social: Soziale Anmeldedaten eingeben
    social-->>experience: Benutzer authentifizieren und zurück zur Logto-Anmeldeseite weiterleiten
    experience->>Logto: Soziale Authentifizierungsdaten und Benutzeridentität validieren
    Logto->>app: Benutzer authentifizieren und zurück zur Anwendung weiterleiten
```

1. **Anmeldung mit sozialem Konto**: Der Benutzer klickt auf den sozialen Anmelde-Button, der auf der Anmeldeseite angezeigt wird.
...
Logto unterstützt auch die Google One-tap-Anmeldemethode für den Google-Connector, die es Benutzern ermöglicht, sich mit einem einzigen Klick anzumelden. Diese Funktion vereinfacht den Anmeldeprozess weiter, indem sie die Notwendigkeit beseitigt, dass Benutzer zur Google-Authentifizierungsseite weitergeleitet werden.

Um die Google One-tap-Anmeldung zu aktivieren, folge den Anweisungen in den Google-Connector-Einstellungen. Sobald aktiviert, sehen Benutzer ein "Mit Google anmelden"-Popup, wenn sie auf der Anmeldeseite landen. Wenn sie darauf klicken, werden sie automatisch mit ihrem Google-Konto authentifiziert und zurück zur Anwendung geleitet.
...
- **Neues Konto erstellen**:
  Wenn sich ein Benutzer mit einer sozialen Identität anmeldet, die im Logto-System nicht existiert, und die angegebene E-Mail-Adresse oder Telefonnummer keinem bestehenden Benutzerkonto entspricht, wird direkt ein neues Konto in Logto erstellt.
- **Bestehende Kontoverknüpfung**:
  ...
  - **Automatische Kontoverknüpfung:** Wenn die Option „Automatische Kontoverknüpfung“ in deinen Anmeldeerfahrungseinstellungen aktiviert ist, wird Logto das soziale Konto automatisch mit dem bestehenden Konto basierend auf einer übereinstimmenden E-Mail-Adresse oder Telefonnummer verknüpfen. Benutzer werden nicht aufgefordert, die Konten zu verknüpfen, und werden sofort in ihr bestehendes Konto eingeloggt. Das soziale Konto wird verknüpft, sodass der Benutzer sich in Zukunft mit beiden Methoden anmelden kann.
  - **Manuelle Kontoverknüpfung**: Wenn die Option „Automatische Kontoverknüpfung“ deaktiviert ist, werden Benutzer während des Anmeldeprozesses aufgefordert, ihr soziales Konto mit dem bestehenden Konto zu verknüpfen. Sie können wählen, ob sie die Konten verknüpfen oder ein neues erstellen möchten.
    ...
    ![soziale Kontoverknüpfung](./assets/sign-in-social-linking.png)

    ```mermaid
    flowchart TD
    A[Mit sozialer Identität authentifizieren] --> B{{Soziale Identität existiert in Logto?}}
...
Wenn die E-Mail-Adresse oder Telefonnummer ein erforderlicher Registrierungsidentifikator ist, wird der Benutzer während des Registrierungsprozesses aufgefordert, eine andere E-Mail-Adresse oder Telefonnummer anzugeben. Siehe [Zusätzliche Benutzerprofilinformationen sammeln](#zusätzliche-benutzerprofilinformationen-sammeln) für weitere Details.
:::

## Zusätzliche Benutzerprofilinformationen sammeln

Während des sozialen Registrierungsprozesses, abhängig von den konfigurierten Einstellungen für den obligatorischen Registrierungsidentifikator (**E-Mail-Adresse**, **Telefonnummer** und **Benutzername**), können Benutzer aufgefordert werden, zusätzliche verifizierte Informationen bereitzustellen, um den Registrierungs- oder Anmeldeprozess nach der Authentifizierung beim sozialen Anbieter abzuschließen.
...
2. Registrierung mit sozialer Identität, die keine verifizierte E-Mail-Adresse bereitstellt.
   ...
   ```mermaid
   flowchart TD
      A[Mit sozialer Identität authentifizieren] --> B{{E-Mail-Adresse erforderlich und fehlt?}}
...
3. Registrierung mit sozialer Identität, die eine registrierte E-Mail-Adresse bereitstellt
   ...
   ```mermaid
      flowchart TD
       A[Mit sozialer Identität authentifizieren] --> B{{E-Mail-Adresse stimmt mit bestehendem Konto überein?}}
...
<details>
  <summary> Warum werden meine E-Mails oder Telefonnummern nach der Registrierung mit sozialen Konten nicht ausgefüllt?</summary>
