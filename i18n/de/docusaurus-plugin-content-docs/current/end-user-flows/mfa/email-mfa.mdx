...
1. **Einschränkung der Anmeldemethode**: E-Mail-Verifizierungscodes können nicht gleichzeitig als [Anmeldemethode (1FA)](/end-user-flows/sign-up-and-sign-in/sign-in) und als MFA-Faktor (2FA) verwendet werden. Wähle einen Authentifizierungsablauf pro E-Mail-Implementierung.
...
Die MFA-Einrichtungsaufforderung kann während der Benutzerregistrierung oder nach der Anmeldung erscheinen, abhängig von deiner konfigurierten [MFA-Richtlinie](/end-user-flows/mfa/configure-mfa#global-mfa-configuration). Benutzer können E-Mail-MFA auch über ihre [Kontoeinstellungen](/end-user-flows/account-settings/by-account-api#update-or-link-new-email) aktivieren.
...
Sind mehrere MFA-Faktoren verfügbar, können Benutzer aus ihren konfigurierten Faktoren wählen. Das System bestimmt, welcher MFA-Faktor zuerst abgefragt wird, basierend auf der Prioritätsreihenfolge in [MFA konfigurieren](/end-user-flows/mfa/configure-mfa#mfa-verification-flow).
