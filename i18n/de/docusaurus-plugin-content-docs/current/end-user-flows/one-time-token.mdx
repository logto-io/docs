...
Manchmal möchtest du neue Benutzer zu deiner Anwendung / Organisation einladen, ohne dass sie zuerst ein Konto erstellen müssen. In solchen Fällen kann die Anwendung einen "Magic Link" an deine E-Mail senden. Du wirst sofort authentifiziert, wenn du auf den Link klickst.
...
- **Einladung von Organisationsmitgliedern**: Für SaaS-Produkte kannst du Magic Links verwenden, um neue Mitglieder zur Teilnahme an einer Organisation einzuladen und so den Onboarding-Prozess zu vereinfachen.
- **Anmeldung / Registrierung**: Sende einen Magic Link für die passwortlose Anmeldung oder Registrierung per E-Mail.

Wenn du beispielsweise die öffentliche Registrierung deaktiviert hast, kannst du einen Magic Link mit einem Einmal-Token (z. B. `https://yourapp.com/landing-page?token=YHwbXSXxQfL02IoxFqr1hGvkB13uTqcd&email=user@example.com`) an die E-Mail des Benutzers senden, um ihn zur Kontenerstellung einzuladen. Du kannst die E-Mail-Vorlage in deinem eigenen E-Mail-Zustelldienst anpassen, zum Beispiel:
...
- Passwortzurücksetzung mit Magic Link.
...
sequenceDiagram
  ...
  C ->> L: Fordere Einmal-Token an
  L -->> C: Gibt Einmal-Token zurück
  C -->> U: Erstelle einen Magic Link, der das Einmal-Token enthält, und sende ihn an die E-Mail des Endbenutzers
...
Der Magic Link Authentifizierungsablauf unterstützt das Zuweisen von Rollen an Benutzer nicht. Du kannst jedoch jederzeit die [Webhooks](/developers/webhooks) und [Management API](/user-management/manage-users#manage-roles-of-users) verwenden, um die Benutzerrollen nach der Registrierung zu aktualisieren.
...
### Kann ich weiterhin Magic Links verwenden, um Benutzer einzuladen, wenn ich die Benutzerregistrierung in der "Anmeldeerfahrung" deaktiviere? \{#if-i-disable-user-registration-in-sign-in-experience-can-i-still-use-magic-link-to-invite-users}

</summary>

Ja, du kannst weiterhin Magic Links verwenden, um Benutzer einzuladen, auch wenn du die Benutzerregistrierung in der "Anmeldeerfahrung" deaktivierst.
...
1. Der Benutzer ist bereits angemeldet und klickt dann auf einen Magic Link, der mit dem aktuellen Benutzerkonto verknüpft ist. In diesem Fall überprüft Logto dennoch das Einmal-Token und ordnet den Benutzer bei Bedarf den angegebenen Organisationen zu.
...
3. Wenn dein Anmelde-Prompt auf "login" gesetzt ist oder "login" enthält, meldet Logto das mit dem Einmal-Token verknüpfte Konto automatisch an, ohne einen Wechsel anzuzeigen. Dies liegt daran, dass der "login"-Prompt eine explizite Authentifizierungsabsicht signalisiert, die Vorrang vor der aktuellen Sitzung hat.
