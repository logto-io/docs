...
## Vorteile der Verwendung der vorgefertigten UI \{#benefits-of-using-the-prebuilt-ui}
...
Wenn dein Tenant-Endpunkt beispielsweise `https://example.logto.app` ist, wäre die Seite zum Aktualisieren der E-Mail unter `https://example.logto.app/account/email` erreichbar.
...
1. Stelle sicher, dass die Berechtigung für die Felder **Name**, **Avatar**, **Profil** oder **Benutzerdefinierte Daten** in den Bereichen Account-Sicherheit / Benutzerprofil auf `Edit` oder `ReadOnly` gesetzt ist.
2. Füge im Bereich **Vorgefertigte UI integrieren** die Profilfelder hinzu, die du aus dem Profilfeld-Selektor bereitstellen möchtest. Diese Felder stammen aus demselben Katalog wie bei [Benutzerprofil erfassen](/end-user-flows/collect-user-profile) — jedes dort definierte Feld kann auch im Account Center angezeigt werden.
...
Die vorgefertigte Account Center UI stellt stabile CSS-Klassennamen bereit, die mit `logto_ac-` bei wichtigen UI-Elementen (Layout-Container, Seitenüberschriften, Abschnitte, Karten usw.) beginnen, um ein einfaches Targeting zu ermöglichen. So kannst du Standardstile überschreiben, ohne dich um Klassennamenänderungen zwischen Releases sorgen zu müssen.
...
Wenn du mehr Anpassungen benötigst, als die vorgefertigte UI und benutzerdefiniertes CSS bieten, erwäge die Verwendung der [Account API](/end-user-flows/account-settings/by-account-api), um eigene Seiten zur Kontoverwaltung zu erstellen.
...
- [MFA vertrauenswürdige Geräte](/end-user-flows/mfa/trusted-devices) – Zeitlich begrenztes Browser-Trust konfigurieren und verwalten
