...
8. Die serverseitige Anwendung prüft die Identität und Berechtigung des Anfragenden anhand des Benutzer-Zugangstokens. Anschließend fordert sie ein Management API Zugangstoken von Logto an.
...
Siehe den Abschnitt [Management API](/integrate-logto/interact-with-management-api/), um zu erfahren, wie du die Management APIs in serverseitige Anwendungen integrierst.
...
- Aus demselben Grund kann Logto den `state`-Parameter in der Autorisierungsantwort nicht prüfen. Stelle sicher, dass du den `state`-Parameter in deiner Client-App speicherst und ihn prüfst, wenn die Autorisierungsantwort empfangen wird.
...
| POST    | [/api/users/\{userId\}/mfa-verifications](https://openapi.logto.io/operation/operation-createusermfaverification)                    | MFA-Verifizierung für den Benutzer nach Benutzer-ID einrichten. |
