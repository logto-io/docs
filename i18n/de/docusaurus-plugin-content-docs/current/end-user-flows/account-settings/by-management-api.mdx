...
3. **Serverseitige Anwendung**: Serverseitige Anwendung, die die Kontoeinstellungen-API für den Client bereitstellt. Kommuniziert mit der Logto Management API.
...
Sieh dir den Abschnitt [Management API](/integrate-logto/interact-with-management-api/) an, um zu erfahren, wie du die Management APIs in serverseitige Anwendungen integrierst.
...
| POST   | [/api/connectors/\{connectorId\}/authorization-uri](https://openapi.logto.io/operation/operation-createconnectorauthorizationuri) | Die Autorisierungs-URI für einen sozialen Identitätsanbieter abrufen. Nutze diese URI, um eine neue Verbindung zu initiieren. |
...
sequenceDiagram
    ...
    Client ->> Server: Anfrage zur Verknüpfung sozialer Identität, Weiterleitung der IdP-Antwort
...
11. Logto holt die Benutzerinformationen vom IdP mit dem Autorisierungscode.
...
### Persönlicher Zugangstoken \{#personal-access-token}

| method | path                                                                                                                                 | description                                                        |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ |
| GET    | [/api/users/\{userId\}/personal-access-tokens](https://openapi.logto.io/operation/operation-listuserpersonalaccesstokens)            | Alle persönlichen Zugangstokens des Benutzers abrufen.             |
| POST   | [/api/users/\{userId\}/personal-access-tokens](https://openapi.logto.io/operation/operation-createuserpersonalaccesstoken)           | Einen neuen persönlichen Zugangstoken für den Benutzer hinzufügen. |
...
Persönliche Zugangstokens bieten eine sichere Möglichkeit für Benutzer, [Zugangstoken (Access token)](https://auth.wiki/access-token) zu gewähren, ohne ihre Zugangsdaten und interaktive Anmeldung zu verwenden. Erfahre mehr über [die Verwendung persönlicher Zugangstokens](/user-management/personal-access-token).
...
| POST   | [/api/users/\{userId\}/mfa-verifications](https://openapi.logto.io/operation/operation-createusermfaverification)                    | Eine MFA-Verifizierung für den Benutzer einrichten.       |
