...
- Vom Benutzer autorisierte Apps (Grants) verwalten
...
SSO-Kontoansicht und Kontolöschungsfunktionen sind derzeit über die Logto Management APIs verfügbar. Siehe [Kontoeinstellungen über Management API](/end-user-flows/account-settings/by-management-api) für Implementierungsdetails.

:::

## Wie aktiviere ich die Account API \{#how-to-enable-account-api}

Navigiere zu <CloudLink to="/sign-in-experience/account-center">Konsole > Anmeldung & Konto > Account Center</CloudLink>.
...
Nach der Aktivierung kannst du feldweise Berechtigungen für Identifikatoren, Profildaten und Drittanbieter-Token-Zugriff konfigurieren. Jedes Feld unterstützt `Off`, `ReadOnly` oder `Edit`; Standard ist `Off`.
...
1. **Sicherheitsfelder**:
   ...
   - Um WebAuthn-Passkeys für MFA zu verwenden, füge die Domains deiner Frontend-App zu **WebAuthn Related Origins** hinzu, damit Account Center und Anmeldungserlebnis Passkeys teilen können. Siehe [Neuen WebAuthn-Passkey verknüpfen](#link-a-new-webauthn-passkey).
2. **Profilfelder**:
   - Felder umfassen: Benutzername, Name, Avatar, [Profil](/user-management/user-data#profile) (weitere Standard-Profilattribute) und [benutzerdefinierte Daten](/user-management/user-data#custom-data).
   - Endbenutzer können diese ohne zusätzliche Verifizierung bearbeiten.
3. **Secret Vault**:
   - Für OIDC- oder OAuth-Sozial- und Enterprise-Connectors speichert der Logto [Secret Vault](/secret-vault/federated-token-set) Drittanbieter-Zugangs- und Auffrischungstokens sicher nach der Authentifizierung. Apps können dann externe APIs aufrufen, z. B. Google-Kalender-Ereignisse synchronisieren, ohne dass Benutzer sich erneut anmelden müssen. Die Token-Abfrage ist automatisch verfügbar, sobald die Account API aktiviert ist.
4. **Sitzungsverwaltung**:
   - Wenn aktiviert, können Benutzer ihre aktiven Sitzungen einsehen und verwalten, einschließlich Geräteinformationen und letzter Anmeldezeit. Benutzer können auch Sitzungen widerrufen, um sich von bestimmten Geräten abzumelden.
   - Diese gleiche Berechtigung für das Feld `Sessions` steuert auch APIs für vom Benutzer autorisierte Apps (Grants) (Anzeigen und Widerrufen von Grants).
   - Bevor Endbenutzer auf die Sitzungsverwaltung zugreifen, müssen sie ihre Identität per Passwort, E-Mail oder SMS verifizieren, um eine 10-minütige Verifizierungs-ID zu erhalten. Siehe [Verifizierungs-ID erhalten](#get-a-verification-record-id).

## Wie greife ich auf die Account API zu \{#how-to-access-account-api}

:::note
Um sicherzustellen, dass das Zugangstoken die entsprechenden Berechtigungen hat, stelle sicher, dass du die entsprechenden Berechtigungen (Scopes) in deiner Logto-Konfiguration korrekt eingerichtet hast.
...
### Zugriff auf Account API mit Zugangstoken \{#access-account-api-using-access-token}
...
Um weitere Profilinformationen zu aktualisieren, einschließlich **familyName, givenName, middleName, nickname, profile (Profilseiten-URL), website, gender, birthdate, zoneinfo, locale und address**, kannst du den [`PATCH /api/my-account/profile`](https://openapi.logto.io/operation/operation-updateotherprofile) Endpunkt verwenden.
...
Zuerst musst du eine **Verifizierungs-ID** mit einer Gültigkeit von 10 Minuten (TTL) erhalten. Diese kann verwendet werden, um die Identität des Benutzers zu verifizieren, bevor sensible Informationen aktualisiert werden. Das bedeutet: Sobald ein Benutzer seine Identität erfolgreich per Passwort, E-Mail-Verifizierungscode oder SMS-Verifizierungscode bestätigt hat, hat er 10 Minuten Zeit, um seine authentifizierungsbezogenen Daten zu aktualisieren, einschließlich Identifikatoren, Zugangsdaten, Social Account Linking und MFA.

Um eine Verifizierungs-ID zu erhalten, kannst du [das Passwort des Benutzers verifizieren](#verify-the-users-password) oder [einen Verifizierungscode an die E-Mail oder das Telefon des Benutzers senden](#verify-by-sending-a-verification-code-to-the-users-email-or-phone).
...
curl -X POST https://[tenant-id].logto.app/api/my-account/primary-email \
  ...
  -H 'content-type: application/json' \
  # Die "newIdentifierVerificationRecordId" bestätigt den Besitz der neuen E-Mail (aus dem oben beschriebenen Verifizierungsablauf)
...
Nachdem der Benutzer die Anwendung autorisiert hat, erhältst du einen Callback an die `redirectUri` mit dem `state`-Parameter. Dann kannst du den [`POST /api/verifications/social/verify`](https://openapi.logto.io/operation/operation-verifyverificationbysocial) Endpunkt verwenden, um die soziale Verbindung zu verifizieren.
...
curl -X POST https://[tenant-id].logto.app/api/my-account/identities \
  ...
  -H 'logto-verification-id: <verification_record_id_from_existing_identifier>' \
  -H 'content-type: application/json' \
  # Die "newIdentifierVerificationRecordId" identifiziert die zu verknüpfende soziale Verbindung (aus dem oben beschriebenen Social-Verifizierungsablauf)
...
WebAuthn-Passkeys sind an einen bestimmten Hostnamen gebunden, den sogenannten **Relying Party ID (RP ID)**. Nur Anwendungen, die auf dem Origin der RP ID gehostet werden, können mit diesen Passkeys registrieren oder authentifizieren.
...
- **Standard-Setup**: Wenn du nur die Standard-Domain von Logto `https://[tenant-id].logto.app` verwendest, ist die RP ID `[tenant-id].logto.app`
...
Verwende den [`PATCH /api/account-center`](https://openapi.logto.io/operation/operation-updateaccountcentersettings) Endpunkt, um den Origin deiner Frontend-Anwendung hinzuzufügen. Wenn das Account Center deiner App z. B. unter `https://account.example.com` läuft:
...
WebAuthn unterstützt bis zu 5 eindeutige eTLD+1-Labels für Related Origins. Die eTLD+1 (effective top-level domain plus one label) ist der registrierbare Domain-Teil. Zum Beispiel:
...
- `verification_record_id`: Eine gültige Verifizierungs-ID, die durch Verifizierung des bestehenden Faktors des Benutzers erhalten wurde. Siehe [Verifizierungs-ID erhalten](#get-a-verification-record-id) für Details.
...
Nachdem der Benutzer das Secret zu seiner Authenticator-App hinzugefügt hat, muss er es verifizieren und an sein Konto binden. Verwende den [`POST /api/my-account/mfa-verifications`](https://openapi.logto.io/operation/operation-addmfaverification) Endpunkt, um den TOTP-Faktor zu binden.
...
### Bestehendes TOTP ersetzen \{#replace-an-existing-totp}

Wenn der Benutzer bereits einen TOTP-Faktor konfiguriert hat, kannst du ihn mit einem neuen ersetzen, indem du den [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-createorreplacetotpmfaverification) Endpunkt verwendest. Dies ist eine idempotente Operation, die den bestehenden TOTP entfernt und einen neuen bindet.

:::note
Um diese Methode zu verwenden, musst du das Feld `mfa` in den [Account Center-Einstellungen](#how-to-enable-account-api) aktivieren.
:::

**Schritt 1: Neues TOTP-Secret generieren**

Verwende den [`POST /api/my-account/mfa-verifications/totp-secret/generate`](https://openapi.logto.io/operation/operation-generatetotpsecret) Endpunkt, um ein neues TOTP-Secret zu generieren, und zeige es dem Benutzer wie unter [Neues TOTP verknüpfen](#link-a-new-totp) beschrieben an.

**Schritt 2: TOTP-Faktor ersetzen**
...
- `verification_record_id`: Eine gültige Verifizierungs-ID, die durch Verifizierung des bestehenden Faktors des Benutzers erhalten wurde. Siehe [Verifizierungs-ID erhalten](#get-a-verification-record-id) für Details.
- `secret`: Das in Schritt 1 generierte neue TOTP-Secret.
- `code`: Der aus dem neuen Secret generierte TOTP-Code zur Bestätigung der Bindung.
...
### Einstellungen für die 2-Schritt-Verifizierung verwalten \{#manage-2-step-verification-settings}

:::note
Um diese Methoden zu verwenden, musst du das Feld `mfa` in den [Account Center-Einstellungen](#how-to-enable-account-api) aktivieren.
:::

Endbenutzer können die 2-Schritt-Verifizierung für ihr Konto aktivieren oder deaktivieren. Ein häufiger Anwendungsfall ist, wenn ein Benutzer MFA-Faktoren (wie Passkeys) konfiguriert hat, diese aber nur für die Anmeldung und nicht als zusätzlichen Verifizierungsschritt verwenden möchte — das Deaktivieren der 2-Schritt-Verifizierung ermöglicht es, die MFA-Aufforderung bei der Anmeldung zu überspringen, während die MFA-Faktoren erhalten bleiben. Dies wird durch die Einstellung `skipMfaOnSignIn` pro Benutzer gesteuert.

**Aktuelle MFA-Einstellungen abrufen**

Verwende den [`GET /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-getmfasettings) Endpunkt, um die aktuellen Einstellungen für die 2-Schritt-Verifizierung des Benutzers abzurufen.
...
**MFA-Einstellungen aktualisieren**

Um die 2-Schritt-Verifizierung zu aktivieren oder zu deaktivieren, verwende den [`PATCH /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-updatemfasettings) Endpunkt.
...
- `verification_record_id`: Eine gültige Verifizierungs-ID. Siehe [Verifizierungs-ID erhalten](#get-a-verification-record-id) für Details.
- `skipMfaOnSignIn`: Setze auf `true`, um die 2-Schritt-Verifizierung für den Benutzer zu deaktivieren, oder auf `false`, um sie zu aktivieren.

:::note
Diese Einstellung wirkt nur, wenn die MFA-Richtlinie auf **Optional** gesetzt ist. Wenn MFA **Verpflichtend** oder **Adaptiv** ist, wird die Einstellung `skipMfaOnSignIn` ignoriert und MFA erzwungen.
:::
...
- `revokeGrantsTarget`: Optional kann das Ziel der Grants angegeben werden, die zusammen mit der Sitzung widerrufen werden sollen. Mögliche Werte:
  - `all`: Widerruft alle Grants, die mit der Sitzung verbunden sind.
  - `firstParty`: Widerruft nur First-Party-App-Grants, die mit der Sitzung verbunden sind. (Empfohlen für die meisten Anwendungsfälle, da dadurch der Zugriff für deine eigene App widerrufen wird, während Third-Party-App-Grants erhalten bleiben und so ein besseres Benutzererlebnis bieten.)
  - nicht angegeben: Das Standardverhalten widerruft Grants, die nicht den `offline_access`-Scope haben, was typischerweise bedeutet, dass nicht-Refresh-Token-Grants für die Sitzung widerrufen werden.

### Vom Benutzer autorisierte Apps (Grants) verwalten \{#manage-user-authorized-apps-grants}

Verwende APIs für vom Benutzer autorisierte Apps (Grants), wenn Benutzer autorisierte Apps auf ihrer Kontoeinstellungsseite überprüfen und widerrufen möchten.

:::note

- App-Grant-APIs teilen sich das gleiche Berechtigungsmodell wie Sitzungs-APIs.
- Die Berechtigung `UserScope.Sessions` ist erforderlich.
- Das Feld `Sessions` in den Account Center-Einstellungen muss aktiviert sein:
...
- Ohne `appType`: Gibt alle aktiven Grants zurück.
...
Wenn ein Grant widerrufen wird, werden zuvor ausgestellte opake Zugangstokens (Opaque tokens) und Auffrischungstokens (Refresh tokens) für diesen Grant ungültig.
