...
- Fülle das `domain`-Feld mit der Domain aus, die du in deinem Mailgun-Konto registriert hast. Dieser Wert kann im **Domains**-Abschnitt des Mailgun-Dashboards gefunden werden. Die Domain sollte im Format `example.com` sein, ohne das Präfix `https://` oder `http://`.
...
- `Generic`: Die E-Mail, die gesendet wird, wenn ein Benutzer eine generische Aktion ausführt, zum Beispiel das Testen des E-Mail-Connectors.

:::note
Wenn der Verwendungstyp nicht in der Lieferkonfiguration angegeben ist, wird die generische E-Mail gesendet. Wenn die generische E-Mail nicht angegeben ist, gibt der Connector einen Fehler zurück.
:::

### Inhaltskonfiguration \{#content-config}

Der Connector unterstützt sowohl direkten HTML-Inhalt als auch Mailgun-Template. Du kannst eines von beiden für jeden Verwendungstyp verwenden.

Sowohl im Betreff als auch im Inhalt kannst du den Platzhalter `{{code}}` verwenden, um den Bestätigungscode einzufügen.
...
Um ein Mailgun-Template zu verwenden, fülle die folgenden Felder aus:

- `subject`: Der Betreff der zu sendenden E-Mail.
- `replyTo`: Die E-Mail-Adresse, die als Antwortadresse verwendet werden soll.
- `template`: (Erforderlich) Der Name des zu verwendenden Mailgun-Templates.
- `variables`: Die Variablen, die an das Mailgun-Template übergeben werden sollen. Sollte eine JSON-Schlüssel-Wert-Zuordnung sein, da sie vor dem Senden an Mailgun in einen String umgewandelt wird. Beachte, dass die `code`-Variable nicht enthalten sein muss, da sie automatisch vom Connector hinzugefügt wird.

### Beispiel \{#example}

Das folgende ist ein Beispiel für die Lieferkonfiguration:
...
```json
{
  "Register": {
    ...
    "html": "<h1>Willkommen bei Logto</h1><p>Dein Bestätigungscode ist {{code}}.</p>",
    "text": "Willkommen bei Logto. Dein Bestätigungscode ist {{code}}."
  },
  "SignIn": {
