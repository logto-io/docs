...
2. Besuche die [Apps](https://developers.facebook.com/apps)-Seite.
...
5. Trage bei **Valid OAuth Redirect URIs** die Logto **Callback URI** ein (kopiere diese aus deinem Logto Facebook Connector). Nachdem sich Benutzer mit Facebook angemeldet haben, werden sie hierher mit einem Autorisierungscode weitergeleitet, den Logto zur Fertigstellung der Authentifizierung verwendet.
6. Navigiere zu **Use cases** und klicke auf **Customize** deines Anwendungsfalls, um die Berechtigungen (Scopes) hinzuzufügen. Wir empfehlen, `email` und `public_profile` hinzuzufügen, die für die Implementierung von „Sign-in with Facebook“ in Logto erforderlich sind.
