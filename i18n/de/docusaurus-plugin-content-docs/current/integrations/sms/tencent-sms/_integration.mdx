...
4. Wähle auf der Seite “[SMS Console Overview](https://console.cloud.tencent.com/smsv2)” (短信服务控制台概览) je nach Bedarf „Inlandsnachrichten“ (国内消息) oder „Internationale / Hongkong, Macao, Taiwan Nachrichten“ (国际/港澳台消息) aus der Seitenleiste.
5. Füge eine Signatur und eine Vorlage für SMS-Nachrichten hinzu. Beachte:
   - Beim Hinzufügen einer „Signatur“ (签名) wähle „Verifizierungscode“ (验证码) unter dem Feld "Anwendbare Szenarien" (模版类型).
   - Beim Hinzufügen einer Vorlage wähle „Verifizierungscode“ als Vorlagentyp.
...
6. Reiche die Signatur und die Vorlage zur Genehmigung ein. Nach der Einreichung:
   ...
   - Test-SMS können von der “[SMS Console Overview](https://console.cloud.tencent.com/smsv2)” (短信服务控制台概览) gesendet werden. Genehmigte Signaturen und Vorlagen können direkt für Tests verwendet werden, oder verwende die bereitgestellten Testvorlagen, wenn sie noch in der Überprüfung sind.
...
Für detaillierte Anleitungen klicke auf die Registerkarte „Getting Started Guide“ (新手配置指引) in der SMS Console Overview.

## Den Connector-JSON zusammenstellen \{#compose-the-connector-json}

1. Gehe zur “[SMS Console Overview](https://console.cloud.tencent.com/smsv2)” und bewege den Mauszeiger über dein Avatar in der oberen rechten Ecke. Navigiere zu „Access Management > Access Keys > API Keys“, um deinen `AccessKey ID` und `AccessKey Secret` nach Abschluss der Sicherheitsüberprüfung zu generieren. Bitte bewahre sie ordnungsgemäß auf.
2. Rufe von der Registerkarte „Inlandsnachrichten“ (国内消息) oder „Internationale / Hongkong, Macao, Taiwan Nachrichten“ (国际/港澳台消息) den „Signaturnamen“ (签名名称) und den „Vorlagencode“ (模版 CODCE) ab.
   - Für Testvorlagen überprüfe die Registerkarte „Getting Started“ (快速开始) unter "Test-Exclusive Templates" (测试专用签名模版).
...
- Konfiguriere den Tencent Cloud SMS Connector mit den folgenden Feldern:
  ...
  - `sdkAppId`: Zu finden in “[Application Management](https://console.cloud.tencent.com/smsv2/app-manage)” (应用管理).
