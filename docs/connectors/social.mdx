...
### Customize your social connectors \{#customize-your-social-connectors}
...
- **Enable token storage**: For supported social connectors, you can enable token storage to securely save access and refresh tokens in Logto’s [Secret Vault](/secret-vault) when users sign in with a social provider. This allows your application to access third-party APIs on behalf of the user without requiring them to re-authenticate. Learn more about [Federated token storage](/secret-vault/federated-token-set).
