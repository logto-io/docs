...
- **Enable token storage**: For OIDC enterprise connectors, you can enable token storage to securely save access and refresh tokens in Logto’s [Secret Vault](/secret-vault) when users sign in with an enterprise IdP. This allows your application to access third-party APIs on behalf of the user without requiring them to re-authenticate. Learn more about [Federated token storage](/secret-vault/federated-token-set).
