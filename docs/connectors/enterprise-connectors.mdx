...
### Customize your enterprise connectors \{#customize-your-enterprise-connectors}
...
## Configuring enterprise connectors \{#configuring-enterprise-connectors}
...
- **Enable token storage**: For OIDC enterprise connectors, you can enable token storage to securely save access and refresh tokens in Logto’s [Secret Vault](/secret-vault/README) when users sign in with an enterprise IdP. This allows your application to access third-party APIs on behalf of the user without requiring them to re-authenticate. Learn more about [Federated token storage](/secret-vault/federated-token-set).
...
### Impact on existing users after Enterprise SSO connector changes? \{#impact-on-existing-users-after-enterprise-sso-connector-changes}
