...
## Keep existing user IDs \{#keep-existing-user-ids}

By default, Logto generates a random ID for every user created through the Management API. If your application already stores user IDs from the previous identity provider, you can keep them by passing an optional `id` in the [create user](https://openapi.logto.io/operation/operation-createuser) request body. This avoids maintaining a mapping table between old and new IDs.

The ID must be 1 to 128 characters long and can contain letters, numbers, and `_ - . @ : + = |`. This covers common formats such as `auth0|5f7c8ec7c33c6c004bbafe82`, `google-oauth2|103547991597142817347`, UUIDs, `user_01H8MZ2QK3V4X5Y6Z7`, and email addresses. Characters that are not valid in URL paths (whitespace, `/`, `?`, `#`, `%`, `\`) are rejected with `400`, since the ID is used in `/api/users/{userId}`. If the ID is already taken, the request fails with `422 user.id_already_in_use`.
...
:::note
Custom user IDs are only supported in self-hosted Logto. Logto Cloud rejects requests that include `id` with `400 request.invalid_input`, because user IDs are shared across tenants.
:::
