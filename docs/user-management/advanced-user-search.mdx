...
Use `GET /api/users` for searching users. Note it is a Management API that requires auth like others. See [Interact with Management API](/integrate-logto/interact-with-management-api) for the interaction recipe.
