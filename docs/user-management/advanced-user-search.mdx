...
Use `GET /api/users` for searching users. Note it is a Management API that requires auth like others. See [Interact with Management API](/integrate-logto/management-api) for the interaction recipe.
...
This API has [pagination](/integrate-logto/management-api/#using-pagination) enabled.
...
All fuzzy search modes only support matching one value per field. If you need to match multiple values for a single field, you should use the "exact" mode. See [Exact match and case sensitivity](https://docs.logto.io/docs/recipes/manage-users/advanced-user-search/#exact-match-and-case-sensitivity) for details.
