...
Want to implement invitation-only registration, we recommend [inviting users with a magic link](/end-user-flows/sign-up-and-sign-in/disable-user-registration#option-1-invite-user-with-magic-link-recommended). This allows only whitelisted users to self-register and set their own password.
...
- **Authentication-related data**:
  - **Email address** (_[primary_email](/user-management/user-data#primary-email)_): Editable
...
On the "User details" page, click "Three dots" -> "Reset password" button, and then Logto will automatically regenrate a random password.
