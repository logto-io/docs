...
You can manage users through the management API in several use cases. Such as [advanced user search](./advanced-user-search), [bulk creation accounts](https://bump.sh/logto/doc/logto-management-api/operation/operation-createuser), [invitation-only sign-up](/end-user-flows/sign-up-and-sign-in/disable-user-registration), etc.
