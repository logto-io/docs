...
1. **Create webhook**: Navigate to **Console > Webhooks** and click the "Create webhook" button.
...
4. **Event**: In the modal that appears, select the desired events that will trigger this webhook. It is recommended to choose a smaller number of events that meet your requirements to avoid overwhelming the server reception. You can change the selected events at any time after creating the webhook.
...
- **Signing key**: Logto generates a unique hash signature, known as the Signing Key, for each webhook. You can include this key as a request header in your endpoint implementation. Verifying the signing key ensures that the webhook payload originates from Logto and has not been tampered with by unauthorized sources. Read [Securing your webhooks](https://docs.logto.io/docs/recipes/webhooks/securing-your-webhooks/) to learn more about the code.
