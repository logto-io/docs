...
1. In the script, an external API call [fetch external data](/developer/custom-token-claims/create-script/#step-3-fetch-external-data) can be used to retrieve the current player's balance from Joe's game server.
2. If the balance is less than or equal to 0, the <Url type="inline" href="/developer/custom-token-claims/create-script/#api">`api.denyAccess()` method</Url> can be used to block token issuance.
