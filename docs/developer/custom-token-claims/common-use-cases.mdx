...
1. In the script, an external API call (Link to [fetch external data](https://www.notion.so/8-1-4-Implement-getCustomJwtClaims-function-13626f6af1dc80afa891c4e6136f5015?pvs=21)) can be used to retrieve the current player's balance from Joe's game server.
2. If the balance is less than or equal to 0, the `api.denyAccess()` method (Link to `api` input params) can be used to block token issuance.
