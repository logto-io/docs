...
4. **Scope** (Optional): Logto automatically includes the required scopes (`openid`, `profile`, and `email`) in all requests. You can specify additional scopes as a space-separated list if your application requires extra permissions or access levels from the IdP.
