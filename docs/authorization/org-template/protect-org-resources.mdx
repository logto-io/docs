...
If you haven’t set up organization roles yet, refer to [this section](../configure-org-template/#create-organization-role).
...
- If you are using a Logto SDK with Organizations support, you can add the `urn:logto:scope:organizations` scope to `scopes` parameter of the configuration object, the same way as [Get organization IDs of the current user](../protect-org-resources/#step-1-get-organization-ids-from-oidc-flow).
