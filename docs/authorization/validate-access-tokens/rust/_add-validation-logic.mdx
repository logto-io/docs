...
impl JwtValidator {
    ...
    pub fn validate_validate_jwt(&self, token: &str) -> Result<AuthInfo, AuthorizationError> {
...
pub async fn jwt_middleware(
    ...
    let auth_info = validator.validate_validate_jwt(token)?;
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            match extract_bearer_token(authorization)
                .and_then(|token| validator.validate_validate_jwt(token))
            {
                Ok(auth_info) => {
                    // Store auth info in request extensions for generic use
