...
Logto uses the `ES384` algorithm to sign the JWTs by default. To decode the JWTs, you need to set the `jwsAlgorithm` to `ES384` explicitly. If you prefer to use `RSA`, feel free to rotate the signing algorithm in the Logto Admin Console. Please refer to [Signing keys](https://docs.logto.io/docs/references/openid-connect/signing-keys-rotation/) for more details.
...
This tutorial assumes you have the Access Token for an API resource `http://localhost:3000/` before making a request. If you are not ready, [read this](../protect-your-api) before continuing.
