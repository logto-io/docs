...
Logto uses the `ES384` algorithm to sign the JWTs by default. To decode the JWTs, you need to set the `jwsAlgorithm` to `ES384` explicitly. If you prefer to use `RSA`, feel free to rotate the signing algorithm in the Logto Admin Console. Please refer to [Signing keys](/developer/signing-keys/) for more details.
