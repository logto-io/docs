...
:::tip Personal Access Token (PAT) support

You can also use [Personal Access Tokens (PATs)](/user-management/personal-access-token) to obtain organization tokens for API access. PAT tokens work identically to refresh tokens for organization-level API resources - the same `resource`, `organization_id`, and `scope` parameters apply. This enables programmatic access to organization-scoped APIs from scripts, CI/CD pipelines, or backend services.
