...
You can also use [Personal Access Tokens (PATs)](/user-management/personal-access-token) to obtain organization tokens. PAT tokens work identically to refresh tokens for organization contexts - the same parameters and flows apply. This is useful for CI/CD, scripts, or applications that need programmatic access to organization-specific features.
