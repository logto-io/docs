...
Organization permissions (non-API) control what users can do **within an organization (tenant) context**, but are not **enforced at the API level**. Instead, they govern access to app features, UI elements, workflows, or business actions, rather than backend APIs.

**Use cases include:**

- Inviting or managing members within an organization
- Assigning or changing organization roles
- Managing billing, settings, or administrative functions for an organization (tenant)
