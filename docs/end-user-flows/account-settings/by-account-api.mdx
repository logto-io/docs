...
### Replace an existing TOTP \{#replace-an-existing-totp}

If the user already has a TOTP factor configured, you can replace it with a new one using the [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-createorreplacetotpmfaverification) endpoint. This is an idempotent operation that removes the existing TOTP and binds a new one.
...
Use the [`POST /api/my-account/mfa-verifications/totp-secret/generate`](https://openapi.logto.io/operation/operation-generatetotpsecret) endpoint to generate a new TOTP secret, then display it to the user as described in [Link a new TOTP](#link-a-new-totp).

**Step 2: Replace the TOTP factor**
...
- `verification_record_id`: a valid verification record ID, granted by verifying the user's existing factor. You can refer to the [Get a verification record ID](#get-a-verification-record-id) section for more details.
- `secret`: the new TOTP secret generated in step 1.
- `code`: the TOTP code generated from the new secret to confirm the binding.
...
### Manage 2-step verification settings \{#manage-2-step-verification-settings}

:::note
To use these methods, you need to enable the `mfa` field in the [account center settings](#how-to-enable-account-api).
:::

End users can toggle 2-step verification on or off for their account. A common use case is when a user has configured MFA factors (such as passkeys) but only wants to use them for sign-in rather than as an additional verification step — disabling 2-step verification allows them to skip the MFA prompt during sign-in while keeping their MFA factors intact. This is controlled by the `skipMfaOnSignIn` setting per user.

**Get current MFA settings**

Use the [`GET /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-getmfasettings) endpoint to retrieve the user's current 2-step verification settings.
...
To enable or disable 2-step verification, use the [`PATCH /api/my-account/mfa-settings`](https://openapi.logto.io/operation/operation-updatemfasettings) endpoint.
...
- `verification_record_id`: a valid verification record ID. You can refer to the [Get a verification record ID](#get-a-verification-record-id) section for more details.
- `skipMfaOnSignIn`: set to `true` to disable 2-step verification for the user, or `false` to enable it.

:::note
This setting only takes effect when the MFA policy is set to **Optional**. If MFA is **Mandatory** or **Adaptive**, the `skipMfaOnSignIn` setting will be ignored and MFA will be enforced.
:::
