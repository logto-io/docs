...
If the user already has a TOTP factor configured, you can replace it with a new one using the [`PUT /api/my-account/mfa-verifications/totp`](https://openapi.logto.io/operation/operation-replacetotp) endpoint. This is an idempotent operation that removes the existing TOTP and binds a new one.
