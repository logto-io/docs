...
- Viewing and editing user profile (name, avatar, and custom profile fields)
...
| Path                                 | Description                                                            |
| ------------------------------------ | ---------------------------------------------------------------------- |
| `/account/profile`                   | User profile page (name, avatar, and custom profile fields)            |
...
#### Configure profile fields \{#configure-profile-fields}

The Account Center displays a **Profile** page where end users can view and manage their profile information. To control which profile fields appear on this page:

1. Make sure the **Name**, **Avatar**, **Profile**, or **Custom data** field permission is set to `Edit` or `ReadOnly` in the Account security / User profile sections.
2. In the **Integrate prebuilt UI** card, add the profile fields you want to expose from the profile fields selector. These fields are drawn from the same catalog used by [Collect user profile](/end-user-flows/collect-user-profile) — any field defined there can also be shown in the Account Center.
3. Drag and drop to reorder the fields as you like.

Only fields that are both listed in profile fields **and** whose corresponding field permission is not `Off` will appear on the Profile page.
