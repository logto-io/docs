...
## Enforce re-authentication every access

For high-security scenarios (e.g., confirming a user identity for sensitive transactions), you may want to require re-authentication on every access. To enforce this, set `prompt=login consent` in your authentication requests.

When `prompt=login` is set on every request, Logto will always show the sign-in page, even if the user has an active session or just closed and reopened the app.

Adding `prompt=consent` forces to consent. For first-screen applications, consent is typically auto-approved, so the user won't see the consent screen. However, this prompt is necessary to receive a refresh token. Without it, refresh tokens may not be returned, and repeated scope requests could trigger login prompts.

In summary, In short, use `prompt=login consent` to require login every time. But apply it carefully, as it increases friction and impacts user experience.
