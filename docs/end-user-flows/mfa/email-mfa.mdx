...
The MFA setup prompt can appear during user registration or after sign-in, depending on your configured [MFA policy](/end-user-flows/mfa/configure-mfa#global-mfa-configuration). Users may also enable email MFA from their [Account settings page](/end-user-flows/account-settings/by-account-api#update-or-link-new-email).
