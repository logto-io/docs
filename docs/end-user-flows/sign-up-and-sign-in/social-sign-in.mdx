...
Set up profile collection through <CloudLink to="/sign-in-experience/collect-user-profile">Console > Sign-in experience > Collect user profile</CloudLink> to choose from pre-configured basic data fields or create custom fields with flexible validation. Learn more: [Collect user profile](/end-user-flows/collect-user-profile)
...
## Link or unlink social accounts \{#link-or-unlink-social-accounts}

After initial social sign-in, users can manage their connected social accounts through their account settings. This allows users to link additional social accounts to their existing profile or remove social connections they no longer wish to use.

Users can access these features through a self-service account center, which provides flexibility to:

- **Link new social accounts**: Connect additional social providers to the same user profile
- **Unlink existing social accounts**: Remove social connections while maintaining access through other methods
- **View connected accounts**: See all currently linked social identities

Learn more about implementing account management features in the [account settings](/end-user-flows/account-settings) documentation.

## Store tokens and call third-party APIs \{#store-tokens-and-call-third-party-apis}

You can optionally enable third-party token storage for supported social connectors so that, after a user authenticates and grants permissions, the provider-issued access and refresh tokens are securely stored in Logto's secret vault. Your application (via Account API) can later retrieve and use them to call provider APIs (e.g., let your app add events to Google Calendar) on behalf of the user without forcing another social re-authentication. See [third-party token storage](/secret-vault/federated-token-set) for setup details.
