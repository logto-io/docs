...
```mermaid
flowchart TD
    A[Visit sign-in page] --> B[Click on Forgot password link]
    B --> C[Enter email/phone]
    C --> D[Enter verification code]
    D --> E[Enter new password]
    E --> F[Successful password reset]
...
For authenticated users, they can update their password by visiting the account settings page. Check the [account settings](TODO) to learn more about implementing a user account settings page.
...
You can implement your own password reset flow by using the Logto's **Management API** and **Account API**. Checkout [Account settings](TODO) for more details.
