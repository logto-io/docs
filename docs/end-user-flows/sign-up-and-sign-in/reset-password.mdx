...
Subscribe to the `PostResetPassword` [webhook event](/developers/webhooks/webhooks-events#user-interaction-hook-events) to receive a notification when a user successfully resets their password. You can then trigger a [sign-out](/end-user-flows/sign-up-and-sign-in/sign-out) action to invalidate the user's current session and redirect them to the sign-in page.
