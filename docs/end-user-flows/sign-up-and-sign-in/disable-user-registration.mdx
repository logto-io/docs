...
5. After the user account is created, redirect them to the Logto sign-in or reset-password page to complete registration. See [Initiate sign-in](TODO) for guidance on customizing the first-screen and pre-filling the user’s email to streamline the sign-in process.
...
4. Redirect users to the Logto sign-in or reset-password page to complete registration. See [Initiate sign-in](TODO) for guidance on customizing the first-screen and pre-filling the user’s email to streamline the sign-in process.
...
- **Username/Email/Phone number with Password**: Create a user account with a **Username**, **Email address**, or **Phone number**, and enable the corresponding identifier for password-based sign-in.
  ...
  - Without a pre-set password: Create the account without a password, and prompt the user to set one upon account activation. (An **Email address** or **Phone number** is required for the password reset flow.) After creating the user, redirect them to the password reset page with their email or phone number pre-filled. For guidance on redirecting users to the reset password page with pre-filled details, refer to the [Initiate Sign-in guide](TODO). Once the user sets a new password, they can sign in directly with their username, email, or phone number and the new password.
