...
Sign-in and sign-up is the core interaction process for end-users to authenticate and authorize access to client applications. As a centralized OIDC-based CIAM platform, Logto provides a universal sign-in experience for users across multiple client applications and platforms.

## User flow

In a typical OIDC authentication flow, the user starts by opening the client app. The client app sends out an authorization request to Logto OIDC provider. If the user does not have an active session, Logto will prompt the user to the Logto-hosted sign-in experience page. The user interacts with the Logto experience page and gets authenticated by providing the necessary credentials. Once the user is successfully authenticated, Logto will redirect the user back to the client app with the authorization code. The client app then sends a token request to Logto OIDC provider with the authorization code to get the tokens.
...
Continue to learn more about the sign-in experience setup and customization in Logto.

## FAQs

<details>
  <summary>Per app SIE method or branding</summary>

For applications that require different sign-in experiences or branding, Logto also supports
per-app customization. Check out the [application settings](todo) for more details.
...
address, or region, you can use the [Custom JWT claims](todo) feature in Logto to
...
your own sign-in UI using the [Bring your own UI](todo) to customize the sign-in and
sign-up experience.
