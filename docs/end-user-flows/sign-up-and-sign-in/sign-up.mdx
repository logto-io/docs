...
- Primary identifier: The first sign-up identifier selected will be set as the primary identifier for the user. Logto will use this identifier to render the first-screen registration form and to verify the user's identity during the sign-in process.
- Secondary identifier: If you select more than one identifier, the additional identifiers will be collected after the primary identifier is submitted. All the identifiers collected during the sign-up process will be stored in the user's profile and can be used to sign in to the application.
