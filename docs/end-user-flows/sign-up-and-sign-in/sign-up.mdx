...
To successfully create a new user account in Logto, users must provide an **identifier** that uniquely identifies them within Logto's system. As the first step, select the identifiers that users must provide during the sign-up process. The available options are:
...
At least one identifier must be selected to enable the sign-up process. If empty, users can only sign in using the [social connectors](/connectors/social-connectors) or [enterprise SSO connectors](/connectors/enterprise-connectors) that are configured in Logto.

- Primary identifier: The first sign-up identifier selected will be set as the primary identifier for the user. Logto will use this identifier to render the first-screen registration form in the sign-up flow.
- Secondary identifier: If you select more than one identifier, the additional identifiers will be collected after the primary identifier is submitted. Users can only create an account with all the required identifiers provided.
...
### Type 4: Email address with verification, username and password creation \{#type-4-email-address-with-verification-username-and-password-creation}
