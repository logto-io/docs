...
Before configuring IdP-initiated SSO, you need to create a SAML connector first. Navigate to the <CloudLink to="/enterprise-sso" /> and follow the step-by-step [guide](todo) to set up a [SAML](todo) connector with your IdP.
...
3. Pass the [direct sign-in](TODO) authentication parameter in the sign-in request to Logto to complete the SSO authentication flow.
