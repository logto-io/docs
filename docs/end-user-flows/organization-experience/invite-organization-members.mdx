...
Please be noted that all the APIs listed above requires a valid "organization token". [Check this guide](/authorization/organization-template/protect-organization-resources#step-2-fetch-organization-token) to learn how to obtain the organization token.
