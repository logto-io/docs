...
As there are lots of properties in connector data, and to make the data loading and updating more efficient, we store part of connector data which will be modified frequently to DB and the rest of that locally.
...
For email and SMS connectors, a _target_ works more like a human readable ID.
...
_type_ should be one of the following connector types:
...
_readme_ is a relative path of the connector's README markdown file whose contexts will show up during set-up.
...
It is where a connector store its configuration. Each connector have different properties in _config_ and it obligated to be valid (connectors have different standard for "valid".) before being saved to DB.
...
_createdAt_ is an auto-generated timestamp string.
