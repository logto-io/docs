...
_Core_ implies the Logto core service, a monolith service for critical duties. The source code is in [`/packages/core`](https://github.com/logto-io/logto/tree/master/packages/core).
...
Backend APIs, including OIDC, are built within the `core` package, while frontend proxies depend on the corresponding sibling packages.
...
- If it's dev, it proxies HTTP requests to the frontend dev server
...
:::note
You may notice that the UI proxy uses the root path. Doesn't like other proxies, the UI proxy is a fallback proxy which means it only takes effect when no other proxy is matched.
:::
