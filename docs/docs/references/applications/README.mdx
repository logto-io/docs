...
:::note
For non-sender-constrained public clients, a new refresh token will always be issued when the client is exchanging for a new access token using the refresh token.
