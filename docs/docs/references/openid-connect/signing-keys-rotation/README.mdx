...
<head>
  <link rel="canonical" href="https://docs.logto.io/developers/signing-keys/" />
</head>
...
The signing keys are applied to the entire Logto instance (a Logto open-source instance or a Cloud tenant). Different Logto instances hold their own set of private keys and cookie keys, which can be managed through CLI (open-source), Management APIs or Console UI.
...
:::info
If you're using a Logto open source version which does not support rotating signing keys in the Console UI, please refer to [this tutorial](/docs/docs/references/using-cli/rotate-signing-keys.mdx) to rotate signing keys via CLI.
:::
...
Navigate to the "Signing keys" page. From there, you can manage both OIDC private keys and OIDC cookie keys.
...
![Console UI - Private keys](./assets/console-private-keys.webp)

![Console UI - Cookie keys](./assets/console-cookie-keys.webp)
