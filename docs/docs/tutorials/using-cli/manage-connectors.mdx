---
sidebar_position: 2
---

import Admonition from '@theme/Admonition';
import TabItem from '@theme/TabItem';
import Tabs from '@theme/Tabs';

import ConnectorsIntro from '../get-started/fragments/_connectors-intro.md';

# Manage connectors

<ConnectorsIntro />

:::note
For each command, you can append `--path /your/path/to/logto` (alias `-p`) to specify the Logto instance path.
:::

## List existing connectors

Use the command below to list existing connectors that have been added to your Logto instance:

<Tabs groupId="cmd">

  <TabItem value="cli" label="CLI">

```bash
logto connector list
```

  </TabItem>
  <TabItem value="npx" label="npx">

```bash
npx -- @logto/cli logto connector list
```

  </TabItem>

</Tabs>

A sample output:

```bash
OFFICIAL
  @logto/connector-alipay-native
  @logto/connector-aliyun-dm
  @logto/connector-aliyun-sms
  @logto/connector-apple
  @logto/connector-azuread

3RD-PARTY
  connector-some-third-party
```

## Add connectors

### Official connectors

All official connectors are listed in [@logto/connectors](https://github.com/logto-io/connectors/tree/master/packages), except those names that start with `connector-mock-` for testing purposes.

<Admonition type="tip" icon="🤠" title="Call for contributors!">

Every connector is a [Tier 1 Bounty Hunter issue](https://github.com/logto-io/logto/issues/1861) by default. Feel free to comment on or create the related GitHub issue for acknowledging a connector if you are interested.

</Admonition>

If you are using the official Docker image or added official connectors during init, there's no need to add them again. If not, use the command below to add all official connectors:

<Tabs groupId="cmd">

  <TabItem value="cli" label="CLI">

```bash
logto connector add --official
```

  </TabItem>
  <TabItem value="npx" label="npx">

```bash
npx -- @logto/cli logto connector add --official
```

  </TabItem>

</Tabs>

### Third-party connectors

Third-party connectors are those made for Logto but NOT in [@logto/connectors](https://github.com/logto-io/connectors/tree/master/packages).

The connector to add should be a valid NPM package, and the package name must start with `connector-` or `@some-org/connector-`. E.g.:

<Tabs groupId="cmd">

  <TabItem value="cli" label="CLI">

```bash
logto connector add service-a connector-service-b @org/service-c
# is equal to
logto connector add connector-service-a connector-service-b @org/connector-service-c
```

  </TabItem>
  <TabItem value="npx" label="npx">

```bash
npx -- @logto/cli logto connector add service-a connector-service-b @org/service-c
# is equal to
npx -- @logto/cli logto connector add connector-service-a connector-service-b @org/connector-service-c
```

  </TabItem>

</Tabs>

:::danger
Connectors are executable code! Inspect and review the COMPILED CODE (the content download from NPM) very, very carefully to avoid potential security issues if you are trying to use a third-party connector.
:::

:::info Note
Remember to restart your Logto instance to get the changes reflected.
:::

## Remove connectors

<Tabs groupId="cmd">

  <TabItem value="cli" label="CLI">

```bash
logto connector remove connector-service-a @org/connector-service-b
```

  </TabItem>
  <TabItem value="npx" label="npx">

```bash
npx -- @logto/cli logto connector remove connector-service-a @org/connector-service-b
```

  </TabItem>

</Tabs>

:::info Note
Remember to restart your Logto instance to get the changes reflected.
:::
