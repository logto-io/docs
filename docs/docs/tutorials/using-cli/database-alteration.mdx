...
- Get prepared for potential failure, say have a revert script
...
Our alteration scripts also come with a `down` script which can revert the changes. For now we don't have a CLI command handy, but it'll be available soon.
