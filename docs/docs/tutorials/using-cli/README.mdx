...
:::caution
This doc contains unreleased content and is sill work in progress.
:::
...
:::note
Currently we don't ship binaries, thus Node.js `^16.13.0` is required in the environment.
:::
...
Note if you want to execute with options, a `--` is needed to pass them down to the CLI, e.g.:

```bash
npm run -- cli db seed --env /some/dir/.env
```
...
This will be helpful for one-off invocations. Also, `--` is needed for options, e.g.:

```bash
npx -- @logto/cli db seed --env /some/dir/.env
```
