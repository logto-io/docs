import TabItem from '@theme/TabItem';
import Tabs from '@theme/Tabs';

# 🖥 Using Logto CLI [WIP]

:::caution
This doc contains unreleased content and is sill work in progress.
:::

## Get started

Logto CLI provide a conveneient way to install, maintain, and update your Logto instance without handling those boring tech stuff manually.

:::note
Currently we don't ship binaries, thus Node.js `^16.13.0` is required in the environment.
:::

### Install CLI globally

Install via your preferred package manager to use `logto` command globally:

<Tabs>

  <TabItem value="npm" label="npm">

```bash
npm i @logto/cli -g
```

  </TabItem>
  <TabItem value="yarn" label="Yarn">

```bash
yarn global add @logto/cli
```

  </TabItem>
  <TabItem value="pnpm" label="pnpm">

```bash
pnpm add -g @logto/cli
```

  </TabItem>

</Tabs>

### Local CLI

A built-in version of CLI is shipped with every Logto release since v1.0.0-beta.11. Say you already have a Logto instance in `~/logto`, then you can run the commands below:

```bash
cd ~/logto
npm run cli # Invoke the local CLI
```

Note if you want to execute with options, a `--` is needed to pass them down to the CLI:

```bash
npm run -- cli db seed --env /some/dir/.env
```

### Use CLI by `npx`

Another way to use Logto CLI is `npx`, which can execute a package without installation.

```bash
npx @logto/cli
```

This will be helpful for one-off invocations. Also, `--` is needed for options:

```bash
npx -- @logto/cli db seed --env /some/dir/.env
```
