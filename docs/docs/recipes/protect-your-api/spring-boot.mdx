...
This tutorial assumes you have created an API Resource `http://localhost:3000/protected` in Admin Console.
If you are not ready, [read this](./README.md#register-the-api-resources-through-logto-admin-console) before continuing.
...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.oauth2ResourceServer(OAuth2ResourceServerConfigurer::jwt).cors().and()
                // Only authenticated requests can access your protected API `/protected`.
                .authorizeRequests(customizer -> customizer.mvcMatchers("/protected").authenticated());
        return http.build();
    }
}
```
...
Add a controller to provide the `/protected` API:
...
This tutorial assumes you have the Access Token for an API resource `http://localhost:3000/protected` before making a request.
If you are not ready, [read this](./README.md#integrate-the-resources-authorization-flow-into-your-client-application) before continuing.
...
If succeeded, you will see the response like this.
