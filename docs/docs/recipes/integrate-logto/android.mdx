...
<ConfigureRedirectUri figureSrc={redirectUriFigure} redirectUri="$(LOGTO_REDIRECT_SCHEME)://$(YOUR_APP_PACKAGE)/callback" />
...
Visit `.isAuthenticated` to know if the user is authenticated or not.
...
Calling `.signOut` will always clear local credentials even if errors occurred.
