...
Logto SDK helps you fetch the user information from the OIDC [UserInfo Endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo) via `logtoClient.FetchUserInfo()` after signing in.
...
When initializing the `LogtoClient`, you should add required scopes to the `LogtoConfig` so that you can get the corresponding user information by calling `logtoClient.FetchUserInfo()` after signing in.
