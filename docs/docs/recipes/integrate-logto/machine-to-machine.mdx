...
Machine to Machine (M2M) is a common practice to authenticate if you have an app that needs to directly talks to resources. E.g., an API service that update users' custom data in Logto, a statistic service that pulls daily orders, etc.

Usually, a M2M app doesn't need user interactions, i.e. it has no UI.

:::info
Logto doesn't support M2M app to represent a user for now. The `sub` of the Access Token will be the App ID.
:::
...
In the applcation details page, locate the App ID and App Secret.

<img alt="App ID and App Secret" src={AppIdSecretSrc} width="600px" />

### Locate the API Resource

In the API Resource tab, find the API identifier that the app needs to access. If you haven't add the API Resource in Logto or don't know what is API Resource, see [API Resource](/docs/references/resources).

<img alt="API identifier" src={AppIdentifierSrc} width="600px" />

### Compose and send the request

Then compose them into a request:
...
For example, you have requested an Access Token with resource `https://api.logto.io`, to get all applicaitons in Logto:
