...
The SDK requires that [express-session](https://www.npmjs.com/package/express-session) is configured first.
...
We're almost there! In order to sign in, we can just redirect the user to Logto's sign-in route. For example, create a sign in link button:
...
To get user profile, we need to add another middleware:
