...
Uses an `application.yml` file (instead of the default `application.properties`) to configure the OAuth2 client and provider.
      ...
      client:
        registration:
          logto:
            ...
            client-secret: <your-app-secret> # Copied from application details in Admin Console
            scope:
              - openid
              - profile
        provider:
          logto:
            issuer-uri: <your-issuer-uri> # e.g. `http://localhost:3001/oidc`. You can replace it with your logto's production issuer URI.
```
...
After the user signs in successfully, it will redirect back to the home page showing "Signed in" that means the user is authenticated.
