...
Logto OIDC signing keys, as known as “OIDC private keys” and “OIDC cookie keys”, are the signing keys used to sign JWTs (access tokens and ID tokens) and browser cookies in Logto sign-in sessions.
...
Set this environment variable on the Logto service when rotating through Console UI or Management API, or in the shell where you run the CLI command when rotating through CLI.
