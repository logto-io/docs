...
sidebar_label: Third-party tokens storage
---
...
## Enable third-party token storage
...
Token storage is available for all OIDC [enterprise connectors](/connectors/enterprise-connectors). Access and refresh tokens can be stored during [enterprise Single Sign-On](/end-user-flows/enterprise-sso). Currently supported connectors include: [Google Workspace](/integrations/google-workspace).
