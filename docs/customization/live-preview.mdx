...
- **Organization ID**: Input the organization ID (space delimited) if you need to refresh the [organization token](/authorization).

If both a **Resource** and an **Organization ID** are provided, the refresh request will return a access token that supports [organization-level API resource protection](/authorization/organization-level-api-resources).
