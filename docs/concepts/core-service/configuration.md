...
| PRIVATE_KEY_ROTATION_GRACE_PERIOD      | `0`                                  | `number`                                                 | Grace period in seconds for staged OIDC private key rotation. When set to a positive value, the new private key is created as `Next` first and only becomes effective after the grace period.                                                                                              |
| OIDC_PROVIDER_SSRF_PROTECTION_DISABLED | `false`                              | `boolean`                                                | Self-hosted only. Set to `true` only when trusted OIDC relying-party endpoints must resolve to private network addresses. See [OIDC provider SSRF protection](#oidc-provider-ssrf-protection).                                                                                             |
