...
For standard OIDC claims, the inclusion in the ID token is strictly determined by the requested scopes. Extended claims (such as `custom_data` and `organizations`) can be additionally configured to appear in the ID token through the <a href="/developers/custom-id-token">Custom ID token</a> settings.
...
<p>
  ...
  and there is no way to remove these default scopes. But you can add more scopes when configuring
  Logto:
</p>
