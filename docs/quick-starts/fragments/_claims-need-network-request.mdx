...
To prevent bloating the user object, some claims require network requests to fetch. For example, the `custom_data` claim is not included in the user object even if it's requested in the scopes. To access these claims, <span>{props.type === 'method' && (<>you can use the <code>{props.method}</code> method</>)}{props.type === 'option' && <>you can configure the <code>{props.configOption}</code> option</>}</span>:
