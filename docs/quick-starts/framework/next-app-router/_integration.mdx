...
```
**Note:** If you're using an environment variable for `cookieSecret` (e.g., `process.env.LOGTO_COOKIE_SECRET`), ensure that the value is at least 32 characters long. If this requirement is not met, Logto will throw the following error during build or runtime:  
