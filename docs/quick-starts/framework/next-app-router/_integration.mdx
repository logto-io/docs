...
If you're using an environment variable for cookieSecret (e.g., `process.env.LOGTO_COOKIE_SECRET`), it must be at least 32 characters long. Otherwise, Logto will throw the following error:
`TypeError: Either sessionWrapper or encryptionKey must be provided for CookieStorage`.
To avoid this, make sure the environment variable is correctly set or provide a fallback value with at least 32 characters.
