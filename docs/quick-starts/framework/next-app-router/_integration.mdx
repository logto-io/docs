...
```
**Note:** If you're using an environment variable for `cookieSecret` (e.g., `process.env.LOGTO_COOKIE_SECRET`), ensure that the value is at least 32 characters long. If this requirement is not met, Logto will throw the following error during build or runtime: `TypeError: Either sessionWrapper or encryptionKey must be provided for CookieStorage`. To prevent this error, make sure the environment variable is correctly set or provide a fallback value with a minimum length of 32 characters.
