...
- **v2**: Opens the sign-in experience in an embedded WebView, which is required by the native social sign-in SDKs, but does not support [passkey sign-in](/end-user-flows/sign-up-and-sign-in/passkey-sign-in) (WebView does not support WebAuthn, the underlying standard of passkeys).
- **v3 (beta)**: Opens the sign-in experience in [Chrome Custom Tabs](https://developer.android.com/develop/ui/views/layout/webapps/overview-of-android-custom-tabs) (the system browser), which unlocks passkey sign-in and shares the browser session. Note that WeChat / Alipay native sign-in is removed in v3 (social connectors keep working through the browser); if you depend on the native social SDKs, stay on v2.
